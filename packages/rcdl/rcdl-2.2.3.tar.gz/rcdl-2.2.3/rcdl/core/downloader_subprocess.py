...
def ffmpeg_concat_build_command(videos: list[Video]) -> dict:
    ...
    for idx in range(len(videos)):
        ...
        if video_has_audio(videos[idx].relative_path):
            logging.info(f"AUDIO found for {videos[idx].relative_path}")
            filter_lines.append(f"[{idx}:a]anull[a{idx}]")
        else:
            logging.info(f"NO AUDIO found for {videos[idx].relative_path}")
            filter_lines.append(
                f"anullsrc=channel_layout=stereo:sample_rate=48000,"
                f"atrim=duration=shortest[a{idx}]"
            )
        concat_inputs.append(f"[v{idx}][a{idx}]")

    # concat inputs
    filter_lines.append(
        f"{''.join(concat_inputs)}concat=n={len(videos)}:v=1:a=1[outv][outa]"
    )
