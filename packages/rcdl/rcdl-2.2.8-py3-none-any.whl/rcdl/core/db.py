...
class DB:
    ...
    def delete_videos_by_creator(self, creator_id: str) -> int:
        """Delete all videos in db from a specified creator_id
        Return number of deleted rows"""
        cur = self.conn.cursor()
        cur.execute(queries.DELETE_VIDEOS_BY_CEATOR, (creator_id,))
        self.conn.commit()

        deleted = cur.rowcount
        if Config.DEBUG:
            logging.debug(f"Deleted {deleted} rows from creator_id={creator_id}")
