...
从 methods/aggregators/fedadam.py 迁移到 src/
...
class FedAdamAggregator(Aggregator):
    ...
    def aggregate(self, updates: List[ClientUpdate], global_model=None) -> Dict[str, torch.Tensor]:
        ...
        # 初始化Adam状态
        if not self.m_state:
            self._initialize_adam_states(aggregated_delta)

        # Adam更新全局权重
        self.global_weights = self._adam_update(aggregated_delta)
    ...
    def _adam_update(self, delta: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        """
        执行Adam更新
        ...
        for param_name, grad in delta.items():
            ...
            # Adam更新: w = w + lr * m / (sqrt(v) + eps)
            updated_weights[param_name] = (
                self.global_weights[param_name] +
                self.server_lr * m_corrected / (torch.sqrt(v_corrected) + self.eps)
            )
