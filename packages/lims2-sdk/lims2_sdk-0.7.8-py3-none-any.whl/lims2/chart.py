...
class ChartService(OSSMixin):
    ...
    def upload(
        ...
        Args:
            ...
            file_name: 自定义基础文件名，最终的 request_data['file_name'] 会在此基础上追加 hash 后缀
    ...
    def _upload_from_file(  # noqa: C901
        ...
        if file_format == "json":
            try:
                ...
                if isinstance(chart_data, dict):
                    # JSON文件上传：优先使用用户指定的 file_name，否则使用文件名
        ...
        # 设置文件信息：优先使用用户指定的 file_name，否则使用文件名
        base_name = request_data.get("file_name") or file_path.stem
        request_data["file_name"] = self._generate_unique_filename(
            request_data, base_name
        )
