        ...
        for i,C1 in enumerate(rMCB):
            for aux,C2 in enumerate(rMCB[i+1:]):
                ...
                if len(paths)==1:
                    ...
                    DualGraph.add_edge(i,j, length=len(P), indices=(idx_1,idx_2))
                    # for debug purposes:
        ...
        while len(badPairs)>0:
            ...
            for k in set(DualGraph[i]).union(DualGraph[j]):
                ...
                if len(paths) == 1: # valid intersection
                    ...
                    if i<k:     DualGraph.add_edge(i,k, length=len(P), indices=(idx_1,idx_2))
                    else:       DualGraph.add_edge(k,i, length=len(P), indices=(idx_2,idx_1))
