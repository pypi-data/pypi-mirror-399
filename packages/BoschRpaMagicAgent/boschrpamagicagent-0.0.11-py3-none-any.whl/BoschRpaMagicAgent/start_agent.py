...
async def start_agent(bot_variant_dict: dict):
    ...
    try:
        ...
        for llm_prompt_block in llm_prompt_block_data:
            ...
            tools = tool_registry.collect_tools(llm_api_key, llm_prompt)
            ...
            remote_file_path = report_save_path + '/' + f'{llm_prompt_block_name}.gif'
            smb_manager_instance.smb_copy_file_local_to_remote('agent_history.gif', remote_file_path)

        if smb_manager_instance:
            try:

                smb_manager_instance.close_smb_connection()
            except:
                pass
...
asyncio.run(start_agent(bot_variant_data))
