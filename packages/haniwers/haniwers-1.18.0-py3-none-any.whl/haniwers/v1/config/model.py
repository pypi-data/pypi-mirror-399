...
@deprecated(
    version="1.5.0",
    reason="DaqConfig has been merged into SamplerConfig. Use SamplerConfig with mode='count_based' instead.",
)
class DaqConfig(BaseModel):
    """Configuration settings for data acquisition (DAQ) output files.

    .. deprecated:: 1.5.0
        Use :class:`SamplerConfig` with ``mode='count_based'`` instead.
        This class will be removed in haniwers v1.11.0.
        Migration guide: https://docs.example.com/migration/

    What is this?
        Controls how detector data is saved to disk during acquisition. Defines file naming,
        organization, and buffering strategy.

    Attributes:
        label: Human-readable name for this DAQ configuration (e.g., "main_daq")
        workspace: Directory where output files will be saved (created if doesn't exist)
        filename_prefix: Start of output filename (e.g., "osechi_data")
        filename_suffix: End of output filename (e.g., ".csv")
        events_per_file: How many detector events to accumulate before starting a new file
    ...
    Beginner note:
        With stream_mode=true, each event is written immediately,
        slower but uses less memory. Good for long-term measurement.
        With stream_mode=false, data is buffered in memory until events_per_file is reached.
        This is faster but uses more memory. Good for short-term measurement or testing.
    """
    ...
    @field_validator("events_per_file", "number_of_files")
    @classmethod
    def validate_positive_int(cls, v):
        """Validate that file-related counts are positive.

        Why this matters:
            Negative or zero values don't make sense for event counts or file counts.
            Catch these configuration errors early with clear messages.
        ...
        Beginner note:
            This validator applies to multiple fields (events_per_file, number_of_files)
    ...
    def model_post_init(self, __context):
        """Emit deprecation warning when DaqConfig is instantiated.

        Why this matters:
            Users need to know that this class is deprecated and should migrate
            to SamplerConfig before the next major version.
        """
        warnings.warn(
            "DaqConfig is deprecated since v1.5.0 and will be removed in v1.11.0. "
...
@deprecated(
    version="1.5.0",
    reason="ScanConfig has been merged into SamplerConfig. Use SamplerConfig with mode='time_based' instead. Will be removed in v1.11.0.",
)
class ScanConfig(BaseModel):
    """Configuration settings for threshold scanning experiment mode.

    .. deprecated:: 1.5.0
        Use :class:`SamplerConfig` with ``mode='time_based'`` instead.
        This class will be removed in haniwers v1.11.0.
        Migration guide: https://docs.example.com/migration/

    What is this?
        Controls behavior during threshold scanning experiments where we systematically
        vary detector thresholds to characterize detector response across a range of values.

    Attributes:
        label: Human-readable name for this scan configuration (e.g., "main_scan")
        workspace: Directory where output files will be saved (created if doesn't exist)
        filename_prefix: Start of output filename (e.g., "scan_")
        filename_suffix: End of output filename (e.g., ".dat")
        events_per_file: How many detector events to accumulate before starting a new file
        number_of_files: Maximum number of output files to create per threshold level
        stream_mode: True for real-time streaming, False for buffered output (see DaqConfig note)
        duration: How many seconds to collect data at each threshold level
        suppress_threshold: Noise floor threshold for event filtering (default: 1000)
        max_retry: Maximum retry attempts if data collection fails (default: 3)
    ...
    Beginner note:
        Unlike normal DAQ which acquires data at a fixed threshold, threshold scanning
        collects data at multiple thresholds to study detector efficiency. Each threshold
        level gets its own set of output files.
    """
    ...
    def validate_duration(cls, v):
        ...
        Why this matters:
            Duration represents scan time and must be positive. Zero or negative values
            don't make sense for acquisition experiments.
        ...
        Beginner note:
            Duration is how long to collect data at each threshold level during scanning.
        """
        if v <= 0:
    ...
    def model_post_init(self, __context):
        """Emit deprecation warning when ScanConfig is instantiated.
        ...
        """
        warnings.warn(
            "ScanConfig is deprecated since v1.5.0 and will be removed in v1.11.0. "
            "Please migrate to SamplerConfig with mode='time_based'. "
            "See migration guide: https://docs.example.com/migration/",
            DeprecationWarning,
            stacklevel=3,
        )
...
class HaniwersConfig(BaseModel):
    ...
    Beginner note:
        - **RECOMMENDED**: Use [sampler] section for all new configurations
        - **DEPRECATED**: [daq] and [scan] sections are deprecated as of v1.5.0
        - For now, existing [daq]/[scan] configs will continue to work with deprecation warnings
    """

    device: DeviceConfig
    sampler: Optional[SamplerConfig] = None
    daq: Optional[DaqConfig] = None  # DEPRECATED in v1.5.0: Use sampler with mode='count_based'
    scan: Optional[ScanConfig] = None  # DEPRECATED in v1.5.0: Use sampler with mode='time_based'
    sensors: Dict[str, SensorConfig]
    mocker: MockerConfig

    @model_validator(mode="after")
    def validate_config_format(self):
        """Validate configuration format and reject invalid section combinations.

        Why this matters:
            During migration, we need to ensure configurations follow the new format.
            Users attempting to use old [daq]/[scan] sections should be guided to migration docs.

        What to test:
            - Accept new [sampler] section
            - Accept old [daq]/[scan] for backward compatibility (for now)
            - Warn if both old and new formats are present
            - Reject if neither [sampler] nor [daq]/[scan] specified

        Beginner note:
            This validator ensures configuration consistency and helps guide users
            toward the new unified [sampler] format.
        """
        log = base_logger.bind(context="HaniwersConfig")

        # Count which formats are present
        sampler_present = self.sampler is not None
        daq_present = self.daq is not None
        scan_present = self.scan is not None
        old_format_present = daq_present or scan_present

        # Validation logic:
        # 1. Reject if neither format is specified
        if not sampler_present and not old_format_present:
            raise ValueError(
                "Configuration must specify either [sampler] (recommended) "
                "or [daq]/[scan] (deprecated). "
                "See migration guide: https://docs.example.com/migration/"
            )

        # 2. Warn if old format is used (but allow for backward compatibility)
        if old_format_present and not sampler_present:
            if daq_present:
                log.warning(
                    "DaqConfig is deprecated since v1.5.0 and will be removed in v1.11.0. "
                    "Please migrate to [sampler] section with mode='count_based'. "
                    "See migration guide: https://docs.example.com/migration/"
                )
            if scan_present:
                log.warning(
                    "ScanConfig is deprecated since v1.5.0 and will be removed in v1.11.0. "
                    "Please migrate to [sampler] section with mode='time_based'. "
                    "See migration guide: https://docs.example.com/migration/"
                )

        # 3. Warn if both old and new formats are present (ambiguous)
        if sampler_present and old_format_present:
            log.warning(
                "Configuration contains both [sampler] (new) and [daq]/[scan] (old) sections. "
                "Using [sampler] section. Please remove deprecated [daq]/[scan] sections."
            )
    ...
    def from_toml(cls, path: Path) -> "HaniwersConfig":
        ...
        Usage Example:
            ...
            print(config.daq.label)
            print(config.sensors["top"].center)
            ```

        Beginner note:
            The file must have sections for [device], [daq], [scan], [mocker], and at least one
