...
from haniwers.v1.log.logger import logger as base_logger, configure_logging
...
# Bind logger context for all threshold operations
logger = base_logger.bind(context="cli.threshold")
...
def serial(
    ...
    try:
        ...
        print_error(f"Invalid configuration values: {e}")
        logger.error(f"Config validation failed: {e}")
        raise typer.Exit(code=1)
    except AttributeError as e:
        print_error(f"Configuration structure error: {e}")
    ...
    if ":" in parameters:
        ...
        for ch_spec in channel_params:
            ...
            try:
                ...
                # Use unified channel validator (P1-6 consolidation)
                validate_channel_range(ch)
                if len(params) != 4:
                    raise ValueError(f"Channel {ch}: expected 4 parameters, got {len(params)}")

                params_dict[ch] = params
            except ValueError as e:
                raise ValueError(f"Invalid per-channel format '{ch_spec}': {e}") from e
