...
if TYPE_CHECKING:
    ...
    from haniwers.v1.config.model import SamplerConfig
...
class Sampler:
    ...
    def __init__(
        ...
        Args:
            ...
            config (SamplerConfig): Settings for session
                - mode: "count_based" (acquire fixed number of events) or "time_based" (acquire for fixed duration)
                - workspace: Directory where output files will be saved
            ...
            output_dir (Path, optional): Directory where CSV files will be saved
                - Uses config.workspace (output_dir parameter is optional)
