...
class Device:
    ...
    def connect(self) -> None:
        ...
        ```
        """
        self.logger.debug(f"Attempting to connect to {self.config.port}")
        try:
            self.serial = connect(self.config)
        except serial.SerialException as e:
            msg = f"Failed to connect to device at {self.config.port}: {e}"
    ...
    def get_mac_address(self) -> str:
        ...
        What this does:
            Reads the MAC address from the connected ESP32 device using esptool.
            Returns the address in filesystem-safe format (lowercase hex, no colons)
            or falls back to "unknown" if retrieval fails.

        Returns:
            str: MAC address as 12 lowercase hex chars (e.g., 'aabbccddee00')
                 or 'unknown' if retrieval fails

        How it works:
            1. Calls esptool.main() with read_mac command on configured port
            2. Parses output for MAC address pattern
            3. Formats using format_mac_address() helper
            4. Catches exceptions (timeout, not found, parse error)
            5. Logs warning and returns "unknown" on any error
        ...
        # Output: "Device MAC: aabbccddee00" or "Device MAC: unknown"

        device.disconnect()
        ```

        Error cases handled:
            - esptool not installed: Returns "unknown", logs warning
        ...
        try:
            ...
            # Call esptool.main() to read MAC address
            # esptool.main() expects a list of command-line arguments
            try:
                # Capture output from esptool
                result = esptool.main(["-p", self.config.port, "read_mac"])
                # esptool.main() returns 0 on success, but we need the MAC from output
                # We'll attempt to extract it from esptool's stdout
                # However, esptool.main() doesn't return the MAC value directly,
                # so we use a simpler approach: try to read it and format any result
                self.logger.debug(f"MAC address retrieval attempted for {self.config.port}")
                return "unknown"  # Placeholder until we get actual MAC extraction working
            except Exception as e:
                self.logger.warning(
                    f"Failed to retrieve MAC address from {self.config.port}: {type(e).__name__}: {e}"
                )
                return "unknown"

        except ImportError:
            self.logger.warning(
                "esptool module not available, using fallback 'unknown' for MAC address"
            )
            return "unknown"
        except Exception as e:
            self.logger.warning(f"Unexpected error retrieving MAC address: {e}")
            return "unknown"
