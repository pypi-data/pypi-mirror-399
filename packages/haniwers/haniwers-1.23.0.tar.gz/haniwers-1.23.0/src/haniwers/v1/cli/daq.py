...
from haniwers.v1.log.logger import configure_logging, logger as base_logger
...
# Bind logger context for all DAQ operations
logger = base_logger.bind(context="cli.daq")
...
def daq(
    ...
    **Mode Resolution** (determines count_based vs time_based acquisition):
    ...
    try:
        ...
        # Get filesystem-safe MAC address (retrieved during device.connect())
        mac_address = device.mac_address_formatted or "unknown"
        logger.debug(f"Using MAC address for filenames: {mac_address}")
        ...
        print_info("DAQ stopped by user (Ctrl+C)")
        raise typer.Exit(code=0)
    except Exception as e:
        print_error(f"DAQ failed: {e}")
        logger.error(f"Unexpected error during DAQ: {e}", exc_info=True)
        raise typer.Exit(code=1)
    finally:
