...
from haniwers.v1.helpers.csv_validator import find_required_columns
...
# Bind logger context for all preprocess operations
logger = base_logger.bind(context="cli.preprocess")
...
def _validate_input_directory(read_from: str) -> Path:
    ...
    CLI wrapper around unified validate_directory_path() (P1-6 consolidation).
...
def _get_run_info(runs_csv_path: Path, run_id: str, workspace: str) -> dict:
    ...
    # Find all required columns using unified column discovery (P1-8 consolidation)
    col_map = find_required_columns(
        df,
        {
            "run_id": ["run_id", "run id", "runid"],
            "raw_data": ["path_raw_data", "raw_data_path", "data_dir"],
            "pattern": ["search_pattern", "pattern", "file_pattern"],
            "preprocessed": ["path_preprocessed_data", "preprocessed_path", "processed_output"],
            "resampled": ["path_resampled_data", "resampled_path", "resampled_output"],
        },
    )

    # Extract column names from map
    run_id_col = col_map["run_id"]
    raw_data_col = col_map["raw_data"]
    pattern_col = col_map["pattern"]
    preprocessed_col = col_map["preprocessed"]
    resampled_col = col_map["resampled"]
