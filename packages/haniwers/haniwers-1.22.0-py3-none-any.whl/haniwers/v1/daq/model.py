...
class RawEvent:
    ...
    What is RawEvent?
        ...
        Supports flexible field formats: firmware build conditions may produce
        different numbers of fields. RawEvent stores all values exactly as received.
    ...
    Data fields:
        ...
        values: All space-separated values received from detector (list[str])
                Format varies by firmware build conditions
                Example with 7 fields: ["10", "5", "0", "823", "24.5", "10020.5", "34.5"]
                Example with 21 fields: ["10", "5", "0", ..., extra_field_n]
        headers: Field names corresponding to values (optional, from GET_HEADERS)
                 Example: ["hit1", "hit2", "hit3", "adc", "tmp_c", "atm_pa", "hmd_pct", ...]
                 When available, enables name-based field access via get_field()

    Example workflow:
        1. Detector sends: "10 5 0 823 24.5 10020.5 34.5" (or any number of fields)
        2. RawEvent created: RawEvent.from_serial(line, timestamp)
        3. Store or process: event.to_list() saves to CSV

    Serialization:
        - to_serial(): Returns space-separated values (without timestamp)
        - to_list(): Returns [timestamp, value1, value2, ...] for CSV
        - from_serial(): Parse detector output (any field count)
        - from_list(): Parse CSV file (any field count)

    Field access:
        - get_field(name): Access value by field name (requires headers)
        - get_field_index(name): Get index of named field (requires headers)
    """

    time: pendulum.DateTime
    values: list[str]  # Flexible: supports any number of fields from firmware
    headers: list[str] | None = None  # Field names from firmware (kurikintons v1.21.0+)
    ...
    def to_serial(self) -> str:
        ...
        Returns:
            str: Space-separated values (timestamp not included)
                 Example with 7 fields: "10 5 0 823 24.50 10020.50 34.50"
                 Example with 21 fields: "10 5 0 823 24.50 ... extra_field_n"
    ...
    def to_list(self) -> list[str]:
        ...
        What this does:
            Creates a list with the timestamp first, then all sensor values.
            This format is perfect for saving to CSV files.

        Returns:
            list[str]: [timestamp, value1, value2, ...]
                       Examples:
                       - 7 fields: ["2025-10-19T14:23:45+09:00", "10", "5", "0", "823", "24.50", "10020.50", "34.50"]
                       - 21 fields: ["2025-10-19T14:23:45+09:00", "10", "5", "0", ..., "extra_field_n"]
        ...
        return [time] + self.values

    def get_field(self, field_name: str) -> str:
        """Get a detector value by field name.

        What this does:
            Looks up a field by name (e.g., "hit1", "adc", "tmp_c") and returns
            the corresponding value. Requires headers to be set (from GET_HEADERS).

        Args:
            field_name: Name of the field to retrieve (e.g., "hit1", "adc", "tmp_c")

        Returns:
            str: The value for this field

        Raises:
            ValueError: If headers are not available
            KeyError: If field_name not found in headers
            IndexError: If field index is out of range

        When to use:
            - Accessing fields by name instead of index
            - Writing code that's independent of field order
            - ProcessedEvent.from_raw_event() uses this for dynamic field support
        ...
        # Device.connect() automatically calls get_headers()
        # so device.headers contains field names

        # Read detector data
        line = device.readline()
        timestamp = pendulum.now()
        raw = RawEvent.from_serial(line, timestamp)
        raw.headers = device.headers  # Attach headers

        # Access by field name
        hit1_value = raw.get_field("hit1")
        adc_value = raw.get_field("adc")
        tmp_value = raw.get_field("tmp_c")
        ...
        if self.headers is None:
            raise ValueError(
                "Cannot get field by name: headers not available. "
                "Headers come from firmware GET_HEADERS command (kurikintons v1.21.0+)"
            )

        try:
            idx = self.headers.index(field_name)
        except ValueError as e:
            raise KeyError(f"Field '{field_name}' not found in headers: {self.headers}") from e

        if idx >= len(self.values):
            raise IndexError(
                f"Field '{field_name}' index {idx} out of range "
                f"(only {len(self.values)} values available)"
            )
    ...
    def get_field_index(self, field_name: str) -> int:
        ...
        What this does:
            Finds the column index of a named field. Useful when you need to know
            where in the values list a field is located.

        Args:
            field_name: Name of the field (e.g., "hit1", "adc")

        Returns:
            int: The index (0-based) of this field in the values list
        ...
        ```python
        raw.headers = ["hit1", "hit2", "hit3", "adc", "tmp_c", ...]

        # Get index of "adc"
        adc_idx = raw.get_field_index("adc")
        print(adc_idx)  # Output: 3

        # Direct value access using index
        adc_value = raw.values[adc_idx]
        ```
        """
        if self.headers is None:
            raise ValueError("Cannot get field index: headers not available")
    ...
    def _from_values(cls, time: pendulum.DateTime, values: list[str]) -> "RawEvent":
        """Internal helper: Create RawEvent from timestamp and sensor values.
        ...
        Args:
            ...
            values: List of sensor values (flexible length based on firmware)
                    Examples: [7 values], [21 values], or any other count

        Returns:
            RawEvent: A new RawEvent object with all values preserved

        Raises:
            ValueError: If values list is empty
        """
        if not values:
            msg = "Expected at least 1 value, got 0"
    ...
    def from_serial(cls, line: str, time: pendulum.DateTime) -> "RawEvent | None":
        ...
        try:
            ...
            # Skip lines with conversion errors (empty value list)
            return None
    ...
    def from_list(cls, values: list[str]) -> "RawEvent":
        ...
        What this does:
            ...
            into a RawEvent object. Supports flexible field counts from
            any firmware configuration.

        Args:
            values: List with timestamp as first element, then any number
                    of sensor values
                    Examples:
                    - 8 elements: [timestamp, value1, ..., value7]
                    - 22 elements: [timestamp, value1, ..., value21]
        ...
        Raises:
            ValueError: If timestamp can't be parsed
        ...
        with open("detector_data.csv") as f:
            ...
            for row in reader:
                # row = ["2025-10-19T14:23:45", "10", "5", ..., "value_n"]
    ...
    def header(cls) -> list[str]:
        """Get the default column names for CSV files.

        What this does:
            Returns the standard column names for a RawEvent CSV file.
            Note: Actual CSV columns may vary based on firmware version,
            so this represents the default/typical configuration.

        Returns:
            list[str]: Default column names from schema.RAW_COLUMNS
            Example: ["timestamp", "value1", "value2", ..., "valueN"]
            Note: Actual field count may differ based on firmware

        When to use:
            - Creating CSV files from detector data (when field count is known)
...
class ProcessedEvent(BaseModel):
    ...
    def from_raw_event(cls, raw: RawEvent, threshold: dict[str, int]) -> "ProcessedEvent":
        ...
        Note (v1.21.0+):
            Supports dynamic field formats via headers from GET_HEADERS command.
            If "hit_type" field exists in detector output, uses it directly.
            Otherwise calculates hit_type from individual hit fields.
        """
        # Extract hit values and hit_type (try headers first, fall back to position)
        try:
            top_hit = int(raw.get_field("hit1")) > threshold.get("top", 0)
            mid_hit = int(raw.get_field("hit2")) > threshold.get("mid", 0)
            btm_hit = int(raw.get_field("hit3")) > threshold.get("btm", 0)

            # Try to get hit_type from field if it exists, otherwise calculate
            try:
                hit_type = int(raw.get_field("hit_type"))
            except KeyError:
                # Calculate from individual channels: ch1=0x01, ch2=0x02, ch3=0x04
                hit_type = (int(top_hit) * 0x01) | (int(mid_hit) * 0x02) | (int(btm_hit) * 0x04)

        except (ValueError, KeyError):
            # Fallback: position-based access (legacy format)
            try:
                top_hit = (
                    int(raw.values[1]) > threshold.get("top", 0) if len(raw.values) > 1 else False
                )
                mid_hit = (
                    int(raw.values[2]) > threshold.get("mid", 0) if len(raw.values) > 2 else False
                )
                btm_hit = (
                    int(raw.values[3]) > threshold.get("btm", 0) if len(raw.values) > 3 else False
                )
                # Calculate hit_type: ch1=0x01, ch2=0x02, ch3=0x04
                hit_type = (int(top_hit) * 0x01) | (int(mid_hit) * 0x02) | (int(btm_hit) * 0x04)
            except (ValueError, IndexError):
                raise ValueError(f"Cannot extract hit values from raw event: {raw.values}")

        # Extract environmental data (adc, tmp, atm, hmd)
        try:
            adc = int(raw.get_field("adc"))
            tmp = float(raw.get_field("tmp_c"))
            atm = float(raw.get_field("atm_pa"))
            hmd = float(raw.get_field("hmd_pct"))
        except (ValueError, KeyError):
            # Fallback: position-based access
            try:
                adc = int(raw.values[4]) if len(raw.values) > 4 else 0
                tmp = float(raw.values[8]) if len(raw.values) > 8 else 0.0
                atm = float(raw.values[9]) if len(raw.values) > 9 else 0.0
                hmd = float(raw.values[10]) if len(raw.values) > 10 else 0.0
            except (ValueError, IndexError):
