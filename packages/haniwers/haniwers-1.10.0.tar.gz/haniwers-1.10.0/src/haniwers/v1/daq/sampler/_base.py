...
class Sampler:
    ...
    def timestamped_filename(self, fid: int, mac_address: str = "unknown") -> Path:
        ...
        Args:
            fid (int): File ID number (incremented for each file in session)
                Example: 0 → "data_0000000_aabbccddee00.csv"
                Example: 1 → "data_0000001_aabbccddee00.csv"
        ...
        Returns:
            Path: Full path to output file
                Example: Path("./data/data_0000000_aabbccddee00.csv")
        ...
        for i in range(3):
            ...
            print(filename)
            # Output:
            # ./data/data_0000000_aabbccddee00.csv
            # ./data/data_0000001_aabbccddee00.csv
            # ./data/data_0000002_aabbccddee00.csv
        ```
        """
        fid_str = f"{fid:07d}"
        filename = f"{self.prefix}_{fid_str}_{mac_address}{self.suffix}"
    ...
    def run(self, mode: str = None, files: int = None):
        ...
        Args:
            mode (str, optional): "daq" (count-based) or "scan" (time-based)
                Default: Uses self.mode from config
                "daq": Collect self.events measurements per file
                "scan": Collect for self.duration seconds per file
