...
class Orchestrator:
    ...
    def __init__(self, argv: list[str] | None = None):
        ...
        Args:
            ...
            when not provided.
        """
        self.args       = parse_args(argv or sys.argv[1:])
        ...
        self.log_dir    = None
        self.log_text   = None
        if not argv: self.orchestrate()  # for tests
    ...
    def find_repo(self) -> tuple[str
        ...
        try:
            ...
            self.gitutils, self.giterr = utils.import_deps()
        except RuntimeError:
            if not CI_MODE:
                ...
                if utils.intent(prompt, "y", "return"):
                    ...
                    self.gitutils, self.giterr = modules
    ...
    def stage_and_commit(self) -> tuple[tuple[str, bool]
        ...
        try:
            ...
            msg = utils.gen_commit_message(self.subpkg)
            ...
            e = self.giterr.normalize_stderr(e)
    ...
    def push(self) -> tuple[str
        ...
        try: self.gitutils.fetch_all(self.repo)
            ...
            self.out.warn(self.giterr.normalize_stderr(e), False)
        ...
        if not branchless:
            try: self.gitutils.push(self.repo,
                ...
                msg = self.giterr.normalize_stderr(e), False
                return msg, utils.StepResult.ABORT
