...
from .mcp_protocol import handle_mcp_method, transform_tools_call_request

# Example tools are now optional - users should import them explicitly if needed
...
async def mcp_endpoint(request: Request):
    ...
    # Check if this is an MCP protocol method first
    mcp_response = await handle_mcp_method(rpc_request)
    if mcp_response is not None:
        # MCP protocol method handled
        if version == "2.0.0":
            logger.info(
                "MCP method handled",
    ...
    # Transform tools/call to tool name if needed
    rpc_request = transform_tools_call_request(rpc_request)
