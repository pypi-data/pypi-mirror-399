...
class Bot:
    def __init__(self, token: str):
        ...
        self.client = httpx.AsyncClient(
            ...
            limits=limits,
            headers={"User-Agent": "Rubio/1.0.0 (High-Performance)"}
        )
        self.handlers = []
    ...
    def on_callback(self, button_id: Optional[str] = None):
        """Decorator for handling button callbacks."""
        def decorator(func):
            if button_id:
    ...
    async def run(self):
        """Start the high-performance polling engine."""
        logger.info("🚀 Rubio V1 Engine Started | @RubioLib")
        offset_id = None
        while True:
            try:
                ...
                await asyncio.sleep(0.05) # Extreme low latency
            except Exception as e:
