...
class TestCLIUpIdempotent:
    ...
    def test_up_command_force_flag_accepted(self, tmp_path: Path) -> None:
...
""")
    ...
    def test_up_command_normal_execution(self, tmp_path: Path) -> None:
...
""")
        ...
        with patch("localargo.core.cluster.cluster_manager.create_cluster", return_value=True):
            ...
            # Note: Output checking is tricky with Click test runner and logging
            # The main test is that the command doesn't crash and returns a reasonable exit code

    def test_up_command_force_execution(self, tmp_path: Path) -> None:
...
""")
        ...
        with patch("localargo.core.cluster.cluster_manager.create_cluster", return_value=True):
            ...
            # Output can vary depending on logging configuration; ensure command completed
            # without crashing when --force is supplied.

    def test_up_command_execution_failure(self, tmp_path: Path) -> None:
...
""")
    ...
    def test_validate_command_status_flag(self, tmp_path: Path) -> None:
...
""")
        ...
        with patch("localargo.cli.commands.up.STANDARD_UP_STEPS", [mock_step]):
            result = runner.invoke(validate_cmd, ["--manifest", str(manifest_file), "--status"])

            assert result.exit_code == 0
            # Output can vary depending on logging; main check is successful exit.

    def test_validate_command_normal_mode(self, tmp_path: Path) -> None:
...
""")
    ...
    def test_validate_command_with_missing_env_vars(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
""")
    ...
    def test_validate_command_status_all_completed(self, tmp_path: Path) -> None:
...
""")
    ...
    def test_validate_command_status_some_pending(self, tmp_path: Path) -> None:
...
""")
    ...
    def test_validate_status_with_cluster_down(self, tmp_path: Path) -> None:
...
""")
        ...
        with patch("localargo.core.checkers.check_cluster") as mock_check_cluster:
            mock_check_cluster.return_value = StepStatus(state="pending", reason="Cluster does not exist")
    ...
    def test_validate_status_with_argocd_down(self, tmp_path: Path) -> None:
...
""")
        ...
        with (
            ...
            mock_check_cluster.return_value = StepStatus(state="completed", reason="Cluster is ready")
            mock_check_argocd.return_value = StepStatus(state="pending", reason="ArgoCD is not installed")
    ...
    def test_validate_status_with_everything_up(self, tmp_path: Path) -> None:
...
""")
        ...
        with (
            ...
            mock_check_cluster.return_value = StepStatus(state="completed", reason="Cluster is ready")
            mock_check_argocd.return_value = StepStatus(state="completed", reason="ArgoCD is ready")
    ...
    def test_validate_command_with_exclude_option(self, tmp_path: Path) -> None:
...
""")
        ...
        result = runner.invoke(validate_cmd, ["--manifest", str(manifest_file), "--exclude", "app1"])
    ...
    def test_up_command_with_exclude_option(self, tmp_path: Path) -> None:
...
""")
        ...
        with patch("localargo.core.execution.create_up_execution_engine") as mock_engine:
            ...
            result = runner.invoke(up_cmd, ["--manifest", str(manifest_file), "--exclude", "app1"])
