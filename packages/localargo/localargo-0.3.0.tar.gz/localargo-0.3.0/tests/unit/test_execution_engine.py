...
class TestUpExecutionEngine:  # pylint: disable=too-few-public-methods
    ...
    def test_create_up_execution_engine(self) -> None:
        ...
        # Should have the standard 11 steps (includes docker-networks, coredns and ca-distribution)
        assert len(engine.steps) == 11  # noqa: PLR2004
        ...
        assert "Docker networks" in descriptions[1]
        assert "Install ArgoCD" in descriptions[2]
        assert "Install nginx ingress" in descriptions[3]
        assert "CoreDNS" in descriptions[4]
        assert "Install cert-manager" in descriptions[5]
        assert "Set up dev Root CA and certificates" in descriptions[6]
        assert "CA certificate" in descriptions[7]

        # Check client requirements
        assert engine.steps[0].requires_client is False  # cluster
        assert engine.steps[1].requires_client is False  # docker-networks
        assert engine.steps[2].requires_client is False  # argocd
        assert engine.steps[3].requires_client is False  # nginx
        assert engine.steps[4].requires_client is False  # coredns
        assert engine.steps[5].requires_client is False  # cert-manager
        assert engine.steps[6].requires_client is False  # ca
        assert engine.steps[7].requires_client is False  # ca-distribution
        assert engine.steps[8].requires_client is False  # secrets
        assert engine.steps[9].requires_client is True  # repo-creds
        assert engine.steps[10].requires_client is True  # apps
