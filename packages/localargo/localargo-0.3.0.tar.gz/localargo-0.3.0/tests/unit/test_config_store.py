...
def test_resolve_config_default(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
...
def test_resolve_config_env_override(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
...
def test_load_returns_empty_when_missing(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    """Loading from a missing file should return an empty dict."""
    monkeypatch.setenv("LOCALARGO_CONFIG", str(tmp_path / "missing.yaml"))
    assert load_config() == {}


def test_save_then_load_round_trip(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
...
def test_config_store_set_and_save(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
