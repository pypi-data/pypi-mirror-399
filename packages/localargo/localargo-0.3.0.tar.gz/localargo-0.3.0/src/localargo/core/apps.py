...
def deploy(  # pylint: disable=too-many-locals  # noqa: PLR0913
    app_name: str | None,
    *,
    all_: bool,
    wait: bool,
...
def _build_steps(manifest_targets: list[AppSpec], argocd_targets: list[AppSpec]) -> list[str]:
    ...
    steps += [step for s in argocd_targets for step in (f"Create/Update {s.name}", f"Sync {s.name}")]
    return steps


def _apply_manifest_targets(
    manifest_targets: list[AppSpec],
    kubeconfig: str | None,
    log: StepLogger,
) -> None:
...
def _deploy_override_app(  # noqa: PLR0913
    app_name: str | None,
    *,
    wait: bool,
    override_repo: str,
...
def _build_override_spec(  # noqa: PLR0913
    app_name: str | None,
    *,
    override_repo: str,
    override_name: str | None,
