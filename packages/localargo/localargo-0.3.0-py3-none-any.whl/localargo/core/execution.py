...
#   1. cluster must exist before docker networks can be connected
#   2. docker networks should be connected before other services are installed
#   3. argocd must be ready before client-dependent steps (repo-creds, apps)
#   4. secrets must be created before apps (apps may reference secrets)
STANDARD_UP_STEPS = [
    ExecutionStep(
        name="cluster",
    ...
    ExecutionStep(
        ...
        description="Connect cluster to Docker networks",
        check_function=check_docker_networks,
        execute_function=execute_docker_networks,
        requires_client=False,
    ),
    ExecutionStep(
        name="argocd",
