...
def _build_spec_from_raw(raw: Any, idx: int) -> AppSpec:  # noqa: ANN401
...
def _parse_helm_values(raw: dict[str, Any], idx: int) -> list[str]:
    ...
    if not isinstance(helm_values, list) or not all(isinstance(v, str | os.PathLike) for v in helm_values):
...
def _normalize_manifest_files(value: Any) -> list[str]:  # noqa: ANN401
    if value is None:
        return []
...
def _apply_overlay_to_map(by_name: dict[str, AppSpec], raw: Any, idx: int) -> None:  # noqa: ANN401
    if not isinstance(raw, dict):
...
def _normalize_overlay_helm_values(value: Any) -> list[str]:  # noqa: ANN401
    hv = value or []
