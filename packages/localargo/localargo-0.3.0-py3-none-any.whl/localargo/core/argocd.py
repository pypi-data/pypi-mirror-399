...
from localargo.core.k8s import get_secret_data
...
class ArgoClient:
    ...
    Args:
        namespace (str): The namespace where ArgoCD is installed.
        server (str | None): The ArgoCD server address. If None, it will be auto-discovered.
        insecure (bool): Whether to use insecure connections.
    ...
    def _login_cli(self, *, force: bool = False) -> None:
        ...
        session_server = None if force else _find_server_with_valid_session(_candidate_servers(self.server))
        ...
        login_server = _login_first_success(
            _candidate_servers(self.server),
            password,
    ...
    def run_json_with_auth(self, args: list[str]) -> Any:  # noqa: ANN401
    ...
    def _get_resources_from_app_json(self, obj: Any) -> list[dict[str, Any]]:  # noqa: ANN401
...
def _repo_already_configured(stderr: str | None) -> bool:
    msg = stderr or ""
    return "AlreadyExists" in msg or "already associated" in msg or "repository is already configured" in msg
...
def _get_initial_admin_password(namespace: str) -> str:
    ...
    data = get_secret_data(namespace, "argocd-initial-admin-secret", "jsonpath={.data.password}")
...
def _dig(obj: Any, path: list[str]) -> Any:  # noqa: ANN401
    cur: Any = obj
    for key in path:
