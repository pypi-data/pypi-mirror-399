...
def _get_application_info(argocd_path: str, app_name: str) -> str:
    ...
    result = run_subprocess([argocd_path, "app", "get", app_name], check=True)
...
def health(namespace: str) -> None:
    ...
    health_checks = [_check_component_health(comp, desc, namespace, kubectl_path) for comp, desc in components]
    ...
    try:
        run_subprocess([argocd_path, "version", "--client"], check=True)
...
def events(app_name: str, output: str | None) -> None:
    ...
    try:
        ...
        logger.info("Fetching events for application '%s' in namespace '%s'...", app_name, namespace)
        ...
        if output:
            ...
            with Path(output).open("w", encoding="utf-8") as f:
                run_subprocess(cmd, stdout=f, check=True)
...
def _check_secret_references(app_name: str, argocd_path: str, kubectl_path: str | None) -> list[tuple[str, str]]:
...
def _extract_secret_refs_from_containers(container_spec: dict[str, Any], namespace: str) -> set[tuple[str, str]]:
...
def _extract_secret_refs_from_env_from(container: dict[str, Any], namespace: str) -> set[tuple[str, str]]:
...
def _extract_secret_refs_from_env_vars(container: dict[str, Any], namespace: str) -> set[tuple[str, str]]:
