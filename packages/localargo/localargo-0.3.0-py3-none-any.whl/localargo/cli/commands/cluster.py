...
@click.option(
    "--docker-network",
    multiple=True,
    help="Docker network(s) to connect the cluster to (can be specified multiple times)",
)
@click.argument("cluster_name", required=False)
def init(provider: str, name: str, docker_network: tuple[str, ...], cluster_name: str | None) -> None:
    ...
    if _do_create_cluster(provider, effective_name):
        if docker_network:
            _connect_to_docker_networks(provider, effective_name, list(docker_network))
...
def _connect_to_docker_networks(provider: str, cluster_name: str, networks: list[str]) -> None:
    """Connect the cluster container to Docker networks."""
    container_name = _get_container_name_for_cluster(cluster_name, provider)

    for network in networks:
        logger.info("Connecting cluster to Docker network '%s'...", network)
        try:
            run_subprocess(
            ...
            if "already exists" in str(e.stderr).lower():
                logger.info("Already connected to network '%s'", network)
            else:
                logger.error("Failed to connect to network '%s': %s", network, e)
                raise
