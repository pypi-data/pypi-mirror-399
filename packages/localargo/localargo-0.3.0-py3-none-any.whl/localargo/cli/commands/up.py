...
from localargo.core.executors import _resolve_secret_value
...
def _validate_environment_variables(upm: UpManifest) -> None:
    ...
    for sec in upm.secrets:
        for v in sec.secret_value:
            if isinstance(v, SecretValueFromEnv) and v.from_env and v.from_env not in os.environ:
...
def _print_completion_message(all_completed: bool) -> None:
...
def _print_step_status(step_number: int, step: ExecutionStep, status: StepStatus) -> None:
...
def _print_step_error(step_number: int, step: ExecutionStep, error: str) -> None:
...
def _describe_secret_source(v: SecretValueSpec) -> str:
    """Describe a secret value source for display."""
    if isinstance(v, SecretValueFromEnv):
        return f"env:{v.from_env}"
    if isinstance(v, SecretValueRandomBase64):
        return f"randomBase64({v.num_bytes})"
    if isinstance(v, SecretValueRandomHex):
        return f"randomHex({v.num_bytes})"
    if isinstance(v, SecretValueStatic):
        return "staticValue:<redacted>"
    if isinstance(v, SecretValueSameAs):
        return f"sameAs:{v.namespace}/{v.secret_name}:{v.secret_key}"
    return "<unknown>"
...
def _print_secrets_plan(upm: UpManifest) -> None:
    if upm.secrets:
        ...
        for sec in upm.secrets:
            sources = ", ".join([_describe_secret_source(v) for v in sec.secret_value]) or "<empty>"
            logger.info(
                "   - [%s] secret '%s': set key '%s' from: %s",
                sec.namespace,
                sec.secret_name,
                sec.secret_key,
                sources,
...
def _print_single_app_plan(app: AppEntry) -> None:
...
def _print_app_namespace(app: AppEntry) -> None:
    if getattr(app, "namespace", None):
        logger.info("     namespace: %s", app.namespace)


def _print_sources_details(app: AppEntry) -> None:
...
def _print_helm_details(helm_cfg: AppHelmConfig | None) -> None:
...
def _print_single_source_details(app: AppEntry) -> None:
    logger.info(
        "     repo=%s path=%s revision=%s",
        app.repo_url,
        app.path,
        app.target_revision,
    )
...
@click.option(
    "--force",
    is_flag=True,
    help="Force execution of all steps, bypassing state checks",
)
@click.option(
    "--exclude",
    "excluded_apps",
    multiple=True,
    help=("Exclude specific apps from deployment. If already installed, they will be removed from ArgoCD. Repeatable."),
)
def up_cmd(manifest_path: str | None, *, force: bool, excluded_apps: tuple[str, ...]) -> None:
    ...
    if force:
        logger.info(
            "✅ Up complete: %s of %s steps executed (force mode)",
            completed,
            total_steps,
        )
    else:
...
def _apply_secrets(upm: UpManifest) -> None:
    for sec in upm.secrets:
        ...
        for v in sec.secret_value:
            env_map[sec.secret_key] = _resolve_secret_value(v)
...
def _create_or_update_app(client: ArgoClient, app: AppEntry) -> None:
...
def _build_app_args(app: AppEntry, *, create: bool) -> list[str]:
...
def _append_repo_path_classic(base: list[str], app: AppEntry) -> None:
...
def _append_destination(base: list[str], app: AppEntry) -> None:
    base.extend(
        [
            "--dest-server",
...
def _append_revision_and_helm(base: list[str], app: AppEntry) -> None:
    ...
    if getattr(app, "sources", None):
        return
    hcfg = getattr(app, "helm", None)
    if hcfg:
        if getattr(hcfg, "release_name", None):
            base.extend(["--release-name", hcfg.release_name])
        for v in getattr(hcfg, "value_files", []) or []:
            base.extend(["--values", v])


def _append_source_helm_filtered(base: list[str], hcfg: AppHelmConfig | None, *, is_chart: bool) -> None:
    if hcfg:
        release_name = getattr(hcfg, "release_name", None)
        if release_name:
            base.extend(["--release-name", release_name])
        value_files = getattr(hcfg, "value_files", None)
        if value_files:
            for v in _filter_values_for_chart(value_files, is_chart=is_chart):
                base.extend(["--values", v])
...
def _compose_source_arg(s: SourceSpec) -> str:
