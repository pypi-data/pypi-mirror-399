...
class PendingGTTOrderResponse(BaseModel):
    """
    PendingGTTOrderResponse
    """ # noqa: E501
    s: Optional[StrictStr] = None
    d: Optional[List[PendingGTTOrderData]] = None
    __properties: ClassVar[List[str]] = ["s", "d"]

    @field_validator('s')
    def s_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in ('ok', 'no-data', 'error'):
            raise ValueError("must be one of enum values ('ok', 'no-data', 'error')")
        return value
    ...
    def from_dict(cls, obj: Dict) -> Self:
        ...
        _obj = cls.model_validate({
            "s": obj.get("s"),
            "d": [PendingGTTOrderData.from_dict(_item) for _item in obj.get("d")] if obj.get("d") is not None else None
        })
        return _obj
