...
class NexusPlatformerEnv(gym.Env):
    ...
    def __init__(
        ...
        # [player_x, player_y, player_vx, player_vy, grounded,
        #  goal_dx, goal_dy, stars_ratio,
        #  nearest_plat_dx, nearest_plat_dy, nearest_plat_w,
        #  nearest_hazard_dx, nearest_hazard_dy,
        #  nearest_star_dx, nearest_star_dy,
        #  level_progress, can_reach_goal,
        #  time_ratio, prev_action_move, prev_action_jump]
    ...
    def _get_game_url(self) -> str:
        ...
        if os.path.exists(os.path.join(game_dir, "index.html")):
            # For Vite build, add training mode query params including theme
            url = f"http://localhost:{self.http_port}/?training=true&ws=ws://{self.ws_host}:{self.ws_port}&theme={self.theme}"
            logger.info(f"Detected Vite build, using training mode URL with theme '{self.theme}'")
            return url
        else:
    ...
    def _get_observation(self) -> np.ndarray:
        ...
        for plat in state.platforms:
            dx = plat.get('x', 0) + plat.get('width', 0) / 2 - state.player_x
            dy = plat.get('y', 0) - state.player_y
            dist = abs(dx) + abs(dy)
            if dist < min_dist:
                min_dist = dist
                nearest_plat_dx = dx / max_x
                nearest_plat_dy = dy / max_y
                nearest_plat_w = plat.get('width', 100) / max_x
        ...
        prev_move = self.prev_action[0] / 2.0  # Normalize to [0, 1]
        prev_jump = float(self.prev_action[1])
    ...
    def _compute_reward(self, prev_state: Optional[GameState], new_state: Optional[GameState]) -> Tuple[float, Dict[str, float]]:
        ...
        if prev_state:
            ...
            components["stars"] = stars_gained * 5.0
            
            # Distance to nearest uncollected star shaping
            dist_prev = self._get_min_dist_to_star(prev_state)
            dist_new = self._get_min_dist_to_star(new_state)

            if dist_prev < float('inf') and dist_new < float('inf'):
                # Reward for getting closer to a star
                # Dense reward to guide exploration
                components["shaping"] = (dist_prev - dist_new) * 0.01

        # Level completion reward
        if new_state.level_complete:
            components["completion"] = 100.0
            
        # Death penalty
        if new_state.is_dead:
            components["death"] = -20.0
