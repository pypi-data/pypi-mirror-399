...
print("[MODULE LOAD] persistent_buffer.py loaded from:", __file__)
...
class PersistentBufferMemory(KaizenMemory):
    ...
    def _load_from_backend(self, session_id: str) -> Dict[str, Any]:
        ...
        """
        print(f"[TRACE] _load_from_backend() called with session_id={session_id}")
        ...
        try:
            print("[TRACE] Calling backend.load_turns()")
            # Load from backend
            turns = self.backend.load_turns(session_id, limit=self.max_turns)
            print(f"[TRACE] load_turns returned {len(turns)} turns")

            print("[TRACE] Calling backend.get_session_metadata()")
            # Get total count
            metadata = self.backend.get_session_metadata(session_id)
            turn_count = metadata.get("turn_count", len(turns))
            print(f"[TRACE] get_session_metadata returned turn_count={turn_count}")
            ...
            print(f"[TRACE] Returning data with {len(turns)} turns")
            return data

        except Exception as e:
            logger.error(f"Failed to load from backend: {e}")
            print(f"[TRACE] Exception in _load_from_backend: {e}")
