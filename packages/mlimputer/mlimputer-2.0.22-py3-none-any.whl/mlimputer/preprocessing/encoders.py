...
class AutoLabelEncoder(AbstractEncoder):
    ...
    def fit(self, X: pd.DataFrame, y: Optional[pd.Series] = None) -> 'AutoLabelEncoder':
        ...
        for col in self._categorical_columns:
            ...
            logger.debug(f"Fitted label encoder for column '{col}' with {len(unique_values)} unique values")
    ...
    def transform(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        for col in self._categorical_columns:
            ...
            X_encoded[col] = X[col].apply(
                lambda x: self._safe_transform(x, le, unknown_idx)
            )
    ...
    def inverse_transform(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        for col in self._categorical_columns:
            ...
            X_decoded[col] = X[col].apply(
                lambda x: self._safe_inverse_transform(x, le)
            )
