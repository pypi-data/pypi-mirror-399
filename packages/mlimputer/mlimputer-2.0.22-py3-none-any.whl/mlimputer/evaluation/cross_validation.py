...
from mlimputer.utils.constants import (
    ProblemType, MetricsComplement, EvaluationResult, CrossValidationResult,
    CrossValidationConfig, FoldStrategy)
...
class CrossValidator(AbstractValidator):
    ...
    def _create_splits(
        ...
        **kwargs
        ...
        if self._problem_type == ProblemType.REGRESSION or \
           self.config.fold_strategy == FoldStrategy.KFOLD:
            splitter = KFold(
    ...
    def _create_splits(
        ...
        **kwargs
        ...
        Create cross-validation splits.
        """
        if self._problem_type == ProblemType.REGRESSION or self.config.fold_strategy == FoldStrategy.KFOLD:
            splitter = KFold(
                n_splits=self.config.n_splits,
                shuffle=self.config.shuffle,
                random_state=self.config.random_state
    ...
    def _evaluate_model(
        ...
        for key, value in metrics_result.metrics.items():
            ...
            if abbreviated_key in ['mae', 'mse', 'rmse', 'r2', 'mape', 'accuracy', 'precision', 'recall', 'f1']:
    ...
    def _add_aggregate_metrics(self, leaderboard: pd.DataFrame) -> pd.DataFrame:
        ...
        metric_columns = metric_columns.drop(['Fold', 'Train Samples', 'Validation Samples'], errors='ignore')
        ...
        for model_name in leaderboard['Model'].unique():
            ...
            for metric in metric_columns:
                ...
                display_name = metric.upper() if metric in ['mae', 'mse', 'rmse', 'r2', 'mape', 'accuracy', 'precision', 'recall', 'f1'] else metric
    ...
    def _prepare_data(
        ...
        if target in X_features.columns:
            ...
            logger.warning(f"Target column '{target}' was found in features and removed to prevent contamination")
