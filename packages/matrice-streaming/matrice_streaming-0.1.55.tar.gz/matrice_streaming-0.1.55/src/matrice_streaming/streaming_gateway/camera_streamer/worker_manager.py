...
# ================================================================
# SHM RING SIZE OPTIMIZATION (Phase 3)
# ================================================================
# Ring mode: "legacy" uses 1000 slots (~5.8GB per camera for 1080p)
#            "optimized" uses 3-4 slots (~17.6MB per camera for 1080p)
# Optimized mode requires consumer to read faster than producer writes
SHM_RING_MODE = os.getenv("SHM_RING_MODE", "legacy")
SHM_OPTIMIZED_SLOT_COUNT = int(os.getenv("SHM_OPTIMIZED_SLOT_COUNT", "3"))

# Calculate effective slot count based on mode
SHM_SLOT_COUNT = SHM_OPTIMIZED_SLOT_COUNT if SHM_RING_MODE == "optimized" else 1000
...
class WorkerManager:
    ...
    def __init__(
        ...
        shm_slot_count: int = SHM_SLOT_COUNT,  # Ring buffer size per camera (env var controlled)
        ...
        # ================================================================
        use_blocking_threads: bool = None,  # None = use env var, True/False = override
    ):
        ...
        Args:
            ...
            use_blocking_threads: Use blocking capture threads instead of asyncio tasks (None = use env var)
        """
        ...
        # ================================================================
        # BLOCKING THREADS: Store optimized capture architecture setting
        # ================================================================
        # None means use the env var default in async_camera_worker.py
        self.use_blocking_threads = use_blocking_threads

        if use_blocking_threads is not None:
            self.logger.info(
                f"BLOCKING THREADS MODE: {'ENABLED' if use_blocking_threads else 'DISABLED'} (explicit override)"
            )
        else:
            self.logger.info(
                f"BLOCKING THREADS MODE: Using env var USE_BLOCKING_THREADS (default: true)"
            )
        ...
        try:
            ...
            worker = ctx.Process(
                ...
                args=(
                    ...
                    self.buffer_size,
                    # BLOCKING THREADS: Pass optimized capture architecture parameter
                    # None means use env var default (handled in async_camera_worker.py)
                    self.use_blocking_threads if self.use_blocking_threads is not None
                    else os.getenv("USE_BLOCKING_THREADS", "true").lower() == "true",
                ),
