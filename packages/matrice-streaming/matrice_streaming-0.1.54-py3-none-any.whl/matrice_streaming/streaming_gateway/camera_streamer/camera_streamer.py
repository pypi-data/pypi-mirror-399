...
class CameraStreamer:
    ...
    def calculate_batch_parameters(num_cameras: int, fps: int = 10) -> Dict[str, Any]:
        ...
        - Formula: max_batch = (throughput_per_worker * 0.1) / 4  (4 flush windows in 100ms)
        - With 50 cameras @ 30 FPS = 1500 msg/s → max_batch ≈ 37
        - Hard cap at 100 messages per batch
        ...
        Examples:
            ...
            - 11-50 cameras: batch_size=50, timeout=25ms (balance)
            - 50+ cameras: batch_size=100, timeout=25ms (high throughput)
        """
        if num_cameras <= 10:
            ...
            return {
                ...
                'flusher_interval': 0.01  # 10ms check interval
            }
        elif num_cameras <= 50:
            # Medium deployment: Balance latency and throughput
            return {
                'batch_size': 50,
                'batch_timeout': 0.025,  # 25ms - allows 4 flushes in 100ms SLA
                'flusher_interval': 0.025
            }
        else:
            # 50+ cameras per worker: Optimize for throughput with cross-stream batching
            # Key insight: With cross-stream batching, a single pipeline handles ALL streams
            # This reduces RTT from O(n streams) to O(1), allowing larger batches
            return {
                'batch_size': 100,
                'batch_timeout': 0.025,  # 25ms - allows 4 flushes in 100ms SLA
                'flusher_interval': 0.025
