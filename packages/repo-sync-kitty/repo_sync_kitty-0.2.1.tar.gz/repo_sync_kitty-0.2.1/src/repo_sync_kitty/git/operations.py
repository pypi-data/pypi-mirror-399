...
class RepoManager:
    ...
    def _remote_has_url(self, remote: "Remote") -> bool:
        """Check if a remote has a URL configured.

        Some remotes (e.g., git-annex special remotes) don't have URLs.
        """
    ...
    def fetch(self, remote_name: str | None = None) -> list[FetchInfo]:
        ...
        Note:
            Skips remotes without URLs (e.g., git-annex special remotes).
        """
        try:
            ...
            if remote_name:
                ...
                for remote in self.repo.remotes:
                    # Skip remotes without URLs (git-annex, special remotes)
                    if not self._remote_has_url(remote):
                        continue
                    results.extend(remote.fetch())
            return results
        except GitCommandError as e:
    ...
    def get_remote_urls(self) -> dict[str, str]:
        ...
        Skips remotes without URLs (e.g., git-annex remotes).
        """
        urls: dict[str, str] = {}
