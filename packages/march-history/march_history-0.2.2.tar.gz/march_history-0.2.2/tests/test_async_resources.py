"""Tests for async resource classes."""
...
class TestAsyncTenantResource:
    """Test async tenant resource operations."""

    @pytest.mark.asyncio
    @respx.mock
    async def test_list_tenants(self, async_client, base_url, tenant_response):
    ...
    async def test_list_tenants_with_pagination(
        ...
        await async_client.tenants.list(offset=10, limit=5)
    ...
    @pytest.mark.asyncio
    @respx.mock
    async def test_get_tenant_by_id(self, async_client, base_url, tenant_response):
        """Test getting tenant by ID."""
        respx.get(f"{base_url}/tenants/1").mock(
            return_value=Response(200, json=tenant_response)
        )

        tenant = await async_client.tenants.get(1)
    ...
    @pytest.mark.asyncio
    @respx.mock
    async def test_get_tenant_by_name(self, async_client, base_url, tenant_response):
        """Test getting tenant by name."""
        respx.get(f"{base_url}/tenants/by-name/test-tenant").mock(
            return_value=Response(200, json=tenant_response)
        )

        tenant = await async_client.tenants.get_by_name("test-tenant")

        assert tenant.id == 1
        assert tenant.name == "test-tenant"


class TestAsyncConversationResource:
    """Test async conversation resource operations."""
    ...
    async def test_list_conversations(
        ...
        convs = await async_client.conversations.list()
    ...
    @pytest.mark.asyncio
    @respx.mock
    async def test_list_conversations_with_filters(self, async_client, base_url):
    ...
    async def test_get_conversation(
        ...
        conv = await async_client.conversations.get(1)
    ...
    async def test_get_conversation_with_messages(
        self, async_client, base_url, conversation_response, message_response
    ):
        ...
        conv = await async_client.conversations.get(1, include_messages=True)
    ...
    async def test_update_conversation(
        ...
        respx.patch(f"{base_url}/conversations/1").mock(
            return_value=Response(200, json=updated_response)
        )

        conv = await async_client.conversations.update(
            1,
            title="Updated Title",
    ...
    async def test_archive_conversation(
        ...
        conv = await async_client.conversations.archive(1)
    ...
    async def test_unarchive_conversation(
        ...
        conv = await async_client.conversations.unarchive(1)

        assert conv.status == ConversationStatus.ACTIVE

    @pytest.mark.asyncio
    @respx.mock
    async def test_delete_conversation(self, async_client, base_url):
        """Test deleting a conversation."""
        route = respx.delete(f"{base_url}/conversations/1").mock(
            return_value=Response(204)
        )

        await async_client.conversations.delete(1)
    ...
    async def test_search_conversations(
        ...
        results = await async_client.conversations.search(
            q="test",
            tenant_name="test-tenant",
            status=ConversationStatus.ACTIVE,
        )
...
class TestAsyncMessageResource:
    """Test async message resource operations."""

    @pytest.mark.asyncio
    @respx.mock
    async def test_create_message(self, async_client, base_url, message_response):
    ...
    @pytest.mark.asyncio
    @respx.mock
    async def test_create_batch_messages(
        self, async_client, base_url, message_response
    ):
        ...
        msgs = await async_client.messages.create_batch(
            conversation_id=1,
            messages=[
    ...
    @pytest.mark.asyncio
    @respx.mock
    async def test_list_messages(self, async_client, base_url, message_response):
        """Test listing messages."""
        respx.get(f"{base_url}/conversations/1/messages").mock(
            return_value=Response(200, json=[message_response])
        )

        msgs = await async_client.messages.list(conversation_id=1)
    ...
    @pytest.mark.asyncio
    @respx.mock
    async def test_list_messages_with_role_filter(self, async_client, base_url):
        """Test listing messages with role filter."""
        route = respx.get(f"{base_url}/conversations/1/messages").mock(
            return_value=Response(200, json=[])
        )

        await async_client.messages.list(conversation_id=1, role=MessageRole.USER)
    ...
    @pytest.mark.asyncio
    @respx.mock
    async def test_get_message(self, async_client, base_url, message_response):
        """Test getting a message by ID."""
        respx.get(f"{base_url}/messages/1").mock(
            return_value=Response(200, json=message_response)
        )

        msg = await async_client.messages.get(1)
    ...
    @pytest.mark.asyncio
    @respx.mock
    async def test_search_messages(self, async_client, base_url, message_response):
        """Test searching messages in a conversation."""
        route = respx.get(f"{base_url}/conversations/1/messages/search").mock(
            return_value=Response(200, json=[message_response])
        )

        results = await async_client.messages.search(conversation_id=1, q="test")
    ...
    @pytest.mark.asyncio
    @respx.mock
    async def test_search_messages_global(
        self, async_client, base_url, message_response
    ):
        ...
        results = await async_client.messages.search_global(
            q="test",
            role=MessageRole.USER,
        )
