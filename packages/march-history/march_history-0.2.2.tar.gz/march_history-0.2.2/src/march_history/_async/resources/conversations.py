"""Async conversation resource for march-history SDK."""
...
class AsyncConversationResource(AsyncBaseResource):
    """
    Async conversation management resource.
    ...
    async def create(
        ...
        data = await self._post("/conversations/", json=request.model_dump(exclude_none=True))
    ...
    def list_iter(
        ...
        Returns:
            AsyncIterator yielding conversations

        Example:
            >>> async for conv in client.conversations.list_iter(tenant_name="acme"):
            ...     print(conv.title)
        """

        async def fetch_page(offset: int, limit: int) -> list[Conversation]:
    ...
    async def get(
        ...
        Example:
            >>> conv = await client.conversations.get(1, include_messages=True)
            >>> print(f"Messages: {len(conv.messages or [])}")
        """
        params = {"include_messages": include_messages}
        data = await self._get(f"/conversations/{conversation_id}", params=params)
        return Conversation(**data)
    ...
    async def update(
        ...
        Example:
            >>> conv = await client.conversations.update(
            ...     1,
            ...     title="Updated Title",
    ...
    async def archive(self, conversation_id: int) -> Conversation:
        """
        ...
        Example:
            >>> conv = await client.conversations.archive(1)
            >>> assert conv.status == ConversationStatus.ARCHIVED
        """
        data = await self._post(f"/conversations/{conversation_id}/archive")
        return Conversation(**data)

    async def unarchive(self, conversation_id: int) -> Conversation:
        """
        ...
        Example:
            >>> conv = await client.conversations.unarchive(1)
            >>> assert conv.status == ConversationStatus.ACTIVE
        """
        data = await self._post(f"/conversations/{conversation_id}/unarchive")
        return Conversation(**data)
    ...
    async def delete(self, conversation_id: int) -> None:
        ...
        Example:
            >>> await client.conversations.delete(1)
        """
        await self._delete(f"/conversations/{conversation_id}")
    ...
    async def search(
        ...
        Example:
            >>> # Search by title
            >>> results = await client.conversations.search(q="support")
            >>>
            >>> # Search by metadata
        ...
        data = await self._get("/conversations/search", params=params)
        return [Conversation(**item) for item in data]
    ...
    def search_iter(
        ...
        Returns:
            AsyncIterator yielding matching conversations

        Example:
            >>> async for conv in client.conversations.search_iter(q="support"):
            ...     print(conv.title)
        """
