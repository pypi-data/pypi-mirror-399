...
class Idb:
    ...
    def mount_developer_image(self, udid: str = "", timeout: int = 30) -> bool:
        """
        挂载开发者镜像
        ...
        """
        target_udid = udid if udid else self.udid
        self._exec_cmd(["-u", target_udid, "developer"], json_parse=False)
        last_error = None
        ...
        while time.time() - start_time < timeout:
            try:
                result = self._exec_cmd(["-u", target_udid, "developer", "-l"], json_parse=False)
        ...
        raise last_error if last_error else TimeoutError(f"Mount developer image timeout after {timeout}s")
