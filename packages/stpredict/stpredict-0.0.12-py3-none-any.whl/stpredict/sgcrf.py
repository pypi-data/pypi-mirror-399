...
class SparseGaussianCRFclassifier(SparseGaussianCRF):

      def fit(self, X, Y):
        """
        replace classes with -1 and 1 values
        """
        assert X.shape[0] == Y.shape[0], 'Inputs and Outputs must have the same number of observations'

        classes = sorted(np.unique(Y))
        first_class = classes[0]
        Y = np.where(Y == first_class, np.full(Y.shape, 1), np.full(Y.shape, -1))
      ...
      def predict(self, X, Y=None):
        ...
        """
        numerical_predictions = -np.dot(np.dot(inv(self.Lam), self.Theta.T), X.T).T
        applysigmoid = np.vectorize(sigmoid)

        return applysigmoid(numerical_predictions)