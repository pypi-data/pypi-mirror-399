...
def train_validate(data, feature_sets, forced_covariates = [], instance_validation_size = 0.3,
    ...
    supported_performance_measures = PERFORMANCE_MEASURES
    ...
    if (model_type == 'regression') and (any([item in CLASSIFICATION_PERFORMANCE_MEASURES for item in performance_measures])):
        performance_measures = [item for item in performance_measures if item not in CLASSIFICATION_PERFORMANCE_MEASURES]
        print("\nWarning: Some of the measures in the performance_measures can not be measured for {0} task and will be ignored.\n".format(model_type))
    
    if (model_type == 'classification') and (any([item in REGRESSION_PERFORMANCE_MEASURES for item in performance_measures])):
        performance_measures = [item for item in performance_measures if item not in REGRESSION_PERFORMANCE_MEASURES]
        print("\nWarning: Some of the measures in the performance_measures can not be measured for {0} task and will be ignored.\n".format(model_type))
    ...
    if (performance_benchmark not in supported_performance_measures) or (performance_benchmark is None):
        ...
        if performance_benchmark is not None:
            print("\nWarning: The specified performance_benchmark must be one of the supported performance measures:\n{0}\nThe incompatible cases will be ignored and replaced with '{1}'.\n".format(PERFORMANCE_MEASURES, alternative_performance_benchmark))
        performance_benchmark = alternative_performance_benchmark
            
    if (performance_benchmark in CLASSIFICATION_PERFORMANCE_MEASURES) and (model_type == 'regression'): 
        if zero_encounter_flag == 0:
    ...
    if (performance_benchmark in REGRESSION_PERFORMANCE_MEASURES) and (model_type == 'classification'):
...
#         print("\nWarning: The input data contain some zero values for Target variable. Therefore 'MAPE' can not be used as a benchmark and the benchmark will be set to 'MAE'.\n")
    ...
    if performance_benchmark in LOWER_IS_BETTER:
