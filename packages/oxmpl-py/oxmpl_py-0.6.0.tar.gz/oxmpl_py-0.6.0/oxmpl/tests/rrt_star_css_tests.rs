...
impl GoalSampleableRegion<CompoundState> for CompoundGoalRegion {
    fn sample_goal(&self, _rng: &mut impl Rng) -> Result<CompoundState, StateSamplingError> {
        let target_rv = self.target.components[0]
            .as_any()
            .downcast_ref::<RealVectorState>()
            .unwrap();
        let target_so2 = self.target.components[1]
            .as_any()
            .downcast_ref::<SO2State>()
            .unwrap();

        // Simply return target for this test to ensure it is in radius
        let rv_sample = RealVectorState::new(target_rv.values.clone());
        let so2_sample = SO2State::new(target_so2.value);
...
fn test_rrt_star_finds_path_in_css() {
    ...
    let mut planner = RRTStar::new(1.0, 0.1, 2.0, &PlannerConfig { seed: Some(0) });
