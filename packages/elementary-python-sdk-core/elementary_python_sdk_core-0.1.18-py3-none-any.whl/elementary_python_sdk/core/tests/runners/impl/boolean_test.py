...
def boolean_test(
    ...
    """Decorator to register a boolean data test.

    Decorate a function that returns a boolean. When called inside
    `elementary_test_context(...)`, the result is recorded as a test execution.

    The decorated function returns its original value when called outside a test
    context; inside a context, it returns None (the execution is recorded in the context).
    ...
    Args:
        ...
        column_name: Optional column name (for column-level tests).
        quality_dimension: Optional quality dimension override.
        skip: If True, the test is recorded as skipped.

    Returns:
        A decorator that records a boolean execution when invoked in a test context.
    ...
    Example:
        ...
        @boolean_test(name="no_null_ids", column_name="id")
        def no_null_ids(df: pd.DataFrame) -> bool:
            return df["id"].isna().sum() == 0
        ```
    """
