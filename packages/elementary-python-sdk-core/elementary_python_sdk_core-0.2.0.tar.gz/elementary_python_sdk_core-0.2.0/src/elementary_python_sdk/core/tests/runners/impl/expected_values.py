...
def expected_values(
    ...
    """Decorator to validate that returned values belong to an allowed set.

    Decorate a function that returns a single value or an iterable of values.
    When called inside `elementary_test_context(...)`, values not in `expected`
    are counted as failures and recorded in the context.

    Args:
        name: Test name.
        expected: Allowed value(s). Typically a list (e.g. `["US", "CA"]`).
        allow_none: If True, `None` values are allowed (treated as valid).
    ...
    Returns:
        A decorator that records an expected-values execution when invoked in a test context.
    """
