...
def expected_range(
    ...
    """Decorator to validate that returned numeric values are within a range.

    Decorate a function that returns either:
    - A single numeric value (e.g. average age)
    - An iterable of numeric values (e.g. a column/pandas Series)

    When called inside `elementary_test_context(...)`, values outside the range
    ...
    Returns:
        A decorator that records an expected-range execution when invoked in a test context.
    """
