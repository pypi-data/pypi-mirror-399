...
class GroundSolidColorAlgorithm(HeightmapColorAlgorithm):
    """Color algorithm that fills the entire texture with a single solid color."""

    DEFAULT_COLOR: tuple[int, int, int] = (192, 192, 192)  # Light gray

    def __init__(self, color: tuple[int, int, int] | None = None) -> None:
        """Initialize with a solid color.

        Args:
            color: RGB color tuple (0-255). Defaults to light gray.
        """
        self.color = color if color is not None else self.DEFAULT_COLOR

    def generate_texture(self, normalized_heights: np.ndarray) -> np.ndarray:
        """Generate RGB texture filled with the solid color."""
        rows, cols = normalized_heights.shape
        texture = np.full((rows, cols, 3), self.color, dtype=np.uint8)
        return texture
