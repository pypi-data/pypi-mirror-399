...
from barkMatterPy.PythonFlavors import isAnacondaPython
from barkMatterPy.PythonVersions import python_version
from barkMatterPy.utils.Distributions import getDistributionFromModuleName
from barkMatterPy.utils.FileOperations import getFileList, listDir
...
"""
        ...
        info = self.queryRuntimeInformationMultiple(
            ...
            values=(
                ...
                (
                    "barkMatterPy_patch_level",
                    applyBindingName(
                        "getattr(%(binding_name)s, '_barkMatterPy_patch_level', 0)"
                    ),
                ),
    ...
    def _getOxNJACPatchLevel(self):
        ...
        return self._getQtInformation().barkMatterPy_patch_level
...
import os
path = os.getenv("PATH", "")
if not path.startswith(__barkMatterPy_binary_dir):
    os.environ["PATH"] = __barkMatterPy_binary_dir + ";" + path
"""
            yield (
                code,
...
full support consider provided information.""",
            ...
            if python_version < 0x360:
                self.sysexit(
                    """\
The standard PySide2 is not supported before CPython <3.6. For full support: https://barkMatterPy.net/pages/pyside2.html"""
                )
