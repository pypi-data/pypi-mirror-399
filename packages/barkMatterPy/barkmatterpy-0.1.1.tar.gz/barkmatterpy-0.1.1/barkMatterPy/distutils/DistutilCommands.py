...
from barkMatterPy.__past__ import Iterable, unicode
...
from barkMatterPy.reports.CompilationReportReader import (
    getEmbeddedDataFilenames,
    parseCompilationReport,
)
from barkMatterPy.Tracing import wheel_logger
from barkMatterPy.utils.Execution import check_call
from barkMatterPy.utils.FileOperations import deleteFile, getFileList, renameFile
from barkMatterPy.utils.ModuleNames import ModuleName


def setupOxNJACDistutilsCommands(dist, keyword, value):
    # If the user project setup.py includes the key "build_with_barkMatterPy=True" all
    ...
    dist.cmdclass["bdist_wheel"] = bdist_barkMatterPy
...
class build(distutils.command.build.build):
    ...
    def _parseOptionsEntry(option, value):
        if option == "build_with_barkMatterPy":
            return
    ...
    def _build(self, build_lib):
        ...
        for is_package, module_name in self._findBuildTasks():
            ...
            if toml_filename:
                ...
                for option, value in toml_options.get("barkMatterPy", {}).items():
                    command.extend(self._parseOptionsEntry(option, value))

                for option, value in (
                    toml_options.get("tool", {}).get("barkMatterPy", {}).items()
                ):
                    command.extend(self._parseOptionsEntry(option, value))
            ...
            if "barkMatterPy" in self.distribution.command_options:
...
# Required by distutils, used as command name, pylint: disable=invalid-name
class bdist_barkMatterPy(wheel.bdist_wheel.bdist_wheel):
    ...
    def write_wheelfile(self, wheelfile_base, generator=None):
        if generator is None:
            from barkMatterPy.Version import getOxNJACVersion
