...
from barkMatterPy import TreeXML
from barkMatterPy.__past__ import unicode
from barkMatterPy.build.DataComposerInterface import getDataComposerReportValues
from barkMatterPy.build.SconsUtils import readSconsErrorReport
from barkMatterPy.code_generation.ConstantCodes import getDistributionMetadataValues
from barkMatterPy.containers.OrderedSets import OrderedSet
from barkMatterPy.freezer.IncludedDataFiles import getIncludedDataFiles
from barkMatterPy.freezer.IncludedEntryPoints import getStandaloneEntryPoints
from barkMatterPy.freezer.Standalone import getRemovedUsedDllsInfo
from barkMatterPy.importing.Importing import getPackageSearchPath
from barkMatterPy.importing.Recursion import getRecursionDecisions
from barkMatterPy.ModuleRegistry import (
    getDoneModules,
...
from barkMatterPy.OutputDirectories import (
    getResultRunFilename,
    getSourceDirectoryPath,
    hasMainModule,
)
from barkMatterPy.plugins.Plugins import getActivePlugins
...
from barkMatterPy.Tracing import ReportingSystemExit, reports_logger
from barkMatterPy.utils.Distributions import (
...
from barkMatterPy.utils.MemoryUsage import getMemoryInfos
...
def _getReportInputData(aborted):
    ...
    barkMatterPy_version = getOxNJACVersion()
    barkMatterPy_commercial_version = getCommercialVersion() or "not installed"

    barkMatterPy_aborted = aborted

    barkMatterPy_exception = sys.exc_info()
...
def writeCompilationReport(report_filename, report_input_data, diffable):
    ...
    exit_message = None
    if not report_input_data["barkMatterPy_aborted"]:
        completion = "yes"
    elif report_input_data["barkMatterPy_exception"][0] is KeyboardInterrupt:
        completion = "interrupted"
    elif report_input_data["barkMatterPy_exception"][0] is SystemExit:
        completion = "error exit (%s)" % report_input_data["barkMatterPy_exception"][1].code
    elif report_input_data["barkMatterPy_exception"][0] is ReportingSystemExit:
        completion = (
            "error exit message (%s)" % report_input_data["barkMatterPy_exception"][1].code
        )
        exit_message = report_input_data["barkMatterPy_exception"][1].exit_message
    else:
        completion = "exception"

    root = TreeXML.Element(
        "barkMatterPy-compilation-report",
        barkMatterPy_version=report_input_data["barkMatterPy_version"],
        barkMatterPy_commercial_version=report_input_data["barkMatterPy_commercial_version"],
...
_crash_report_filename = "barkMatterPy-crash-report.xml"
