...
import barkMatterPy.code_generation.BinaryOperationHelperDefinitions
import barkMatterPy.code_generation.CodeGeneration
import barkMatterPy.code_generation.ComparisonCodes
import barkMatterPy.code_generation.Namify
import barkMatterPy.nodes.NetworkxNodes
import barkMatterPy.nodes.PackageMetadataNodes
import barkMatterPy.nodes.PackageResourceNodes
import barkMatterPy.nodes.SideEffectNodes
import barkMatterPy.nodes.TensorflowNodes
...
import barkMatterPy.specs.BuiltinTypeOperationSpecs
import barkMatterPy.specs.HardImportSpecs
import barkMatterPy.tree.Building
from barkMatterPy.containers.OrderedDicts import OrderedDict
from barkMatterPy.containers.OrderedSets import OrderedSet
from barkMatterPy.nodes.ImportNodes import hard_modules_non_stdlib
from barkMatterPy.nodes.NodeMetaClasses import NodeCheckMetaClass
...
from barkMatterPy.utils.Jinja2 import getTemplate
...
def _getMixinForShape(shape):
    ...
    if shape is tshape_str:
        return "barkMatterPy.nodes.ExpressionShapeMixins.ExpressionStrShapeExactMixin"
    elif shape is tshape_list:
        return "barkMatterPy.nodes.ExpressionShapeMixins.ExpressionListShapeExactMixin"
    elif shape is tshape_tuple:
        return "barkMatterPy.nodes.ExpressionShapeMixins.ExpressionTupleShapeExactMixin"
    elif shape is tshape_int:
        return "barkMatterPy.nodes.ExpressionShapeMixins.ExpressionIntShapeExactMixin"
    elif shape is tshape_bool:
        return "barkMatterPy.nodes.ExpressionShapeMixins.ExpressionBoolShapeExactMixin"
    elif shape is tshape_none:
        return "barkMatterPy.nodes.ExpressionShapeMixins.ExpressionNoneShapeExactMixin"
    elif shape is tshape_dict:
        return "barkMatterPy.nodes.ExpressionShapeMixins.ExpressionDictShapeExactMixin"
    elif shape is tshape_bytes:
        return "barkMatterPy.nodes.ExpressionShapeMixins.ExpressionBytesShapeExactMixin"
    else:
        assert False, shape
...
processTypeShapeAttribute(
    "tshape_type",
    barkMatterPy.specs.BuiltinTypeOperationSpecs,
    python2_type_methods,
    python3_type_methods,
)
...
def emitGenerationWarning(emit, doc_string, template_name):
    ...
    for spec_descriptions in getSpecVersions(barkMatterPy.specs.HardImportSpecs):
...
def makeAttributeNodes():
    filename_python = "barkMatterPy/nodes/AttributeNodesGenerated.py"
    ...
    with withFileOpenedAndAutoFormatted(
        ...
        emit("from barkMatterPy.specs.BuiltinParameterSpecs import extractBuiltinArgs")

        emit("from barkMatterPy.nodes.ConstantRefNodes import makeConstantRefNode")
        emit(
            """\
from barkMatterPy.nodes.NodeMakingHelpers import (
    ...
    makeRaiseExceptionReplacementExpression
        ...
        emit(
            "from barkMatterPy.nodes.KeyValuePairNodes import makeKeyValuePairExpressionsFromKwArgs"
        )

        emit("from barkMatterPy.nodes.AttributeNodes import makeExpressionAttributeLookup")

        emit("from barkMatterPy.PythonVersions import getDictFromkeysNoArgErrorMessage")
...
def makeBuiltinOperationNodes():
    filename_python = "barkMatterPy/nodes/BuiltinOperationNodeBasesGenerated.py"
...
def makeChildrenHavingMixinNodes():
    ...
    filename_python = "barkMatterPy/nodes/ChildrenHavingMixins.py"
    filename_python2 = "barkMatterPy/nodes/ExpressionBasesGenerated.py"
    filename_python3 = "barkMatterPy/nodes/StatementBasesGenerated.py"
    ...
    with withFileOpenedAndAutoFormatted(
        ...
        emit1(
            """
from barkMatterPy.nodes.Checkers import (
    checkStatementsSequenceOrNone,
...
def makeHardImportNodes():
    ...
    filename_python = "barkMatterPy/nodes/HardImportNodesGenerated.py"
