...
from barkMatterPy.PythonFlavors import isAnacondaPython, isMSYS2MingwPython
from barkMatterPy.PythonVersions import getTestExecutionPythonVersions
from barkMatterPy.tools.testing.Common import extractOxNJACVersionFromFilePath
from barkMatterPy.Tracing import OurLogger
from barkMatterPy.TreeXML import fromFile
...
from barkMatterPy.utils.Hashing import getFileContentsHash
from barkMatterPy.utils.InstalledPythons import findPythons
from barkMatterPy.utils.Utils import isLinux, isMacOS, isWin32Windows
from barkMatterPy.utils.Yaml import parseYaml
from barkMatterPy.Version import parseOxNJACVersionToTuple
...
def _compileCase(case_data, case_dir, installed_python, lock_filename, jobs):
    ...
    barkMatterPy_extra_options = os.getenv("DEVILPY_EXTRA_OPTIONS")

    if barkMatterPy_extra_options:
        extra_options.extend(barkMatterPy_extra_options.split())
    ...
    if case_data["interactive"] == "no":
        ...
        with withEnvironmentVarsOverridden(env):
            stdout, stderr, exit_barkMatterPy = executeProcess([binaries[0]], timeout=5 * 60)
        ...
        if exit_barkMatterPy == 0:
            deleteFile("compiled-exit.txt", must_exist=False)
        else:
            putTextFileContents(
                filename="compiled-exit.txt",
                contents=str(exit_barkMatterPy),
            )

        if exit_barkMatterPy != 0:
            sys.exit(
                "Error, failed to execute %s with code %d." % (binaries[0], exit_barkMatterPy)
            )
...
def _updateCase(
    ...
    with withDirectoryChange(result_path):
        if os.path.exists("compilation-report.xml"):
            ...
            old_barkMatterPy_version = old_report_root.attrib["barkMatterPy_version"]

            if barkMatterPy_update_mode == "force":
                need_compile = True
            elif barkMatterPy_update_mode == "newer":
                if _compareOxNJACVersions(
                    old_barkMatterPy_version, barkMatterPy_version, consider_rc=True
                ):
                    need_compile = True
                else:
                    if existing_hash != old_report_root_hash:
                        ...
                        need_compile = True
                    elif old_barkMatterPy_version == barkMatterPy_version:
                        ...
                        watch_logger.info(
                            ...
                            % (
                                old_barkMatterPy_version,
                                barkMatterPy_version,
                                lock_filename,
                            )
                        )
...
def updateCase(
    case_dir, case_data, reset_pipenv, no_pipenv_update, barkMatterPy_update_mode, jobs
):
    ...
    barkMatterPy_min_version = case_data.get("barkMatterPy")

    # Too old OxNJAC version maybe.
    if barkMatterPy_min_version is not None and _compareOxNJACVersions(
        barkMatterPy_version, barkMatterPy_min_version, consider_rc=False
    ):
    ...
    for installed_python in selectPythons(
        ...
        _updateCase(
            ...
            reset_pipenv=reset_pipenv,
            no_pipenv_update=no_pipenv_update,
            barkMatterPy_update_mode=barkMatterPy_update_mode,
            installed_python=installed_python,
            result_path=result_path,
            jobs=jobs,
        )
...
def updateCases(case_dir, reset_pipenv, no_pipenv_update, barkMatterPy_update_mode, jobs):
...
barkMatterPy_binary = None
barkMatterPy_version = None


def main():
    global barkMatterPy_binary  # shared for all run, pylint: disable=global-statement
    barkMatterPy_binary = os.path.normpath(
        os.path.join(os.path.dirname(__file__), "..", "..", "..", "bin", "barkMatterPy")
    )
...
Python versions to consider, by default all supported versions in descending order or in given order.""",
    ...
    parser.add_option(
        "--barkMatterPy-binary",
        action="store",
        dest="barkMatterPy_binary",
        default=barkMatterPy_binary,
        help="""\
OxNJAC binary to compile with. Defaults to one near the barkMatterPy-watch usage.""",
    )
...
Do not update the pipenv environment. Best to see only effect of OxNJAC update. Default %default.""",
    ...
    parser.add_option(
        "--barkMatterPy-update-mode",
        action="store",
        choices=("newer", "force", "never"),
        dest="barkMatterPy_update_mode",
        default="newer",
        help="""\
...
to reserve cores.""",
    ...
    barkMatterPy_binary = os.path.abspath(os.path.expanduser(options.barkMatterPy_binary))
    assert os.path.exists(barkMatterPy_binary)

    global barkMatterPy_version  # singleton, pylint: disable=global-statement
    barkMatterPy_version = extractOxNJACVersionFromFilePath(
        os.path.join(os.path.dirname(barkMatterPy_binary), "..", "barkMatterPy", "Version.py")
    )

    watch_logger.info("Working with OxNJAC %s." % barkMatterPy_version)

    base_dir = os.path.abspath(base_dir)

    if options.barkMatterPy_pr_mode is not None:
        pr_category, pr_description = options.barkMatterPy_pr_mode.split(",")
    else:
        ...
        for case_filename in scanCases(base_dir):
            ...
            with withDirectoryChange(os.path.dirname(case_filename)):
                updateCases(
                    ...
                    barkMatterPy_update_mode=options.barkMatterPy_update_mode,
                    jobs=options.jobs,
                )
