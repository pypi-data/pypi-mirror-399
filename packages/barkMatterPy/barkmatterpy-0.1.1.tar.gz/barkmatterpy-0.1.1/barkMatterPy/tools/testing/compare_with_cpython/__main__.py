...
from barkMatterPy.OptionParsing import getOxNJACProjectOptions
...
from barkMatterPy.tools.testing.OutputComparison import compareOutput
...
from barkMatterPy.utils.FileOperations import deleteFile
from barkMatterPy.utils.Importing import getExtensionModuleSuffix
from barkMatterPy.utils.Timing import StopWatch
from barkMatterPy.utils.Utils import isMacOS
...
Taking coverage of '{filename}' using '{python}' with flags {args} ...""".format(
    ...
    if "DEVILPY" in os.environ:
        ...
        barkMatterPy_call = [os.environ["PYTHON"], os.environ["DEVILPY"]]
    else:
        if comparison_mode:
            barkMatterPy_call = [
                os.environ["PYTHON"],
                "-m",
                "barkMatterPy.__main__",  # Note: Needed for Python2.6
            ]
        else:
    ...
    if not two_step_execution:
        ...
        barkMatterPy_cmd = barkMatterPy_call + extra_options + ["--run", filename]

        if no_site:
            barkMatterPy_cmd.insert(len(barkMatterPy_cmd) - 1, "--python-flag=-S")
        ...
        if package_mode:
            ...
            barkMatterPy_cmd1 = barkMatterPy_call + extra_options + ["--mode=onefile", filename]
        elif standalone_mode:
            barkMatterPy_cmd1 = barkMatterPy_call + extra_options + ["--mode=standalone", filename]
        else:
            barkMatterPy_cmd1 = barkMatterPy_call + extra_options + [filename]

        if no_site:
            barkMatterPy_cmd1.insert(len(barkMatterPy_cmd1) - 1, "--python-flag=-S")
    ...
    if module_mode:
        ...
        barkMatterPy_cmd2 = [os.environ["PYTHON"], "-W", "ignore", "-c", mini_script]
    else:
        ...
        barkMatterPy_cmd2 = [os.path.join(output_dir, exe_filename)]
    ...
    if not two_step_execution:
        if trace_command:
            traceExecutedCommand("OxNJAC command", barkMatterPy_cmd)

        # Try a couple of times for permission denied, on Windows it can
        # be transient.
        for _i in range(5):
            with withPythonPathChange(barkMatterPy_package_dir):
                process = createProcess(command=barkMatterPy_cmd)

            stdout_barkMatterPy, stderr_barkMatterPy = process.communicate()
            exit_barkMatterPy = process.returncode

            if checkNoPermissionError(stdout_barkMatterPy) and checkNoPermissionError(
                stderr_barkMatterPy
            ):
                break

            my_print("Retrying barkMatterPy exe due to permission problems after delay.")
            time.sleep(2)

    else:
        if trace_command:
            my_print("OxNJAC command 1:", barkMatterPy_cmd1)

        for _i in range(5):
            with withPythonPathChange(barkMatterPy_package_dir):
                stdout_barkMatterPy1, stderr_barkMatterPy1, exit_barkMatterPy1 = executeProcess(
                    barkMatterPy_cmd1
                )

                python_path_used = os.environ["PYTHONPATH"]

            if exit_barkMatterPy1 != 0:
                if (
                    not expect_failure
...
%s
"""
                        % (barkMatterPy_cmd1, python_path_used, stderr_barkMatterPy1)
                    )

                exit_barkMatterPy = exit_barkMatterPy1
                stdout_barkMatterPy, stderr_barkMatterPy = stdout_barkMatterPy1, stderr_barkMatterPy1
                stdout_barkMatterPy2 = b"not run due to compilation error:\n" + stdout_barkMatterPy1
                stderr_barkMatterPy2 = stderr_barkMatterPy1
            else:
                # No execution second step for coverage mode.
                if comparison_mode:
                    if os.path.exists(barkMatterPy_cmd2[0][:-4] + ".cmd"):
                        barkMatterPy_cmd2[0] = barkMatterPy_cmd2[0][:-4] + ".cmd"

                    if trace_command:
                        my_print("OxNJAC command 2:", barkMatterPy_cmd2)

                    # Need full manual control
                    process = createProcess(command=barkMatterPy_cmd2, new_group=send_kill)
                    ...
                    stdout_barkMatterPy2, stderr_barkMatterPy2 = process.communicate()
                    stdout_barkMatterPy = stdout_barkMatterPy1 + stdout_barkMatterPy2
                    stderr_barkMatterPy = stderr_barkMatterPy1 + stderr_barkMatterPy2
                    exit_barkMatterPy = process.returncode

                    # In case of segfault or assertion triggered, run in debugger.
                    if exit_barkMatterPy in (-11, -6) and sys.platform != "nt":
                        barkMatterPy_cmd2 = wrapCommandForDebuggerForSubprocess(
                            command=barkMatterPy_cmd2
                        )

                        callProcess(barkMatterPy_cmd2, shell=False)
                else:
                    exit_barkMatterPy = exit_barkMatterPy1
                    stdout_barkMatterPy, stderr_barkMatterPy = stdout_barkMatterPy1, stderr_barkMatterPy1
    ...
    stop_watch.stop()
    barkMatterPy_time = stop_watch.getDelta()

    if "--encrypt-stderr" in project_options:
        with withPythonPathChange(barkMatterPy_package_dir):
            stderr_barkMatterPy = decryptOutput(project_options, stderr_barkMatterPy)

    if "--encrypt-stdout" in project_options:
        with withPythonPathChange(barkMatterPy_package_dir):
            stdout_barkMatterPy = decryptOutput(project_options, stdout_barkMatterPy)

    if not silent_mode:
        displayOutput(stdout_barkMatterPy, stderr_barkMatterPy)

        if coverage_mode:
            assert not stdout_barkMatterPy
            assert not stderr_barkMatterPy
    ...
    if comparison_mode:
        ...
        def makeComparisons(trace_result):
            exit_code_stdout = compareOutput(
                "stdout",
                stdout_cpython,
                stdout_barkMatterPy2 if two_step_execution else stdout_barkMatterPy,
                ignore_warnings,
                syntax_errors,
            )
            ...
            if ignore_stderr:
                ...
                exit_code_stderr = compareOutput(
                    "stderr",
                    stderr_cpython,
                    stderr_barkMatterPy2 if two_step_execution else stderr_barkMatterPy,
                    ignore_warnings,
                    syntax_errors,
                )

            exit_code_return = exit_cpython != exit_barkMatterPy

            if exit_code_return and trace_result:
                my_print(
                    """Exit codes {exit_cpython:d} (CPython) != {exit_barkMatterPy:d} (OxNJAC)""".format(
                        exit_cpython=exit_cpython, exit_barkMatterPy=exit_barkMatterPy
                    )
                )
        ...
        if (
            exit_code_return
            and exit_barkMatterPy in (-11, -6)
            and sys.platform != "nt"
            and not module_mode
            and not two_step_execution
        ):
            barkMatterPy_cmd.insert(len(barkMatterPy_cmd) - 1, "--debugger")

            with withPythonPathChange(barkMatterPy_package_dir):
                callProcess(barkMatterPy_cmd, shell=False)
    ...
    if remove_output:
        if not module_mode:
            if os.path.exists(barkMatterPy_cmd2[0]) and remove_binary:
                if os.name == "nt":
                    ...
                    if os.path.exists(barkMatterPy_cmd2[0] + ".away"):
                        os.unlink(barkMatterPy_cmd2[0] + ".away")

                    for _i in range(10):
                        try:
                            os.rename(barkMatterPy_cmd2[0], barkMatterPy_cmd2[0] + ".away")
                        except OSError:
                            time.sleep(0.1)
                            continue
    ...
    if comparison_mode and timing:
        my_print("CPython took %.2fs vs %0.2fs OxNJAC." % (cpython_time, barkMatterPy_time))

    if comparison_mode and not silent_mode:
        my_print("OK, same outputs.")


barkMatterPy_package_dir = os.path.normpath(
    os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", "..", ".."))
)


if __name__ == "__main__":
    # Unchanged, running from checkout, use the parent directory, the barkMatterPy
    # package ought be there.
    sys.path.insert(0, barkMatterPy_package_dir)
