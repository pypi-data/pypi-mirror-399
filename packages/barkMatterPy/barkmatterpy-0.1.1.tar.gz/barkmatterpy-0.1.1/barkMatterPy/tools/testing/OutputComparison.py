...
logging_info_re = re.compile(r"^OxNJAC(-\w+)?:([-\w]+:)? ")
logging_warning_re = re.compile(r"^OxNJAC.*?:WARNING")

# Python3.11 style traceback carets are not done by OxNJAC (yet?)
...
def makeDiffable(output, ignore_warnings, syntax_errors):
    ...
    for line in lines:
        ...
        if line.startswith("OxNJAC-Inclusion:WARNING: Cannot follow import to module"):
            continue
        if line.startswith("OxNJAC:WARNING: Cannot detect Linux distribution"):
            continue
        if line.startswith(
            "OxNJAC-Options:WARNING: You did not specify to follow or include"
        ):
            continue
        if line.startswith("OxNJAC:WARNING: Using very slow fallback for ordered sets"):
            continue
        if line.startswith("OxNJAC:WARNING: On Windows, support for input/output"):
            continue
        if line.startswith("OxNJAC:WARNING:     Complex topic"):
            continue
        if line.startswith("OxNJAC:WARNING:") and "matching checksum" in line:
            continue
        ...
        # This URL is updated, and OxNJAC outputs the new one, but we test
        ...
        # we are going to ignore the issue as OxNJAC is fine.
        if (
            line
            == """\
...
def compareOutput(
    kind, out_cpython, out_barkMatterPy, ignore_warnings, syntax_errors, trace_result=True
):
    from_date = ""
    to_date = ""
    ...
    diff = difflib.unified_diff(
        ...
        makeDiffable(out_barkMatterPy, ignore_warnings, syntax_errors),
        "{program} ({detail})".format(program=os.environ["PYTHON"], detail=kind),
        "{program} ({detail})".format(program="barkMatterPy", detail=kind),
        from_date,
        to_date,
        n=3,
    )
