...
from barkMatterPy.__past__ import md5, subprocess
from barkMatterPy.containers.OrderedDicts import OrderedDict
from barkMatterPy.Options import getCommercialVersion
from barkMatterPy.PythonVersions import getTestExecutionPythonVersions, isDebugPython
from barkMatterPy.Tracing import OurLogger, flushStandardOutputs, my_print
...
from barkMatterPy.utils.InstalledPythons import findInstalledPython
from barkMatterPy.utils.Jinja2 import getTemplate
from barkMatterPy.utils.Utils import isFreeBSD, isLinux, isMacOS, isWin32Windows
...
def compareWithCPython(dirname, filename, extra_flags, search_mode, on_error=None):
    ...
    if os.getenv("DEVILPY_TEST_INSTALLED", "") == "1":
        command = [
            sys.executable,
            "-m",
            "barkMatterPy.tools.testing.compare_with_cpython",
            path,
            "silent",
        ]
    else:
...
#     "# barkMatterPy-skip-unless-expression: expression to be evaluated"
#       OR
#     "# barkMatterPy-skip-unless-imports: module1,module2,..."
def checkTestRequirements(filename):
    ...
    for line in readSourceCodeFromFilename(None, filename).splitlines():
        if line.startswith("# barkMatterPy-skip-unless-"):
...
def decryptOutput(project_options, output):
    barkMatterPy_decrypt_call = [
        os.environ.get("PYTHON", sys.executable),
        "-m",
        "barkMatterPy.tools.commercial.decrypt",  # Note: Needed for Python2.6
    ]

    for project_option in project_options:
        if project_option.startswith("--encryption-key="):
            barkMatterPy_decrypt_call.append("--key=" + project_option.split("=", 1)[1])

    stdout, stderr, exit_code = executeProcess(barkMatterPy_decrypt_call, stdin=output)
