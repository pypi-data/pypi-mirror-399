...
from barkMatterPy.Options import getUserProvidedYamlFiles
...
def getYamlPackageConfiguration():
    ...
    if _package_config is None:
        _package_config = parsePackageYaml(
            "barkMatterPy.plugins.standard",
            "standard.barkMatterPy-package.config.yml",
        )
        _package_config.update(
            parsePackageYaml(
                "barkMatterPy.plugins.standard",
                "stdlib2.barkMatterPy-package.config.yml",
            )
        )
        _package_config.update(
            parsePackageYaml(
                "barkMatterPy.plugins.standard",
                "stdlib3.barkMatterPy-package.config.yml",
            )
        )
        ...
        try:
            _package_config.update(
                parsePackageYaml(
                    "barkMatterPy.plugins.commercial", "commercial.barkMatterPy-package.config.yml"
                )
            )
        except IOError:
...
def getYamlPackageConfigurationSchemaFilename():
    ...
    return os.path.join(
        ...
        "..",
        "misc",
        "barkMatterPy-package-config-schema.json",
    )
