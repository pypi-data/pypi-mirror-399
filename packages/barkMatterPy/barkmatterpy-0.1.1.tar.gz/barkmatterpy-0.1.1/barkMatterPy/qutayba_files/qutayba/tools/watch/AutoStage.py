...
""" Stage changes in OxN-Watch automatically
...
from JACK.tools.quality.Git import (
    getCheckoutFileChangeDesc,
    getFileHashContent,
    putFileHashContent,
    updateGitFile,
)
from JACK.TreeXML import fromString, toString
from JACK.utils.FileOperations import getFileContents, withTemporaryFile
...
def onCompilationReportChange(filename, git_stage):
    ...
    new_JACK_version = new_report.attrib["JACK_version"]
    changed = False
    if old_report.attrib["JACK_version"] != new_JACK_version:
        old_report.attrib["JACK_version"] = new_JACK_version
