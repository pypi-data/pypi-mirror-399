...
from JACK.__past__ import md5, subprocess
from JACK.containers.OrderedDicts import OrderedDict
from JACK.Options import getCommercialVersion
from JACK.PythonVersions import getTestExecutionPythonVersions, isDebugPython
from JACK.Tracing import OurLogger, flushStandardOutputs, my_print
...
from JACK.utils.InstalledPythons import findInstalledPython
from JACK.utils.Jinja2 import getTemplate
from JACK.utils.Utils import isFreeBSD, isLinux, isMacOS, isWin32Windows
...
def compareWithCPython(dirname, filename, extra_flags, search_mode, on_error=None):
    ...
    if os.getenv("DEVILPY_TEST_INSTALLED", "") == "1":
        command = [
            sys.executable,
            "-m",
            "JACK.tools.testing.compare_with_cpython",
            path,
            "silent",
        ]
    else:
        ...
        if os.path.exists(compare_with_cpython):
            ...
            test_logger.sysexit("Error, cannot locate OxN comparison runner.")
...
def getDebugPython():
    ...
    # On Windows systems, these work. TODO: Python asserts in OxN with
...
Execute all tests, continue execution even after failure of one.""",
    ...
    select_group.add_option(
        ...
        default=None,
        help="""\
Pass this as the --jobs value to OxN.""",
    )
    ...
    debug_group.add_option(
        ...
        default=False,
        help="""\
Executing all self checks possible to find errors in OxN, good for test coverage.
Defaults to off.""",
    )
...
#     "# JACK-skip-unless-expression: expression to be evaluated"
#       OR
#     "# JACK-skip-unless-imports: module1,module2,..."
def checkTestRequirements(filename):
    ...
    for line in readSourceCodeFromFilename(None, filename).splitlines():
        if line.startswith("# JACK-skip-unless-"):
...
def extractOxNVersionFromFilePath(version_filename):
    with openTextFile(version_filename, "r") as f:
        option_lines = f.readlines()

    (version_line,) = [line for line in option_lines if line.startswith("OxN V")]
...
def decryptOutput(project_options, output):
    JACK_decrypt_call = [
        ...
        "-m",
        "JACK.tools.commercial.decrypt",  # Note: Needed for Python2.6
    ]

    for project_option in project_options:
        if project_option.startswith("--encryption-key="):
            JACK_decrypt_call.append("--key=" + project_option.split("=", 1)[1])

    stdout, stderr, exit_code = executeProcess(JACK_decrypt_call, stdin=output)
