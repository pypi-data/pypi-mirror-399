...
""" Tool to compare output of CPython and OxN.
...
from JACK.OptionParsing import getOxNProjectOptions
...
from JACK.tools.testing.OutputComparison import compareOutput
...
from JACK.utils.FileOperations import deleteFile
from JACK.utils.Importing import getExtensionModuleSuffix
from JACK.utils.Timing import StopWatch
...
Taking coverage of '{filename}' using '{python}' with flags {args} ...""".format(
    ...
    if "DEVILPY" in os.environ:
        ...
        JACK_call = [os.environ["PYTHON"], os.environ["DEVILPY"]]
    else:
        if comparison_mode:
            JACK_call = [
                os.environ["PYTHON"],
                "-m",
                "JACK.__main__",  # Note: Needed for Python2.6
            ]
        else:
    ...
    if coverage_mode:
        # Coverage modules hates OxN to re-execute, and so we must avoid
        # that.
    ...
    # Now build the command to run OxN.
    if not two_step_execution:
        ...
        JACK_cmd = JACK_call + extra_options + ["--run", filename]

        if no_site:
            JACK_cmd.insert(len(JACK_cmd) - 1, "--python-flag=-S")
        ...
        if package_mode:
            ...
            JACK_cmd1 = JACK_call + extra_options + ["--mode=onefile", filename]
        elif standalone_mode:
            JACK_cmd1 = JACK_call + extra_options + ["--mode=standalone", filename]
        else:
            JACK_cmd1 = JACK_call + extra_options + [filename]

        if no_site:
            JACK_cmd1.insert(len(JACK_cmd1) - 1, "--python-flag=-S")
    ...
    if module_mode:
        ...
        JACK_cmd2 = [os.environ["PYTHON"], "-W", "ignore", "-c", mini_script]
    else:
        ...
        JACK_cmd2 = [os.path.join(output_dir, exe_filename)]
    ...
    if not two_step_execution:
        if trace_command:
            traceExecutedCommand("OxN command", JACK_cmd)

        # Try a couple of times for permission denied, on Windows it can
        # be transient.
        for _i in range(5):
            with withPythonPathChange(JACK_package_dir):
                process = createProcess(command=JACK_cmd)

            stdout_JACK, stderr_JACK = process.communicate()
            exit_JACK = process.returncode

            if checkNoPermissionError(stdout_JACK) and checkNoPermissionError(
                stderr_JACK
            ):
                break

            my_print("Retrying JACK exe due to permission problems after delay.")
            time.sleep(2)

    else:
        if trace_command:
            my_print("OxN command 1:", JACK_cmd1)

        for _i in range(5):
            with withPythonPathChange(JACK_package_dir):
                stdout_JACK1, stderr_JACK1, exit_JACK1 = executeProcess(
                    JACK_cmd1
                )
...
%s
"""
                        % (JACK_cmd1, python_path_used, stderr_JACK1)
                    )

                exit_JACK = exit_JACK1
                stdout_JACK, stderr_JACK = stdout_JACK1, stderr_JACK1
                stdout_JACK2 = b"not run due to compilation error:\n" + stdout_JACK1
                stderr_JACK2 = stderr_JACK1
            else:
                # No execution second step for coverage mode.
                if comparison_mode:
                    if os.path.exists(JACK_cmd2[0][:-4] + ".cmd"):
                        JACK_cmd2[0] = JACK_cmd2[0][:-4] + ".cmd"

                    if trace_command:
                        my_print("OxN command 2:", JACK_cmd2)

                    # Need full manual control
                    process = createProcess(command=JACK_cmd2, new_group=send_kill)
                    ...
                    if send_kill:
                        ...
                        executeAfterTimePassed(
                            ...
                            func=lambda: killProcessGroup(
                                "OxN compiled program", process.pid
                            ),
                        )

                    stdout_JACK2, stderr_JACK2 = process.communicate()
                    stdout_JACK = stdout_JACK1 + stdout_JACK2
                    stderr_JACK = stderr_JACK1 + stderr_JACK2
                    exit_JACK = process.returncode

                    # In case of segfault or assertion triggered, run in debugger.
                    if exit_JACK in (-11, -6) and sys.platform != "nt":
                        JACK_cmd2 = wrapCommandForDebuggerForSubprocess(
                            command=JACK_cmd2
                        )

                        callProcess(JACK_cmd2, shell=False)
                else:
                    exit_JACK = exit_JACK1
                    stdout_JACK, stderr_JACK = stdout_JACK1, stderr_JACK1
    ...
    stop_watch.stop()
    JACK_time = stop_watch.getDelta()

    if "--encrypt-stderr" in project_options:
        with withPythonPathChange(JACK_package_dir):
            stderr_JACK = decryptOutput(project_options, stderr_JACK)

    if "--encrypt-stdout" in project_options:
        with withPythonPathChange(JACK_package_dir):
            stdout_JACK = decryptOutput(project_options, stdout_JACK)

    if not silent_mode:
        displayOutput(stdout_JACK, stderr_JACK)

        if coverage_mode:
            assert not stdout_JACK
            assert not stderr_JACK
    ...
    if comparison_mode:
        ...
        def makeComparisons(trace_result):
            exit_code_stdout = compareOutput(
                "stdout",
                stdout_cpython,
                stdout_JACK2 if two_step_execution else stdout_JACK,
                ignore_warnings,
                syntax_errors,
            )
            ...
            if ignore_stderr:
                ...
                exit_code_stderr = compareOutput(
                    "stderr",
                    stderr_cpython,
                    stderr_JACK2 if two_step_execution else stderr_JACK,
                    ignore_warnings,
                    syntax_errors,
                )

            exit_code_return = exit_cpython != exit_JACK

            if exit_code_return and trace_result:
                my_print(
                    """Exit codes {exit_cpython:d} (CPython) != {exit_JACK:d} (OxN)""".format(
                        exit_cpython=exit_cpython, exit_JACK=exit_JACK
                    )
                )
        ...
        if (
            exit_code_return
            and exit_JACK in (-11, -6)
            and sys.platform != "nt"
            and not module_mode
            and not two_step_execution
        ):
            JACK_cmd.insert(len(JACK_cmd) - 1, "--debugger")

            with withPythonPathChange(JACK_package_dir):
                callProcess(JACK_cmd, shell=False)
    ...
    if remove_output:
        if not module_mode:
            if os.path.exists(JACK_cmd2[0]) and remove_binary:
                if os.name == "nt":
                    ...
                    if os.path.exists(JACK_cmd2[0] + ".away"):
                        os.unlink(JACK_cmd2[0] + ".away")

                    for _i in range(10):
                        try:
                            os.rename(JACK_cmd2[0], JACK_cmd2[0] + ".away")
                        except OSError:
                            time.sleep(0.1)
                            continue
    ...
    if comparison_mode and timing:
        my_print("CPython took %.2fs vs %0.2fs OxN." % (cpython_time, JACK_time))
