...
from JACK.__past__ import basestring, iter_modules
from JACK.build.DataComposerInterface import deriveModuleConstantsBlobName
from JACK.containers.OrderedDicts import OrderedDict
from JACK.containers.OrderedSets import OrderedSet
from JACK.Errors import OxNSyntaxError
from JACK.freezer.IncludedDataFiles import IncludedDataFile
from JACK.freezer.IncludedEntryPoints import IncludedEntryPoint
from JACK.ModuleRegistry import addUsedModule
from JACK.PythonVersions import python_version
from JACK.Tracing import plugins_logger, printLine
from JACK.utils.CommandLineOptions import OurOptionGroup
...
from .PluginBase import OxNPluginBase, control_tags
...
def _addActivePlugin(plugin_class, args, force=False):
    ...
    assert isinstance(plugin_instance, OxNPluginBase), plugin_instance
    ...
    for item in iter_modules(scan_path):
        ...
        for detector in detectors:
            ...
            if detector.__name__.replace(
                "OxNPluginDetector", ""
            ) != plugin_class.__name__.replace("OxNPlugin", ""):
                plugins_logger.warning(
                    "Class names %r and %r do not match OxNPlugin* and OxNPluginDetector* naming convention."
...
def loadStandardPluginClasses():
    ...
    Returns:
        None
    """
    import JACK.plugins.standard

    _loadPluginClassesFromPackage(JACK.plugins.standard)

    try:
        import JACK.plugins.commercial
    except ImportError:
        pass
    else:
        _loadPluginClassesFromPackage(JACK.plugins.commercial)
...
class Plugins(object):
    ...
    def _reportImplicitImports(plugin, module, implicit_imports):
        from JACK.importing.Importing import getModuleNameAndKindFromFilename
        from JACK.importing.Recursion import decideRecursion, recurseTo
    ...
    def _createTriggerLoadedModule(cls, module, trigger_name, code, flags):
        ...
        from JACK.tree.Building import buildModule
    ...
    def onModuleInitialSet():
        ...
        from JACK.ModuleRegistry import addRootModule
    ...
    def writeExtraCodeFiles(onefile):
        # Circular dependency.
        from JACK.tree.SourceHandling import writeSourceCode
...
def listPlugins():
    ...
    printLine("The following plugins are available in OxN".center(80))
    printLine("-" * 80)
...
def isObjectAUserPluginBaseClass(obj):
    ...
    try:
        return (
            obj is not OxNPluginBase
            and issubclass(obj, OxNPluginBase)
    ...
    for plugin_name, (plugin_class, plugin_detector) in sorted(
        ...
        if plugin_name in Options.getPluginsEnabled():
            ...
            if plugin_class.isRelevant():
                ...
                plugin_class.warning(
                    "Not relevant with this OS, or OxN arguments given, not activated."
                )
...
def getPluginOptions(plugin_name):
    ...
    for option in plugin_options.get(plugin_name, {}):
        ...
        if "[REQUIRED]" in option.help:
            if not arg_value:
                plugins_logger.sysexit(
                    "Error, required plugin argument '%s' of OxN plugin '%s' not given."
