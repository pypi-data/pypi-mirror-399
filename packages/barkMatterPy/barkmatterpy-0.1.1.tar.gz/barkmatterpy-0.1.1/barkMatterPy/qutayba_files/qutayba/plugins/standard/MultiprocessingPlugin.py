...
class OxNPluginMultiprocessingWorkarounds(OxNPluginBase):
    """This is to make multiprocessing work with OxN and use compiled code.
...
"""
                % ("__JACK_binary_exe" if Options.isStandaloneMode() else "argv0"),
                """\
...
Monkey patching "multiprocessing" for compiled methods.""",
    ...
    def createFakeModuleDependency(module):
        ...
        # into the code and let OxN compile time optimize it, which will also
        # make sure joblib support is there for Python2 as well.
        if python_version >= 0x300:
            source_code += """
def __JACK_freeze_support():
    import sys
