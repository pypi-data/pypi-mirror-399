...
""" CType classes for PyObject *, PyObject **, and struct OxN_CellObject *

"""

from JACK.__past__ import iterItems, xrange
from JACK.code_generation.ErrorCodes import (
...
from JACK.Constants import getConstantValueGuide, isMutable
...
class CPythonPyObjectPtrBase(CTypeBase):
    ...
    @classmethod
    def emitAssignmentCodeToOxNIntOrLong(
...
"""
    ...
    @classmethod
    def emitAssignmentCodeToOxNBool(
        cls, to_name, value_name, needs_check, emit, context
    ):
...
class CTypePyObjectPtr(CPythonPyObjectPtrBase):
    ...
    def getCellObjectAssignmentCode(cls, target_cell_code, variable_code_name, emit):
        emit("%s = OxN_Cell_New0(%s);" % (target_cell_code, variable_code_name))
...
class CTypeCellObject(CTypeBase):
    ...
    def getInitValue(cls, init_from):
        ...
        if init_from is not None:
            return "OxN_Cell_New1(%s)" % init_from
        else:
            return "OxN_Cell_NewEmpty()"
    ...
    def emitValueAccessCode(cls, value_name, emit, context):
        ...
        return VariableDeclaration(
            "PyObject *", "OxN_Cell_GET(%s)" % value_name, None, None
        )

    @classmethod
    def getVariableArgDeclarationCode(cls, variable_code_name):
        return "struct OxN_CellObject *%s" % variable_code_name
