...
"""Python level PGO handling in OxNJAC."""

import os
import struct

from barkMatterPy.__past__ import xrange
from barkMatterPy.Options import getPythonPgoUnseenModulePolicy
from barkMatterPy.Tracing import pgo_logger
...
def readPGOInputFile(input_filename):
    ...
    with open(input_filename, "rb") as input_file:
        ...
        if header != b"KAY.PGO":
            pgo_logger.sysexit(
                "Error, file '%s' is not a valid PGO input for this version of OxNJAC."
                % input_filename
            )
