...
from barkMatterPy.optimizations.TraceCollections import (
    TraceCollectionPureFunction,
    withChangeIndicationsTo,
)
from barkMatterPy.PythonVersions import python_version
from barkMatterPy.specs.ParameterSpecs import (
    ParameterSpec,
    TooManyArguments,
    matchCall,
)
from barkMatterPy.Tracing import optimization_logger, printError
from barkMatterPy.tree.Extractions import updateVariableUsage
from barkMatterPy.tree.SourceHandling import readSourceLines
from barkMatterPy.tree.TreeHelpers import makeDictCreationOrConstant2
...
class ExpressionFunctionRef(ExpressionNoSideEffectsMixin, ExpressionBase):
    ...
    def getFunctionBody(self):
        if self.function_body is None:
            ...
            from barkMatterPy.ModuleRegistry import getModuleFromCodeName
...
class ExpressionFunctionCall(ChildrenHavingFunctionValuesTupleMixin, ExpressionBase):
    ...
    def computeExpression(self, trace_collection):
        ...
        if cost is not None and cost < 50:
            from barkMatterPy.optimizations.FunctionInlining import (
