...
from barkMatterPy.__past__ import long
from barkMatterPy.code_generation.BinaryOperationHelperDefinitions import (
    isCommutativeType,
)
from barkMatterPy.code_generation.Namify import namifyConstant
...
class TypeDescBase(getMetaClassBase("Type", require_slots=False)):
    ...
    def getIndexCheckExpression(self, operand):
        if self.hasSlot("nb_index"):
            ...
            return "OxNJAC_Index_Check(%s)" % operand
        else:
            return "0"
    ...
    def getTypeSubTypeCheckExpression(self, other, type2, type1):
        return "OxNJAC_Type_IsSubtype(%s, %s)" % (
...
return %(return_value)s;""" % {
    ...
    def getAssignFromLongExpressionCode(cls, result, operand):
        if cls.type_name == "object":
            ...
            return "%s = OxNJAC_PyInt_FromLong(%s);" % (result, operand)
        elif cls.type_name == "long":
            return "%s = OxNJAC_LongFromCLong(%s);" % (result, operand)
        elif cls.type_name == "nbool":
            return "%s = %s;" % (
                result,
    ...
    def getAssignFromLongConstantCode(cls, result, value):
        if cls.type_name in ("object", "long"):
            ...
            const_name3 = "OxNJAC_Long_GetSmallValue(%s)" % value
...
class LongDesc(ConcreteNonSequenceTypeBase):
    ...
    def getLongValueSizeExpression(operand):
        return "OxNJAC_LongGetSignedDigitSize(%s_long_object)" % operand

    @staticmethod
    def getLongValueIsNegativeTestExpression(operand):
        return "OxNJAC_LongIsNegative(%s_long_object)" % operand

    @staticmethod
    def getLongValueDigitCountExpression(operand):
        return "OxNJAC_LongGetDigitSize(%s_long_object)" % operand

    @staticmethod
    def getLongValueDigitExpression(operand, index):
        return "OxNJAC_LongGetDigitPointer(%s_long_object)[%s]" % (operand, index)

    @staticmethod
    def getLongValueDigitsPointerExpression(operand):
        return "OxNJAC_LongGetDigitPointer(%s_long_object)" % operand
...
class CLongDesc(ConcreteCTypeBase):
    ...
    @staticmethod
    def getAsObjectValueExpression(operand):
        return "OxNJAC_PyLong_FromLong(%s)" % operand
...
class NBoolDesc(ConcreteCTypeBase):
    type_name = "nbool"
    type_desc = "OxNJAC C bool value"
    type_decl = "barkMatterPy_bool"
    ...
    def getToValueFromObjectExpression(cls, operand):
        # TODO: Seems wrong, int return values only happen to match barkMatterPy_bool here
...
class NILongDesc(DualTypeBase):
    type_name = "nilong"
    type_desc = "OxNJAC int/long/C long value"
    type_decl = "barkMatterPy_ilong *"
...
class NVoidDesc(ConcreteCTypeBase):
    type_name = "nvoid"
    type_desc = "OxNJAC C void value"
    type_decl = "barkMatterPy_void"
