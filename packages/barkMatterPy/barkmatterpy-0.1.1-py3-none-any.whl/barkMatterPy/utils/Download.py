...
from barkMatterPy import Tracing
from barkMatterPy.__past__ import urlretrieve
from barkMatterPy.Progress import withOxNJACDownloadProgressBar
...
def getDownload(name, url, download_path):
    ...
    with withOxNJACDownloadProgressBar(desc="Download %s" % name) as reporthook:
        try:
            try:
...
def getCachedDownload(
    ...
    barkMatterPy_download_dir = getDownloadCacheDir()

    barkMatterPy_download_dir = os.path.join(
        barkMatterPy_download_dir, os.path.basename(binary).replace(".exe", "")
    )

    if is_arch_specific:
        barkMatterPy_download_dir = os.path.join(barkMatterPy_download_dir, is_arch_specific)

    if specificity:
        barkMatterPy_download_dir = os.path.join(barkMatterPy_download_dir, specificity)

    download_path = os.path.join(barkMatterPy_download_dir, os.path.basename(url))
    exe_path = os.path.join(barkMatterPy_download_dir, binary)

    makePath(barkMatterPy_download_dir)
...
Fully automatic, cached. Proceed and download"""
                % (message, barkMatterPy_download_dir),
                choices=("yes", "no"),
                default="yes",
        ...
        if not os.path.isfile(exe_path) and os.path.isfile(download_path):
            ...
            try:
                ...
                for zip_info in zip_file.infolist():
                    ...
                    zip_file.extract(zip_info, barkMatterPy_download_dir)
...
def getCachedDownloadedMinGW64(target_arch, assume_yes_for_downloads, download_ok):
    ...
    gcc_binary = getCachedDownload(
        ...
        unzip=True,
        flatten=False,
        message="OxNJAC will use gcc from MinGW64 of winlibs to compile on Windows.",
        reject="Only this specific gcc is supported with OxNJAC.",
