...
from barkMatterPy.__past__ import basestring, unicode
from barkMatterPy.containers.OrderedDicts import OrderedDict
from barkMatterPy.Progress import enableThreading
from barkMatterPy.PythonFlavors import isTermuxPython
from barkMatterPy.Tracing import scons_details_logger, scons_logger
from barkMatterPy.utils.Execution import executeProcess
...
from barkMatterPy.utils.Utils import isLinux, isMacOS, isPosixWindows, isWin32Windows
...
def createEnvironment(
    ...
    env.barkMatterPy_python = getArgumentBool("barkMatterPy_python", False)
...
def provideStaticSourceFile(env, sub_path, c11_mode):
    source_filename = os.path.join(env.barkMatterPy_src, "static_src", sub_path)
...
def makeResultPathFileSystemEncodable(env, result_exe):
    ...
    if os.name == "nt" and not isFilesystemEncodable(result_exe):
        result_exe = os.path.join(
            ...
            "_barkMatterPy_temp.pyd" if env.module_mode else "_barkMatterPy_temp.exe",
        )
