...
def main():
    ...
    barkMatterPy_binary_name = getLaunchingOxNJACProcessEnvironmentValue("DEVILPY_BINARY_NAME")
    if barkMatterPy_binary_name is not None:
        sys.argv[0] = barkMatterPy_binary_name

    barkMatterPy_pythonpath_ast = getLaunchingOxNJACProcessEnvironmentValue(
        "DEVILPY_PYTHONPATH_AST"
    )

    if barkMatterPy_pythonpath_ast is not None:
        ...
        sys.path = [barkMatterPy_pythonpath_ast]
        import ast
        ...
        # from "barkMatterPy.distutils". Also ignore path elements that do not really
        # exist.
        sys.path = [
            path_element
    ...
    if needs_re_execution:
        from barkMatterPy.utils.ReExecute import reExecuteOxNJAC  # isort:skip
    ...
    # module of barkMatterPy package and renamed to where we can get at easily for
    # other uses.
    __import__("barkMatterPy").getLaunchingOxNJACProcessEnvironmentValue = (
        getLaunchingOxNJACProcessEnvironmentValue
    )

    from barkMatterPy import Options  # isort:skip
    ...
    from barkMatterPy.plugins.Plugins import activatePlugins
    ...
    if "DEVILPY_NAMESPACES" in os.environ:
        ...
        from barkMatterPy.importing.PreloadedPackages import setPreloadedPackagePaths
    ...
    if "DEVILPY_PTH_IMPORTED" in os.environ:
        ...
        from barkMatterPy.importing.PreloadedPackages import setPthImportedPackages
    ...
    if "DEVILPY_USER_SITE" in os.environ:
        from barkMatterPy.utils.Distributions import setUserSiteDirectory
    ...
    from barkMatterPy import MainControl  # isort:skip
...
if __name__ == "__main__":
    _barkMatterPy_package_home = getLaunchingOxNJACProcessEnvironmentValue(
        "DEVILPY_PACKAGE_HOME"
    )
    if _barkMatterPy_package_home is not None:
        sys.path.insert(0, _barkMatterPy_package_home)

        import barkMatterPy  # just to have it loaded from there, pylint: disable=unused-import

        assert sys.path[0] is _barkMatterPy_package_home
        del sys.path[0]
