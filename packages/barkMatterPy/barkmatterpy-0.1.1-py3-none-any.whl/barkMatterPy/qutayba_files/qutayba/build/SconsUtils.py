...
from JACK.__past__ import basestring, unicode
from JACK.containers.OrderedDicts import OrderedDict
from JACK.Progress import enableThreading
from JACK.PythonFlavors import isTermuxPython
from JACK.Tracing import scons_details_logger, scons_logger
from JACK.utils.Execution import executeProcess
...
from JACK.utils.Utils import isLinux, isMacOS, isPosixWindows, isWin32Windows
...
def createEnvironment(
    ...
    env.JACK_python = getArgumentBool("JACK_python", False)
...
def provideStaticSourceFile(env, sub_path, c11_mode):
    source_filename = os.path.join(env.JACK_src, "static_src", sub_path)
...
b) Using "--mingw64" lets OxN download MinGW64 for you. Note: MinGW64
...
def makeResultPathFileSystemEncodable(env, result_exe):
    ...
    if os.name == "nt" and not isFilesystemEncodable(result_exe):
        result_exe = os.path.join(
            ...
            "_JACK_temp.pyd" if env.module_mode else "_JACK_temp.exe",
        )
