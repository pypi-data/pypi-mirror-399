...
""" This is the main actions of OxN.
...
from JACK.build.DataComposerInterface import runDataComposer
from JACK.build.SconsUtils import (
    getSconsReportValue,
    readSconsErrorReport,
    readSconsReport,
)
from JACK.code_generation.ConstantCodes import (
    addDistributionMetadataValue,
    getDistributionMetadataValues,
)
from JACK.freezer.IncludedDataFiles import (
    addIncludedDataFilesFromFileOptions,
    addIncludedDataFilesFromPackageOptions,
    addIncludedDataFilesFromPlugins,
    copyDataFiles,
)
from JACK.freezer.IncludedEntryPoints import (
    addExtensionModuleEntryPoint,
    addIncludedEntryPoints,
    getStandaloneEntryPoints,
    setMainEntryPoint,
)
from JACK.importing.Importing import locateModule, setupImportingFromOptions
from JACK.importing.Recursion import (
    scanIncludedPackage,
    scanPluginFilenamePattern,
    scanPluginPath,
    scanPluginSinglePath,
)
from JACK.Options import (
    getPythonPgoInput,
...
from JACK.PostProcessing import executePostProcessing
from JACK.Progress import (
    closeProgressBar,
    reportProgressBar,
    setupProgressBar,
)
from JACK.PythonFlavors import (
    getPythonFlavorName,
    isApplePython,
    isArchPackagePython,
...
from JACK.PythonVersions import (
    getModuleLinkerLibs,
    getPythonABI,
...
from JACK.Serialization import ConstantAccessor
from JACK.Tracing import (
    doNotBreakSpaces,
    general,
    inclusion_logger,
    pgo_logger,
)
from JACK.tree import SyntaxErrors
from JACK.tree.ReformulationMultidist import createMultidistMainSourceCode
from JACK.utils import InstanceCounters
from JACK.utils.Distributions import getDistribution
...
from JACK.utils.Importing import getPackageDirFilename
from JACK.utils.MemoryUsage import reportMemoryUsage, showMemoryTrace
from JACK.utils.ModuleNames import ModuleName
from JACK.utils.ReExecute import callExecProcess, reExecuteOxN
from JACK.utils.StaticLibraries import getSystemStaticLibPythonPath
from JACK.utils.Utils import getArchitecture, isMacOS, isWin32Windows
from JACK.Version import getCommercialVersion, getOxNVersion
...
def runSconsBackend():
    ...
    scons_options["JACK_python"] = asBoolStr(isOxNPython())
...
OxN is very syntax compatible with standard Python. It is currently running
with Python version '%s', you might want to specify more clearly with the use
of the precise Python interpreter binary and '-m JACK', e.g. use this
'python%s -m JACK' option, if that's not the one the program expects.
""" % (
            python_version_str,
...
def _main():
    """Main program flow of OxN

    At this point, options will be parsed already, OxN will be executing
    ...
    if Options.shallCreatePythonPgoInput():
        ...
        reExecuteOxN(pgo_filename=pgo_filename)
...
""" Command line options of OxN.

These provide only the optparse options to use, and the mechanic to actually
do it, but updating and checking module "JACK.Options" values is not in
...
from JACK.PythonFlavors import getPythonFlavorName
from JACK.PythonVersions import isPythonWithGil
from JACK.utils.CommandLineOptions import SUPPRESS_HELP, makeOptionsParser
from JACK.utils.FileOperations import getFileContentByLine
...
# Indicator if we were called as "JACK-run" in which case we assume some
# other defaults and work a bit different with parameters.
_JACK_binary_name = os.path.basename(sys.argv[0])
if _JACK_binary_name == "__main__.py":
    _JACK_binary_name = "%s -m JACK" % os.path.basename(sys.executable)
is_JACK_run = _JACK_binary_name.lower().endswith("-run")
...
parser = makeOptionsParser(usage=usage_template % _JACK_binary_name)
...
parser.add_option(
    ...
    default=[],
    help="""\
Python flags to use. Default is what you are using to run OxN, this
...
parser.add_option(
    ...
    default=None,
    help="""\
Use debug version or not. Default uses what you are using to run OxN, most
...
Python binary to use for Scons. Otherwise OxN can
use what you run OxN with, or find Python installation,
...
and OxN will then find it and include it and all the modules found below that
...
and OxN will then find it and include it in the binary or extension module
...
with "JACK-onefile-unpack" rather than a stream that only the onefile
...
warnings_group = parser.add_option_group("Control the warnings to be given by OxN")
...
warnings_group.add_option(
    ...
    help="""\
Allow OxN to download external code if necessary, e.g. dependency
...
certain topics, and typically point to the OxN website. The mnemonic is the part
...
remove bloat, add hidden dependencies. Check the OxN Package Configuration
...
output_group.add_option(
    ...
    default=True,
    help="""\
Do not create a '.pyi' file for extension modules created by OxN. This is
...
created by OxN. They expose your API, but stubgen may cause issues.
Defaults to off.""",
)
...
debug_group.add_option(
    ...
    default=False,
    help="""\
Executing all self checks possible to find errors in OxN, do not use for
...
development_group = parser.add_option_group("OxN Development features")
...
development_group.add_option(
    ...
    github_action=False,
    help="""\
This is not incremental compilation, but for OxN development only. Takes
...
Note: This is experimental and not working with standalone modes of OxN yet.
Defaults to off.""",
)
...
disabled console and programs using the Windows Services Plugin of OxN commercial.
...
intended for developing plugins and OxN package configuration. Use e.g.
...
def getOxNProjectOptions(logger, filename_arg, module_mode):
    """Extract the OxN project options.

    Note: This is used by OxN project and test tools as well.
    """
    ...
    # The file specified may not exist, let the later parts of OxN handle this.
    try:
    ...
    for line_number, line in enumerate(contents_by_line):
        match = re.match(b"^\\s*#(\\s*)JACK-project(.*?):(.*)", line)
        ...
        if match:
            ...
            if expect_block and level <= cond_level:
                sysexit(
                    line_number,
                    "Error, 'JACK-project-if|else' is expected to be followed by block start.",
                )
            ...
            if command == "-if":
                if not arg.endswith(":"):
                    sysexit(
                        line_number,
                        "Error, 'JACK-project-if' needs to start a block with a colon at line end.",
                    )
                ...
                if r is not True and r is not False:
                    sys.exit(
                        "Error, 'JACK-project-if' condition %r (expanded to %r) does not yield boolean result %r"
                        % (arg, expanded, r)
                    )
                ...
                if arg:
                    sysexit(
                        line_number,
                        "Error, 'JACK-project-else' cannot have argument.",
                    )

                if cond_level != level:
                    sysexit(
                        line_number,
                        "Error, 'JACK-project-else' not currently allowed after nested JACK-project-if.",
                    )
...
def parseOptions(logger):
    ...
    for filename in filename_args:
        sys.argv = (
            [sys.argv[0]]
            + list(getOxNProjectOptions(logger, filename, module_mode))
            + sys.argv[1:]
        )
...
pass them to the compiled program execution."""
    ...
    return is_JACK_run, options, positional_args, extra_args


def runSpecialCommandsFromOptions(options):
    if options.plugin_list:
        from JACK.plugins.Plugins import listPlugins

        listPlugins()
        sys.exit(0)

    if options.list_package_dlls:
        from JACK.tools.scanning.DisplayPackageDLLs import displayDLLs

        displayDLLs(options.list_package_dlls)
        sys.exit(0)

    if options.list_package_exe:
        from JACK.tools.scanning.DisplayPackageDLLs import displayEXEs

        displayEXEs(options.list_package_exe)
        sys.exit(0)

    if options.list_package_data:
        from JACK.tools.scanning.DisplayPackageData import displayPackageData

        displayPackageData(options.list_package_data)
        sys.exit(0)

    if options.list_distribution_metadata:
        from JACK.tools.scanning.DisplayDistributions import (
    ...
    if options.edit_module_code:
        from JACK.tools.general.find_module.FindModuleCode import (
            editModuleCode,
        )
    ...
    if options.create_environment_from_report:
        from JACK.tools.environments.CreateEnvironment import (
    ...
    if options.generate_mingw64_header:
        from JACK.tools.general.generate_header.GenerateHeader import (
            generateHeader,
        )
...
from JACK.Constants import isConstant
from JACK.nodes.BuiltinOpenNodes import makeBuiltinOpenRefNode
from JACK.nodes.ConstantRefNodes import ExpressionConstantSysVersionInfoRef
from JACK.PythonVersions import (
    getFutureModuleKeys,
...
def _addHardImportNodeClasses():
    from JACK.nodes.HardImportNodesGenerated import hard_import_node_classes
...
import sys
import time 
from JACK import Progress, Tracing
from JACK.containers.OrderedDicts import OrderedDict
from JACK.containers.OrderedSets import (
    OrderedSet,
    recommended_orderedset_package_name,
)
from JACK.importing.StandardLibrary import isStandardLibraryPath
from JACK.OptionParsing import (
    parseOptions,
    run_time_variable_names,
...
from JACK.utils.Images import checkIconUsage
from JACK.utils.Importing import getInlineCopyFolder
from JACK.utils.StaticLibraries import getSystemStaticLibPythonPath
from JACK.utils.Utils import (
    getArchitecture,
    getCPUCoreCount,
    getLaunchingOxNProcessEnvironmentValue,
    getLinuxDistribution,
    getMacOSRelease,
    getOS,
    getWindowsRelease,
...
def parseArgs():
    ...
    global is_JACK_run, options, positional_args, extra_args, is_debug, is_non_debug
...
Error, the Python from Windows app store is not supported.""",
    ...
    is_JACK_run, options, positional_args, extra_args = parseOptions(
    ...
    if (
        getLaunchingOxNProcessEnvironmentValue("DEVILPY_RE_EXECUTION")
    ...
    if options.clean_caches:
        from JACK.CacheCleanup import cleanCaches
    ...
    if isMacOS():
        ...
        if (options.macos_target_arch or "native") != "native":
            from JACK.utils.SharedLibraries import (
    ...
    if python_version_str not in getSupportedPythonVersions():
        ...
        if "PYTHON" not in os.environ:
            Tracing.general.warning(
                """\
The Python version '%s' is only experimentally supported by OxN '%s', \
but an upcoming release will change that. In the mean time use Python \
version '%s' instead or newer OxN."""
                % (
                    python_version_str,
                    getOxNVersion(),
    ...
    if sys.version_info.releaselevel not in ("final", "candidate"):
        if python_version_str not in getNotYetSupportedPythonVersions():
            Tracing.general.sysexit(
                """\
Non-final versions '%s' '%s' are not supported by OxN, use the \
...
and recommended only for use in OxN development and testing."""
                % (python_version_str, sys.version_info.releaselevel)
            )
            ...
            Tracing.general.sysexit(
                """\
The Python version '%s' is not supported by OxN '%s', but an upcoming \
...
def _couldUseStaticLibPython():
    ...
    # OxN-Python is good to to static linking.
    if isOxNPython():
        return True, "OxN-Python is unexpectedly broken."

    if isHomebrewPython():
        return True, "Homebrew Python is unexpectedly broken."

    # Debian packages with are usable if the OS is new enough
    from JACK.utils.StaticLibraries import isDebianSuitableForStaticLinking
    ...
    if (
        ...
        if python_version >= 0x3C0 and not os.path.exists(
            ...
            return (
                False,
                "OxN on Debian-Python needs inline copy of hacl not included.",
            )

        return True, "OxN on Debian-Python needs package '%s' installed." % (
            "python2-dev" if str is bytes else "python3-dev"
        )

    if isMSYS2MingwPython():
        return True, "OxN on MSYS2 needs package 'python-devel' installed."
    ...
    if isAnacondaPython():
        if isMacOS():
            ...
            return (
                True,
                """\
OxN on Anaconda needs package for static libpython installed. \
...
improve it for best performance of the result."""
    ...
    if isPyenvPython():
        return True, "OxN on pyenv should not use '--enable-shared'."

    if isManyLinuxPython():
        return (
            True,
            """\
OxN on 'manylinux' has no shared libraries. Use container with \
...
def getSupportedPythonVersions():
    """Officially supported Python versions for OxN."""
...
def getPartiallySupportedPythonVersions():
    """Partially supported Python versions for OxN."""
...
def getTargetPythonDLLPath():
    ...
    from JACK.Options import shallUsePythonDebug
...
def getLaunchingSystemPrefixPath():
    from JACK.utils.Utils import getLaunchingOxNProcessEnvironmentValue

    return getLaunchingOxNProcessEnvironmentValue("DEVILPY_SYS_PREFIX")
...
def getSystemPrefixPath():
    ...
    Note:
        ...
        For OxN, it often is OK to break out of the virtualenv, and use the
    ...
    if _the_sys_prefix is None:
        ...
        if os.name == "nt":
            from JACK.utils.FileOperations import getDirectoryRealPath
...
def getInstalledPythonRegistryPaths(version):
    ...
    from JACK.__past__ import WindowsError
