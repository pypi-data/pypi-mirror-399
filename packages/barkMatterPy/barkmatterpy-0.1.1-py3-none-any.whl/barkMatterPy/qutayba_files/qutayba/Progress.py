...
from JACK.utils.ThreadedExecutor import RLock
...
def setupProgressBar(stage, unit, total, min_total=0):
    ...
    if use_progress_bar:
        Tracing.progress = OxNProgressBar(
            iterable=None,
            stage=stage,
            total=total,
            min_total=min_total,
...
@contextmanager
def withOxNDownloadProgressBar(*args, **kwargs):
    if not use_progress_bar or _getTqdmModule() is None:
        yield
    else:

        class OxNDownloadProgressBar(tqdm):
        ...
        with OxNDownloadProgressBar(*args, **kwargs) as progress_bar:
