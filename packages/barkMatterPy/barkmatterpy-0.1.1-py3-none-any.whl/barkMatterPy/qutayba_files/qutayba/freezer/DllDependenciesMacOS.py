...
from JACK.Errors import OxNForbiddenDLLEncounter
from JACK.plugins.Plugins import Plugins
from JACK.PythonFlavors import isAnacondaPython, isOxNPython
...
from JACK.utils.Importing import getExtensionModuleSuffixes
from JACK.utils.Json import loadJsonFromFilename
...
from JACK.utils.Utils import getArchitecture
...
def _resolveBinaryPathDLLsMacOS(
    ...
    for path in paths:
        if path.startswith("@rpath/"):
            ...
            continue
        elif isOxNPython() and not os.path.isabs(path) and not os.path.exists(path):
            # Although OxN Python statically links all packages, some of them
            ...
            # dependencies, so we need to scan the OxN Python library
        ...
        if not os.path.exists(resolved_path):
            ...
            if acceptable is True:
                raise OxNForbiddenDLLEncounter(binary_filename, plugin_name)
