...
class AstriOpticalSystem(OpticalSystem):
    ...
    def configure(self, config: Union[str, Path, Dict[str, Any]]) -> None:
        ...
        if 'window' in os_conf:
            ...
            if "Window0" in self:
                top_layers = ["Window1", "Window3", "Window5"]
                bottom_layers = ["Window0", "Window2", "Window4"]
            else:
                top_layers = ["Window"]
                bottom_layers = []

            for s_name in top_layers:
                if 'top_layer_properties' in surface_dict:
                    prop_file = surface_dict['top_layer_properties']
                    surface_properties = SurfaceProperties.load_json(prop_file)
                    self[s_name].properties = surface_properties   
            
            for s_name in bottom_layers:
                if 'bottom_layer_properties' in surface_dict:
                    prop_file = surface_dict['bottom_layer_properties']
                    surface_properties = SurfaceProperties.load_json(prop_file)
                    self[s_name].properties = surface_properties
