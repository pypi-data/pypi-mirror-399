...
class OpticalStackSimulator:
    ...
    def _load_default_materials(self) -> None:
        ...
        self.register_material("HfO2", lambda lam: 1.875 + 6.28e-3 * (lam*1e6)**(-2) + 5.80e-4 * (lam*1e6)**(-4)) # Al-Kuhaili 2004
    ...
    def _get_silicon_n_interp(lam_array: np.ndarray) -> np.ndarray:
        ...
        # Wavelength
        wl = np.array([
            0.2066, 0.2101, 0.2138, 0.2175, 0.2214, 0.2254, 0.2296, 0.2339, 
            0.2384, 0.2431, 0.2480, 0.2530, 0.2583, 0.2638, 0.2695, 0.2755, 
            0.2818, 0.2883, 0.2952, 0.3024, 0.3100, 0.3179, 0.3263, 0.3351, 
            0.3444, 0.3542, 0.3647, 0.3757, 0.3875, 0.3999, 0.4133, 0.4275, 
            0.4428, 0.4592, 0.4769, 0.4959, 0.5166, 0.5391, 0.5636, 0.5904, 
            0.6199, 0.6525, 0.6888, 0.7293, 0.7749, 0.8266
        ])

        # Refractive index
        n = np.array([
            1.010, 1.083, 1.133, 1.186, 1.247, 1.340, 1.471, 1.579, 
            1.589, 1.571, 1.570, 1.597, 1.658, 1.764, 1.988, 2.452, 
            3.120, 4.087, 4.888, 5.020, 5.010, 5.016, 5.065, 5.156, 
            5.296, 5.610, 6.522, 6.709, 6.062, 5.570, 5.222, 4.961, 
            4.753, 4.583, 4.442, 4.320, 4.215, 4.123, 4.042, 3.969, 
            3.906, 3.847, 3.796, 3.752, 3.714, 3.673
        ])

        # Extinction coefficient
        k = np.array([
            2.909, 2.982, 3.045, 3.120, 3.206, 3.302, 3.366, 3.353, 
            3.354, 3.429, 3.565, 3.749, 3.979, 4.278, 4.678, 5.082, 
            5.344, 5.395, 4.639, 3.979, 3.586, 3.346, 3.182, 3.058, 
            2.987, 3.014, 2.705, 1.320, 0.630, 0.387, 0.269, 0.203, 
            0.163, 0.130, 0.090, 0.073, 0.060, 0.048, 0.032, 0.030, 
            0.022, 0.016, 0.013, 0.010, 0.008, 0.005
        ])
    ...
    def plot_simulation_results(
        ...
        # Extract desired data (R or T) and compute unpolarized average
        data_s = res_s[mode]
        data_p = res_p[mode]
        data_unpol = 0.5 * (data_s + data_p)
        ...
        aspect_ratio = (wl_max_nm - wl_min_nm) / (ang_max_deg - ang_min_deg) / 5

        fig = plt.figure(figsize=(3*4*aspect_ratio, 4))  
