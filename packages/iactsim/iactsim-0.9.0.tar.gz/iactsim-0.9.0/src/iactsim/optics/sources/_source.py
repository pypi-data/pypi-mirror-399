...
class Source():
    ...
    def visualize(self, n_photons: int = 1000, opacity=None, length=None, show_hits=True, show_rays=True, resolution=None):
        ...
        ps_start = ps.get()
        vs_start = vs.get()
        self._tel.trace_photons(ps, vs, wls, ts)
        ps_stop = ps.get()

        # transform into the local reference system
        tel_to_local = local_to_telescope_rotation(*self._tel.pointing).T
        ps_stop = ps_stop @ tel_to_local.T - self._tel.position

        # VTK renderer
        renderer = VTKOpticalSystem(self._tel, resolution=resolution)
        renderer.add_rays(ps_start, ps_stop, length=length, directions=vs_start, show_hits=show_hits, show_rays=show_rays)
