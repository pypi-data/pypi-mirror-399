...
class QBitwave:
    ...
    def wave_complexity(self, eps: float = 1e-3) -> int:
        """
        Compute the spectral complexity of the wavefunction.

        This method counts the number of significant amplitude components in
        the wavefunction, providing a measure of informational or algorithmic
        complexity. Low spectral complexity corresponds to smooth, regular,
        compressible wavefunctions, while high spectral complexity indicates
        irregular or noisy, incompressible structures.

        Conceptually:
            - Smooth, predictable paths have few significant amplitudes → low complexity.
            - Singular or highly oscillatory paths have many active amplitudes → high complexity.
            - This aligns with the idea that Euclidean path integral weights
              suppress high-frequency, incompressible geometries.
