...
class ROCmGPUDetector:
    ...
    def detect_via_pytorch() -> Dict[str, Any]:
        ...
        # Map GCN architecture to product names (for fallback when name is empty)
        gcn_to_product = {
        ...
        try:
            ...
            if torch.cuda.is_available():
                ...
                for i in range(torch.cuda.device_count()):
                    ...
                    # Get GCN architecture for compute capability and fallback name
                    arch_name = None
                    if hasattr(props, 'gcnArchName'):
                        arch_name = props.gcnArchName
                    
                    # Fallback name based on GCN architecture if PyTorch name is empty
                    if not name or name.strip() == "":
                        if arch_name:
                            name = gcn_to_product.get(arch_name, f"AMD GPU ({arch_name})")
                        else:
                            name = "AMD GPU"
                    ...
                    device_info = {
                        ...
                        "compute_capability": arch_name or "N/A",  # Use compute_capability for frontend compatibility
                        "arch": arch_name,  # Also keep arch for backward compatibility
                    }
    ...
    def detect() -> Dict[str, Any]:
        ...
        Merges info from rocm-smi and PyTorch for complete GPU information.
        """
        ...
        # Prefer PyTorch devices (includes memory info)
        # But merge in names from rocm-smi if PyTorch names are missing/generic
        gpus = torch_info["devices"] if torch_info["devices"] else smi_info["gpus"]
        
        # If we have both sources and PyTorch has generic names, try to get better names from SMI
        if torch_info["devices"] and smi_info["gpus"]:
            for i, gpu in enumerate(gpus):
                # Check if the GPU name is generic (missing or just "AMD GPU")
                if not gpu.get("name") or gpu["name"].startswith("AMD GPU"):
                    # Try to get a better name from rocm-smi
                    if i < len(smi_info["gpus"]) and smi_info["gpus"][i].get("name"):
                        smi_name = smi_info["gpus"][i]["name"]
                        if smi_name and not smi_name.startswith("AMD GPU"):
                            gpu["name"] = smi_name
