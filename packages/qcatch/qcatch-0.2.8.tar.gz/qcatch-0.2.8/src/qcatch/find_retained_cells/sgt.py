...
class SimpleGoodTuringError(Exception):
    """Exception raised for errors in Simple Good-Turing estimation."""

    pass


def _averaging_transform(r: np.ndarray, nr: np.ndarray) -> np.ndarray:
    """Compute the averaging transform for frequency counts."""
...
def _rstest(r: np.ndarray, coef: float) -> np.ndarray:
    """Apply the RSTest adjustment to frequencies."""
    return r * np.power(1 + 1 / r, 1 + coef)


def simple_good_turing(xr: np.ndarray, xnr: np.ndarray) -> tuple[np.ndarray, float]:
    ...
    if slope > -1:
        raise SimpleGoodTuringError(
            f"The log-log slope is > -1 ({slope}); the SGT estimator is not applicable to these data."
        )
...
def sgt_proportions(frequencies: np.ndarray) -> tuple[np.ndarray, float]:
    ...
    if len(use_freqs) < 10:
        raise SimpleGoodTuringError(f"Too few non-zero frequency items ({len(use_freqs)}). Aborting SGT.")
    ...
    rstar_dict = dict(zip(use_freqs, rstar, strict=False))
...
def test_prosody() -> None:
    """Unit test for Simple Good-Turing proportions on example data."""
    data = (
        (1, 120),
        (2, 40),
        (3, 24),
