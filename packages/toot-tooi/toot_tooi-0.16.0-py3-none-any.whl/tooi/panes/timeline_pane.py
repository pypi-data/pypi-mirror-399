...
from tooi.messages import EventHighlighted, GotoMessage, ShowImages, StatusReply
...
from tooi.widgets.event_detail_container import EventDetailContainer
...
class TimelinePane(TabPane):
    ...
    def event_detail(self):
        return self.query_one(EventDetailContainer)
    ...
    def action_show_thread(self):
        if status := self._current_status():
            self.post_message(GotoMessage(GotoContextTimeline(status.id)))
    ...
    def action_scroll_right(self):
        self.event_detail.query_one(EventDetail).focus()
