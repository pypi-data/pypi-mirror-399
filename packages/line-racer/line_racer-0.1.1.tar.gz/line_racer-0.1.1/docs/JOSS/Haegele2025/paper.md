...
To be directly compatible with one of the most used atmospheric modeling and retrieval codes, `line racer` can output the opacities petitRADTRANS [@Molliere2019] format in addition to a simple standard format.
...
To do that, the wavelength grid is divide into sub-grids. For every sub-grid, the lines with their center inside the sub-grid are calculated on the fine grid. 
...
By that, it still contributes as much as it is supposed to, but the time spend to calculate the line is minimized. 

The simplest way of the sampling works as follows: 
...
$$
However, with this weight, the line not normalized to the correct integrated area. The normalization afterward is very time-consuming. 
