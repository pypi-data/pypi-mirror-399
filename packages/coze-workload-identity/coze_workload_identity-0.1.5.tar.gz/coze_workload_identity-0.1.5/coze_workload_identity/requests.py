...
from .proxy import HttpsProxy, CaBundleContent, CaBundlePath
...
class ConfiguredSession(requests.Session):
    ...
    def _configure_session(self):
        ...
        # Configure CA certificate - prioritize CA content over file path
        # First try to get CA content directly
        ca_content = CaBundleContent()
        if ca_content:
            self.verify = ca_content
            self._configure_adapter()
            return

        # If CA content is not configured, try file path
        ...
        raise ValueError(
            "Neither CA certificate content nor CA bundle file path is configured. "
            "Please set either COZE_OUTBOUND_AUTH_PROXY_CA (for certificate content) "
            "or COZE_OUTBOUND_AUTH_PROXY_CA_PATH (for file path)."
        )
