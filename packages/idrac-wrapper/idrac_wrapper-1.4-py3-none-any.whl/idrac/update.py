...
def check_supported_idrac_version():
    if update_parameters["x"]:
        response = requests.get('https://%s/redfish/v1/UpdateService' % idrac_ip, verify=verify_cert, headers={'X-Auth-Token': update_parameters["x"]})
    else:
...
def get_idrac_version():
    ...
    if update_parameters["x"]:
        response = requests.get('https://%s/redfish/v1/Managers/iDRAC.Embedded.1?$select=FirmwareVersion' % idrac_ip, verify=verify_cert, headers={'X-Auth-Token': update_parameters["x"]})
    else:
...
def get_FW_inventory():
    ...
    if update_parameters["x"]:
        response = requests.get('https://%s/redfish/v1/UpdateService/FirmwareInventory?$expand=*($levels=1)' % idrac_ip, verify=verify_cert, headers={'X-Auth-Token': update_parameters["x"]})
    else:
...
def download_image_payload():
    ...
    logging.info("\n- INFO, downloading \"%s\" image, this may take a few minutes depending on the size of the payload" % update_parameters["image"])
    if update_parameters["x"]:
        response = requests.get('https://%s/redfish/v1/UpdateService' % idrac_ip, verify=verify_cert, headers={'X-Auth-Token': update_parameters["x"]})
    else:
        response = requests.get('https://%s/redfish/v1/UpdateService' % idrac_ip, verify=verify_cert, auth=(idrac_username, idrac_password))
    ...
    if update_parameters["x"]:
        response = requests.get('https://%s%s' % (idrac_ip, http_push_uri), verify=verify_cert, headers={'X-Auth-Token': update_parameters["x"]})
    else:
        response = requests.get('https://%s%s' % (idrac_ip, http_push_uri), verify=verify_cert, auth=(idrac_username, idrac_password))
    data = response.json()
    ImageLocation = update_parameters["location"]
    filename = update_parameters["image"]
    ...
    if update_parameters["x"]:
        headers = {'X-Auth-Token': update_parameters["x"], "if-match": ETag}
...
def install_image_payload():
    ...
    if update_parameters["x"]:
        headers = {'content-type': 'application/json', 'X-Auth-Token': update_parameters["x"]}
...
def check_job_status():
    ...
    while True:
        ...
        try:
            if update_parameters["x"]:
                response = requests.get('https://%s/redfish/v1/TaskService/Tasks/%s' % (idrac_ip, job_id), verify=verify_cert, headers={'X-Auth-Token': update_parameters["x"]})
            else:
        ...
        if str(current_time)[0:7] >= "0:30:00":
            ...
            if not update_parameters["reboot"]:
...
def loop_check_final_job_status():
    ...
    while True:
        ...
        try:
            if update_parameters["x"]:
                response = requests.get('https://%s/redfish/v1/Managers/iDRAC.Embedded.1/Jobs/%s' % (idrac_ip, job_id), verify=verify_cert, headers={'X-Auth-Token': update_parameters["x"]})
            else:
...
def reboot_server():
    if update_parameters["x"]:
        response = requests.get('https://%s/redfish/v1/Systems/System.Embedded.1' % idrac_ip, verify=verify_cert, headers={'X-Auth-Token': update_parameters["x"]})
    else:
...
if __name__ == "__main__":
    if update_parameters["script_examples"]:
        script_examples()
    if update_parameters["ip"] or update_parameters["ssl"] or update_parameters["u"] or update_parameters["p"] or update_parameters["x"]:
        idrac_ip = update_parameters["ip"]
        idrac_username = update_parameters["u"]
        if update_parameters["p"]:
            idrac_password = update_parameters["p"]
        if not update_parameters["p"] and not update_parameters["x"] and update_parameters["u"]:
            idrac_password = getpass.getpass("\n- Argument -p not detected, pass in iDRAC user %s password: " % update_parameters["u"])
        if update_parameters["ssl"]:
            if update_parameters["ssl"].lower() == "true":
                verify_cert = True
            elif update_parameters["ssl"].lower() == "false":
                verify_cert = False
            else:
                verify_cert = False
        else:
        ...
        sys.exit(0)
    if update_parameters["get"]:
        get_FW_inventory()
    elif update_parameters["location"] and update_parameters["image"]:
        get_idrac_version()
        ...
        if update_parameters["reboot"]:
            if int(idrac_fw_version[0]) >= 5:
                ...
                reboot_server()
                loop_check_final_job_status()
            if update_parameters["final_shutdown"]:
                shutdown_server()
    else:
        logging.error("\n- FAIL, invalid argument values or not all required parameters passed in. See help text or argument --script-examples for more details.")
