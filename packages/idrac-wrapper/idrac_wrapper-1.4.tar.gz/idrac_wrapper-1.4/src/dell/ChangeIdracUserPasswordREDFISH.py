...
def get_iDRAC_user_account_info():
    if args["x"]:
        ...
        q = 'https://%s/redfish/v1/Managers/iDRAC.Embedded.1/Accounts?$expand=*($levels=1)' % idrac_ip
        with open('/tmp/path','w') as f:/R
            print(q,file=f)
        response = requests.get(q, verify=verify_cert, auth=(idrac_username, idrac_password))
    data = response.json()
    if response.status_code != 200:
