"""Test start_free_threaded() approach."""
import threading
import time
...
# Start in free-threaded mode (non-blocking!)
print("Starting capture (free-threaded)...", flush=True)
capture.start_free_threaded()
print("Capture started (non-blocking)", flush=True)
...
for i in range(50):
    ...
    with frame_lock:
        if latest_frame:
            ...
            latest_frame.save("test_free_threaded.png")
            print("[MAIN] Saved to test_free_threaded.png", flush=True)
            break
