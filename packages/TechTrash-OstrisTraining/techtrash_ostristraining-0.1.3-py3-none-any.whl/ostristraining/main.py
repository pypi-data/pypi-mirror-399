...
class OstrisTraining:
    ...
    def _train_model(self, write_logs: bool = False) -> str:
        """
        ...
        # If enabled, we capture stdout+stderr into a deterministic log file so a serverless
        # wrapper (ex: RunPod handler) can tail it and stream to Supabase in real-time.
        output_log_file = f"{self.absolute_path_output}/{self.user_name}/log.txt"
        ...
        if write_logs:
            # Only needed when we write `.../{user_name}/log.txt`.
            Path(self.absolute_path_output, self.user_name).mkdir(parents=True, exist_ok=True)
        ...
        if write_logs:
            # IMPORTANT:
            # - We add "-u" to reduce buffering so logs appear in `log.txt` *during* training.
            # - This is critical for "tail -f" style streaming (Supabase/UI).
            command_launch_training.insert(1, "-u")

        logger.info("Launching training command: %s", command_launch_training)

        if not write_logs:
            # Original behavior: run training and let stdout/stderr go to the caller/container logs.
            ...
            # Stream stdout+stderr into `output_log_file` as the process runs.
            #
            # Why not `subprocess.run(..., capture_output=True)`?
            # - It buffers everything in memory and only returns at the end.
            # - We want incremental writes so an external system can follow progress.
            #
            # Note:
            # - We merge stderr into stdout (STDOUT) to keep log ordering consistent.
            # - We open the log file in text mode with line buffering.
            try:
                with open(output_log_file, "w", encoding="utf-8", errors="replace", buffering=1) as log_f:
                    log_f.write("================================================\n")
                    log_f.write("AI-Toolkit training log (captured by OstrisTraining)\n")
                    log_f.write(f"cwd: {self.absolute_path_ostris}\n")
                    log_f.write(f"command: {command_launch_training}\n")
                    log_f.write("================================================\n")

                    proc = subprocess.Popen(
                        command_launch_training,
                        cwd=self.absolute_path_ostris,
                    ...
                    for line in proc.stdout:
                        # Write exactly what the training prints.
                        # We keep it simple: one line in, one line out.
                        log_f.write(line)

                    return_code = proc.wait()
                    if return_code != 0:
                        # Match `subprocess.run(check=True)` behavior.
                        raise subprocess.CalledProcessError(return_code, command_launch_training)
            except Exception:
                # Keep the log file on disk for debugging if anything fails.
                logger.exception("Training process failed. See log file: %s", output_log_file)
                raise
    ...
    def train(self, url_zip_dataset: str, config_yaml_content: str, *, write_logs: bool = False) -> str:
        """
        ...
        trained_model_path = self._train_model(write_logs=write_logs)
