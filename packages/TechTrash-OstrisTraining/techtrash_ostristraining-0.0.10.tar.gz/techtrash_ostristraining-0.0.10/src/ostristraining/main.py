...
class OstrisTraining:
    def __init__(self, user_name: str, absolute_path_ostris: str, absolute_path_racine: str, gender: Literal["woman", "man"], absolute_path_output: str = "/app/output"):
    ...
    def _train_model(self) -> str:
        ...
        # Expected output path: keep your current convention.
        expected_model_path = Path(self.absolute_path_output) / f"{self.user_name}.safetensors"
        ...
        # We pick the most recently modified `.safetensors` in the output folder.
        candidates = list(Path(self.absolute_path_output).glob("*.safetensors"))
        if candidates:
