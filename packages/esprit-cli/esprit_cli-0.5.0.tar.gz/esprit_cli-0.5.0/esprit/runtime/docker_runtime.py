...
class DockerRuntime(AbstractRuntime):
    ...
    def _create_container_with_retry(self, scan_id: str, max_retries: int = 3) -> Container:
        ...
        for attempt in range(max_retries):
            try:
                self._verify_image_available(ESPRIT_IMAGE)
                ...
                container = self.client.containers.run(
                    ...
                    detach=True,
                    name=container_name,
                    hostname=f"esprit-scan-{scan_id}",
                    ports={
    ...
    def _get_or_create_scan_container(self, scan_id: str) -> Container:  # noqa: PLR0912
        ...
        try:
            ...
            if (
                "esprit-scan-id" not in container.labels
                or container.labels["esprit-scan-id"] != scan_id
            ):
                logger.warning(
    ...
    def _initialize_container(
        ...
        # Note: strix-sandbox container has strix code, not esprit
        container.exec_run(
