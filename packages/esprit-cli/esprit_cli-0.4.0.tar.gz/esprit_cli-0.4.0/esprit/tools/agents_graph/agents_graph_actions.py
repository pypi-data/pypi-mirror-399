...
def _run_agent_in_thread(
    ...
    <instructions>
        ...
        _agent_graph["nodes"][state.agent_id]["result"] = {"error": str(e), "success": False}
        _running_agents.pop(state.agent_id, None)
        _agent_instances.pop(state.agent_id, None)

        # Notify parent agent about the failure so it doesn't wait forever
        parent_id = state.parent_id
        if parent_id and parent_id in _agent_graph["nodes"]:
            agent_name = _agent_graph["nodes"][state.agent_id].get("name", "Unknown")
            error_message = f"""<agent_error_report>
    <agent_info>
    ...
    <error>
        <type>{type(e).__name__}</type>
        <message>{str(e)}</message>
    </error>
    <note>This sub-agent encountered an error and has terminated. You may need to retry the task or proceed without its results.</note>
</agent_error_report>"""
        ...
        # Don't re-raise - let the thread terminate gracefully
...
def create_agent(
    ...
    try:
        ...
        if module_list:
            from esprit.prompts import get_all_module_names, validate_module_names
        ...
        agent_config = {
            ...
            "state": state,
            # Sub-agents must ALWAYS be non_interactive since they can't receive
            # direct user input. This ensures they fail gracefully on LLM errors
            # (like rate limits) instead of hanging in a waiting state.
