...
Debug_SUBJECT = 'debug'
Debug_SUBJECT_PARAMS = []
Debug_SUBJECT_PARAMS_COUNT = 0


class DebugHandler:
    methods = {
        'info': ('Info', 0, xbus_dot_control__pb2.DebugInfoRequest, True, False, True),
    }
    ...
    def subject(self, method='>'):
        return '.'.join([
            "xbus.default.control", Debug_SUBJECT, method
        ])
    ...
    async def handler(self, msg):
        try:
            pkg_params, svc_params, mt_subject, tail = nrpc.parse_subject(
                PKG_SUBJECT, PKG_SUBJECT_PARAMS_COUNT,
                Debug_SUBJECT, Debug_SUBJECT_PARAMS_COUNT,
                msg.subject)
            ...
            import traceback; traceback.print_exc()
            print("Error in Debug.%s handler:" % mname, e)
...
class DebugClient:
    ...
    async def Info(
        ...
        subject = PKG_SUBJECT + '.' + Debug_SUBJECT + '.' + 'info'
        rawReq = req.SerializeToString()
