...
# =============================================================================
# Solves the cross-task cancel scope issue when using async generators with
# resources that use anyio cancel scopes (like MCP servers).
#
# Problem: MCP servers use exit_stack.enter_async_context() which creates
# cancel scopes. When an async generator yields, it can be resumed in a
# different task (e.g., FastAPI request handlers). Cleanup then runs in the
# wrong task, causing "Attempted to exit cancel scope in a different task".
#
# Solution: Run the producer (with MCP lifecycle) in a dedicated task that
# never yields externally. Events flow through an asyncio.Queue to the
# consumer generator, which only yields queue items.
# =============================================================================


class _StreamMessageType(Enum):
    """Type of message in the task-isolated stream."""
    EVENT = auto()
    DONE = auto()
    ERROR = auto()
...
class _StreamMessage:
    """Message passed through the task-isolated stream queue."""
    type: _StreamMessageType
    payload: Any = None
...
class OpenAIProvider:
        ...
        try:
            ...
            try:
                ...
                if mcp_configs:
                    ...
                    logger.debug(f"Creating {len(unique_mcp_configs)} unique MCP servers (from {len(mcp_configs)} total configs)")
                    ...
                    # Manually manage MCP server lifecycle for consistency with streaming pattern
                    # and to avoid potential cross-task cancel scope issues
                    connected_servers = []
                    try:
                        # Connect all MCP servers explicitly
                        ...
                        logger.debug(f"Successfully connected and assigned {len(connected_servers)} MCP servers")
                        ...
                        # Clean up MCP servers explicitly
                        for server in connected_servers:
                            try:
                                await server.cleanup()
                            except Exception as cleanup_error:
                                logger.warning(f"Error cleaning up MCP server {getattr(server, 'name', 'unknown')}: {cleanup_error}")
                else:
    ...
    async def run_agent_stream(self, agent_run: AgentRun) -> AsyncGenerator[StreamEvent, None]:
        """
        Execute agent with streaming using task-isolated MCP lifecycle.

        Uses a dedicated background task for MCP server lifecycle (connect/cleanup)
        to avoid cross-task cancel scope issues when the async generator is consumed
        across task boundaries (e.g., by FastAPI/Starlette).
        """
        # Strict validation
        if agent_run is None:
            raise OpenAIProviderError("agent_run parameter cannot be None")
        ...
        logger.debug(f"Starting streaming agent execution: agent_id={agent_run.agent.id}")
        self._ensure_initialized()
        ...
        try:
            ...
            openai_session = await session_manager.get_session()
            logger.info(f"Session ready for streaming agent {agent_run.agent.id}")
            ...
            if mcp_configs:
                # Use task-isolated streaming for MCP servers
                async for event in self._stream_with_mcp_isolation(
                    agent, agent_run, openai_session, mcp_configs
                ):
                    yield event
            else:
                # No MCP servers - direct streaming
                async for event in self._stream_without_mcp(
                    agent, agent_run, openai_session
                ):
                    yield event

            logger.info(f"Streaming completed for session {agent_run.metadata.session_id}")
            ...
            import traceback
            logger.error(f"Streaming failed for agent {agent_run.agent.id}: {e}\n{traceback.format_exc()}")
    ...
    async def _stream_with_mcp_isolation(
        self,
        agent,
        agent_run: AgentRun,
        openai_session,
        ...
        """
        Stream agent execution with MCP servers in a task-isolated context.

        MCP servers use anyio cancel scopes internally. These scopes must be
        entered and exited in the same task. By running the entire MCP lifecycle
        in a dedicated background task and communicating via queue, we ensure
        cleanup always happens in the same task as connect.
        """
        from agents import Runner

        # Prepare MCP servers
        unique_mcp_configs = self._deduplicate_mcp_configs(mcp_configs)
        logger.debug(f"Creating {len(unique_mcp_configs)} MCP servers with task isolation")
        ...
        # Queue for task-isolated event streaming
        event_queue: asyncio.Queue[_StreamMessage] = asyncio.Queue()

        async def _isolated_producer():
            """
            Runs in a dedicated task. MCP connect() and cleanup() happen here,
            ensuring cancel scopes are entered and exited in the same task.
            """
            connected_servers = []
            try:
                # Connect all MCP servers in THIS task
                for server in mcp_servers:
                    await server.connect()
                    connected_servers.append(server)
                    logger.debug(f"Connected MCP server: {getattr(server, 'name', 'unknown')}")

                # Assign servers to agents
                await self._assign_mcp_servers_to_agents(
                    agent, agent_run, connected_servers, unique_mcp_configs
                )

                # Run agent and stream events to queue
                ...
                async for event in result.stream_events():
                    await event_queue.put(_StreamMessage(_StreamMessageType.EVENT, event))

                await event_queue.put(_StreamMessage(_StreamMessageType.DONE))

            except Exception as e:
                logger.error(f"Error in isolated MCP producer: {e}")
                await event_queue.put(_StreamMessage(_StreamMessageType.ERROR, e))

            finally:
                # Cleanup in THIS task - same task that called connect()
                for server in connected_servers:
                    try:
                        await server.cleanup()
                        logger.debug(f"Cleaned up MCP server: {getattr(server, 'name', 'unknown')}")
                    except Exception as cleanup_error:
                        # Log but don't raise - cleanup errors shouldn't mask original errors
                        logger.warning(f"MCP cleanup error: {cleanup_error}")

        # Start the isolated producer task
        producer_task = asyncio.create_task(_isolated_producer())

        try:
            # Consume events from queue and yield to caller
            # This generator only yields from queue, never from inside MCP context
            while True:
                msg = await event_queue.get()

                if msg.type == _StreamMessageType.DONE:
                    break
                elif msg.type == _StreamMessageType.ERROR:
                    raise msg.payload
                else:
                    # Transform and yield the event
                    async for stream_event in self._handle_openai_event(msg.payload):
                        yield stream_event

        finally:
            # Ensure producer task completes (for cleanup)
            if not producer_task.done():
                # Wait for producer to finish cleanup
                try:
                    await asyncio.wait_for(producer_task, timeout=10.0)
                except asyncio.TimeoutError:
                    logger.warning("Producer task cleanup timed out, cancelling")
    ...
    async def _stream_without_mcp(
        ...
        """Stream agent execution without MCP servers (no isolation needed)."""
        from agents import Runner

        logger.debug(f"Starting streaming without MCP for session {agent_run.metadata.session_id}")
        result = Runner.run_streamed(
            agent,
            input=agent_run.message,
            session=openai_session,
