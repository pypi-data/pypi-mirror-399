...
class ReplayBuffer:  # for off-policy
    def __init__(self,
        ...
        """
        assert (action_dim < 256) or (
            not if_discrete)  # if_discrete==True, then action_dim < 256
        self.states = th.empty(
            (max_size, num_seqs, state_dim), dtype=th.float32, device=self.device)
        ...
        self.rewards = th.empty((max_size, num_seqs),
                                dtype=th.float32, device=self.device)
        self.undones = th.empty((max_size, num_seqs),
                                dtype=th.float32, device=self.device)
        self.unmasks = th.empty((max_size, num_seqs),
                                dtype=th.float32, device=self.device)
        ...
        if if_use_per:
            self.sum_trees = [SumTree(buf_len=max_size)
    ...
    def sample(self, batch_size: int) -> Tuple[TEN, TEN, TEN, TEN, TEN, TEN]:
        ...
        ids = th.randint(sample_len * self.num_seqs, size=(batch_size,),
                         requires_grad=False, device=self.device)
        self.ids0 = ids0 = th.fmod(ids, sample_len)  # ids % sample_len
        self.ids1 = ids1 = th.div(
            ids, sample_len, rounding_mode='floor')  # ids // sample_len
    ...
    def sample_for_per(self, batch_size: int) -> Tuple[TEN, TEN, TEN, TEN, TEN, TEN, TEN, TEN]:
        beg = -self.max_size
        end = (self.cur_size - self.max_size) if (self.cur_size <
                                                  self.max_size) else -1
        ...
        for env_i in range(self.num_seqs):
            ...
            _is_indices, _is_weights = sum_tree.important_sampling(
                batch_size, beg, end, self.per_beta)
        ...
        # is_indices % sample_len
        self.ids0 = ids0 = th.fmod(is_indices, self.cur_size)
        # is_indices // sample_len
        self.ids1 = ids1 = th.div(
            is_indices, self.cur_size, rounding_mode='floor')
        return (
            self.states[ids0, ids1],
            self.actions[ids0, ids1],
            self.rewards[ids0, ids1],
    ...
    def td_error_update_for_per(self, is_indices: TEN, td_error: TEN):
        ...
        for env_i in range(self.num_seqs):
            ...
            sum_tree.update_ids(
                is_indices[slice_i:slice_j].cpu(), prob[slice_i:slice_j].cpu())
    ...
    def save_or_load_history(self, cwd: str, if_save: bool):
        ...
        if if_save:
            for item, name in item_names:
                ...
                print(
                    f"| buffer.save_or_load_history(): Save {file_path}", flush=True)
            ...
            for item, name in item_names:
                file_path = f"{cwd}/replay_buffer_{name}.pth"
                print(
                    f"| buffer.save_or_load_history(): Load {file_path}", flush=True)
...
class SumTree:
    ...
    def important_sampling(self, batch_size: int, beg: int, end: int, per_beta: float) -> Tuple[TEN, TEN]:
        ...
        values = (th.arange(batch_size) + th.rand(batch_size)) * \
            (self.tree[0] / batch_size)

        # get proportional prioritization
        leaf_ids, leaf_values = list(
            zip(*[self.get_leaf_id_and_value(v) for v in values]))
