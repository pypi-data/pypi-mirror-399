...
class TestCLIIntegration:
    ...
    @pytest.fixture
    def server_supports_http(self, server_available):
        """Check if server supports HTTP (not just mTLS)."""
        ...
        try:
            with httpx.Client(timeout=2.0) as client:
                response = client.post(
                    "http://localhost:8009/api/jsonrpc",
                    json={"jsonrpc": "2.0", "method": "health", "id": 1},
                )
                return response.status_code in (200, 400, 500)  # Any response means HTTP works
        except Exception:
            return False
    ...
    async def test_cli_health_http(self, server_available, server_supports_http):
        """Test CLI health command over HTTP."""
        if not server_available:
            pytest.skip("Server not available on localhost:8009")
        if not server_supports_http:
            pytest.skip("Server does not support HTTP (mTLS only)")
    ...
    async def test_cli_chunk_http(self, server_available, server_supports_http):
    ...
    async def test_cli_with_config_file(self, server_available, mtls_certs):
        """Test CLI with configuration file (uses mTLS)."""
        if not server_available:
            pytest.skip("Server not available on localhost:8009")
    ...
    async def test_cli_env_vars(self, server_available, server_supports_http, monkeypatch):
    ...
    async def test_cli_submit_and_status(self, server_available, mtls_certs):
        """Test submit and status commands."""
        if not server_available:
            pytest.skip("Server not available on localhost:8009")

        # Submit job (using mTLS since server requires it)
...
class TestClientIntegration:
    ...
    async def test_client_http(self, server_available, server_supports_http):
    ...
    async def test_client_chunk_http(self, server_available, server_supports_http):
    ...
    async def test_client_with_config_priority(self, server_available, server_supports_http, monkeypatch):
    ...
    async def test_client_all_protocols(
        self, server_available, server_supports_http, mtls_certs
    ):
