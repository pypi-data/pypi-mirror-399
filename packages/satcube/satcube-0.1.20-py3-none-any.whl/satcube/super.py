...
# Ensure sen2sr is installed
try:
    import sen2sr
except ImportError:
    raise ImportError(
        "Please install sen2sr: pip install git+https://github.com/tacofoundation/sen2sr.git mlstac"
    )
...
def fix_lastchunk(iterchunks, s2dim, chunk_size):
    ...
    # Remove duplicates preserving order (critical for determinism)
    return sorted(list(set(itercontainer)))
...
def predict_optimized(
    ...
    res_n = scale
    patch_size_out = INPUT_CHUNK_SIZE * res_n
    ...
    **Hardware Recommendations:**
    ...
    for _, row in tqdm(
        ...
        try:
            ...
            logger.error(f"Failed to super-resolve {row['id']}: {exc}")
