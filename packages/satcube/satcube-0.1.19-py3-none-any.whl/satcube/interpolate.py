...
def interpolate_fn(
    ...
    **When to adjust despike_threshold:**
    ...
    try:
        file_dates = pd.to_datetime([f.stem for f in all_raw_files])
    except ValueError:
        file_dates = pd.to_datetime(metadata["date"])
    ...
    with ThreadPoolExecutor(max_workers=num_workers) as executor:
        ...
        for future in tqdm(
            ...
            try:
                ...
                logger.error(f"Failed to process chunk ({r_start}, {c_start}): {exc}")
