...
class Assessments:
    ...
    async def create_atomic_assessment(
        ...
        2. POST /v2/assessments/{id}/update_defaults - Assign test points and asset tags
        ...
        try:
            ...
            # Step 2: Assign test points (assets) and asset tags using v2 endpoint
            logger.info(f"Assigning {len(asset_ids)} test points to assessment {assessment_id}")
            update_payload = {
                "selected_asset_ids": asset_ids,
                "deselected_asset_ids": [],
                "selected_tag_ids": asset_tag_ids or [],
                "deselected_tag_ids": [],
            }
            defaults_result = await client.post_object(
                f"v2/assessments/{assessment_id}/update_defaults",
                data=update_payload,
            )
