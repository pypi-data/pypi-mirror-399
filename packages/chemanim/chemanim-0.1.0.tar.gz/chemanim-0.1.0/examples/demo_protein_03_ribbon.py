"""ManimGL Demo: Protein Ribbon (Flat Backbone)
"""
...
class ProteinRibbonDemo(Scene):
    def construct(self):
        ...
        # Title
        title = Text("Protein Ribbon", font_size=36).to_edge(UP)
        title.fix_in_frame()
        self.add(title)
        ...
        # Load
        print("Loading Protein Ribbon...")
        mol = Protein(pdb_path, render_style="ribbon", include_hydrogens=False)
        mol.scale(1.5)
        mol.move_to(ORIGIN)
        self.add(mol)
