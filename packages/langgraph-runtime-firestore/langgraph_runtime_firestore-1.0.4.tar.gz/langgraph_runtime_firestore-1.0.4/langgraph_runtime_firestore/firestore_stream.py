...
class StreamManager:
    ...
    def get_queues(
        ...
        logger.info(f"Getting queues from StreamManager")
    ...
    def get_control_queues(
        ...
        logger.info(f"Getting control queues from StreamManager")
        run_id = _ensure_uuid(run_id)
        if thread_id is None:
            thread_id = THREADLESS_KEY
        else:
    ...
    def get_control_key(
        ...
        logger.info(f"Getting control key from StreamManager")
        run_id = _ensure_uuid(run_id)
        if thread_id is None:
            thread_id = THREADLESS_KEY
        else:
    ...
    async def put(
        ...
        logger.info(
            f"Message put: resumable={resumable}, topic={topic}, "
            f"run_id={run_id}, msg_id={message.id.decode()} msg_data={message.data} msg_topc={message.topic}"
        )
        if resumable:
    ...
    async def put_thread(
        ...
        thread_id = _ensure_uuid(thread_id)
        logger.info(f"Putting thread message into StreamManager")
        message.id = _generate_ms_seq_id().encode()
    ...
    async def add_queue(
        ...
        logger.info(f"Adding queue to StreamManager")
        run_id = _ensure_uuid(run_id)
        queue = ContextQueue()
        if thread_id is None:
            thread_id = THREADLESS_KEY
    ...
    async def add_control_queue(
        ...
        logger.info(f"Adding control queue to StreamManager")
        run_id = _ensure_uuid(run_id)
        if thread_id is None:
            thread_id = THREADLESS_KEY
        else:
    ...
    async def add_thread_stream(self, thread_id: UUID | str) -> asyncio.Queue:
        logger.info(f"Adding thread stream to StreamManager")
        thread_id = _ensure_uuid(thread_id)
        queue = ContextQueue()
    ...
    async def remove_queue(
        ...
        logger.info(f"Removing queue from StreamManager")
        run_id = _ensure_uuid(run_id)
        if thread_id is None:
            thread_id = THREADLESS_KEY
        else:
    ...
    async def remove_control_queue(
        ...
        logger.info(f"Removing control queue from StreamManager")
        run_id = _ensure_uuid(run_id)
        if thread_id is None:
            thread_id = THREADLESS_KEY
        else:
    ...
    def restore_messages(
        ...
        logger.info(f"Restoring messages from StreamManager")
        run_id = _ensure_uuid(run_id)
        if thread_id is None:
            thread_id = THREADLESS_KEY
        else:
    ...
    def get_queues_by_thread_id(self, thread_id: UUID | str) -> list[asyncio.Queue]:
        ...
        logger.info(f"Getting all queues by thread_id from StreamManager")
        all_queues = []
