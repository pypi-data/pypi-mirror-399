...
@dataclass_json
@dataclass
class V4VpnIpsecVpnConnectionUpdateRequest(CtyunOpenAPIRequest):
    regionID: str  # 资源池ID
    vpnConnectionName: str  # VPN连接名称
    srcSubnet: List[str]  # 本端子网
    dstCidr: List[str]  # 对端网络
    psk: str  # 预共享密钥
    vpnConnectionID: str  # VPN连接ID
    ...
    ipsecLifeTime: int  # IPsec策略:IPsec生命周期
    ...
    triggerMode: Optional[bool] = None  # 协商生效模式，默认值为：false
...
@dataclass_json
@dataclass
class V4VpnIpsecVpnConnectionUpdateResponse(CtyunOpenAPIResponse):
