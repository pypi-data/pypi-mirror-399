    ...
    async def test_medium_list_expansion(self, test_config: Config):
        ...
        try:
            async with Connection(test_config.connection_string) as conn:
                ...
                result = await conn.query(query, medium_list)
    ...
    async def test_near_limit_expansion(self, test_config: Config):
        ...
        try:
            async with Connection(test_config.connection_string) as conn:
                ...
                placeholders = ", ".join([f"@P{i}" for i in range(1, 2001)])
                query = f"SELECT 1 WHERE 1=1"  # Just test parameter parsing
                
                # This should succeed because 2000 < 2100
                try:
                    result = await conn.query("SELECT COUNT(*) as cnt WHERE 1=0")
...
class TestParameterExpansionEdgeCases:
    ...
    @pytest.mark.integration
    @pytest.mark.asyncio
    async def test_parameter_expansion_limits_are_sql_server_spec(self, test_config: Config):
    ...
    async def test_expansion_limit_prevents_huge_list_dos(self, test_config: Config):
        ...
        try:
            async with Connection(test_config.connection_string) as conn:
                ...
                with pytest.raises(ValueError) as exc_info:
                    await conn.query(
                        "SELECT 1 WHERE 1 IN (" + ", ".join([f"@P{i}" for i in range(1, 5001)]) + ")",
                        huge_list
                    )
                
                error_msg = str(exc_info.value).lower()
                # Should contain error about too many parameters
                assert "too many parameters" in error_msg or "exceed" in error_msg or "2100" in error_msg
        except Exception as e:
    ...
    async def test_expansion_rejects_at_exact_limit_boundary(self, test_config: Config):
        ...
        try:
            async with Connection(test_config.connection_string) as conn:
                ...
                error_msg = str(exc_info.value).lower()
                assert "too many parameters" in error_msg or "exceed" in error_msg or "2100" in error_msg or "2101" in error_msg
        except Exception as e:
            if "database not available" not in str(e).lower():
                pytest.skip(f"Database not available: {e}")
