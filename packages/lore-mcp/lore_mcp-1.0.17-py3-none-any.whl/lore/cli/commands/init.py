...
- Context preservation: `save context`, `record decision`, `capture intent`, `document reasoning`
- Code archaeology: `why was this`, `context for`, `reasoning behind`, `what led to`
- History search: `find decision`, `search context`, `past reasoning`

### Flags (Explicit activation)
- `--lore` - Enable Lore functionality
- `--context` - Context preservation mode

### Situations (Context-based)
...
### lore_commit
**Keywords**: `save context`, `record decision`, `capture intent`, `document reasoning`
**Use when**: Feature implementation complete, architectural decisions made

### lore_blame
**Keywords**: `why was this`, `context for`, `reasoning behind`, `what led to`
**Use when**: Understanding code change reasons, tracing decisions

### lore_search
**Keywords**: `find decision`, `search context`, `past reasoning`, `previous discussion`
**Use when**: Searching past decisions, exploring context history

### lore_status
**Keywords**: `lore status`, `check lore`, `context status`
**Use when**: Checking connection, viewing team/project info

### lore_init
**Keywords**: `setup lore`, `initialize lore`, `enable context tracking`
**Use when**: Setting up new project for context capture
...
## Examples
```
# Trigger: "save context for this authentication implementation"
...
# Trigger: "why was this code written this way?"
lore_blame(file_path="src/auth/jwt.py", line_number=42)

# Trigger: "find previous decisions about authentication"
lore_search(query="authentication")

# Trigger: "check lore status"
lore_status()
```
