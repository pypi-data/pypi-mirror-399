...
class ELDBUtils:
    ...
    # ----------------- Data Import/Export -----------------
    @staticmethod
    def export_to_csv(table_path: str, output_csv: str) -> bool:
        """
            ...
            for row in rows:
                ...
                for col_name, value in row.items():
                    if col_name in db_instance.tables[table_name]['header']['columns']:
                        ...
                        if col_type == 'int':
                            processed_row[col_name] = int(value) if value else 0
                        else:
...
def export_table_to_csv(db_instance, table_name: str, output_csv: str) -> bool:
...
def get_table_stats(db_instance, table_name: str) -> Dict:
    ...
    return ELDBUtils.get_database_stats(db_instance.db_folder)['tables'].get(table_name, {})
