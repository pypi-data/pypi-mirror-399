...
class ELDB:
    ...
    def create_table(self, name: str, columns: Dict, 
        ...
        if auto_timestamps:
            columns['created_at'] = {'type': 'timestamp', 'size': 8, 'nullable': False, 'auto': True}
            columns['updated_at'] = {'type': 'timestamp', 'size': 8, 'nullable': False, 'auto': True}
            ...
            while True:
                ...
                for col, opts in columns.items():
                    ...
                    if typ == 'int':
                        ...
                        row[col] = struct.unpack('q', data[offset:offset+8])[0]
                        offset += 8
                    else:  # 'str'
                ...
                for col, opts in columns.items():
                    ...
                    if opts.get('auto', False) and update_timestamps:
                        if col == 'created_at' and val is None:
                            # Only set created_at if it's None (new row)
                            val = current_time
                        elif col == 'updated_at':
                ...
                for row in rows:
                    for col, val in row.items():
                        ...
                        if col_def.get('type') == 'timestamp' and val is not None:
                            row[col] = self._format_timestamp(val)
