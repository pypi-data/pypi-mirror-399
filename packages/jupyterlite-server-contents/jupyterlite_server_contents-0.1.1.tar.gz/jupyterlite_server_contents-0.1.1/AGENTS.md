...
```bash
python -m py_compile jupyterlite_server_contents/__init__.py  # Check single file for syntax errors
```
...
```typescript
const PLUGIN_ID = 'jupyterlite_server_contents:plugin';
```
...
export namespace CommandIDs {
  export const openPanel = 'jupyterlite_server_contents:open-panel';
  export const refreshData = 'jupyterlite_server_contents:refresh-data';
}
...
const plugin: JupyterFrontEndPlugin<void> = {
  id: 'jupyterlite_server_contents:plugin',
  autoStart: true,
...
- **Output**: Creates files in `lib/` and `jupyterlite_server_contents/labextension/`
...
- Ask user to search for the extension ID (`jupyterlite_server_contents`) to see if it loaded
...
ls -la jupyterlite_server_contents/labextension/  # Should contain bundled extension
```
...
```css
.jp-jupyterlite-server-contents-widget {
  padding: 8px;
}
```
...
- **Plugin ID** (in `src/index.ts`): `'jupyterlite_server_contents:plugin'`
- **Command IDs** (in `src/commands.ts` or `src/index.ts`): `'jupyterlite_server_contents:command-name'`
...
- **CSS classes** (in `style/index.css`): `.jp-jupyterlite-server-contents-ClassName`
