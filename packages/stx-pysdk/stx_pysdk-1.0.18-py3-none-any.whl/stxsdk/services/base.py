...
class StxClient:
    ...
    def __init__(self, env: Optional[str] = None, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        # Try to build the schema with validation first
        # If it fails due to duplicate enum values, build without validation
        try:
            ...
            # Check if this is the duplicate enum value error
            if "can only be defined once" in str(e):
                warnings.warn(
                    f"GraphQL schema validation warning: {str(e)}. "
                    "The SDK will continue with relaxed validation.",
                    UserWarning,
                    stacklevel=2
                )
                # Build schema without SDL validation by parsing and building the AST manually
