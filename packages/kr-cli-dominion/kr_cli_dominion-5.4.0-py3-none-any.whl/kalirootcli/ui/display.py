...
def print_ai_response(response: str, mode: str = "CONSULTATION", command: str = None) -> None:
    ...
    # CRITICAL FIX: Remove literal \1, \2, etc. that Groq generates
    # The LLM sometimes uses these as placeholders thinking they're regex backreferences
    response = re.sub(r'\\\d+', '[valor]', response)  # Replace \1, \2, etc
    ...
    def highlight_keywords(text):
        ...
        text = re.sub(r'`([^`]+)`', r'[bold ' + STYLE_BLUE + r']\1[/bold ' + STYLE_BLUE + ']', text)
        
        # 2. Bold markers **text** → Bold White
        text = re.sub(r'\*\*([^*]+)\*\*', r'[bold ' + STYLE_TEXT + r']\1[/bold ' + STYLE_TEXT + ']', text)
        
        # 3. Technical keywords → Cyan
        keywords = ["apache", "nginx", "openssh", "nmap", "curl", "ubuntu", "linux", "kali", "tcp", "udp", "http", "https", "ssl", "tls"]
        for kw in keywords:
            pattern = re.compile(r'\b(' + re.escape(kw) + r')\b', re.IGNORECASE)
            text = pattern.sub(r"[" + STYLE_CYAN + r"]\1[/" + STYLE_CYAN + "]", text)
