...
# v037: Train a Llama-style model using the new Length-MAX vocab trained with v036:
...
LEN_TOK_DIR="/home/arxiv_code/tokenizers_rust/tokenizer_out_wikitext103_32k_n9_full_maxchars48_nopunct_noincomplete_v036"
...
TAG="lenmax_nopunct_noincomplete_pack${PACK_CHARS}_evalval_steps${STEPS}_v037"
...
wait_for_file "$LEN_TOK_DIR/vocab.json" "Length-MAX HF vocab.json (v036 nopunct+noincomplete)"
...
# 3) train Length-MAX model (v036 vocab)
run_model "$LEN_TOK_DIR" "$LEN_MODEL_LOG" "$LEN_SAVE_DIR" "$STEPS" "Length-MAX(v036 nopunct+noincomplete vocab)"
...
python3 /home/arxiv_code/tokenizers_rust/plot_loss_overlay.py \
  --csv "$LEN_CURVE" --label "Length-MAX (v036 nopunct+noincomplete vocab)" \
  --csv "$BPE_CURVE" --label "BPE (baseline)" \
  --out_png "$LOSS_OVERLAY_PNG" \
  --out_pdf "$LOSS_OVERLAY_PDF" \
  --title "Loss vs step (lenmax v036 nopunct+noincomplete; eval=validation; pack=${PACK_CHARS})"
...
python3 /home/arxiv_code/tokenizers_rust/plot_loss_overlay.py \
  ...
  --title "Train bpc proxy vs step (lenmax v036; quality uses eval_loss->bpc)"
