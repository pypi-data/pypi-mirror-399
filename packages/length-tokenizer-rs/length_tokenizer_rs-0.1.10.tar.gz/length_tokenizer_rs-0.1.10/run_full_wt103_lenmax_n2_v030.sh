...
# v030: Try a more LM-friendly Length-MAX vocabulary:
# - Train Length-MAX tokenizer with ONLY bigram merges (n_min=n_max=2),
#   i.e. classic BPE-style merge building, but keep Length-MAX DP decoding.
# Hypothesis: more compositional tokens => lower eval_loss, while DP still gives good TPC,
# potentially improving bpc vs the n=2..9 Length-MAX baseline.
...
LEN_TOK_DIR="/home/arxiv_code/tokenizers_rust/tokenizer_out_wikitext103_32k_n2_full_v030"
...
TAG="lenmax_n2_full_tokfixed_pack${PACK_CHARS}_evalval_steps${STEPS}_v030"

# Logs / outputs
PIPE_LOG="/home/arxiv_code/tokenizers_rust/pipeline_${TAG}.log"
LEN_TOK_LOG="/home/arxiv_code/tokenizers_rust/train_wikitext103_32k_n2_full_v030.log"

LEN_MODEL_LOG="/home/arxiv_code/tokenizers_rust/run_llama_lenmax_n2_${TAG}.log"
BPE_MODEL_LOG="/home/arxiv_code/tokenizers_rust/run_llama_bpe_${TAG}.log"

LEN_SAVE_DIR="/home/arxiv_code/tokenizers_rust/model_lenmax_n2_${TAG}"
BPE_SAVE_DIR="/home/arxiv_code/tokenizers_rust/model_bpe_${TAG}"

LEN_CURVE="/home/arxiv_code/tokenizers_rust/loss_curve_lenmax_n2_${TAG}.csv"
BPE_CURVE="/home/arxiv_code/tokenizers_rust/loss_curve_bpe_${TAG}.csv"

LOSS_OVERLAY_PNG="/home/arxiv_code/tokenizers_rust/loss_overlay_lenmax_n2_vs_bpe_${TAG}.png"
LOSS_OVERLAY_PDF="/home/arxiv_code/tokenizers_rust/loss_overlay_lenmax_n2_vs_bpe_${TAG}.pdf"
BPC_OVERLAY_PNG="/home/arxiv_code/tokenizers_rust/bpc_overlay_lenmax_n2_vs_bpe_${TAG}.png"
BPC_OVERLAY_PDF="/home/arxiv_code/tokenizers_rust/bpc_overlay_lenmax_n2_vs_bpe_${TAG}.pdf"
...
train_len_tokenizer() {
  ...
  echo "[pipeline] start Length-MAX tokenizer (n=2 only) -> $LEN_TOK_LOG" | tee -a "$PIPE_LOG"
  mkdir -p "$LEN_TOK_DIR"
...
# 1) train Length-MAX tokenizer (n=2 only)
train_len_tokenizer
...
# 4) train Length-MAX model (n=2 only)
run_model "$LEN_TOK_DIR" "$LEN_MODEL_LOG" "$LEN_SAVE_DIR" "$STEPS" "Length-MAX(n=2 only)"
...
python3 /home/arxiv_code/tokenizers_rust/plot_loss_overlay.py \
  --csv "$LEN_CURVE" --label "Length-MAX (n=2 only)" \
  --csv "$BPE_CURVE" --label "BPE (baseline)" \
  --out_png "$LOSS_OVERLAY_PNG" \
  --out_pdf "$LOSS_OVERLAY_PDF" \
  --title "Loss vs step (lenmax n=2 only; eval=validation; pack=${PACK_CHARS})"
...
python3 /home/arxiv_code/tokenizers_rust/plot_loss_overlay.py \
  ...
  --title "Train bpc proxy vs step (lenmax n=2 only; quality uses eval_loss->bpc)"
