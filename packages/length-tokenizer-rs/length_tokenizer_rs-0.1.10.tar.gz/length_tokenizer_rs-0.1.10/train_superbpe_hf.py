#!/usr/bin/env python3
"""
训练一个“SuperBPE”风格的 ByteLevel BPE tokenizer，并导出 HF 可加载目录。

与标准 GPT-2 ByteLevel BPE 的关键差异：
- ByteLevel pre-tokenizer 设为 use_regex=False
  => 不做 GPT-2 的 regex 分词切分，从而允许 BPE merge 跨越空格（即形成 multi-word / superword token）。

我们保持与 LengthTokenizer 一致的 special tokens：
  <unk>=0, <pad>=1, <s>=2, </s>=3, <mask>=4   （通过 special_tokens 顺序保证）
"""
...
def main() -> None:
    ...
    ap.add_argument(
        "--add_prefix_space",
        action="store_true",
        help="与 GPT-2 行为更一致（默认关闭以对齐我们现有 bpe_v019 配置）",
    )
    ap.add_argument(
        "--use_regex",
        action="store_true",
        help="若打开则退化为标准 ByteLevel BPE（不推荐；SuperBPE 核心是 use_regex=False）",
    )
    args = ap.parse_args()
    ...
    # Save both the tokenizer.json and vocab/merges for transparency.
    ...
    print("bytelevel   =", f"add_prefix_space={bool(args.add_prefix_space)} use_regex={bool(args.use_regex)}")
