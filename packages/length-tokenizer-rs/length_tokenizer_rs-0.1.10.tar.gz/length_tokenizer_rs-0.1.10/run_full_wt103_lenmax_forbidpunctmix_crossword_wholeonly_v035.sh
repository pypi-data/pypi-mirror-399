...
# v035: Evaluate whether combining:
# - v033 Length-MAX vocab (trained with forbid_punct_word_mix)
# - decode constraint: LENGTH_TOKENIZER_CROSS_WORD_WHOLE_ONLY=1
# improves bpc vs v034 (default decode).
...
TAG="lenmax_forbidpunctmix_crossword_wholeonly_pack${PACK_CHARS}_evalval_steps${STEPS}_v035"
...
wait_for_file "$LEN_TOK_DIR/vocab.json" "Length-MAX HF vocab.json (v033 forbidpunctmix)"
...
run_model "$LEN_TOK_DIR" "$LEN_MODEL_LOG" "$LEN_SAVE_DIR" "$STEPS" "Length-MAX(v033 forbidpunctmix + cross_word_whole_only)"
...
python3 /home/arxiv_code/tokenizers_rust/plot_loss_overlay.py \
  --csv "$LEN_CURVE" --label "Length-MAX (v033 + cross_word_whole_only)" \
  --csv "$BPE_CURVE" --label "BPE (baseline)" \
  --out_png "$LOSS_OVERLAY_PNG" \
  --out_pdf "$LOSS_OVERLAY_PDF" \
  --title "Loss vs step (lenmax v033 forbidpunctmix; cross_word_whole_only; eval=validation; pack=${PACK_CHARS})"
...
python3 /home/arxiv_code/tokenizers_rust/plot_loss_overlay.py \
  ...
  --title "Train bpc proxy vs step (lenmax v033; cross_word_whole_only; quality uses eval_loss->bpc)"
