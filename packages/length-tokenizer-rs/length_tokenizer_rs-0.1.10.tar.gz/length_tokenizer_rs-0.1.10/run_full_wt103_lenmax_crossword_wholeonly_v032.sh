...
# v032: bpc-oriented decoding rule:
# Allow cross-word tokens ONLY if they cover whole words.
#
# Implementation: LENGTH_TOKENIZER_CROSS_WORD_WHOLE_ONLY=1
# - Disallow tokens that span a word boundary but do NOT end with END_TOKEN (incomplete last word)
# - Disallow tokens that start with END_TOKEN (token must start with a word)
# - For allowed cross-word tokens, require they are only used at word-start positions in DP
#
# We reuse an existing Length-MAX vocab and train a new model under this decode rule.

export LENGTH_TOKENIZER_CROSS_WORD_WHOLE_ONLY=1
...
# Reuse best-performing Length-MAX vocab from earlier runs
LEN_TOK_DIR="/home/arxiv_code/tokenizers_rust/tokenizer_out_wikitext103_32k_n9_full_maxchars48_v028"
...
TAG="lenmax_crossword_wholeonly_pack${PACK_CHARS}_evalval_steps${STEPS}_v032"
...
echo "[pipeline] env LENGTH_TOKENIZER_CROSS_WORD_WHOLE_ONLY=1" | tee -a "$PIPE_LOG"
...
wait_for_file "$LEN_TOK_DIR/vocab.json" "Length-MAX HF vocab.json (reused)"

# 1) token totals (exact full-corpus TPC under cross_word_whole_only)
echo "[pipeline] count full-corpus token totals (cross_word_whole_only)" | tee -a "$PIPE_LOG"
...
# 3) train Length-MAX model under cross_word_whole_only decode
run_model "$LEN_TOK_DIR" "$LEN_MODEL_LOG" "$LEN_SAVE_DIR" "$STEPS" "Length-MAX(cross_word_whole_only)"
...
python3 /home/arxiv_code/tokenizers_rust/plot_loss_overlay.py \
  --csv "$LEN_CURVE" --label "Length-MAX (cross_word_whole_only)" \
  --csv "$BPE_CURVE" --label "BPE (baseline)" \
  --out_png "$LOSS_OVERLAY_PNG" \
  --out_pdf "$LOSS_OVERLAY_PDF" \
  --title "Loss vs step (lenmax cross_word_whole_only; eval=validation; pack=${PACK_CHARS})"
...
python3 /home/arxiv_code/tokenizers_rust/plot_loss_overlay.py \
  ...
  --title "Train bpc proxy vs step (lenmax cross_word_whole_only; quality uses eval_loss->bpc)"
...
echo "${LEN_TOK_DIR},cross_word_whole_only,${LEN_TPC},${BPE_TPC},${BPE_BEST_LOSS},${LEN_BEST_LOSS},${BPE_BPC},${LEN_BPC},${LEN_MODEL_LOG},${BPE_BASE_LOG}" >> "$SUMMARY_CSV"
