...
# v042: Train a Llama-style model using the new Length-MAX vocab v041 (SuperBPE-style staged superwords):
# - v041 vocab training: punct whitelist + forbid_incomplete_cross_word + cross_word_start_vocab + max_token_words
#
...
LEN_TOK_DIR="/home/arxiv_code/tokenizers_rust/tokenizer_out_wikitext103_32k_n9_full_maxchars48_punctwhitelist_stage30000_maxw3_v041"
...
TAG="lenmax_punctwhitelist_stage30000_maxw3_pack${PACK_CHARS}_evalval_steps${STEPS}_v042"
...
wait_for_file "$LEN_TOK_DIR/vocab.json" "Length-MAX HF vocab.json (v041 staged)"
...
# 3) train Length-MAX model (v041 vocab)
run_model "$LEN_TOK_DIR" "$LEN_MODEL_LOG" "$LEN_SAVE_DIR" "$STEPS" "Length-MAX(v041 staged vocab)"
...
python3 /home/arxiv_code/tokenizers_rust/plot_loss_overlay.py \
  --csv "$LEN_CURVE" --label "Length-MAX (v041 staged vocab)" \
  --csv "$BPE_CURVE" --label "BPE (baseline)" \
  --out_png "$LOSS_OVERLAY_PNG" \
  --out_pdf "$LOSS_OVERLAY_PDF" \
  --title "Loss vs step (lenmax v041 staged; eval=validation; pack=${PACK_CHARS})"
...
python3 /home/arxiv_code/tokenizers_rust/plot_loss_overlay.py \
  ...
  --title "Train bpc proxy vs step (lenmax v041 staged; quality uses eval_loss->bpc)"
