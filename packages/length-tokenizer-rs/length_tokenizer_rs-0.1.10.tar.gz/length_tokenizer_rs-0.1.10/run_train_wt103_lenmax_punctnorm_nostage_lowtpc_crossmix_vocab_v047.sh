...
# v047: 在 v046 的“非两段式 + 词表规范 + 低 TPC”基础上，
#       **进一步引入“跨词 + 标点+词 混合”的受控 token**（吸收 SuperBPE 的核心低 TPC 优势）。
#
# 新增开关：
# - allow_cross_word_punct_word_mix_tokens=1
#   允许生成诸如 ",ĠtheĠ" ".ĠAtĠtheĠ" "Ġ(ĠtheĠ" 等“跨词 + 标点段 + 词段”组合 token，
#   但按 'Ġ' 分段做严格白名单约束，避免失控噪声。
...
# NEW: 允许“跨词 + 标点+词 混合”token（受控白名单）
LEN_ALLOW_CROSS_WORD_PUNCT_WORD_MIX_TOKENS=1
...
TAG="lenmax_n${LEN_N_MAX}_full_maxchars${LEN_MAX_TOKEN_CHARS}_punctnorm_nostage_lowtpc_crossmix_v047"
OUT_DIR="/home/arxiv_code/tokenizers_rust/tokenizer_out_wikitext103_32k_n${LEN_N_MAX}_full_maxchars${LEN_MAX_TOKEN_CHARS}_punctnorm_nostage_lowtpc_crossmix_v047"
LOG="/home/arxiv_code/tokenizers_rust/train_wikitext103_32k_n${LEN_N_MAX}_full_maxchars${LEN_MAX_TOKEN_CHARS}_punctnorm_nostage_lowtpc_crossmix_v047.log"

echo "[vocab-train] start $(date -Is)"
echo "[vocab-train] tag=$TAG"
...
echo "[vocab-train] allow_cross_word_punct_word_mix_tokens=$LEN_ALLOW_CROSS_WORD_PUNCT_WORD_MIX_TOKENS"
...
length_tokenizer_rs.train_to_hf(
    ...
    allow_cross_word_punct_word_mix_tokens=bool(${LEN_ALLOW_CROSS_WORD_PUNCT_WORD_MIX_TOKENS}),
