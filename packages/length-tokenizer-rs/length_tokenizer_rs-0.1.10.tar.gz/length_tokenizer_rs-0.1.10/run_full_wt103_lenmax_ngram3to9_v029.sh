...
# Bold experiment (v029): train a new Length-MAX tokenizer using only n-grams in [3..=9]
# (i.e. exclude bigrams), then train a matched Llama-style model and compare against an
# existing BPE baseline on WikiText-103 full corpus.
#
# - Token budget equalization: pack_chars=4000 + seq_len=512 + bs=64, active_frac≈1.0
# - Quality metric reported: bpc = best_eval_loss * (full_corpus_tpc) / ln(2)
#   (stable across tokenizer implementations / eval_bpc printing)
...
LEN_TOK_DIR="/home/arxiv_code/tokenizers_rust/tokenizer_out_wikitext103_32k_n3to9_full_v029"
...
TAG="lenmax_n3to9_full_tokfixed_pack${PACK_CHARS}_evalval_steps${STEPS}_v029"

# Logs / outputs
PIPE_LOG="/home/arxiv_code/tokenizers_rust/pipeline_${TAG}.log"
LEN_TOK_LOG="/home/arxiv_code/tokenizers_rust/train_wikitext103_32k_n3to9_full_v029.log"

LEN_MODEL_LOG="/home/arxiv_code/tokenizers_rust/run_llama_lenmax_n3to9_${TAG}.log"
BPE_MODEL_LOG="/home/arxiv_code/tokenizers_rust/run_llama_bpe_${TAG}.log"

LEN_SAVE_DIR="/home/arxiv_code/tokenizers_rust/model_lenmax_n3to9_${TAG}"
BPE_SAVE_DIR="/home/arxiv_code/tokenizers_rust/model_bpe_${TAG}"

LEN_CURVE="/home/arxiv_code/tokenizers_rust/loss_curve_lenmax_n3to9_${TAG}.csv"
BPE_CURVE="/home/arxiv_code/tokenizers_rust/loss_curve_bpe_${TAG}.csv"

LOSS_OVERLAY_PNG="/home/arxiv_code/tokenizers_rust/loss_overlay_lenmax_n3to9_vs_bpe_${TAG}.png"
LOSS_OVERLAY_PDF="/home/arxiv_code/tokenizers_rust/loss_overlay_lenmax_n3to9_vs_bpe_${TAG}.pdf"
BPC_OVERLAY_PNG="/home/arxiv_code/tokenizers_rust/bpc_overlay_lenmax_n3to9_vs_bpe_${TAG}.png"
BPC_OVERLAY_PDF="/home/arxiv_code/tokenizers_rust/bpc_overlay_lenmax_n3to9_vs_bpe_${TAG}.pdf"
...
echo "[pipeline] len tokenizer: aim_vocab=$LEN_AIM_VOCAB n_min=$LEN_N_MIN n_max=$LEN_N_MAX max_token_chars=$LEN_MAX_TOKEN_CHARS workers=$LEN_WORKERS multi_process=$LEN_MULTI_PROCESS" | tee -a "$PIPE_LOG"
...
train_len_tokenizer() {
  ...
  echo "[pipeline] start Length-MAX tokenizer (n=3..9) -> $LEN_TOK_LOG" | tee -a "$PIPE_LOG"
  mkdir -p "$LEN_TOK_DIR"
...
# 0) ensure BPE tokenizer exists
wait_for_file "$BPE_TOK_DIR/tokenizer.json" "BPE HF tokenizer.json"

# 1) train Length-MAX tokenizer (n=3..9)
train_len_tokenizer
...
# 3) train BPE baseline if needed (otherwise reuse default log)
...
# 4) train Length-MAX model (n=3..9)
run_model "$LEN_TOK_DIR" "$LEN_MODEL_LOG" "$LEN_SAVE_DIR" "$STEPS" "Length-MAX(n=3..9)"

# 5) curves + overlay (optional but useful)
...
python3 /home/arxiv_code/tokenizers_rust/plot_loss_overlay.py \
  --csv "$LEN_CURVE" --label "Length-MAX (n=3..9)" \
  --csv "$BPE_CURVE" --label "BPE (baseline)" \
  --out_png "$LOSS_OVERLAY_PNG" \
  --out_pdf "$LOSS_OVERLAY_PDF" \
  --title "Loss vs step (lenmax n=3..9; eval=validation; pack=${PACK_CHARS})"
...
python3 /home/arxiv_code/tokenizers_rust/plot_loss_overlay.py \
  ...
  --title "Train bpc proxy vs step (lenmax n=3..9; quality uses eval_loss->bpc)"

# 6) summary (stable bpc from eval_loss + full-corpus TPC)
...
echo "len_tok_dir,len_n_min,len_n_max,len_tpc,bpe_tpc,bpe_best_eval_loss,len_best_eval_loss,bpe_best_bpc,len_best_bpc,len_model_log,bpe_model_log" > "$SUMMARY_CSV"
echo "${LEN_TOK_DIR},${LEN_N_MIN},${LEN_N_MAX},${LEN_TPC},${BPE_TPC},${BPE_BEST_LOSS},${LEN_BEST_LOSS},${BPE_BPC},${LEN_BPC},${LEN_MODEL_LOG},${BPE_BASE_LOG}" >> "$SUMMARY_CSV"
