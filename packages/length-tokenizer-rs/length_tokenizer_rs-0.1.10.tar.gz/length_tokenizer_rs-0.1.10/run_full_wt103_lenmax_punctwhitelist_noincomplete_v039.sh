...
# v039: Train a Llama-style model using the new Length-MAX vocab v038:
# - forbid_punct_tokens=True with allow-list exceptions:
#     allow_space_punct_tokens=True / allow_abbrev_tokens=True / allow_hyphen_tokens=True
# - forbid_incomplete_cross_word=True
#
# Decode mode: default (no cross_word_whole_only / forbid_end_inner / forbid_punct_tokens env filtering)
...
LEN_TOK_DIR="/home/arxiv_code/tokenizers_rust/tokenizer_out_wikitext103_32k_n9_full_maxchars48_punctwhitelist_noincomplete_v038"
...
TAG="lenmax_punctwhitelist_noincomplete_pack${PACK_CHARS}_evalval_steps${STEPS}_v039"
...
echo "[pipeline] env LENGTH_TOKENIZER_CROSS_WORD_WHOLE_ONLY=(unset) LENGTH_TOKENIZER_FORBID_END_INNER=(unset) LENGTH_TOKENIZER_FORBID_PUNCT_TOKENS=(unset)" | tee -a "$PIPE_LOG"
...
wait_for_file "$LEN_TOK_DIR/vocab.json" "Length-MAX HF vocab.json (v038 punct whitelist)"
...
# 3) train Length-MAX model (v038 vocab)
run_model "$LEN_TOK_DIR" "$LEN_MODEL_LOG" "$LEN_SAVE_DIR" "$STEPS" "Length-MAX(v038 punct whitelist vocab)"
...
python3 /home/arxiv_code/tokenizers_rust/plot_loss_overlay.py \
  --csv "$LEN_CURVE" --label "Length-MAX (v038 punct whitelist vocab)" \
  --csv "$BPE_CURVE" --label "BPE (baseline)" \
  --out_png "$LOSS_OVERLAY_PNG" \
  --out_pdf "$LOSS_OVERLAY_PDF" \
  --title "Loss vs step (lenmax v038 punct whitelist; eval=validation; pack=${PACK_CHARS})"
...
python3 /home/arxiv_code/tokenizers_rust/plot_loss_overlay.py \
  ...
  --title "Train bpc proxy vs step (lenmax v038; quality uses eval_loss->bpc)"
...
# v039: Train a Llama-style model using the new Length-MAX vocab trained with v038:
# - forbid_punct_tokens=True, but allow-list exceptions:
#   - allow_space_punct_tokens=True  (e.g. ",Ġ", ".Ġ", "@-@Ġ")
#   - allow_abbrev_tokens=True       (e.g. "U.S.Ġ", "e.g.Ġ")
#   - allow_hyphen_tokens=True       (e.g. "well-knownĠ" if it appears)
# - forbid_incomplete_cross_word=True
#
# Decode mode: default (no cross_word_whole_only / forbid_end_inner / forbid_punct_tokens env filtering)
...
# 0) ensure tokenizers exist
wait_for_file "$BPE_TOK_DIR/tokenizer.json" "BPE HF tokenizer.json"
wait_for_file "$LEN_TOK_DIR/vocab.json" "Length-MAX HF vocab.json (v038 punct whitelist + no-incomplete)"
...
# 3) train Length-MAX model (v038 vocab)
run_model "$LEN_TOK_DIR" "$LEN_MODEL_LOG" "$LEN_SAVE_DIR" "$STEPS" "Length-MAX(v038 punct whitelist + no-incomplete vocab)"
...
python3 /home/arxiv_code/tokenizers_rust/plot_loss_overlay.py \
  --csv "$LEN_CURVE" --label "Length-MAX (v038 punct whitelist + no-incomplete)" \
  --csv "$BPE_CURVE" --label "BPE (baseline)" \
  --out_png "$LOSS_OVERLAY_PNG" \
  --out_pdf "$LOSS_OVERLAY_PDF" \
  --title "Loss vs step (lenmax v038 punct whitelist; eval=validation; pack=${PACK_CHARS})"
