...
class Label(Block):
    ...
    def reveal(self, target_id, direction="top", duration="0.4s", mode="overlay"):
        ...
        Rules:
        - reveal() can be called BEFORE the label has a parent
        - checkbox insertion is deferred until the label is mounted
        - checkbox, label and target must share the same parent
        """
        ...
        # --------------------------------------------------
        # 2. Prepare deferred mount hook
        # --------------------------------------------------
        if not hasattr(self, "_post_mount"):
            self._post_mount = []

        def _mount_checkbox():
            parent = self._parent
            if parent is None:
                return  # still not mounted, safe exit
            ...
            # Insert checkbox BEFORE the label
            try:
        ...
        self._post_mount.append(_mount_checkbox)
        ...
        # --------------------------------------------------
        # 6. Focus style for keyboard users
        # --------------------------------------------------
        CSS_RULES_STYLE.append({
