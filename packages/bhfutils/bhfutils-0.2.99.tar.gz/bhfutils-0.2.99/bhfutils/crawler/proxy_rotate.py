...
class RotatingProxyMiddleware(object):
    ...
    * ``ROTATING_PROXY_BACKOFF_CAP`` - backoff time cap, in seconds.
    ...
    def __init__(self, proxy_list, logstats_interval: float, stop_if_no_proxies: bool,
                 max_proxies_to_try: int, backoff_base: float, backoff_cap: float, crawler: Crawler):
    ...
    def from_crawler(cls, crawler: Crawler):
        s = crawler.settings
        proxy_path: str = s.get('ROTATING_PROXY_LIST_PATH', None)
    ...
    def _handle_result(self, request: Request, spider: Spider):
...
class BanDetectionMiddleware(object):
    ...
    to subclass and modify default BanDetectionPolicy::
        ...
        # myproject/policy.py
        from bhfutils.crawler.proxy_rotate import BanDetectionPolicy
    ...
    @classmethod
    def _load_policy(cls, crawler: Crawler):
        policy_path = crawler.settings.get(
            'ROTATING_PROXY_BAN_POLICY',
            'bhfutils.crawler.policy.BanDetectionPolicy'
        )
