...
def get_cra_surface_80m_meteos(
    ...
    """
    获取 CRA 80m 高度层气象要素（每个变量单独 GRIB2 文件）

    参数:
        year, month, day, hour: 时间（用于构建路径）
        vars: 需要提取的变量列表（大写，如 "UGRD", "TMP" 等）
        get_values: 是否返回 numpy 值 (默认为 True)
        interp_lon / interp_lat: 精确插值位置（可选）
        around_radius: 周边方阵半径
                        0 = 只中心点
                        1 = 3x3 方阵
                        21 = 43x43 方阵 等
        file_structure: 文件路径模板（支持 {YYYY}, {YYYYMMDDHH}, {VAR} 等）
...
def get_cra_atmos_meteos(
    ...
    """
    获取 CRA 大气层（等压面）气象要素（每个变量单独 GRIB2 文件）

    参数:
        year, month, day, hour: 时间
        vars: 变量列表（如 "TMP", "UGRD", "RH" 等）
        pressure_levels: 等压面高度列表（字符串，如 ["500", "700"]）
        get_values: 是否返回 numpy 值 (默认为 True)
        interp_lon / interp_lat: 精确插值位置（可选）
        around_radius: 周边方阵半径（0=只中心，1=3x3，21=43x43 等）
        file_structure: 文件路径模板（支持 {YYYY}, {YYYYMMDDHH}, {VAR}）

    返回:
        dict，key 格式：
          - 中心: "{VAR}_{LEVEL}"
          - 偏移: "{VAR}_{LEVEL}_shift_lat±i_lon±j"
    """
    if around_radius < 0:
        raise ValueError("around_radius 必须为非负整数")
    ...
    for base_var in vars:
        ...
        # 打开变量文件（包含所有等压面层）
        with xr.open_dataset(file_path, backend_kwargs={'indexpath': ''}) as ds:
            ds = ds.rename({'longitude': 'lon', 'latitude': 'lat', 'isobaricInhPa': 'level'})
            mapped_var_name = var_dict[base_var]
            da_full = ds[mapped_var_name]  # shape: (level, lat, lon)

            for level in pressure_levels:
                try:
                    level_float = float(level)
                except ValueError:
                    raise ValueError(f"pressure_levels 中的值必须是可转换为浮点的字符串，如 '500'")

                # 提取该层
                da_layer = da_full.sel(level=level_float)

                if use_interp:
                    # 先插值到精确位置 → 单点 DataArray（不带 level 维度）
                ...
                for di, dj in shifts:
                    if di == 0 and dj == 0:
                        da_shifted = da_center
                        var_name = f"{base_var}_{level}"
                    else:
                        da_shifted = da_center.shift(lat=di, lon=dj)
                        sign_i = "+" if di >= 0 else "-"
                        sign_j = "+" if dj >= 0 else "-"
                        var_name = f"{base_var}_{level}_shift_lat{sign_i}{abs(di)}_lon{sign_j}{abs(dj)}"

                    if get_values:
                        result[var_name] = da_shifted.values  # 标量 / 小数组 / 全局二维数组
                    else:
                        result[var_name] = da_shifted
