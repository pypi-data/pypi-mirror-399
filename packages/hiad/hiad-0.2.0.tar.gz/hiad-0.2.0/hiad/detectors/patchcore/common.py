...
class FaissNN(object):
    def __init__(self, device, on_gpu: bool = False, num_workers: int = 4) -> None:
    ...
    def _create_index(self, dimension):
        if self.on_gpu:
            ...
            if self.device.index is not None:
                gpu_config.device = self.device.index
    ...
    def convert_to_segmentation(self, patch_scores):
        ...
        with torch.no_grad():
            ...
            _scores = F.interpolate(
                _scores, size=[self.target_size[1], self.target_size[0]], mode="bilinear", align_corners=False
            )
