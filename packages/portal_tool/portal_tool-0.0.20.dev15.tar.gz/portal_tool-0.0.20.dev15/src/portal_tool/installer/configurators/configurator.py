    ...
    def _validate_cmake(self) -> None:
        try:
            ...
            if not version_match:
                raise typer.Abort("CMake is not installed, Please install CMake and try again.")

            version = tuple(map(int, version_match.group(1).split('.')))
            if version < (3, 30, 0):
                raise typer.Abort("CMake version must be at least 3.30.0, Please install CMake and try again.")
        except subprocess.CalledProcessError:
            raise typer.Abort("CMake is not installed, Please install CMake and try again.")
