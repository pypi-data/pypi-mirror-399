    ...
    def __init__(self, config: WorkLogConfig, credentials_path: Optional[Path] = None):
        ...
        Args:
            ...
            credentials_path: Path to credentials file (OAuth token or Service Account JSON).
                            If None, tries config.google_sheets.credentials_path, then haunts OAuth
        ...
        # Determine credentials_path: parameter > config > None (will try haunts)
        if credentials_path is None and config.google_sheets.credentials_path:
            credentials_path = Path(config.google_sheets.credentials_path).expanduser()
        ...
        if config.google_sheets.use_haunts_format:
            try:
                ...
                raise ValueError(
                    ...
                    f"Solutions:\n"
                    f"  1. Set google_sheets.credentials_path in config.yaml\n"
