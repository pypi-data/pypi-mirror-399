...
class Model:
    ...
    def compare(
        ...
        Uses an efficient two-phase approach:
        1. Submit all runs in parallel (fast)
        2. Single polling loop checks all pending runs (reduces API load)
        ...
        Args:
            ...
            max_workers: Maximum number of parallel workers for submission (default: 10)
            progress_callback: Optional callback(completed, total) for progress updates
        ...
        # Phase 1: Submit all runs in parallel
        ...
        logger.info(f"Submitting {total_tasks} runs...")
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            futures = []
            for model_name in models:
                for idx, inputs in enumerate(inputs_list):
                    future = executor.submit(submit_single, model_name, idx, inputs)
                    futures.append(future)

            for future in as_completed(futures):
                model_name, idx, run_number, error = future.result()
                if error:
                    logger.warning(
                        f"Model {model_name} submission failed for input {idx}: {error}"
                    )
                    results[model_name][idx] = {"error": error, "status": "failed"}
                else:
                    pending_runs.append((model_name, idx, run_number))

        logger.info(f"Submitted {len(pending_runs)} runs, polling for completion...")

        # Phase 2: Single polling loop for all pending runs
        start_time = time.time()
        poll_interval = 1.0
        completed = total_tasks - len(pending_runs)  # Count already failed submissions

        if progress_callback and completed > 0:
            progress_callback(completed, total_tasks)

        while pending_runs and (time.time() - start_time) < timeout:
            still_pending = []
