...
def sanitize_model_name(model_name: str) -> str:
    ...
    Args:
        model_name: Full model name (e.g., "fal-ai/flux-pro")

    Returns:
        Sanitized name suitable for column names (e.g., "flux_pro")

    Example:
        >>> sanitize_model_name("fal-ai/flux-pro")
        'flux_pro'
...
def generate_name(prefix: str, suffix: str = "") -> str:
    ...
    Args:
        prefix: Name prefix (e.g., "t2i")
        suffix: Optional suffix before date (e.g., "eval")

    Returns:
        Name with date (e.g., "t2i-eval-dec-29-2025")

    Example:
        >>> generate_name("t2i", "eval")  # Returns "t2i-eval-dec-29-2025"
        >>> generate_name("results")  # Returns "results-dec-29-2025"
    """
    date_str = datetime.now().strftime("%b-%d-%Y").lower()
    if suffix:
        return f"{prefix}-{suffix}-{date_str}"
