...
from mixtrain.routers import Router, get_router, list_routers as list_routers_func
...
def list_routers_cmd():
    ...
    try:
        client = MixClient()
        # Use direct HTTP call since Router.list() creates Router objects (eager loading)
        response = client._make_request(
            "GET",
            f"/workspaces/{client._workspace_name}/inference/routers",
        )
...
def create(
    ...
    try:
        ...
        if output:
            ...
            rprint(f"[green]✓[/green] Router '{router.name}' created successfully")
