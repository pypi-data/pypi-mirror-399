...
- `add_polygon(df, lon, lat, num_sides, radius=..., side_length=..., angle_value=None, rotation=0.0, geometry='geometry')`
    ...
    - `angle_value`: optional interior angle in degrees. If provided, function enters interior-mode; if `None` (default), operates in exterior/regular mode.
    - `rotation`: overall rotation in degrees applied after interior/exterior handling. Can be scalar or column name for per-row rotation.
    - `geometry`: output geometry column name.

    Note: vertex computation is vectorized for performance on large datasets; both `angle_value` and `rotation` accept scalars or per-row column names.
