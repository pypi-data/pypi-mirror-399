...
class ShmRingBuffer:
    ...
    def read_frame_copy(self, frame_idx: int) -> Optional[bytes]:
        ...
        # CRASH DETECTION: If seq_start is ODD, producer crashed mid-write OR is currently writing
        if seq_start & 1:
            # DEBUG: Log with more context
            header = self._read_header()
            current_write_idx = header['write_idx']
            self.logger.warning(
                f"[ODD_SEQ_DEBUG] frame_idx={frame_idx}, slot={slot}, "
                f"seq_start={seq_start} (odd = write in progress or crashed), "
                f"write_idx={current_write_idx}, slot_count={self.slot_count}, "
        ...
        if seq_start != seq_end:
            # DEBUG: Log with more context to understand why torn with small frames_behind
            header = self._read_header()
            current_write_idx = header['write_idx']
            self.logger.warning(
                f"[TORN_DEBUG] frame_idx={frame_idx}, slot={slot}, "
