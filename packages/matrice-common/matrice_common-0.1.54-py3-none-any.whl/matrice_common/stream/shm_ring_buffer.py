...
class ShmRingBuffer:
    ...
    def write_frame(self, raw_bytes: Union[bytes, memoryview, np.ndarray]) -> Tuple[int, int]:
        ...
        if isinstance(raw_bytes, np.ndarray):
            ...
            if raw_bytes.flags['C_CONTIGUOUS']:
                raw_mv = memoryview(raw_bytes).cast('B')
            else:
        ...
        # TORN FRAME DETECTION: Set seq_end = seq_start (now EVEN = committed)
        # Increment to make even, then write same value to seq_end
    ...
    def read_frame_copy(self, frame_idx: int) -> Optional[bytes]:
        ...
        - Crashed if: seq_start is ODD (producer crashed mid-write)
        ...
        # CRASH DETECTION: If seq_start is ODD, producer crashed mid-write
        if seq_start & 1:
            self.logger.debug(
                f"Crashed/incomplete frame detected for frame_idx={frame_idx}, slot={slot}: "
                f"seq_start={seq_start} (odd = write never completed)"
            )
            return None
    ...
    def is_frame_valid(self, frame_idx: int, max_wait_ms: float = 5.0) -> bool:
        ...
        This method handles cross-process memory visibility delays by retrying
        with a short spin wait when the frame appears to be "in the future".

        Args:
            frame_idx: Frame index to validate
            max_wait_ms: Max time to wait if frame appears not-yet-written (default 5ms)
        ...
        # Frame appears to be in the future - may be a visibility delay
        # Retry with short spin wait (handles cross-process memory visibility)
        if frame_idx > current_write_idx:
            ...
            while time.time() - start_time < max_wait_sec:
                ...
                if frame_idx <= current_write_idx:
                    break
                time.sleep(0.0001)  # 0.1ms sleep to avoid busy spin

            # Still in future after waiting - genuinely hasn't been written
            if frame_idx > current_write_idx:
                return False
