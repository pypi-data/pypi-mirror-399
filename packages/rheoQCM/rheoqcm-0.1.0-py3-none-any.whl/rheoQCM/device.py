...
def get_jax_backend() -> str:
    """Get the current JAX backend (cpu, cuda, tpu).

    Returns
    -------
    str
        The current JAX backend name, or "not installed" if JAX is unavailable.
    """
    try:
        import jax
...
def get_device_info() -> dict[str, str | list[str] | int | bool | float | None]:
    ...
    """
    info: dict[str, str | list[str] | int | bool | float | None] = {
        "jax_version": None,
        "jax_backend": None,
        "devices": [],
        "gpu_count": 0,
