...
from matplotlib.container import (
    ErrorbarContainer,  # for checking if l[ln] is ErrorbarContainer
)
...
class MatplotlibWidget(QWidget):
    ...
    def init_sp(
        ...
        **kwargs,
        ...
        self.l["srec"] = self.ax[0].plot(
            [], [], marker="x", linestyle="none", color="g"
        ...
        self.l["lPfit"] = self.ax[0].plot([], [], color="k")  # polar plot fit
        self.l["strk"] = self.ax[0].plot(
            [], [], marker="+", linestyle="none", color="r"
        )  # center of tracking peak
        self.l["srec"] = self.ax[0].plot(
    ...
    def update_sp_text_chi(self, chi=None):
        ...
        if not chi:
            ...
            self.txt["chi"].set_text(rf"$\chi^2$ = {chi:.4f}")
    ...
    def init_sp_polar(
        ...
        **kwargs,
        ...
        self.l["lfit"] = self.ax[0].plot([], [], color="k")  # fit

        self.l["lsp"] = self.ax[0].plot([], [], color=color[2])  # fit in span range
    ...
    def init_data(
        ...
        **kwargs,
        ...
        icon_sel = QIcon()
        icon_sel.addPixmap(
            QPixmap(":/button/rc/selector.svg"), QIcon.Mode.Normal, QIcon.State.Off
        )
        self.pushButton_selectorswitch.setIcon(icon_sel)

        self.pushButton_selectorswitch.clicked.connect(
            self.data_rectsleector_picker_switch
        )
    ...
    def data_onpick_callback(self, event):
        ...
        )  # transfer the label of picked line and ind to 'lp'
        self.canvas_draw()
    ...
    def init_contour(
        ...
        **kwargs,
        ...
        if self.ax:
            ...
            self.ax.append(
                make_axes_locatable(self.ax[0]).append_axes(
                    "right", size="5%", pad="2%"
                )
            )
        ...
        # set label of ax[1]
        self.set_ax(
            self.ax[0], xlabel=r"$d/\lambda$", ylabel=r"$\Phi$ ($\degree$)", title=title
        )
    ...
    def data_show_all(self):
        for ax in self.ax:
            ax.set_autoscale_on(
                True
            )  # this reactive autoscale which might be turnned of by zoom/pan
            self.reset_ax_lim(ax)
