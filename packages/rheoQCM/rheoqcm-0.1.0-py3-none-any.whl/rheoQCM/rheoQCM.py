...
if UIModules.system_check() == "win32":  # windows
    ...
    if struct.calcsize("P") * 8 == 32:  # 32-bit version Python
            ...
            with AccessMyVNA() as vna:
                if vna.Init() == 0:  # connection with myVNA is available
                    try:
                        ...
                        logger.warning(
                            "Failed to import TempDevices and/or TempModules.\nTemperature functions of the UI will not avaiable!"
                        )
                        logger.exception(
                            "Failed to import TempDevices and/or TempModules."
                        )

        except Exception:  # no myVNA connected. Analysis only
        ...
        logger.warning(
            "Current version of MyVNA does not work with 64-bit Python!\nData analysis only!"
        )
else:  # linux or MacOS
    # MyVNA hardware interface not available on non-Windows platforms
    logger.warning(
        "Current version of MyVNA does not work with MacOS and Linux! Data analysis only."
    )
...
def setup_logging():
    ...
    try:
        if os.path.exists(
            config_default["logger_setting_config"]["config_file"]
        ):  # use json file
            with open(config_default["logger_setting_config"]["config_file"]) as f:
...
class VNATracker:
    ...
    def get_cal_filenames(self):
        ...
        """
        cal = {"ADC1": "", "ADC2": ""}
        if (UIModules.system_check() == "win32") and (
            struct.calcsize("P") * 8 == 32
        ):  # windows (if is win32, struct will already be imported above)
...
class QCMApp(QMainWindow):
    ...
    def __init__(
        ...
        *,
        ...
        self.settings = (
            settings_default.copy()
        )  # import default settings. It will be initalized later

        self.peak_tracker = PeakTracker.PeakTracker(
            max_harm=self.settings["max_harmonic"]
        )
        ...
        self.data_saver = DataSaver.DataSaver(
            ver=_version.__version__, settings=self.settings
        )
        ...
        self.settings_chn = {
            "name": "samp",
            "chn": "1",
        }  # active channel 'samp' or 'ref' in Settings Tab
        ...
        self.hide_widgets("version_hide_list")
    ...
    def init_ui(self):
        ...
        self.ui.pushButton_settings_la.clicked.connect(
            lambda: self.set_stackedwidget_index(
                self.ui.stackedWidget_spectra, diret=-1
            )
        )  # set index-1
        ...
        self.ui.dateTimeEdit_reftime.dateTimeChanged.connect(
            self.on_dateTimeChanged_dateTimeEdit_reftime
        )
        ...
        self.ui.comboBox_settings_control_dispmode.currentIndexChanged.connect(
            self.update_widget
        )
        self.ui.comboBox_settings_control_dispmode.currentIndexChanged.connect(
            self.update_freq_display_mode
        )

        # set pushButton_gotofolder
        self.ui.pushButton_gotofolder.clicked.connect(
            self.on_clicked_pushButton_gotofolder
        )
        ...
        self.ui.plainTextEdit_settings_sampledescription.textChanged.connect(
            lambda: self.update_widget(
        ...
        self.ui.radioButton_spectra_showGp.clicked.connect(
            self.mpl_sp_clr_lines_set_label
        )
        self.ui.radioButton_spectra_showBp.toggled.connect(self.update_widget)
        self.ui.radioButton_spectra_showBp.clicked.connect(
            self.mpl_sp_clr_lines_set_label
        )
        self.ui.radioButton_spectra_showpolar.toggled.connect(self.update_widget)
        self.ui.radioButton_spectra_showpolar.clicked.connect(
            self.mpl_sp_clr_lines_set_label
        )
        ...
        self.ui.checkBox_control_rectemp.clicked["bool"].connect(
            self.on_clicked_set_temp_sensor
        )

        # set checkBox_dynamicfitbyharm
        self.ui.checkBox_dynamicfitbyharm.clicked["bool"].connect(
            self.on_clicked_checkBox_dynamicfitbyharm
        )

        # set checkBox_fitfactorbyharm
        self.ui.checkBox_fitfactorbyharm.clicked["bool"].connect(
        ...
        self.ui.tabWidget_settings_settings_samprefchn.tabBar().setTabButton(
            self.ui.tabWidget_settings_settings_samprefchn.indexOf(
                self.ui.tab_settings_settings_harmchnsamp
            ),
            QTabBar.ButtonPosition.LeftSide,
        ...
        self.ui.tabWidget_settings_settings_samprefchn.tabBar().setTabButton(
            self.ui.tabWidget_settings_settings_samprefchn.indexOf(
                self.ui.tab_settings_settings_harmchnref
            ),
            QTabBar.ButtonPosition.LeftSide,
        ...
        # set signal
        self.ui.tabWidget_settings_settings_samprefchn.currentChanged.connect(
            self.update_widget
        )
        self.ui.tabWidget_settings_settings_samprefchn.currentChanged.connect(
            self.update_settings_chn
        )
        ...
        self.ui.tabWidget_settings_settings_harm.currentChanged.connect(
        ...
        if not self.vna_tracker.cal["ADC1"] and (
            curr_chn != 1
        ):  # no calibration file for ADC1
            # delete ADC1 from both lists
            if self.ui.comboBox_samp_channel.findData(1) != -1:
                self.ui.comboBox_samp_channel.removeItem(
                    self.ui.comboBox_samp_channel.findData(1)
                )
            if self.ui.comboBox_ref_channel.findData(1) != -1:
                self.ui.comboBox_ref_channel.removeItem(
                    self.ui.comboBox_ref_channel.findData(1)
                )
        if not self.vna_tracker.cal["ADC2"] and (
            curr_chn != 2
        ):  # no calibration file for ADC1
            # delete ADC1 from both lists
            if self.ui.comboBox_samp_channel.findData(2) != -1:
                self.ui.comboBox_samp_channel.removeItem(
                    self.ui.comboBox_samp_channel.findData(2)
                )
            if self.ui.comboBox_ref_channel.findData(2) != -1:
                self.ui.comboBox_ref_channel.removeItem(
                    self.ui.comboBox_ref_channel.findData(2)
                )
        ...
        if self.vna and self.system == "win32":
            config_default["tempdevs_opts"] = TempDevices.dict_available_devs(
                config_default["tempdevices_dict"]
            )
            self.create_combobox(
                "comboBox_tempdevice",
            ...
            self.settings["comboBox_tempdevice"] = self.ui.comboBox_tempdevice.itemData(
                self.ui.comboBox_tempdevice.currentIndex()
            )
            self.ui.comboBox_tempdevice.currentIndexChanged.connect(
                self.update_tempdevice
            )
        ...
        self.ui.lineEdit_scan_harmstart.editingFinished.connect(
            self.on_editingfinished_harm_freq
        )
        self.ui.lineEdit_scan_harmend.editingFinished.connect(
            self.on_editingfinished_harm_freq
        )
        self.ui.comboBox_base_frequency.currentIndexChanged.connect(
            self.update_base_freq
        )
        ...
        self.ui.radioButton_peaks_num_max.toggled["bool"].connect(
            self.update_harmwidget
        )
        self.ui.radioButton_peaks_num_fixed.toggled["bool"].connect(
            self.update_harmwidget
        )
        self.ui.radioButton_peaks_policy_minf.toggled["bool"].connect(
            self.update_harmwidget
        )
        self.ui.radioButton_peaks_policy_maxamp.toggled["bool"].connect(
            self.update_harmwidget
        )
        self.ui.checkBox_settings_settings_harmlockphase.toggled["bool"].connect(
            self.update_harmwidget
        )
        ...
        self.ui.comboBox_samp_channel.currentIndexChanged.connect(
            self.update_vnachannel
        )
        self.ui.comboBox_samp_channel.currentIndexChanged.connect(
            self.update_settings_chn
        )
        ...
        self.ui.comboBox_ref_channel.currentIndexChanged.connect(
            self.update_settings_chn
        )

        # self.ui.checkBox_settings_temp_sensor.stateChanged.connect(self.update_tempsensor)
        self.ui.checkBox_settings_temp_sensor.stateChanged.connect(
            self.on_clicked_set_temp_sensor
        )
        # self.ui.comboBox_thrmcpltype.currentIndexChanged.connect(self.update_tempdevice) # ??
        self.ui.comboBox_thrmcpltype.currentIndexChanged.connect(
            self.update_thrmcpltype
        )
        ...
        # load opts to combox
        self.build_comboBox(
            self.ui.comboBox_settings_data_samprefsource, "ref_channel_opts"
        )
        self.build_comboBox(
            self.ui.comboBox_settings_data_refrefsource, "ref_channel_opts"
        )
        ...
        self.ui.pushButton_settings_data_resetshiftedt0.clicked.connect(
            self.reset_shiftedt0
        )
        ...
        self.ui.dateTimeEdit_settings_data_t0shifted.dateTimeChanged.connect(
            self.on_dateTimeChanged_dateTimeEdit_t0shifted
        )
        ...
        self.ui.comboBox_settings_data_samprefsource.currentIndexChanged.connect(
            self.update_widget
        )
        self.ui.lineEdit_settings_data_sampidx.textChanged.connect(self.update_widget)
        self.ui.lineEdit_settings_data_samprefidx.textChanged.connect(
            self.update_widget
        )

        # NOTE: following two only emitted when value manually edited (activated)
        self.ui.comboBox_settings_data_samprefsource.activated.connect(
            self.save_data_saver_sampref
        )
        self.ui.comboBox_settings_data_samprefsource.activated.connect(
            self.set_mech_layer_0_indchn_val
        )
        self.ui.lineEdit_settings_data_sampidx.editingFinished.connect(
            self.save_data_saver_sampidx
        )
        self.ui.lineEdit_settings_data_samprefidx.editingFinished.connect(
            self.save_data_saver_sampref
        )
        self.ui.lineEdit_settings_data_samprefidx.editingFinished.connect(
            self.set_mech_layer_0_indchn_val
        ...
        self.ui.comboBox_settings_data_refrefsource.currentIndexChanged.connect(
            self.update_widget
        )
        ...
        # NOTE: following two only emitted when value manually edited (activated)
        self.ui.comboBox_settings_data_refrefsource.activated.connect(
            self.save_data_saver_refref
        )
        self.ui.comboBox_settings_data_refrefsource.activated.connect(
            self.set_mech_layer_0_indchn_val
        )
        self.ui.lineEdit_settings_data_refidx.editingFinished.connect(
            self.save_data_saver_refidx
        )
        self.ui.lineEdit_settings_data_refrefidx.editingFinished.connect(
            self.save_data_saver_refref
        )
        self.ui.lineEdit_settings_data_refrefidx.editingFinished.connect(
            self.set_mech_layer_0_indchn_val
        ...
        self.build_comboBox(
            self.ui.comboBox_settings_data_ref_fitttype, "ref_interp_opts"
        )
        self.ui.comboBox_settings_data_ref_fitttype.currentIndexChanged.connect(
            self.update_widget
        )
        # initially hiden
        self.ui.comboBox_settings_data_ref_fitttype.hide()
        self.ui.comboBox_settings_data_ref_fitttype.currentIndexChanged.connect(
            self.on_ref_mode_changed
        )
        # comboBox_settings_data_ref_crystmode
        self.build_comboBox(
            self.ui.comboBox_settings_data_ref_crystmode, "ref_crystal_opts"
        )
        self.ui.comboBox_settings_data_ref_crystmode.currentIndexChanged.connect(
            self.update_widget
        )
        self.ui.comboBox_settings_data_ref_crystmode.currentIndexChanged.connect(
            self.on_ref_mode_changed
        ...
        self.build_comboBox(
            self.ui.comboBox_settings_data_ref_tempmode, "ref_temp_opts"
        )
        self.ui.comboBox_settings_data_ref_tempmode.currentIndexChanged.connect(
            self.update_widget
        )
        self.ui.comboBox_settings_data_ref_tempmode.currentIndexChanged.connect(
            self.on_ref_mode_changed
        ...
        self.ui.stackedWidget_settings_mechanics_modeswitch.addAction(
            self.ui.actionfilm_construction_mode_switch
        )
        self.ui.actionfilm_construction_mode_switch.triggered.connect(
            self.film_construction_mode_switch
        )
        ...
        self.ui.spinBox_settings_mechanics_nhcalc_n1.setMaximum(
            self.settings["max_harmonic"]
        )
        self.ui.spinBox_settings_mechanics_nhcalc_n1.setMaximum(
            self.settings["max_harmonic"]
        )
        self.ui.spinBox_settings_mechanics_nhcalc_n3.setMaximum(
            self.settings["max_harmonic"]
        )
        self.ui.spinBox_settings_mechanics_nhcalc_n1.valueChanged.connect(
            self.update_widget
        )
        self.ui.spinBox_settings_mechanics_nhcalc_n2.valueChanged.connect(
            self.update_widget
        )
        self.ui.spinBox_settings_mechanics_nhcalc_n3.valueChanged.connect(
            self.update_widget
        )

        self.ui.pushButton_settings_mechanics_clrallprops.clicked.connect(
            self.mech_clear
        )

        self.ui.checkBox_settings_mechanics_witherror.toggled.connect(
            self.update_widget
        )

        # spinBox_mech_expertmode_layernum
        self.ui.spinBox_mech_expertmode_layernum.setMinimum(
            config_default["min_mech_layers"]
        )
        self.ui.spinBox_mech_expertmode_layernum.setMaximum(
            config_default["max_mech_layers"]
        )
        self.ui.spinBox_mech_expertmode_layernum.valueChanged.connect(
            self.update_mechchnwidget
        )
        self.ui.spinBox_mech_expertmode_layernum.valueChanged.connect(
            self.build_mech_layers
        )

        self.ui.spinBox_mech_expertmode_layernum.setValue(
            self.settings.get("spinBox_mech_expertmode_layernum", 0)
        )

        # comboBox_settings_mechanics_calctype
        self.build_comboBox(
            self.ui.comboBox_settings_mechanics_calctype, "calctype_opts"
        )
        self.ui.comboBox_settings_mechanics_calctype.currentIndexChanged.connect(
            self.update_widget
        )

        # doubleSpinBox_settings_mechanics_bulklimit
        self.ui.doubleSpinBox_settings_mechanics_bulklimit.setMinimum(
            config_default["mech_bulklimit"]["min"]
        )
        self.ui.doubleSpinBox_settings_mechanics_bulklimit.setMaximum(
            config_default["mech_bulklimit"]["max"]
        )
        self.ui.doubleSpinBox_settings_mechanics_bulklimit.setSingleStep(
            config_default["mech_bulklimit"]["step"]
        )
        self.ui.doubleSpinBox_settings_mechanics_bulklimit.valueChanged.connect(
            self.update_widget
        )
        ...
        self.ui.comboBox_settings_mechanics_selectmodel.currentIndexChanged.connect(
            self.update_widget
        )
        self.ui.comboBox_settings_mechanics_selectmodel.currentIndexChanged.connect(
            self.set_mechmodel_widgets
        )
        self.build_comboBox(
            self.ui.comboBox_settings_mechanics_selectmodel, "qcm_model_opts"
        )
        # initiate data
        ...
        self.build_comboBox(
            self.ui.comboBox_settings_mechanics_model_overlayer_chn, "ref_channel_opts"
        )
        # comboBox_settings_mechanics_model_samplayer_chn
        self.build_comboBox(
            self.ui.comboBox_settings_mechanics_model_samplayer_chn, "ref_channel_opts"
        )
        ...
        self.build_comboBox(
            self.ui.comboBox_settings_mechanics_contourdata, "contour_data_opts"
        )
        self.ui.comboBox_settings_mechanics_contourdata.currentIndexChanged.connect(
            self.update_widget
        )

        # comboBox_settings_mechanics_contourtype
        self.build_comboBox(
            self.ui.comboBox_settings_mechanics_contourtype, "contour_type_opts"
        )
        self.ui.comboBox_settings_mechanics_contourtype.currentIndexChanged.connect(
            self.update_widget
        )
        self.ui.comboBox_settings_mechanics_contourtype.currentIndexChanged.connect(
            self.make_contours
        ...
        self.build_comboBox(
            self.ui.comboBox_settings_mechanics_contourcmap, "contour_cmap_opts"
        )
        self.ui.comboBox_settings_mechanics_contourcmap.currentIndexChanged.connect(
            self.update_widget
        )
        self.ui.comboBox_settings_mechanics_contourcmap.currentIndexChanged.connect(
            self.make_contours
        ...
        self.ui.tableWidget_settings_mechanics_contoursettings.itemChanged.connect(
            self.on_changed_mech_contour_lim_tab
        )

        self.ui.pushButton_settings_mechanics_contourplot.clicked.connect(
        ...
        self.ui.frame_spectra_fit_polar.setLayout(
            self.set_frame_layout(self.ui.mpl_spectra_fit_polar)
        )
        ...
        self.ui.frame_spectra_fit.setLayout(
            self.set_frame_layout(self.ui.mpl_spectra_fit)
        )
        # connect signal
        self.ui.mpl_spectra_fit.ax[0].cidx = self.ui.mpl_spectra_fit.ax[
            0
        ].callbacks.connect("xlim_changed", self.on_fit_lims_change)
        self.ui.mpl_spectra_fit.ax[0].cidy = self.ui.mpl_spectra_fit.ax[
            0
        ...
        self.ui.horizontalSlider_spectra_fit_spanctrl.valueChanged.connect(
            self.on_changed_slider_spanctrl
        )
        self.ui.horizontalSlider_spectra_fit_spanctrl.sliderReleased.connect(
            self.on_released_slider_spanctrl
        )
        self.ui.horizontalSlider_spectra_fit_spanctrl.actionTriggered.connect(
            self.on_acctiontriggered_slider_spanctrl
        )

        # pushButton_spectra_fit_refresh
        self.ui.pushButton_spectra_fit_refresh.clicked.connect(
            self.on_clicked_pushButton_spectra_fit_refresh
        )
        self.ui.pushButton_spectra_fit_showall.clicked.connect(
            self.on_clicked_pushButton_spectra_fit_showall
        )
        self.ui.pushButton_spectra_fit_fit.clicked.connect(
            self.on_clicked_pushButton_spectra_fit_fit
        )
        ...
        self.ui.spinBox_spectra_mechanics_currid.valueChanged.connect(
            self.on_changed_spinBox_spectra_mechanics_currid
        )
        ...
        self.ui.frame_legend.setFixedHeight(
            int(mpl_legend_p.p1[1] - mpl_legend_p.p0[1])
        )
        ...
        # selector menu
        self.ui.mpl_plt1.canvas.setContextMenuPolicy(
            Qt.ContextMenuPolicy.CustomContextMenu
        )
        self.ui.mpl_plt1.canvas.customContextMenuRequested.connect(
            lambda position,
            mpl=self.ui.mpl_plt1,
            plt_str="plt1": self.mpl_data_open_custom_menu(position, mpl, plt_str)
        )

        self.ui.mpl_plt2.canvas.setContextMenuPolicy(
            Qt.ContextMenuPolicy.CustomContextMenu
        )
        self.ui.mpl_plt2.canvas.customContextMenuRequested.connect(
            lambda position,
            mpl=self.ui.mpl_plt2,
            plt_str="plt2": self.mpl_data_open_custom_menu(position, mpl, plt_str)
        )
        ...
        self.ui.radioButton_data_showmarked.toggled["bool"].connect(
            self.set_mpl_lm_style
        )  # when toggled clicked, this toggled too.
        ...
        for harm in self.all_harm_list(as_str=True):
            getattr(self.ui, "checkBox_plt1_h" + harm).stateChanged.connect(
                self.update_widget
            )
            getattr(self.ui, "checkBox_plt1_h" + harm).stateChanged.connect(
                self.update_mpl_plt1
            )
            getattr(self.ui, "checkBox_plt1_h" + harm).stateChanged.connect(
                self.clr_mpl_harm
            )

            getattr(self.ui, "checkBox_plt2_h" + harm).stateChanged.connect(
                self.update_widget
            )
            getattr(self.ui, "checkBox_plt2_h" + harm).stateChanged.connect(
                self.update_mpl_plt2
            )
            getattr(self.ui, "checkBox_plt2_h" + harm).stateChanged.connect(
                self.clr_mpl_harm
            )
        ...
        self.ui.mpl_contour1 = MatplotlibWidget(
            parent=self.ui.frame_spectra_mechanics_contour1, axtype="contour"
        )
        self.ui.frame_spectra_mechanics_contour1.setLayout(
            self.set_frame_layout(self.ui.mpl_contour1)
        )
        ...
        self.ui.frame_spectra_mechanics_contour2.setLayout(
            self.set_frame_layout(self.ui.mpl_contour2)
        )
        ...
        self.ui.progressBar_status_interval_time.setAlignment(
        ...
        self.ui.menu_settings_data_refit = QMenu(
            self.ui.toolButton_settings_data_regenerate_from_raw
        )
        ...
        self.ui.menu_settings_mechanics_solve = QMenu(
            self.ui.toolButton_settings_mechanics_solve
        )
        ...
        self.ui.menu_spectra_mechanics_plotrows = QMenu(
            self.ui.toolButton_spectra_mechanics_plotrows
        )
        ...
        # add menu to toolbutton
        self.ui.toolButton_spectra_mechanics_plotrows.setMenu(
            self.ui.menu_spectra_mechanics_plotrows
        )
        ...
        self.ui.actionExport_Settings.triggered.connect(
            self.on_triggered_export_settings
        )
        ...
        # import QCM-D
        self.ui.actionImport_QCM_D.triggered.connect(
            self.on_triggered_actionImport_QCM_D
        )
        # import QCM-Z
    ...
    def find_text_item(self, parent, text):
        ...
        if not, return None
        """
        item = parent.findItems(
            text, Qt.MatchFlag.MatchExactly | Qt.MatchFlag.MatchRecursive, 0
        )
        if len(item) == 1:
            item = item[0]
        else:
    ...
    def on_clicked_pushButton_runstop(self, checked):
        if checked:
            ...
            if (
                not self.settings["checkBox_activechn_samp"]
                and not self.settings["checkBox_activechn_ref"]
            ):
            ...
            if not self.data_saver.path:  # no filename
                if self.tempPath:  # new file name is set
                    ...
                    path = os.path.abspath(
                        os.path.join(
                            config_default["unsaved_path"],
                            datetime.datetime.now().strftime(
                                config_default["unsaved_filename"]
                            )
                            + ".h5",
                        )
                    )
            ...
            if (
                not self.ui.pushButton_resetreftime.isVisible()
                and not self.ui.dateTimeEdit_reftime.isVisible()
                and self.ui.pushButton_resetreftime.isEnabled()
                and self.ui.dateTimeEdit_reftime.isEnabled()
            ):
                self.reset_reftime()  # use current time as t0

            # disable features
            self.disable_widgets("pushButton_runstop_disable_list")
    ...
    def process_saving_when_stop(self):
        ...
        # enable features
        self.enable_widgets("pushButton_runstop_enable_list")
    ...
    def on_dateTimeChanged_dateTimeEdit_reftime(self, datetime):
        ...
        self.settings["dateTimeEdit_reftime"] = (
            self.ui.dateTimeEdit_reftime.dateTime()
            .toPyDateTime()
    ...
    def on_dateTimeChanged_dateTimeEdit_t0shifted(self, datetime):
        ...
        self.settings["dateTimeEdit_settings_data_t0shifted"] = (
            self.ui.dateTimeEdit_settings_data_t0shifted.dateTime()
            .toPyDateTime()
            .strftime(config_default["time_str_format"])
        )
    ...
    def reset_shiftedt0(self):
        ...
        self.ui.dateTimeEdit_settings_data_t0shifted.setDateTime(
            datetime.datetime.strptime(
                self.settings["dateTimeEdit_reftime"], config_default["time_str_format"]
            )
        )
    ...
    def save_data_saver_chn_idx(self, chn_name):
        ...
        chn_idx = self.data_saver.get_idx(
            chn_name
    ...
    def save_data_saver_refsource(self, chn_name):
        ...
        if (not ref_idx) and (
            list(self.data_saver.get_queue_id("samp"))
            != list(self.data_saver.get_queue_id("ref"))
        ):  # samp and ref were not collected together
            ref_idx = [0]
    ...
    def on_triggered_actionOpen_MyVNA(self):
        ...
        if myvna_path and os.path.exists(
            myvna_path
        ):  # user defined myVNA.exe path exists and correct
    ...
    def on_triggered_actionImport_QCM_D(self):
        ...
        fileName = self.openFileNameDialog(
            title="Choose an existing file to append",
            filetype=config_default["external_qcm_datafiletype"],
        )  # !! add path of last opened folder

        if fileName:
            self.data_saver.import_qcm_with_other_format(
                "qcmd", fileName, config_default, settings=self.settings
            )
    ...
    def on_triggered_actionImport_QCM_Z(self):
        ...
        if fileName:
            self.data_saver.import_qcm_with_other_format(
                "qcmz", fileName, config_default, settings=self.settings
            )
    ...
    def msg_about(self):
        ...
        """
        msg_text = []
        msg_text.append(f"Version: {_version.__version__}")
        msg_text.append("Authors: {}".format(" ,".join(_version.__authors__)))
        msg_text.append(f"Contact: {_version.__contact__}")
        msg_text.append(f"Copyright: {_version.__copyright__}")
        msg_text.append(
            f"Source: <a href='{_version.__source__}'>{_version.__source__}</a>"
        )
        msg_text.append(
            f"Report issues: <a href='{_version.__report__}'>{_version.__report__}</a>"
        )
    ...
    def set_recording_time(self):
        ...
        if sender == "spinBox_recordinterval":
            ...
            self.ui.spinBox_scaninterval.setValue(
                int(record_interval / refresh_resolution)
            )
        elif sender in ["spinBox_refreshresolution", "spinBox_scaninterval", None]:
            # change record interval
            self.ui.spinBox_recordinterval.setValue(
                int(scaninterval * refresh_resolution)
            )
        ...
        if fileName:
            ...
            if not self.data_saver.settings:  # failed to load file
                process = self.process_messagebox(
                    ...
                    message=[
                        f"Failed to load file:\n{fileName}\nPlease check if the right file is selected!"
                    ],
                    opts=False,
                    forcepop=True,
                )
        ...
        if fileName:
            ...
            if ext == ".json":
                with open(fileName, "w") as f:
                    ...
                    settings.pop(
                        "dateTimeEdit_settings_data_t0shifted", None
                    )  # time shift
    ...
    def on_triggered_actionSave(self):
        ...
        if self.data_saver.path:  # there is file
            ...
            logger.warning("No file information!")
    ...
    def on_triggered_actionSave_As(self):
        ...
        fileName = self.saveFileDialog(
            title="Choose a new file",
            filetype=config_default["default_datafiletype"],
            path=self.data_saver.path,
        )  # !! add path of last opened folder
    ...
    def on_triggered_actionExport(self):
        ...
        fileName = self.saveFileDialog(
            title="Choose a file and data type",
            filetype=config_default["export_datafiletype"],
            path=self.data_saver.path,
        )  # !! add path of last opened folder
        # codes for data exporting
        if fileName:
    ...
    def on_triggered_actionClear_All(self):
        ...
        process = self.process_messagebox(
            message=["All data in the file will be deleted!"], forcepop=True
        )
    ...
    def on_triggered_del_menu(self, chn_name, plt_harms, sel_idx_dict, mode, marks):
        ...
        process = self.process_messagebox(
            message=[
                f"Raw data of \n{alert_dic.get(mode, None)}\nin the file will be PERMANENTLY DELETED!"
            ],
            forcepop=True,
        )
        ...
        self.data_saver.selector_del_sel(
            chn_name, UIModules.sel_ind_dict(plt_harms, sel_idx_dict, mode, marks)
        )
    ...
    def on_changed_slider_spanctrl(self):
        ...
        if n >= 1:
            # n = f'{round(n)} *'
            n = "{} *".format(
                min(config_default["span_ctrl_steps"], key=lambda x: abs(x - n))
            )  # python < 3.5
        else:
            # n = f'1/{round(1/n)} *'
            n = "1/{} *".format(
                min(config_default["span_ctrl_steps"], key=lambda x: abs(x - 1 / n))
            )  # python < 3.5
    ...
    def spectra_fit_get_data(self):
        ...
        if self.get_spectraTab_mode() == "center":  # for peak centering
            ...
            logger.warning("Change Tab to Settings or Data to active the function.")
    ...
    def get_vna_data(self, harm=None, chn_name=None):
        ...
        steps = int(
            self.get_harmdata("lineEdit_scan_harmsteps", harm=harm, chn_name=chn_name)
        )
        setflg = self.vna_tracker.set_check(
            f=freq_span, steps=steps, chn=self.get_chn_by_name(chn_name)
        )
        logger.info(setflg)
        ...
        with self.vna:
            ...
            if ret == 0:
                ...
                logger.error("There is an error while setting VNA!")
        return f, G, B
    ...
    def tab_spectra_fit_update_mpls(self, f, G, B):
        ...
        if (f is not None) and (len(f) > 0) and (f[0] != f[-1]):  # f is available
            ...
            self.ui.mpl_spectra_fit.ax[0].set_ylim(
                min(G) - 0.05 * (max(G) - min(G)), max(G) + 0.05 * (max(G) - min(G))
            )
    ...
    def update_lineedit_fit_span(self, f):
        ...
        if f is not None:
            ...
            self.ui.lineEdit_spectra_fit_span.setText(
                UIModules.num2str((span / 1000), precision=5)
            )  # in kHz
        else:
            self.ui.lineEdit_spectra_fit_span.setText("")
    ...
    def mpl_connect_cid(self, mpl, fun, axis="xy"):
        ...
        if "y" in axis:
            mpl.ax[0].cidy = self.ui.mpl_spectra_fit.ax[0].callbacks.connect(
                "ylim_changed", fun
            )
    ...
    def on_clicked_pushButton_spectra_fit_fit(self):
        ...
        self.peak_tracker.update_input(
            ...
            freq_span=self.settings["freq_span"],
            fGB=[data_lG[0], data_lG[1], data_lB[1]],
        )
        ...
        self.ui.mpl_spectra_fit.update_data(
            ...
            {"ln": "lBfit", "x": data_lB[0], "y": fit_result["fit_b"]},
        )
        self.ui.mpl_spectra_fit_polar.update_data(
            {"ln": "lfit", "x": fit_result["fit_g"], "y": fit_result["fit_b"]}
        )
        ...
        self.ui.mpl_spectra_fit.add_temp_lines(
            ...
            ylist=fit_result["comp_g"],
        )
        self.ui.mpl_spectra_fit_polar.add_temp_lines(
            self.ui.mpl_spectra_fit_polar.ax[0],
            xlist=fit_result["comp_g"],
            ylist=fit_result["comp_b"],
        )

        # logger.info('fit_result.comp_g', fit_result['comp_g'])

        # update lsp
        factor_span = self.peak_tracker.get_output(
            key="factor_span",
            chn_name=self.settings_chn["name"],
            harm=self.settings_harm,
        )
        ...
        if self.get_spectraTab_mode() == "center":  # center mode
            ...
            cen_trk_G = self.peak_tracker.get_output(
                key="gmod", chn_name=self.settings_chn["name"], harm=self.settings_harm
            ).eval(
                self.peak_tracker.get_output(
                    key="params",
                    chn_name=self.settings_chn["name"],
                    harm=self.settings_harm,
                ),
        ...
        cen_rec_G = self.peak_tracker.get_output(
            key="gmod", chn_name=self.settings_chn["name"], harm=self.settings_harm
        ).eval(
            self.peak_tracker.get_output(
                key="params",
        ...
        self.ui.plainTextEdit_spectra_fit_result.setPlainText(
            self.peak_tracker.fit_result_report()
        )
    ...
    def regenerate_from_raw(self, chn_name="samp", mode="all"):
        ...
        process = self.process_messagebox(
            text="Your selection was paused!", message=[msg], opts=True, forcepop=True
        )
        ...
        for queue_id in chn_queue_list:
            ...
            # get harms
            harms = self.data_saver.get_queue_id_harms_from_raw(
                chn_name, queue_id
            )  # list of strings
            # add empty data
            self.data_saver._append_new_queue([chn_name], queue_id=queue_id)
            # get index of queue_id in data
            ind = self.data_saver.get_queue_id(chn_name).index[
                -1
            ]  # the empty just appended should be the last one
    ...
    def get_active_queueid_from_l_harm_ind(self):
        ...
        queue_list = self.data_saver.get_marked_harm_queue_id(
    ...
    def get_active_raw(self):
        ...
        """
        queue_id = self.get_active_queueid_from_l_harm_ind()
        f, G, B = self.data_saver.get_raw(
            self.active["chn_name"], queue_id, self.active["harm"]
        )
    ...
    def init_manual_refit(self):
        ...
        if self.ui.pushButton_manual_refit.isChecked():
            ...
            self.settings["freq_span"]["refit"] = self.settings["freq_span"][
                self.active["chn_name"]
            ]
            self.settings["harmdata"]["refit"] = self.settings["harmdata"][
                self.active["chn_name"]
            ]
            ...
            self.ui.tabWidget_settings_settings_harm.setCurrentIndex(
                self.ui.tabWidget_settings_settings_harm.indexOf(
                    getattr(self.ui, "tab_settings_settings_harm" + self.active["harm"])
                )
            )  # set to active harm and emit update_settings_chn
    ...
    def add_manual_refit_tab(self, signal):
        ...
        if signal:
            if (
                self.ui.tabWidget_settings_settings_samprefchn.currentIndex()
                != self.ui.tabWidget_settings_settings_samprefchn.indexOf(
                    self.ui.tab_settings_settings_harmchnrefit
                )
            ):  # refit is current tab
                self.ui.tabWidget_settings_settings_samprefchn.addTab(
                    self.ui.tab_settings_settings_harmchnrefit, "Refit"
                )
                self.ui.tabWidget_settings_settings_samprefchn.setCurrentWidget(
                    self.ui.tab_settings_settings_harmchnrefit
                )
        else:
            self.ui.tabWidget_settings_settings_samprefchn.removeTab(
                self.ui.tabWidget_settings_settings_samprefchn.indexOf(
                    self.ui.tab_settings_settings_harmchnrefit
                )
            )
    ...
    def get_plt_opt(self, plt_str):
        ...
        """
        return [
            self.settings.get("comboBox_" + plt_str + "_optsy"),
            self.settings.get("comboBox_" + plt_str + "_optsx"),
        ]  # use the first one if failed
    ...
    def get_plt_harms(self, plt_str):
        ...
        return [
            harm
            for harm in self.all_harm_list(as_str=True)
            if self.settings.get("checkBox_" + plt_str + "_h" + harm, False)
        ]
    ...
    def update_mpl_dataplt(self, plt_str="none"):
        ...
        for plt_harm in plt_harms:
            # get y data
            harm_ydata = self.get_harm_data_by_typestr(
                plt_opt[0],
                plt_chnname,
                plt_harm,
                mark=mark,
            ...
            harm_xdata = self.get_harm_data_by_typestr(
                plt_opt[1],
                plt_chnname,
                plt_harm,
                mark=mark,
            ...
            data_list.append(
                {"ln": line_group + plt_harm, "x": harm_xdata, "y": harm_ydata}
            )
            ...
            if config_default["show_marked_when_all"]:
                ...
                if self.settings["radioButton_data_showall"]:
                    if self.data_saver.with_marks(plt_chnname):
                        mark_list = (
                            self.data_saver.get_harm_marks(plt_chnname, plt_harm) == 1
                        )
                    ...
                    if isinstance(mark_list, pd.Series):
                        data_list.append(
                            {
                                "ln": "lm" + plt_harm,
                                "x": harm_xdata[mark_list],
                                "y": harm_ydata[mark_list],
                            }
                        )
    ...
    def prepare_harm_data_for_mpl_update(
        self,
        plt_chnname,
        plt_harms,
        line_group,
        ...
        if show_marked_when_all:
            mark_df = self.data_saver.get_list_column_to_columns_marked_rows(
                plt_chnname,
                "marks",
                mark=False,
        ...
        for harm in plt_harms:  # selected
            ...
            if len(xdata.shape) == 1:  # one column e.g.: tuple (1,) is series
                ...
                harm_xdata = xdata.filter(regex=rf"\D{harm}$", axis=1).squeeze(
                    axis=1
                )  # convert to series
            ...
            if len(ydata.shape) == 1:  # series
                ...
                harm_ydata = ydata.filter(regex=rf"\D{harm}$", axis=1).squeeze(
                    axis=1
                )  # convert to series

            data_list.append(
                {"ln": line_group + harm, "x": harm_xdata, "y": harm_ydata}
            )
    ...
    def prepare_harm_data_for_mpl_prop_update(
        self,
        plt_chnname,
        plt_harms,
        line_group,
        ...
        for harm in plt_harms:  # selected
            ...
            if xerr is not None:  # there is err data
                if len(xdata.shape) == 1:  # one column e.g.: tuple (1,) is series
                    ...
                    harm_xerr = xerr.filter(regex=rf"\D{harm}$", axis=1).squeeze(
                        axis=1
                    )  # convert to series
            # set ydata for harm
            if len(ydata.shape) == 1:  # series
            ...
            if yerr is not None:  # there is err data
                if len(ydata.shape) == 1:  # series
                    ...
                    harm_yerr = yerr.filter(regex=rf"\D{harm}$", axis=1).squeeze(
                        axis=1
                    )  # convert to series
                ...
                if self.settings["radioButton_data_showall"]:
                    ...
                    if isinstance(mark_list, pd.Series):
                        if (xerr is not None) or (yerr is not None):  # exist error
                            data_list.append(
                                {
                                    "ln": line_group + "m" + harm,
                                    "x": harm_xdata[mark_list],
                                    "y": harm_ydata[mark_list],
                                    "xerr": harm_xerr[mark_list],
                                    "yerr": harm_yerr[mark_list],
                                }
                            )
                        else:
                            data_list.append(
    ...
    def get_harm_data_by_typestr(
        self, typestr, chn_name, harm, mark=False, unit_t=None, unit_temp=None
    ):
        """
        ...
        if typestr in ["df", "delf_exps"]:  # get delf
            ...
            data = self.data_saver.get_marked_harm_col_from_list_column(
                chn_name, harm, "gs", deltaval=True, norm=False, mark=mark
            )
        elif "dfn" == typestr:  # get delfn
            data = self.data_saver.get_marked_harm_col_from_list_column(
                chn_name, harm, "fs", deltaval=True, norm=True, mark=mark
            )
            ...
            data = self.data_saver.get_marked_harm_col_from_list_column(
                chn_name, harm, "gs", deltaval=True, norm=True, mark=mark
            )
            ...
            # get delg first
            delf = self.data_saver.get_marked_harm_col_from_list_column(
                chn_name, harm, "fs", deltaval=True, norm=False, mark=mark
            )
            f1 = self.data_saver.get_harm_marked_f1(chn_name, harm, mark=mark)
            Zq = self.qcm.Zq
            ...
            data = self.data_saver.get_marked_harm_col_from_list_column(
                chn_name, harm, "fs", deltaval=False, norm=False, mark=mark
            )
        elif "g" == typestr:  # get g
            data = self.data_saver.get_marked_harm_col_from_list_column(
                chn_name, harm, "gs", deltaval=False, norm=False, mark=mark
            )
        elif "p" == typestr:  # get p
            data = self.data_saver.get_marked_harm_col_from_list_column(
                chn_name, harm, "ps", deltaval=False, norm=False, mark=mark
            )
        elif "gp" == typestr:  # get gp
            data = self.data_saver.get_marked_harm_col_from_list_column(
                chn_name, harm, "ps", deltaval=False, norm=False, mark=mark
            ) * self.data_saver.get_marked_harm_col_from_list_column(
                chn_name, harm, "gs", deltaval=False, norm=False, mark=mark
            )
            ...
            data = self.data_saver.get_marked_harm_t(
                chn_name, harm, mark=mark, unit=unit_t
            )
        elif "temp" == typestr:  # get temp
            data = self.data_saver.get_marked_harm_temp(
                chn_name, harm, mark=mark, unit=unit_temp
            )
    ...
    def get_data_by_typestr(
        ...
        if typestr in ["df", "delf_exps"]:  # get delf
            ...
            data = self.data_saver.get_list_column_to_columns_marked_rows(
                ...
                deltaval=False,
                norm=False,
            ) * self.data_saver.get_list_column_to_columns_marked_rows(
                chn_name,
                "gs",
                mark=mark,
                dropnanmarkrow=False,
            ...
            data = self.data_saver.get_t_marked_rows(
                chn_name, dropnanmarkrow=False, unit=unit_t
            )
        elif "temp" == typestr:  # get temp
            data = self.data_saver.get_temp_by_uint_marked_rows(
                chn_name, dropnanmarkrow=False, unit=unit_temp
            )
        elif "id" == typestr:  # get queue_id
            data = self.data_saver.get_queue_id_marked_rows(
                chn_name, dropnanmarkrow=False
            )
        elif "idx" == typestr:  # get indices
    ...
    def update_data_axis(self, signal):
        ...
        if ("plt1" in sender_name) or (
            "plt2" in sender_name
        ):  # signal sent from one of the plots
            ...
            if plt_opt[0] not in [
                "t",
                "temp",
            ]:  # other type in y-axis w/o changing the unit
            ...
            if plt_opt[1] not in [
                "t",
                "temp",
            ]:  # other type in x-axis w/o changing the unit
        ...
        if (
            sender_name == "comboBox_timeunit"
        ):  # update both axises of mpl_plt1 & mpl_plt2
            for plt_str in ["plt1", "plt2"]:
                # plot option str in list [y, x]
                plt_opt = self.get_plt_opt(plt_str)
                if (
                    "t" in plt_opt
                ):  # there is time axis in the plot (plt_opt[0] == 't' or plt_opt[1] == 't')
                    self.update_time_unit(plt_str, plt_opt)
                    # getattr(self.ui, 'mpl_' + plt_str).canvas.draw()

        if (
            sender_name == "comboBox_tempunit"
        ):  # update both axises of mpl_plt1 & mpl_plt2
            for plt_str in ["plt1", "plt2"]:
                # plot option str in list [y, x]
                plt_opt = self.get_plt_opt(plt_str)
        ...
        if (
            sender_name == "comboBox_xscale"
        ):  # update both axises of mpl_plt1 & mpl_plt2
            for plt_str in ["plt1", "plt2"]:
                # plot option str in list [y, x]
                getattr(self.ui, "mpl_" + plt_str).ax[0].set_xscale(
                    self.sender().itemData(signal)
                )
                getattr(self.ui, "mpl_" + plt_str).canvas.draw()

        if (
            sender_name == "comboBox_yscale"
        ):  # update both axises of mpl_plt1 & mpl_plt2
            for plt_str in ["plt1", "plt2"]:
                # plot option str in list [y, x]
                getattr(self.ui, "mpl_" + plt_str).ax[0].set_yscale(
                    self.sender().itemData(signal)
                )
                getattr(self.ui, "mpl_" + plt_str).canvas.draw()
        ...
        if sender_name == "checkBox_linkx":  # link x axis of mpl_plt1 & mpl_plt2
            if signal:
                ...
                try:
                    self.ui.mpl_plt2.ax[0]._shared_axes["x"].remove(
    ...
    def clr_mpl_harm(self):
        ...
        if not self.settings[sender]:  # unchecked
            self.clr_mpl_l(
                str_list[1], line_group_list=["l", "lm"], harm_list=[sender[-1]]
            )  # sender[-1] is the harm from checkBox_plt<n>_h<harm>
    ...
    def mpl_data_open_selector_menu(self, position, mpl, plt_str):
        ...
        marks = self.data_saver.get_marks(
            chn_name, tocolumns=True
        )  # df of boolean shows if has data
        ...
        actionMark_all.triggered.connect(
            lambda: self.data_saver.selector_mark_sel(
                chn_name,
                UIModules.sel_ind_dict(plt_harms, sel_idx_dict, "all", marks),
                1,
            )
        )
        if selflg:
            actionMark_selpts = QAction("Mark selected points", self)
            actionMark_selpts.triggered.connect(
                lambda: self.data_saver.selector_mark_sel(
                    chn_name,
                    UIModules.sel_ind_dict(plt_harms, sel_idx_dict, "selpts", marks),
                    1,
                )
            )
            actionMark_selidx = QAction("Mark selected indices", self)
            actionMark_selidx.triggered.connect(
                lambda: self.data_saver.selector_mark_sel(
                    chn_name,
                    UIModules.sel_ind_dict(plt_harms, sel_idx_dict, "selidx", marks),
                    1,
                )
            )
            actionMark_selharm = QAction("Mark selected harmonics", self)
            actionMark_selharm.triggered.connect(
                lambda: self.data_saver.selector_mark_sel(
                    chn_name,
                    UIModules.sel_ind_dict(plt_harms, sel_idx_dict, "selharm", marks),
                    1,
                )
            )
        ...
        actionUnmark_all.triggered.connect(
            lambda: self.data_saver.selector_mark_sel(
                chn_name,
                UIModules.sel_ind_dict(plt_harms, sel_idx_dict, "all", marks),
                0,
        ...
        if selflg:
            ...
            actionUnmark_selpts.triggered.connect(
                lambda: self.data_saver.selector_mark_sel(
                    chn_name,
                    UIModules.sel_ind_dict(plt_harms, sel_idx_dict, "selpts", marks),
                    0,
            ...
            actionUnmark_selidx.triggered.connect(
                lambda: self.data_saver.selector_mark_sel(
                    chn_name,
                    UIModules.sel_ind_dict(plt_harms, sel_idx_dict, "selidx", marks),
                    0,
            ...
            actionUnmark_selharm.triggered.connect(
                lambda: self.data_saver.selector_mark_sel(
                    chn_name,
                    UIModules.sel_ind_dict(plt_harms, sel_idx_dict, "selharm", marks),
                    0,
        ...
        actionDel_all.triggered.connect(
            lambda: self.on_triggered_del_menu(
                chn_name, plt_harms, sel_idx_dict, "all", marks
            )
        )
        if selflg:
            actionDel_selpts = QAction("Delete selected points", self)
            actionDel_selpts.triggered.connect(
                lambda: self.on_triggered_del_menu(
                    chn_name, plt_harms, sel_idx_dict, "selpts", marks
                )
            )
            actionDel_selidx = QAction("Delete selected indices", self)
            actionDel_selidx.triggered.connect(
                lambda: self.on_triggered_del_menu(
                    chn_name, plt_harms, sel_idx_dict, "selidx", marks
                )
            )
            actionDel_selharm = QAction("Delete selected harmonics", self)
            actionDel_selharm.triggered.connect(
                lambda: self.on_triggered_del_menu(
                    chn_name, plt_harms, sel_idx_dict, "selharm", marks
                )
            )
        ...
        actionRefit_all.triggered.connect(
            lambda: self.data_refit(
                chn_name, UIModules.sel_ind_dict(plt_harms, sel_idx_dict, "all", marks)
            )
        )
        if selflg:
            actionRefit_selpts = QAction("Refit selected points", self)
            actionRefit_selpts.triggered.connect(
                lambda: self.data_refit(
                    chn_name,
                    UIModules.sel_ind_dict(plt_harms, sel_idx_dict, "selpts", marks),
                )
            ...
            actionRefit_selidx.triggered.connect(
                lambda: self.data_refit(
                    chn_name,
                    UIModules.sel_ind_dict(plt_harms, sel_idx_dict, "selidx", marks),
                )
            ...
            actionRefit_selharm.triggered.connect(
                lambda: self.data_refit(
                    chn_name,
                    UIModules.sel_ind_dict(plt_harms, sel_idx_dict, "selharm", marks),
                )
    ...
    def marknpts(self):
        ...
        chn_idx = self.data_saver.get_idx(
            chn_name
        ).values  # use list of index for comperison (ndarray)
        # get channel t
    ...
    def film_construction_mode_switch(self):
        ...
        self.ui.stackedWidget_settings_mechanics_modeswitch.setCurrentIndex(
            (currInd + 1) % count
        )
    ...
    def build_mech_layers(self):
        ...
        if (
            ...
            self.get_mechchndata("radioButton_mech_expertmode_calc_0", mech_chn="ref")
            is not None
        ):
            bottom_row = self.ui.gridLayout_mech_expertmode_layers.getItemPosition(
                self.ui.gridLayout_mech_expertmode_layers.indexOf(
                    self.ui.radioButton_mech_expertmode_calc_0
                )
            )[0]
        ...
        if pre_nlayers == nlayers:
            ...
            for i in range(pre_nlayers, nlayers):
                ...
                setattr(
                    self.ui,
                    "radioButton_mech_expertmode_calc_" + str(i),
                    QRadioButton(self.ui.stackedWidgetPage_mech_expertmode),
                )
                getattr(
                    self.ui, "radioButton_mech_expertmode_calc_" + str(i)
                ).setObjectName("radioButton_mech_expertmode_calc_" + str(i))
                self.ui.gridLayout_mech_expertmode_layers.addWidget(
                    getattr(self.ui, "radioButton_mech_expertmode_calc_" + str(i)),
                    nlayers - i,
                    0,
                    1,
                    1,
                ...
                if i == 0:
                    getattr(
                        self.ui, "radioButton_mech_expertmode_calc_" + str(i)
                    ).setText(QCoreApplication.translate("MainWindow", "Electrode"))
                    getattr(
                        self.ui, "radioButton_mech_expertmode_calc_" + str(i)
                    ).setCheckable(False)
                    ...
                    getattr(
                        ...
                        QCoreApplication.translate("MainWindow", "layer " + str(i))
                    )

                # combobox comboBox_mech_expertmode_source_
                setattr(
                    self.ui,
                    "comboBox_mech_expertmode_source_" + str(i),
                    QComboBox(self.ui.stackedWidgetPage_mech_expertmode),
                )
                sizePolicy = QSizePolicy(
                    QSizePolicy.Policy.Preferred, QSizePolicy.Policy.Fixed
                )
                ...
                sizePolicy.setVerticalStretch(0)
                sizePolicy.setHeightForWidth(
                    getattr(self.ui, "comboBox_mech_expertmode_source_" + str(i))
                    .sizePolicy()
                    .hasHeightForWidth()
                )
                getattr(
                ...
                getattr(
                    self.ui, "comboBox_mech_expertmode_source_" + str(i)
                ).setObjectName("comboBox_mech_expertmode_source_" + str(i))
                self.ui.gridLayout_mech_expertmode_layers.addWidget(
                    getattr(self.ui, "comboBox_mech_expertmode_source_" + str(i)),
                    nlayers - i,
                    1,
                ...
                setattr(
                    self.ui,
                    "comboBox_mech_expertmode_indchn_" + str(i),
                    QComboBox(self.ui.stackedWidgetPage_mech_expertmode),
                )
                getattr(
                    self.ui, "comboBox_mech_expertmode_indchn_" + str(i)
                ).setObjectName("comboBox_mech_expertmode_indchn_" + str(i))
                self.ui.gridLayout_mech_expertmode_layers.addWidget(
                    getattr(self.ui, "comboBox_mech_expertmode_indchn_" + str(i)),
                    nlayers - i,
                    2,
                    1,
                    1,
                ...
                setattr(
                    self.ui,
                    "lineEdit_mech_expertmode_value_" + str(i),
                    QLineEdit(self.ui.stackedWidgetPage_mech_expertmode),
                )
                # getattr(self.ui, 'lineEdit_mech_expertmode_value_'+str(i)).setReadOnly(True)
                getattr(
                    self.ui, "lineEdit_mech_expertmode_value_" + str(i)
                ).setObjectName("lineEdit_mech_expertmode_value_" + str(i))
                ...
                getattr(
                    self.ui, "lineEdit_mech_expertmode_value_" + str(i)
                ).textChanged.connect(self.update_mechchnwidget)

                # combobox: add signal/slot
                getattr(
                    self.ui, "comboBox_mech_expertmode_indchn_" + str(i)
                ).currentIndexChanged.connect(self.on_mech_layer_indchn_changed)
                getattr(
                    self.ui, "comboBox_mech_expertmode_indchn_" + str(i)
                ).currentIndexChanged.connect(self.update_mechchnwidget)

                # combobox: add signal/slot
                getattr(
                    self.ui, "comboBox_mech_expertmode_source_" + str(i)
                ).currentIndexChanged.connect(self.on_mech_layer_source_changed)
                getattr(
                    self.ui, "comboBox_mech_expertmode_source_" + str(i)
                ).currentIndexChanged.connect(self.update_mechchnwidget)
                ...
                getattr(
                    self.ui, "radioButton_mech_expertmode_calc_" + str(i)
                ).toggled.connect(self.on_mech_layer_calc_changed)
                getattr(
                    self.ui, "radioButton_mech_expertmode_calc_" + str(i)
                ).toggled.connect(self.update_mechchnwidget)
        ...
        for i in range(nlayers):
            ...
            self.ui.gridLayout_mech_expertmode_layers.addWidget(
                getattr(self.ui, "lineEdit_mech_expertmode_value_" + str(i)),
                (rowcount - 1 - i + del_nlayers),
                3,
                1,
                1,
    ...
    def on_mech_chn_changed(self):
        ...
        self.load_comboBox(
            self.ui.comboBox_settings_mechanics_model_samplayer_chn, val=self.mech_chn
        )
    ...
    def on_mech_layer_calc_changed(self, signal):
        ...
        if signal:  # is checked
            # set mechchandata
            self.set_mechchndata(
                "comboBox_mech_expertmode_indchn_" + layer_num, val=self.mech_chn
            )
            # reload the comboBox
            self.load_comboBox(
                getattr(self.ui, "comboBox_mech_expertmode_indchn_" + layer_num),
                mech_chn=self.mech_chn,
            )
    ...
    def on_mech_layer_source_changed(self, signal):
        ...
        if sender_val == "ind":  # use index
            ...
            getattr(self.ui, "comboBox_mech_expertmode_indchn_" + layer_num).setVisible(
                True
            )
            ...
            if sender_val == "prop":  # use property
                getattr(self.ui, "lineEdit_mech_expertmode_value_" + layer_num).setText(
                    "{0}''grho': {grho}, 'phi': {phi}, drho': {drho}, 'n': {n}{1}".format(
                        "{", "}", **QCM.prop_default["air"]
                    )
                )
    ...
    def make_film_layers_dict(self):
        ...
        for n in range(n_layers):  # all layers
            ...
            film_dict[n] = {
                key: self.get_mechchndata(pre_name + n)
    ...
    def load_film_layers_widgets(self):
        ...
        for n in range(layernum + 1):  # all layers
            ...
            if (
                self.get_mechchndata("radioButton_mech_expertmode_calc_" + n) is None
            ):  # no data saved for this layer or new layer
                # save current value to self.settings
                self.set_mechchndata(
                    "lineEdit_mech_expertmode_value_" + n,
                    getattr(self.ui, "lineEdit_mech_expertmode_value_" + n).text(),
                )
                # save current value to self.settings
                self.set_mechchndata(
                    "comboBox_mech_expertmode_indchn_" + n,
                    getattr(self.ui, "comboBox_mech_expertmode_indchn_" + n).itemData(
                        getattr(
                            self.ui, "comboBox_mech_expertmode_indchn_" + n
                        ).currentIndex()
                    ),
                )
                ...
                # save current value to self.settings
                self.set_mechchndata(
                    "comboBox_mech_expertmode_source_" + n,
                    getattr(self.ui, "comboBox_mech_expertmode_source_" + n).itemData(
                        getattr(
                            self.ui, "comboBox_mech_expertmode_source_" + n
                        ).currentIndex()
                    ),
                )
                ...
                # save current value to self.settings
                self.set_mechchndata(
                    "radioButton_mech_expertmode_calc_" + n,
                    getattr(
                        self.ui, "radioButton_mech_expertmode_calc_" + n
                    ).isChecked(),
                )
            else:  # data in mechchndata
                # the order of setting values should not change
                getattr(self.ui, "radioButton_mech_expertmode_calc_" + n).setChecked(
                    self.get_mechchndata("radioButton_mech_expertmode_calc_" + n)
                )
                self.load_comboBox(
                    getattr(self.ui, "comboBox_mech_expertmode_source_" + n),
                    val=self.get_mechchndata("comboBox_mech_expertmode_source_" + n),
                    mech_chn=mech_chn,
                )
                self.load_comboBox(
                    getattr(self.ui, "comboBox_mech_expertmode_indchn_" + n),
                    val=self.get_mechchndata("comboBox_mech_expertmode_indchn_" + n),
                    mech_chn=mech_chn,
                )
                getattr(self.ui, "lineEdit_mech_expertmode_value_" + n).setText(
                    str(self.get_mechchndata("lineEdit_mech_expertmode_value_" + n))
                )
    ...
    def mech_solve_chn(self, chn_name=None, chn_queue_ids=None, chn_idx=None):
        ...
        self.qcm.refh = self.settings[
            "spinBox_settings_mechanics_nhcalc_n3"
        ]  # use the dissipatione harmonic as reference
        ...
        bulklimit = self.settings[
            "doubleSpinBox_settings_mechanics_bulklimit"
        ]  # the rd limit for bulk calculation

        # logger.info('nhcalc_list', nhcalc_list)

        # check which is using: model or layers
        if (
            self.ui.stackedWidget_settings_mechanics_modeswitch.currentIndex() == 0
        ):  # model mode
        ...
        if calc_num is None:
            logger.warning("Stoped! No layer is marked to calculate")
            return
        ...
        if dic["source"] == "ind":  # dic is still where the loop break
            ...
            calc_idx = UIModules.index_from_str(
                calc_idx_str, chn_idx, join_segs=False
            )  # overwrite idx with given index
            calc_idx_joined = UIModules.index_from_str(
                calc_idx_str, chn_idx, join_segs=True
            )  # overwrite idx with given index
            if calc_idx_joined:
                idx = calc_idx
        ...
        #     pass
            ...
            logger.error("source not defined!")
            return

        logger.info("Calculating %s ...", nhcalc)

        # 4. iterate all layers to get props
        # prop_dict = {}
        prop_dict = {
            ind: {int(n): {"calc": film_dict[n]["calc"]} for n in film_dict.keys()}
            for ind in idx_joined
        }  # initiate dict for storing the prop
        ...
        for n in sorted([int(n) for n in film_dict.keys()]):
            ...
            if dic["calc"]:  # layer to calc
                ...
                #  prop_dict[ind][n]['calc'] = dic['calc']
                    ...
                    # get prop_guess from qcm
                    for ind in idx_joined:
                        prop_dict[ind][n].update(
                            **self.qcm.get_prop_by_name(dic["val"])
                        )
                        #  prop_dict[ind][n]['calc'] = dic['calc']
                ...
                #     pass
                    ...
                    layer_queue_ids = self.data_saver.get_queue_id_marked_rows(
                        layer_chn, dropnanmarkrow=False
                    )
                    layer_chn_idx = list(layer_queue_ids.index)
                    idx_layer = UIModules.index_from_str(
                        idx_str, layer_chn_idx, join_segs=False
                    )
                    idx_layer_joined = UIModules.index_from_str(
                        idx_str, layer_chn_idx, join_segs=True
                    )
                    ...
                    qcm_df_layer = qcm_df_layer_chn.loc[
                        idx_layer_joined
                    ]  # df of current layer
                    # create qcm_df by interpolation
                    qcm_df_layer = self.data_saver.shape_qcmdf_b_to_a(
                        qcm_df_calc, qcm_df_layer, idx, idx_layer
                    )
                    # get values for each
                    ...
                    for ind in idx_joined:
                        if n == 0:  # electrode layer
                            ...
                            qcm_queue = qcm_df_layer.loc[
                                [ind], :
                            ].copy()  # as a dataframe
                            # get prop
                            drho, grho_refh, phi, dlam_refh, err = (
                            ...
                            prop_dict[ind][n].update(
                                drho=drho, grho=grho_refh, phi=phi, n=refh
                            )
                else:
                    logger.error("source not defined!")
        ...
        mech_df = self.data_saver.update_mech_df_shape(
            chn_name, nhcalc
        ...
        for ind in idx_joined:  # iterate all ids
            ...
            if self.qcm.all_nhcaclc_harm_not_na(nh, qcm_queue):
                ...
                mech_queue = self.qcm.solve_single_queue(
                    nh,
                    qcm_queue,
                    mech_queue,
                    calctype=calctype,
                ...
                self.data_saver.update_mech_queue(
                    chn_name, nhcalc, mech_queue
                )  # update to mech_df in data_saver
    ...
    def backup_mech_solve_chn(self, chn_name, queue_ids):
        ...
        self.qcm.refh = int(
            self.settings["comboBox_settings_mechanics_refG"]
    ...
    def mech_solve_all(self):
        queue_ids = self.data_saver.get_queue_id_marked_rows(
            self.mech_chn, dropnanmarkrow=False
        )
    ...
    def data_mech_queue_ids_diff(self):
        ...
        """
        data_queue_ids = self.data_saver.get_queue_id_marked_rows(
            self.mech_chn, dropnanmarkrow=False
        )
        ...
        queue_id_diff = data_queue_ids[
            data_queue_ids.isin(set(data_queue_ids) - set(mech_queue_ids))
        ]
    ...
    def set_mech_layer_0_indchn_val(self):
        ...
        if ref_tempmode == "const":
            self.set_mechchndata(
                "comboBox_mech_expertmode_indchn_0", samprefsource, mech_chn="samp"
            )
            self.set_mechchndata(
                "comboBox_mech_expertmode_indchn_0", refrefsource, mech_chn="ref"
            )
        ...
        self.set_mechchndata(
            "lineEdit_mech_expertmode_value_0", samprefidx, mech_chn="samp"
        )
        self.set_mechchndata(
            "lineEdit_mech_expertmode_value_0", refrefidx, mech_chn="ref"
        )
    ...
    def on_changed_spinBox_spectra_mechanics_currid(self, ind):
        ...
        if ind not in qcm_df.index:
            ...
            if mech_key not in self.data_saver.get_prop_keys(
                chn_name
            ):  # no solution stored of given combination
    ...
    def make_film_dict_by_mechmodel_widgets(self):
        ...
        ]:  # in case comboBox_settings_mechanics_selectmodel is empty
            self.settings["comboBox_settings_mechanics_selectmodel"] = (
                self.ui.comboBox_settings_mechanics_selectmodel.itemData(
                    self.ui.comboBox_settings_mechanics_selectmodel.currentIndex()
                )
            )
        ...
        for n in range(1, n_layers + 1):
            ...
            film_dict[n] = {
                ...
                "indchn": getattr(
                    self.ui,
                    "comboBox_settings_mechanics_model_" + layer_name[n] + "_chn",
                ).itemData(
                    getattr(
                        self.ui,
                        "comboBox_settings_mechanics_model_" + layer_name[n] + "_chn",
                ...
                "val": getattr(
                    self.ui,
                    "lineEdit_settings_mechanics_model_" + layer_name[n] + "_idx",
                ).text(),
            }
        ...
        film_dict["0"] = {
            "calc": False,
            "source": "ind",
            "indchn": self.settings[
                "comboBox_settings_data_" + self.mech_chn + "refsource"
            ],
        ...
        for tb_row in range(tb_rows):
            ...
            if df_colname:
                ...
                for tb_col in range(tb_cols):
                    ...
                    tableitem = self.ui.tableWidget_spectra_mechanics_table.item(
                        tb_row, tb_col
                    )
    ...
    def mechanics_plot(self, plot_type):
        ...
        if (
            mech_key not in getattr(self.data_saver, chn_name + "_prop").keys()
        ):  # no corresponding prop data
            logger.warning("There is no property data of %s", mech_key)
            return

        if not self.data_mech_queue_ids_diff().empty:
            logger.warning(
                "there are new data not calculated. Slove those before plotting."
            )
            return
        ...
        if (plot_type == "contourdata") and (
            self.settings["comboBox_settings_mechanics_contourdata"] == "w_curr"
        ):  # contour data with current data
            prop_plot_list.extend(
                [
                    self.ui.mpl_contour1,
                    self.ui.mpl_contour2,
        ...
        elif self.show_marked_data() and self.data_saver.with_marks(
            chn_name
        ):  # show marked data only
            prop_group = "pm"
            line_group = "l"
            mark = True
        else:  # show all data
        ...
        if plot_type in x_list:  # y vs. time/temp/idx
            ...
            if len(varplot) < 2:  # not enough variables selected
                logger.warning("Not enough rows are selected! Please select 2 rows.")
                return
            elif len(varplot) > 2:  # too many variables selected
                logger.warning("Too many rows are selected! Please select 2 rows.")
                return
        ...
        for var_yx in varplots:  # for each plot to plot
            # var_yx[0] as y
            ydata, yerr = self.get_data_from_data_or_prop(
                chn_name, mech_key, var_yx[0], mark=mark, idx=idx
            )
            ylabel = self.get_label_replace_refh_unit(var_yx[0], refh)

            # var_yx[1] as x
            xdata, xerr = self.get_data_from_data_or_prop(
                chn_name, mech_key, var_yx[1], mark=mark, idx=idx
            )
            ...
            if var_yx[0].endswith("s") or var_yx[1].endswith("s"):
                ...
                figharms = [str(refh)]
            # prepare marker data
            prop_list = self.prepare_harm_data_for_mpl_prop_update(
                chn_name,
                figharms,
                prop_group,
                xdata,
            ...
            if (
                "_calc" in var_yx[0] or "_calc" in var_yx[1]
            ):  # var in mech_keys_multiple
                line_xdata = xdata
                line_ydata = ydata
                if "_calc" in var_yx[0]:
                    line_ydata, _ = self.get_data_from_data_or_prop(
                        chn_name,
                        mech_key,
                        var_yx[0].replace("calc", "exp"),
                        mark=mark,
                        idx=idx,
                    )
                if "_calc" in var_yx[1]:
                    line_xdata, _ = self.get_data_from_data_or_prop(
                        chn_name,
                        mech_key,
                        var_yx[1].replace("calc", "exp"),
                        mark=mark,
                        idx=idx,
                    )
                # prepare line data
                line_list = self.prepare_harm_data_for_mpl_prop_update(
                    chn_name,
                    figharms,
                    line_group,
                    line_xdata,
    ...
    def get_data_from_data_or_prop(self, chn_name, mech_key, var, mark=False, idx=None):
        ...
        if var in data_cols + [
            "delf_exps",
            "delg_exps",
        ]:  # delg delf is in data, so add them to the check list
            # get timeunit
            timeunit = self.settings["comboBox_timeunit"]
            # get tempunit
            tempunit = self.settings["comboBox_tempunit"]

            data = self.get_data_by_typestr(
                var, chn_name, mark=mark, unit_t=timeunit, unit_temp=tempunit
            )

        elif var in prop_cols:  # delg delf is in data, so add them to the check list
            if not idx:  # use mark
                data = self.data_saver.get_mech_column_to_columns_marked_rows(
                    chn_name, mech_key, var, mark=mark, dropnanmarkrow=False
                )
            else:  # use index list
                data = self.data_saver.get_mech_column_to_columns_by_idx(
                    chn_name, mech_key, var, idx=idx
                )
            data = self.qcm.convert_mech_unit(data)  # convert unit for plot
            # get yerr
            if self.settings["checkBox_settings_mechanics_witherror"] and (
                var + "_err"
                in getattr(self.data_saver, chn_name + "_prop")[mech_key].keys()
            ):  # corresponding error exists
                err = self.data_saver.get_mech_column_to_columns_marked_rows(
                    chn_name, mech_key, var + "_err", mark=mark, dropnanmarkrow=False
                )
    ...
    def update_mpl_to_prop_scrollarea(self):
        ...
        if not (n - 1) % 2:  # n is odd
            self.ui.gridLayout_propplot.setRowMinimumHeight(
                (n - 1) // 2, config_default["prop_plot_minmum_row_height"]
            )
        # return
    ...
    def prep_contour_mesh(self, contour_type=None):
        ...
        phi = np.linspace(
            contour_lim["phi"]["min"], contour_lim["phi"]["max"], contour_array["num"]
        )
        dlam = np.linspace(
            contour_lim["dlam"]["min"], contour_lim["dlam"]["max"], contour_array["num"]
        )
        ...
        if contour_type.lower() == "normfnormg":
            ...
            mesh1["Z"], mesh2["Z"] = (
                np.random.rand(*dlam_i.shape),
                np.random.rand(*dlam_i.shape),
            )  # just make some random numbers for fun
    ...
    def make_contours(self):
        ...
        if contour_type.lower() == "normfnormg":
            levels1 = self.make_contour_levels(
                contour_lim["normf"]["min"],
                contour_lim["normf"]["max"],
                contour_array["levels"],
            )
            levels2 = self.make_contour_levels(
                contour_lim["normg"]["min"],
                contour_lim["normg"]["max"],
                contour_array["levels"],
            )
        ...
        self.ui.mpl_contour1.init_contour(
            **mesh1, levels=levels1, cmap=cmap, title=title1
        )
        self.ui.mpl_contour2.init_contour(
            **mesh2, levels=levels2, cmap=cmap, title=title2
        )
    ...
    def add_data_to_contour(self):
        ...
        if contour_data == "none":  # w/o data
            ...
            "w_curr",
            "w_data",
        ]:  # w/ current data or w/ data (all or marked)
    ...
    def on_clicked_set_temp_sensor(self, checked):
        ...
        if self.vna:  # add not for testing code
            if checked:  # checkbox is checked
                ...
                tempmodule_name = self.settings[
                    "comboBox_tempmodule"
                ]  # get temp module

                thrmcpltype = self.settings[
                    "comboBox_thrmcpltype"
                ]  # get thermocouple type
                tempdevice = TempDevices.device_info(
                    self.settings["comboBox_tempdevice"]
                )  # get temp device info
                ...
                try:
                    ...
                    self.disable_widgets("temp_settings_enable_disable_list")
                ...
                self.enable_widgets("temp_settings_enable_disable_list")
    ...
    def statusbar_temp_update(self, curr_temp=None):
        ...
        if self.settings["checkBox_settings_temp_sensor"]:  # checked
            ...
            try:
                ...
                unit = config_default["temp_unit_opts"].get(
                    self.settings["comboBox_tempunit"]
                )
                self.ui.pushButton_status_temp_sensor.setText(
                    f"{self.data_saver.temp_C_to_unit(curr_temp, unit=unit):.1f} {unit}"
                )
                self.ui.pushButton_status_temp_sensor.setIcon(
                    QIcon(":/icon/rc/temp_sensor.svg")
                )
    ...
    def on_ref_mode_changed(self, signal):
        ...
        if (
            self.settings.get("comboBox_settings_data_ref_tempmode", None) == "const"
        ):  # constant temp experiment
            self.ui.comboBox_settings_data_ref_fitttype.hide()
        elif (
            self.settings.get("comboBox_settings_data_ref_tempmode", None) == "var"
        ):  # variable temp experiment
    ...
    def set_stackedwidget_index(self, stwgt, idx=None, diret=None):
        ...
        if idx:  # if index is not []
            ...
            stwgt.setCurrentIndex(
                (current_index + diret) % count
            )  # increase or decrease index by diret
    ...
    def get_harmdata(self, objname, harm=None, chn_name=None):
        ...
        try:
            ...
            logger.warning("%s is not found! Use default data", objname)
            return self.settings["harmdata"][chn_name][str(harm)][objname]
    ...
    def update_base_freq(self, base_freq_index):
        self.settings["comboBox_base_frequency"] = (
            self.ui.comboBox_base_frequency.itemData(base_freq_index)
        )  # in MHz
    ...
    def update_range(self, range_index):
        self.settings["comboBox_range"] = self.ui.comboBox_range.itemData(
            range_index
        )  # in MHz
    ...
    def statusbar_f0bw_update(self):
        ...
        self.ui.label_status_f0RNG.setText(f"{fbase}\u00b1{BW} MHz")
        self.ui.label_status_f0RNG.setToolTip(
            f"base frequency = {fbase} MHz; range = {BW} MHz"
        )
    ...
    def set_freq_span(self, span, harm=None, chn_name=None):
        ...
        if any(np.isnan(span)):  # failed to track peak
            logger.warning("failed to set freq_span")
        ...
        if (
            ...
            for harm in self.all_harm_list(as_str=True):
                if harm in self.settings["freq_span"]["samp"]:
                    freq_span["samp"][harm] = self.span_check(
                        harm, *self.settings["freq_span"]["samp"][harm]
                    )
                    freq_span["ref"][harm] = self.span_check(
                        harm, *self.settings["freq_span"]["ref"][harm]
                    )
                else:
    ...
    def update_frequencies(self):
        ...
        for harm in self.all_harm_list(as_str=True):
            ...
            getattr(self.ui, "lineEdit_startf" + harm).setText(
                UIModules.num2str(f1, precision=1)
            )  # display as Hz
            getattr(self.ui, "lineEdit_endf" + harm).setText(
                UIModules.num2str(f2, precision=1)
            )  # display as Hz
            getattr(self.ui, "lineEdit_startf" + harm + "_r").setText(
                UIModules.num2str(f1r, precision=1)
            )  # display as Hz
            getattr(self.ui, "lineEdit_endf" + harm + "_r").setText(
                UIModules.num2str(f2r, precision=1)
            )  # display as Hz
        ...
        # Set Start
        self.ui.lineEdit_scan_harmstart.setText(UIModules.num2str(f1, precision=1))
        # set End
        self.ui.lineEdit_scan_harmend.setText(UIModules.num2str(f2, precision=1))
    ...
    def setvisible_samprefwidgets(self):
        ...
        ) and self.settings["checkBox_activechn_samp"]
        ref_value = (self.settings["comboBox_ref_channel"] != "none") and self.settings[
        ...
        if samp_value and ref_value:  # both samp and ref channels are selected
            ...
            not samp_value and not ref_value
        ):  # neither of samp or ref channel is selected
    ...
    def statusbar_signal_chn_update(self):
        ...
        # set tabWidget_settings_settings_samprefchn
        if (
            samp_value and ref_value
        ):  # both samp and ref  (value is '1' or '2')channels are selected
            self.ui.pushButton_status_signal_ch.setIcon(
                QIcon(":/icon/rc/signal_ch_sr.svg")
            )
        elif (
            not samp_value and not ref_value
        ):  # neither of samp or ref channel is selected
            self.ui.pushButton_status_signal_ch.setIcon(
                QIcon(":/icon/rc/signal_ch_off.svg")
            )
        elif samp_value and not ref_value:  # only samp selected
            self.ui.pushButton_status_signal_ch.setIcon(
                QIcon(":/icon/rc/signal_ch_s.svg")
            )
        elif not samp_value and ref_value:  # only ref selected
            self.ui.pushButton_status_signal_ch.setIcon(
                QIcon(":/icon/rc/signal_ch_r.svg")
            )
        else:  # somthing wrong
            self.ui.pushButton_status_signal_ch.setIcon(
                QIcon(":/icon/rc/signal_ch_off.svg")
    ...
    def check_checked_activechn(self):
        if config_default["activechn_num"] == 1:
            ...
            if (
                sender_name.startswith("checkBox_") and self.settings[sender_name]
            ):  # checkBox_ is checked
    ...
    def create_combobox(self, name, contents, box_width, row_text="", parent=""):
        ...
        if isinstance(contents, list):  # if given a list, add only the text
            ...
            contents, dict
        ):  # if given a dict, add the text (val) and userData (key)
    ...
    def load_normal_widgets(self, name_list):
        ...
        for name in name_list:
            ...
            if name.startswith("lineEdit_"):
                ...
                obj.setDateTime(
                    datetime.datetime.strptime(
                        self.settings[name], config_default["time_str_format"]
                    )
                )
    ...
    def load_settings(self):
        ...
        # set window title
        self.setWindowTitle(
            _version.__projectname__ + " Version " + _version.__version__
        )
        # set window size
        ...
        self.ui.stackedWidget_settings_mechanics_modeswitch.setCurrentIndex(
            config_default["mechanics_modeswitch"]
        )
        # set progressbar
        self.set_progressbar(val=0, text="")
        ...
        for harm in self.all_harm_list(as_str=True):
            ...
            getattr(self.ui, "checkBox_harm" + harm).setChecked(
                self.settings.get("checkBox_harm" + harm, False)
            )
            getattr(self.ui, "checkBox_tree_harm" + harm).setChecked(
                self.settings.get("checkBox_harm" + harm, False)
            )
        ...
        if (
            ...
            logger.info(
                type(
                    datetime.datetime.strptime(
                        self.settings["dateTimeEdit_reftime"],
                        config_default["time_str_format"],
                    )
                )
            )
        ...
        # spinBox_mech_expertmode_layernum
        self.ui.spinBox_mech_expertmode_layernum.setValue(
            self.get_mechchndata("spinBox_mech_expertmode_layernum")
        )
    ...
    def update_refsource(self):
        ...
        self.ui.lineEdit_settings_data_sampidx.setText(
            str(self.settings["lineEdit_settings_data_sampidx"])
        )
        self.ui.lineEdit_settings_data_refidx.setText(
            str(self.settings["lineEdit_settings_data_refidx"])
        )
        self.ui.lineEdit_settings_data_samprefidx.setText(
            str(self.settings["lineEdit_settings_data_samprefidx"])
        )
        self.ui.lineEdit_settings_data_refrefidx.setText(
            str(self.settings["lineEdit_settings_data_refrefidx"])
        )
    ...
    def load_refsource(self):
        ...
        self.settings["comboBox_settings_data_refrefsource"] = self.data_saver.exp_ref[
            "ref_ref"
        ][0]

        if len(self.data_saver.exp_ref["samp_ref"]) > 2:  # ver >= 0.17.0
            self.settings["lineEdit_settings_data_sampidx"] = self.data_saver.exp_ref[
                "samp_ref"
            ][2]
            self.settings["lineEdit_settings_data_refidx"] = self.data_saver.exp_ref[
                "ref_ref"
            ][2]
        else:
            pass  # do not change default value

        self.settings["lineEdit_settings_data_samprefidx"] = self.data_saver.exp_ref[
            "samp_ref"
        ][1]
        self.settings["lineEdit_settings_data_refrefidx"] = self.data_saver.exp_ref[
            "ref_ref"
        ][1]

        if "mode" in self.data_saver.exp_ref:
            self.settings["comboBox_settings_data_ref_crystmode"] = (
                self.data_saver.exp_ref["mode"].get("cryst")
            )
            self.settings["comboBox_settings_data_ref_tempmode"] = (
                self.data_saver.exp_ref["mode"].get("temp")
            )
            self.settings["comboBox_settings_data_ref_fitttype"] = (
                self.data_saver.exp_ref["mode"].get("fit")
            )
    ...
    def data_collection(self):
        ...
        ]  # get all checked harmonic into a list
        ...
        for chn_name in chn_name_list:
            ...
            self.reading = True
            # read time
            curr_time[chn_name] = datetime.datetime.now().strftime(
                config_default["time_str_format"]
            )
            ...
            with self.vna:
                ...
                for harm in harm_list:
                    ...
                    f[chn_name][harm], G[chn_name][harm], B[chn_name][harm] = (
                        self.get_vna_data_no_with(harm=harm, chn_name=chn_name)
                    )

                    logger.info("check:")
                    logger.info(f[chn_name][harm] is None)
                    logger.info(f[chn_name][harm][0] == f[chn_name][harm][-1])
                    if (f[chn_name][harm] is None) or (
                        f[chn_name][harm][0] == f[chn_name][harm][-1]
                    ):  # vna error
                        logger.error("Analyzer connection error!")
                        # stop test
                        self.idle = True
                        self.ui.pushButton_runstop.setChecked(False)
                        # alert
                    ...
                    self.peak_tracker.update_input(
                        ...
                        harmdata=self.settings["harmdata"],
                        freq_span=self.settings["freq_span"],
                        fGB=[f[chn_name][harm], G[chn_name][harm], B[chn_name][harm]],
                    )

                    # plot data in sp<harm>
                    if self.settings["radioButton_spectra_showGp"]:  # checked
                        getattr(self.ui, "mpl_sp" + str(harm)).update_data(
                            {"ln": "lG", "x": f[chn_name][harm], "y": G[chn_name][harm]}
                        )
                        ...
                        getattr(self.ui, "mpl_sp" + str(harm)).update_data(
                            {"ln": "lP", "x": G[chn_name][harm], "y": B[chn_name][harm]}
                        )
            ...
            for harm in harm_list:
                if self.get_harmdata(
                    "checkBox_harmfit", harm=harm, chn_name=chn_name
                ):  # checked to fit
                    fit_result = self.peak_tracker.peak_fit(
                    ...
                    logger.info(gc_list)
                    if (
                        self.settings["radioButton_spectra_showGp"]
                        or self.settings["radioButton_spectra_showBp"]
                    ):  # show G or GB
                        getattr(self.ui, "mpl_sp" + harm).update_data(
                            {"ln": "lsp", "x": factor_span, "y": gc_list}
                        )
                        ...
                        idx = np.where(
                            (f[chn_name][harm] >= factor_span[0])
                            & (f[chn_name][harm] <= factor_span[1])
                        )

                        getattr(self.ui, "mpl_sp" + harm).update_data(
                            {
                                "ln": "lsp",
                                "x": fit_result["fit_g"][idx],
                                "y": fit_result["fit_b"][idx],
                            }
                        )
                    ...
                    cen_rec_freq = fit_result["v_fit"]["cen_rec"]["value"]
                    cen_rec_G = self.peak_tracker.get_output(
                        key="gmod", chn_name=chn_name, harm=harm
                    ).eval(
                        self.peak_tracker.get_output(
                            key="params", chn_name=chn_name, harm=harm
                        ),
                        x=cen_rec_freq,
                    )
                    ...
                    gs[chn_name].append(
                        fit_result["v_fit"]["wid_rec"]["value"]
                    )  # gs = half_width
                    ...
                    # clear lines
                    getattr(self.ui, "mpl_sp" + harm).clr_lines(
                        l_list=["lGfit", "lBfit", "lPfit", "lsp", "srec"]
                    )

                ## get tracking data
                # get span from tracking
                span, cen_trk_freq = self.peak_tracker.peak_track(
                    chn_name=chn_name, harm=harm
                )
                ...
                if (
                    ...
                    getattr(self.ui, "mpl_sp" + harm).update_data(
                        {"ln": "strk", "x": cen_trk_G, "y": cen_trk_B}
                    )
        ...
        ):  # if timer is stopped (test stopped while collecting data)
            # save data
            self.process_saving_when_stop()
        ...
        for idx in indeces:
            ...
            for harm in harm_list:  # TODO add poll here
                ...
                self.peak_tracker.update_input(
                    ...
                    freq_span=[],
                    fGB=[f, G, B],
                )  # freq_span set to [], since we don't need to track the peak
            ...
            if regenerate:
                ...
                marks = [
                    0 for _ in harm_list
                ]  # 'samp' and 'ref' chn have the same harmonics

                self.data_saver._save_queue_data(
                    [chn_name],
                    harm_list,
                    queue_id=queue_id,
                    t={chn_name: t},
                ...
                self.data_saver.update_refit_data(
                    chn_name, queue_id, harm_list, fs=fs, gs=gs, ps=ps
                )
    ...
    def update_progressbar(self):
        ...
        if self.spectra_refresh_modulus() == 0:  # going to save data
            txt = f"{timer_remain:.1f} s"
        else:
            txt = "{:.1f} s + {}".format(
                timer_remain,
                self.settings["spinBox_refreshresolution"]
                - self.spectra_refresh_modulus(),
            )

        self.set_progressbar(
            val=min(round((1 - timer_remain / timer_interval) * 100), 100), text=txt
        )
