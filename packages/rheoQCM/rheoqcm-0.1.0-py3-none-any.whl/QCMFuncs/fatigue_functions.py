...
E_sym, nu_sym, B_sym, W_sym, L_sym, P_sym, alpha_sym, beta_sym, C_in_sym = symbols(
    ["E", "nu", "B", "W", "L", "P", "alpha", "beta", "C_in"], positive=True, real=True
)
...
C_sym = (beta_sym**3 / (48 * E_sym * B_sym)) + (1 / (E_sym * B_sym)) * (
    c1 + beta_sym * c2 + beta_sym**2 * c3
)
...
K4 = (
    (P_sym / (B_sym * W_sym**0.5))
    * 6
    * alpha_sym**0.5
    * (
        1.99
        - alpha_sym * (1 - alpha_sym) * (2.15 - 3.93 * alpha_sym + 2.7 * alpha_sym**2)
    )
...
alpha_calc = (C_in_sym * E_sym * B_sym) ** 0.5 / (
    (
        C_in_sym * E_sym * B_sym
        + q1 * (C_in_sym * E_sym * B_sym) ** 0.5
        + q2 * (C_in_sym * E_sym * B_sym) ** (1 / 3)
        + q3
    )
    ** 0.5
)
...
def make_data_dict(directory, df_in, row):
    ...
    for g in np.arange(ngroups):
        ...
        borders_idx = sorted(
            np.concatenate([t.iloc[peaks].index, t.iloc[valleys].index])
        )
        ...
        # amplitude
        while (
            abs(P_array[borders_idx[1]] - P_array[borders_idx[0]])
            < 0.1 * (max(P_array) - min(P_array))
            and len(borders_idx) > 2
        ):
            borders_idx = np.delete(borders_idx, 0)

        while (
            abs(P_array[borders_idx[-2]] - P_array[borders_idx[-1]])
            < 0.1 * (max(P_array) - min(P_array))
            and len(borders_idx) > 2
        ):
        ...
        Wdiss[g] = np.average(
            np.array(detail[g]["all"]["diss"][100:])
        ...
        phi[g] = (180 / np.pi) * np.arcsin(
            (4 * Wdiss[g]) / (np.pi * (dmax[g] - dmin[g]) * (Pmax[g] - Pmin[g]))
        )
...
def calculate_slope(x, y, low_strain_threshold=0.03):
    ...
    slope, intercept = np.polyfit(x[low_strain_indices], y[low_strain_indices], 1)
...
def make_intermediate_plots_KIC(data_dict, win_size):
    ...
    fig, ax = plt.subplots(
        2, 2, figsize=(8, 6), constrained_layout=True, num="intermediate plots"
    )
    ...
    if win_size == 2:
        ...
        ax[1, 0].semilogy(
            cycle[win_size // 2 : -win_size // 2 + 1], dadn * 1e9, "o", fillstyle="none"
        )
        ax[0, 1].plot(
            cycle[win_size // 2 : -win_size // 2 + 1], delK / 1e6, "o", fillstyle="none"
        )
