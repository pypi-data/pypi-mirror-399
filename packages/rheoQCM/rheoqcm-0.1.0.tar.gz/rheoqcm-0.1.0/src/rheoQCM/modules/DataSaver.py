...
class DataSaver:
    ...
    ----------
    samp : pd.DataFrame
        Sample channel data (queue_id, time, frequency shifts).
    ref : pd.DataFrame
        Reference channel data.
    samp_prop : dict
        Calculated mechanical properties for sample channel.
        Keys are harmonic combinations like '353', values are DataFrames.
    raw : dict
        Most recent raw scan data before processing.
    path : str
        Path to the current HDF5 file.
    saveflg : bool
        True if all changes have been saved to disk.
    ...
    def __init__(self, ver="", settings=None):
        ...
        ----------
        ver : str, optional
            Version string for file metadata.
        settings : dict, optional
            Configuration dictionary. Defaults to empty dict.
        """
        if settings is None:
            settings = {}
        ...
        self._ref_keys = {
            "fs": "f0",
            "gs": "g0",
        }  # corresponding keys storing the reference
    ...
    def _make_exp_ref(self):
        ...
        return {
            ...
            # temp ref
            # mode
            "mode": {"cryst": "single", "temp": "const", "fit": "linear"},
    ...
    def nan_interp_func_list(self):
        ...
        def nan_func(temp):
            return np.array([np.nan] * len(temp))
    ...
    def update_mech_df_shape(self, chn_name, nhcalc):
        ...
        if mech_key in getattr(self, chn_name + "_prop").keys():
            ...
            if mech_queue_id.equals(data_queue_id):
                ...
                df_mech = df_mech[
                    df_mech["queue_id"].isin(set(mech_queue_id) & set(data_queue_id))
                ]
        ...
        with h5py.File(self.path, "r") as fh:
            ...
            for chn_name in self._chn_keys:
                ...
                setattr(
                    self,
                    chn_name,
                    self.add_missed_cols_to_df(getattr(self, chn_name), ["ps"]),
                )
                ...
                # load prop
                if ("prop" in fh.keys()) and (
                    chn_name in fh["prop"].keys()
                ):  # prop exists
    ...
    def check_file_format(self, path):
        ...
        if {"data", "exp_ref", "raw", "settings"}.issubset(key_list) and {
            "ver"
        }.issubset(attr_list):
            return True
        else:
            ...
            for chn_name in self._chn_keys:
                for mech_key, mech_df in getattr(self, chn_name + "_prop").items():
                    if ("prop" not in fh.keys()) or (chn_name not in fh["prop"].keys()):
                        fh.create_dataset(
                            "prop/" + chn_name + "/" + mech_key,
                            data=mech_df.to_json(),
                            dtype=h5py.special_dtype(vlen=str),
                        )
                    else:
    ...
    def get_raw(self, chn_name, queue_id, harm, with_t_temp=False):
        ...
        with h5py.File(self.path, "r") as fh:
            ...
            if self._raw_exists(fh, chn_name, queue_id, harm):  # raw data exist
                ...
                logger.warning(
                    "No raw data found for %s, %s, %s", chn_name, queue_id, harm
                )
                self.raw = {
                    "f": None,
                    "G": None,
    ...
    def _raw_exists(self, file_handle, chn_name, queue_id, harm):
        ...
        if (
            "raw" in file_handle.keys()
            and chn_name in file_handle["raw"]
    ...
    def update_queue_col(self, chn_name, queue_id, col, val):
        ...
        if queue_id in self.get_queue_id(chn_name).values:  # overwrite
            ...
            getattr(self, chn_name).merge(
                pd.DataFrame.from_dict({col: [val], "queue_id": [queue_id]}),
                how="outer",
            )
            ...
            for ext in ["", "_ref"]:  # samp/ref and samp_ref/ref_ref
                ...
                for col in col_endswith_s:
                    ...
                    df[col] = df[col].apply(
                        lambda row: [
                            row[i]
                            for i in range(int((self.settings["max_harmonic"] + 1) / 2))
                        ]
                    )
                    ...
                    for col in cols:
                        ...
                        df[col] = df[col].apply(
                            lambda row: [
                                row[i]
                                for i in range(
                                    int((self.settings["max_harmonic"] + 1) / 2)
                                )
                            ]
                        )
    ...
    def data_exporter(
        ...
        df_samp = pd.merge(
            # f
            self.reshape_data_df(
                "samp",
                mark=mark,
                dropnanmarkrow=dropnanmarkrow,
                dropnancolumn=dropnancolumn,
                unit_t=unit_t,
                unit_temp=unit_temp,
                keep_mark=False,
            ),
        ...
        df_samp_ref = self.reshape_data_df(
            "samp_ref",
            mark=mark,
            dropnanmarkrow=dropnanmarkrow,
            dropnancolumn=dropnancolumn,
        ...
        if self.ref_ref.shape[0] > 0:
            df_ref_ref = self.reshape_data_df(
                "ref_ref",
                mark=mark,
                dropnanmarkrow=dropnanmarkrow,
                dropnancolumn=dropnancolumn,
        ...
        try:
            if ext.lower() == ".xlsx":
                with pd.ExcelWriter(fileName) as writer:
                    ...
                    # time reference
                    t_ref = {
                        key: self.exp_ref[key]
                        for key in self.exp_ref.keys()
                        if "t0" in key
                    }
                    pd.DataFrame.from_dict(t_ref, orient="index").to_excel(
                        writer, sheet_name="time_reference", header=False
                    )
                    # sample description
                    sample_description = {
                        "sample Description": self.settings.get(
                            "plainTextEdit_settings_sampledescription", ""
                        ).split("\n")
                    }  # split convert string to list of strings
                    pd.DataFrame.from_dict(
                        sample_description, orient="columns"
                    ).to_excel(
                        writer,
                        sheet_name="sample_description",
                        header=False,
                        index=False,
                    )
    ...
    def reshape_data_df(
        ...
        for col in cols:
            df = df.assign(
                **self.get_list_column_to_columns(
                    chn_name, col, mark=mark, deltaval=deltaval, norm=norm
                )
        ...
        if "marks" in df.columns:
            ...
            if keep_mark:
                df = df.assign(
                    **self.get_list_column_to_columns(
                        chn_name, "marks", mark=mark, norm=False
                    )
                )
    ...
    def reshape_mech_df(
        self, chn_name, mech_key, mark=False, dropnanmarkrow=True, dropnancolumn=True
    ):
        """
        ...
        for col in cols:
            if col.endswith(("s", "s_err")):  # value varies with harmonic
                df = df.assign(
                    **self.get_mech_column_to_columns(
                        chn_name, mech_key, col, mark=mark
                    )
                )  # split columns: fs and gs
                df = df.drop(columns=col)  # drop columns: fs and gs
            else:  # single value
    ...
    def get_marked_harm_col_from_list_column(
        self, chn_name, harm, col, deltaval=False, norm=False, mark=False
    ):
        cols = self.get_list_column_to_columns(
            chn_name, col, mark=False, deltaval=deltaval, norm=norm
        )  # get all data
        harm_col = cols.filter(regex=rf"\D{harm}$", axis=1).squeeze(
            axis=1
        )  # convert to series
    ...
    def get_marked_harm_mech_col_from_list_mech_column(
        self, chn_name, harm, mech_key, col, mark=False
    ):
        cols = self.get_mech_column_to_columns(
            chn_name, mech_key, col, mark=False
        )  # get all data
        harm_col = cols.filter(regex=rf"\D{harm}$", axis=1).squeeze(
            axis=1
        )  # convert to series
    ...
    def get_t_ref(self):
        ...
        if "t0" in self.exp_ref.keys() and self.exp_ref.get(
            "t0", None
        ):  # t0 exist and != None or 0
            if self.exp_ref.get("t0_shifted", None):
                t0 = datetime.datetime.strptime(
                    self.exp_ref.get("t0_shifted"), self.settings["time_str_format"]
                )  # used shifted t0
            else:
                t0 = datetime.datetime.strptime(
                    self.exp_ref.get("t0"), self.settings["time_str_format"]
                )  # use t0
            ...
            if not t0:
                if self.samp.shape[0] > 0:  # use the first queque time
                    t0 = datetime.datetime.strptime(
                        self.samp["t"][0], self.settings["time_str_format"]
                    )
                else:
                    t0 = None
            else:
    ...
    def get_list_column_to_columns_marked_rows(
        self,
        chn_name,
        col,
        mark=False,
    ...
    def get_list_column_to_columns(
        self, chn_name, col, mark=False, deltaval=False, norm=False
    ):
        """
        ...
        if not mark:
            ...
            if np.any(arr_m == 1):  # there are marks (1)
                ...
                return pd.DataFrame(data=arr_s, index=idx).rename(
                    columns=lambda x: col[:-1] + str(x * 2 + 1)
                )
    ...
    def get_mech_column_to_columns_marked_rows(
        self, chn_name, mech_key, col, mark=False, dropnanmarkrow=False
    ):
        """
    ...
    def get_mech_column_to_columns(self, chn_name, mech_key, col, mark=False):
        ...
        if not mark:
            return pd.DataFrame(s.values.tolist(), s.index).rename(
                columns=lambda x: col + str(x * 2 + 1)
            )
        else:
    ...
    def calc_fg_ref(self, chn_name, mark=True):
        ...
        if mode["cryst"] == "single":
            if mode["temp"] == "const":  # single crystal and constant temperature
                ...
                if all(
                    isinstance(idx_item, list)
                    for idx_item in self.exp_ref[chn_name + "_ref"][1]
                ):  # all list
                    reference_idx = self.exp_ref[chn_name + "_ref"][1]
                elif all(
                    isinstance(idx_item, int)
                    for idx_item in self.exp_ref[chn_name + "_ref"][1]
                ):  # all int
                    reference_idx = [
                        self.exp_ref[chn_name + "_ref"][1]
                    ]  # put into a list
                else:
                ...
                for ind_list in reference_idx:  # iterate each list
                    ...
                    # get data
                    ref_fs_df = self.get_list_column_to_columns_marked_rows(
                        chn_ref_source,
                        "fs",
                        mark=False,
                        dropnanmarkrow=False,
                    ...
                    ref_gs_df = self.get_list_column_to_columns_marked_rows(
                        chn_ref_source,
                        "gs",
                        mark=False,
                        dropnanmarkrow=False,
                    ...
                    func_f_list = [
                        lambda x, f=f: np.full_like(x, f, dtype=float)
                        for f in func_f_list
                    ]
                    func_g_list = [
                ...
                fs = self.get_list_column_to_columns_marked_rows(
                    chn_ref_source,
                    "fs",
                    mark=False,
                    dropnanmarkrow=False,
                    deltaval=False,
                    norm=False,
                )  # absolute freq in Hz. If marked, set mark=True
                gs = self.get_list_column_to_columns_marked_rows(
                    chn_ref_source,
                    "gs",
                    mark=False,
                    dropnanmarkrow=False,
                ...
                for ind_list in reference_idx:  # iterate each list
                    ...
                    for harm in range(
                        1, self.settings["max_harmonic"] + 2, 2
                    ):  # calculate each harm
    ...
    def interp_film_ref(self, chn_name, col=None):
        ...
        if mode["cryst"] == "single":
            if mode["temp"] == "const":  # single crystal and constant temperature
                ...
                if all(isinstance(idx_item, list) for idx_item in chn_idx):  # all list
                    film_idx = chn_idx
                elif all(isinstance(idx_item, int) for idx_item in chn_idx):  # all int
    ...
    def set_t0(self, t0=None, t0_shifted=None):
        ...
        if t0_shifted is not None:
            if isinstance(
                t0_shifted, datetime.datetime
            ):  # if t0_shifted is datetime obj
                t0_shifted = t0_shifted.strftime(
                    self.settings["time_str_format"]
                )  # convert to string
    ...
    def selector_mark_all(self, chn_name, mark_val):
        ...
        """
        setattr(
            self, chn_name, self.mark_all_to(getattr(self, chn_name), mark_val=mark_val)
        )
    ...
    def selector_del_sel(self, chn_name, sel_idx_dict):
        ...
        for harm, idx in sel_idx_dict.items():
            # set marks to -1
            df_chn = self.mark_data(
                df_chn, idx=idx, harm=harm, mark_val=np.nan
            )  # set the marks to nan
            # set fs, gs to nan
            df_chn.fs[idx] = df_chn.fs[idx].apply(
                lambda x, harm=harm: [
                    np.nan if str(i * 2 + 1) in harm else val for i, val in enumerate(x)
                ]
            )  # set all to nan. May not necessary
            df_chn.gs[idx] = df_chn.gs[idx].apply(
                lambda x, harm=harm: [
                    np.nan if str(i * 2 + 1) in harm else val for i, val in enumerate(x)
                ]
            )  # set all to nan. May not necessary
            ...
            for harm, idxs in sel_idx_dict.items():
                ...
                for ind in idxs:
                    if ind in df_chn.queue_id.index:  # index in queue_id
                        ...
                        logger.warning(
                            "index %s does not exist (%s, %s)", ind, chn_name, harm
                        )
    ...
    def df_qcm(self, chn_name):
        ...
        df["temp"] = self.get_temp_by_uint_marked_rows(
            chn_name, dropnanmarkrow=False, unit="C"
        )
            ...
            if col not in [
                ...
                if mode["cryst"] == "single":
                    ...
                    if (
                        ...
                        mode["temp"] == "var"
                    ):  # single crystal and variable temperature
                        ## calc interp fun
                        # check if all elements in self.exp_ref.ref_ref[1] is list
                        if all(
                            isinstance(idx_item, list) for idx_item in idx_b
                        ):  # all list
                            pass
                        elif all(
                            isinstance(idx_item, int) for idx_item in idx_b
                        ):  # all int
                        ...
                        for ind_list in idx_b:
                            ...
                            func_list.append(
                                lambda temp, func_seg_list=func_seg_list: [
                                    func_seg(temp) for func_seg in func_seg_list
                                ]
                            )

                        # check if all elements in self.exp_ref.samp_ref[1] is list
                        if all(
                            isinstance(idx_item, list) for idx_item in idx_a
                        ):  # all list
                            pass
                        elif all(
                            isinstance(idx_item, int) for idx_item in idx_a
                        ):  # all int
    ...
    def import_qcm_with_other_format(
        self,
        data_format,
        path,
        config_default,
        ...
        self.mode = (
            data_format  # set mode. it will be used to determine how to import data
        )
    ...
    def import_data_from_df(self, df, t0, t0_str, f1, g1, config_default):
        ...
        df["t"] = df["t"].apply(
            lambda x: (t0 + datetime.timedelta(seconds=x)).strftime(
        ...
        # temperature: temp
        temp_str = list(
            set(config_default["data_saver_import_data"]["temp"]) & set(columns)
        )
