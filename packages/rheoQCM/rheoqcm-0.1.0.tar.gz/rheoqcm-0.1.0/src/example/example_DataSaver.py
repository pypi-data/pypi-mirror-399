...
deltaval = (
    True  # True: delta value for freq or gamma; False: absolute value for freq or gamma
)
norm = False  # True: normalize delta values with harmonic number (e.g.: delfreq/harm)

col_data = data_saver.get_list_column_to_columns_marked_rows(
    chn_name, data_col, mark=mark, dropnanrow=dropnanrow, deltaval=deltaval, norm=norm
)
print(col_data.head())
...
temp = data_saver.get_temp_by_uint_marked_rows(
    chn_name, dropnanrow=dropnanrow, unit=unit_temp
)
print(temp.head())
...
mark = False  # False: all data; True: only marked rows
mech_key = (
    "353_3"  # string of combination of solving combination and reference harmonic
)
prop_col = "drho"  # column name in property dataframe
dropnanrow = False  # False: leave the row with all nan as it is; True: remove the rows with no marks
prop_data = data_saver.get_mech_column_to_columns_marked_rows(
    chn_name, mech_key, prop_col, mark=mark, dropnanrow=dropnanrow
)
...
data_df = data_saver.reshape_data_df(
    chn_name,
    mark=mark,
    dropnanrow=dropnanrow,
...
prop_df = data_saver.reshape_mech_df(
    chn_name, mech_key, mark=mark, dropnanrow=dropnanrow, dropnancolumn=dropnancolumn
)
print(prop_df.head())
