...
def run(detach: bool, port: tuple[int, ...], preview: bool, env: tuple[str, ...], command: tuple[str, ...]) -> None:
    ...
    """
    # Check auth first (PRODUCT LAUNCH DEC 2024)
    config = load_config()
    if not config.auth_token:
        console.print()
        console.print("[yellow]🐰 not logged in[/yellow]")
        console.print("[dim]run 'vmux login' first to authenticate with GitHub.[/dim]")
        console.print()
        sys.exit(1)
    ...
    try:
        ...
        err_str = str(e)
        # Handle waitlist/quota errors gracefully (PRODUCT LAUNCH DEC 2024)
        if "waitlist" in err_str.lower() or "need to join" in err_str.lower():
            console.print()
            console.print("[yellow]🐰 not on the waitlist yet[/yellow]")
            console.print("[dim]run: vmux waitlist your@email.com[/dim]")
            console.print()
        elif "running job" in err_str.lower() or "quota" in err_str.lower() or "jobs running" in err_str.lower():
            console.print()
            console.print("[yellow]🐰 too many jobs running[/yellow]")
            console.print(f"[dim]{err_str}[/dim]")
            console.print()
            console.print("[dim]run 'vmux ps' to see your jobs, 'vmux stop <id>' to free a slot.[/dim]")
            console.print()
        else:
            error(err_str)
        sys.exit(1)
...
def _list_jobs(limit: int) -> None:
    ...
    if not config.auth_token:
        console.print()
        console.print("[yellow]🐰 not logged in[/yellow]")
        console.print("[dim]run: vmux login your@email.com[/dim]")
        console.print()
        sys.exit(1)
...
@cli.command()
def login() -> None:
    """Login with GitHub and join the waitlist."""
    ...
    try:
        ...
        # Prompt for email to join waitlist (PRODUCT LAUNCH DEC 2024)
        console.print()
        email = click.prompt("email (for early preview)", type=str)

        if email:
            with TupClient(cfg) as client:
                waitlist_result = client.join_waitlist(email.strip())

                console.print()
                if waitlist_result.get("status") == "admin":
                    console.print("[bold green]🐰 you're an admin[/bold green]")
                elif waitlist_result.get("early_access_slot") or waitlist_result.get("approved_at"):
                    console.print("[bold green]🐰 you're in![/bold green]")
                    console.print()
                    console.print("[dim]run 'vmux run <command>' to start.[/dim]")
                elif waitlist_result.get("position"):
                    pos = waitlist_result["position"]
                    console.print(f"[green]🐰 you're #{pos} in line[/green]")
                    console.print()
                    console.print("[dim]we'll email you when there's room.[/dim]")
...
# --- Waitlist Commands (PRODUCT LAUNCH DEC 2024) ---

@cli.command()
@click.argument("email")
def waitlist(email: str) -> None:
    """Join the waitlist with your email.

    \b
    Examples:
        vmux waitlist you@example.com
    """
    config = load_config()
    if not config.auth_token:
        error("Not logged in. Run: vmux login")
    ...
    with TupClient(config) as client:
        try:
            result = client.join_waitlist(email)
            ...
            if result.get("status") == "admin":
                console.print("[bold green]🐰 you're an admin - hop right in![/bold green]")
            elif result.get("status") == "approved" or result.get("early_access_slot"):
                slot = result.get("early_access_slot", "")
                if slot:
                    console.print(f"[bold green]🐰 you're in! early access #{slot}[/bold green]")
                else:
                    console.print("[bold green]🐰 you're already in the burrow![/bold green]")
                console.print(f"  [dim]max concurrent jobs: {result.get('max_concurrent_jobs', 2)}[/dim]")
                console.print()
                console.print("[dim]run 'vmux run <command>' to start.[/dim]")
            elif "approved_at" in result:
                console.print("[bold green]🐰 you're already in the burrow![/bold green]")
                console.print(f"  [dim]max concurrent jobs: {result.get('max_concurrent_jobs', 2)}[/dim]")
            elif "position" in result:
                pos = result["position"]
                console.print(f"[bold green]🐰 you're in the queue![/bold green]")
                console.print(f"  [dim]position:[/dim] #{pos}")
                console.print(f"  [dim]email:[/dim] {result.get('email', email)}")
                console.print()
                console.print("[dim]we'll email you when there's room in the burrow.[/dim]")
            else:
                console.print(result.get("message", "Joined waitlist"))
            console.print()
...
@cli.command()
def status() -> None:
    """Check your waitlist/approval status.
    ...
    with TupClient(config) as client:
        try:
            result = client.get_waitlist_status()

            console.print()
            status_val = result.get("status", "none")

            if status_val == "admin":
                console.print("[bold green]🐰 admin bunny (unlimited)[/bold green]")
            elif status_val == "approved":
                max_jobs = result.get("max_concurrent_jobs", 2)
                running = result.get("running_jobs", 0)
                console.print("[bold green]🐰 you're in the burrow![/bold green]")
                console.print(f"  [dim]running jobs:[/dim] {running}/{max_jobs}")
                console.print(f"  [dim]approved:[/dim] {result.get('approved_at', 'N/A')[:10]}")
            elif status_val == "waitlist":
                pos = result.get("position", "?")
                console.print(f"[green]🐰 waiting outside (position #{pos})[/green]")
                console.print(f"  [dim]email:[/dim] {result.get('email', 'N/A')}")
                console.print(f"  [dim]signed up:[/dim] {result.get('signed_up_at', 'N/A')[:10]}")
            else:
                console.print("[yellow]🐰 not in line yet[/yellow]")
                console.print("[dim]run: vmux waitlist your@email.com[/dim]")
