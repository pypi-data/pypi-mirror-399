...
@click.option("--preview", "-p", is_flag=True, help="Show preview URL for web servers (port 8000)")
@click.option("--env", "-e", multiple=True, help="Environment variable (KEY=VALUE)")
@click.argument("command", nargs=-1, required=True)
def run(detach: bool, preview: bool, env: tuple[str, ...], command: tuple[str, ...]) -> None:
    ...
    Examples:
        ...
        vmux run --preview uvicorn main:app    # Get a public URL
        vmux run -p python server.py           # Short form
    """
    env_vars = {}
    for e in env:
        if "=" not in e:
    ...
    try:
        run_command(" ".join(command), env_vars=env_vars or None, detach=detach, preview=preview)
