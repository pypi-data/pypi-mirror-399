...
class TupClient:
    ...
    # --- Waitlist (PRODUCT LAUNCH DEC 2024) ---
    ...
    def join_waitlist(self, email: str) -> dict:
        ...
        # Don't raise_for_status - we want to handle 4xx responses gracefully
        return response.json()

    def get_waitlist_status(self) -> dict:
        """Get current waitlist/approval status."""
        response = self._client.get("/waitlist/status", headers=self._headers())
        response.raise_for_status()
        return response.json()
