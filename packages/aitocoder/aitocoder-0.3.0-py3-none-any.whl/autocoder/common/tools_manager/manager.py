...
class ToolsManager:
    ...
    3. ~/.ac_config/.autocodertools
    4. ~/.ac_config/.autocodertools/repos/<项目名>
    """
    ...
    def _find_tools_directories(self) -> List[str]:
        ...
        # 3. ~/.ac_config/.autocodertools
        config.add_directory(
            str(GLOBAL_CONFIG_DIR / ".autocodertools"),
            priority=3,
            validation_mode=ValidationMode.HAS_FILES
        )

        # 4. ~/.ac_config/.autocodertools/repos/<项目名> (最低优先级)
        config.add_directory(
            str(GLOBAL_CONFIG_DIR / ".autocodertools" / "repos" / project_name),
            priority=4,
            validation_mode=ValidationMode.HAS_FILES
        )
    ...
    def get_tools_prompt(self) -> str:
        ...
        **Usage Instructions**:
        ...
        3. **Global user**: `~/.ac_config/.autocodertools/`
        4. **Project-specific global**: `~/.ac_config/.autocodertools/repos/{{ project_name }}/` (lowest priority)
        ...
        **Method 2: File header comments**
        ...
        ## Important Rules
        1. If you use python, try to use uv to create project {{ autocoder_home }}/.ac_config/tool_repos and run the tool. (we not recommend using python to build tools, please try to use Go first)
        ...
        3. **STRONGLY RECOMMENDED**: Prefer Go for building tools， here is the steps:
           - Create a tool project in {{ autocoder_home }}/.ac_config/tool_repos
