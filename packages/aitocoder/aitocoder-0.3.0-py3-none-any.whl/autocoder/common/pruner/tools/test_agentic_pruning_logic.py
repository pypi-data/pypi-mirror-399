...
def verify_pruning_with_message_ids():
    ...
    if original_tokens <= args.conversation_prune_safe_zone_tokens:
        print(f"\u26A0 注意: token数未超过阈值，可能不会触发裁剪逻辑")
    ...
    try:
        ...
        if len(range_pruned) < len(original_conversations):
            ...
            print(f"   \u26A0 Message IDs pruning 没有删除任何消息")
    ...
    if message_ids_config.preserve_pairs:
        ...
        print(f"   配对完整性: {'\u2713 良好' if len(user_assistant_pairs) > 0 else '\u26A0 需要检查'}")
...
def verify_tool_cleanup_logic():
    ...
    if tool_result_cleaned:
        ...
        print(f"\u26A0 注意: 工具输出可能未被清理（可能因为token数仍在阈值内）")
        return True
...
def main():
    ...
    print(f"\n{'\u2713 所有测试通过!' if all_passed else '\u26A0 部分测试失败，请检查上述结果'}")
