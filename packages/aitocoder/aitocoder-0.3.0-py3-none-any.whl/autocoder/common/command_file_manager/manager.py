...
class CommandManager:
    ...
    def __init__(self, commands_dir: Optional[str] = None):
        ...
        Args:
            commands_dir: 命令文件目录路径，如果为None则使用优先级目录查找器按优先级查找和合并:
                        ...
                        3. ~/.ac_config/.autocodercommands
                        4. ~/.ac_config/.autocodercommands/repos/{项目名}/
        """
    ...
    def _find_commands_directories(self) -> List[str]:
        ...
        try:
            ...
            # 3. 全局目录
            config.add_directory(
                path=str(GLOBAL_CONFIG_DIR / ".autocodercommands"),
                priority=3,
                validation_mode=ValidationMode.HAS_SPECIFIC_FILES,
                file_filter=md_filter,
                description="全局命令目录"
            ...
            repos_dir = str(GLOBAL_CONFIG_DIR / ".autocodercommands" / "repos" / project_name)
            config.add_directory(
                path=repos_dir,
                priority=4,
                validation_mode=ValidationMode.HAS_SPECIFIC_FILES,
