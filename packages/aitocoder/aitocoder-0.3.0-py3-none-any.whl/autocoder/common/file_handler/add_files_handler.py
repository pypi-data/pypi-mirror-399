...
def find_files_simple(patterns: List[str], project_root: str = None) -> List[str]:
    """
    Find files matching patterns without requiring ProjectScanner.

    This is a fallback implementation that works with:
    - Direct paths: ./src/foo.py
    - Glob patterns: src/**/*.py, *.py
    ...
    for pattern in patterns:
        ...
        if "*" in pattern or "?" in pattern:
            # Make pattern relative to project root if not absolute
            ...
            if os.path.isabs(pattern):
                ...
                abs_path = os.path.abspath(os.path.join(project_root, pattern))
...
def find_files_in_project_with_fallback(patterns: List[str]) -> List[str]:
    """
    Find files using ProjectScanner if available, otherwise use simple fallback.
    ...
    """
    try:
        # Try to use the scanner-based function
        ...
        if completer.is_scanner_initialized():
            ...
            return find_files_in_project(patterns)
    except (ImportError, AttributeError):
        pass

    # Fallback to simple implementation
    return find_files_simple(patterns)
...
class AddFilesHandler:
    ...
    def _handle_regular_files(self, args: List[str]) -> None:
        ...
        matched_files = find_files_in_project_with_fallback(args)
