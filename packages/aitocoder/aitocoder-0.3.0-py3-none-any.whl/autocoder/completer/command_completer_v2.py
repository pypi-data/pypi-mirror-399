...
class CommandCompleterV2(Completer):
    ...
    Supports lazy initialization of ProjectScanner for fast startup.
    Call initialize_project_scanner() or use /init command to enable
    file path and symbol autocompletion.
    """
    ...
    def __init__(self, commands: List[str], memory_model: MemoryConfig, project_root: Optional[str] = None):
        ...
        # Scanner is NOT created at init time - deferred for fast startup
        self.scanner = None
        self._scanner_initialized = False

        # Data stores, initialized empty (populated after scanner init)
        ...
        # Load dynamic data that doesn't require scanner (groups, libs, models)
    ...
    def initialize_project_scanner(self) -> bool:
        """
        Initialize the ProjectScanner for file/symbol autocompletion.

        This is called explicitly via /init command or can be triggered
        programmatically. Returns True if initialization was successful.
        """
        if self._scanner_initialized:
            return True
        ...
        try:
            ...
            if hasattr(self.memory_model, 'get_memory_func'):
            ...
            import sys
            print(f"Warning: Failed to initialize project scanner: {e}", file=sys.stderr)
            return False

    def is_scanner_initialized(self) -> bool:
        """Check if the project scanner has been initialized."""
        return self._scanner_initialized
    ...
    def _update_dynamic_data(self):
        ...
        # Update scanner's extra exclude dirs if changed (only if scanner is initialized)
        if self._scanner_initialized and self.scanner is not None:
            memory = self.memory_model.get_memory_func()
            extra_exclude_dirs = memory.get("exclude_dirs", [])
            self.scanner.update_extra_exclude_dirs(extra_exclude_dirs)
    ...
    def refresh_files(self):
        ...
        If scanner is not initialized, this only updates dynamic data
        (groups, libs, current files) without file/symbol lists.
        """
        if not self._scanner_initialized or self.scanner is None:
            # Scanner not ready - only update dynamic data
            self._update_dynamic_data()
            self._update_command_files_cache()
            return
    ...
    def _handle_mode(self, document: Document, complete_event: CompleteEvent, word: str, text: str) -> Iterable[Completion]:
        ...
        modes = ["normal", "auto_detect", "shell"]
