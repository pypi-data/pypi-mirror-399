...
def show_help():
    def cmd(name, args="", desc="", indent=2):
        """Format a command line"""
        prefix = " " * indent
        if args:
            console.print(f"{prefix}[{THEME.COL1}]{name}[/{THEME.COL1}] [{THEME.COL2}]{args}[/{THEME.COL2}] - [{THEME.COL2}]{desc}[/{THEME.COL2}]")
        else:
            console.print(f"{prefix}[{THEME.COL1}]{name}[/{THEME.COL1}] - [{THEME.COL2}]{desc}[/{THEME.COL2}]")

    console.print()
    console.print(f"[bold]{i18n_('help.official_doc')}[/bold]")
    console.print()
    console.print(f"[{THEME.COL2}]│ {i18n_('help.start_building')}[/{THEME.COL2}]")
    console.print(f"[{THEME.COL2}]│ {i18n_('help.change_model_hint')}[/{THEME.COL2}]")
    console.print()
    console.print(f"[bold]{i18n_('help.supported_commands')}[/bold]")
    console.print()
    cmd("/conf", "<key>:<value>", i18n_('help.conf_desc'))
    # cmd("/shell", "<command>", i18n_('help.shell_desc'))
    # cmd("/shell", "", i18n_('help.shell_interactive_desc'))
    # cmd("!", "<command>", i18n_('help.shell_single_command_desc'))
    # cmd("/add_files", "<file1> <file2> ...", i18n_('help.add_files_desc'))
    # cmd("/remove_files", "<file1>,<file2> ...", i18n_('help.remove_files_desc'))
    # cmd("/chat", "<query>", i18n_('help.chat_desc'))
    # cmd("/coding", "<query>", i18n_('help.coding_desc'))
    # cmd("/revert", "", i18n_('help.revert_desc'))
    # cmd("/index/query", "<args>", i18n_('help.index_query_desc'))
    # cmd("/index/build", "", i18n_('help.index_build_desc'))
    # cmd("/list_files", "", i18n_('help.list_files_desc'))
    cmd("/help", "", i18n_('help.help_desc'))
    # cmd("/mode", "", i18n_('help.mode_desc'))
    # cmd("/lib", "", i18n_('help.lib_desc'))
    cmd("/models", "", i18n_('help.models_desc'))
    # cmd("/plugins", "", i18n_('help.plugins_desc'))
    # cmd("/active_context", "", i18n_('help.active_context_desc'))
    cmd("/language", "", i18n_('help.language_desc'))
    # cmd("/init", "", "Initialize project scanner for file/symbol autocompletion")
    cmd("/exit", "", i18n_('help.exit_desc'))

    # # Plugin commands
    # if plugin_manager.command_handlers:
    #     console.print(f"[bold]{i18n_('plugin.commands_title')}[/bold]")
    #     console.print(f"  [{THEME.COL1}]{i18n_('plugin.command_header')}[/{THEME.COL1}] - [{THEME.COL2}]{i18n_('plugin.description_header')}[/{THEME.COL2}]")
    #     for cmd_name, (_, desc, plugin_id) in plugin_manager.command_handlers.items():
    #         plugin = plugin_manager.get_plugin(plugin_id)
    #         if plugin:
    #             console.print(f"  [{THEME.COL1}]{cmd_name}[/{THEME.COL1}] - [{THEME.COL2}]{desc} ({i18n_('plugin.from')} {plugin.plugin_name()})[/{THEME.COL2}]")
    #         else:
    #             console.print(f"  [{THEME.COL1}]{cmd_name}[/{THEME.COL1}] - [{THEME.COL2}]{desc} ({i18n_('plugin.from_unknown')})[/{THEME.COL2}]")
    #     console.print()
...
def load_builtin_plugins():
    ...
    try:
        ...
        for plugin_class in discovered_plugins:
            ...
            if (
                ...
                try:
                    ...
                    print(
                        i18n_("plugin.load_failed", plugin_name=plugin_class.plugin_name(), error=str(e))
                    )
        ...
        print(i18n_("plugin.load_error", error=str(e)))
    ...
    while not stop_event.is_set():
        try:
            ...
            try:
                ...
                async_agent_dir = GLOBAL_CONFIG_DIR / "async_agent"
            ...
            if ARGS and ARGS.debug:
                print(i18n_("background.task_error", error=str(e)))
...
async def process_user_input(user_input: str, new_prompt_callback, session=None):
    ...
    try:
        ...
        # Handle /conf and its subcommands like /conf /export, /conf /import
            ...
            # Temporarily install a handler that raises KeyboardInterrupt
            # so Rich Prompt's try/except can catch it properly.
            import signal
            def raise_keyboard_interrupt(signum, frame):
                raise KeyboardInterrupt()
            old_handler = signal.signal(signal.SIGINT, raise_keyboard_interrupt)
            try:
                language_command()
            finally:
            ...
            # Initialize project scanner for file/symbol autocompletion
            if completer.is_scanner_initialized():
                print("\033[93mProject scanner is already initialized.\033[0m")
            else:
                print("\033[94mInitializing project scanner for file/symbol autocompletion...\033[0m")
                success = completer.initialize_project_scanner()
                if success:
                    print("\033[92m✓ Project scanner initialized successfully. File and symbol autocompletion is now available.\033[0m")
                else:
                    print("\033[91m✗ Failed to initialize project scanner. File autocompletion will not be available.\033[0m")
            ...
            if not query:
                print(f"\033[91m{i18n_('input.please_enter_request')}\033[0m")
                return
            ...
            if not query:
                print(f"\033[91m{i18n_('input.please_enter_design_request')}\033[0m")
            else:
                design(query)
            ...
            if not query:
                print(i18n_("input.please_enter_query"))
            else:
                mcp(query)
            ...
            try:
                result = eval(code)
                print(i18n_("debug.result", result=str(result)))
            except Exception as e:
                print(i18n_("debug.error", error=str(e)))
            ...
            if not command:
                ...
                print(i18n_("mode.switched_to_shell"))
            else:
        ...
        print(
            f"\033[91m{i18n_('task.error_occurred', error_type=type(e).__name__, message=str(e))}\033[0m"
        )
        if ARGS and ARGS.debug:
            import traceback
...
async def run_app():
    """运行聊天应用"""
    global ask, coding, chat, design, auto_command, run_agentic, active_context, execute_shell_command
    ...
    async def trigger_task_completion_callbacks():
        ...
        for callback in task_completion_callbacks:
            try:
                ...
                print(
                    f"\033[91m{i18n_('task.callback_error', error_type=type(e).__name__, message=str(e))}\033[0m"
                )
                if ARGS and ARGS.debug:
                    import traceback
    ...
    def get_bottom_toolbar():
        ...
        try:
            ...
            if os.path.exists(meta_dir):
                ...
                if running_count > 0:
                    async_tasks_info = f" | {i18n_('toolbar.async_tasks')} 🔄 {running_count}"
        except Exception:
            # 静默处理异常，不影响底部工具栏的显示
            pass
        ...
        return FormattedText([
            ("class:toolbar-label", f"│ {i18n_('toolbar.project_dir')} "),
            ("class:toolbar-value", pwd),
            ("class:toolbar-label", async_tasks_info),
            ("", "\n"),
            ("class:toolbar-label", f"│ {i18n_('toolbar.model')} "),
    ...
    style = Style.from_dict(
        {
            "ac-prompt": f"{THEME.COL2} bold",
            # Transparent bottom toolbar styles (override default reverse video)
            "bottom-toolbar": "noreverse",
            "toolbar-label": "ansigray",
            "toolbar-value": f"{THEME.COL1}",
            "toolbar-model": f"{THEME.COL2}",
            "toolbar-async": "#00aa00",
            # Completion menu styles
            "completion-menu": "bg:default",
            "completion-menu.completion": "fg:ansigray bg:default",
            "completion-menu.completion.current": f"fg:{THEME.COL2} bg:default",
            "completion-menu.meta": "ansigray bg:default",
            "completion-menu.meta.current": f"ansiwhite bg:{THEME.COL2}",
            "scrollbar.background": "bg:default",
            "scrollbar.button": f"bg:{THEME.COL2}",
        }
    )
    ...
    try:
        ...
        print(
            f"\033[91m{i18n_('task.cleanup_error', error_type=type(e).__name__, message=str(e))}\033[0m"
        )

    print(f"\n\033[93m{i18n_('repl.goodbye')}\033[0m")
    print()
