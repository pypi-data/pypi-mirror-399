...
class AgentManager:
    ...
    def _get_agents_search_paths(self) -> List[Path]:
        ...
        # 3. ~/.ac_config/.autocoderagents (lowest priority)
        home_autocoder_dir = GLOBAL_CONFIG_DIR / ".autocoderagents"
    ...
    def _load_agents(self) -> None:
        ...
        try:
            ...
            config.add_directory(
                path=str(GLOBAL_CONFIG_DIR / ".autocoderagents"),
                priority=3,
                validation_mode=ValidationMode.HAS_SPECIFIC_FILES,
                file_filter=md_filter,
            ...
            # 添加repos目录支持
            repos_dir = str(GLOBAL_CONFIG_DIR / ".autocoderagents" / "repos" / project_name)
            config.add_directory(
                path=repos_dir,
                priority=4,
                validation_mode=ValidationMode.HAS_SPECIFIC_FILES,
