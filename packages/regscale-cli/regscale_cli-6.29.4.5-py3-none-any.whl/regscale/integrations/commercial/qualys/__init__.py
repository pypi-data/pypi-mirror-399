...
def diagnostics(output, fetch_samples):
    ...
    from rich.console import Console
    from regscale.integrations.commercial.qualys.qualys_diagnostics import (
        run_all_diagnostics,
    ...
    base_url = config.get("qualysUrl", "https://qualysapi.qualys.com")

    if not username or not password:
        error_and_exit("Missing qualysUserName or qualysPassword in init.yaml")

    # Run all diagnostic tests
    console.print("\n[cyan]Testing VMDR/Total Cloud API...[/cyan]")
    if fetch_samples:
        console.print("  [cyan](Fetching sample data...)[/cyan]")
    results = run_all_diagnostics(username, password, base_url, fetch_samples)

    # Display test results
    vmdr_status = results["tests"]["vmdr"]["status"]
    if vmdr_status == "success":
        console.print("  [OK] VMDR API authentication successful")
    else:
        console.print(f"  [FAIL] VMDR API {vmdr_status}: {results['tests']['vmdr'].get('error', 'Unknown')}")

    console.print("\n[cyan]Testing Container Security API...[/cyan]")
    container_status = results["tests"]["container_security"]["status"]
    if container_status == "success":
        console.print("  [OK] Container Security API authentication successful")
    else:
        error_msg = results["tests"]["container_security"].get("error", "Unknown")
        console.print(f"  [FAIL] Container Security {container_status}: {error_msg}")

    console.print("\n[cyan]Testing WAS (Web Application Scanning) API...[/cyan]")
    was_status = results["tests"]["was"]["status"]
    if was_status == "success":
        console.print("  [OK] WAS API authentication successful")
    else:
        console.print(f"  [FAIL] WAS {was_status}: {results['tests']['was'].get('error', 'Unknown')}")
    ...
    if available:
        console.print(f"  [green][OK] Available modules: {', '.join(available)}[/green]")
    if unavailable:
        console.print(f"  [yellow][WARN] Unavailable modules: {', '.join(unavailable)}[/yellow]")

    # Save to file
    if not output:
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        output = f"qualys_diagnostics_{timestamp}.json"

    try:
        save_diagnostics_report(results, output)
        console.print(f"\n[green][OK] Diagnostics saved to: {output}[/green]")
    except Exception as e:
        logger.error("Failed to save diagnostics: %s", e)
        console.print(f"\n[red][FAIL] Failed to save diagnostics: {e}[/red]")
