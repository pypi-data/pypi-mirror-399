...
The SDK provides a unified `VirshSandbox` client that simplifies API usage:
...
async def main():
    ...
    response = await client.sandbox.start_sandbox("sandbox-id", request)

    # Run commands via tmux
    result = await client.command.run_command(cmd_request)

    # File operations
    await client.file.read_file(request)

    # Tmux operations
    await client.tmux.list_tmux_panes()
...
async def main_with_context():
    async with VirshSandbox(host="http://localhost:8080") as client:
        ...
        # - client.file      (FileApi)
        # - client.tmux      (TmuxApi)
        # - client.ansible   (AnsibleApi)
        # - client.audit     (AuditApi)
        # - client.health    (HealthApi)
        # - client.human     (HumanApi)
        # - client.plan      (PlanApi)
        # - client.vms       (VMsApi)
        pass
...
For more control, you can use the individual API classes directly:
