        ...
        try:
            async with ZenableMCPClient() as client:
                ...
                results = await client.check_conformance(
                    files, show_progress=False, format="json"
                )
                ...
                # Parse JSON response to get authoritative pass/fail status
                has_findings = True  # Default to having findings if parsing fails
                findings_text = result_text  # Fallback to raw text
                try:
                    json_result = json.loads(result_text)
                    echo(
                        f"Received JSON response: {result_text}",
                        persona=Persona.DEVELOPER,
                        log=False,
                    )
                    # Use the authoritative 'passed' field from JSON
                    has_findings = not json_result.get("passed", False)
                    echo(
                        f"has_findings (from JSON passed field): {has_findings}",
                        persona=Persona.DEVELOPER,
                    )
                    # Format findings for followup_message
                    findings_list = json_result.get("findings", [])
                    if findings_list:
                        findings_text = "\n".join(
                            f"- {f.get('finding', '')} ({f.get('location', 'unknown')})"
                            for f in findings_list
                        )
                    else:
                        findings_text = ""
                except json.JSONDecodeError:
                    # Fallback to text-based check if JSON parsing fails
                    echo(
                        "JSON parsing failed, falling back to text-based check",
                        persona=Persona.DEVELOPER,
                    )
                    has_findings = client.has_findings(result_text)
                    echo(
                        f"has_findings (text-based fallback): {has_findings}",
                        persona=Persona.DEVELOPER,
                    )

                # Build handler-specific response
                formatted_response = active_handler.build_response_to_hook_call(
                    has_findings, findings_text
                )
