...
from trexlib.utils.google.bigquery_util import create_bigquery_client, stream_data,\
    update_stream_data
from trexconf import conf as analytics_conf, conf
...
class TriggerImportAllUpstreamData(TriggerImportDataBaseResource):
    ...
    def get_data_payload(self, request_values):
        import_limit    = request_values.get('limit') or 100
        return {
...
class InitImportAllUpstreamData(InitImportDataBaseResource):
    def get_import_data_count(self, **kwargs):
        import_limit    = kwargs.get('limit')
        logger.info('import_limit=%s', import_limit)
        
        if is_empty(import_limit):
            count =  count_all_unsend_upstream_data()
            logger.info('InitImportAllUpstreamData debug: unsend upstream data count = %d', count)
            return count
        else:
            return int(import_limit)
    ...
    @request_values
    def get_data_payload(self, request_values):
        import_limit    = request_values.get('limit') or analytics_conf.STREAM_DATA_PAGE_SIZE
        return {
                'limit': int(import_limit)
                }
...
def import_upstream_data_by_limit_function(credential_info=None, dataset_name='system_dataset', 
    ...
    logger.info('credential_filepath=%s', analytics_conf.BIGQUERY_SERVICE_CREDENTIAL_PATH)
    bg_client       = create_bigquery_client(credential_filepath=analytics_conf.BIGQUERY_SERVICE_CREDENTIAL_PATH, project_id=analytics_conf.BIGQUERY_GCLOUD_PROJECT_ID)
    
    logger.info('bg_client.project=%s', bg_client.project)
    ...
    logger.info('import_upstream_data_by_limit_function limit=%s', limit)
    ...
    if errors==[]:
        logger.info('imported_count=%d', imported_count)
    else:
