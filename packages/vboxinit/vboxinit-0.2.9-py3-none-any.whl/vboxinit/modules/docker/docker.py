...
# GPG 密钥配置
DOCKER_GPG_URL_TEMPLATE = "https://download.docker.com/linux/{distro}/gpg"
...
def setup_gpg_key(conn: Connection, distro: str | None = None) -> bool:
    ...
    1. 确保 /etc/apt/keyrings 目录存在且权限为 0755
    2. 下载 Docker GPG 密钥到 keyrings 目录
    3. 对密钥进行 sha256 校验并记录结果
    ...
    # region 下载 GPG 密钥
    gpg_url = DOCKER_GPG_URL_TEMPLATE.format(distro=distro)
    conn.sudo(f"curl -fsSL {gpg_url} -o {DOCKER_GPG_PATH}", env=ENV_LANG_C)
...
def setup_docker_repo(conn: Connection, mirror: str = DEFAULT_MIRROR) -> None:
    ...
    arch = _normalize_arch(conn.hostvars["architecture"])
    # endregion      
...
@task
def configure_daemon(conn: Connection, config: dict | None = None) -> None:
    """配置 Docker daemon

    1. 确保 /etc/docker 目录存在且权限为 0755
    2. 生成并写入 daemon.json 配置文件

    Args:
        conn: Fabric 连接对象
        config: daemon.json 配置字典，默认使用 DEFAULT_DAEMON_CONFIG
    """
    # region 创建 /etc/docker 目录
    conn.sudo("install -m 0755 -d /etc/docker", env=ENV_LANG_C)
    # endregion

    # region 生成并写入 daemon.json
    daemon_config = config if config is not None else DEFAULT_DAEMON_CONFIG
    daemon_json = json.dumps(daemon_config, indent=2)
    daemon_path = "/etc/docker/daemon.json"
    conn.sudo(f"tee {daemon_path} > /dev/null << 'EOF'\n{daemon_json}\nEOF", env=ENV_LANG_C)
    print(f"[OK] Docker daemon config written to {daemon_path}")
    # endregion
...
def enable_docker(conn: Connection) -> None:
    ...
    使用 systemctl enable docker 设置 Docker 服务开机自启
...
def restart_docker(conn: Connection) -> None:
    ...
    使用 systemctl restart docker 重启 Docker 服务
...
def _normalize_arch(uname_arch: str) -> str:
