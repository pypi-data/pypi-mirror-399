...
__all__ = ["ObjectText", "ObjectInputTextLine", "ObjectInputTextBox",
           "ObjectInputPassLine", "GetClipboardText", "SetClipboardText"]
...
class _ObjectText:
    def __init__(self, font_size, text, font):
        ...
        self._text_surface_size = sdlttf.TTF_SizeUTF8(self._text_font, self._text.encode())
    ...
    @property
    def surfaceSize(self) -> tuple[int, int]:
    ...
    def _single_threading_render(self):
        self._surface = sdlttf.TTF_RenderUTF8_Blended_Wrapped(self._text_font, self.text if self._text else " ", self._color.SDLColor, 0).contents
...
class ObjectText(_ObjectText):
    def __init__(self, location=(0, 0), rotation: int = 0, font_size: int = 20, text: str = "", font=_DefaultFont):
        ...
        self.variables["_type"] = "ObjectText"
...
class _ObjectInputText(_ObjectText):
    def __init__(self, font_size, text, font):
        ...
        self._charFilter = None

        self.variables["_type"] = "ObjectInputText"
    ...
    def _inputText(self, text):
        if text is not None:
            if self._charFilter is not None:
                if text not in self._charFilter:
                    return False
    ...
    @property
    def charFilter(self):
        return self._charFilter

    @charFilter.setter
    def charFilter(self, chars):
        self._charFilter = chars if chars else None

    def setNumericFilter(self):
        self._charFilter = string.digits
...
class ObjectInputTextLine(_ObjectInputText):
    ...
    def update(self, window, camera: Camera = None):
        ...
        if self._State:
            ...
            if keyboard(KeyboardButton.KeyCtrl, True):
            ...
            if keyboard(KeyboardButton.KeyShift):
                self.selected[0] = self._position
            if keyboard(KeyboardButton.KeyShift, True):
            ...
            if click is not None:
                self._p_start_timer.restart()
                self._p_timer.flipflop = True
                if click == KeyboardButton.KeyBack:
                    if self.selected[0] != self.selected[1]:
                        self._inputText("")
                    else:
                ...
                elif click == KeyboardButton.KeyTab:
                    self._inputText("   ")

                # motion
                elif click in (KeyboardButton.KeyLeft, KeyboardButton.KeyRight):
                    if click == KeyboardButton.KeyLeft:
                        self._position -= 1
                    elif click == KeyboardButton.KeyRight:
                        self._position += 1
                    if not keyboard(KeyboardButton.KeyShift, True):
                        self.selected[0] = self.selected[1] = self._position

                elif click in (KeyboardButton.KeyUp, KeyboardButton.KeyDown):
                    if not keyboard(KeyboardButton.KeyShift, True):
                        self.selected[0] = self.selected[1] = self._position
...
class ObjectInputTextBox(_ObjectInputText):
    ...
    def update(self, window, camera: Camera = None):
        ...
        if self._State:
            ...
            if click is not None:
                ...
                elif click == KeyboardButton.KeyReturn:
                    self._inputText("\n")
                elif click == KeyboardButton.KeyTab:
                    self._inputText(" " * 4)
    ...
    def drawObject(self, window, camera: Camera = None):
        ...
        if _text and self.selected[0] != self.selected[1]:
            _selected_texts = self.selectedText.split("\n")

            _line = _text[:min(self.selected[0], self.selected[1])].count("\n")
            _size = sdlttf.TTF_SizeUTF8(
                self._text_font,
                _texts[_line][:self.selectedBox[0] - sum(
                    len(_texts[lo]) + (1 if lo != 0 else 0) for lo in range(_line)
                )].encode()
            )
            ...
            for num in range(len(_selected_texts)):
                ...
                sdlgfx.boxRGBA(
                    self._surface_object_rd,
                    _loc[0] + self.scroll[0],
                    _loc[1] + self.scroll[1],
                    _loc[0] + _ss[0] - 1 + self.scroll[0],
                    _loc[1] + _ss[1] - 1 + self.scroll[1],
                    75, 75, 255, 170
                )
        ...
        if _text:
            ...
            if self._surface is not None:
                self._text_surface_tx = sdl2.SDL_CreateTextureFromSurface(
                    self._surface_object_rd, self._surface
                )
                sdl2.SDL_SetTextureScaleMode(self._text_surface_tx, 1)
                sdl2.SDL_RenderCopy(
                    self._surface_object_rd,
                    self._text_surface_tx,
                    None,
                    sdl2.SDL_Rect(
                        self.scroll[0] + 1,
                        self.scroll[1],
        ...
        if self._State:
            if self._p_timer.tickFlipFlop() or not self._p_start_timer.tick():
                _line = self.line if self.line < len(_texts) else 0
                _column = self.column if _text else 0

                _size = sdlttf.TTF_SizeUTF8(
                    self._text_font,
                    _texts[_line][:_column].encode()
                )

                _loc_start = [
                    _size[0] + 1 + self.scroll[0],
                    _size[1] * _text[:self._position].count("\n") + 1 + self.scroll[1]
                ]
                _loc_end = [_loc_start[0], _loc_start[1] + _size[1] - 3]

                sdlgfx.thickLineRGBA(
                    self._surface_object_rd,
                    *_loc_start,
                    *_loc_end,
                    2,
