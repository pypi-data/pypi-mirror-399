...
from .Common import AllEvents, WindowEvents, _temp, _calculate_object2d, _calculate_size, _calculate_location
...
from .Constants import All, WindowFlag, ImageFormat, Pivot2D, Meshes2D
from .Mesh import rotate_point, CreateMeshPlane, Mesh2D
...
class _drawnObjectsList:
    ...
    def addObject(self, obj_id, metadata=None):
        if obj_id not in self._drawnObjects:
            self._drawnObjects[obj_id] = metadata
            return True
        return False

    def hasObject(self, obj_id):
        return obj_id in self._drawnObjects

    def getObject(self, obj_id):
        return self._drawnObjects.get(obj_id)
        #if self.hasObject(obj_id):
        #    return self._drawnObjects[obj_id]
        #return

    def getAllObjects(self):
        return [obj for obj in self._drawnObjects.values() if obj is not None]

    def getAll(self):
        return self._drawnObjects

    def count(self):
        return len(self._drawnObjects)

    def clear(self):
        self._drawnObjects.clear()

    def removeObject(self, obj_id):
        return self._drawnObjects.pop(obj_id, None)
...
class _Window:
    def __init__(self):
        ...
        self.drawnObjects = _drawnObjectsList()
    ...
    @property
    def isUsingOpenGLContext(self):
        return self.renderer is None and self.context is not None and (self.flags >> 1) & 1
    ...
    def clear(self, force=False, color=(0, 0, 0, 255)):
        if not self.__WindowActive__ or self.isUsingOpenGLContext:
            return
    ...
    def blit(self, surface, location=(0, 0), size=None, rotation=0):
        if not self.__WindowActive__ or self.isUsingOpenGLContext:
            return
    ...
    def drawSquare(self, location, size, rotation, radius, color):
        if not self.__WindowActive__ or self.isUsingOpenGLContext:
            return

        def _square(renderer_, loc_, sz_, rad_, col_):
            if isinstance(rad_, (tuple, list)) and len(rad_) == 4:
                if all(r == rad_[0] for r in rad_):
                    sdlgfx.roundedBoxRGBA(renderer_, loc_[0], loc_[1], loc_[0] + sz_[0] - 1, loc_[1] + sz_[1] - 1, rad_[0], *col_.RGBA)
                else:
                    x, y, w, h = loc_[0], loc_[1], sz_[0], sz_[1]
                    r0, r1, r2, r3 = rad_
                    sdlgfx.filledPieRGBA(renderer_, x + r0, y + r0, r0, 180, 270, *col_.RGBA)
                    sdlgfx.filledPieRGBA(renderer_, x + w - r1 - 1, y + r1, r1, 90, 180, *col_.RGBA)
                    sdlgfx.filledPieRGBA(renderer_, x + w - r2 - 1, y + h - r2 - 1, r2, 0, 90, *col_.RGBA)
                    sdlgfx.filledPieRGBA(renderer_, x + r3, y + h - r3 - 1, r3, 270, 360, *col_.RGBA)
                    sdlgfx.boxRGBA(renderer_, x + r0, y, x + w - r1 - 1, y + h // 2 - 1, *col_.RGBA)
                    sdlgfx.boxRGBA(renderer_, x + r3, y + h // 2, x + w - r2 - 1, y + h - 1, *col_.RGBA)
                    sdlgfx.boxRGBA(renderer_, x, y + r0, x + w // 2 - 1, y + h - r3 - 1, *col_.RGBA)
                    sdlgfx.boxRGBA(renderer_, x + w // 2, y + r1, x + w - 1, y + h - r2 - 1, *col_.RGBA)
            elif isinstance(rad_, int) and rad_ > 0:
                sdlgfx.roundedBoxRGBA(renderer_, loc_[0], loc_[1], loc_[0] + sz_[0] - 1, loc_[1] + sz_[1] - 1, rad_, *col_.RGBA)
            else:
                sdlgfx.boxRGBA(renderer_, loc_[0], loc_[1], loc_[0] + sz_[0] - 1, loc_[1] + sz_[1] - 1, *col_.RGBA)
        if rotation not in (0, 90, 180, 240, 360):
            if (isinstance(radius, (tuple, list)) and len(radius) == 4 and not all(r == 0 for r in radius)) or (isinstance(radius, int) and radius > 0):
                ...
                self.blit(_texture, location, size, rotation)
                sdl2.SDL_DestroyTexture(_texture)
                sdl2.SDL_DestroyRenderer(_renderer)
                sdl2.SDL_FreeSurface(mask)
            else:
                self.drawPolygon(location, (1, 1), rotation, CreateMeshPlane(size), color)
        else:
            _square(self.renderer, location, size, radius, color)

    def drawEdgesSquare(self, location, size, rotation, line_size, radius, color):
        if not self.__WindowActive__ or self.isUsingOpenGLContext:
            return
        ...
        if line_size != 0:
            ...
            if rotation not in (0, 90, 180, 240, 360):
                if (isinstance(radius, (tuple, list)) and len(radius) == 4 and not all(r == 0 for r in radius)) or (isinstance(radius, int) and radius > 0):
                    ...
                    self.drawEdgesPolygon(location, (1, 1), rotation, CreateMeshPlane(size), color)
            else:
                _hollow_square(self.renderer, location, size, line_size, radius, color)

    def drawLine(self, start, end, size, color):
        if not self.__WindowActive__ or self.isUsingOpenGLContext:
            return
        if size <= 1:
            sdlgfx.aalineRGBA(self.renderer, *start, *end, *color.RGBA)
        else:
            sdlgfx.thickLineRGBA(self.renderer, *start, *end, size, *color.RGBA)

    def drawSimpleLine(self, start, end, color):
        if not self.__WindowActive__ or self.isUsingOpenGLContext:
            return
        sdlgfx.lineRGBA(self.renderer, *start, *end, *color.RGBA)

    def drawPolygon(self, location, scale, rotation, mesh, color):
        if not self.__WindowActive__ or self.isUsingOpenGLContext:
            return

        vertices = mesh.vertices

        _mesh = [(round(point[0] * scale[0]), round(point[1] * scale[1])) for point in vertices]

        if rotation != 0:
            _max_vx = max(round(point[0] * scale[0]) for point in vertices)
            _max_vy = max(round(point[1] * scale[1]) for point in vertices)
    ...
    def drawEdgesPolygon(self, location, scale, rotation, mesh, color):
        if not self.__WindowActive__ or self.isUsingOpenGLContext:
            return
    ...
    @property
    def color(self):
        if not self.isUsingOpenGLContext:
...
class Window(_Window):
    def __init__(self, title="MGE", icon=DefaultIcon, resolution=(1280, 720), location=(300, 300), logical_resolution=(0, 0), shape=None, camera=Camera(), render_driver=All, flags=WindowFlag.Shown | WindowFlag.Resizable):
        super().__init__()
    ...
    def update(self, still_frame_optimization=False):
        if not self.__WindowActive__:
            return
        ...
        if self._cache["times"]["standard_time"].tick(True):
            ...
            self._resolution = tuple(sdl2.SDL_GetWindowSize(self.window))
    ...
    def recreate(self, title="MGE", icon=DefaultIcon, resolution=(1280, 720), location=(300, 300), logical_resolution=(0, 0), shape=None, camera=Camera(), flags=WindowFlag.Shown | WindowFlag.Resizable):
        self.close()
    ...
    def logicalResolution(self, resolution):
        if not (self.flags >> 1) & 1:
            ...
            self._logical_resolution = resolution[0], resolution[1]
    ...
    def set_TitleBarColor(self, color):
    ...
    def set_BorderColor(self, color):
        if Platform.system.lower() == "windows" and Platform.system_version >= 22000:
    ...
    def set_TitleColor(self, color):
        if Platform.system.lower() == "windows" and Platform.system_version >= 22000:
            windll.dwmapi.DwmSetWindowAttribute(self._hwnd, 36, byref(c_int(color.Uint32 & ~0xFF000000)), sizeof(c_int))
        else:
            LogError("")

def CreateGlWindow(title="MGE", icon=DefaultIcon, resolution=(1280, 720), location=(300, 300), shape=None, msaa=0, flags=WindowFlag.Shown | WindowFlag.Resizable):
    if msaa:
...
class InternalWindow(_Window):
    def __init__(self, location=(0, 0), rotation=0, size=(1280, 720), scale=(1, 1), resolution=(1280, 720), camera=Camera()):
        super().__init__()
        ...
        self.variables = {"_type": "InternalWindow"}
    ...
    def hover(self, window, camera=None) -> bool:
        if object2dSimpleHover(window, camera, self._location, self._size, self._scale, self._pivot):
            _temp.MouseCursor = self._cursor
            return True
        return False

    def recreate(self, location=(0, 0), rotation=0, size=(1280, 720), scale=(1, 1), resolution=(1280, 720), camera=Camera()):
        self.close()
    ...
    def getTrueLocation(self, window: Window, camera: Camera = None):
    ...
    def getTrueSize(self, window, camera=None):
        _location_camera = camera.location if camera is not None else window.camera.location
        _size_window = window.logicalResolution
