from ..Log import *
from ..Version import __version__, __versionData__, __projectName__
...
def depsInstall():
    ...
    if 'error' in data:
        ...
        def download_with_progress(url, filename):
            Log(f"\rDownloading dependencies...", ConsoleColors.Reset, '')
            with urllib.request.urlopen(url) as response:
                ...
                with open(filename, 'wb') as out_file:
                    while True:
                        ...
                        Log(f"\rDownloading dependencies ({ConsoleColors.Yellow}{percent_complete}%{ConsoleColors.Reset}) {downloaded_size // 1024}Kb / {total_size // 1024}KB", ConsoleColors.Reset, '')
...
def help():
    ...
    Log(f"  LibMGE version         →  Show installed dependency versions", ConsoleColors.Reset)
    Log(f"  LibMGE help            →  Show this help message", ConsoleColors.Reset)
    Log(f"  LibMGE deps install    →  Install required dependencies", ConsoleColors.Reset)
    Log(f"  LibMGE deps versions   →  Show installed dependency versions", ConsoleColors.Reset, "\n\n")
