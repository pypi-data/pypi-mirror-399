...
from .Constants import ImageFormat, Colors
...
class Image:
    ...
    def crop(self, location: list | tuple, size: list | tuple):
        _images = []
        for _image in self.images:
            new_surface = sdl2.SDL_CreateRGBSurfaceWithFormat(0, size[0], size[1], self._format[0], self._format[1])
...
def LoadIcon(path: str) -> Icon:
    ...
    if os.path.exists(path):
        if path[-4:] == ".ico":
            _Icon.icon = sdlimage.IMG_LoadICO_RW(sdl2.SDL_RWFromFile(path, "r")).contents
    if not _Icon.icon:
...
def NewImage(mode=ImageFormat.ARGB, size: list | tuple = (32, 32), color: Color = Colors.StandardColor) -> Image:
    _image = Image()
...
def SaveImagePNG(image: Image, path: str = "image.png"):
    sdlimage.IMG_SavePNG(image.images[0], path)

def SaveImageJPG(image: Image, path: str = "image.jpg", quality: int = 80):
    sdlimage.IMG_SaveJPG(image.images[0], path, quality)
