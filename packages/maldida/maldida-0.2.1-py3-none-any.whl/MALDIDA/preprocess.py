"""Preprocessing utilities for MALDIDA spectra."""
...
class Normalizer:
    ...
    def __init__(self, sum=1.0):
        self.sum = sum

    def __call__(self, SpectrumObj):
        total_intensity = SpectrumObj.intensity.sum()
        if total_intensity == 0:
            raise ValueError("Cannot normalize a spectrum with zero total intensity.")

        return SpectrumObject(
            intensity=SpectrumObj.intensity / total_intensity * self.sum,
            mz=SpectrumObj.mz,
        )
...
class BaselineCorrecter:
    ...
    def __call__(self, SpectrumObj):
        if self.method is None:
            raise ValueError("Set 'method' to 'SNIP', 'ALS', or 'ArPLS'.")
        ...
        if "LS" in self.method:
            ...
            raise ValueError(f"Unknown baseline correction method '{self.method}'.")
...
class NoiseRemoval:
    """Remove peaks whose intensities are below mean minus one standard deviation.
    ...
    def __call__(self, SpectrumObj):
        ...
        mask = data > mean_data - std_data

        return SpectrumObject(intensity=SpectrumObj.intensity[mask], mz=SpectrumObj.mz[mask])

class PIKEScaleNormalizer:
    """Scale spectrum intensities to the [1, 2] range."""
    ...
    def __call__(self, SpectrumObj):
        ...
        if data_max == data_min:
            scaled = np.ones_like(data)
        else:
            scaled = (data - data_min) / (data_max - data_min) + 1

        return SpectrumObject(intensity=scaled, mz=SpectrumObj.mz)
