"""Test to check if choose bindings have similar issues to set/let bindings.

This test checks if variables introduced via choose statements are properly
...
def test_choose_binding_bug_goal_context_parsing() -> None:
    """Test that goal context parsing correctly extracts variables from choose bindings."""
    from goedels_poetry.parsers.util import __parse_goal_context

    # Goal context with choose binding variable
    goal = "x : ℕ\ny : Prop\n⊢ Prop"  # noqa: RUF001
...
def test_choose_binding_bug_variable_used_in_subgoal() -> None:
    """
    Test that choose binding variables are included when used in subgoal type.

    Based on problem-1.log pattern:
    - choose introduces variables before h_partition
    ...
    if not (has_x_param or has_y_param):
        ...
        print("__determine_general_binding_type is called for choose bindings.")
