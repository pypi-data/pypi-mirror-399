...
async def service_create(
    ...
    Design principle: Services are created as active by default.
    Use service_activate with active=False to deactivate if needed.
...
async def service_publish(service_id: str | int) -> dict[str, Any]:
    ...
    3. Create PENDING audit record
    4. Execute POST /service/{id}/publish API call
    5. Update audit record to SUCCESS or FAILED

    Args:
        service_id: ID of the service to publish (required).

    Returns:
        dict: Publish result including:
            - service_id: The published service ID
            - published: True if successful
            - audit_id: The audit record ID
...
async def service_activate(
    ...
    """Activate or deactivate a BPA service.

    This operation follows the audit-before-write pattern:
    1. Validate parameters (pre-flight, no audit if validation fails)
    2. Verify service exists
    3. Create PENDING audit record
    4. Execute PUT /service/{service_id}/activate/{active} API call
    5. Update audit record to SUCCESS or FAILED

    Args:
        service_id: ID of the service to activate/deactivate (required).
