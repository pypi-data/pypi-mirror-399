...
async def role_create(
    ...
    Args:
        ...
        start_role: Whether this is the workflow entry point (default: False).
        visible_for_applicant: Whether role is visible to applicants
            (default: True).
        sort_order_number: Ordering position in workflow (default: 0).
        allow_to_confirm_payments: Permission to confirm payments
            (default: False, UserRole only).
        allow_access_to_financial_reports: Permission to access financial
            reports (default: False, UserRole only).
        repeat_until_successful: Retry bot execution on failure (BotRole only).
        repeat_in_minutes: Retry interval in minutes (BotRole only).
        repeat_in_hours: Retry interval in hours (BotRole only).
        repeat_in_days: Retry interval in days (BotRole only).
        duration_in_minutes: Execution timeout in minutes (BotRole only).
    ...
    Returns:
        dict: Created role details including:
            ...
            - start_role, visible_for_applicant, sort_order_number
            - For UserRole: allow_to_confirm_payments, allow_access_to_financial_reports
            - For BotRole: repeat_*, duration_* fields
            - service_id: The parent service ID
...
async def role_update(
    ...
    Args:
        ...
        start_role: Whether this is the workflow entry point (optional).
        visible_for_applicant: Whether visible to applicants (optional).
        sort_order_number: Ordering position in workflow (optional).
        allow_to_confirm_payments: Payment confirmation permission (optional).
        allow_access_to_financial_reports: Financial reports permission (optional).
    ...
    Returns:
        dict: Updated role details including:
            ...
            - start_role, visible_for_applicant, sort_order_number
            - allow_to_confirm_payments, allow_access_to_financial_reports
            - service_id: The parent service ID
...
async def roleinstitution_create(
    ...
    """Assign an institution to a BPA role.

    This operation follows the audit-before-write pattern:
    1. Validate parameters (pre-flight, no audit if validation fails)
    2. Verify parent role exists (no audit if not found)
    3. Create PENDING audit record
    4. Execute POST /role/{role_id}/role_institution API call
    5. Update audit record to SUCCESS or FAILED

    Institution assignment is required for publishing a service.
    Each role must have an institution assigned for the service to publish.

    Args:
        role_id: ID of the role to assign institution to (required).
...
async def roleregistration_create(
    ...
    """Assign a registration to a BPA role.

    This operation follows the audit-before-write pattern:
    1. Validate parameters (pre-flight, no audit if validation fails)
    2. Verify parent role exists (no audit if not found)
    3. Create PENDING audit record
    4. Execute POST /role/{role_id}/role_registration API call
    5. Update audit record to SUCCESS or FAILED

    Registration assignment is required for publishing a service.
    Each role must have a registration assigned for the service to publish.

    Args:
        role_id: ID of the role to assign registration to (required).
        registration_id: ID of the registration to assign (required).
