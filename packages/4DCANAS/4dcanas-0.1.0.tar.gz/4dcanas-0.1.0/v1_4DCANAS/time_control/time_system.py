...
class TimeManipulation4D: 
    ...
    def set_time_direction(self, direction: str) -> None:
    ...
    def apply_4d_time_dilation(
        self, vertices: np.ndarray, time_value: float, w_factor: float = 1.0
    ) -> np.ndarray:
        """Apply 4D time dilation effect"""
    ...
    def apply_relativistic_time_dilation(
        self,
        vertices: np.ndarray,
        velocity: np.ndarray,
        c:  float = 3e8,
    ...
    def time_loop_animation(self, loop_count: int = 3) -> List[np.ndarray]:
        """Loop timeline multiple times"""
        looped = []
        for _ in range(loop_count):
            looped.extend(self.timeline_cache)
        return looped
    
    def export_timeline_as_sequence(self, output_dir: str, format: str = "obj") -> None:
        """Export timeline as frame sequence"""
        
        from .export import AdvancedExportTools
        import os
        ...
        for frame_idx, frame_data in enumerate(self.timeline_cache):
            if isinstance(frame_data, np.ndarray):
                ...
                AdvancedExportTools.quick_export(
                    shape_dict, format=format, filename=filename
                )