...
# This logger is always available (included in published package)
# unlike ProductionLogger which is only available in development
from obra.hybrid.event_logger import HybridEventLogger

# Optional: ProductionLogger for extended observability (ISSUE-OBS-002)
# Only available in development environment, not in published package
try:
...
class HybridOrchestrator:
    ...
    def __init__(
        ...
        # Always available, unlike ProductionLogger (development-only)
        self._event_logger = HybridEventLogger()

        # ISSUE-OBS-002: Initialize ProductionLogger for extended observability (dev only)
    ...
    def _log_session_event(self, event_type: str, **kwargs) -> None:
        ...
        Uses HybridEventLogger (always available) as the primary logger.
        Also logs to ProductionLogger if available (development mode only).
        ...
        # ISSUE-SAAS-042: Always log to HybridEventLogger (included in package)
        try:
        ...
        # ISSUE-OBS-002: Also log to ProductionLogger if available (dev only)
        if self._production_logger and session_id:
            try:
                self._production_logger._log_event(event_type, session_id, **kwargs)
            except Exception as e:
                logger.debug(f"ProductionLogger event failed (expected in package): {e}")
