...
class NeighborsDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        neighbor_index = np.random.choice(self.indices[index].cpu().numpy(), 10)[0]
        # print("neighbour_index:",neighbor_index)
        ...
        # print(neighbor[0].shape)
        # a=[]
        # for i in range(3):
        #     item=torch.cat(torch.unbind(neighbor[i], dim=1), dim=0)
        #     print(item.shape)
        #     a.append(item)
        output['neighbor'] = neighbor[:3]
        # output['possible_neighbors'] = torch.from_numpy(self.indices[index])

        output['possible_neighbors'] = self.indices[index]
