...
class SimpleTrainer:
    def __init__(
        ...
        if args is None:
            logger.info(
                "No 'TrainingArgumenets' passed, using the current path as 'output_dir'"
            )
    ...
    def generate_positive_sample(self, label: torch.Tensor):
        ...
        for index in range(label.shape[0]):
            ...
            if positive_num > len(self.negative_data[input_label]):
                temp = self.negative_data[input_label] + random.sample(
                    self.negative_data[input_label], 1
                ) * (positive_num - len(self.negative_data[input_label]))
                positive_sample.extend(random.sample(temp, positive_num))
            else:
                positive_sample.extend(
                    random.sample(self.negative_data[input_label], positive_num)
                )
    ...
    def train_mocoknn(
        ...
        for epoch in range(int(self.args.num_train_epochs)):
            epoch_iterator = tqdm(
                train_dataloader, initial=global_step, desc="Iter (sup_loss)"
            )
            model.train()
            ...
            print(
                "Epoch: [{0}]: Loss {loss:.4f}".format(
                    epoch, loss=tr_loss / global_step
                )
            )
