...
def create_embed_f(old_embed_f, dataset, limit_num_sents, type: str):
    ...
    if type == 'cosface':
        ...
        batch_size = 16  # same as above - to guarantee...
    ...
    model.fit(X, y_train, epochs=10, shuffle=shuffle, batch_size=batch_size)

    def embed_f(X):
        embeddings_lst = []

        for batch in batches(X, 8):  # iterate in batches of size 32
            X = old_embed_f(batch)
