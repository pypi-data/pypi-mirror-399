...
class MemoryBank(object):
    ...
    def weighted_knn(self, predictions):
        ...
        probs = torch.sum(
            torch.mul(
                retrieval_one_hot.view(batchSize, -1, self.C),
    ...
    def mine_nearest_neighbors(self, topk, calculate_accuracy=True):
        ...
        features = F.normalize(self.f, dim=1)
        ...
        if calculate_accuracy:
            ...
            targets = self.targets.to("cpu").numpy()
            neighbor_targets = np.take(targets, indices.cpu().numpy(), axis=0)
