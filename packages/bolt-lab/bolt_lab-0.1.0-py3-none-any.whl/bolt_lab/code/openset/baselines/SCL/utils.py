...
def get_score(cm):
    ...
    print(
        f"=====> Uneen(Experiment) <=====: , f:{f_unseen}, acc:{acc_ood}, p:{p_unseen}, r:{r_unseen}"
    )
...
def mahalanobis_distance(x: np.ndarray, y: np.ndarray, covariance: np.ndarray) -> float:
    num_features = x.shape[0]

    vec = x - y
    cov_inv = np.linalg.inv(covariance)
    bef_sqrt = np.matmul(
        np.matmul(vec.reshape(1, num_features), cov_inv), vec.reshape(num_features, 1)
    )
    return np.sqrt(bef_sqrt).item()


def confidence(
    features: np.ndarray, means: np.ndarray, distance_type: str, cov: np.ndarray = None
) -> np.ndarray:
    ...
    features = features.reshape(num_samples, 1, num_features).repeat(
        num_classes, axis=1
    )
    ...
    bef_sqrt = np.matmul(
        np.matmul(vectors.reshape(num_samples, num_classes, 1, num_features), cov_inv),
        vectors.reshape(num_samples, num_classes, num_features, 1),
    ).squeeze()
    result = np.sqrt(bef_sqrt)
    result[np.isnan(result)] = 1e12
    return result
...
def get_test_info(
    ...
    df["softmax_prediction"] = [
        softmax_classes[idx] for idx in softmax_prob.argmax(axis=-1)
    ]
...
def estimate_best_threshold(
    seen_m_dist: np.ndarray, unseen_m_dist: np.ndarray
) -> float:
    lst = []
...
def pca_visualization(X: np.ndarray, y: pd.Series, classes: List[str], save_path: str):
    ...
    for _class in classes:
        if _class == "unseen":
            ax.scatter(
                red_features[y == _class, 0],
                red_features[y == _class, 1],
                label=_class,
                alpha=0.5,
                s=20,
                edgecolors="none",
