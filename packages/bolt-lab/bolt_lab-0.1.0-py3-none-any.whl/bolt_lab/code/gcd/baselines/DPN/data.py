...
class Data:
    ...
    def __init__(self, args):
        ...
        self.pretrain_labeled_dataloader = self.get_loader(
            self.train_labeled_examples, args, "pretrain"
        )
        ...
        self.pretrain_semi_dataloader = self.get_semi_loader(
            self.semi_input_ids,
            self.semi_input_mask,
            self.semi_segment_ids,
            self.semi_label_ids,
        ...
        if mode == "train":
            ...
            for i, row in labeled_df.iterrows():
                ...
                train_labeled_examples.append(
                    InputExample(guid=guid, text=row["text"], label=row["label"])
                )
    ...
    def get_semi(self, labeled_examples, unlabeled_examples, args):
        ...
        labeled_features = self.convert_examples_to_features(
            labeled_examples, self.known_label_list, args.max_seq_length, tokenizer
        )
        unlabeled_features = self.convert_examples_to_features(
    ...
    def get_semi_loader(
        ...
        if mode == "pretrain":
            semi_dataloader = DataLoader(
                semi_data, sampler=semi_sampler, batch_size=args.pretrain_batch_size
            )
