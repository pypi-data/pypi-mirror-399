...
all_label_list = pd.read_csv(
    f"{args.data_dir}/{args.dataset}/label/label.list", header=None
)[0].tolist()
all_label_list = known_label_list + [
    i for i in all_label_list if i not in known_label_list
]
...
data_in_test["labels"] = data_in_test["label"].apply(
    lambda x: all_label_list.index(x) if x in known_label_list else -1
)
data_out_test["labels"] = data_out_test["label"].apply(
    lambda x: all_label_list.index(x)
)
