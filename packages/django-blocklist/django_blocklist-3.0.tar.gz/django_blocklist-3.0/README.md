...
* `report_blocklist` &mdash; information on the current entries (see the [sample report][])
...
* `should_block(request: HttpRequest)` — Checks the request IP and method against the blockslit.
...
* Run tests: `tox` in project root
* Linting: `ruff check django_blocklist`
* The project's [settings.py][] supports running the tests
* `manage.py runserver` works for trying out the admin
