...
/**
 * Node class for building hierarchical ID trees based on similarity matrices.
 * Each node represents either a leaf (single entity) or internal node (merged entities).
 * Uses priority queue-based clustering to build trees from bottom-up.
 *
...
//			assert(Tools.max(parent.left.max, parent.right.max)-parent.max<0.4) : parent+"\n"+parent.left+"\n"+parent.right+"\n";
	...
	/** Returns string representation with node number, max similarity, and array values.
	 * @return Formatted string showing node details */
	@Override
