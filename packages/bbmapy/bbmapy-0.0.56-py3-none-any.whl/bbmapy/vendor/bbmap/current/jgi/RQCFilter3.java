 ...
 * Calls SendSketch, Clumpify, KmerCountExact, BBNorm, BBDukS, BBMap, BBMerge, and SplitNexteraLMP.
 * Trims adapters, removes contaminants, and does quality-trimming.
 * @author Brian Bushnell
 * @date Nov 26, 2013
 *
 */
public class RQCFilter3 {
	...
	public static void main(String[] args){
		...
		RQCFilter3 x=new RQCFilter3(args);
	...
	RQCFilter3(String[] args){
		...
		for(int i=0; i<args.length; i++){
			...
			//These no longer do anything
				...
				//Uncaptured arguments are passed to BBDukS
				primaryArgList.add(arg);
			}
		}
...
//				outstream.println("Trim. step="+step+", in="+in1+", out="+out1+", inPrefix="+inPrefix+", outPrefix="+outPrefix);
				...
				lastReadsObserved=BBDukS.lastReadsOut;
...
//				outstream.println("Nextera. step="+step+", in="+in1+", out="+out1+", inPrefix="+inPrefix+", outPrefix="+outPrefix);
		...
		if(doMerge){
			BBDukProcessor.putRqc("outputReads", BBMerge.readsProcessedTotal*2, true, false);
			BBDukProcessor.putRqc("outputBases", BBMerge.basesProcessedTotal, true, false);
		}
		...
		if(rqcStatsName!=null){
			...
			tsw.start();
			tsw.println(BBDukProcessor.rqcString());
...
//				assert(false) : KmerSort.lastReadsIn+", "+KmerSort.lastBasesIn+", "+KmerSort.lastReadsOut+", "+KmerSort.lastBasesOut;
	...
	/**
	 * Runs BBDukS to recalibrate quality scores.
	 * 
	 * @param in1 Primary input reads file (required)
	 * @param in2 Secondary input reads file
	 * @param out1 Primary output reads file (required)
	...
	private void recalibrate(String in1, String in2, String out1, String out2, 
		...
		{//Fill list with BBDukS arguments
			if(pigz!=null){argList.add("pigz="+pigz);}
			if(unpigz!=null){argList.add("unpigz="+unpigz);}
			argList.add("zl="+(zl==null ? ""+ReadWrite.ZIPLEVEL : zl));
...
//				refs.add(clipLinker);
		...
		if(!dryrun){//Run BBDukS
			...
			try {
				...
				if(duk.errorState){throw new Exception("BBDukS did not finish successfully; see above for the error message.");}

				filterstats.readsFTrimmed=duk.proc.readsFTrimmed;
				filterstats.basesFTrimmed=duk.proc.basesFTrimmed;
				
				filterstats.readsAdapter=duk.proc.readsIn-duk.proc.readsOut;
				filterstats.basesAdapter=duk.proc.basesIn-duk.proc.basesOut-duk.proc.basesFTrimmed;
				if(filterstats.readsIn<1){
					filterstats.readsIn=duk.proc.readsIn;
					filterstats.basesIn=duk.proc.basesIn;
				}
				filterstats.readsOut=duk.proc.readsOut;
				filterstats.basesOut=duk.proc.basesOut;
				
				filterstats.gcPolymerRatio=duk.proc.getPolymerRatio();
...
//			if(rqcStatsName!=null){al.add("rqc="+rqcStatsName);} //Old style for 2 log files
		...
		if(!dryrun){//Run BBDukS
			...
			try {
				duk.process();
				outstream.println(format("Synthetic Contam Sequence Removed:", duk.proc.readsIn, duk.proc.readsOut, duk.proc.basesIn, duk.proc.basesOut));
				log("#Input:\t"+duk.proc.readsIn+" reads\t"+duk.proc.basesIn+" bases\t"+duk.proc.loglogIn.cardinality()+" kmers", true, false);
				log("#Remaining:\t"+duk.proc.readsOut+" reads\t"+duk.proc.basesOut+" bases\t"+duk.proc.loglogOut.cardinality()+" kmers", true, false);
				if(duk.errorState){throw new Exception("BBDukS did not finish successfully; see above for the error message.");}
...
//				System.err.println(filterstats.toString());
				...
				long rRemoved=duk.proc.readsIn-duk.proc.readsOut;
				long bRemoved=duk.proc.basesIn-duk.proc.basesOut;

				filterstats.readsArtifact+=duk.proc.readsKFiltered;
				filterstats.basesArtifact+=duk.proc.basesKFiltered;
				filterstats.readsPolyG+=duk.proc.readsPolyTrimmed;
				filterstats.basesPolyG+=duk.proc.basesPolyTrimmed;
				filterstats.readsLowQuality+=(rRemoved-duk.proc.readsKFiltered);
				filterstats.basesLowQuality+=(bRemoved-duk.proc.basesKFiltered);
				filterstats.readsN+=duk.proc.readsNFiltered;
				filterstats.basesN+=duk.proc.basesNFiltered;
				if(filterstats.readsIn<1){
					filterstats.readsIn=duk.proc.readsIn;
					filterstats.basesIn=duk.proc.basesIn;
				}
...
//			if(rqcStatsName!=null){argList.add("rqc=hashmap");} //TODO
		...
		if(!dryrun){//Run BBDukS
			...
			try {
				duk.process();
				outstream.println(format("Short Synthetic Contam Sequence Removed:", duk.proc.readsIn, duk.proc.readsOut, duk.proc.basesIn, duk.proc.basesOut));
				log("#Input:\t"+duk.proc.readsIn+" reads\t"+duk.proc.basesIn+" bases\t"+duk.proc.loglogIn.cardinality()+" kmers", true, false);
				log("#Remaining:\t"+duk.proc.readsOut+" reads\t"+duk.proc.basesOut+" bases\t"+duk.proc.loglogOut.cardinality()+" kmers", true, false);
				if(duk.errorState){throw new Exception("BBDukS did not finish successfully; see above for the error message.");}
...
//			if(scaffoldStatsName!=null){argList.add("stats="+scaffoldStatsName);}
		...
		if(!dryrun){//Run BBDukS
			...
			try {
				duk.process();
				outstream.println(format("Ribosomal Sequence Removed:", duk.proc.readsIn, duk.proc.readsOut, duk.proc.basesIn, duk.proc.basesOut));
				log("#Input:\t"+duk.proc.readsIn+" reads\t"+duk.proc.basesIn+" bases\t"+duk.proc.loglogIn.cardinality()+" kmers", true, false);
				log("#Remaining:\t"+duk.proc.readsOut+" reads\t"+duk.proc.basesOut+" bases\t"+duk.proc.loglogOut.cardinality()+" kmers", true, false);
				if(duk.errorState){throw new Exception("BBDukS did not finish successfully; see above for the error message.");}
				
				filterstats.readsRiboKmer+=duk.proc.readsKFiltered;
				filterstats.basesRiboKmer+=duk.proc.basesKFiltered;
				if(filterstats.readsIn<1){
					filterstats.readsIn=duk.proc.readsIn;
					filterstats.basesIn=duk.proc.basesIn;
				}
...
//		sdf.setTimeZone(TimeZone.getTimeZone("PST"));
	...
	/** Arguments to pass to BBDukS */
	private ArrayList<String> primaryArgList=new ArrayList<String>();
	/** References to pass to BBDukS for artifact removal */
