...
//		LOCK=LOCK_;
	...
	//Called by start()
	/** Main thread execution method that processes neural network jobs until
	 * termination. Handles job processing lifecycle and sets success status. */
	@Override
	public void run(){
	...
	/**
	 * Core job processing loop that retrieves jobs from queue, prepares neural
	 * network state, processes samples, and sends results. Continues until
	 * receiving poison pill termination signal.
	 */
	void processInner(){
...
//		}
	...
	/**
	 * Processes individual sample through neural network, updating sample
	 * state and accumulating performance statistics.
	 *
	 * @param sample Training sample to process
	 * @param backprop Whether to perform backpropagation learning
	 * @param weightMult Weight multiplier for sample importance
	 */
...
//			boolean goal=(s.goal[0]>=Trainer.booleanCutoffGoal);
	...
	/**
	 * Compares worker threads by thread ID for consistent ordering.
	 * @param o Other WorkerThread to compare against
	 * @return Negative if this thread ID is lower, positive if higher, zero if equal
	 */
	@Override
