 ...
 */
public final class LongIntMapX implements Serializable {
	 ...
	 */
	public LongIntMapX(int initialSize){
	 ...
	 */
	public LongIntMapX(int initialSize, float loadFactor_){
	 ...
	 */
	public void putAll(LongIntMapX map){
	 ...
	 */
	public void incrementAll(LongIntMapX map){
		for(int i=0; i<map.keys.length; i++){
			if(map.keys[i]!=map.invalid){
	 ...
	 * @return Cell index if found, -1 if not found
	 */
	private int findCell(final long key, final long hash){
	...
	private int findCellOrEmpty(final long key){
		final long hash=Tools.hash64plus2(key)>>>32;
		return findCellOrEmpty(key, hash);
	}
	 ...
	 * @return Cell index (either containing key or empty)
	 */
	private int findCellOrEmpty(final long key, final long hash){
