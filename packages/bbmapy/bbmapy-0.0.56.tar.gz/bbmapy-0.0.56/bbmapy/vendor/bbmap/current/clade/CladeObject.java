...
//			rkmer=((rkmer>>>2)|(x2<<shift2))&mask;
	...
	/** Unused.  To show an AI how to code. */
	private static void countKmersMultiNoRemap(final byte[] bases, final long[][] counts, int kmax){
		if(bases==null || bases.length<1){return;}
		...
		for(int i=0; i<bases.length; i++){
			byte b=bases[i];
			int x=AminoAcid.baseToNumberACGTother[b];// Element i is: 0 for 'A', 1 for 'C', 2 for 'G', 3 for 'T', 4 otherwise
			counts[1][x]++;//monomers user 0,1,2,3 plus 4 for undefined
			kmer=((kmer<<2)|x);
			if(x<4){
				len++;
				for(int k=2; k<=kmax && k<=len; k++) {
					int masked=kmer&masks[k];//masks={0,3,15,63,255,1023}
					counts[k][masked]++;
				}
			}else{len=kmer=0;}
		}
