...
/**
 * Creates filtered k-mer sets through iterative passes of counting and filtering.
 * Processes input sequences to extract high-quality k-mers, removing low-count k-mers and retaining those with sufficient depth of coverage.
 * Uses multiple passes to progressively refine the k-mer set by filtering out reads that match already-selected k-mers.
 * @author Brian Bushnell
 * @date Nov 22, 2013
 */
public class KmerFilterSetMaker {
	
	/** Program entry point that creates and runs a KmerFilterSetMaker instance.
	 * @param args Command line arguments for configuring the k-mer filtering process */
...
//		outstream.println("Initialization Time:      \t"+t2);
	...
	/**
	 * Constructs a KmerFilterSetMaker instance from command-line arguments.
	 * Parses arguments to configure input/output files, filtering parameters, and processing options.
	 * Sets global defaults for compression and threading.
	 * @param args Command line arguments including input files, output files, k-mer count thresholds, and processing parameters
	 */
