 ...
 *
 */
public class BBDukIndexMask2 extends BBDukIndex {

	/**
	 * Constructor.
	 * @param p Parser with command line arguments
	 */
	public BBDukIndexMask2(BBDukParser p){
		...
		keySets=Allocator.preallocate(WAYS, schedule[0]);
	}
	...
	@Override
	void rebalance(int way) {//TODO
//		LongIntMapX map=keySets[way];
...
//		}
		...
		for(int i=k-1; i>=mink; i--){
			...
			if(verbose){
				...
				if((hash&WAYMASK)==tnum){
					...
					final LongIntMapX map=keySets[tnum];
					if(map.contains(value, hash>>>32)){outstream.println("Found "+value);}
				}
			}
		}
		return added;
	...
	long addToMap(final long kmer, final long rkmer, final int len, final long extraBase, final int id, final long kmask0, final int hdist, final int edist, final int tnum){
		...
		if(hdist==0){
			...
			added=map.setIfNotPresent(key, id, hash>>>32);
		}else if(edist>0){
	...
	private final int getValueInner(final long kmer, final long rkmer, final long lengthMask, final int len, final int qPos){
		...
		if(passesSpeed(hash)){
			...
			LongIntMapX set=keySets[(int)(hash&WAYMASK)];
			final int id=set.get(key, hash>>>32);
	...
	private static final long hash(long value) {
		return Tools.hash64plus2(value);
	}
	...
	private final LongIntMapX[] keySets;
