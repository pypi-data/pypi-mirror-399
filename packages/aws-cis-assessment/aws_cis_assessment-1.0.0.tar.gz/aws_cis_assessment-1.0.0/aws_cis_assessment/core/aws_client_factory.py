...
class AWSClientFactory:
    ...
    def test_service_access(self, service_name: str, region: Optional[str] = None) -> bool:
        ...
        try:
            ...
            test_calls = {
                ...
                's3': lambda c: c.list_buckets(MaxBuckets=1),
                'rds': lambda c: c.describe_db_instances(MaxRecords=1),
                'cloudtrail': lambda c: c.describe_trails(MaxItems=1),
            ...
            if error_code in ['AccessDenied', 'UnauthorizedOperation']:
                logger.warning(f"Access denied for {service_name} in region {region}")
            else:
                logger.warning(f"Service {service_name} test failed in region {region}: {e}")
            return False
        except Exception as e:
    ...
    def get_supported_services(self) -> List[str]:
        ...
        return [
            ...
            'elasticloadbalancing', 'elasticloadbalancingv2', 'apigateway',
