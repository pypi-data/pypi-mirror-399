...
@click.group()
@click.version_option(version="1.0.0", prog_name="aws-cis-assess")
...
@click.option('--baseline-file', '-b',
              type=click.Path(exists=True, file_okay=True, dir_okay=False),
              help='Baseline file for regression detection')
@click.option('--regression-threshold',
              type=float,
              default=1.3,
              help='Performance regression threshold (1.3 = 30% degradation)')
@click.option('--verbose', '-v', is_flag=True, help='Enable verbose output')
@click.pass_context
def benchmark(ctx, output_dir, iterations, baseline_file, regression_threshold, verbose):
    """Run performance benchmarks for regression testing.
    
    This command runs a comprehensive performance benchmark suite to measure
    assessment execution time, memory usage, and other performance metrics.
    Results can be compared against a baseline to detect performance regressions.
    ...
    """
    from aws_cis_assessment.core.benchmark_suite import PerformanceBenchmark, BenchmarkSuite
    
    try:
        click.echo("🚀 Starting performance benchmark suite...")
        
        # Create benchmark instance
        benchmark_runner = PerformanceBenchmark(output_dir=output_dir)
        
        # Create standard benchmark suite
        suite = benchmark_runner.create_standard_benchmark_suite()
        
        # Update suite configuration
        if baseline_file:
            suite.baseline_file = baseline_file
        suite.regression_threshold = regression_threshold
        
        click.echo(f"   Running {len(suite.tests)} benchmark tests with {iterations} iterations each")
        if baseline_file:
            click.echo(f"   Using baseline: {baseline_file}")
        click.echo(f"   Regression threshold: {regression_threshold:.1f}x")
        click.echo()
        
        # Run benchmark suite
        results = benchmark_runner.run_benchmark_suite(suite, iterations=iterations)
        
        # Generate performance report
        report = benchmark_runner.generate_performance_report(results)
        ...
        for test_name, test_results in results.items():
            ...
            if successful_results:
                ...
                if verbose:
                    click.echo(f"   {test_name}: {avg_time:.2f}s, {avg_memory:.1f}MB")
        
        click.echo(f"   Successful tests: {successful_tests}/{total_tests}")
        if successful_tests > 0:
            click.echo(f"   Average execution time: {total_avg_time/successful_tests:.2f}s")
            click.echo(f"   Average memory usage: {total_avg_memory/successful_tests:.1f}MB")
        
        # Save performance report
        report_file = Path(output_dir) / f"performance_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
        with open(report_file, 'w') as f:
            f.write(report)
        
        click.echo(f"   📄 Performance report saved: {report_file}")
        click.echo()
        click.echo("✅ Benchmark suite completed successfully!")
