...
class LayerScale(nn.Module):
    """Per-channel residual scaling (Touvron et al.)."""
...
def _zero_init_linear(m: nn.Linear):
...
class ResidualFFNBlock(nn.Module):
    """Pre-norm residual MLP block with GEGLU + LayerScale + DropPath."""
    def __init__(
        self,
        dim: int,
        hidden: int,
...
class ResidualMLPAdapterDeep(nn.Module):
    """Residual MLP adapter used to map source embeddings → target space.

    This matches the architecture you trained:

    - LayerNorm on input
    - Linear to `width`
    - `depth` ResidualFFNBlock layers (GEGLU + LayerScale + DropPath)
    - LayerNorm
    - Linear to out_dim
    - Final LayerNorm
    ...
    def __init__(
        ...
        # Initialize first/last linears with small std deviation
        nn.init.trunc_normal_(self.in_fc.weight, std=0.02)
        nn.init.zeros_(self.in_fc.bias)
        nn.init.trunc_normal_(self.out_fc.weight, std=0.02)
