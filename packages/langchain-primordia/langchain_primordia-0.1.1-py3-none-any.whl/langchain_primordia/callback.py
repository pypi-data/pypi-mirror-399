...
def get_default_handler() -> Optional["PrimordiaCallbackHandler"]:
    """Get handler from environment if PRIMORDIA_AGENT_ID is set.

    Set these environment variables for automatic metering:
        PRIMORDIA_AGENT_ID=your-agent-id
        PRIMORDIA_MODE=shadow (default) or submit
        PRIMORDIA_KERNEL_URL=https://clearing.kaledge.app (optional)
    """
    agent_id = os.environ.get("PRIMORDIA_AGENT_ID")
    if not agent_id:
        return None

    mode = os.environ.get("PRIMORDIA_MODE", "shadow")
    kernel_url = os.environ.get("PRIMORDIA_KERNEL_URL", "https://clearing.kaledge.app")

    return PrimordiaCallbackHandler(
        agent_id=agent_id,
...
# Auto-instantiate if env is set
