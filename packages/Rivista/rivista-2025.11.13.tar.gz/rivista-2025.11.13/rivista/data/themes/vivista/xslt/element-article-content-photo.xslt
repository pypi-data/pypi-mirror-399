...
<xsl:stylesheet version="1.0"
    ...
    <!-- This template creates an XHTML entry, based on properties, content, and enclosures with images. -->
    <xsl:template name="element-article-content-photo">
        <xsl:param name="atom"/>
        <xsl:comment>
        ...
        <article class="entry h-entry">
            ...
            <table class="graphics">
                <tr>
                    <!-- Avatar -->
                    <xsl:variable name="link-enclosure-avatar" select="atom:link[@rel='avatar']"/>
                    <xsl:if test="$link-enclosure-avatar">
                        <td>
                            <xsl:comment>
                                <xsl:text>Entry link (avatar)</xsl:text>
                            </xsl:comment>
                            <xsl:call-template name="element-image">
                                <xsl:with-param name="alt" select="$link-enclosure-avatar/@title"/>
                                <xsl:with-param name="class" select="'avatar'"/>
                                <xsl:with-param name="src" select="$link-enclosure-avatar/@href"/>
                                <xsl:with-param name="title" select="$link-enclosure-avatar/@title"/>
                                <xsl:with-param name="type" select="$link-enclosure-avatar/@type"/>
                            </xsl:call-template>
                        </td>
                    </xsl:if>
                    <!-- Figure -->
                    <xsl:variable name="link-enclosure-photo" select="atom:link[@rel='photo']"/>
                    <xsl:if test="$link-enclosure-photo">
                        <td>
                            <xsl:comment>
                                <xsl:text>Entry link (photo)</xsl:text>
                            </xsl:comment>
                            <xsl:call-template name="element-figure">
                                <xsl:with-param name="alt" select="$link-enclosure-photo/@title"/>
                                <xsl:with-param name="class" select="'photo'"/>
                                <xsl:with-param name="src" select="$link-enclosure-photo/@href"/>
                                <xsl:with-param name="title" select="$link-enclosure-photo/@title"/>
                                <xsl:with-param name="type" select="$link-enclosure-photo/@type"/>
                            </xsl:call-template>
                        </td>
                    </xsl:if>
                </tr>
            ...
            <!-- Gallery -->
            <xsl:call-template name="elements-link-enclosure-image">
                <xsl:with-param name="atom" select="atom"/>
                <xsl:with-param name="links-enclosure" select="atom:link[@rel='image']"/>
            </xsl:call-template>
            <!-- Terms -->
            ...
            <!-- Enclosures -->
            <!-- NOTE Create an array to exclude titles of images. -->
