            ...
            <body>
                ...
                <div class="h-feed" id="feed">
                    ...
                    <xsl:call-template name="element-menu">
                        ...
                        <xsl:with-param name="type" select="' publications'"/>
                    </xsl:call-template>
                    <!-- Document entries -->
                    <xsl:comment>
                        <xsl:text>Document entries</xsl:text>
                    </xsl:comment>
                    <section id="figures">
                        <xsl:choose>
                            <xsl:when test="atom:entry">
                                <xsl:for-each select="atom:entry[not(position() &gt; 20)]">
                                    <!-- TODO u-uid of Microformat -->
                                    <xsl:call-template name="element-figure-enclosure">
                ...
                <footer>
                    ...
                    <xsl:text>stylesheet. This document can </xsl:text>
                    <xsl:text>be viewed with syndication </xsl:text>
                    <xsl:text>Feed Readers (also referred to </xsl:text>
                    <xsl:text>as News Reader or RSS Reader) </xsl:text>
                ...
                <xsl:if test="string-length(atom:rights) &gt; 0">
                    <footer id="rights">
