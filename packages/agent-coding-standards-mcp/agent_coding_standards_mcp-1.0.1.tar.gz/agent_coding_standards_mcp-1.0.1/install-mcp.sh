...
CHECK_RESULT=$(python3 - "$CONFIG_FILE" "$SERVERS_KEY" << 'PYTHON_CHECK_EOF'
import json
import sys
import os
...
def strip_json_comments(text):
    ...
    for line in lines:
        ...
        line = line.rstrip()
        if line.endswith(','):
            next_char_index = len(cleaned_lines)
...
try:
    ...
    if servers_key in config and 'agent-coding-standards-mcp' in config[servers_key]:
        print("EXISTS")
        # Print current config details
        mcp_config = config[servers_key]['agent-coding-standards-mcp']
...
if echo "$CHECK_RESULT" | grep -q "^EXISTS"; then
    ...
    echo -e "${YELLOW}⚠️  WARNING: agent-coding-standards-mcp already exists!${NC}"
    echo ""
...
# ============================================
# QUESTION 2: GitLab Configuration
# ============================================
echo -e "${BOLD_CYAN}🔑 GitLab Configuration${NC}"
echo ""
echo -e "${WHITE}  Please enter your GitLab Personal Access Token${NC}"
echo -e "${CYAN}  ℹ️  This token will be used to access:${NC}"
echo -e "     ${BLUE}https://git.teqnological.asia${NC}"
echo -e "     ${BLUE}Project: openai/mcp/agent-coding-standards-mcp${NC}"
echo ""
...
echo -e "${GREEN}  ✓ Token received (${#GITLAB_TOKEN} characters)${NC}"
...
echo ""
echo -e "  ${WHITE}GitLab:${NC}"
echo -e "    ${CYAN}URL:${NC}          https://git.teqnological.asia"
echo -e "    ${CYAN}Project:${NC}      openai/mcp/agent-coding-standards-mcp"
echo -e "    ${CYAN}Branch:${NC}       main"
echo -e "    ${CYAN}Token:${NC}        ${GREEN}✓ Configured${NC}"
echo ""
echo -e "${WHITE}  ════════════════════════════════════════════════════════${NC}"
echo ""
...
python3 - "$CONFIG_FILE" "$GITLAB_TOKEN" "$OVERWRITE_MODE" "$SERVERS_KEY" "$TIMEOUT_VALUE" "$HAS_TYPE" "$HAS_AUTO_APPROVE" "$HAS_DISABLED" "$HAS_TIMEOUT" "$CONFIG_FORMAT" << 'PYTHON_EOF'
import json
import sys
import os
...
overwrite_mode = sys.argv[3] == "true"
servers_key = sys.argv[4]
timeout_value = sys.argv[5]
has_type = sys.argv[6] == "true"
has_auto_approve = sys.argv[7] == "true"
has_disabled = sys.argv[8] == "true"
has_timeout = sys.argv[9] == "true"
config_format = sys.argv[10]
...
mcp_config = {
    ...
    "args": [
        "--from",
        "agent-coding-standards-mcp@latest",
        "--index-url",
    ...
    "env": {
        "GITLAB_URL": "https://git.teqnological.asia",
        "GITLAB_TOKEN": gitlab_token,
        "GITLAB_PROJECT_PATH": "openai/mcp/agent-coding-standards-mcp",
        "GITLAB_BRANCH": "main"
    }
}
...
if has_timeout and timeout_value:
    mcp_config["timeout"] = int(timeout_value)

# Add/Update agent-coding-standards-mcp
config[servers_key]['agent-coding-standards-mcp'] = mcp_config
...
try:
    ...
    if overwrite_mode:
        print_color(GREEN, "  ✓ Successfully overwritten agent-coding-standards-mcp")
    else:
        print_color(GREEN, "  ✓ Successfully added agent-coding-standards-mcp")
    ...
    for server_name in config[servers_key].keys():
        ...
        marker = "⚡" if server_name == "agent-coding-standards-mcp" else " "
