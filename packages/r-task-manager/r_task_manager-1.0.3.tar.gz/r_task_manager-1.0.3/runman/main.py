...
# Windows 兼容：条件导入 pwd 模块
try:
    import pwd
except ImportError:
    pwd = None

# 全局配置目录（可通过环境变量或命令行参数设置）
...
def get_config_dir() -> Path:
    ...
    优先级：
    1. 命令行参数设置的配置目录（通过 set_config_dir()）
    ...
    env_config_dir = os.environ.get("RUNMAN_CONFIG_DIR")
    if env_config_dir:
        return Path(env_config_dir).expanduser().resolve()
    
    # 默认配置目录
    return Path.home() / ".runman"
...
def ensure_permissions():
    """确保任务目录权限（root用户时设置为777，Windows兼容）"""
    task_dir = get_task_dir()
    task_dir.mkdir(parents=True, exist_ok=True)
    # Windows 不支持 geteuid，使用 hasattr 检查
...
def get_user_info() -> str:
    """检测用户环境信息（Windows兼容）"""
    # 检查是否在 Docker 容器中（Windows 和 Linux 都支持）
    docker_env_path = Path("/.dockerenv")
    if docker_env_path.exists() and docker_env_path.stat().st_size > 0:
        # 在docker容器中
        container_name = os.environ.get("HOSTNAME") or os.environ.get("CONTAINER_NAME", "unknown")
        return f"docker:{container_name}"
    else:
        # 在host机器上
        # 优先使用环境变量（Windows 使用 USERNAME，Linux 使用 USER）
        username = os.environ.get("USER") or os.environ.get("USERNAME")
        if not username:
            try:
                # Linux/Unix 系统：使用 pwd 模块获取用户名
                if hasattr(os, 'getuid') and pwd is not None:
                    username = pwd.getpwuid(os.getuid()).pw_name
                else:
                    # Windows 或其他系统：使用 getpass
                    import getpass
                    username = getpass.getuser()
            except (KeyError, AttributeError, ImportError, OSError):
                username = "unknown"
...
def assign_alias() -> str:
    ...
    for task_file in get_task_dir().glob("*.task"):
...
def kill_by_alias(alias: str) -> bool:
    ...
    for task_file in get_task_dir().glob("*.task"):
        ...
        if task_user_info == current_user_info and task_alias == alias and is_process_running(pid):
            ...
            try:
                # 优先使用进程组 kill（可以终止所有子进程，Windows兼容）
                if pgid and hasattr(os, 'killpg'):
                    try:
                        ...
                        os.kill(pid, signal.SIGTERM)
                elif sys.platform == 'win32' and pgid:
                    ...
                    try:
                        ...
                        log_debug(f"使用 taskkill 终止进程树 PID {pid}")
                    except Exception as e:
                        log_debug(f"taskkill 失败 ({e})，回退到 kill PID {pid}")
                        os.kill(pid, signal.SIGTERM)
                else:
...
def init_bashrc():
    """初始化 shell 配置：Linux/Mac 初始化 .bashrc，Windows 初始化 PowerShell 配置文件"""
    if sys.platform == 'win32':
        return init_powershell()
    else:
        return init_bashrc_linux()


def init_bashrc_linux():
    """初始化 .bashrc，添加常用别名（Linux/Mac）"""
...
def init_powershell():
    """初始化 PowerShell 配置文件，删除 r 别名并添加常用别名（Windows）"""
    try:
        ...
        if not profile_path or not profile_path.exists():
            # PowerShell 配置文件的默认位置
            home = Path.home()
            if 'APPDATA' in os.environ:
                # Windows 10/11
                profile_path = Path(os.environ['APPDATA']) / "PowerShell" / "Microsoft.PowerShell_profile.ps1"
            else:
                # 备用路径
        ...
        # 要添加的配置
        config_to_add = [
            "# RunMan configuration - start",
            "# 删除 PowerShell 的 r 别名（避免与 r-task-manager 冲突）",
            ...
            "# RunMan configuration - end",
        ]
        
        # 检查标记，避免重复添加
        marker_start = "# RunMan configuration - start"
        marker_end = "# RunMan configuration - end"
        ...
        # 检查是否已经添加过
        if marker_start in content and marker_end in content:
            print("✅ PowerShell 配置文件中已包含 RunMan 配置，跳过添加")
            return True
        ...
        try:
            ...
            # 如果 UTF-8 失败，尝试 UTF-16（PowerShell 默认）
            with open(profile_path, 'w', encoding='utf-16') as f:
                f.write(new_content)
        
        print("✅ 已成功配置 PowerShell 配置文件")
        print(f"📝 配置文件路径: {profile_path}")
        print("📝 配置内容：")
        print("   - 删除 PowerShell 的 r 别名（避免冲突）")
        print("   - 添加常用别名（ll, la, l）")
        print("\n💡 提示: 请重新打开 PowerShell 窗口使配置生效")
        print("   或者运行: . $PROFILE")
        
        return True
        
    except Exception as e:
        print(f"❌ 初始化 PowerShell 配置失败: {e}")
        print("\n💡 手动配置方法：")
        print("   1. 运行: $PROFILE 查看配置文件路径")
        print("   2. 如果文件不存在，运行:")
        print("      if (!(Test-Path -Path $PROFILE)) { New-Item -ItemType File -Path $PROFILE -Force }")
        print("   3. 在配置文件中添加:")
        print("      Remove-Item Alias:r -Force -ErrorAction SilentlyContinue")
        return False
...
def list_tasks_grouped():
    ...
    # 重新检查是否有任务文件
    task_files = list(get_task_dir().glob("*.task"))
    if not task_files:
        print("📋 当前后台任务：")
        print("----------------------------------------")
        print("（暂无任务）")
...
def save_command_history(target: str):
    ...
    try:
        ...
        if hasattr(os, 'geteuid') and os.geteuid() == 0:
            try:
                os.chmod(get_history_file(), 0o666)
            except (OSError, AttributeError):
                # Windows 上 chmod 可能失败，忽略
                pass
    except Exception:
...
def start_task(target: str, extra_args: List[str] = None):
    ...
    if abs_target.suffix == ".py":
        # Windows 上使用 python，Linux 上使用 python3
        python_cmd = "python" if sys.platform == 'win32' else "python3"
        cmd = [python_cmd, "-u", str(abs_target)]
    elif abs_target.suffix == ".sh":
        # Windows 上可能需要使用 Git Bash 或其他 shell
        if sys.platform == 'win32':
            # Windows: 尝试使用 bash（如果安装了 Git Bash 或 WSL）
            cmd = ["bash", str(abs_target)]
        else:
            cmd = ["bash", str(abs_target)]
    elif abs_target.suffix in [".bat", ".cmd"]:
        # Windows 批处理文件
        cmd = [str(abs_target)]
    else:
        # 默认使用 bash（Linux/Mac）或直接执行（Windows）
        if sys.platform == 'win32':
            cmd = [str(abs_target)]
        else:
            cmd = ["bash", str(abs_target)]
    ...
    try:
        with open(logfile, 'w', encoding='utf-8') as log_f:
            # Windows 兼容：使用不同的进程创建方式
            preexec_fn = None
            creationflags = 0
            if hasattr(os, 'setsid'):
                # Unix/Linux: 使用 setsid 创建新进程组
        ...
        try:
            if hasattr(os, 'getpgid'):
                ...
                # 如果使用了 setsid，PGID 就是 PID
                pgid = pid
            else:
                # Windows 等不支持进程组的系统，使用 None
                pgid = None
        except (OSError, AttributeError):
            # 如果获取失败（如 Windows），使用 None
            pgid = None
        ...
        task_file = get_task_dir() / f"{pid}.task"
        task_info = {
            "PID": pid,
            "PGID": pgid,  # 保存进程组 ID
...
用法示例:
  ...
  %(prog)s init              初始化 shell 配置（Linux/Mac: .bashrc, Windows: PowerShell）
  %(prog)s debug script.py   调试模式启动脚本
  %(prog)s --config-dir DIR  指定配置目录（覆盖环境变量和默认值）
  
环境变量:
  RUNMAN_CONFIG_DIR          设置配置目录路径（默认: ~/.runman）
        """
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help='指定配置目录路径（覆盖环境变量 RUNMAN_CONFIG_DIR 和默认值 ~/.runman）'
    )
    
    args = parser.parse_args()
    
    # 如果指定了 --config-dir，设置配置目录
    if args.config_dir:
