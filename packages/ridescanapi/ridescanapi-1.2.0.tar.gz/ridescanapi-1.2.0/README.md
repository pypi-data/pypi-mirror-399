...
* **`get_model_status`**: Removed the `..` hack. The request path is now simply `/model/mission-details`, which correctly appends to the `/api` base URL.
* **`calibrate_model`**: Updated the docstring to list the valid string values ("SPOT", "UR6") for `robot_type`.
