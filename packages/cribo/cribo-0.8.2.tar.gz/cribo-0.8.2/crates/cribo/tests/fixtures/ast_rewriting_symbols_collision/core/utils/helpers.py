...
class Logger:
    ...
    def __init__(self, name: str):
        ...
        # Internal conflict with module-level User
        self.User = User  # Reference to module-level string
...
def process(data: Any) -> str:
    ...
    result + 1  # Reference but no assignment
...
def process_with_conflicts(
    data: Any,
    User: str = "param_user",  # Parameter name conflicts with module-level User
    result: int = 100,  # Parameter name conflicts with module-level result
    ...
    local_result = {
        "data": data,
        "user_param": User,  # Using parameter, not module-level
        "result_param": result,  # Using parameter, not module-level
