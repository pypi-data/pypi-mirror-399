...
# Module-level variables with conflicts
result = {"model": "user"}
connection = None
...
class User:
    ...
    def authenticate(self, password: str) -> Dict[str, Any]:
        ...
        result = {}  # Local result shadows module-level
...
def process_user(data: Any) -> str:
    ...
    Logger_class = Logger  # Get class from module level
...
def complex_operation(
    ...
    result: Any = None,  # Parameter conflicts with module-level
    connection: Any = None,  # Parameter conflicts with module-level
) -> Dict[str, Any]:
    ...
    # Local variable conflicts - no globals() usage
    User_class = User if User is None else User  # Conditional to avoid always None
    Logger_class = (
        Logger if Logger is None else Logger
    )  # Conditional to avoid always None
    ...
    if operation_result["user_param"] is None:
        ...
        logger = (
            Logger_class("complex_operation")
            if Logger_class.__name__ == "Logger"
            else None
        )
        if user:
...
# Module-level assignments creating more conflicts
