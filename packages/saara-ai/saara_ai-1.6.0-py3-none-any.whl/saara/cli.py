...
def run_training_wizard(default_data_path: str = None, config: dict = None):
    ...
    # Base Models - Gemma highlighted for Google Gemini Hackathon
    base_models = [
        ("google/gemma-2-2b", "2B", "⭐ Gemma 2"),
        ("google/gemma-2-9b", "9B", "⭐ Gemma 2"),
        ("google/gemma-2b", "2B", "Gemma"),
        ("google/gemma-7b", "7B", "Gemma"),
        ("sarvamai/sarvam-1", "2B", "Indic"),
        ("meta-llama/Llama-3.2-1B", "1B", "Llama"),
        ("Qwen/Qwen2.5-7B", "7B", "Qwen"),
        ("TinyLlama/TinyLlama-1.1B", "1.1B", "Tiny"),
    ]
    ...
    if custom_tokenizer_path:
        ...
        # Note: Custom tokenizer support needs to be added to LLMTrainer if not already present
...
@app.command()
def cloud(
    action: str = typer.Argument("info", help="Action: info, setup, or quickstart")
):
    """
    Cloud runtime management for Colab/Kaggle/cloud environments.
    
    Actions:
        info       - Show cloud environment information
        setup      - Configure cloud API providers
        quickstart - Display quickstart guide for Colab
    
    Examples:
        saara cloud info       # Check environment
        saara cloud setup      # Configure API keys
        saara cloud quickstart # Colab quickstart guide
    """
    from saara.cloud_runtime import (
        CloudRuntime, 
        detect_environment,
    ...
    if action == "info":
        ...
        console.print("Use: info, setup, or quickstart")
