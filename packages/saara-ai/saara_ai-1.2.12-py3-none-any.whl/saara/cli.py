...
def train(
    ...
    # --- 1. Select Data ---
    if not data:
        # Search for .jsonl files in datasets/
        ...
        if not candidates:
            console.print("[red]❌ No JSONL files found in 'datasets/' directory.[/red]")
            console.print("Please generate data first using [bold]saara process[/bold] or [bold]saara distill[/bold]")
            raise typer.Exit(1)
            
        console.print(Panel.fit("[bold cyan]Select Training Dataset[/bold cyan]", border_style="cyan"))
        for i, f in enumerate(candidates, 1):
             console.print(f"[{i}] {f.name} ({f.stat().st_size / 1024:.1f} KB)")
             
        choice = Prompt.ask("Choose dataset", choices=[str(i) for i in range(1, len(candidates)+1)], default="1")
