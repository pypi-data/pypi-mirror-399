...
def xgo_extern_motor(position: int):
    """
    控制XGO-Mini3W外接电机位置
    
    Args:
        position: 电机位置，int16有符号整数
            - 范围: [-32768, 32767]
            - 使用struct.pack("h", position)转为2字节发送
            - 具体范围取决于外接电机型号
    ...
    try:
        _xgo_instance.extern_motor(position)
        time.sleep(0.5)
        return ToolResponse(content=[TextBlock(type="text", text=f"✓ XGO-Mini3W外接电机位置设置为{position}")])
    except Exception as e:
        return ToolResponse(content=[TextBlock(type="text", text=f"❌ 外接电机控制失败: {str(e)}")])


def xgo_enable_wheel_control(mode: int):
    """
    启用/禁用XGO-Mini3W轮控模式
    ...
    try:
        if mode not in [0, 1]:
            return ToolResponse(content=[TextBlock(type="text", text="❌ 模式参数错误，必须为0(启用)或1(禁用)")])
        
        _xgo_instance.enable_wheel_control(mode)
        time.sleep(0.3)
        status = "启用" if mode == 0 else "禁用"
        return ToolResponse(content=[TextBlock(type="text", text=f"✓ XGO-Mini3W轮控模式已{status}")])
    except Exception as e:
        return ToolResponse(content=[TextBlock(type="text", text=f"❌ 轮控模式设置失败: {str(e)}")])


def xgo_wheel_control(data: list):
    """
    控制XGO-Mini3W的四个轮子
    
    Args:
        data: 四个轮子的控制数据列表 [wheel1, wheel2, wheel3, wheel4]
            - 必须为包含4个元素的列表
            - 每个元素范围: [0, 255] (uint8)
            - 默认值128表示停止，小于128反转，大于128正转
            - 数值越接近0或255，转速越快
    ...
    try:
        if not isinstance(data, list) or len(data) != 4:
            return ToolResponse(content=[TextBlock(type="text", text="❌ 轮控数据错误，必须为包含4个元素的列表")])
        
        _xgo_instance.wheel_control(data)
        time.sleep(0.3)
        return ToolResponse(content=[TextBlock(type="text", text=f"✓ XGO-Mini3W轮控制已执行: {data}")])
    except Exception as e:
        return ToolResponse(content=[TextBlock(type="text", text=f"❌ 轮控制失败: {str(e)}")])


def register_tools(toolkit, api_key: str):
    """
    注册Mini3W专用工具到toolkit
    ...
    # 创建工具组
    toolkit.create_tool_group("xgo_mini3w", "XGO-Mini3W控制工具", active=True)
    
    # 注册Mini3W运动工具
    toolkit.register_tool_function(xgo_move_x, group_name="xgo_mini3w")
    toolkit.register_tool_function(xgo_move_y, group_name="xgo_mini3w")
    toolkit.register_tool_function(xgo_turn, group_name="xgo_mini3w")
    toolkit.register_tool_function(xgo_translation, group_name="xgo_mini3w")
    toolkit.register_tool_function(xgo_attitude, group_name="xgo_mini3w")
    toolkit.register_tool_function(xgo_action, group_name="xgo_mini3w")
    toolkit.register_tool_function(xgo_stop, group_name="xgo_mini3w")
    toolkit.register_tool_function(xgo_mark_time, group_name="xgo_mini3w")
    toolkit.register_tool_function(xgo_periodic_tran, group_name="xgo_mini3w")
    toolkit.register_tool_function(xgo_periodic_rot, group_name="xgo_mini3w")
    toolkit.register_tool_function(xgo_gait_type, group_name="xgo_mini3w")
    toolkit.register_tool_function(xgo_pace, group_name="xgo_mini3w")
    toolkit.register_tool_function(xgo_imu, group_name="xgo_mini3w")
    toolkit.register_tool_function(xgo_find_person, group_name="xgo_mini3w")
    
    # 注册单腿和舵机控制
    toolkit.register_tool_function(xgo_leg, group_name="xgo_mini3w")
    toolkit.register_tool_function(xgo_motor, group_name="xgo_mini3w")
    toolkit.register_tool_function(xgo_motor_speed, group_name="xgo_mini3w")
    toolkit.register_tool_function(xgo_unload_motor, group_name="xgo_mini3w")
    toolkit.register_tool_function(xgo_load_motor, group_name="xgo_mini3w")
    
    # 注册通用工具
    toolkit.register_tool_function(xgo_battery, group_name="xgo_mini3w")
    toolkit.register_tool_function(xgo_read_roll, group_name="xgo_mini3w")
    toolkit.register_tool_function(xgo_read_pitch, group_name="xgo_mini3w")
    toolkit.register_tool_function(xgo_read_yaw, group_name="xgo_mini3w")
    toolkit.register_tool_function(xgo_read_motor, group_name="xgo_mini3w")
    toolkit.register_tool_function(xgo_reset, group_name="xgo_mini3w")

    
    # 注册显示和语音工具
    toolkit.register_tool_function(xgo_display_text, group_name="xgo_mini3w")
    toolkit.register_tool_function(xgo_display_clear, group_name="xgo_mini3w")
    toolkit.register_tool_function(xgo_play_http_audio, group_name="xgo_mini3w")
    toolkit.register_tool_function(xgo_display_http_image, group_name="xgo_mini3w")
    
    # 注册Mini3W专用工具
    toolkit.register_tool_function(xgo_arm_control, group_name="xgo_mini3w")
    toolkit.register_tool_function(xgo_find_ball, group_name="xgo_mini3w")
    toolkit.register_tool_function(xgo_catch_ball, group_name="xgo_mini3w")
    
    # 注册Mini3W特有工具（轮控功能）
    toolkit.register_tool_function(xgo_extern_motor, group_name="xgo_mini3w")
    toolkit.register_tool_function(xgo_enable_wheel_control, group_name="xgo_mini3w")
    toolkit.register_tool_function(xgo_wheel_control, group_name="xgo_mini3w")
    ...
    if api_key:
        ...
        toolkit.register_tool_function(photo_understand_with_key, group_name="xgo_mini3w")
        toolkit.register_tool_function(speech_recognition_with_key, group_name="xgo_mini3w")
        toolkit.register_tool_function(text_to_speech_with_key, group_name="xgo_mini3w")
        toolkit.register_tool_function(generate_image_with_key, group_name="xgo_mini3w")
    
    print("✓ XGO-Mini3W工具集注册完成")
