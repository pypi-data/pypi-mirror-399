    ...
    **Block Structure**: The model supports block-structured factors (factors organized
    ...
    def _update_parameters(self, A: np.ndarray, C: np.ndarray, Q: np.ndarray,
        ...
        def _ensure_dtype(arr: Optional[np.ndarray]) -> Optional[np.ndarray]:
            # Conditional logic: Return None if input is None (not validation)
            if arr is None:
                return None
    ...
    def predict(
        ...
        *,
        ...
        # Conditional logic: Generate result if not already computed (not validation)
        ...
        # Conditional logic: Compute default horizon if not provided (not validation)
        if horizon is None:
        ...
        check_condition(
            not np.any(np.isnan(Z_last)),
            NumericalError,
            f"DFM prediction failed: {np.sum(np.isnan(Z_last))}/{len(Z_last)} factors contain NaN ({np.sum(np.isnan(Z_last)) / len(Z_last):.1%})",
            details="Model may not have converged. Try increasing max_iter or checking data quality"
        )
        
        # Validate parameters are finite
        check_condition(
            np.all(np.isfinite(A)) and np.all(np.isfinite(C)),
            NumericalError,
        ...
        check_condition(
            target_series is not None and len(target_series) > 0,
            ValueError,
        ...
        check_condition(
            target_indices is not None and len(target_indices) > 0,
            DataError,
    ...
    def result(self) -> DFMResult:
        ...
        if self._result is None:
            ...
            self._result = self.get_result()
        check_condition(
            isinstance(self._result, DFMResult),
            ModelNotTrainedError,
