...
class Names:
    ...
    def __set_name(self, attr: dict | str):
        if self.mode == 'tag':
            return list(set([value for keys, values in attr.items() for value in values['tags']]))[0]
    ...
    def __set_name_snake(self):
        ...
        if self.mode == 'endpoint':
            return f'{Text.to_snake_case(self.name.replace("/api/v1/", "").replace("/", "_").replace("{", "").replace("}", ""))}'
        if self.mode == 'schema':
...
class Configuration:
    ...
    def schemas(self) -> list[Schema]:
        return [
            ...
            ) for schema, schema_values in self.open_api["components"]["schemas"].items()]
...
class Creator:
    ...
    def __init__(self, *, mode: Literal["+a", "+w"], line: list[Line]):
    ...
    def __call__(self, func):
        def wrapper(*args, **kwargs):
            for _ in self.line:
                ...
                with open(f'{self.configuration.paths['path']}{_.path}', self.mode, encoding="UTF-8") as file:
                    file = File(file)
...
class Create:
    ...
    def set_endpoints(self):
        @self.creator(mode='+a', line=[Line(line=list(filter(lambda __: __.tags.name == _.name, self.configuration.endpoints)), path=f'{self.configuration.paths["swagger_endpoints"]}{_.name_snake}.py') for _ in self.configuration.tags])
        def wrapper(line: list[Endpoint]):
            return [
                ...
                f'            self.response = {endpoint.names.name_camel}.{method.name.capitalize() + "Status" + schema.status}(**response.json())' if schema.names.name == None else f'            self.response = {schema.names.name}(**response.json())',] for schema in method.schemas if schema.status != None and (schema.names.name != None or schema.properties != None)],
                f'        return response'] for method in endpoint.methods]
                ] for endpoint in line]
            ]
        wrapper()

    def set_schemas(self):
        @self.creator(mode='+w', line=[Line(line=schema, path=f'{self.configuration.paths["swagger_schemas"]}{schema.names.name_snake}.py') for schema in self.configuration.schemas])
    ...
    def set_conftest_api(self):
        @self.creator(mode='+w', line=[Line(line=self.configuration.environments, path=f'{self.configuration.paths["tests"]}conftest.py')])
        def wrapper(line):
            return [
                'import pytest',
                'from httpx import AsyncClient',
    ...
    def set_conftest_ui(self):
        @self.creator(mode='+w', line=[Line(line=self.configuration.environments, path=f'{self.configuration.paths["tests_test_ui"]}conftest.py')])
        def wrapper(line):
            return [
                'import pytest',
                'import allure',
    ...
    def set_pytest(self):
        @self.creator(mode='+w', line=[Line(line={'api': self.configuration.tags, 'ui': self.configuration.pages}, path='pytest.ini')])
    ...
    def set_readme(self):
        @self.creator(mode='+w', line=[Line(line='', path='README.md')])
        def wrapper(line):
            return [
    ...
    def set_requirements(self):
        @self.creator(mode='+w', line=[Line(line='', path='requirements.txt')])
        def wrapper(line):
            return sorted([
                'pytest>=8.4.2',
    ...
    def set_gitignore(self):
        @self.creator(mode='+w', line=[Line(line='', path='.gitignore')])
        def wrapper(line):
            return sorted([
    ...
    def set_environments(self):
        @self.creator(mode='+w', line=[Line(line=_, path=_) for _ in [".env", ".env.example"]])
        def wrapper(line):
            return [
    ...
    def set_blank_client(self):
        @self.creator(mode='+w', line=[Line(line='', path=f'{self.configuration.paths["swagger"]}blank_client.py')])
        def wrapper(line):
            return [
                'import os',
                ...
                [f'        self.{tag.name_snake} = {tag.name_camel}(self.__client)' for tag in self.configuration.tags]
            ]
        wrapper()

    def set_param_env(self):
        @self.creator(mode='+w', line=[Line(line='', path=f'{self.configuration.paths["params"]}param_env.py')])
        def wrapper(line):
            return [
                'import pytest',
                'from dataclasses import dataclass',
    ...
    def set_params_api(self):
        @self.creator(mode='+a', line=[Line(line=list(filter(lambda __: __.tags.name == _.name, self.configuration.endpoints)), path=f'{self.configuration.paths['params_api']}param_{_.name_snake}.py') for _ in self.configuration.tags])
        def wrapper(line: list[Endpoint]):
            return [
                'import pytest',
                'from dataclasses import dataclass',
    ...
    def set_tests_api(self):
        @self.creator(mode='+a', line=[Line(line=_, path=f'{self.configuration.paths["tests_test_api"]}test_{_.tags.name_snake}/test_{_.names.name_snake}.py') for _ in self.configuration.endpoints])
    ...
    def set_pages(self):
        @self.creator(mode='+a', line=[Line(line={"page": _, "components": [___ for ___, count in Counter([__ for _ in self.configuration.pages for __ in _["components"]]).items() if count > 1]}, path=f'{self.configuration.paths["browser_pages"]}{_["name"]}.py') for _ in self.configuration.pages])
    ...
    def set_components(self):
        @self.creator(mode='+a', line=[Line(line=_, path=f'{self.configuration.paths['browser_components']}component_{_}.py') for _, count in Counter([__ for _ in self.configuration.pages for __ in _["components"]]).items() if count > 1])
        def wrapper(line):
            return [
                'from playwright.async_api import Page, expect',
    ...
    def set_blank_page(self):
        @self.creator(mode='+w', line=[Line(line='', path=f'{self.configuration.paths['browser']}blank_page.py')])
        def wrapper(line):
            return [
                'from typing import Literal',
    ...
    def set_tests_ui(self):
        @self.creator(mode='+a', line=[Line(line={'page': page, 'component': _}, path=f'{self.configuration.paths["tests_test_ui"]}test_{page["name"]}/test_{Text.to_snake_case(page["name"] + '_' + _.replace(f'{page["name"]}_', ''))}.py') for page in self.configuration.pages for _ in page["components"]])
    ...
    def set_params_ui(self):
        @self.creator(mode='+a', line=[Line(line=_, path=f'{self.configuration.paths["params_ui"]}param_{_["name"]}.py') for _ in self.configuration.pages])
