...
Parse X12 837I files into structured data with multiple output formats.
"""
...
def parse(content: str) -> "Claim837I":
    """Parse 837I institutional claim content and return parser object."""
    return Claim837I(content)
...
class Claim837I:
    """
    Parse and access X12 837I (Institutional Claims) data.

    Usage:
        claim = Claim837I(edi_content)
    ...
    def _parse(self) -> None:
        ...
        for segment_raw in self._segments:
            ...
            if seg_id == 'ISA':
                ...
                elif entity_type == '71':  # Attending Physician
                    if current_claim:
                        current_claim['attending_physician'] = entity
                elif entity_type == '72':  # Operating Physician
                    if current_claim:
                        current_claim['operating_physician'] = entity
                elif entity_type == '77':  # Service Facility
                    if current_claim:
                        current_claim['service_facility'] = entity
            ...
            elif seg_id == 'CL1' and current_claim:
                current_claim['institutional_info'] = self._parse_cl1(elements)
                ...
                service = self._parse_sv2(elements)
    ...
    def _parse_cl1(self, elements: List[str]) -> Dict[str, Any]:
        return {
            'admission_type': elements[0] if len(elements) > 0 else '',
            'admission_source': elements[1] if len(elements) > 1 else '',
            'patient_status': elements[2] if len(elements) > 2 else ''
        }
    ...
    def _apply_dtp(self, claim: Dict[str, Any], service: Optional[Dict[str, Any]], elements: List[str]) -> None:
        ...
        elif date_format == 'DT' and len(date_value) >= 8:
            date_value = f"{_format_date(date_value[:8])} {date_value[8:10]}:{date_value[10:12]}" if len(date_value) >= 12 else _format_date(date_value[:8])
    ...
    def _parse_sv2(self, elements: List[str]) -> Dict[str, Any]:
        ...
        if ':' in procedure_info:
            ...
            if len(parts) > 1 and parts[0] == 'HC':
                hcpcs_code = parts[1]
                modifiers = [p for p in parts[2:6] if p]

        return {
            'revenue_code': elements[0] if len(elements) > 0 else '',
            'procedure_qualifier': 'HC' if hcpcs_code else '',
            'hcpcs_code': hcpcs_code,
            'modifiers': modifiers,
            'charge': _to_float(elements[2]) if len(elements) > 2 and elements[2] else 0.0,
            'unit_qualifier': elements[3] if len(elements) > 3 else 'UN',
            'units': elements[4] if len(elements) > 4 else '1'
        }
    ...
    def _parse_hi(self, elements: List[str]) -> List[Dict[str, Any]]:
        ...
        # Handle DRG format: HI*DR*291 (separate elements, no colon)
        if len(elements) >= 2 and elements[0] == 'DR' and ':' not in elements[0]:
            diagnoses.append({
                'qualifier': 'DR',
                'diagnosis_code': elements[1] if len(elements) > 1 else '',
                'type': 'drg'
            })
            return diagnoses
        ...
        for element in elements:
            ...
            if ':' in element:
                ...
                if qualifier in ('ABK', 'BK'):
                    dx_type = 'principal'
                elif qualifier in ('ABJ', 'BJ'):
                    dx_type = 'admitting'
                elif qualifier == 'APR':
                    dx_type = 'patient_reason'
                elif qualifier == 'DR':
                    dx_type = 'drg'
                elif qualifier in ('ABF', 'BF'):
                    dx_type = 'primary' if is_first else 'secondary'
                else:
                    dx_type = 'other'
    ...
    @property
    def billing_facility(self) -> Dict[str, Any]:
        """First/primary billing facility."""
    ...
    def summary(self) -> Dict[str, Any]:
        ...
        for claim in self.claims:
            ...
            if dates.get('admission_date'):
                all_dates.append(dates['admission_date'])
        ...
        for claim in self.claims:
            ...
            atype = inst.get('admission_type', 'Unknown')
        ...
        return {
            ...
            'billing_facility': self.billing_facility.get('last_name', ''),
            'billing_npi': self.billing_facility.get('id_value', ''),
