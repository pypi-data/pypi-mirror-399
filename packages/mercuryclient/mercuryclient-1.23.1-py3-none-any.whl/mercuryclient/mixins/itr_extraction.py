...
class ItrExtractionMixin:
    """
    Mixin for extract details itr
    """

    def initiate_itr_extraction_request(
        self,
        request_data,
    ) -> str:
        api = "api/v1/itr/extract"
    ...
    def get_itr_extraction_response(self, request_id: str) -> Tuple[str, dict]:
        api = "api/v1/itr/extract"
    ...
    def extract_itr_details(
        ...
        if date_of_birth:
            ...
            request_data["dob"] = date_of_birth.strftime("%Y-%m-%d")

        mercury_request_id = self.initiate_itr_extraction_request(request_data)

        for _ in range(max_attempts):
            time.sleep(retry_backoff)
            mercury_request_id, result = self.get_itr_extraction_response(
                mercury_request_id
            )
