...
class TestLocalRunnerExport(unittest.TestCase):
    
    @patch('simtrading.runners.local_runner.TradeTpClient')
    @patch('simtrading.runners.local_runner.ResultExporter')
    def test_run_local_backtest_with_export(self, MockResultExporter, MockTradeTpClient):
        # Setup Mocks
    ...
    @patch('simtrading.runners.local_runner.TradeTpClient')
    def test_run_local_backtest_no_export(self, MockTradeTpClient):
        strategy = DummyStrategy()
        candles = {"BTC-USD": []}
