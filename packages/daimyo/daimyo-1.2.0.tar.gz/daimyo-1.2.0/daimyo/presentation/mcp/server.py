...
from daimyo.application.formatters import CategoryTreeBuilder, MarkdownFormatter
...
def get_rules(scope_name: str, categories: str | None = None) -> str:
    """Get rules for a scope in markdown format.\f
...
def get_category_index(scope_name: str) -> str:
    ...
    try:
        ...
        if merged_scope.metadata.description:
            lines.append(f"{merged_scope.metadata.description}\n")

        if merged_scope.sources:
            lines.append(f"**Sources**: {', '.join(merged_scope.sources)}\n")
