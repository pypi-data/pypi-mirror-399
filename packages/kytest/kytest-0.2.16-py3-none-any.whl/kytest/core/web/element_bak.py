...
class Elem:
    ...
    def assert_hidden(self, timeout=5):
        logger.info("断言：是否隐藏")
        expect(self.find(timeout=timeout, n=0)).to_be_hidden()
        logger.info("断言完成")

    def assert_text_ct(self, text: str, timeout=5):
        logger.info(f"断言：是否包含文本-{text}")
        expect(self.find(timeout=timeout, n=0)).to_contain_text(text, timeout=timeout)
        logger.info("断言完成")

    def assert_text_eq(self, text: str, timeout=5):
        logger.info(f"断言：是否等于文本-{text}")
        expect(self.find(timeout=timeout, n=0)).to_have_text(text)
        logger.info("断言完成")

    def assert_count_eq(self, count, timeout=5):
        logger.info(f"断言：locator定位到几个元素")
        expect(self.find(timeout=timeout, n=0)).to_have_count(count)
        logger.info("断言完成")
    ...
    def screenshot(self, file_path, timeout=5, full_screen=True):
        ...
        if full_screen is True:
            ...
            self.find(timeout=timeout, n=0).screenshot(path=file_path)
        logger.info("截屏完成")
