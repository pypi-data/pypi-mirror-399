...
# Download weights from local directory
...
def construction_identification(input_path,
                                csv_path="CSV_verb_dependency_information.csv",
                                xlsx_path="XLSX_verb_dependency_information.xlsx",
...
def save_constructional_outputs(identification_csv="CSV_construction_identification.csv",
    ...
    Returns:
        tuple: (df_diversity, df_elaboration, df_verb, df_construction)
            df_diversity (pd.DataFrame): Constructional diversity results
            df_elaboration (pd.DataFrame): Constructional elaboration results
    ...
    # --- Overall elaboration per file ---
    df = pd.read_csv(identification_csv)

    overall_elaboration = (
        df.groupby("file_name")["dependent_count"]
        .mean()
        .reset_index(name="overall_elaboration")
    )

    # --- Elaboration per construction per file ---
    construction_elaboration = (
        df.groupby(["file_name", "predicted_label"])["dependent_count"]
        .mean()
        .reset_index(name="constructional_elaboration")
    )

    # Pivot construction elaboration to wide format
    construction_elaboration_wide = construction_elaboration.pivot_table(
        index="file_name",
    ...
    # Round values
    construction_elaboration_wide.iloc[:, 1:] = construction_elaboration_wide.iloc[:, 1:].round(4)

    # Rename columns for clarity
    construction_elaboration_wide.rename(
        columns={col: f"constructional_elaboration_{col}" for col in construction_elaboration_wide.columns if
                 col != "file_name"},
        inplace=True
    )

    # --- Merge overall + per-construction elaboration ---
    df_elaboration = pd.merge(overall_elaboration, construction_elaboration_wide, on="file_name")

    # Save to CSV
    df_elaboration.to_csv(elaboration_out, index=False)
    print(f"Saved overall + constructional elaboration indices to {elaboration_out}")
    ...
    # --- Compute construction frequency / proportion indices ---
    counts = (
