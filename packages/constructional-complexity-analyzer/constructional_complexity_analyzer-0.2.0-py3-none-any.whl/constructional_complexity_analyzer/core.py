...
def save_constructional_outputs(identification_csv="CSV_construction_identification.csv",
    ...
    # --- Compute elaboration index per file (ignoring construction) ---
    df = pd.read_csv(identification_csv)

    df_elaboration = (
        df.groupby("file_name")["dependent_count"]
        .mean()
        .reset_index(name="elaboration_index")
    )

    # Round values
    df_elaboration["elaboration_index"] = df_elaboration["elaboration_index"].round(4)

    # Save to CSV
    df_elaboration.to_csv(elaboration_out, index=False)
    print(f"Saved file-level elaboration indices to {elaboration_out}")
    ...
    return results_constructional_diversity, df_elaboration, pivot_table
...
def run_constructional_analysis(
    ...
    df_diversity, df_elaboration, df_verb = save_constructional_outputs(
    ...
    return df_diversity, df_elaboration, df_verb
