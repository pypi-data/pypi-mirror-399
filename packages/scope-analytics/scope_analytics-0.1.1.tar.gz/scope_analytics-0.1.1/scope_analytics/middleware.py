...
def _set_request_context(headers: dict, path: str = None) -> None:
    """
    Set request context variables from HTTP headers.
    This context is used to enrich LLM events with request information.
    ...
    if path:
        ScopeContext.set_url(path)

    # Extract and set referrer (case-insensitive)
    referrer = None
    for key, value in headers.items():
        key_lower = key.lower()
        if key_lower == "referer" or key_lower == "referrer":
            referrer = value
            break
    if referrer:
        ScopeContext.set_referrer(referrer)

    # Extract and set thread_id (X-Scope-Thread-ID header for ChatGPT-style apps)
    thread_id = None
    for key, value in headers.items():
        if key.lower() == "x-scope-thread-id":
            thread_id = value
            break
    if thread_id:
        ScopeContext.set_thread_id(thread_id)

    # Extract and set user_agent
    user_agent = None
    for key, value in headers.items():
        if key.lower() == "user-agent":
            ...
            break
    if user_agent:
        ScopeContext.set_user_agent(user_agent)
...
class ScopeSessionMiddleware:
    ...
    async def __call__(self, scope, receive, send):
        ...
        # Set request context (URL, referrer, thread_id, user_agent) for LLM events
        _set_request_context(headers, path)
...
class DjangoScopeMiddleware:
    ...
    def __call__(self, request):
        ...
        # Set request context (URL, referrer, thread_id, user_agent) for LLM events
        _set_request_context(django_headers, path)
