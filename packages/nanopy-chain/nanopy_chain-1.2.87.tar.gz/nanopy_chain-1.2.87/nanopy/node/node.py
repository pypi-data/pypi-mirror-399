...
class NanoPyNode:
    ...
    def send_raw_transaction(self, raw_tx: str) -> str:
        """Send raw signed transaction (supports legacy, EIP-2930, and EIP-1559)"""
        import rlp
        ...
        # Type 1 = EIP-2930 (access list), Type 2 = EIP-1559 (dynamic fee)
        tx_type = None
        if tx_bytes[0] <= 0x7f:
            tx_type = tx_bytes[0]
            tx_bytes = tx_bytes[1:]  # Remove type prefix

        # Decode RLP
        decoded = rlp.decode(tx_bytes)

        if tx_type == 2:
            # EIP-1559 transaction: [chainId, nonce, maxPriorityFeePerGas, maxFeePerGas, gasLimit, to, value, data, accessList, signatureYParity, signatureR, signatureS]
            if len(decoded) != 12:
                raise ValueError(f"Invalid EIP-1559 transaction: expected 12 fields, got {len(decoded)}")

            chain_id = int.from_bytes(decoded[0], 'big') if decoded[0] else self.chain_id
            ...
            gas_limit = int.from_bytes(decoded[4], 'big') if decoded[4] else 21000
            to = "0x" + decoded[5].hex() if decoded[5] else None
            value = int.from_bytes(decoded[6], 'big') if decoded[6] else 0
            data = decoded[7] if decoded[7] else b''
            # decoded[8] = accessList (ignored for now)
            y_parity = int.from_bytes(decoded[9], 'big') if decoded[9] else 0
            r = int.from_bytes(decoded[10], 'big') if decoded[10] else 0
            s = int.from_bytes(decoded[11], 'big') if decoded[11] else 0

            # Convert to legacy-style v for compatibility
            # EIP-1559 uses y_parity (0 or 1), we need v = 27 + y_parity for signing
            v = 27 + y_parity

            # Use maxFeePerGas as gasPrice for our simplified model
            gas_price = max_fee
            ...
            # EIP-2930 transaction: [chainId, nonce, gasPrice, gasLimit, to, value, data, accessList, signatureYParity, signatureR, signatureS]
            if len(decoded) != 11:
                raise ValueError(f"Invalid EIP-2930 transaction: expected 11 fields, got {len(decoded)}")

            chain_id = int.from_bytes(decoded[0], 'big') if decoded[0] else self.chain_id
            nonce = int.from_bytes(decoded[1], 'big') if decoded[1] else 0
            gas_price = int.from_bytes(decoded[2], 'big') if decoded[2] else 0
            gas_limit = int.from_bytes(decoded[3], 'big') if decoded[3] else 21000
            to = "0x" + decoded[4].hex() if decoded[4] else None
            value = int.from_bytes(decoded[5], 'big') if decoded[5] else 0
            data = decoded[6] if decoded[6] else b''
            # decoded[7] = accessList (ignored for now)
            y_parity = int.from_bytes(decoded[8], 'big') if decoded[8] else 0
            r = int.from_bytes(decoded[9], 'big') if decoded[9] else 0
            s = int.from_bytes(decoded[10], 'big') if decoded[10] else 0
        ...
        elif tx_type is None and len(decoded) == 9:
            ...
            raise ValueError(f"Unsupported transaction format: type={tx_type}, fields={len(decoded)}")
    ...
    def call(self, tx_data: dict, block: str = "latest") -> str:
        ...
        context = ExecutionContext(
            ...
            gas=gas,
            code=code,
            is_static=False,  # Allow state changes on temp_state copy for simulation
    ...
    def estimate_gas(self, tx_data: dict, block: str = "latest") -> int:
        ...
        # Use state copy for simulation (like eth_call)
        temp_state = self.state.copy()

        from nanopy.vm.evm import NanoPyEVM, ExecutionContext
        evm = NanoPyEVM(temp_state)

        # Snapshot for safety (though we're using a copy)
        snapshot = temp_state.snapshot()
        ...
        try:
            result = evm.execute(context)
            # No need to revert - we used a state copy
