...
class ProducerWorker:
    ...
    async def _async_process_messages(self) -> None:
        ...
        """
        BATCH_SIZE = 16  # Process up to 16 messages per iteration
    ...
    async def _get_batch_from_queues(self, max_size: int = 16) -> List[Dict[str, Any]]:
        ...
        if first_task:
            ...
            return tasks  # No tasks available

        # Then, drain additional tasks without blocking
        num_queues = len(self.output_queues)
        for _ in range(max_size - 1):
            try:
                # Try non-blocking get from current queue
