...
class AsyncConsumerManager:
    ...
    # ================================================================
    BATCH_SIZE = 5000       # Messages to read per XREAD (high for batch efficiency)
    BLOCK_MS = 5            # XREAD block time in ms (low for responsiveness)
    QUEUE_MAX = 10_000      # Internal queue size (only used for metrics/backpressure tracking)
    BACKPRESSURE_THRESHOLD = 0.8  # Skip frames when queue exceeds threshold
    ...
    async def _consume_all_streams_direct(self) -> None:
        ...
        self.logger.info(
            ...
            f"(BATCH_SIZE={batch_size}, BLOCK_MS={block_ms})"
        )
                ...
                for stream_name_bytes, entries in messages:
                    ...
                    for msg_id, fields in entries:
                        ...
                        if shm_mode in (b"1", b"true", b"True"):
                            ...
                            if frame_task is not None and self.use_shm:
                                try:
                                    shm_buffer = self._get_or_attach_shm_buffer(
                                        camera_id,
                                        frame_task.shm_name.decode() if isinstance(frame_task.shm_name, bytes) else frame_task.shm_name,
                                        frame_task.width,
                                        frame_task.height,
                                        frame_task.frame_format,  # Use actual format from frame
                                    )
                                    if shm_buffer:
                                        current_write_idx = shm_buffer.get_current_frame_idx()
                                        frames_behind = current_write_idx - frame_task.frame_idx
                                        # Skip if more than half slot_count behind
                                        if frames_behind > shm_buffer.slot_count // 2:
                                            frames_stale_skipped += 1
                                            continue  # Skip stale frame at Redis level
                                except Exception:
                                    pass  # On error, let inference worker handle it
                        else:
