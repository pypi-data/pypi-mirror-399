            ...
            if key == "properties" and isinstance(sanitized_value, dict):
                ...
                for prop_name, prop_value in sanitized_value.items():
                    ...
                    if prop_name.startswith("_"):
                        ...
                        # Ensure we don't create empty names or duplicates
                        if not new_name or new_name in new_properties:
                            new_name = f"param_{prop_name.lstrip('_')}"
...
def _patch_mcp_use_schema_handler() -> None:
    ...
    try:
        from mcp_use.agents.adapters.base import BaseAdapter  # type: ignore[import-untyped]
...
class Astha:
    ...
    def __init__(
        ...
        *,
        ...
        if not aztp_id or not isinstance(aztp_id, str):
            raise ValueError("aztp_id must be a non-empty string")

        if not policy_service_url or not isinstance(policy_service_url, str):
            raise ValueError("policy_service_url must be a non-empty string")
    ...
    def _get_mcp_config(self) -> dict[str, Any]:
        ...
        if self._mcp_config is not None:
            if not isinstance(self._mcp_config, dict):
                raise InitializationError("mcp_config must be a dictionary")
            if "mcpServers" not in self._mcp_config:
                raise InitializationError("mcp_config missing 'mcpServers' key")
            ...
            try:
                ...
                if not config_path.exists():
                    raise InitializationError(
                        f"MCP config file not found: {self._mcp_config_file}"
                    )

                if not config_path.is_file():
                    raise InitializationError(
                        f"MCP config path is not a file: {self._mcp_config_file}"
                    )
                ...
                if "mcpServers" not in config:
                    raise InitializationError(
                        f"MCP config file missing 'mcpServers' key: {self._mcp_config_file}"
                    )
                ...
                raise InitializationError(
                    f"Invalid JSON in MCP config file {self._mcp_config_file}: {e}"
                ) from e
            except Exception as e:
                raise InitializationError(
