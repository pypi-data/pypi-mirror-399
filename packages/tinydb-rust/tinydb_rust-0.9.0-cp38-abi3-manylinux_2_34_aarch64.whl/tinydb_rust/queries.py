...
Starting with :class:`~tinydb_rust.queries.Query` you can construct complex
queries:
...
from ._tinydb_core import freeze, Query as RustQuery
...
class QueryInstance:
    ...
    :class:`~tinydb_rust.queries.Query` class acts like a query builder and
    generates :class:`~tinydb_rust.queries.QueryInstance` objects which will
    ...
    def __init__(self, test: Callable[[Mapping], bool], hashval: Optional[Tuple]):
        self._test = test
        self._hash = hashval
        self._rust_query = None  # Shadow Rust Query object
    ...
    def __and__(self, other: 'QueryInstance') -> 'QueryInstance':
        ...
        # Try to combine Rust queries if both exist
        rust_query = None
        try:
            if hasattr(self, '_rust_query') and self._rust_query is not None:
                if hasattr(other, '_rust_query') and other._rust_query is not None:
                    rust_query = self._rust_query & other._rust_query
        except Exception:
            # If Rust operation fails, rust_query remains None
            pass
        
        result = QueryInstance(lambda value: self(value) and other(value), hashval)
        result._rust_query = rust_query
        return result
    ...
    def __or__(self, other: 'QueryInstance') -> 'QueryInstance':
        ...
        result = QueryInstance(lambda value: self(value) or other(value), hashval)
        result._rust_query = rust_query
        return result
    ...
    def __invert__(self) -> 'QueryInstance':
        ...
        # Try to invert Rust query if it exists
        rust_query = None
        try:
            if hasattr(self, '_rust_query') and self._rust_query is not None:
                rust_query = ~self._rust_query
        except Exception:
            # If Rust operation fails, rust_query remains None
            pass
        
        result = QueryInstance(lambda value: not self(value), hashval)
        result._rust_query = rust_query
        return result
...
class Query(QueryInstance):
    ...
    def __init__(self) -> None:
        ...
        # Initialize shadow Rust Query object
        self._rust_query = RustQuery()
    ...
    def __getattr__(self, item: str):
        ...
        # Update Rust query path if it exists
        try:
            if hasattr(self, '_rust_query') and self._rust_query is not None:
                query._rust_query = getattr(self._rust_query, item)
            else:
                query._rust_query = None
        except Exception:
            # If Rust operation fails, set to None
            query._rust_query = None
    ...
    def __getitem__(self, item: str):
        ...
        # Update Rust query path using __getitem__ to match Python's __getitem__ behavior
        try:
            if self._rust_query is not None:
                query._rust_query = self._rust_query[item]
            else:
                query._rust_query = None
        except Exception:
            # If Rust operation fails, set to None
    ...
    def _generate_test(
        ...
        :return: A :class:`~tinydb_rust.queries.QueryInstance` object
        """
        ...
        # If the path contains callables (from map()), we can't use Rust query
        # Check if any part of the path is a callable
        has_callable = any(callable(part) for part in self._path)
        if has_callable:
            inst._rust_query = None
        else:
            # Try to preserve _rust_query if it exists and path doesn't have callables
            # (This will be set by the calling method like __eq__, __ne__, etc.)
            pass
    ...
    def __eq__(self, rhs: Any):
        ...
        """
        inst = self._generate_test(
            lambda value: value == rhs,
            ('==', self._path, freeze(rhs))
        )
        
        # Try to perform equivalent operation on Rust query
        try:
            if self._rust_query is not None:
                inst._rust_query = self._rust_query == rhs
        except Exception:
            # If Rust operation fails, rust_query remains None
            inst._rust_query = None
    ...
    def exists(self) -> QueryInstance:
        ...
        try:
            if self._rust_query is not None:
                inst._rust_query = self._rust_query.exists()
        except Exception:
            # If Rust operation fails, rust_query remains None
            inst._rust_query = None
    ...
    def matches(self, regex: str, flags: int = 0) -> QueryInstance:
        ...
        inst = self._generate_test(test, ('matches', self._path, regex))
        
        # Try to perform equivalent operation on Rust query
        try:
            if self._rust_query is not None:
                inst._rust_query = self._rust_query.matches(regex)
        except Exception:
            # If Rust operation fails, rust_query remains None
            inst._rust_query = None
    ...
    def search(self, regex: str, flags: int = 0) -> QueryInstance:
        ...
        inst = self._generate_test(test, ('search', self._path, regex))
        
        # Try to perform equivalent operation on Rust query
        try:
            if self._rust_query is not None:
                inst._rust_query = self._rust_query.search(regex)
        except Exception:
            # If Rust operation fails, rust_query remains None
            inst._rust_query = None
    ...
    def test(self, func: Callable[[Mapping], bool], *args) -> QueryInstance:
        ...
        .. warning::
            ...
            may mess up the query cache that :class:`~tinydb_rust.table.Table`
            implements.
    ...
    def map(self, fn: Callable[[Any], Any]) -> 'Query':
        ...
        # map() uses Python callables which Rust doesn't support, so set _rust_query to None
        query._rust_query = None
