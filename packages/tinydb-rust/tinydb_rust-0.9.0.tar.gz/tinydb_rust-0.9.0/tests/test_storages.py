...
from tinydb_rust import TinyDB, where
from tinydb_rust.storages import JSONStorage, MemoryStorage, Storage, touch
from tinydb_rust.table import Document
...
def test_read_once():
    ...
    with TinyDB(storage=MyStorage) as db:
        ...
        assert count == 2  # One for getting the next ID (cached), one for the insert

        db.all()

        assert count == 2  # Cached: no additional read needed
