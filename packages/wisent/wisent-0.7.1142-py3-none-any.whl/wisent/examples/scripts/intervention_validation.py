...
def apply_steering_hook(
    ...
    def hook(module, input, output):
        ...
        if isinstance(output, tuple):
            ...
            steering = coefficient * direction.to(hidden_states.device, dtype=hidden_states.dtype)
            hidden_states = hidden_states + steering.unsqueeze(0).unsqueeze(0)  # [1, 1, H] broadcasts to [B, T, H]
            return (hidden_states,) + output[1:]
        else:
            # Add steering vector to ALL positions (broadcast)
            steering = coefficient * direction.to(output.device, dtype=output.dtype)
...
def evaluate_steering(
    ...
    layer_module = model._layers[layer]
