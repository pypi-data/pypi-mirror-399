...
def setUpVersions(name, resource, outNames=None, options=None):
...
def saveResource(name, resource, output, data=None):
    if data is None:
        with open(resource.path, 'rb') as inFile:
            data = inFile.read()
...
def storeResource(name, resource, output=None, outNames=None):
    if outNames is None:
        outNames = []
    if output is None:
...
def getSourceOutputPaths(name, resource, output=None, outNames=None):
    if outNames is None:
        outNames = []
    if output is None:
        output = resource.manager.output
