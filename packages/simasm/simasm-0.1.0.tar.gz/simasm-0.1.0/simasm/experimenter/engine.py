...
class ExperimenterEngine:
    ...
    def _run_replications(
        ...
        for i in range(rep_settings.count):
            ...
            warmup = rep_settings.warm_up_time
            end_time = rep_settings.run_length
...
class VerificationEngine:
    ...
    def _create_labeling_function(
        ...
        def make_evaluator(func_name: str, te):
            """Factory function to properly capture function name and evaluator."""
            def evaluate(state) -> bool:
                try:
                    term = LocationTerm(func_name=func_name, arguments=[])
                    result = te.eval(term, Environment())
                    return bool(result)
                except Exception as e:
                    logger.warning(f"Error evaluating '{func_name}': {e}")
                    return False
            return evaluate
