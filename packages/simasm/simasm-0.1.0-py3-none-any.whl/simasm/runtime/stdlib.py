...
class StandardLibrary:
    ...
    def sort(self, lst: List, key: str) -> None:
        """
        Sort list in place by key function.
        
        The key parameter specifies how to extract sort keys from items.
        ...
        Example:
            # Sort by dynamic function (ASMObjects)
            lib.sort(FEL, "event_scheduled_time")  # Uses state.get_func("event_scheduled_time", (item,))
            
            # Sort by attribute (Python objects)
            lib.sort(events, "time")               # Uses getattr(item, "time")
        ...
        def get_key(item: Any) -> Any:
            ...
            try:
                ...
                if isinstance(item, ASMObject):
                    raise StdlibError(
                        f"lib.sort: neither dynamic function '{key}' nor attribute found for {item}"
                    )
                raise StdlibError(f"lib.sort: attribute '{key}' not found on {type(item).__name__}")
