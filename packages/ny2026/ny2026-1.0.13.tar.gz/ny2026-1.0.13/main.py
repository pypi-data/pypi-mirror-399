...
def get_countdown_to_newyear_2026() -> Tuple[str, bool, bool]:
    """
    ...
    return f"{hours:02d}:{minutes:02d}:{seconds:02d}", False, diff.total_seconds() < 2.5
...
def fireworks():
    ...
    try:
        ...
        while True:
            ...
            countdown, midnight_reached, show_fireworks = (
                get_countdown_to_newyear_2026()
            )
            ...
            if show_fireworks and elapsed - last_spawn_time > spawn_interval:
                fireworks.append(
                    Firework(canvas_width, canvas_height, camera_z, sound_manager)
                )
