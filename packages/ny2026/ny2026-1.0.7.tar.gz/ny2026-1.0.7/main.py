...
class SoundManager:
    ...
    def __init__(self):
        ...
        try:
            ...
            self.explosion_sound = self._generate_explosion_sound()
        except Exception:
            # If sound initialization fails, disable sound
            self.enabled = False
    ...
    def _generate_explosion_sound(self) -> Optional[np.ndarray]:
        ...
        Returns:
            Mono audio data as numpy array (16-bit int), or None if disabled
        """
        if not self.enabled:
            return None
        ...
        wave_filtered = np.clip(wave_filtered, -1, 1)
        wave_filtered = (wave_filtered * 32767 * 0.8).astype(np.int16)
    ...
    def play_explosion(self, x: float, screen_width: float):
        ...
        """
        if not self.enabled or self.explosion_sound is None:
            return

        try:
            # Calculate pan position: -1 (left) to 1 (right)
            # Normalize x to 0-1 range, then scale to -1 to 1
            pan = (x / screen_width) * 2 - 1
            # Clamp to valid range
            pan = max(-1, min(1, pan))
            
            # Create stereo wave with panning
            # Pan = -1: full left (left=1.0, right=0.0)
            # Pan = 0: center (left=0.707, right=0.707) - equal power panning
            # Pan = 1: full right (left=0.0, right=1.0)
            
            # Calculate left and right volumes using equal power panning
            # This maintains perceived loudness better than linear panning
            angle = (pan + 1) * 0.25 * math.pi  # Map -1..1 to 0..pi/2
            left_vol = math.cos(angle)
            right_vol = math.sin(angle)
            
            # Apply volume to each channel
            left_channel = (self.explosion_sound * left_vol).astype(np.int16)
            right_channel = (self.explosion_sound * right_vol).astype(np.int16)
