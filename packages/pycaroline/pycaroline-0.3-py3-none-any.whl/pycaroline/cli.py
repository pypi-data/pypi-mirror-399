...
# Exit code for file loading errors
EXIT_FILE_ERROR = 4


@cli.command("compare-files")
@click.option(
    "--source",
    "-s",
    required=True,
...
@click.option(
    ...
    "-j",
    required=True,
    help="Comma-separated list of columns for matching rows.",
)
@click.option(
    "--output",
    "-o",
    type=click.Path(),
    default="./file_comparison_results",
    help="Output directory for reports.",
)
@click.option(
    "--source-format",
    default=None,
    help="Source file format override (parquet, csv, xlsx, json, ndjson, avro, ipc).",
)
@click.option(
    "--target-format",
    default=None,
    help="Target file format override (parquet, csv, xlsx, json, ndjson, avro, ipc).",
)
@click.option(
    "--source-options",
    default=None,
    help='Source file options as JSON string (e.g., \'{"delimiter": ";"}\')',
)
@click.option(
    "--target-options",
    default=None,
    help='Target file options as JSON string (e.g., \'{"sheet_name": "Data"}\')',
)
@click.option(
    "--tolerance",
    type=float,
...
def compare_files_cmd(
    ...
    Compares source and target files and generates comparison reports.
    Supports multiple file formats including CSV, Parquet, Excel, JSON,
    NDJSON, Avro, and IPC/Feather.

    Format-specific options can be passed via --source-options and
    --target-options as JSON strings.

    Example:
        pycaroline compare-files \\
            --source data/source.csv \\
            --target data/target.parquet \\
            --join-columns id,date

        pycaroline compare-files \\
            -s source.csv -t target.xlsx \\
            -j customer_id \\
            --source-options '{"delimiter": ";"}' \\
            --target-options '{"sheet_name": "Data"}'
    """
    ...
    try:
        ...
        # Parse source options JSON
        src_opts: dict | None = None
        if source_options:
            try:
                src_opts = json.loads(source_options)
            except json.JSONDecodeError as e:
                raise ValueError(f"Invalid JSON in --source-options: {e}") from e

        # Parse target options JSON
        tgt_opts: dict | None = None
        if target_options:
            try:
                tgt_opts = json.loads(target_options)
            except json.JSONDecodeError as e:
                raise ValueError(f"Invalid JSON in --target-options: {e}") from e

        logger.info(f"Comparing files: {source} -> {target}")
        logger.info(f"Join columns: {join_cols}")
        ...
        report_gen.generate(result, source_name)

        # Print summary
        match_pct = result.matching_rows / max(result.source_row_count, 1) * 100

        click.echo("\n" + "=" * 60)
        click.echo("FILE COMPARISON RESULT")
        click.echo("=" * 60)
        click.echo(f"Source: {source} ({result.source_row_count} rows)")
        click.echo(f"Target: {target} ({result.target_row_count} rows)")
        click.echo(f"Match: {match_pct:.2f}% ({result.matching_rows} rows)")
        ...
        if match_pct == 100:
            ...
            click.echo("\n✗ Files have differences. Check reports for details.")
        ...
        logger.exception(f"Unexpected error: {e}")
        click.echo(f"Error: {e}", err=True)
        raise SystemExit(EXIT_VALIDATION_FAILED) from None
