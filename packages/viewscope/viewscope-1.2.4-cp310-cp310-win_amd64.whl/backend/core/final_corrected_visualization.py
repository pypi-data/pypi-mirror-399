...
class CorrectedVisualizationRenderer:
    """修正版可视化渲染器"""
    ...
    def improve_and_correct_ocr(self, text_regions):
        ...
        for region in text_regions:
            ...
            if (content == 'C' and
                ...
                # 扩展边界框以包含完整的温度显示
                extended_x1 = max(0, x1 - 40)  # 向左扩展以包含 "--°"
                bounds = (extended_x1, y1, x2, y2)

                print(f"温度符号修正: '{content}' -> '{corrected_content}' 位置: {bounds}")
    ...
    def filter_conflicting_detections(self, text_regions, bar_charts, circles, overlap_threshold=0.3):
        ...
        return text_regions, filtered_bar_charts, filtered_circles
...
def create_final_corrected_visualization():
    ...
    renderer = CorrectedVisualizationRenderer()
    ...
    # 过滤冲突检测 - OCR优先
    print("过滤冲突检测...")
    final_text_regions, final_bar_charts, final_circles = renderer.filter_conflicting_detections(
        corrected_text_regions, bar_charts, circles
    )
    ...
    for i, text_region in enumerate(final_text_regions):
        ...
        correction_applied = text_region.get('correction_applied', False)
        ...
        # 如果应用了修正，显示原始内容
        if correction_applied and original_content != content:
    ...
    # 保存最终结果
    output_path = "../../final_corrected_result.png"
    cv2.imwrite(output_path, result_image)

    print(f"\n最终修正版可视化结果已保存: {output_path}")

    # 显示修正详情
    corrected_items = [t for t in final_text_regions if t.get('correction_applied', False)]
    if corrected_items:
        print(f"\n应用的OCR修正:")
    ...
    print(f"检测规则: OCR文字识别优先，图形检测避开文字区域")
    print(f"特别修正: 单独的'C'字符识别为完整的'--°C'温度格式")
    print("=" * 60)

if __name__ == "__main__":
    create_final_corrected_visualization()