...
def test_chinese_text_rendering():
    ...
    def draw_text_on_image(img, text, pos, color):
        """使用PIL在图像上绘制中文文字"""
        try:
            # 转换为PIL图像
    ...
    for i, circle in enumerate(circles):
        ...
        result_image = draw_text_on_image(result_image, label_text, label_pos, color)
    ...
    for i, text_region in enumerate(text_regions):
        ...
        result_image = draw_text_on_image(result_image, content_label, content_pos, color)
    ...
    for i, (text, color) in enumerate(legend_items):
        y_pos = 25 + i * 18
        result_image = draw_text_on_image(result_image, text, (10, y_pos), color)

    # 保存结果
    output_path = "../../chinese_display_test_result.png"
    cv2.imwrite(output_path, result_image)

    print(f"中文显示测试结果已保存: {output_path}")
