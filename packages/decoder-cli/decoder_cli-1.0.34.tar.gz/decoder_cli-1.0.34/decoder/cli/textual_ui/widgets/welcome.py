...
class WelcomeBanner(Static):
    ...
    def refresh_content(self, config: DecoderConfig | None = None) -> None:
        if config:
            self.config = config
        self._initialize_static_line_suffixes()
    ...
    def _initialize_static_line_suffixes(self) -> None:
        self._static_line1_suffix = (
            f"{self.LOGO_TEXT_GAP}[b]Decoder v{__version__}[/]"
        )
    ...
    def _build_line(self, line_idx: int, color: str) -> str:
        patterns = [
            f"    [{color}]____  ______[/]{self._static_line1_suffix}",
            f"   [{color}]/ __ \\/ ____/[/]{self._static_line2_suffix}",
            f"  [{color}]/ / / / __/   [/]{self._static_line3_suffix}",
            f" [{color}]/ /_/ / /___   [/]",
            f"[{color}]/_____/_____/   [/]{self._static_line5_suffix}",
        ]
