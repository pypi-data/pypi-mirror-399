...
from decoder.cli.textual_ui.widgets.messages import BashOutputMessage, ErrorMessage
from decoder.core.config import SessionLoggingConfig, DecoderConfig


@pytest.fixture
def vibe_config(tmp_path: Path) -> DecoderConfig:
...
async def _wait_for_bash_output_message(
    vibe_app: DecoderApp, pilot, timeout: float = 1.0
) -> BashOutputMessage:
...
def assert_no_command_error(vibe_app: DecoderApp) -> None:
...
@pytest.mark.asyncio
async def test_ui_reports_no_output(vibe_app: DecoderApp) -> None:
...
@pytest.mark.asyncio
async def test_ui_shows_success_in_case_of_zero_code(vibe_app: DecoderApp) -> None:
    async with vibe_app.run_test() as pilot:
        chat_input = vibe_app.query_one(ChatInputContainer)
        chat_input.value = "!true"
...
@pytest.mark.asyncio
async def test_ui_shows_failure_in_case_of_non_zero_code(vibe_app: DecoderApp) -> None:
    async with vibe_app.run_test() as pilot:
        chat_input = vibe_app.query_one(ChatInputContainer)
...
@pytest.mark.asyncio
async def test_ui_handles_non_utf8_output(vibe_app: DecoderApp) -> None:
...
@pytest.mark.asyncio
async def test_ui_handles_utf8_output(vibe_app: DecoderApp) -> None:
    async with vibe_app.run_test() as pilot:
        chat_input = vibe_app.query_one(ChatInputContainer)
...
@pytest.mark.asyncio
async def test_ui_handles_non_utf8_stderr(vibe_app: DecoderApp) -> None:
    async with vibe_app.run_test() as pilot:
        chat_input = vibe_app.query_one(ChatInputContainer)
