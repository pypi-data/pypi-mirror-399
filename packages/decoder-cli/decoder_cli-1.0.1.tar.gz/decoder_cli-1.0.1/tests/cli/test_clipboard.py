...
from decoder.cli.clipboard import (
    _copy_osc52,
...
@patch("decoder.cli.clipboard._get_copy_fns")
...
def test_copy_selection_to_clipboard_multiple_widgets(mock_app: MagicMock) -> None:
    ...
    with patch("decoder.cli.clipboard._get_copy_fns") as mock_get_copy_fns:
        mock_copy_fn = MagicMock()
        mock_get_copy_fns.return_value = [mock_copy_fn]
        copy_selection_to_clipboard(mock_app)
...
@patch("decoder.cli.clipboard.subprocess.run")
...
@patch("decoder.cli.clipboard.shutil.which")
