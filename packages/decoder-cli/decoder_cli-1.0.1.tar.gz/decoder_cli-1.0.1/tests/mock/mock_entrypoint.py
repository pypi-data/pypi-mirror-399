...
Mocked returns are stored in the DECODER_MOCK_LLM_DATA environment variable.
"""
...
if __name__ == "__main__":
    ...
    from decoder.core.types import LLMChunk

    sys.path.insert(0, str(TESTS_ROOT))

    # Apply mocking before importing any decoder modules
    ...
    patch(
        "decoder.core.llm.backend.mistral.MistralBackend.complete",
        side_effect=mock_complete,
    ).start()
    patch(
        "decoder.core.llm.backend.generic.GenericBackend.complete",
        side_effect=mock_complete,
    ).start()
    patch(
        "decoder.core.llm.backend.mistral.MistralBackend.complete_streaming",
        side_effect=mock_complete_streaming,
    ).start()
    patch(
        "decoder.core.llm.backend.generic.GenericBackend.complete_streaming",
        side_effect=mock_complete_streaming,
    ).start()

    from decoder.acp.entrypoint import main
