...
from decoder.core.llm.exceptions import BackendErrorBuilder
...
from decoder.core.utils import async_generator_retry, async_retry

if TYPE_CHECKING:
    from decoder.core.config import ModelConfig, ProviderConfig
...
class OpenAIAdapter(APIAdapter):
    ...
    def prepare_request(
        ...
        *,
        ...
        if provider.name == "onlysq" and reasoning is not False:
            # Based on OnlySq docs:
            # Non-streaming: extra_body={"reasoning": "high"} or extra={"reasoning": "high"}
            # We'll use "extra" inside the main payload as it's common for raw API requests
            payload.setdefault("extra", {})["reasoning"] = reasoning_effort or "medium"
...
class GenericBackend:
    ...
    async def complete(
        ...
        *,
        ...
        url = f"{self._provider.api_base.rstrip('/')}{endpoint}"
    ...
    async def complete_streaming(
        ...
        *,
        ...
        try:
            ...
            from decoder.core.llm.exceptions import BackendError
            raise BackendError(
                message=str(e),
                provider=self._provider.name,
                model=model.name,
