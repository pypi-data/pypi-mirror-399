...
from decoder.core.tools.builtins.todo import TodoArgs
...
async def test_act_flushes_batched_messages_with_injection_middleware(
    ...
    assert observed[0][1] == "You are Decoder, a super useful programming assistant."
...
async def test_act_streams_batched_chunks_in_order() -> None:
    backend = FakeBackend([
        ...
        mock_llm_chunk(content=" Decoder"),
    ...
    assert [event.content for event in events if isinstance(event, AssistantEvent)] == [
        "Hello from Decoder! More",
        " and end",
    ]
    assert agent.messages[-1].role == Role.assistant
    assert agent.messages[-1].content == "Hello from Decoder! More and end"
