...
from decoder.cli.textual_ui.widgets.chat_input.completion_popup import CompletionPopup
from decoder.cli.textual_ui.widgets.chat_input.container import ChatInputContainer
from decoder.core.config import SessionLoggingConfig, DecoderConfig


@pytest.fixture
def vibe_config() -> DecoderConfig:
    return DecoderConfig(session_logging=SessionLoggingConfig(enabled=False))


@pytest.fixture
def vibe_app(vibe_config: DecoderConfig) -> DecoderApp:
    return DecoderApp(config=vibe_config)


@pytest.mark.asyncio
async def test_popup_appears_with_matching_suggestions(vibe_app: DecoderApp) -> None:
...
@pytest.mark.asyncio
async def test_popup_hides_when_input_cleared(vibe_app: DecoderApp) -> None:
    async with vibe_app.run_test() as pilot:
        popup = vibe_app.query_one(CompletionPopup)
...
@pytest.mark.asyncio
async def test_arrow_navigation_updates_selected_suggestion(vibe_app: DecoderApp) -> None:
    async with vibe_app.run_test() as pilot:
        popup = vibe_app.query_one(CompletionPopup)
...
@pytest.mark.asyncio
async def test_arrow_navigation_cycles_through_suggestions(vibe_app: DecoderApp) -> None:
    async with vibe_app.run_test() as pilot:
        popup = vibe_app.query_one(CompletionPopup)
...
def file_tree(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    (tmp_path / "decoder" / "acp" / "entrypoint.py").write_text("", encoding="utf-8")
    (tmp_path / "decoder" / "acp" / "agent.py").write_text("", encoding="utf-8")
...
async def test_path_completion_popup_lists_files_and_directories(
    vibe_app: DecoderApp, file_tree: Path
) -> None:
    async with vibe_app.run_test() as pilot:
        popup = vibe_app.query_one(CompletionPopup)
...
async def test_fuzzy_matches_subsequence_characters(
    file_tree: Path, vibe_app: DecoderApp
) -> None:
    async with vibe_app.run_test() as pilot:
        popup = vibe_app.query_one(CompletionPopup)
...
async def test_finds_files_recursively_by_filename(
    ...
    async with vibe_app.run_test() as pilot:
        ...
        popup_content = str(popup.render())
        assert "@decoder/acp/entrypoint.py" in popup_content
        assert popup.styles.display == "block"
