...
from decoder import __version__ as CORE_VERSION
from decoder.cli.clipboard import copy_selection_to_clipboard
from decoder.cli.commands import CommandRegistry
from decoder.cli.terminal_setup import setup_terminal
from decoder.cli.textual_ui.handlers.event_handler import EventHandler
from decoder.cli.textual_ui.terminal_theme import (
    TERMINAL_THEME_NAME,
    capture_terminal_theme,
)
from decoder.cli.textual_ui.widgets.approval_app import ApprovalApp
from decoder.cli.textual_ui.widgets.chat_input import ChatInputContainer
from decoder.cli.textual_ui.widgets.compact import CompactMessage
from decoder.cli.textual_ui.widgets.config_app import ConfigApp
from decoder.cli.textual_ui.widgets.context_progress import ContextProgress, TokenState
from decoder.cli.textual_ui.widgets.loading import LoadingWidget
from decoder.cli.textual_ui.widgets.messages import (
    AssistantMessage,
    BashOutputMessage,
    ErrorMessage,
    InterruptMessage,
...
from decoder.cli.textual_ui.widgets.mode_indicator import ModeIndicator
from decoder.cli.textual_ui.widgets.path_display import PathDisplay
from decoder.cli.textual_ui.widgets.tools import ToolCallMessage, ToolResultMessage
from decoder.cli.textual_ui.widgets.welcome import WelcomeBanner
from decoder.cli.textual_ui.screens import SplashScreen
...
from decoder.core.autocompletion.path_prompt_adapter import render_path_prompt
from decoder.core.config import DecoderConfig
from decoder.core.modes import AgentMode, next_mode
from decoder.core.paths.config_paths import HISTORY_FILE
from decoder.core.tools.base import BaseToolConfig, ToolPermission
from decoder.core.types import ApprovalResponse, LLMMessage, Role
from decoder.core.utils import (
    CancellationReason,
    get_user_cancellation_message,
    is_dangerous_directory,
    logger,
)
from decoder.core.usage_stats import usage_stats_manager
...
class DecoderApp(App):  # noqa: PLR0904
    ...
    async def on_mount(self) -> None:
        ...
        if self.config.auto_compact_threshold > 0:
            self._context_progress.tokens = TokenState(
                max_tokens=self.config.auto_compact_threshold, current_tokens=0, session_tokens=0
            )
    ...
    def _set_tool_permission_always(
        ...
        if save_permanently:
            DecoderConfig.save_updates({"tools": {tool_name: {"permission": "always"}}})
    ...
    def _save_config_changes(self, changes: dict[str, str]) -> None:
        ...
        for key, value in changes.items():
            match key:
                ...
                case "reasoning":
                    new_val = value == "on"
                    if new_val != self.config.reasoning:
                        updates["reasoning"] = new_val
                case "reasoning_effort":
                    if value != self.config.reasoning_effort:
                        updates["reasoning_effort"] = value

        if updates:
            DecoderConfig.save_updates(updates)
            ...
            async for event in self.agent.act(rendered_prompt):
                if self._context_progress and self.agent:
                    ...
                    self._context_progress.tokens = TokenState(
                        ...
                        session_tokens=self.agent.stats.session_total_llm_tokens,
                    )
...
"""
    ...
    async def _show_stats(self) -> None:
        ...
        stats_report = usage_stats_manager.format_stats_report()
        await self._mount_and_scroll(UserCommandMessage(stats_report))
    ...
    async def _reload_config(self) -> None:
        try:
            new_config = DecoderConfig.load(**self._current_agent_mode.config_overrides)
            ...
            if self._context_progress:
                if self.config.auto_compact_threshold > 0:
                    ...
                    session_tokens = (
                        self.agent.stats.session_total_llm_tokens if self.agent else 0
                    )
                    self._context_progress.tokens = TokenState(
            ...
            try:
                welcome_banner = self.query_one(WelcomeBanner)
                welcome_banner.refresh_content(new_config)
            except Exception:
                pass
    ...
    async def _run_compact(self, compact_msg: CompactMessage, old_tokens: int) -> None:
        ...
        try:
            ...
            if self._context_progress:
                current_state = self._context_progress.tokens
                session_tokens = self.agent.stats.session_total_llm_tokens if self.agent else 0
                self._context_progress.tokens = TokenState(
                    max_tokens=current_state.max_tokens, current_tokens=new_tokens, session_tokens=session_tokens
                )
        except asyncio.CancelledError:
...
def _print_session_resume_message(session_id: str | None) -> None:
    ...
    print()
    print("To continue this session, run: decoder --continue")
    print(f"Or: decoder --resume {session_id}")
...
def run_textual_ui(
    ...
    update_notifier = PyPIVersionUpdateGateway(project_name="decoder")
