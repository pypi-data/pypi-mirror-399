...
from decoder.core import run_programmatic
from decoder.core.config import Backend, SessionLoggingConfig, DecoderConfig
from decoder.core.types import LLMMessage, OutputFormat, Role
...
def test_run_programmatic_preload_streaming_is_batched(
    ...
    monkeypatch.setattr(
        "decoder.core.programmatic.create_formatter", lambda *_args, **_kwargs: spy
    )
    ...
    with mock_backend_factory(
        ...
        assert (
            spy.emitted[0][1] == "You are Decoder, a super useful programming assistant."
        )
