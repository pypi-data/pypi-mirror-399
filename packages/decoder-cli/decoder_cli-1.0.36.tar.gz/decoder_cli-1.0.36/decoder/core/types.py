...
class ToolCall(BaseModel):
    ...
    type: str = "function"
    extra_content: dict[str, Any] | None = None  # OnlySq requires thought_signature
...
class LLMUsage(BaseModel):
    ...
    @model_validator(mode="before")
    @classmethod
    def _validate_usage(cls, v: Any) -> Any:
        if isinstance(v, dict):
            # API might return tokens as strings or weird values
            for key in ["prompt_tokens", "completion_tokens"]:
                val = v.get(key)
                if val is not None and not isinstance(val, int):
                    try:
                        # Try parsing as int, but if it's e.g. "T" it will fail
                        v[key] = int(str(val))
