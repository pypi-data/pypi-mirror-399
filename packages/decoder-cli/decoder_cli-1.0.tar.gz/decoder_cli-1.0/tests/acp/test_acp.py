...
from decoder.acp.utils import ToolOption
from decoder.core.types import FunctionCall, ToolCall
...
def _create_vibe_home_dir(tmp_path: Path, *sections: dict[str, Any]) -> Path:
    """Create a temporary decoder home directory with a minimal config file."""
    vibe_home = tmp_path / ".decoder"
    vibe_home.mkdir()
    ...
    if base_config_dict.get("models"):
        for model in base_config_dict["models"]:
            if model.get("name") == "decoder-cli-latest":
...
def vibe_home_grep_ask(tmp_path: Path) -> Path:
    """Create a temporary decoder home directory with grep configured to ask permission."""
...
async def get_acp_agent_process(
    ...
    env["DECODER_HOME"] = str(vibe_home)
