...
from decoder.core.types import LLMChunk, LLMMessage, LLMUsage, Role
...
def acp_agent(backend: FakeBackend) -> DecoderAcpAgent:
    ...
    def _create_agent(connection: AgentSideConnection) -> DecoderAcpAgent:
...
class TestACPSetMode:
    @pytest.mark.asyncio
    async def test_set_mode_to_default(self, acp_agent: DecoderAcpAgent) -> None:
    ...
    @pytest.mark.asyncio
    async def test_set_mode_to_auto_approve(self, acp_agent: DecoderAcpAgent) -> None:
        session_response = await acp_agent.newSession(
            NewSessionRequest(cwd=str(Path.cwd()), mcpServers=[])
        )
        session_id = session_response.sessionId
    ...
    @pytest.mark.asyncio
    async def test_set_mode_to_plan(self, acp_agent: DecoderAcpAgent) -> None:
        session_response = await acp_agent.newSession(
            NewSessionRequest(cwd=str(Path.cwd()), mcpServers=[])
        )
        session_id = session_response.sessionId
    ...
    @pytest.mark.asyncio
    async def test_set_mode_to_accept_edits(self, acp_agent: DecoderAcpAgent) -> None:
        session_response = await acp_agent.newSession(
            NewSessionRequest(cwd=str(Path.cwd()), mcpServers=[])
        )
        session_id = session_response.sessionId
    ...
    @pytest.mark.asyncio
    async def test_set_mode_to_same_mode(self, acp_agent: DecoderAcpAgent) -> None:
        session_response = await acp_agent.newSession(
            NewSessionRequest(cwd=str(Path.cwd()), mcpServers=[])
        )
        session_id = session_response.sessionId
    ...
    @pytest.mark.asyncio
    async def test_set_mode_with_empty_string(self, acp_agent: DecoderAcpAgent) -> None:
        session_response = await acp_agent.newSession(
            NewSessionRequest(cwd=str(Path.cwd()), mcpServers=[])
        )
        session_id = session_response.sessionId
