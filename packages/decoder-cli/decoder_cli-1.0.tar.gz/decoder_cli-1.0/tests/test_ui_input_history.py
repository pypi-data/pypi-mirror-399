...
from decoder.cli.history_manager import HistoryManager
...
@pytest.fixture
def vibe_app(vibe_config: DecoderConfig, tmp_path: Path) -> DecoderApp:
...
def inject_history_file(vibe_app: DecoderApp, history_file: Path) -> None:
...
async def test_ui_navigation_through_input_history(
    vibe_app: DecoderApp, history_file: Path
) -> None:
...
async def test_ui_resumes_arrow_down_after_manual_move(
    vibe_app: DecoderApp, tmp_path: Path
) -> None:
