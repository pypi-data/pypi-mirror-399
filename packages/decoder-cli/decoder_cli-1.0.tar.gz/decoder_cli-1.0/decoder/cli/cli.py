...
from decoder.cli.textual_ui.app import run_textual_ui
from decoder.core.config import (
    MissingAPIKeyError,
...
from decoder.core.paths.config_paths import CONFIG_FILE, HISTORY_FILE, INSTRUCTIONS_FILE
from decoder.core.programmatic import run_programmatic
from decoder.core.types import LLMMessage, OutputFormat
from decoder.core.utils import ConversationLimitException
...
def load_config_or_exit(
    ...
    try:
        return DecoderConfig.load(agent, **mode.config_overrides)
...
def bootstrap_config_files() -> None:
    if not CONFIG_FILE.path.exists():
        try:
            DecoderConfig.save_updates(DecoderConfig.create_default())
    ...
    if not HISTORY_FILE.path.exists():
        try:
            ...
            HISTORY_FILE.path.write_text("Hello Decoder!\n", "utf-8")
        except Exception as e:
            rprint(f"[yellow]Could not create history file: {e}[/]")


def load_session(
    args: argparse.Namespace, config: DecoderConfig
