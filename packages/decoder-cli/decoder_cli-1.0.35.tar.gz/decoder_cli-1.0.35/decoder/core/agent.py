...
from decoder.core.interaction_logger import InteractionLogger
from decoder.core.llm.backend.factory import BACKEND_FACTORY
from decoder.core.llm.format import APIToolFormatHandler, ResolvedMessage
from decoder.core.llm.types import BackendLike
from decoder.core.middleware import (
...
from decoder.core.modes import AgentMode
from decoder.core.prompts import UtilityPrompt
from decoder.core.skills.manager import SkillManager
from decoder.core.system_prompt import get_universal_system_prompt
from decoder.core.tools.base import (
    BaseTool,
    ToolError,
    ToolPermission,
    ToolPermissionError,
)
from decoder.core.tools.manager import ToolManager
from decoder.core.types import (
    AgentStats,
    ApprovalCallback,
    ApprovalResponse,
    AssistantEvent,
...
class Agent:
        ...
        match result.action:
            case MiddlewareAction.STOP:
                yield AssistantEvent(
                    content=f"<{DECODER_STOP_EVENT_TAG}>{result.reason}</{DECODER_STOP_EVENT_TAG}>",
    ...
    async def _perform_llm_turn(self) -> AsyncGenerator[BaseEvent, None]:
        ...
        # If provider doesn't support streaming with tools, check if tools are available
        if use_streaming:
            try:
                active_model = self.config.get_active_model()
                provider = self.config.get_provider_for_model(active_model)
                has_tools = active_model.supports_tools and bool(self.format_handler.get_available_tools(
                    self.tool_manager, self.config
                ))
                # OnlySq doesn't support streaming with tools (returns empty response)
                is_onlysq = provider.name.lower() == "onlysq"
                if has_tools and (not provider.supports_streaming_with_tools or is_onlysq):
                    use_streaming = False
            except ValueError:
                pass
        ...
        if use_streaming:
            ...
            if llm_result.message.reasoning_content:
                yield ReasoningEvent(content=llm_result.message.reasoning_content)
            if llm_result.message.content:
                yield AssistantEvent(content=llm_result.message.content)
    ...
    async def _chat(self, max_tokens: int | None = None) -> LLMChunk:
        ...
        try:
            ...
            # Some APIs don't provide usage data - use defaults if missing
            if result.usage is None:
                result = LLMChunk(message=result.message, usage=LLMUsage())
    ...
    async def _chat_streaming(
        ...
        try:
            ...
            # Some APIs don't provide usage data in streaming - use defaults if missing
            if chunk_agg.usage is None:
                chunk_agg = LLMChunk(message=chunk_agg.message, usage=LLMUsage())
            self._update_stats(usage=usage or LLMUsage(), time_seconds=end_time - start_time)
    ...
    def _update_stats(self, usage: LLMUsage, time_seconds: float) -> None:
        ...
        # Record to persistent usage stats
        try:
            model_name = self.config.get_active_model().name
            usage_stats_manager.record_request(
