...
from decoder.core.paths.config_paths import AGENT_DIR, CONFIG_DIR, CONFIG_FILE, PROMPT_DIR
from decoder.core.paths.global_paths import GLOBAL_ENV_FILE, SESSION_LOG_DIR
from decoder.core.prompts import SystemPrompt
from decoder.core.tools.base import BaseToolConfig

PROJECT_DOC_FILENAMES = ["AGENTS.md", "DECODER.md", ".decoder.md"]
...
def load_api_keys_from_env() -> None:
    ...
    # Default OnlySq key if not provided
    if not os.getenv("ONLYSQ_API_KEY"):
        os.environ.setdefault("ONLYSQ_API_KEY", "openai")
...
class ProviderConfig(BaseModel):
    ...
    supports_streaming_with_tools: bool = True
...
DEFAULT_PROVIDERS = [
    ProviderConfig(
        name="onlysq",
        api_base="https://api.onlysq.ru/ai/openai/",
        api_key_env_var="ONLYSQ_API_KEY",
        backend=Backend.GENERIC,
        supports_streaming_with_tools=False,  # OnlySq returns empty when streaming + tools
    ),
]
...
class DecoderConfig(BaseSettings):
    active_model: str = "gemini-3-flash"
    ...
    model_config = SettingsConfigDict(
        env_prefix="DECODER_", case_sensitive=False, extra="ignore"
    )
    ...
    def settings_customise_sources(
        ...
        into os.environ for use by providers. Only DECODER_* prefixed environment
    ...
    @model_validator(mode="after")
    def _check_api_key(self) -> DecoderConfig:
        try:
    ...
    @model_validator(mode="after")
    def _validate_model_uniqueness(self) -> DecoderConfig:
    ...
    @model_validator(mode="after")
    def _check_system_prompt(self) -> DecoderConfig:
    ...
    @classmethod
    def load(cls, agent: str | None = None, **overrides: Any) -> DecoderConfig:
        cls._migrate()
        agent_config = cls._get_agent_config(agent)
        init_data = {**(agent_config or {}), **overrides}
        config = cls(**init_data)

        # Merge or update models to ensure critical flags like supports_tools are synced
        for default_model in DEFAULT_MODELS:
            found = False
            for i, existing in enumerate(config.models):
                if existing.name == default_model.name:
                    # Sync critical capability flags
                    existing.supports_tools = default_model.supports_tools
                    found = True
                    break
            if not found:
                config.models.append(default_model)

        # Sync providers
        for default_provider in DEFAULT_PROVIDERS:
            for existing in config.providers:
                if existing.name == default_provider.name:
                    existing.api_base = default_provider.api_base
                    break
