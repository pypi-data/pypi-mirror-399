...
async def _wait_for_notification(
    app: DecoderApp, pilot, *, timeout: float = 1.0, interval: float = 0.05
) -> Notification:
...
@pytest.fixture
def vibe_config_with_update_checks_enabled() -> DecoderConfig:
    return DecoderConfig(
        session_logging=SessionLoggingConfig(enabled=False), enable_update_checks=True
    )


class DecoderAppFactory(Protocol):
    def __call__(
        self,
        *,
...
@pytest.fixture
def make_vibe_app(vibe_config_with_update_checks_enabled: DecoderConfig) -> DecoderAppFactory:
...
@pytest.mark.asyncio
async def test_ui_displays_update_notification(make_vibe_app: DecoderAppFactory) -> None:
...
async def test_ui_does_not_display_update_notification_when_not_available(
    make_vibe_app: DecoderAppFactory,
) -> None:
...
async def test_ui_does_not_invoke_gateway_nor_show_error_notification_when_update_checks_are_disabled(
    vibe_config_with_update_checks_enabled: DecoderConfig, make_vibe_app: DecoderAppFactory
) -> None:
