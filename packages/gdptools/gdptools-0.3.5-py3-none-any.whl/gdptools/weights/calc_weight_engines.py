...
class SerialWghtGenEngine(CalcWeightEngine):
    ...
    def area_tables_binning_and_intersections(
        ...
        if source_type == "grid":
            ...
            return (
                target_df[self.target_poly_idx].iloc[ids_tgt].values.astype(object).tolist(),
                source_df[source_idx_col].iloc[ids_src].values.astype(object).tolist(),
                weights.astype(float).tolist(),
                gdf_inter,
            )
...
def _area_tables_binning_parallel_and_intersections(
    ...
    geometries and their corresponding area weights using spatial partitioning.
    Target polygons are partitioned using Hilbert curve ordering, and only
    spatially-relevant source polygons are sent to each worker.
    ...
    # Collect spatial chunks as a list for joblib
    spatial_chunks = list(_spatial_chunks_for_parallel(source_df, target_df, n_jobs))

    if not spatial_chunks:
        # No intersections found
        empty_gdf = gpd.GeoDataFrame(geometry=[], crs=target_df.crs)
        empty_gdf["weights"] = []
        if source_type == "grid":
            return ([], [], [], [], empty_gdf)
        else:
    ...
    # Process each spatial chunk in parallel
    with parallel_backend("loky", inner_max_num_threads=1):
        worker_results = Parallel(n_jobs=n_jobs)(
            delayed(_process_spatial_chunk_with_intersections)(
                source_chunk, target_chunk, source_poly_idx, target_poly_idx, source_type
            )
            for source_chunk, target_chunk in spatial_chunks
        )
    ...
    if source_type == "grid":
        ...
        for result in worker_results:
            if result is not None:
                all_target_ids.extend(result[0])
                all_i_indices.extend(result[1])
                all_j_indices.extend(result[2])
                all_weights.extend(result[3])
                all_intersections.append(result[4])
        inter_sect = pd.concat(all_intersections, ignore_index=True) if all_intersections else gpd.GeoDataFrame()
        return (all_target_ids, all_i_indices, all_j_indices, all_weights, inter_sect)
    else:
        all_target_ids = []
        all_source_ids = []
        all_weights = []
        ...
        for result in worker_results:
            if result is not None:
                all_target_ids.extend(result[0])
                all_source_ids.extend(result[1])
                all_weights.extend(result[2])
                all_intersections.append(result[3])
        inter_sect = pd.concat(all_intersections, ignore_index=True) if all_intersections else gpd.GeoDataFrame()
        return (all_target_ids, all_source_ids, all_weights, inter_sect)


def _process_spatial_chunk_with_intersections(
    source_chunk: gpd.GeoDataFrame,
    target_chunk: gpd.GeoDataFrame,
    source_poly_idx: str,
    target_poly_idx: str,
    source_type: SOURCE_TYPES,
) -> tuple[list, ...] | None:
    """Process a single spatial chunk to compute intersection weights and geometries.

    This worker function computes area-weighted intersections and the actual
    intersection geometries between a subset of source and target polygons.

    Args:
        source_chunk: Subset of source polygons for this spatial partition.
        target_chunk: Subset of target polygons for this spatial partition.
        source_poly_idx: Column name for source polygon IDs.
        target_poly_idx: Column name for target polygon IDs.
        source_type: Type of source geometry ('grid' or 'poly').

    Returns:
        Tuple of weight components and intersection GeoDataFrame, or None if no intersections.

    """
    if len(source_chunk) == 0 or len(target_chunk) == 0:
        return None

    # Spatial index query to find intersecting pairs
    ids_tgt, ids_src = source_chunk.sindex.query(target_chunk.geometry, predicate="intersects")

    if len(ids_src) == 0:
        return None

    source_geoms = source_chunk.geometry.values[ids_src]
    target_geoms = target_chunk.geometry.values[ids_tgt]
    ...
    # Build intersection GeoDataFrame
    gdf_inter = target_chunk.iloc[ids_tgt].copy()
    gdf_inter = gdf_inter.set_geometry(f_intersect)
    gdf_inter["weights"] = weights.astype(float)

    if source_type == "grid":
        return (
            target_chunk[target_poly_idx].iloc[ids_tgt].values.astype(object).tolist(),
            source_chunk.i_index.iloc[ids_src].values.astype(int).tolist(),
            source_chunk.j_index.iloc[ids_src].values.astype(int).tolist(),
            weights.astype(float).tolist(),
            gdf_inter,
        )
    else:
        source_idx_col = source_poly_idx[0] if isinstance(source_poly_idx, list) else source_poly_idx
        return (
            target_chunk[target_poly_idx].iloc[ids_tgt].values.astype(object).tolist(),
            source_chunk[source_idx_col].iloc[ids_src].values.astype(object).tolist(),
            weights.astype(float).tolist(),
            gdf_inter,
        )
...
def _spatial_chunks_for_parallel(
    source_df: gpd.GeoDataFrame,
    target_df: gpd.GeoDataFrame,
    n_jobs: int,
) -> Generator[tuple[gpd.GeoDataFrame, gpd.GeoDataFrame], Any, Any]:
    """Generate spatially partitioned chunks for parallel processing.

    This function spatially partitions the target GeoDataFrame using Hilbert
    curve ordering, then for each target partition extracts only the source
    geometries that could possibly intersect (based on bounding box).

    This approach reduces memory per worker and improves cache locality
    compared to passing the entire source dataset to each worker.

    Args:
        source_df: GeoDataFrame of source polygons (grid cells or polygons).
        target_df: GeoDataFrame of target polygons.
        n_jobs: Number of spatial partitions to create.

    Yields:
        Tuples of (source_subset, target_chunk) where source_subset contains
        only geometries that could intersect target_chunk.

    """
    # Create spatially partitioned target using Hilbert curve
    target_dask = dgpd.from_geopandas(target_df, npartitions=n_jobs)
    target_dask = target_dask.spatial_shuffle(by="hilbert", npartitions=n_jobs)

    # Build spatial index on source for fast bounding box queries
    source_sindex = source_df.sindex

    for part in target_dask.partitions:
        target_chunk = part.compute()
        if len(target_chunk) == 0:
            continue

        # Get bounding box of target chunk
        bnds = target_chunk.total_bounds  # (minx, miny, maxx, maxy)

        # Query source spatial index for geometries intersecting the bounding box
        candidate_idx = list(source_sindex.intersection(bnds))

        if len(candidate_idx) == 0:
            continue

        source_subset = source_df.iloc[candidate_idx]
...
def _area_tables_binning_parallel(
    ...
    This function orchestrates the parallel calculation of area weights using
    spatial partitioning. Target polygons are partitioned using Hilbert curve
    ordering, and only spatially-relevant source polygons are sent to each worker.
    ...
    with parallel_backend("loky", inner_max_num_threads=1):
        worker_results = Parallel(n_jobs=n_jobs)(
            delayed(_process_spatial_chunk)(
                source_chunk, target_chunk, source_poly_idx, target_poly_idx, source_type
            )
            for source_chunk, target_chunk in spatial_chunks
        )
    ...
    if source_type == "grid":
        ...
        for result in worker_results:
            if result is not None:
                ...
                all_j_indices.extend(result[2])
                all_weights.extend(result[3])
        return (all_target_ids, all_i_indices, all_j_indices, all_weights)
    else:
        all_target_ids = []
        all_source_ids = []
        all_weights = []
        for result in worker_results:
            if result is not None:
                ...
                all_source_ids.extend(result[1])
                all_weights.extend(result[2])
        return (all_target_ids, all_source_ids, all_weights)
...
def _process_spatial_chunk(
    ...
    """Process a single spatial chunk to compute intersection weights.

    This worker function computes area-weighted intersections between a subset
    of source and target polygons. It uses the same optimized algorithm as
    the serial engine (skip intersection for fully contained polygons).
    ...
    Returns:
        Tuple of weight components, or None if no intersections found.
