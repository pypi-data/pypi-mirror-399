...
class SerialAgg(AggEngine):
    ...
    def calc_agg(
        ...
        # Pre-compute weight chunks (same as ParallelAgg) - avoids repeated
        # groupby lookups and type conversions inside the loop
        ...
        # Process each chunk - no exception handling or string conversion needed
        for chunk in chunks:
            val_interp[:, chunk.index] = self.stat(
                array=chunk.ma, weights=chunk.wghts, def_val=chunk.def_val
            ).get_stat()
