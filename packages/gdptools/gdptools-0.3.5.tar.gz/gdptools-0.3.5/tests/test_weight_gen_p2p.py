...
class TestWeightGenP2P:
    ...
    def test_serial_weight_calculation_with_intersections(
        self,
        source_polygons: gpd.GeoDataFrame,
        target_polygons: gpd.GeoDataFrame
    ) -> None:
        """Test that intersections=True populates intersection geometries."""
        weight_gen = WeightGenP2P(
            ...
            target_poly_idx='target_id',
            method='serial',
            weight_gen_crs='EPSG:6931',
            intersections=True,
        )

        weights = weight_gen.calculate_weights()

        # Weights should be a DataFrame
        assert isinstance(weights, pd.DataFrame), "Weights should be a DataFrame"
        assert len(weights) > 0, "Weights DataFrame should not be empty"

        # Intersections should be accessible via property
        intersections_gdf = weight_gen.intersections
        assert isinstance(intersections_gdf, gpd.GeoDataFrame), "Intersections should be a GeoDataFrame"
        assert len(intersections_gdf) > 0, "Intersections GeoDataFrame should not be empty"

        # Verify intersection geometries are valid
        assert intersections_gdf.geometry.is_valid.all(), "All intersection geometries should be valid"
        assert 'weights' in intersections_gdf.columns, "Intersections GDF should have 'weights' column"
