...
class TagCell(Section, Cell):
    ...
    def exit(self):
        ...
        # Check if this cell has an image_ref set by {% img %} tag
        if hasattr(self, 'image_ref') and self.image_ref and not getattr(self, '_image_ref_processed', False):
            self.set_image_ref(self.image_ref)
...
class XvCell(Cell):
    ...
    def exit(self):
        ...
        # Check if this cell has an image_ref set by img filter
        if hasattr(self, 'image_ref') and self.image_ref and not getattr(self, '_image_ref_processed', False):
            self.set_image_ref(self.image_ref)
