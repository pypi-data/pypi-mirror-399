...
from .filters import img_filter, sha256_filter, mtime_filter, to_fullwidth_filter
...
class JinjaEnvx(Env):
    ...
    def __init__(self, node_map):
        def finalize_with_image_marker(value):
            """Finalize hook to intercept ImageMarker objects and store them"""
            from .filters import ImageMarker
            if isinstance(value, ImageMarker):
                # Store the ImageMarker for later processing
                if not hasattr(self, '_pending_image_marker'):
                    self._pending_image_marker = None
                self._pending_image_marker = value
                # Return empty string for the cell value
                return ''
            return value if value is not None else ''
        
        Env.__init__(self, extensions=[NodeExtension, SegmentExtension, YnxExtension,
                                       XvExtension, ImagexExtension, OpExtension],
                     finalize=finalize_with_image_marker)
        self.node_map = node_map
        self.offset = 0
        # Register filters
        self.filters['img'] = img_filter
        self.filters['sha256'] = sha256_filter
        self.filters['mtime'] = mtime_filter
        self.filters['to_fullwidth'] = to_fullwidth_filter
        # Storage for pending image refs from filters (for {% xv %} compatibility)
