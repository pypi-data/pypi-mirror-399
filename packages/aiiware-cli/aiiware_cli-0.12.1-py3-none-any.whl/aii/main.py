...
```"""
        ...
        # RuntimeError from client contains user-friendly error message
        error_msg = str(e)
        # Check if it's an LLM configuration error
        if "LLM provider" in error_msg or "API key" in error_msg:
            print(f"\n❌ {error_msg}")
            print("\n💡 To fix this, run: aii config init")
        else:
            print(f"\n❌ Git commit failed: {error_msg}")
        return 1
...
async def _handle_git_pr_local(config_manager: Any, output_config: Any, extra_args: list) -> int:
    ...
    try:
        ...
        # Truncate diff if too long and sanitize for LLM
        # v0.12.0: Reduce max chars to avoid intent recognition JSON parsing issues
        # Large diffs with special characters can cause the server's JSON parser to fail
        max_diff_chars = 3000
        ...
        # Sanitize diff to avoid JSON parsing issues in intent recognition
        # Replace problematic escape sequences that can break JSON parsing
        truncated_diff = truncated_diff.replace('\x00', '')  # Remove null bytes
        # Remove any stray backslashes before spaces (invalid JSON escape)
        truncated_diff = truncated_diff.replace('\\ ', ' ')
