...
class LinearRegression:
    ...
    def loss_history_plot(self):
        """
        Plot the training loss history for the Linear Regression model.

        Uses the values accumulated in `self.loss_history` (a Python list)
        during `fit()` and renders a line chart of MSE vs. epoch.
        """
        if not self.trained:
