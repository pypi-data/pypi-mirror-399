...
class LinearRegression:
    ...
    def residuals_plot(self):
        """
        The scatter residuals_plot function for Linear Regression
    ...
    def predictions_vs_actuals_plot(self):
        """
        The scatter plot predictions_vs_actuals_plot function for Linear Regression

        Plots the predicted values vs actual values of the trained model
        """
        if not self.trained:
        ...
        plt.figure(figsize=(8, 6))
        sns.scatterplot(x=self.y_train.numpy(), y=train_predictions)
        plt.plot([self.y_train.min(), self.y_train.max()], [self.y_train.min(), self.y_train.max()], 'r--')
