...
class AigieCallbackHandler(BaseCallbackHandler):
    ...
    def on_chain_start(
        ...
        **kwargs: Any,
        ...
        parent_span_id = None
        if parent_run_id:
            if parent_run_id in self._span_contexts:
                ...
                logger.debug(f"Parent run_id {parent_run_id[:8]} not in span_contexts for {span_name}")
        else:
            logger.debug(f"No parent_run_id for {span_name} (root span)")
