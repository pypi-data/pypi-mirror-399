...
from .OVFM.source.em.online_expectation_maximization import OnlineExpectationMaximization
from .OVFM.source.onlinelearning.ftrl_adp import FTRL_ADP
from .OVFM.source.onlinelearning.ensemble import sigmoid
...
class OVFM:
    ...
    def partial_fit(self, X, Y):
        ...
        if self.decay_coef_change:
            this_decay_coef = self.batch_c / (self.j/(self.batch_size_denominator*2) + self.batch_c) #type: ignore
        else:
            this_decay_coef = 0.5
        
        self.j += 1
        
        z_i, _ = self.oem.partial_fit_and_predict(X, max_workers=1,decay_coef=this_decay_coef) #type: ignore
        ...
        if denominator == 0:
            ...
            self.lamda=(np.exp(-self.eta*self.x_loss)/denominator).item()
