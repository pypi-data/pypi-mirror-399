...
class FunctionTracer:
    ...
    def _identify_stdlib_paths(self) -> None:
        ...
        for path in paths_to_add:
            if path:
                ...
                self.stdlib_paths.add(normalized)
    ...
    def _should_trace(self, frame: Any) -> bool:
        ...
        # Skip common internal function names for cleaner output
        skip_names = {'<module>', '<listcomp>', '<dictcomp>', '<setcomp>', '<genexpr>', '<lambda>'}
        ...
        if self.traced_file:
            ...
            if self.traced_file_dir and filename_normalized.startswith(self.traced_file_dir):
                # Don't trace files from site-packages even if in same dir
                if 'site-packages' in filename_normalized:
                    return False
                return True
            return False
