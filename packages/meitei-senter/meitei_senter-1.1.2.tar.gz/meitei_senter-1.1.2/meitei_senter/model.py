...
class MeiteiSentenceSplitter:
    ...
    def tokenize(self, text: str) -> Tuple[List[str], List[int]]:
        ...
        raw_pieces = self.sp.encode_as_pieces(text)
        raw_ids = self.sp.encode_as_ids(text)
        
        # Pre-process: merge standalone whitespace markers with following token
        pieces = []
        ids = []
        i = 0
        while i < len(raw_pieces):
            ...
            # Check if this is a standalone whitespace marker
            if piece == self.WHITESPACE_MARKER:
                # Merge with next token if exists
                if i + 1 < len(raw_pieces):
                    next_piece = raw_pieces[i + 1]
                    next_id = raw_ids[i + 1]
                    # Add marker to next piece if it doesn't already have one
                    if not next_piece.startswith(self.WHITESPACE_MARKER):
                        pieces.append(self.WHITESPACE_MARKER + next_piece)
                    else:
                        pieces.append(next_piece)
                    ids.append(next_id)  # Use the next token's ID
                    i += 2  # Skip next piece since we merged it
                    continue
            pieces.append(piece)
            ids.append(piece_id)
            i += 1
