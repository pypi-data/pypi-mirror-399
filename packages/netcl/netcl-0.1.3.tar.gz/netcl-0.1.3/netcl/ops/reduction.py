...
def _get_reduce_kernels(ctx: "cl.Context", dtype_c: str):
    ...
    kernels = (program.reduce_all, program.reduce_axis0, program.reduce_axis1)
...
def reduce_sum(x: Tensor, axis: Optional[int] = None, out: Optional[Tensor] = None, pool: Optional[BufferPool] = None) -> Tensor:
    ...
    if axis is None:
        ...
        kernel, _, _ = _get_reduce_kernels(ctx, dtype_c)
        if out is None:
    ...
    _, k_axis0, k_axis1 = _get_reduce_kernels(ctx, dtype_c)
    kernel = k_axis0 if axis == 0 else k_axis1
    if out is None:
