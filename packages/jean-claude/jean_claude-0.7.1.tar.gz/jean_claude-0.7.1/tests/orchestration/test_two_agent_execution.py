"""Tests for two_agent.py function calls.

This test verifies that two_agent.py calls the correct public function
...
def test_two_agent_calls_execute_prompt_async():
    """Verify two_agent.py calls execute_prompt_async, not execute_prompt_async."""
    ...
    assert not found_private_call, (
        f"two_agent.py should NOT call execute_prompt_async. "
        f"Found calls at: {', '.join(private_call_locations)}"
    )

    assert found_public_call, (
        "two_agent.py should call execute_prompt_async instead of execute_prompt_async"
    )
...
def test_two_agent_function_call_arguments():
