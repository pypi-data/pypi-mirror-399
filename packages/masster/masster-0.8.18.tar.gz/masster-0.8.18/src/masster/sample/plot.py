...
def plot_chrom(
    ...
    if feature_ids is None:
        ...
        feats = self.features_df[
            self.features_df["feature_id"].is_in(feature_ids)
        ].clone()

    # make sure feature_id is a list of integers

    chrom_plots = []
    feature_ids = feats["feature_id"].values.tolist()
    ...
    for feature_id in feature_ids:
        ...
        feature_row = feats[feats["feature_id"] == feature_id]
    ...
    try:
        ...
        if ipython is not None and "IPKernelApp" in ipython.config:
            ...
            if "bokeh" not in hv.Store.loaded_backends():
                hv.extension("bokeh")
            # Initialize panel for inline rendering in VS Code notebooks
            panel.extension(comms="default", inline=True)
    except (ImportError, AttributeError):
...
def plot_ms2_eic(
    ...
    **kwargs,
    ...
    for cycle in cycles:
        ...
        scan_uids.append(scan["scan_id"][0])
...
def plot_ms2_q1(
    ...
    **kwargs,
    ...
    scan_uid = scans[(scans["prec_mz"] - feature["mz"][0]).abs().arg_sort()[:1]][
        "scan_id"
    ][0]
    # get q1_width scans before and after the scan_uid
    scans = self.scans_df.filter(pl.col("scan_id") >= scan_uid - q1_width)
    scans = scans.filter(pl.col("scan_id") <= scan_uid + q1_width)
    scan_uids = scans["scan_id"].to_list()
...
def plot_ms2(
    ...
    Parameters:
        features: Feature selection using same format as features_select():
            ...
            - DataFrame: with feature_id or feature_id column
    ...
    feature_ids = self._get_feature_ids(features=features, verbose=False)
    ...
    for idx, row in enumerate(feats_list):
        ...
        feature_id = row.get("feature_id", feature_id)
        mz = row["mz"]
        rt = row["rt"]

        self.logger.debug(
            f"Processing feature {idx + 1}/{len(feats_list)}: UID={feature_id}, ID={feature_id}",
        )
        ...
        if ms2_specs is None or (isinstance(ms2_specs, list) and len(ms2_specs) == 0):
            self.logger.warning(
                f"Feature {feature_id} has null or empty ms2_specs, skipping.",
            )
            continue
        ...
        # Build title
        title = (
            f"MS2 spectrum for m/z {mz:.4f}, rt {rt:.2f}, e {energy}, uid {feature_id}"
        )

        # Extract m/z and intensity
        if not (hasattr(spectrum, "mz") and hasattr(spectrum, "inty")):
            self.logger.warning(
                f"Feature {feature_id} spectrum missing mz or inty attributes, skipping.",
            )
            continue
        ...
        if len(spec_mz) == 0:
            self.logger.info(f"Feature {feature_id} has empty spectrum, skipping.")
            continue

        self.logger.debug(f"Feature {feature_id}: spectrum has {len(spec_mz)} peaks")
        ...
        self.logger.debug(f"Created plot for feature {feature_id}")
        plots.append(stems)
