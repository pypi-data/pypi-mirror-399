    ...
    def bit_score_starting_theta(
        ...
        if guessing_probabilities is None:
            if start_all_incorrect:
                ...
                starting_theta = self.model.latent_scores(torch.zeros((1, len(items))).float(), theta_estimation=theta_estimation, device=ml_map_device, lbfgs_learning_rate=lbfgs_learning_rate)
            else:
                ...
                for theta in range(self.model.latent_variables):
                    ...
                    starting_theta[:, theta] = self.model.latent_scores(min_sum_score.float()[:, theta], theta_estimation=theta_estimation, device=ml_map_device, lbfgs_learning_rate=lbfgs_learning_rate)[:, theta]
        else:
            ...
            if start_all_incorrect:
                ...
                guessing_theta = self.model.latent_scores(random_data, theta_estimation=theta_estimation, device=ml_map_device, lbfgs_learning_rate=lbfgs_learning_rate)
            else:
                ...
                for theta in range(self.model.latent_variables):
                    ...
                    guessing_theta[:, theta] = self.model.latent_scores(random_data_theta, theta_estimation=theta_estimation, device=ml_map_device, lbfgs_learning_rate=lbfgs_learning_rate)[:, theta]
    ...
    def bit_scores_from_theta(
        ...
        if population_theta is None:
            if theta_estimation != "NN":
                ...
                population_theta = self.model.latent_scores(self.model.algorithm.train_data, theta_estimation=theta_estimation, device=ml_map_device, lbfgs_learning_rate=lbfgs_learning_rate)
            else:
                if isinstance(self.model.algorithm, (AE, VAE)):
