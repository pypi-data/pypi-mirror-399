...
def setUpPatchScript(srcFile, dstFile):
    """Setup patch script"""
    generated = []
    with io.open(srcFile, 'r', encoding='utf-8', newline=None) as f:
        src = f.read()
    src = '\n'.join(['    '+line for line in src.split('\n')])
    dest = PATCH_TEMPLATE % dict(script=src, srcFile=srcFile)
    
    if os.path.exists(dstFile):
        with io.open(dstFile, 'r', encoding='utf-8') as f:
            existing = f.read()
        changed = existing != dest
    else:
        changed = True

    if changed:
        with io.open(dstFile, 'w', encoding='utf-8') as f:
            f.write(six.text_type(dest))
