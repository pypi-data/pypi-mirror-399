...
# Load and run with mandatory C++ core (no Python fallback!)
...
git clone https://github.com/mle-runtime/mle-runtime
cd mle-runtime
python setup_cpp_mandatory.py build_ext --inplace
```

### 🔍 Validation
The C++ core is validated at import time. If unavailable, you'll get a clear error message:
...
print(f"Devices: {core_info['supported_devices']}")     # Available devices
```

### 🚫 No More Python Fallback
Previous versions had Python fallback for compatibility. This has been **completely removed** because:
- Python fallback was 10-100x slower
- Created inconsistent performance characteristics  
- Added complexity and maintenance burden
- Users expected native performance

**Result**: Guaranteed native C++ performance on every inference call.
