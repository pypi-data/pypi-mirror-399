...
class SecsHandler:  # pylint: disable=too-many-instance-attributes,too-many-public-methods
    ...
    def send_response(self, *args, **kwargs):
        ...
        if display_log_func := getattr(self, "display_equipment_log", None):
            display_log_func("回复", str(args[0]))
    ...
    def _handle_stream_function(self, message):
        stream_num, function_num  = message.header.stream, message.header.function
        sf_callback_index = self._generate_sf_callback_name(stream_num, function_num)
        ...
        try:
            if sf_pre_func := getattr(self, f"{sf_callback_index}_pre", None):
                pre_result = sf_pre_func(message)
                if not pre_result[0]:
                    self.send_response(
                        self.stream_function(stream_num, function_num + 1)(pre_result[-1]), message.header.system
                    )
                    return
