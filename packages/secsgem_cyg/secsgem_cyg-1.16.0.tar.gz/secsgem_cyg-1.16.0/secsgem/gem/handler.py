...
class GemHandler(secsgem.secs.SecsHandler):  # pylint: disable=too-many-instance-attributes
    ...
    def _on_message_received(self, data: dict[str, typing.Any]):
        ...
        sf_str = self._generate_sf_callback_name(message.header.stream, message.header.function)
        decoded_message = self._settings.streams_functions.decode(message)
        if display_log_func := getattr(self, "display_factory_log", None):
            if message.header.function % 2 == 0:
                title = "回复"
            else:
                title = "发送"
            display_log_func(title, str(decoded_message))
                ...
                if self._is_host:
                    self.send_response(
                        self.stream_function(1, 14)({
                            "COMMACK": self.on_commack_requested(), "MDLN": []
    ...
    def send_process_program(self,
        ...
        return self.settings.streams_functions.decode(
            self.send_and_waitfor_response(
                self.stream_function(7, 3)({"PPID": ppid, "PPBODY": ppbody})
            )
        ).get()
    ...
    def request_process_program(self,
        ...
        # send remote command
        s7f6 = self.settings.streams_functions.decode(
            self.send_and_waitfor_response(self.stream_function(7, 5)(ppid))
        )
