...
class CommandsAddon(Addon):
    ...
    async def _register_commands(self):
        ...
        # global registry
        global_commands = await self.bot.command(self.application_id).fetch_all()
        for g_cmd in global_commands:
            await self.bot.command(self.application_id, command_id=g_cmd.id).delete()
        for cmd in self._global_commands:
            await self.bot.command(self.application_id).create(cmd)

        # guild registry (only guilds in the registry are updated)
        for guild_id, cmds in self._guild_commands.items():
            commands_ = await self.bot.command(self.application_id, guild_id).fetch_all()
            for cmd in commands_:
                await self.bot.command(self.application_id, guild_id, cmd.id).delete()
            for cmd in cmds:
                await self.bot.command(self.application_id, guild_id).create(cmd)
