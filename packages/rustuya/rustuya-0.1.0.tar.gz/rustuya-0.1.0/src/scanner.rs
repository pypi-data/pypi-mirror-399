...
impl Scanner {
                ...
                tokio::select! {
                    ...
                    Some((data, _addr)) = rx.recv() => {
                        if let Some(res) = scanner_temp.parse_packet(&data) {
                            ...
                            if should_log {
                                let mode = if is_active { "active" } else { "passive" };
                                info!("Discovered device {} at {} ({})", res.id, res.ip, mode);
                            }
    ...
    pub async fn scan(&self) -> Result<Vec<DiscoveryResult>> {
        ...
        // Global scan cooldown check
        {
            ...
            if let Some(instant) = *last_scan {
                if instant.elapsed() < GLOBAL_SCAN_COOLDOWN {
    ...
    fn check_cache_and_cooldown(
        ...
        if let Some(res) = guard.get(device_id).cloned() {
            if !force_scan {
                if res.discovered_at.elapsed() < GLOBAL_SCAN_COOLDOWN {
                    debug!("Found device {} in discovery cache", device_id);
                    return Some(res);
                }
                debug!(
                    "Cached device {} expired (older than 30m), re-scanning...",
                    device_id
                );
            }
        }
        ...
        if !force_scan {
            if let Some(last) = *state.last_scan_time.read() {
