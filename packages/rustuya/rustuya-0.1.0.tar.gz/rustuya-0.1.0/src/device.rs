...
const SLEEP_RECONNECT_MAX: Duration = Duration::from_secs(600);
...
impl SubDevice {
    ...
    pub async fn status(&self) {
        self.request(CommandType::DpQuery, None, None::<String>)
            .await
    }

    pub async fn set_dps(&self, dps: Value) {
        self.request(CommandType::Control, Some(dps), None::<String>)
            .await
    }
...
impl Device {
    ...
    pub async fn status(&self) {
        self.request(CommandType::DpQuery, None, None, None).await
    }

    pub async fn set_dps(&self, dps: Value) {
        self.request(CommandType::Control, Some(dps), None, None)
            .await
    }
    ...
    fn drain_rx(&self, rx: &mut mpsc::Receiver<DeviceCommand>, err: TuyaError, close: bool) {
        ...
        while let Ok(cmd) = rx.try_recv() {
            let _ = cmd.respond(Err(err.clone()));
        }
    }
    ...
    async fn process_command<W: AsyncWriteExt + Unpin>(
        ...
        match cmd {
            DeviceCommand::Request {
                ...
                let res = async {
                    let (cmd_id, payload) = self
                        .generate_payload(command, data, cid.as_deref(), req_type.as_deref())
                        .await?;
    ...
    async fn unpack_and_check_dev22(
        ...
        unpack_message(packet, hmac_key, Some(header.clone()), Some(false)).or_else(|e| {
            if version == 3.3 && self.dev_type() != DeviceType::Device22 {
