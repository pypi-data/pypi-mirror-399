...
impl SubDevice {
            ...
            while let Some(req) = rx.recv().await {
                match req.command {
                    ...
                    SubDeviceCommand::Request {
                        ...
                        data,
                        req_type,
                    } => inner_clone.request(command, data, req_type).await,
                }
    ...
    pub fn set_value(&self, index: u32, value: Value) {
        let _ = send_sync(&self.cmd_tx, SubDeviceCommand::SetValue(index, value));
    }
...
impl Device {
            ...
            while let Some(req) = rx.recv().await {
                match req.command {
                    ...
                    DeviceCommand::Request {
                        ...
                        cid,
                        req_type,
                    } => inner_clone.request(command, data, cid, req_type).await,
    ...
    pub fn set_value(&self, dp_id: u32, value: Value) {
        let _ = send_sync(&self.cmd_tx, DeviceCommand::SetValue(dp_id, value));
    }
    ...
    pub fn sub_device(&self, cid: &str) -> SubDevice {
        SubDevice::new(self.inner.sub_device(cid))
    }
...
impl Manager {
    ...
    pub fn list(&self) -> HashMap<String, bool> {
        wait_for_response!(self.cmd_tx, |resp_tx| ManagerCommand::List(resp_tx))
            .ok()
            .unwrap_or_default()
    }
    ...
    pub fn shutdown(self) {
        let _ = wait_for_response!(self.cmd_tx, |resp_tx| ManagerCommand::Shutdown(resp_tx));
    }
}
