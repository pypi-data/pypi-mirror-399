...
def test_with_tokens():
    ...
    try:
        ...
        print("✓ 成功插入包含 token、cost 和 cache 的 log")
        ...
        print(f"  - cache_creation_input_tokens: {log_entry.cache_creation_input_tokens}")
        print(f"  - cache_read_input_tokens: {log_entry.cache_read_input_tokens}")
        return True
...
def verify_schema():
    ...
    try:
        ...
        required_fields = ['input_tokens', 'output_tokens', 'total_tokens', 'input_cost', 'output_cost', 'total_cost', 'cache_creation_input_tokens', 'cache_read_input_tokens']
        found_fields = set()
