...
import tracemalloc
import random
from .constants import MEMORY_SAMPLING_RATE, DISCERANCE_EVENTS_ENDPOINT, REQUEST_TIMEOUT_SECONDS
import threading
import requests
import json
import base64
...
class Ekaros:
    ...
    def send_metrics_async(
        ...
        def _send():
            try:
                headers = {
                    "X-DISCERANCE-SERVER-SDK-KEY": sdk_key,
                    "X-DISCERANCE-SERVER-ID": server_id,
                    "Content-Type": "application/json",
                }

                response = requests.post(
                    DISCERANCE_EVENTS_ENDPOINT,
                    headers=headers,
