...
class TorrentHealthAnalyzer:
    ...
    async def _query_single_tracker(
        ...
        if parsed.scheme in ["http", "https"]:
            ...
            return await self._query_udp_tracker(
                info_hash, tracker_url, start_time
            )
        else:
    ...
    async def _query_udp_tracker(
        self, info_hash_hex: str, tracker_url: str, start_time: float
    ) -> TrackerResponse:
        """
        Query UDP tracker using UdpTrackerClient.
        """
        from torrentcli.trackers.udp import UdpTrackerClient

        try:
             # Convert hex to bytes
            info_hash_bytes = bytes.fromhex(info_hash_hex)

            client = UdpTrackerClient(timeout=self.tracker_timeout)
            seeders, leechers, success, error = await client.announce(tracker_url, info_hash_bytes)
