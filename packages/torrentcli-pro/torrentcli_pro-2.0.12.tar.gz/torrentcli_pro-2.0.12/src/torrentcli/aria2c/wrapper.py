...
class Aria2cWrapper:
    ...
    def __init__(
        ...
        self._stdout_lines: deque = deque(maxlen=1000)
    ...
    def _build_command(self) -> List[str]:
        ...
        # CRITICAL: Add summary-interval for parseable progress output
        # Without this, aria2c uses \r for progress which readline() can't capture
        cmd.extend(["--summary-interval", "1"])
        ...
        for key, value in self.options.items():
            ...
            if isinstance(value, bool):
                ...
                # aria2c uses --option=true or --option=false
                cmd.append(f"{option_name}={str(value).lower()}")
        ...
        try:
            ...
            while True:
                try:
                    ...
                    if not line:
                        # No more data currently available (or EOF if we handled that above)
                        break
    ...
    def _parse_line(self, line: str) -> None:
        ...
        # Check for SEED status (download complete, now seeding)
        if "SEED(" in line:
            logger.debug(f"Detected SEED status: {line}")
            # Mark as complete - download is done
            if self.last_stats:
                self.last_stats.status = "complete"
                self.last_stats.progress_percent = 100.0
            else:
                self.last_stats = Aria2cStats(
                    download_speed=0,
                    upload_speed=0,
        ...
        logger.debug(f"Parsing line: {line[:80]}... match={match is not None}")
        if match:
    ...
    def get_logs(self, last_n: int = 50) -> List[str]:
        ...
        Returns:
            List of log lines
        """
        return list(self._stdout_lines)[-last_n:]
