...
from torrentcli.models import TorrentHealth
...
# TUI Imports
from torrentcli.tui.renderer import TUIRenderer, render_completion_message, render_error_message
from torrentcli.tui.theme import get_theme
...
class DownloadEngine:
    ...
    def download(
        ...
        try:
            ...
            # Step 1.5: Start TUI IMMEDIATELY for visual feedback
            tui: Optional[TUIRenderer] = None
            ui_mode = getattr(self.config, "ui_mode", "normal")
            health_enum: Optional[TorrentHealth] = None
            original_log_level = logging.getLogger().level  # Save for restoration

            if ui_mode in ("normal", "interactive"):
                logger.info(f"Initializing TUI in mode: {ui_mode} (Theme: {self.config.ui_theme})")
                try:
                    theme = get_theme(self.config.ui_theme)
                    tui = TUIRenderer(theme=theme, compact=False, quiet=False)
                    tui.start()
                    logger.info("✅ TUI started successfully")
                    # Suppress logging during TUI to prevent display interference
            ...
            if source.startswith("magnet:?"):
                if tui:
                    tui.set_status("🔍 Analyzing torrent health...")
                else:
                    logger.info("Analyzing torrent health...")
                health_result = asyncio.run(self.health_analyzer.analyze(source))
                health_enum = health_result.classification
                if tui:
                    tui.set_status(f"✓ Health: {health_result.classification.value} ({health_result.total_seeders} seeders)")
                else:
                    logger.info(
            ...
            if fetch_trackers:
                if tui:
                    tui.set_status("📡 Fetching tracker lists...")
                else:
                ...
                if tui:
                    tui.set_status(f"✓ Added {len(extra_trackers)} trackers")
                else:
            ...
            aria2c_opts = profile_config.aria2c_options.copy()

            # Apply overrides
            # Max speed: check CLI arg, then global config
            effective_max_speed = max_speed if max_speed is not None else self.config.max_download_speed
            if effective_max_speed:
                aria2c_opts["max-overall-download-limit"] = effective_max_speed

            # Seeding: check CLI args only (to avoid stomping profile defaults with generic config defaults)
            if seed_time is not None:
                aria2c_opts["seed-time"] = seed_time
            if seed_ratio is not None:
                aria2c_opts["seed-ratio"] = seed_ratio
    ...
    def _monitor_download(
        ...
        # TUI is passed from caller (already started)
        ui_mode = getattr(self.config, "ui_mode", "normal")
        if ui_mode == "quiet":
        ...
        try:
            ...
            while wrapper.is_running():
                ...
                if loop_count <= 5 or loop_count % 10 == 0:
                    logger.debug(f"Monitor loop #{loop_count}: stats={stats is not None}")

                if stats:
                    # Update TUI
                    if tui:
                        logs = wrapper.get_logs(last_n=10)
                        logger.debug(f"TUI update: progress={stats.progress_percent:.1f}%")
                        # Tracker stats? For now assume empty or fetch if possible
                        # Ideally tracker_manager provides this, but it's separate.
                        # We can skip for now or poll later.
                        tui.update(stats, health=health, logs=logs)
                    ...
                    if stats.status == "complete":
                        if tui:
                            ...
                            render_completion_message(
                                ...
                                size_bytes / (time.time() - start_time + 0.1)
                            )
                        else:
                    ...
                    if stats.status == "error":
                        ...
                        if tui:
                            tui.stop()
                            render_error_message(tui.theme, error_msg)
    ...
    def _extract_info_hash(self, source: str) -> str:
        ...
        if source.startswith("magnet:?"):
            ...
            # Parse .torrent file
            from torrentcli.utils import bencode
            ...
            try:
                ...
                # Check for standard 'info' key (bytes)
                if b"info" not in torrent:
                     raise ValueError("Missing 'info' dictionary in .torrent file")

                info = torrent[b"info"]

                # Re-encode info dictionary to get exact bytes for SHA1
                info_bytes = bencode.encode(info)
                return hashlib.sha1(info_bytes).hexdigest()

            except Exception as e:
                logger.error(f"Failed to parse .torrent file {source}: {e}")
                # Fallback or raise? We must raise, as invalid hash breaks everything.
    ...
    def _select_profile_for_health(self, health: TorrentHealth) -> str:
        ...
        """
        mapping = {
            TorrentHealth.HEALTHY: "default",
            TorrentHealth.MODERATE: "default",
    ...
    def _cleanup(self) -> None:
        """Cleanup active downloads on exit."""
        if not self.active_downloads:
            return

        logger.info(f"Cleaning up {len(self.active_downloads)} active downloads...")
        # Create a list of wrappers to stop to avoid concurrent modification during iteration
        wrappers = list(self.active_downloads.values())
        for wrapper in wrappers:
            if wrapper.is_running():
                # Force stop if needed, but try graceful first
                wrapper.stop(graceful=True)
    ...
    def resume(self, hash_or_name: str) -> DownloadResult:
        ...
        return self.download(
            ...
            profile=session["profile"] or self.config.default_profile,
            select_files=False,
        ...
        if wrapper:
            ...
            if delete_files:
                ...
                if download and download["destination"]:
                    ...
                    # SAFETY CHECK: Never delete the root download directory
                    # Only delete if it's a subdirectory of the main download dir,
                    # or if it's a specific file.
                    root_download_dir = Path(self.config.download_dir).expanduser().resolve()
                    target_path = dest_path.expanduser().resolve()

                    if target_path == root_download_dir:
                        logger.warning(f"Refusing to delete download directory root: {target_path}")
                    elif root_download_dir in target_path.parents or target_path.parent == root_download_dir:
                        # It's a file or subdir inside download dir - safe(r) to delete
                        if target_path.exists():
                            if target_path.is_dir():
                                shutil.rmtree(target_path, ignore_errors=True)
                            else:
                                target_path.unlink(missing_ok=True)
                            logger.info(f"Deleted files: {target_path}")
                    else:
                        logger.warning(f"Refusing to delete path outside download dir: {target_path}")
    ...
    def get_metadata(self, source: str) -> Any:
        ...
        if source.startswith("magnet:?"):
            ...
            return {
                ...
                "trackers": [r.tracker_url for r in health_result.tracker_responses],
