...
class DownloadEngine:
    ...
    def download(
        ...
        try:
            ...
            # Pass health_result (classification) if available
            health_enum = health_result.classification if health_result else None
    ...
    def _monitor_download(
        ...
        ui_mode = getattr(self.config, "ui_mode", "normal") # Default to normal if not set
        ...
        if ui_mode in ("normal", "interactive"):
            ...
            try:
                ...
                # Fallback to simple logging if TUI fails
                tui = None
        else:
            logger.info(f"Skipping TUI initialization (ui_mode='{ui_mode}')")
