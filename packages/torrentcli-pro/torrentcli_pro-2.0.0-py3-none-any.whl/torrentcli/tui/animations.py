...
def play_completion_animation(
    ...
    details = Panel(
        Text.assemble(
            ...
            (f"{format_duration(duration_sec)}", theme.colors.text_primary),
        ),
...
def play_confetti_animation(theme: Theme, duration: float = 2.0) -> None:
    ...
    with console.screen():
        while (time.time() - start_time) < duration:
            ...
            # Render particles
            # Optimization: Re-use or only draw active particles
            # Instead of full grid, we build a set of text lines row by row
            # But constructing a full grid is easiest for logic.
            # To optimize: Reduce grid size or only compute affected cells?
            # For 20 FPS, full grid rebuild is okay-ish on modern CPU, but let's try to limit overhead.

            # Simple optimization: Pre-allocate row strings? No, colors vary.
            # Stick to grid but maybe ensure width is not excessive
            eff_width = min(width, 100) # Limit confetti width to 100 cols

            grid = [[" " for _ in range(eff_width)] for _ in range(height)]
            styles = [[theme.colors.text_dim for _ in range(eff_width)] for _ in range(height)]

            for particle in particles:
                x, y = particle["x"], particle["y"]
                if 0 <= x < eff_width and 0 <= y < height:
