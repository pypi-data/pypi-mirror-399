...
class DownloadEngine:
    ...
    def download(
        ...
        try:
            ...
            if profile == "auto" and health_result:
                ...
                profile = self.config.general.default_profile

            # Step 4: Fetch additional trackers if requested
            if fetch_trackers:
                logger.info("Fetching external tracker lists...")
                extra_trackers = asyncio.run(self.tracker_manager.fetch_trackers_for_profile(profile))
                source = self.tracker_manager.inject_trackers_into_magnet(source, extra_trackers)
            ...
            download_dir = Path(self.config.general.download_dir).expanduser()
    ...
    def _select_profile_for_health(self, health: TorrentHealth) -> str:
        ...
        return mapping.get(health, self.config.general.default_profile)
