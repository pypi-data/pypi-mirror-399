        ...
        if operator.is_vararg or operator.is_varret:
            ...
            if operator.is_vararg:
                ...
                assembly_operand_types = " `,` ".join(
                    qualified(f'type(${arg["name"]})') for arg in operator.arguments
                )
            if operator.is_varret:
                assembly_result_types = qualified("type($results)")
            else:
                assembly_result_types = " `,` ".join(
                    qualified(f'type(${ret["name"] or generic_result_name(e)})')
                    for e, ret in enumerate(operator.returns)
                )
            ...
            p_td(f"let parser = [{{ return parseDefaultTorchOp(parser, result, {len(operator.arguments)}, {len(operator.returns)}); }}];")
            p_td(f"let printer = [{{ return printDefaultTorchOp(p, *this, {len(operator.arguments)}, {len(operator.returns)}); }}];")
        else:
...
def emit_ops(emitter_td: TextEmitter, registry: Registry):
    ...
    emit("aten::_weight_norm_interface_backward : (Tensor, Tensor, Tensor, Tensor, int) -> (Tensor, Tensor)")
    ...
    emit("aten::slice_scatter : (Tensor, Tensor, int, int?, int?, int) -> (Tensor)", has_folder=True)
...
def _create_argparse() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--legacy_asm",
        action="store_true",
        help="If given, use MLIR 13 style printer/parser"
    )
    return parser
