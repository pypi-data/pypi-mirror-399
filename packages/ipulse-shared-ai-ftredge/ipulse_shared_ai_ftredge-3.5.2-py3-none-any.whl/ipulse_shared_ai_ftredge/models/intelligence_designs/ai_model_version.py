...
class AIModelVersion(BaseVersionedModel):
    ...
    model_version_metadata_version: int = Field(default=1, ge=1, description="Metadata version for tracking non-breaking changes to this version record (1, 2, 3...)")
    ...
    version_specs: Optional[Dict[str, Any]] = Field(default_factory=dict, description="Version-specific specifications (stored as JSON string in BigQuery): parameters_count, training_data_size, compute_used, etc.")
    ...
    version_artifact_details: Optional[Dict[str, Any]] = Field(default_factory=dict, description="Artifact details (stored as JSON string in BigQuery): checksum, size_mb, format, compression, etc.")
    ...
    recommended_use_cases: Optional[List[str]] = Field(default=None, description="Recommended use cases for this model version.")
