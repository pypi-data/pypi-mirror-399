...
@pytest.fixture
async def redis_connection_generator():
    """
...
@pytest.fixture
async def redis_connection(redis_connection_generator):
    """
    Fixture that returns the actual Redis connection object from the generator.
    This resolves the issue where the fixture was coming through as an async generator
    instead of as the realized Redis connection.
    """
    # Get the connection from the generator
    connection = await redis_connection_generator.__anext__()
    return connection
...
async def test_real_redis_lock_acquire_release(redis_connection):
    ...
    # realize redis connection
    redis_connection = await redis_connection
...
async def test_real_redis_lock_concurrent_access(redis_connection):
    ...
    async def acquire_and_hold(lock_name: str, hold_time: float) -> Tuple[str, bool]:
        ...
        try:
            await lock.acquire(timeout=0.1)
            print(f'acquired {lock_name} for {hold_time} s')
            await asyncio.sleep(hold_time)
            return lock_name, True
        except AlreadyLocked:
            return lock_name, False
        except Exception as e:
            return str(e), False  # if something else happens...
        finally:
            await lock.release()
