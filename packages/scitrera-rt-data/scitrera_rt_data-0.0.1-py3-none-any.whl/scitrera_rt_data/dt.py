...
def dt_from_unix_ns(ns, utc=True) -> Timestamp:
    ...
    :return: Timestamp
    """
    return Timestamp(ns, unit='ns', tz=UTC if utc else _tz_local)
...
def dt_from_unix_ms(ms, utc=True) -> Timestamp:
    ...
    :return: Timestamp
    """
    return Timestamp(ms, unit='ms', tz=UTC if utc else _tz_local)
...
def dt_from_unix_s(s, utc=True) -> Timestamp:
    ...
    :return: Timestamp
    """
    return Timestamp(s, unit='s', tz=UTC if utc else _tz_local)
