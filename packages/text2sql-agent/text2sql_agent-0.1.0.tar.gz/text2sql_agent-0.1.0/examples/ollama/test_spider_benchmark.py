"""
Spider Benchmark Evaluation with Ollama

Evaluates the SQL Agent Toolkit on the Spider benchmark using Ollama (local LLM).
Spider is a large-scale cross-domain text-to-SQL benchmark.

Usage:
    python test_spider_benchmark.py                    # Run on first 50 examples with mistral:7b
    ...
    python test_spider_benchmark.py --model mistral    # Use different model
...
class SpiderBenchmarkEvaluator:
    ...
    def __init__(
        self,
        spider_dir: str = "../groq/spider_data",  # Share with groq by default
        ...
        Args:
            ...
            model: Ollama model to use (e.g., mistral:7b, mistral, codellama)
        ...
        if not self.spider_dir.exists():
            raise FileNotFoundError(
                ...
                "Please run ../groq/setup_spider.py first."
            )
    ...
    def run_benchmark(
        ...
        """
        print("="*80)
        print("SPIDER BENCHMARK EVALUATION WITH OLLAMA")
        print("="*80)
        print(f"\nConfiguration:")
        ...
        for i, example in enumerate(examples):
            ...
            if i < len(examples) - 1:
                time.sleep(0.5)  # 0.5 second delay (local, so faster)
...
def main():
    """Main function."""
    parser = argparse.ArgumentParser(description="Run Spider benchmark with Ollama")
    parser.add_argument(
        '--limit',
        type=int,
        default=50,
    ...
    parser.add_argument(
        '--spider-dir',
        type=str,
        default='../groq/spider_data',
        help='Path to Spider dataset directory (default: ../groq/spider_data)'
    )
    parser.add_argument(
        '--ollama-base-url',
        type=str,
    ...
    if not spider_path.exists() or not (spider_path / "dev.json").exists():
        ...
        print("\nOr specify a different path with --spider-dir")
        return 1

    # Check if Ollama is running
    try:
        import requests
        response = requests.get(args.ollama_base_url)
        if response.status_code != 200:
            print(f"⚠ Warning: Ollama server might not be running at {args.ollama_base_url}")
    except Exception as e:
        print(f"⚠ Warning: Could not connect to Ollama at {args.ollama_base_url}")
        print(f"   Error: {e}")
