...
from oskaragent.helpers import ToolContext, log_msg
                ...
                for tool in tools.tools:
                    for tool_name in server["tools"]:
                        if tool_name == tool.name:
                            ...
                            tool_schema["parameters"]["title"] = tool_schema["name"] + "_arguments"
...
async def mcp_tool(tool_name: str,
    ...
    tool_parameters = {
        param_name: args_obj[param_name] if param_name in args_obj else tool_schema["parameters"][param_name].get("default")
        for param_name in tool_schema.get("parameters", {}).get("properties", {}).keys()
    }
