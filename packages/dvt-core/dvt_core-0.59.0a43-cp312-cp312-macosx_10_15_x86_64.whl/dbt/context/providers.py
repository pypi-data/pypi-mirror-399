...
class BaseDatabaseWrapper:
    ...
    def _get_search_packages(self, namespace: Optional[str] = None) -> List[Optional[str]]:
        ...
        if namespace is None:
            ...
            elif not macro_search_order and self._adapter.config.dependencies and namespace in self._adapter.config.dependencies:
...
class RuntimeSourceResolver(BaseSourceResolver):
    def resolve(self, source_name: str, table_name: str):
        ...
        # DVT: Get connection-specific adapter for this source
        connection_name = (
            target_source.connection if hasattr(target_source, "connection") else None
        )
        if connection_name and hasattr(self.config, "get_adapter"):
        ...
        # DVT: Create relation using connection-specific adapter
        return self.Relation.create_from(
...
class ProviderContext(ManifestContext):
    ...
    def __init__(
        ...
        # Select adapter BEFORE super().__init__() because _get_namespace_builder()
        # (called during parent init) needs self.adapter to be set
        self.adapter = self._select_adapter_for_node(model, config)
    ...
    def _select_adapter_for_node(self, node, config: RuntimeConfig):
        """
        Select the appropriate adapter for this node's Jinja context.

        DVT v0.51.7: Target Hierarchy (Rule 2.1):
        - Level 3 (highest): CLI --target overrides everything
        - Level 2: Model config 'target' override
        - Level 1: Profile default target

        This enables adapter-specific Jinja functions to work correctly
        in federated mode with target overrides.
        """
        target_name = None

        # DVT v0.51.7: Check CLI --target first (highest priority - Rule 2.1)
        if hasattr(config, "args") and hasattr(config.args, "TARGET") and config.args.TARGET:
            target_name = config.args.TARGET
        # Check model config target override (Level 2)
        elif hasattr(node, "config") and hasattr(node.config, "target") and node.config.target:
            target_name = node.config.target

        # If we have a target override, try to get that adapter
        if target_name:
            # Get adapter for target (DVTProfile supports multiple adapters)
            if hasattr(config, "get_adapter"):
                try:
                    adapter = config.get_adapter(target_name)
                    if adapter is not None:
                        return adapter
                except Exception:
                    # Target not found - fall back to default
                    pass
