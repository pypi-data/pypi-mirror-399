...
class DVTSeedRunner(BaseRunner):
    ...
    # Adapters that require sanitized column names (no spaces/special chars)
    STRICT_COLUMN_NAME_ADAPTERS = {"databricks", "spark", "delta"}

    def _sanitize_column_names(self, spark_df, adapter_type: str):
        """Sanitize column names for adapters that don't support special characters.

        DVT v0.59.0a46: Databricks/Delta tables don't allow spaces, semicolons,
        or other special characters in column names. This method:
        1. Strips leading/trailing whitespace
        2. Replaces spaces with underscores
        3. Removes invalid characters for Delta: ,;{}()\n\t=

        Args:
            spark_df: Spark DataFrame to sanitize
            adapter_type: The adapter type (e.g., 'databricks', 'postgres')

        Returns:
            Spark DataFrame with sanitized column names
        """
        if adapter_type not in self.STRICT_COLUMN_NAME_ADAPTERS:
            # Only sanitize for adapters that need it
            return spark_df
        ...
        for col in spark_df.columns:
            ...
            # Remove Delta-invalid characters: ,;{}()\n\t=
            new_col = re.sub(r'[,;{}()\n\t=]', '', new_col)
    ...
    def execute(self, model, manifest):
        ...
        try:
            ...
            # Get adapter to determine if we need column sanitization
            adapter = get_adapter(self.config)
            adapter_type = adapter.type()

            # Sanitize column names for strict adapters (Databricks/Delta)
            # Must happen before transformations to ensure clean column names
