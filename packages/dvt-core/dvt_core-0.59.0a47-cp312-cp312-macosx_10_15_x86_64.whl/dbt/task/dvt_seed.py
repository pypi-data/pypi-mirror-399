...
class DVTSeedRunner(BaseRunner):
    ...
    def _sanitize_column_names(self, spark_df):
        """Sanitize column names for all adapters during seed operations.

        DVT v0.59.0a47: Universal column name sanitization for seeds.
        Clean column names are best practice for all databases:
        1. Strips leading/trailing whitespace
        2. Replaces spaces with underscores
        3. Removes problematic characters: ,;{}()\n\t=
        4. Ensures SQL-safe identifiers

        This matches the behavior of `dvt run` which also uses clean identifiers.
        ...
        for col in spark_df.columns:
            ...
            new_col = re.sub(r'[,;{}()\n\t=]', '', new_col)
            # Remove any other non-word characters (keep letters, digits, underscore)
    ...
    def execute(self, model, manifest):
        ...
        try:
            ...
            # Sanitize column names for all adapters (spaces, special chars -> underscores)
            # Must happen before transformations to ensure clean column names
            spark_df = self._sanitize_column_names(spark_df)

            # Get adapter for writing
