...
class TargetSyncTask:
    ...
    def _cleanup_dbt_core_conflict(self, verbose: bool = True) -> None:
        """
        Remove dbt-core if installed and reinstall dvt-core to restore files.
        ...
        namespace. This method cleans up that conflict.
        ...
        try:
            ...
            if verbose:
                print("\n  🔧 Cleaning up dbt-core conflict...")
                print("    (dbt adapters pull in dbt-core, which conflicts with dvt-core)")

            # Uninstall dbt-core
            subprocess.run(
                [sys.executable, "-m", "pip", "uninstall", "dbt-core", "-y", "--quiet"],
                capture_output=True,
                text=True,
            )
            if verbose:
                print("    ✓ Removed dbt-core")
