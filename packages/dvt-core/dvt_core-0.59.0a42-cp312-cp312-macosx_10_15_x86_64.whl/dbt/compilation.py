...
class Compiler:
    ...
    def _write_node(
        ...
        if node.compiled_code:
            # DVT v0.4.3: Add execution metadata header for federated models
            compiled_code_with_metadata = self._add_execution_metadata(node)

            node.compiled_path = node.get_target_write_path(
                self.config.target_path, "compiled", split_suffix
            )
            node.write_node(self.config.project_root, node.compiled_path, compiled_code_with_metadata)
        return node

    def _add_execution_metadata(self, node: ManifestSQLNode) -> str:
        """
        DVT v0.4.3: Add execution metadata header to compiled SQL.

        For models using Spark compute, add comments indicating:
        - Compute engine used
        - Execution mode (federated vs pushdown)
        - Target connection

        This provides backward compatibility with dbt tooling while showing DVT-specific execution details.
        """
        if not hasattr(node, 'config'):
            return node.compiled_code or ""

        compute_engine = getattr(node.config, 'compute', None)
        target_connection = getattr(node.config, 'target', None)

        # Only add metadata if compute or target override is specified
        if not compute_engine and not target_connection:
            return node.compiled_code or ""

        metadata_lines = ["-- DVT Execution Metadata"]
        metadata_lines.append("--")

        if compute_engine:
            metadata_lines.append(f"-- Compute Engine: {compute_engine}")
            metadata_lines.append("-- Execution Mode: Federated (sources loaded via Spark JDBC)")
            metadata_lines.append("-- SQL Dialect: Spark SQL (for execution)")

        if target_connection:
            metadata_lines.append(f"-- Target Connection: {target_connection}")

        metadata_lines.append("-- ")
        metadata_lines.append("-- Note: Source references below use source adapter dialect for readability.")
        metadata_lines.append("-- During execution, sources are loaded into Spark as temporary views.")
        metadata_lines.append("--")
        metadata_lines.append("")

        return "\n".join(metadata_lines) + (node.compiled_code or "")
