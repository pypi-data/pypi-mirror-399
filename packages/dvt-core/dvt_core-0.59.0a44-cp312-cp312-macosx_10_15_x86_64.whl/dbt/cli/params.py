...
class ExpandedPath(click.Path):
    """A Click Path type that expands ~ to the user's home directory.

    This is needed because environment variables like DVT_PROFILES_DIR=~/.dvt
    don't get tilde-expanded by the shell during assignment, and Click's
    built-in Path type doesn't expand tildes.

    Note: Always returns a string, not a Path object, for compatibility with
    functions like make_directory() that expect str arguments.
    """
...
# DVT: Dry run flag for retract command
dry_run = _create_option_and_track_env_var(
    "--dry-run",
    envvar="DVT_DRY_RUN",
...
target_compute = _create_option_and_track_env_var(
    "--target-compute",
    "-tc",
    envvar="DVT_TARGET_COMPUTE",
    help="Which compute engine to use for federated queries (overrides target_compute in .dvt/computes.yml)",
)

# DVT Profile command parameters (v0.58.7)
profile_sample = _create_option_and_track_env_var(
    "--sample",
    "-S",
    envvar="DVT_PROFILE_SAMPLE",
    type=click.STRING,  # String to support both "10000" and "10%"
    default=None,  # None = profile all rows
    help="Sample size: row count (e.g., 10000) or percentage (e.g., 10%)",
)
