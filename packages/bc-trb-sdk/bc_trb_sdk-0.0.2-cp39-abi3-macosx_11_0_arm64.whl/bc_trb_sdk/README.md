...
async def main():
    # 打开设备（自动发现串口）
    device = await open_triggerhub_serial()
...
async def main():
    # 通过设备名称连接
    device = await open_triggerhub_ble("BRSC1")
...
**连接函数：**
- `open_triggerhub_serial()` - Serial 连接（自动发现）
- `open_triggerhub_ble(device_name)` - BLE 按名称连接
- `open_triggerhub_ble_by_id(device_id)` - BLE 按 ID 连接
