...
@click.option(
    ...
    '{"page": 0, "left": 200, "bottom": 200, "height": 60, "width": 60}'
    '''
)
@click.option(
    '-si', '--stamp_image',
    type=str,
    help='Path to custom image file (PNG, JPG, etc.) to use as signature stamp'
)
def sign(input_file, output_file, api_token, tenant_id, pki, user_id=None, email=None, environment='stg', pdf_password=None, position=None, stamp_image=None):
    """Sign document"""
    if environment == 'dev':
    ...
    if res:
        ...
        elif msg == 'error.image.position_required':
            click.echo(f'''Status: failure \nError: error.image.position_required \nErrorMessage: Position (-pos) is required when using stamp image (-si).''', 
                err=True
            )
