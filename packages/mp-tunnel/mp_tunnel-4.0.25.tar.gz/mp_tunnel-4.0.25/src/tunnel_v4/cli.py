...
def agent_cmd(node_id, worker, token, config, tags, heartbeat, debug, log_file, bg, yes, services):
    if services and services[0] in ('list', 'add', 'remove'):
        ...
        if subcmd == 'list':
            ...
            if not args:
                print('Usage: tunnel agent add <service> [--id <node>]', file=sys.stderr)
                sys.exit(1)
            _agent_add(args, node)
        elif subcmd == 'remove':
            if not args:
...
def _agent_add(services, node_id=None):
    ...
    if agent.get('_just_started'):
        click.echo(f"Agent started with services: {', '.join(services)}")
        sys.exit(0)
...
def _get_local_agent_auto(node_id, services):
    ...
    if node_id:
        for agent in agents:
            if agent.get('node_id') == node_id:
                return agent
        click.echo(f'Node "{node_id}" is not a local agent.')
        if agents:
            click.echo('Local agents: ' + ', '.join(a.get('node_id', '?') for a in agents))
        return None
    if not agents:
        click.echo('No local agent, starting one...')
    ...
    for agent in agents:
        ...
        if not any(name in existing_names for name in service_names):
            click.echo(f'Auto-selected agent: {agent.get("node_id")}')
            return agent
    click.echo('All local agents have conflicting services:')
    for a in agents:
...
def _auto_start_agent(services):
    ...
    cmd.extend(services)
    click.echo(f'Starting agent: {node_id}')
    agents_dir = get_agents_dir()
    agents_dir.mkdir(parents=True, exist_ok=True)
    log_file = agents_dir / f'{node_id}.log'
    with open(log_file, 'w') as log:
        subprocess.Popen(cmd, stdout=log, stderr=log, start_new_session=True, env=os.environ.copy())
    for _ in range(10):
        time.sleep(0.5)
        ...
        if agent_file.exists():
            try:
                ...
                if data.get('pid') and _is_pid_alive(data['pid']):
                    click.echo(f'Agent started: {node_id}')
                    data['_just_started'] = True
                    return data
            except Exception:
                pass
    click.echo('Failed to start agent')
    return None
