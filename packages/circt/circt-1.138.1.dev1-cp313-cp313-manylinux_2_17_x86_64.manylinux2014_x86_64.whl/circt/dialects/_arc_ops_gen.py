...
class ModelOp(_ods_ir.OpView):
  ...
  def __init__(self, sym_name, io, *, initialFn=None, finalFn=None, loc=None, ip=None):
    operands = []
    attributes = {}
    regions = None
    _ods_context = _ods_get_default_loc_context(loc)
    ...
    not _ods_ir.AttrBuilder.contains('anonymous_531')) else
      _ods_ir.AttrBuilder.get('anonymous_531')(io, context=_ods_context))
...
def model(sym_name, io, *, initial_fn=None, final_fn=None, loc=None, ip=None) -> ModelOp:
  return ModelOp(sym_name=sym_name, io=io, initialFn=initial_fn, finalFn=final_fn, loc=loc, ip=ip)
...
def sim_instantiate(*, loc=None, ip=None) -> SimInstantiateOp:
  return SimInstantiateOp(loc=loc, ip=ip)
...
class StateWriteOp(_ods_ir.OpView):
  ...
  def __init__(self, state, value, *, condition=None, loc=None, ip=None):
    operands = []
    attributes = {}
    regions = None
    operands.append(state)
...
def state_write(state, value, *, condition=None, loc=None, ip=None) -> StateWriteOp:
  return StateWriteOp(state=state, value=value, condition=condition, loc=loc, ip=ip)
