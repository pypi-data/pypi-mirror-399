...
class optimizer:
    ...
    def GA(
        ...
        for i in range(int(batch_size * survival_rate * elitism_rate), batch_size): # elites are passed to the next generation as is
            if i >= int(batch_size * survival_rate): # chromosomes that failed to survive are replaced by offsprings of chromosomes that survived
                #Tournament Selection (to select 2 parents for reproduction)
                while True:
                    gene_ts_final = np.zeros(2).astype(int)
                    for j in range(2):
                        gene_ts = np.random.randint(0, int(batch_size * survival_rate), size=(tournament_size))
                ...
                x[:cut1,i] = x[:cut1,gene_ts_final[0]]
                x[cut1:cut2,i] = x[cut1:cut2,gene_ts_final[1]]
                x[cut2:,i] = x[cut2:,gene_ts_final[0]]
                    
            #Mutation (chromosomes that survived but are not elites undergo mutations)
            for j in range(self.Ndim):
                if np.random.random() < mutation_rate:
                    x[j,i] = np.random.uniform(lb, ub)
