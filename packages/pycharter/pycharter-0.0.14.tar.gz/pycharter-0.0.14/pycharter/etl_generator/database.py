...
def _build_bulk_upsert_sql_and_params(
    ...
    # Build VALUES clauses and parameters for actual batch size
    values_clauses = []
    params = {}
...
async def load_data_postgresql(
    ...
    for i in range(0, len(data), batch_size):
        ...
        if write_method == 'upsert' and primary_key_list:
            ...
            # Build bulk upsert SQL and parameters
            sql, params = _build_bulk_upsert_sql_and_params(
                full_table_name, columns, primary_key_list, processed_batch
            )
            ...
            try:
                ...
                logger.error(f"Failed to bulk upsert batch: {e}")
                logger.error(f"Batch size: {len(processed_batch)}")
                logger.error(f"SQL (first 500 chars): {sql[:500]}")
                raise
...
async def load_data(
    ...
    if db_type == DB_POSTGRESQL:
        if isinstance(session, AsyncSession):
            ...
            # Fallback to sync version if sync session provided
            # This should not happen in normal usage, but kept for backward compatibility
            raise ValueError("PostgreSQL load requires AsyncSession. Use sync load functions for sync sessions.")
    else:
        # For other databases, use generic async fallback
        # TODO: Convert other database load functions to async
