...
class Upload:
    ...
    def write(self, path:str, buf: bytes, offset: int) -> int:
        ...
        if st is None:
            ...
            metrics.counts.incr(f'upload_put_block')
    ...
    def flush(self, path: str) -> int:
        if common.offline:
            metrics.counts.incr('upload_flush_offline')
            return 0
        
        st = metadata.cache.getattr(path)
        if st is None:          
            metrics.counts.incr(f'upload_flush_noattr')
            return 0
        ...
        if db.cache.get(f'{UPLOAD}:{st.local_id}', UPLOAD) == None:         
            metrics.counts.incr(f'upload_flush_nopending')
            return 0

        if not data.cache.isEntireFileCached(path, st.local_id, st):       
            metrics.counts.incr('upload_flush_file_is not_cached')
            return 0
        
        if gitignore.parser.isIgnored(path):
            logger.debug(f'upload.flush: {path} is ignored by .gitignore, skipping upload')
            metrics.counts.incr('upload_flush_ignored_by_gitignore')
            return 0
        ...
        try:
            with lock.get(path):              
                ...
                if flushFileToRemote:
                    ...
                    metrics.counts.incr(f'upload_flush_noop')
                    return 0
    ...
    def uploadThread(self, number: int):
        ...
        common.threadLocal.operation = 'upload_%d' % number
        common.threadLocal.path = None
        while not self.stopped:            
            (path, localId) = self.uploadQueue.get()            

            common.threadLocal.path = (path,)
            metrics.counts.incr('upload_dequeued')
            metrics.counts.startExecution('upload_%d' % number)
            try:
                ...
                metrics.counts.incr(f'upload_flush_wrote_local_file_bytes', fileSize)

                sentBytes = api.interface.writefile(path, localId, localPath, st)
                
                metrics.counts.incr(f'upload_flush_network_bytes_sent', sentBytes) 
                
                if fileSize != sentBytes:
                    logger.warning('upload.flush: %s local_id=%s local file size %d differs from sent bytes %d', path, localId, fileSize, sentBytes)
                    metrics.counts.incr(f'upload_flush_size_mismatch')
                    raise FuseOSError(errno.EIO)
                
            except Exception as e:
                if isinstance(e, HttpError):
                    metrics.counts.incr('upload_httperror')
                    logger.error(f"<-- uploadThread: HttpError writing file {path}: {e}")
                elif isinstance(e, TimeoutError):
                    metrics.counts.incr('upload_timeouterror')
                    logger.error(f'<-- uploadThread: TimeoutError writing file {path}: {e}')
                else:
                    self.exceptionCount += 1
                    metrics.counts.incr('upload_exception')
                    raisedBy = log.exceptionRaisedBy(e)
                    logger.exception(f"<-- uploadThread: exception writing file {path}: {raisedBy}")
            finally:
                self.activeThreadCount -= 1
                metrics.counts.endExecution('upload_%d' % number)
                logger.info('<- uploadThread %s local_id=%s', path, localId)
