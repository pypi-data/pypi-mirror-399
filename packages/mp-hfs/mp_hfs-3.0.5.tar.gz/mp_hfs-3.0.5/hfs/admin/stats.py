...
@click.pass_context
def rotation_history(ctx, project, node, limit):
    """Show rotation history for project/node"""
    r = ctx.obj['redis']
    
    # 收集所有相关 Space 的审计日志
    rotations = []
    
    for key in r.keys('hfs:audit:*'):
        space_id = key.replace('hfs:audit:', '')
        entries = r.lrange(key, 0, -1)
        
        # 解析日志，找 bind 和 transition 操作
        binds = []
        for e in entries:
            entry = json.loads(e)
            op = entry.get('op')
            
            if op == 'bind' and entry.get('result') in ('bound', 'instance_updated'):
                # 检查是否属于目标项目
                by = entry.get('by', '')
                if f'/{project}/' in by or entry.get('project') == project:
                    binds.append(entry)
        ...
        for b in binds:
            ...
            rotations.append({
                ...
                'space': space_id,
                'op': b.get('op'),
                'result': b.get('result', 'bound'),
            })
    
    # 按时间排序
    rotations.sort(key=lambda x: x['ts'], reverse=True)
    
    # 显示
    title = f"🔄 Rotation History: {project}"
    if node:
        title += f" / {node}"
    click.echo(f"\n{title}\n")
    ...
    rows = []
    for rot in rotations[:limit]:
        ts = time.strftime('%m-%d %H:%M:%S', time.localtime(rot['ts'] / 1000))
        rows.append([ts, rot['node'], rot['space'], rot['result']])
    
    click.echo(tabulate(rows, headers=['Time', 'Node', 'Space', 'Result']))
    
    # 统计
    click.echo(f"\n📊 Summary:")
    click.echo(f"   Total rotations: {len(rotations)}")
    ...
    if len(node_counts) > 1:
        click.echo(f"   By node: {', '.join(f'{n}={c}' for n, c in sorted(node_counts.items()))}")
