...
def list_presets() -> str:
    """
    Get formatted help text for all matching presets.

    Returns dynamically generated text from PRESETS dict.
    When a new preset is added, this help updates automatically.

    Example:
        >>> print(list_presets())
        Matching Presets (use preset='name'):

          exact
            Almost exact matching. For legal docs, code, identifiers.
            jw_threshold=0.98, max_edit=5%
          ...

        >>> from vibe_finder import list_presets
        >>> help_text = list_presets()
    """
    lines = ["Matching Presets (use preset='name'):", ""]
