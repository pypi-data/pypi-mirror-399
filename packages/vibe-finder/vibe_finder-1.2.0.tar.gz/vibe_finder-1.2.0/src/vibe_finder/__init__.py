...
class CloudFinder:
    ...
    def __init__(self, text: str, fuzzy_threshold: float = 0.85):
        """
        Initialize CloudFinder with document text.

        Args:
            text: Document text to analyze
            fuzzy_threshold: Jaro-Winkler similarity threshold for fuzzy matching (0-1)
        """
        self.text = text
        ...
        for ki, kw_toks in enumerate(keyword_tokens):
            if len(kw_toks) == 1:
                ...
                for tok in self.tokens:
                    sim = jaro_winkler_similarity(target, tok['word'])
                    if sim >= self.fuzzy_threshold:
                        keyword_positions[ki].append((tok['start'], tok['end'], tok['word']))
            else:
                # Multi-word keyword - find sequence
                for i, tok in enumerate(self.tokens):
                    if i + len(kw_toks) > len(self.tokens):
                        break
                    matched = True
                    for j, kw_tok in enumerate(kw_toks):
                        sim = jaro_winkler_similarity(kw_tok, self.tokens[i+j]['word'])
                        if sim < self.fuzzy_threshold:
                            matched = False
                            break
                    if matched:
