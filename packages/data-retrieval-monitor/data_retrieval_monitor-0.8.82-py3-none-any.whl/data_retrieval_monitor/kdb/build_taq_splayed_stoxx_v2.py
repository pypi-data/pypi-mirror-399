...
def main() -> None:
    ...
    print(f"[LOCAL] rows={local_all.height} unique_days={n_days}", flush=True)
    ...
    print(f"[INIT] date_range={dates[0]} .. {dates[-1]} (count={len(dates)})", flush=True)

    # >>> IMPORTANT: TEMP TEST LIMIT <<<
    # Start with a tiny run so you can see output & confirm writing works.
    TEST_DAYS = 3
    dates = dates[:TEST_DAYS]
    print(f"[INIT] TEST_DAYS={TEST_DAYS} -> running only first {len(dates)} day(s)", flush=True)

    # 3) Compass client + SMOKE TEST
    ...
    # Smoke test so you know you can talk to kdb at all
    try:
        res = compass.run_query_sync("1+1")
        # res is usually a PyKX object; printing may be verbose, so just confirm it returned
        print("[INIT] Compass smoke test OK (1+1 returned).", flush=True)
    except Exception as e:
    ...
    for di, d in enumerate(dates, 1):
        ...
        rics_day: List[str] = local_day.select("RIC").unique().to_series().to_list()

        print(f"[{di}/{len(dates)}] {d} START local_rows={local_rows} unique_rics={len(rics_day)}", flush=True)

        # 4) Fetch remote in chunks
        remote_parts: List[pl.DataFrame] = []
        n_chunks = (len(rics_day) + MAX_RICS_PER_QUERY - 1) // MAX_RICS_PER_QUERY
        for i in range(0, len(rics_day), MAX_RICS_PER_QUERY):
            chunk = rics_day[i:i + MAX_RICS_PER_QUERY]
            chunk_idx = (i // MAX_RICS_PER_QUERY) + 1
            print(f"[{d}] fetching remote chunk {chunk_idx}/{n_chunks} rics={len(chunk)} ...", flush=True)
            try:
                part = fetch_remote_day(compass, d, chunk)
                print(f"[{d}] fetched chunk {chunk_idx}/{n_chunks}: rows={part.height}", flush=True)
                remote_parts.append(part)
            except Exception as e:
                print(f"[WARN] {d} remote chunk {chunk_idx}/{n_chunks} failed: {e}", flush=True)

        remote_day = pl.concat(remote_parts, how="vertical") if remote_parts else pl.DataFrame()
        print(f"[{d}] remote_day total_rows={remote_day.height}", flush=True)

        # 5) Assign trades -> csv rows using BOTH time-bases; pick better coverage
        print(f"[{d}] assigning trades -> csv_row_id using StartTime/EndTime ...", flush=True)
        ...
        print(f"[{d}] stacked_exch rows={stacked_exch.height}", flush=True)

        print(f"[{d}] assigning trades -> csv_row_id using LocalStartTime-shifted ...", flush=True)
        stacked_local = stack_assign_by_timewindow(
            remote_day, local_day,
            start_col="start_local", end_col="end_local",
            pre_buffer_s=PRE_BUFFER_S, post_buffer_s=POST_BUFFER_S
        )
        print(f"[{d}] stacked_local rows={stacked_local.height}", flush=True)
        ...
        print(
            f"[{d}] DONE local_rows={local_rows} matched_rows={matched_rows} "
    ...
    print(f"[DONE] parquet_files={len(manifest_paths)} total_saved_trade_rows={total_saved_trades}", flush=True)
