...
def test_workflow_clear_context_store_true(
    ...
    workflow_yaml = WorkflowYamlModel(
        ...
        states=[
            ...
            StateModel(
                ...
                assistant_id="data_processor",
                task=(
                    "Try to access: User ID {{user_id}}, Project ID {{project_id}}. "
                    "If you see literal '{{...}}' text (unresolved variables), say 'Variables not in context'"
                ),
                next={"state_id": "end"},
            ),
        ],
    ...
    # Verify variables are NOT accessible (context was completely cleared)
