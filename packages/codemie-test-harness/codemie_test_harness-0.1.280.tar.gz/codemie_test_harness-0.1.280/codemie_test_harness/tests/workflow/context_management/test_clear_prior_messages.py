...
def test_workflow_clear_prior_messages_multi_phase(
    ...
    2. Phase 2: Clear history, output data B (can't see A)
    3. Phase 3: Clear history again, output data C (can't see A or B)
    ...
    workflow_yaml = WorkflowYamlModel(
        ...
        states=[
            StateModel(
                ...
                task=f"Say: 'Phase 1 - User ID is {TEST_DATA['user_id']}'",
                next={"state_id": "phase2", "clear_prior_messages": True},
            ),
            StateModel(
                id="phase2",
                assistant_id="multi_phase",
                task=f"Say: 'Phase 2 - Project ID is {TEST_DATA['project_id']}'. Can you see any user ID in the conversation? Answer yes or no.",
                next={"state_id": "phase3", "clear_prior_messages": True},
            ),
            StateModel(
                id="phase3",
                assistant_id="multi_phase",
                task="List all IDs (user ID, project ID, file ID) you can see in the conversation history. If none, say 'No IDs visible in history'",
                next={"state_id": "end"},
            ),
        ],
    )
    ...
    # Phase 3 should not see any IDs from previous phases
    assert_that(output, is_not(contains_string(TEST_DATA["user_id"])))
    assert_that(output, is_not(contains_string(TEST_DATA["project_id"])))
