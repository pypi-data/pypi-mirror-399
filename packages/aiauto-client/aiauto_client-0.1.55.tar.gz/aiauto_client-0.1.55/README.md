...
> **주의**: artifact-store (`/artifacts`)는 `top_n_artifacts` 자동 정리 로직이 동작합니다. 직접 파일을 쓰지 말고 `AIAutoController.get_artifact_store()` + `optuna.artifacts.upload_artifact()` API를 사용하세요.
...
**저장소 옵션**:
- 기본: 노드 디스크 사용
- `tmp_cache_medium="Memory"`: RAM 기반 emptyDir 사용
...
def objective(trial):
    ...
    # AIAutoController 를 objective 함수 안에서 다시 선언해야 함 (Front 또는 이전 실행으로 workspace가 이미 존재하면 아래 설정값들은 무시됨)
