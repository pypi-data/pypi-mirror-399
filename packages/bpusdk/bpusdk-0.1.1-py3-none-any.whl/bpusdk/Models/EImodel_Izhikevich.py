...
class EINet(bp.DynamicalSystem):
    def __init__(self, population_sizes, conn, method, allow_multi_conn=False):
        ...
        tauRef = 0+0.0001 
        self.E = bp.dyn.IzhikevichRef(population_sizes[0], V_th=30.,tau=20.,tau_ref=tauRef, d=8.0, 
                               V_initializer=bp.init.Uniform(-100.,-17.5),
                                u_initializer=bp.init.Constant(0.),
                                method=method)
        self.I = bp.dyn.IzhikevichRef(population_sizes[1],V_th=30., tau=20.,tau_ref=tauRef, d=8.0, 
                               V_initializer=bp.init.Uniform(-100.,-17.5),
                               u_initializer=bp.init.Constant(0.),
                               method=method)
