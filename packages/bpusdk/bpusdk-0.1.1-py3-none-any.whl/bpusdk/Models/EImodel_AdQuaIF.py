...
class EINet(bp.DynamicalSystem):
    def __init__(self, population_sizes, conn, method, allow_multi_conn=False):
        ...
        tauRef = 0+0.0001 
        self.E = bp.dyn.AdQuaIF(population_sizes[0], V_th=-30.,tau=10.,V_rest=-65.,V_reset=-68.,V_c=-50,b=0.1,
                               V_initializer=bp.init.Uniform(-100.0,-20.0),
                               w_initializer=bp.init.Constant(0.0),
                               method=method)
        self.I = bp.dyn.AdQuaIF(population_sizes[1], V_th=-30., tau=10., V_rest=-65.,V_reset=-68.,V_c=-50,b=0.1,
                               V_initializer=bp.init.Uniform(-100.0,-20.0),
                               w_initializer=bp.init.Constant(0.0),
                               method=method)
