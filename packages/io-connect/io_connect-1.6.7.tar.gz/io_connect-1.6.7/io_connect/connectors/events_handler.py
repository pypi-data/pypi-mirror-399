...
class EventsHandler:
    ...
    def create_parameter_payloads_in_chunks(
        ...
        for i in range(0, len(df), chunk_size):
            ...
            if parameter_versions:
                payload = {"parameterVersions": parameter_versions}
    ...
    def add_parameter_version(
        ...
        Use `publish_parameter_version` available in DataAccess instead.
        ...
        warnings.warn(
            "`add_parameter_version` is deprecated and will be removed in a future "
            "release. Use `publish_parameter_version` available in DataAccess instead.",
        ...
        # Construct the URL based on the on_prem flag
        protocol = "http" if on_prem else "https"
        url = c.ADD_PARAMETER_VERSION.format(protocol=protocol, data_url=self.data_url)

        # Step 2: Send payloads to API
        results = []
        header = {"Content-Type": "application/json", "userID": self.user_id}

        for i, payload in enumerate(payloads, 1):
            try:
                with Logger(self.logger, f"API {url} response time:", self.log_time):
                    response = requests.post(
                        url, headers=header, json=payload, verify=False
                    )
                ...
                print(
                    f"✓ Chunk {i}/{len(payloads)}: Successfully published {len(payload['parameterVersions'])} parameter versions"
                )
            except requests.exceptions.RequestException as e:
                error_message = (
                    ERROR_MESSAGE(response, url)
                ...
                self.logger.error(
                    f"[EXCEPTION] Chunk {i}: {type(e).__name__}: {error_message}"
                )
                results.append(
                    {
                        "chunk": i,
                ...
                print(
                    f"✗ Chunk {i}/{len(payloads)}: Failed to publish {len(payload['parameterVersions'])} parameter versions - {str(e)}"
                )
