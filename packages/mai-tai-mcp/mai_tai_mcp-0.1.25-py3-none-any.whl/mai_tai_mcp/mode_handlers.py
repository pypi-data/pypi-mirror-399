"""
Mai-Tai mode entry and exit handlers (v2).
...
async def send_progress_update(
    ...
    """Send a progress update to the mai-tai channel.
    ...
    """
    if not state.mai_tai_mode or not state.channel_id:
    ...
    await send_message(state.channel_id, message)
...
async def send_blocked_message(
    ...
    """Send a blocked/need-input message to the mai-tai channel.
...
async def handle_enter_mai_tai_mode(
    ...
    Args:
        ...
        channel_id: Channel for communications
        send_message: Async function to send messages (channel_id, content)
    ...
    logger.info(f"Sent initial mai-tai mode message to channel {channel_id}")
...
async def handle_exit_mai_tai_mode(
    ...
    if channel_id:
        ...
        await send_message(channel_id, message)
        logger.info(f"Sent exit acknowledgment to channel {channel_id}")
...
async def handle_plan_received(
    ...
    if state.channel_id:
        message = format_initial_status_message(plan, repo_name)
        await send_message(state.channel_id, message)
        logger.info(f"Sent plan confirmation to channel {state.channel_id}")
...
async def handle_task_complete(
    ...
    if state.channel_id:
        ...
        await send_message(state.channel_id, message)
        logger.info(f"Sent completion summary to channel {state.channel_id}")
