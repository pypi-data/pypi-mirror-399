"""
Tests for v2 completion semantics (Step 5).
...
class TestCompletionSemantics:
    ...
    def test_channel_preserved_after_completion(self):
        """Channel ID should be preserved for receiving new instructions."""
        state = MaiTaiState()
        plan = WorkPlan(
            task_summary="Test task",
            steps=[PlanStep("Step 1", completed=True)],
        ...
        # Channel should still be set for receiving new instructions
    ...
    async def test_handle_task_complete_sends_summary(self):
        ...
        channel_id, content = send_message.call_args[0]
