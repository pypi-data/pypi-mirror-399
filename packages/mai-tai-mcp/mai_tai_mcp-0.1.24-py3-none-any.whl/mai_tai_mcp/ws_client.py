"""
WebSocket client for real-time Mai-Tai channel updates (v2).
...
class ChannelSubscription:
    """Tracks subscription state for a channel."""
    channel_id: str
...
class WebSocketClient:
    ...
    _subscriptions: dict[str, ChannelSubscription] = field(default_factory=dict)
    _shutdown: bool = field(default=False)
    _receive_task: Optional[asyncio.Task] = field(default=None, repr=False)

    def _get_ws_url(self, channel_id: str) -> str:
        ...
        return f"{ws_base}/api/v1/ws/channels/{channel_id}?{params}"

    async def connect(self, channel_id: str, on_message: Optional[Callable[[dict], None]] = None) -> bool:
        """Connect to a channel's WebSocket endpoint.
        ...
        url = self._get_ws_url(channel_id)

        try:
            logger.info(f"Connecting to WebSocket for channel {channel_id}")
            self._ws = await websockets.connect(url, ping_interval=20, ping_timeout=10)
            self.state = ConnectionState.CONNECTED
            self.backoff.reset()

            # Store subscription
            self._subscriptions[channel_id] = ChannelSubscription(
            ...
            if e.status_code in (401, 403):
                ...
                raise FatalRuntimeError(f"Channel not found: {channel_id}")
            else:
    ...
    async def _handle_message(self, data: dict) -> None:
        ...
        if msg_type == "connected":
            ...
            logger.debug(f"Connection confirmed for channel {channel_id}")
            ...
            if channel_id in self._subscriptions:
                sub = self._subscriptions[channel_id]
    ...
    def get_last_message_id(self, channel_id: str) -> Optional[str]:
        ...
        """
        if channel_id in self._subscriptions:
            return self._subscriptions[channel_id].last_message_id
        return None

    def set_last_message_id(self, channel_id: str, message_id: str) -> None:
        """Set the last seen message ID for a channel.

        Used to initialize state before connecting.
        """
        if channel_id in self._subscriptions:
            self._subscriptions[channel_id].last_message_id = message_id
