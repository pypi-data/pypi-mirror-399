...
def _wait_for_response_polling(
    ...
    # timeout_seconds=0 means wait forever
...
def chat_with_human(
    ...
    try:
        return _chat_with_human_impl(message, channel_name, timeout_seconds)
    finally:
...
def _chat_with_human_impl(
    ...
    # API keys are bound to a single channel - use that channel
    if not backend.channel_id:
        return {
            "status": "error",
            "error": "No channel bound to this API key.",
        }

    # If a channel name is specified, verify it matches the bound channel
    if channel_name and channel_name != backend.channel_name:
        return {
            "status": "error",
            "error": f"This API key is bound to channel '{backend.channel_name}', not '{channel_name}'.",
            "bound_channel": backend.channel_name,
        }

    # Send the message to the bound channel
...
def update_status(
    ...
    Args:
        ...
        channel_name: Optional channel name. If not provided, uses the bound channel.
    ...
    # Verify we have a bound channel
    if not backend.channel_id:
        return {
            "status": "error",
            "error": "No channel bound to this API key. Cannot send status update.",
        }
...
def list_channels() -> dict[str, Any]:
    ...
    return {
        ...
        "bound_channel": backend.channel_name,
        "note": "This API key is bound to a single channel. You can only access the bound channel.",
    }
...
def get_channel(channel_name: Optional[str] = None) -> dict[str, Any]:
    ...
    return backend.get_channel(backend.channel_id)
...
def get_messages(
    ...
    result = backend.get_messages(backend.channel_id, limit=limit)
    result["channel"] = backend.channel_name
    return result
...
def get_project_info() -> dict[str, Any]:
    ...
    return {
        ...
        "channel_id": backend.channel_id,
        "channel_name": backend.channel_name,
