...
def register_range(kbrange, pyrange, conn_or_curs, globally=False):
    ...
    :param kbrange: the name of the Kingbase |range| type. Can be
        schema-qualified
    ...
    *kbrange* type and raises `~ksycopg2.ProgrammingError` if the type is not
    found.  If querying the database is not advisable, use directly the
    `RangeCaster` class and register the adapter and typecasters using the
    provided functions.

    """
    caster = RangeCaster._from_db(kbrange, pyrange, conn_or_curs)
...
class RangeCaster(object):
    ...
    """
    def __init__(self, kbrange, pyrange, oid, subtype_oid, array_oid=None):
        self.subtype_oid = subtype_oid
        self._create_ranges(kbrange, pyrange)
    ...
    def _create_ranges(self, kbrange, pyrange):
        ...
        self.adapter = None
        if isinstance(kbrange, string_types):
            self.adapter = type(kbrange, (RangeAdapter,), {})
            self.adapter.name = kbrange
        else:
            try:
                if issubclass(kbrange, RangeAdapter) \
                        and kbrange is not RangeAdapter:
                    self.adapter = kbrange
            except TypeError:
                pass

        if self.adapter is None:
            raise TypeError(
                'kbrange must be a string or a RangeAdapter strict subclass')
    ...
    def _from_db(self, name, pyrange, conn_or_curs):
        """Return a `RangeCaster` instance for the type *kbrange*.
