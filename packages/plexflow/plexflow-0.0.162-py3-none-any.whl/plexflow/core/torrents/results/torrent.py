...
class Torrent(ABC):
    ...
    def is_bad_quality(self):
        ...
        return parsed_title.get('quality', '').upper() in bad_qualities or any((q in parsed_title.get('excess', '').upper() if isinstance(parsed_title.get('excess'), str) else q in list(map(lambda t: t.upper(), parsed_title.get('excess', [])))) or q in self.release_name.upper() for q in bad_qualities)
