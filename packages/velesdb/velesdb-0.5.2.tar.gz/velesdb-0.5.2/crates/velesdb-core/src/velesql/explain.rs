...
use super::ast::{Condition, DistanceMetricType, SelectStatement};
...
impl QueryPlan {
    ...
    pub fn from_select(stmt: &SelectStatement) -> Self {
        ...
        if has_vector_search {
            ...
            nodes.push(PlanNode::VectorSearch(VectorSearchPlan {
                ...
                metric: Self::extract_metric(stmt.where_clause.as_ref()),
    ...
    /// Extracts the distance metric from a WHERE clause.
    fn extract_metric(where_clause: Option<&Condition>) -> DistanceMetricType {
        where_clause.map_or(DistanceMetricType::default(), |condition| {
            Self::find_metric_in_condition(condition)
        })
    }

    fn find_metric_in_condition(condition: &Condition) -> DistanceMetricType {
        match condition {
            Condition::VectorSearch(vs) => vs.metric,
            Condition::And(left, right) | Condition::Or(left, right) => {
                let left_metric = Self::find_metric_in_condition(left);
                if left_metric == DistanceMetricType::default() {
                    Self::find_metric_in_condition(right)
                } else {
                    left_metric
                }
            }
            Condition::Not(inner) | Condition::Group(inner) => {
                Self::find_metric_in_condition(inner)
            }
            _ => DistanceMetricType::default(),
        }
    }
    ...
    fn render_node(node: &PlanNode, output: &mut String, prefix: &str, is_last: bool) {
        ...
        match node {
            PlanNode::VectorSearch(vs) => {
                ...
                let _ = writeln!(output, "{child_prefix}├─ Metric: {}", vs.metric.as_str());
...
mod tests {
    ...
    fn test_plan_to_tree_format() {
        ...
        assert!(tree.contains("Metric: cosine"));
    ...
    fn test_distance_metric_as_str() {
        assert_eq!(DistanceMetricType::Cosine.as_str(), "cosine");
        assert_eq!(DistanceMetricType::Euclidean.as_str(), "euclidean");
        assert_eq!(DistanceMetricType::Dot.as_str(), "dot");
    }
