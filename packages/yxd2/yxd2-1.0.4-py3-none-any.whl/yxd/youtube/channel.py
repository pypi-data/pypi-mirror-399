...
def get_channel_tab(channel_id, page="1", sort=3, tab='videos', view=1,
    ...
    while True:
        try:
            ...
            attempts += 1
            if attempts <= 5 and (e.code == '429' or (e.code and str(e.code).startswith('5'))):
                print(f"Transient error {e.code}. Retrying in {backoff}s (attempt {attempts}/5)")
                time.sleep(backoff)
                backoff = min(backoff * 1.5, 10)  # Faster backoff: 1s, 1.5s, 2.25s, 3.4s, 5s
                continue
            raise
...
def get_channel_page_general_url(base_url, tab, request, channel_id=None):
    ...
    if tab == 'videos' and sort in ('3', '4'):
        ...
        # Force using channel API if UU playlist doesn't have continuation token
        # This handles channels where the UU playlist method doesn't work properly
        if info['items'] and pl_info.get('ctoken'):   # Success with continuation
            page_size = 100
            try_channel_api = False
        else:   # Try the channel API method
            try_channel_api = True
...
def get_videos(channel_id, tab):
    # if duplicates > 20 consecutive then break
    cache = set()
    cache.add("")
    counter = 0
    duplicate_counter = 0  # Track consecutive duplicates separately
    ...
    while True:
        ...
        for item in items:
            if not item:
                continue  # Skip empty items instead of stopping
            video = parse(item)
            video_id = video.get('id')
            if video_id in cache:
                duplicate_counter += 1
                if duplicate_counter > 20:
                    print(f"Stopping: {duplicate_counter} consecutive duplicates found. Total videos: {counter}")
                    return
                continue
            # Reset duplicate counter when we find a new video
            duplicate_counter = 0
            counter += 1
            cache.add(video_id)
        ...
        # Minimal delay for proxy rotation
        time.sleep(0.1)

        # Use YouTube's continuation token
        polymer_json = get_channel_tab(channel_id=channel_id, tab=tab, ctoken=ctoken)

        # Check if polymer_json is valid before parsing
        if not polymer_json or not polymer_json.strip():
            print(f"Warning: Empty response from YouTube for channel {channel_id}")
            break

        try:
            parsed_json = json.loads(polymer_json)
        except json.JSONDecodeError as e:
            print(f"Error: Failed to parse JSON response from YouTube")
            print(f"Response preview (first 500 chars): {polymer_json[:500]}")
            print(f"JSONDecodeError: {e}")
            break

        info = yt_data_extract.extract_channel_info(parsed_json, tab=tab,
                                                    continuation=True)
...
def parse(item):
    ...
    try:
        ...
        video["time_published"] = item.get("time_published", "Unknown")
        video["author"] = item.get("author", None)
        video['duration'] = item.get('duration', None)
        video['badges'] = item.get('badges', [])

        # If critical fields are missing, mark as error
        if not video["id"]:
            video["error"] = True
