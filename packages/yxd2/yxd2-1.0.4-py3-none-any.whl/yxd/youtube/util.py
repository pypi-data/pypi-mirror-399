...
def get_pool(use_tor):
    # Check for proxy in environment variables (for DataImpulse, etc.)
    proxy_url = os.environ.get('HTTPS_PROXY') or os.environ.get('HTTP_PROXY')

    if use_tor:
        return tor_manager.get_tor_connection_pool()
    elif proxy_url:
        # Create NEW pool each time to enable proxy rotation
        # Critical for rotating proxy services like DataImpulse
        try:
            import urllib.parse as urlparse_helper
            parsed = urlparse_helper.urlparse(proxy_url)
            proxy_headers = None
            if parsed.username:
                user = urlparse_helper.unquote(parsed.username)
                pwd = urlparse_helper.unquote(parsed.password or '')
                proxy_headers = urllib3.util.make_headers(proxy_basic_auth=f"{user}:{pwd}")
            # num_pools=1 to force new connection each time
            return urllib3.ProxyManager(proxy_url, proxy_headers=proxy_headers,
                                       cert_reqs='CERT_REQUIRED', num_pools=1, maxsize=1)
        except Exception:
            return urllib3.ProxyManager(proxy_url, cert_reqs='CERT_REQUIRED', num_pools=1, maxsize=1)
    else:
...
def fetch_url_response(url, headers=(), timeout=15, data=None,
    ...
    if cookiejar_send is not None or cookiejar_receive is not None:     # Use urllib
        ...
        # Retry logic with proxy rotation for connection errors
        ...
        for attempt in range(max_connection_retries):
            ...
            try:
            ...
            except (urllib3.exceptions.ProtocolError, urllib3.exceptions.MaxRetryError) as e:
                # Check if it's a RemoteDisconnected or connection aborted error
                ...
                if isinstance(e, urllib3.exceptions.ProtocolError):
                    ...
                    exception_cause = e.__context__.__context__ if hasattr(e, '__context__') and e.__context__ and hasattr(e.__context__, '__context__') else None

                    # Handle Tor-specific errors
                ...
                if is_connection_error and attempt < max_connection_retries - 1:
                    proxy_url = os.environ.get('HTTPS_PROXY') or os.environ.get('HTTP_PROXY')
                    if proxy_url:
                        print(f'Connection error on attempt {attempt + 1}/{max_connection_retries}. Rotating proxy and retrying...')
                    else:
                        print(f'Connection error on attempt {attempt + 1}/{max_connection_retries}. Retrying...')
                    ...
                    # Not a connection error or out of retries - raise the exception
                    if isinstance(e, urllib3.exceptions.MaxRetryError):
                        if hasattr(e, '__context__') and isinstance(e.__context__, urllib3.exceptions.NewConnectionError):
                            msg = 'Failed to establish a connection.\n\n' + str(e)
                            raise FetchError('502', reason='Bad Gateway', error_message=msg)
                    raise
...
def fetch_url(url, headers=(), timeout=15, report_text=None, data=None,
    ...
    while True:
        ...
        if response.status == 429 or (
            ...
            # Handle proxy-based blocking (DataImpulse, etc.)
            if not use_tor:
                # Check if we have HTTP_PROXY/HTTPS_PROXY set (rotating proxy)
                proxy_url = os.environ.get('HTTPS_PROXY') or os.environ.get('HTTP_PROXY')
                if proxy_url:
                    print(f'IP blocked (429). Rotating proxy... Blocked IP: {ip}')
                    # Simply retry - get_pool() will create a new pool with rotated proxy
                    time.sleep(0.5)  # Minimal delay for speed
                    continue
                else:
