...
def time_to_seconds(time_str):
    # Handle special cases like "Upcoming", "Live", "None", etc.
    time_str_lower = str(time_str).lower()
    if time_str_lower in ['upcoming', 'live', 'none', 'n/a', '']:
        return 0
    ...
    # Try to convert to integers, return 0 if any part is not a valid integer
    try:
        parts = list(map(int, splitted_str))
    except ValueError:
        # Duration contains non-numeric values
        return 0
