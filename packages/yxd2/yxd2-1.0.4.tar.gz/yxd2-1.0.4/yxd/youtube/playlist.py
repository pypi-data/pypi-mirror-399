...
def get_playlist_page_cli(playlist_id, page):
    """CLI-friendly version that returns (video_list, num_pages, metadata) tuple"""
    if page == '1':
    ...
    if info['error']:
        raise Exception(f"Error extracting playlist: {info['error']}")
    ...
    return video_list, num_pages, metadata
