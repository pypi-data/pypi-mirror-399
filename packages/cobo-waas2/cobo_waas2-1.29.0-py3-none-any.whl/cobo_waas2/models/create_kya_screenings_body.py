...
class CreateKyaScreeningsBody(BaseModel):
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in screenings (list)
        _items = []
        if self.screenings:
            for _item in self.screenings:
                if _item:
                    _items.append(_item.to_dict())
            _dict['screenings'] = _items
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "screenings": [KyaScreeningRequest.from_dict(_item) for _item in obj["screenings"]] if obj.get("screenings") is not None else None
        })
        return _obj
