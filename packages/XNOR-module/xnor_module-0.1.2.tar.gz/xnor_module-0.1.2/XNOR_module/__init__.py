...
XKey = random.randint(10000, 99999)
...
def encode(text: str, Xkey: int, Fkey: bytes) -> bytes:
    bob = encode_via_64Xcv(text, Xkey)
    Fbob = str_to_bytes(bob)
    f = Fernet(Fkey)
    Nbob = f.encrypt(Fbob)
    return Nbob

def decode(text: bytes, Xkey: int, Fkey: bytes) -> str:
    f = Fernet(Fkey)
    bob = f.decrypt(text)
    Fbob = bytes_to_str(bob)
    Nbob = decode_via_64Xcv(Fbob, Xkey)
    return Nbob

def bob_encode(text: str, Fkey: bytes, Zkey1: int):
    for line in range(Zkey1):
        W = encode(text, line, Fkey)
        funny.append(W)
