    ...
    if force_plain or (not args.via_mpy and args.emit == "bytecode"):
        ...
        try:
            subprocess.check_output(
                [MPYCROSS] + args.mpy_cross_flags.split() + ["-o", mpy_filename, "-X", "emit=" + args.emit, script_filename],
...
def run_script_on_remote_target(pyb, args, test_file, is_special):
    had_crash, script = prepare_script_for_target(args, script_filename=test_file, force_plain=is_special)
    if had_crash:
        return True, script
    ...
    if pyb is None:
        ...
        if is_special:
            ...
            try:
                if "repl_" in test_file:
                    ...
                    with open(test_file, "rb") as f:
                        ...
                        p = subprocess.Popen(args, stdin=slave, stdout=slave, stderr=subprocess.STDOUT, bufsize=0)
                        banner = get(True)
            ...
            if args.via_mpy:
                ...
                subprocess.check_output(
                    [MPYCROSS] + args.mpy_cross_flags.split() + ["-o", mpy_filename, "-X", "emit=" + args.emit, test_file]
                )
            ...
            # run the actual test
            try:
                output_mupy = subprocess.check_output(cmdlist, stderr=subprocess.STDOUT, timeout=TEST_TIMEOUT)
            except subprocess.CalledProcessError as er:
                had_crash = True
...
def run_tests(pyb, tests, args, result_dir, num_threads=1):
    ...
    if args.target != "unix":
        ...
        if args.target == "wipy":
            ...
            skip_tests.add("extmod/utime_time_ns.py")  # RA fsp rtc function doesn't support nano sec info
