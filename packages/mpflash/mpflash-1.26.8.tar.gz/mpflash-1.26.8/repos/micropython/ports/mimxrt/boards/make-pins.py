...
class Pin(object):
    ...
    def parse_adc(self, adc_str):
        ...
        for match in matches:
            self.adc_fns.append(AdcFunction(instance=match.group("instance"), channel=match.group("channel")))
    ...
    def print_pin_af(self):
        if self.alt_fn:
            print("static const machine_pin_af_obj_t pin_{0}_af[{1}] = {{".format(self.name, len(self.alt_fn)))
    ...
    def print_pin_adc(self):
        if self.adc_fns:
            print("static const machine_pin_adc_obj_t pin_{0}_adc[{1}] = {{".format(self.name, len(self.adc_fns)))
...
class Pins(object):
            ...
            for regex in regexes:
                ...
                for match in matches:
                    if match.group("pin") not in iomux_pin_config:
                        iomux_pin_config[match.group("pin")] = {int((match.groupdict()["muxMode"].strip("U")), 16): match.groupdict()}
                    else:
                        iomux_pin_config[match.group("pin")][int((match.groupdict()["muxMode"].strip("U")), 16)] = match.groupdict()
                    ...
                    if af and af_supported(af):
                        pin.add_af(
                            AlternateFunction(
                                ...
                                int(iomux_pin_config[pin.name][af_idx]["inputDaisy"].strip("U"), 16),
                                af,
                            )
                        )
    ...
    def print_named(label, pins):
        ...
        for pin in pins:
            print("    {{ MP_ROM_QSTR(MP_QSTR_{}), MP_ROM_PTR(&pin_{}) }},".format(pin.name, pin.pad))
        print("};")
        print("MP_DEFINE_CONST_DICT(machine_pin_{:s}_pins_locals_dict, pin_{:s}_pins_locals_dict_table);".format(label, label))
        ...
        for idx, alt_fn in enumerate(pin.alt_fn):
            if name in alt_fn.instance:
                ...
                if alt_fn.instance not in module_instances:
                    module_instances[alt_fn.instance] = [format_string.format(pin.name, alt_fn.af_str, idx)]
                else:
                    module_instances[alt_fn.instance].append(format_string.format(pin.name, alt_fn.af_str, idx))
