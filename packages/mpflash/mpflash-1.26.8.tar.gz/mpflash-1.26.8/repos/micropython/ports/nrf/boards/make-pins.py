...
class AlternateFunction(object):
    ...
    def print(self):
        ...
        if fn_num is None:
            fn_num = 0
        print("({:2d}, {:8s}, {:2d}, {:10s}, {:8s}), // {:s}".format(self.idx, self.func, fn_num, self.pin_type, self.ptr(), self.af_str))
...
class Pin(object):
    ...
    def print_const_table_entry(self):
        print("  PIN({:d}, {:s}, {:s}, {:d}),".format(self.pin, self.alt_fn_name(null_if_0=True), self.adc_num_str(), self.adc_channel))
...
class Pins(object):
    ...
    def print_named(self, label, named_pins):
        ...
        for named_pin in named_pins:
            ...
            if pin.is_board_pin():
                print(
                    "  {{ MP_ROM_QSTR(MP_QSTR_{:s}), MP_ROM_PTR(&machine_board_pin_obj[{:d}]) }},".format(named_pin.name(), pin.board_index)
                )
        print("};")
    ...
    def print_adc(self, adc_num):
        ...
        for channel in range(16):
            ...
            for named_pin in self.cpu_pins:
                pin = named_pin.pin()
                if pin.is_board_pin() and (pin.adc_num & (1 << (adc_num - 1))) and (pin.adc_channel == channel):
