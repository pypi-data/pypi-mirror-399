...
def process_anpr_xml(xml_data, config, listener_url=None):
    ...
    # Log raw listener XML data
    try:
        import opscalesrv
        if opscalesrv.ENABLE_LOGGING:
            opscalesrv.log_raw_data('LISTENER', xml_data, {
                'listener_url': listener_url,
                'plate_tag': config.get('plate_tag', 'licensePlate'),
                'response_tag': config.get('response_tag', ''),
                'tolerance_minutes': config.get('tolerance_minutes', 5),
    ...
    try:
        ...
        plate_tag = config.get('plate_tag', 'licensePlate')
        country_tag = config.get('country_tag', '')
        ...
        # Extract Country Code (if country_tag is configured)
        country_code = None
        if country_tag:
            for elem in root.iter():
                if country_tag in elem.tag and elem.text:
        ...
        if not opscalesrv.verify_plate_with_country(plate, country_code, country_tag, full_config):
            logger.warning(f"ANPR Listener: Plate '{plate}' rejected by country verification (country: {country_code})")
            return
        ...
        if diff <= tolerance_min:
            ...
            listener_data['country_code'] = country_code
            ...
            listener_data['country_tag'] = country_tag
        ...
        try:
            ...
            if opscalesrv.ENABLE_LOGGING:
                opscalesrv.log_raw_data('ERROR', {
                    ...
                    'listener_url': listener_url,
                    'xml_data': xml_data[:500] if xml_data else None
                })
        except:
            pass  # Silently fail if import not available
