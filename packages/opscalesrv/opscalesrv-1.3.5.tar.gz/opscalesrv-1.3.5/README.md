...
- 🐛 **Fixed Password Protection Bug**: Fixed AttributeError when closing application with password protection enabled
- 🔧 **Improved Close Event Handling**: Better handling of application close events from both menu and window close button

### Version 1.3.1

- 🔒 **Password Protection on Exit**: Added password protection when closing the application from menu or window close button
- 🛡️ **Enhanced Security**: Application now requires password verification before closing if password is set

### Version 1.3.0

- 🔧 **Serial Port Reading Fix**: Improved serial port reading to wait for new line start before reading complete line
- 🐛 **Fixed Partial Data Issue**: Resolved issue where subsequent reads were getting partial/corrupted data
- ⚡ **Better Buffer Management**: Enhanced buffer clearing and line synchronization
- 📊 **Improved Reliability**: More consistent serial data reading across multiple requests
