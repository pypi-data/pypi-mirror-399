...
from PySide6.QtGui import QFont, QColor, QPainter, QBrush, QAction, QIcon, QTextCursor, QPixmap
...
class GeneralSettingsDialog(QDialog):
    def __init__(self, parent=None):
        ...
        # Plate Verify Button
        plate_verify_btn = QPushButton(T("btn_plate_verify_settings", "Plate Verification Settings"))
        plate_verify_btn.clicked.connect(self.open_plate_verify_settings)
        layout.addWidget(plate_verify_btn)
    ...
    def open_plate_verify_settings(self):
        """Open plate verification settings dialog"""
        dialog = PlateVerifyDialog(self)
        dialog.exec()

class PlateVerifyDialog(QDialog):
    """Dialog for managing plate verification regex patterns by country code"""
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setWindowTitle(T("title_plate_verify", "Plate Verification Settings"))
        self.resize(600, 500)
        
        layout = QVBoxLayout(self)
        
        # Info label
        info_label = QLabel(T("msg_plate_verify_info", "Define regex patterns for plate validation by country code."))
        info_label.setWordWrap(True)
        info_label.setStyleSheet("color: #888888; font-size: 11px; padding: 5px;")
        layout.addWidget(info_label)
        
        # Table for country codes and regex patterns
        self.table = QTableWidget()
        self.table.setColumnCount(2)
        self.table.setHorizontalHeaderLabels([T("lbl_country_code", "Country Code"), T("lbl_regex_pattern", "Regex Pattern")])
        ...
        # Buttons
        btn_layout = QHBoxLayout()
        self.add_btn = QPushButton(T("btn_add", "Add"))
        self.add_btn.clicked.connect(self.add_entry)
        self.edit_btn = QPushButton(T("btn_edit", "Edit"))
        self.edit_btn.clicked.connect(self.edit_entry)
        self.delete_btn = QPushButton(T("btn_delete", "Delete"))
    ...
    def load_data(self):
        """Load plate_verify config from file"""
        full_config = ConfigManager.load_config()
        plate_verify_config = full_config.get('plate_verify', {})
        
        # If empty, load defaults
        if not plate_verify_config:
            plate_verify_config = {
                "tr": "^[0-9]{2}[A-Z]{1,3}[0-9]{1,4}$",
                "en": "^[A-Z]{1,2}[0-9]{1,2}[A-Z]{1,3}[0-9]{1,3}$",
                "de": "^[A-Z]{1,3}[A-Z]{1,2}[0-9]{1,4}[H]?$",
                "fr": "^[0-9]{1,4}[A-Z]{2}[0-9]{2}$"
            }
        
        # Populate table
        self.table.setRowCount(len(plate_verify_config))
        for row, (country_code, regex_pattern) in enumerate(sorted(plate_verify_config.items())):
            self.table.setItem(row, 0, QTableWidgetItem(country_code))
            self.table.setItem(row, 1, QTableWidgetItem(regex_pattern))
    ...
    def add_entry(self):
        """Add a new country code and regex pattern"""
        dialog = PlateVerifyEditDialog(self, None)
        if dialog.exec():
            country_code, regex_pattern = dialog.get_data()
            if country_code and regex_pattern:
                # Check if country code already exists
                for row in range(self.table.rowCount()):
                    if self.table.item(row, 0) and self.table.item(row, 0).text() == country_code:
                        QMessageBox.warning(self, T("msg_error"), T("msg_country_code_exists", f"Country code '{country_code}' already exists."))
                        return
    ...
    def edit_entry(self):
        ...
        country_code = self.table.item(row, 0).text()
        regex_pattern = self.table.item(row, 1).text()
        
        dialog = PlateVerifyEditDialog(self, (country_code, regex_pattern))
        if dialog.exec():
            new_country_code, new_regex_pattern = dialog.get_data()
            if new_country_code and new_regex_pattern:
                # Check if country code already exists (if changed)
                if new_country_code != country_code:
                    for r in range(self.table.rowCount()):
                        if r != row and self.table.item(r, 0) and self.table.item(r, 0).text() == new_country_code:
                            QMessageBox.warning(self, T("msg_error"), T("msg_country_code_exists", f"Country code '{new_country_code}' already exists."))
                            return
                
                # Update row
                self.table.setItem(row, 0, QTableWidgetItem(new_country_code))
                self.table.setItem(row, 1, QTableWidgetItem(new_regex_pattern))
    ...
    def delete_entry(self):
        ...
        reply = QMessageBox.question(
            ...
            T("msg_confirm_delete_regex", f"Are you sure you want to delete regex for '{country_code}'?"),
    ...
    def save_data(self):
        """Save plate_verify config to file"""
        plate_verify_dict = {}
        
        # Collect data from table
        for row in range(self.table.rowCount()):
            country_item = self.table.item(row, 0)
            regex_item = self.table.item(row, 1)
            
            if country_item and regex_item:
                country_code = country_item.text().strip()
                regex_pattern = regex_item.text().strip()
                
                if country_code and regex_pattern:
                    plate_verify_dict[country_code] = regex_pattern
        
        # Save to config
        full_config = ConfigManager.load_config()
        full_config['plate_verify'] = plate_verify_dict
        ConfigManager.save_config(full_config)
        
        QMessageBox.information(self, T("msg_success"), T("msg_plate_verify_saved", "Plate verification settings saved."))
        self.accept()


class PlateVerifyEditDialog(QDialog):
    """Dialog for adding/editing a single plate verify entry"""
    def __init__(self, parent=None, data=None):
        super().__init__(parent)
        self.setWindowTitle(T("title_edit_regex", "Edit Regex") if data else T("title_add_regex", "Add Regex"))
        self.resize(400, 150)
        
        layout = QFormLayout(self)
        
        self.country_edit = QLineEdit()
        self.country_edit.setPlaceholderText("tr")
        layout.addRow(T("lbl_country_code", "Country Code"), self.country_edit)
        
        self.regex_edit = QLineEdit()
        self.regex_edit.setPlaceholderText("^[0-9]{2}[A-Z]{1,3}[0-9]{1,4}$")
        layout.addRow(T("lbl_regex_pattern", "Regex Pattern"), self.regex_edit)
        
        # Load data if editing
        if data:
            self.country_edit.setText(data[0])
            self.regex_edit.setText(data[1])
    ...
    def get_data(self):
        """Get country code and regex pattern from dialog"""
        return self.country_edit.text().strip(), self.regex_edit.text().strip()
...
class CameraEditDialog(QDialog):
    ...
    def __init__(self, parent=None, camera_data=None):
        ...
        # Country Code Tag (XML tag to read country code from camera)
        self.country_tag_edit = QLineEdit()
        form_layout.addRow(T("lbl_country_tag", "Country Code XML Tag"), self.country_tag_edit)
        ...
        if camera_data:
            ...
            self.country_tag_edit.setText(str(camera_data.get('country_tag', '')))
    ...
    def get_camera_data(self):
        ...
        return {
            ...
            'country_tag': self.country_tag_edit.text().strip(),
...
class ANPRListenerSettingsDialog(QDialog):
    def __init__(self, parent=None):
        ...
        # Test Button
        test_btn = QPushButton(T("btn_test_listener", "Test Listeners"))
        ...
        """)
        test_btn.clicked.connect(self.test_listeners)
    ...
    def test_listeners(self):
        """Test all listeners and show received data"""
        from . import anpr_listener
        listeners = self.working_config.get('listeners', [])
        
        if not listeners:
            QMessageBox.information(self, T("msg_info"), T("msg_no_listeners", "No listeners configured."))
            return
        ...
        # Show test dialog
        dialog = ListenerTestDialog(self, listeners, all_listener_data)
        dialog.exec()
...
class ListenerEditDialog(QDialog):
    ...
    def __init__(self, parent=None, listener_data=None):
        ...
        if listener_data:
            ...
            self.country_tag_edit.setText(str(listener_data.get('country_tag', '')))
...
class ListenerTestDialog(QDialog):
    """Dialog to test and view listener data"""
    def __init__(self, parent=None, listeners=None, listener_data=None):
        super().__init__(parent)
        self.setWindowTitle(T("title_test_listeners", "Test Listeners"))
        ...
        # Splitter for left (list) and right (content)
        splitter = QSplitter(Qt.Horizontal)
        
        # Left side - Listener list
        left_widget = QWidget()
        left_layout = QVBoxLayout(left_widget)
        left_layout.setContentsMargins(0, 0, 0, 0)
        
        list_label = QLabel(T("lbl_select_listener", "Select Listener:"))
        list_label.setStyleSheet("font-weight: bold; font-size: 12px;")
        left_layout.addWidget(list_label)
        
        self.listener_list = QListWidget()
        self.listener_list.itemSelectionChanged.connect(self.on_listener_selected)
        left_layout.addWidget(self.listener_list)
        
        splitter.addWidget(left_widget)
        
        # Right side - Content (XML and images)
        right_widget = QWidget()
        right_layout = QVBoxLayout(right_widget)
        right_layout.setContentsMargins(0, 0, 0, 0)
        
        # XML Section
        xml_group = QGroupBox(T("lbl_xml_data", "XML Data"))
        xml_layout = QVBoxLayout(xml_group)
        
        self.xml_text = QTextEdit()
        self.xml_text.setReadOnly(True)
        self.xml_text.setFont(QFont("Courier New", 10))
        self.xml_text.setStyleSheet("background-color: #1E1E1E; color: #E0E0E0;")
        xml_layout.addWidget(self.xml_text)
        
        right_layout.addWidget(xml_group)
        
        # Images Section
        images_group = QGroupBox(T("lbl_images", "Images"))
        images_layout = QVBoxLayout(images_group)
        
        # Scroll area for images
        images_scroll = QScrollArea()
        images_scroll.setWidgetResizable(True)
        images_scroll.setStyleSheet("QScrollArea { border: none; background: transparent; }")
        
        images_widget = QWidget()
        self.images_layout = QVBoxLayout(images_widget)
        ...
        btns.button(QDialogButtonBox.Close).setText(T("btn_close", "Close"))
        btns.rejected.connect(self.reject)
        layout.addWidget(btns)
        ...
        # Select first listener if available
        if self.listener_list.count() > 0:
            self.listener_list.setCurrentRow(0)
            self.on_listener_selected()
    
    def populate_listeners(self):
        """Populate the listener list"""
        self.listener_list.clear()
        for listener in self.listeners:
            name = listener.get('name', 'Unnamed')
            url = listener.get('url', '')
            enabled = "✓" if listener.get('enabled', True) else "✗"
            item_text = f"{enabled} {name} ({url})"
            item = QListWidgetItem(item_text)
            item.setData(Qt.UserRole, listener)
            self.listener_list.addItem(item)
    ...
    def on_listener_selected(self):
        ...
        listener = current.data(Qt.UserRole)
        if not listener:
            return
        ...
        # Get listener data
        data = self.listener_data.get(listener_url, {})
        
        # Show XML
        xml_content = data.get('last_received_xml') or data.get('xml_content') or ""
        if xml_content:
            self.xml_text.setText(xml_content)
        else:
            self.xml_text.setText(T("msg_no_xml_received", "No XML data received yet."))
        
        # Show images
        self.load_images(listener)
    
    def load_images(self, listener):
        """Load and display images from listener's picture path (new folder structure: Gün/Ay/Yıl/Plaka)"""
        ...
        picture_path = listener.get('picture_path', '')
        if not picture_path or not os.path.exists(picture_path):
            no_images_label = QLabel(T("msg_no_images", "No images found."))
            no_images_label.setStyleSheet("color: #888888; padding: 20px;")
            self.images_layout.addWidget(no_images_label)
            return
        
        # Get listener data to find plate
        listener_url = listener.get('url', '').rstrip('/')
        data = self.listener_data.get(listener_url, {})
        plate = data.get('plate', '')
        
        # Get image files from new folder structure: GUNAYYIL/Plaka
        ...
        try:
            ...
            # Try to find images in date-based folders
            # Structure: picture_path/GUNAYYIL/Plaka/
            date_folders = []
            
            # Check current date folder first
            current_date_folder_name = now.strftime("%d%m%Y")  # GUNAYYIL format: 15122024
            current_date_folder = os.path.join(picture_path, current_date_folder_name)
            if os.path.exists(current_date_folder):
                date_folders.append(current_date_folder)
            
            # Also check all date folders (for older images)
            if os.path.exists(picture_path):
                for folder_name in os.listdir(picture_path):
                    folder_path = os.path.join(picture_path, folder_name)
                    # Check if it's a date folder (8 digits: GUNAYYIL)
                    if os.path.isdir(folder_path) and len(folder_name) == 8 and folder_name.isdigit():
                        if folder_path not in date_folders:
                            date_folders.append(folder_path)
            
            # Search in date folders for plate folders
            for date_folder in date_folders:
                if not os.path.exists(date_folder):
                    continue
                
                # If we have a plate, search in that plate's folder
                if plate and plate != "NO_PLATE":
                    plate_folder = os.path.join(date_folder, plate)
                    if os.path.exists(plate_folder):
                        # Get images from this plate folder
                        for file in os.listdir(plate_folder):
                            if file.lower().endswith(image_extensions):
                                file_path = os.path.join(plate_folder, file)
                                mtime = os.path.getmtime(file_path)
                                image_files.append((mtime, file_path, file, plate))
                else:
                    # No plate, search in all plate folders
                    for plate_folder_name in os.listdir(date_folder):
                        plate_folder = os.path.join(date_folder, plate_folder_name)
                        if os.path.isdir(plate_folder):
                            for file in os.listdir(plate_folder):
                                if file.lower().endswith(image_extensions):
                                    file_path = os.path.join(plate_folder, file)
                                    mtime = os.path.getmtime(file_path)
                                    image_files.append((mtime, file_path, file, plate_folder_name))
            
        except Exception as e:
            error_label = QLabel(f"{T('msg_error_loading_images', 'Error loading images')}: {e}")
            error_label.setStyleSheet("color: #FF5252; padding: 20px;")
            self.images_layout.addWidget(error_label)
            return
        ...
        # Show last 20 images (increased from 10)
        for mtime, file_path, file_name, plate_name in image_files[:20]:
            try:
                # Image container
                ...
                if not pixmap.isNull():
                    ...
                    image_label.setStyleSheet("background-color: #1E1E1E; padding: 5px;")
                    image_container_layout.addWidget(image_label)
                    
                    # File name, plate, and date
                    from datetime import datetime
                    ...
                    image_container_layout.addWidget(info_label)
                    
                    self.images_layout.addWidget(image_container)
            except Exception as e:
                error_label = QLabel(f"{T('msg_error_loading_image', 'Error loading image')} {file_name}: {e}")
                error_label.setStyleSheet("color: #FF5252; padding: 5px;")
                self.images_layout.addWidget(error_label)
...
class MainWindow(QMainWindow):
    ...
    def create_menu_bar(self):
        ...
        exit_act.triggered.connect(self.request_close)
    ...
    def update_anpr_display(self, plate):
        ...
        # Add plate to scrollbar (plates list)
        # Find the listener that captured this plate by checking all listener data
        from . import anpr_listener
        all_listener_data = anpr_listener.get_all_listener_plates()
        
        # Find the listener with this plate (most recent one)
        response_tag = None
        for listener_url, data in all_listener_data.items():
            if data.get('plate') == plate and data.get('response_tag'):
                response_tag = data.get('response_tag')
                break
        
        # If no response_tag found, use 'listener' as default
        if not response_tag:
            response_tag = 'listener'
        
        # Check if plate is valid (not error codes)
        error_codes = ["NO_PLATE", "ANPR_DISABLED", "DIRECTION_DISABLED", "ANPR_ERROR", "CAM_ERROR", "ERROR", "TIMEOUT"]
        if plate and plate not in error_codes:
            # Remove existing widget for this response_tag if exists
            if response_tag in self.plate_widgets:
                old_widget = self.plate_widgets[response_tag]
                old_widget.setParent(None)
                old_widget.deleteLater()
                # Also remove from layout
                for i in range(self.plates_layout.count()):
                    item = self.plates_layout.itemAt(i)
                    if item and item.widget() == old_widget:
                        self.plates_layout.removeItem(item)
                        break
            
            # Create new plate card
            plate_card = self.create_plate_card(response_tag, plate)
            self.plates_layout.addWidget(plate_card)
            self.plate_widgets[response_tag] = plate_card
            
            # Remove "No Plate" message if exists
            for i in range(self.plates_layout.count()):
                item = self.plates_layout.itemAt(i)
                if item and item.widget():
                    widget = item.widget()
                    if isinstance(widget, QLabel) and widget.text() == T("lbl_no_plate"):
                        self.plates_layout.removeWidget(widget)
                        widget.deleteLater()
                        break
    ...
    def update_ui_from_request(self, data):
        ...
        try:
            ...
            for key, value in msg_dict.items():
                ...
                if key not in ['value', 'msg', 'mode', 'result', 'isapi_result'] and value:
            ...
            # Always check 'plate' field (for test mode and legacy support)
            if plate_val and plate_val not in ["NO_PLATE", "ANPR_DISABLED", "DIRECTION_DISABLED", "ANPR_ERROR", "CAM_ERROR", "ERROR", "TIMEOUT"]:
                # Use 'plate' as key if not already in plates_found
    ...
    def request_close(self):
        """Request to close the application - checks password if set"""
        if self.check_password():
            # Password verified, proceed with closing
            # Stop server thread before closing
            if hasattr(self, 'server_thread') and self.server_thread.is_alive():
                opscalesrv.stop_server()
                # Wait for thread to finish (threading.Thread doesn't have quit(), just stop server)
                self.server_thread.join(timeout=3.0)  # Wait up to 3 seconds
            # Close the application
            QApplication.instance().quit()
    
    def closeEvent(self, event):
        """Override closeEvent to check password before closing"""
        # Check if password is set
        config = ConfigManager.load_config()
        stored_hash = config.get('settings', {}).get('password')
        
        if not stored_hash:
            # No password set, allow closing
            if hasattr(self, 'server_thread') and self.server_thread.is_alive():
                opscalesrv.stop_server()
        ...
        if self.check_password():
            # Password verified, stop server and close
            if hasattr(self, 'server_thread') and self.server_thread.is_alive():
                opscalesrv.stop_server()
                self.server_thread.join(timeout=3.0)
            event.accept()
        else:
            # Password check failed or cancelled, prevent closing
            event.ignore()
