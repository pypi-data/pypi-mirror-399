...
# Serial cache for server URL requests (to avoid duplicate reads)
SERIAL_CACHE = {
    'value': None,
    'msg': None,
    'result': None,
    'timestamp': None,
    'cache_duration': 2.0  # Cache valid for 2 seconds
}
...
def get_serial_response(plate="NO_PLATE", include_plate=False):
    ...
    """
    global SERIAL_STATUS, SERIAL_CACHE
    import time
    ...
    try:
        ...
        # Update cache
        SERIAL_CACHE['value'] = value
        SERIAL_CACHE['msg'] = msg
        SERIAL_CACHE['result'] = result
        SERIAL_CACHE['timestamp'] = time.time()
        ...
        SERIAL_CACHE['value'] = -1
        SERIAL_CACHE['msg'] = str(e)
        SERIAL_CACHE['result'] = "FAIL"
        SERIAL_CACHE['timestamp'] = time.time()
    ...
    def do_GET(self):
        ...
        try:
            ...
            if not should_process:
                logger.info(f"Response processing already in progress for path '{processing_key}', waiting...")
                ...
                if processing_info['event'].wait(timeout=30.0):
                    ...
                    if result_data and response_bytes:
                        logger.info(f"Using cached response for path '{processing_key}'")
                        ...
                        logger.info("Response sent successfully (from cache)")
                        return
                    else:
                        logger.warning("Cached response not available, processing new request")
