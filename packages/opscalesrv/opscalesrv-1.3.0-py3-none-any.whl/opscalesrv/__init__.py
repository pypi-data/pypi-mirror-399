...
# Thread-safe mechanism for concurrent requests - wait for full response
# Key: path, Value: {'lock': Lock, 'in_progress': bool, 'event': Event, 'result': dict}
RESPONSE_PROCESSING = {}
RESPONSE_PROCESSING_LOCK = threading.Lock()

def verify_plate_with_country(plate, country_code, country_tag, config):
    """
    Verify plate using country-specific regex if country_tag is defined and regex exists.
    Returns True if plate should be accepted, False if it should be rejected.
    
    Logic:
    - If country_tag is not defined: accept plate (True)
    - If country_tag is defined but country_code is None: accept plate (True)
    - If country_tag is defined and country_code exists:
      - If regex for country_code exists in config: verify plate with regex
      - If regex for country_code does not exist: accept plate (True)
    """
    # If country_tag is not defined, accept plate directly
    if not country_tag:
        return True
    
    # If country_tag is defined but country_code is None, accept plate
    if not country_code:
        return True
    
    # Get plate_verify config
    plate_verify_config = config.get('plate_verify', {})
    
    # Check if regex exists for this country_code
    regex_pattern = plate_verify_config.get(country_code.lower())
    
    # If no regex defined for this country, accept plate
    if not regex_pattern:
        return True
    
    # Verify plate with regex
    try:
        if re.match(regex_pattern, plate):
            logger.info(f"Plate '{plate}' verified successfully for country '{country_code}' with pattern '{regex_pattern}'")
            return True
        else:
            logger.warning(f"Plate '{plate}' failed verification for country '{country_code}' with pattern '{regex_pattern}'")
            return False
    except Exception as e:
        logger.error(f"Error verifying plate '{plate}' with regex '{regex_pattern}': {e}")
        # On error, accept plate to avoid blocking valid plates due to regex errors
        return True
...
def log_raw_data(category, data, metadata=None):
    """
    Log raw data to requests.log file (only if logging is enabled)
    category: 'REQUEST', 'SERIAL', 'LISTENER', 'QUERY', 'ERROR', 'RESPONSE'
    data: Raw data to log (string, bytes, dict, etc.)
    ...
    if metadata:
        log_entry += f"METADATA: {json.dumps(metadata, indent=2, ensure_ascii=False)}\n"
    ...
    if isinstance(data, bytes):
        try:
            ...
            log_entry += f"RAW BYTES (hex): {data.hex()}\n"
            log_entry += f"RAW BYTES (repr): {repr(data)}\n"
            try:
                decoded = data.decode('utf-8', errors='replace')
                log_entry += f"DECODED (utf-8): {decoded}\n"
            except:
                pass
        except Exception as e:
            log_entry += f"RAW BYTES (error): {str(e)}\n"
    elif isinstance(data, dict):
        try:
            log_entry += f"DATA (JSON):\n{json.dumps(data, indent=2, ensure_ascii=False)}\n"
        except Exception as e:
            log_entry += f"DATA (str): {str(data)}\n"
    else:
        log_entry += f"DATA:\n{str(data)}\n"
    ...
    try:
        ...
        logger.error(f"Error writing raw data to log file: {e}")
...
def read_serial_data_once():
    ...
    try:
        ...
        # Ensure minimum timeout for reliable reading (at least 2 seconds)
        configured_timeout = optional_params.get('timeout', 1.0)
        if configured_timeout is not None and configured_timeout < 2.0:
            optional_params['timeout'] = 2.0
            logger.debug(f"Timeout increased to 2.0s for reliable reading (was {configured_timeout})")
        elif configured_timeout is None:
            optional_params['timeout'] = 2.0
            logger.debug("Timeout set to 2.0s (default)")
        ...
        with serial.Serial(**serial_params) as ser:
            ...
            # Clear input buffer to remove any stale/partial data from previous reads
            try:
                ...
                try:
                    ser.flushInput()
                    logger.debug("Input buffer flushed (legacy method)")
                except:
                    logger.warning(f"Could not clear input buffer: {e}")
            
            # Small delay to allow port to stabilize after opening
            import time
            time.sleep(0.2)
            
            # Wait for a new line start: read and discard any partial data until we get a newline
            # This ensures we start reading from the beginning of a new line
            logger.debug("Waiting for new line start...")
            start_time = time.time()
            timeout_seconds = 1.5  # Max 1.5 seconds to find newline
            ...
            while time.time() - start_time < timeout_seconds:
                ...
                if not char:
                    ...
                    time.sleep(0.05)
                    continue
                # Check if we hit a newline character (start of new line)
                if char in (b'\n', b'\r'):
                    # Found newline, read the rest of the line if it's \r\n
                    if char == b'\r':
                        next_char = ser.read(1)
                        if next_char == b'\n':
                            # \r\n found, we're at the start of a new line
                            logger.debug("New line start detected (\\r\\n)")
                            break
                        elif next_char:
                            # \r but not followed by \n, we're at start of new line
                            logger.debug("New line start detected (\\r)")
                            break
                    else:
                        # \n found, we're at the start of a new line
                        logger.debug("New line start detected (\\n)")
                        break
                # If we read something that's not a newline, continue waiting
                # This discards partial data from previous reads
            
            # Now read the complete line
            logger.debug("Reading complete line...")
            raw_data = ser.readline()
            
            logger.debug(f"Raw data received: {raw_data} (length: {len(raw_data)})")
            
            # Log raw serial data
            log_raw_data('SERIAL', raw_data, {
                'port': port,
                'baudrate': baudrate,
                'bytesize': bytesize,
                'encoding': ENCODING,
            ...
            if data:
                ...
                log_raw_data('ERROR', "No data received from serial port (empty string)", {'port': port})
        ...
        log_raw_data('ERROR', {
            'error_type': 'UnicodeDecodeError',
            'error_message': str(e),
            'raw_data_hex': raw_data.hex() if 'raw_data' in locals() else None,
            'raw_data_repr': repr(raw_data) if 'raw_data' in locals() else None
        }, {'port': port})
...
def get_test_response(path=None):
    ...
    """
    # Always return a test plate value
    plate = "34ABC123"
...
def get_serial_response(plate="NO_PLATE", include_plate=False):
    ...
    try:
        ...
        # Update SERIAL_STATUS based on read result
        if success:
            SERIAL_STATUS = True
        else:
            SERIAL_STATUS = False
        ...
        elif not success or (value == 0 and any(error_keyword in msg.lower() for error_keyword in ['error', 'failed', 'exception', 'timeout', 'no data'])):
            result = "FAIL"
...
class SerialServerHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        try:
            ...
            # Thread-safe: Check if response is being processed for this path
            # If yes, wait for it and return the same response
            global RESPONSE_PROCESSING, RESPONSE_PROCESSING_LOCK
            processing_key = path_check  # Use normalized path as key
            
            with RESPONSE_PROCESSING_LOCK:
                if processing_key not in RESPONSE_PROCESSING:
                    # First request for this path - we will process it
                    RESPONSE_PROCESSING[processing_key] = {
                        'lock': threading.Lock(),
                        'in_progress': True,
                    ...
                    # Another request is processing this path
                    should_process = False
                processing_info = RESPONSE_PROCESSING[processing_key]
            
            # If another request is processing, wait for it
            if not should_process:
                logger.info(f"Response processing already in progress for path '{processing_key}', waiting for concurrent request to complete...")
                # Wait for the response to be ready (with timeout)
                if processing_info['event'].wait(timeout=30.0):
                    # Response is ready, get it
                    with processing_info['lock']:
                        result_data = processing_info['result']
                        response_bytes = processing_info['response_bytes']
                    
                    if result_data and response_bytes:
                        logger.info(f"Using response from concurrent request for path '{processing_key}'")
                        ...
                        # Log request
                        log_request(self.client_address[0], self.client_address[1], 'GET', self.path, 'ACCEPTED', len(response_bytes), self.headers.get('User-Agent', ''), result_data)
                        logger.info("Response sent successfully (from concurrent request)")
                        return
                    else:
                        logger.warning("Concurrent response not available, processing new request")
                        # Fall through to process normally
                else:
                    logger.warning("Timeout waiting for concurrent response, processing new request")
                    # Clean up on timeout
                    with RESPONSE_PROCESSING_LOCK:
                        if processing_key in RESPONSE_PROCESSING:
                            del RESPONSE_PROCESSING[processing_key]
                    # Fall through to process normally
            ...
            if ON_START_CALLBACK:
                try:
                    ...
                    log_raw_data('ERROR', {
                        ...
                        'error_message': str(e),
                        'callback': 'ON_START_CALLBACK'
                    }, {'client_ip': client_ip, 'path': self.path})
            ...
            if TEST_MODE or force_test_mode:
                ...
                # In test mode, set both result and isapi_result to OK
                message_data['result'] = "OK"
                message_data['isapi_result'] = "OK"
                
                # Skip all ANPR processing in test mode
                ...
                # Store response for concurrent requests and notify waiting threads
                with processing_info['lock']:
                    processing_info['result'] = response_data
                    processing_info['response_bytes'] = response_bytes
                    processing_info['in_progress'] = False
                processing_info['event'].set()
                
                # Clean up entry immediately after notifying waiting threads
                # Waiting threads will wake up immediately when event is set
                def cleanup_entry():
                    import time
                    time.sleep(0.1)  # Very short delay (100ms) for waiting threads to wake up and get response
                    with RESPONSE_PROCESSING_LOCK:
                        if processing_key in RESPONSE_PROCESSING:
                            del RESPONSE_PROCESSING[processing_key]
                            logger.debug(f"Cleaned up processing entry for path '{processing_key}'")
                
                threading.Thread(target=cleanup_entry, daemon=True).start()
                
                # Log raw response
                log_raw_data('RESPONSE', response_bytes, {
                    'client_ip': self.client_address[0],
                    'client_port': self.client_address[1],
                    'path': self.path,
                    'status_code': 200,
                ...
                logger.info("Test mode response sent successfully")
                ...
                # If a read is already in progress, concurrent requests will wait and get the same result
                # Read serial data directly (don't include plate in normal mode)
                        ...
                        if enabled_cameras:
                            ...
                            with concurrent.futures.ThreadPoolExecutor(max_workers=len(enabled_cameras)) as executor:
                                ...
                                for response_tag, future in camera_futures.items():
                                    try:
                                        license_plate, cam_status, plate_tag, country_code = future.result(timeout=ANPR_CONFIG.get('timeout', 10) + 5)
                                        # Define error codes
                                        error_codes = ["ANPR_ERROR", "CAM_ERROR", "CAM_TIMEOUT", "CAM_CONNECTION_ERROR", "ERROR", "TIMEOUT"]
                                        
                                        # Verify plate with country code if applicable (already done in fetch_from_camera, but log here)
                                        if country_code:
                                            logger.debug(f"Camera '{response_tag}' returned country_code: {country_code}")
                                        ...
                                        if license_plate:
                                            if license_plate in error_codes:
                                                ...
                                                # Valid plate
                                                plates[response_tag] = license_plate
                                                logger.info(f"Camera '{response_tag}' returned plate: {license_plate} (country: {country_code})")
                                        else:
                                        ...
                                        log_raw_data('ERROR', {
                                            ...
                                            'camera': response_tag,
                                            'timeout': ANPR_CONFIG.get('timeout', 10) + 5,
                                            'traceback': traceback.format_exc()
                                        })
            ...
            # Log any unhandled exception in do_GET
            import traceback
            ...
            try:
                if processing_key and processing_key in RESPONSE_PROCESSING:
                    processing_info = RESPONSE_PROCESSING[processing_key]
                    with processing_info['lock']:
                        processing_info['in_progress'] = False
                    processing_info['event'].set()
                    # Remove entry immediately on error
                    with RESPONSE_PROCESSING_LOCK:
                        if processing_key in RESPONSE_PROCESSING:
                            del RESPONSE_PROCESSING[processing_key]
            except:
            ...
            try:
                ...
                pass  # If we can't send error response, just fail silently
    ...
    def do_POST(self):
        ...
        if matched_listener:
             try:
                ...
                # Log incoming POST request with raw data
                request_headers = dict(self.headers)
                log_raw_data('REQUEST', {
                    'method': 'POST',
                    'path': self.path,
                ...
                xml_content = None
                extracted_plate = None  # Plate extracted from XML for folder structure
                image_parts = []  # Store image parts to save after XML is parsed
                ...
                if 'multipart/form-data' in content_type:
                    try:
                        ...
                        # First pass: Extract XML and collect image parts
                        for part in msg.get_payload():
                            if not part.is_multipart():
                                ...
                                if 'xml' in part_ctype or (part_filename and part_filename.endswith('.xml')):
                                    ...
                                    # Extract plate from XML for folder structure
                                    try:
                                        import xml.etree.ElementTree as ET
                                        root = ET.fromstring(xml_content)
                                        plate_tag = matched_listener.get('plate_tag', 'licensePlate')
                                        for elem in root.iter():
                                            if plate_tag in elem.tag and elem.text:
                                                extracted_plate = elem.text.strip()
                                                break
                                    except Exception as e:
                                        logger.warning(f"Could not extract plate from XML: {e}")
                                
                                # Identify Images - store for later saving
                                elif 'image' in part_ctype or (part_filename and part_filename.lower().endswith(('.jpg', '.jpeg', '.png', '.bmp', '.gif'))):
                                    image_parts.append({
                        ...
                        # Log raw POST data (without image data)
                        # Create a log-safe version of raw_data (without images)
                        log_data = raw_data
                        if image_parts:
                            # For logging, we'll just note that images were present
                            log_raw_data('LISTENER', {
                                'listener_url': matched_listener.get('url', ''),
                                'content_type': self.headers.get('Content-Type', ''),
                                'content_length': content_length,
                                'client_ip': self.client_address[0],
                                'note': 'Multipart data contains images (not logged)',
                                'xml_length': len(xml_content) if xml_content else 0,
                                'image_count': len(image_parts)
                            })
                        else:
                            # No images, safe to log raw data
                            log_raw_data('LISTENER', raw_data, {
                                'listener_url': matched_listener.get('url', ''),
                                'content_type': self.headers.get('Content-Type', ''),
                                'content_length': content_length,
                                'client_ip': self.client_address[0]
                        ...
                        # Second pass: Save images with proper folder structure
                        save_dir = matched_listener.get('picture_path')
                        if save_dir and image_parts:
                            # Use extracted plate or fallback to "UNKNOWN"
                            plate_folder = extracted_plate if extracted_plate and extracted_plate != "NO_PLATE" else "UNKNOWN"
                            
                            # Create date-based folder structure: GUNAYYIL/Plaka
                            now = datetime.now()
                            date_folder_name = now.strftime("%d%m%Y")  # GUNAYYIL format: 15122024
                            date_folder = os.path.join(save_dir, date_folder_name)
                            plate_folder_path = os.path.join(date_folder, plate_folder)
                            
                            # Create directories
                            os.makedirs(plate_folder_path, exist_ok=True)
                            
                            # Save each image
                            for img_part in image_parts:
                                try:
                                    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S_%f")
                                    clean_name = img_part['filename'] or "unknown.jpg"
                                    # Sanitize filename
                                    clean_name = clean_name.replace('/', '_').replace('\\', '_')
                                    final_path = os.path.join(plate_folder_path, f"{timestamp}_{clean_name}")
                                    
                                    with open(final_path, 'wb') as f:
                                        f.write(img_part['payload'])
                                    logger.info(f"Saved ANPR image: {final_path}")
                                except Exception as e:
                                    logger.error(f"Error saving image {img_part.get('filename', 'unknown')}: {e}")
                    except Exception as e:
                        ...
                        # Log raw data (without images if possible)
                        log_raw_data('LISTENER', {
                            'listener_url': matched_listener.get('url', ''),
                            'content_type': self.headers.get('Content-Type', ''),
                            'content_length': content_length,
                ...
                if ANPR_LISTENER_AVAILABLE and xml_content:
                    ...
                    # Log processed XML content
                    log_raw_data('LISTENER', xml_content, {
                    ...
                    # Log if XML content exists but listener not available
                    log_raw_data('ERROR', {
                        'error_type': 'ListenerNotAvailable',
                        'message': 'ANPR_LISTENER_AVAILABLE is False but xml_content exists',
                        'xml_content': xml_content[:500]  # First 500 chars
                    })
