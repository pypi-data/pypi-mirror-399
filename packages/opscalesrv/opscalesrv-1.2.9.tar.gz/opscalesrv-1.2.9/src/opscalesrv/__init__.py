...
def read_serial_data_once():
    ...
    try:
        ...
        with serial.Serial(**serial_params) as ser:
            ...
            # Read data - readline() will wait for newline or timeout
            # Try to read a complete line, with retries if data seems incomplete
            raw_data = b''
            max_retries = 2
            retry_count = 0
            ...
            while retry_count <= max_retries:
                ...
                if line:
                    raw_data += line
                    # Check if we have a reasonable amount of data (at least 5 characters)
                    # or if line ends with newline/return
                    if len(raw_data) >= 5 and (raw_data.endswith(b'\n') or raw_data.endswith(b'\r\n') or raw_data.endswith(b'\r')):
                        break
                    # If we have some data but it seems incomplete, wait a bit more
                    if len(raw_data) > 0 and len(raw_data) < 5:
                        time.sleep(0.1)
                        retry_count += 1
                    else:
                        break
                else:
                    # No data received, break if we already have something
                    if raw_data:
                        break
                    retry_count += 1
            ...
            # If we still don't have enough data, try one more read
            if len(raw_data) < 3:
                additional = ser.readline()
                if additional:
                    raw_data += additional
