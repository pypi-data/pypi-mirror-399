...
class BranchStatus(int, Enum):
    ...
    - DRAFT: Main development branch (one per repo)
...
BRANCH_STATUS_CONTEXT: dict[int, str] = {
    ...
    2: "Previous published release - superseded by newer Active version",
...
# Human-readable names for application types
APPLICATION_TYPE_NAMES: dict[int, str] = {
    1: "Web",
    2: "Mobile",
    3: "ComponentModule",
    6: "API",
...
APPLICATION_TYPE_ALIASES: dict[str, int] = {
    "web": 1,
    "mobile": 2,
    "componentmodule": 3,
    "api": 6,
...
class LocksDto(BaseModel):
    ...
    operations_and_roles: LockedAssetDto | None = Field(
        alias="operationsAndRoles", default=None
    )
...
class MarketplaceApplicationDto(BaseModel):
    ...
    application_definition_id: int | None = Field(
        alias="applicationDefinitionId", default=None
    )
...
class OrganizationOutput(BaseModel):
    """Simplified organization info for output."""
    ...
    def from_api(cls, data: dict) -> "OrganizationOutput | None":
        ...
        if not data:
            return None
        return cls(id=data.get("id", 0), name=data.get("name", ""))
