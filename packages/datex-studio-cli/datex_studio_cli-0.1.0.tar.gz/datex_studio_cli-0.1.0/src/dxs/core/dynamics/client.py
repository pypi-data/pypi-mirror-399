...
def get_dynamics_token() -> str:
    ...
    if cached and cached.refresh_token:
        try:
            # Manually use MSAL to refresh with Dynamics scopes
    ...
    raise AuthenticationError(
        ...
        suggestions=[
            "Run 'dxs auth login' to authenticate and grant Dynamics CRM API consent",
            "Ensure your account has Dynamics CRM / Dataverse access",
            "Check that the app registration has the Dynamics CRM API permission",
        ],
    )
...
class DynamicsClient:
    ...
    def _handle_response(self, response: httpx.Response) -> Any:
        ...
        if response.status_code >= 400:
            ...
            try:
                ...
                message = error_info.get("message", f"Dynamics CRM API error: {response.status_code}")
            except Exception:
    ...
    def get(self, path: str, params: dict[str, Any] | None = None, page_size: int | None = None) -> Any:
    ...
    def get_by_next_link(self, url: str) -> dict[str, Any]:
        ...
        try:
            ...
            raise ConfigurationError(
                message=f"Cannot connect to Dynamics CRM",
                code="DXS-DYNAMICS-CONN",
                details={"url": url, "error": str(e)},
                suggestions=[
                    "Check your network connection",
