...
def _log_batch(
    ...
    for branch_id in branch_ids:
        ...
        try:
            ...
            branch_name = (
                branch_info.get("name")
                or branch_info.get("referenceName")
...
def _extract_code_diff(
    old_content: dict | None, new_content: dict | None
) -> str | None:
    ...
    for i, item in enumerate(sorted_history):
        ...
        if config_id:
            try:
                ...
                if config_type:
                    endpoint = (
                        f"/applications/{item_app_id}/"
                        f"{config_type}configurations/{config_id}"
                    )
...
def _add_diffs_to_changes(
    ...
    for change in config_changes:
        ...
        if mod_type == "add" or not base_config_id:
            try:
                endpoint = _config_endpoint(
                    feature_branch_id, config_type, feature_config_id
                )
        ...
        try:
            ...
            feature_endpoint = _config_endpoint(
                feature_branch_id, config_type, feature_config_id
            )
            ...
            base_endpoint = _config_endpoint(
                base_branch_id, config_type, base_config_id
            )
...
def _get_committed_diff(
    ...
    current_endpoint = _config_endpoint(
        current_app_id, config_type, current_config_id
    )
    current_content = client.get(current_endpoint)
    current_content = _extract_config_content(current_content)

    previous_endpoint = _config_endpoint(
        previous_app_id, config_type, previous_config_id
    )
...
@pass_context
@require_auth
def diff(ctx: DxsContext, branch: int | None) -> None:
    """Show upstream changes (changes in base branch).

    Displays changes available in the base branch that haven't been pulled.
    Useful for understanding what changes are available to merge.
...
@pass_context
@require_auth
def changes(ctx: DxsContext, branch: int | None, with_diffs: bool) -> None:
    """Show pending changes in feature branch.

    Displays changes in the current feature branch compared to its base branch.
    Useful for generating commit messages.

    \b
    Options:
        --with-diffs  Fetch and include actual JSON diffs for each change
...
def _build_graph_adjacency_map(
    ...
    for dep in deps:
        ...
        try:
            ...
            if child_deps:
                child_adjacency = _build_graph_adjacency_map(
                    client, child_deps, ctx, visited
                )
...
def _find_graph_overlapping_deps(
    ...
    dep_info = {
        dep.get("marketPlaceApplicationVersionId"): dep
        for dep in all_deps
    }
    ...
    overlapping.sort(key=lambda x: (-x["occurrences"], x["name"] or ""))
    return overlapping
...
@click.option(
    ...
    type=int,
    required=True,
    help="Source marketplace version ID",
)
@click.option(
    "--to",
    "to_version",
    type=int,
    required=True,
    help="Target marketplace version ID",
)
@pass_context
@require_auth
def compare(ctx: DxsContext, from_version: int, to_version: int) -> None:
    """Compare changes between two published versions.

    Shows commits, configuration changes, work items, and dependency
    changes between two MarketplaceApplicationVersion IDs. Useful for
    generating release notes.

    \b
    Options:
        --from ID   Source version ID (older)
        --to ID     Target version ID (newer)

    \b
    Example:
        dxs source compare --from 100 --to 105
    """
    client = ApiClient()

    # Get version details
    ctx.log("Fetching version details...")
    from_ver = client.get(MarketplaceEndpoints.version(from_version))
    to_ver = client.get(MarketplaceEndpoints.version(to_version))

    from_date = from_ver.get("releaseDate")
    to_date = to_ver.get("releaseDate")

    # Get commits in range
    app_def_id = from_ver.get("applicationDefinitionId") or to_ver.get("applicationDefinitionId")
    commits_in_range = []

    if app_def_id:
        ctx.log("Fetching commit history...")
        history = client.get(SourceControlEndpoints.history(app_def_id))
        if isinstance(history, list):
            commits_in_range = [
                c for c in history
                if from_date and to_date and from_date < c.get("commitDate", "") <= to_date
            ]

    # Get dependency changes
    from_app_id = from_ver.get("applicationId")
    to_app_id = to_ver.get("applicationId")

    dep_changes = {"added": [], "removed": [], "updated": []}
    if from_app_id and to_app_id:
        ctx.log("Comparing dependencies...")
        try:
            from_deps = client.get(DependencyEndpoints.list(from_app_id))
            to_deps = client.get(DependencyEndpoints.list(to_app_id))
            ...
            for ref, dep in to_by_ref.items():
                if ref not in from_by_ref:
                    ...
                    from_ver = from_by_ref[ref].get("marketPlaceApplicationVersionId")
    ...
    if to_app_id:
        try:
            workitems_data = client.get(WorkitemEndpoints.list(to_app_id))
            if not isinstance(workitems_data, list):
                workitems_data = [workitems_data] if workitems_data else []
    ...
    ctx.output(
        single(
            item={
                "from_version": {
                    "id": from_ver.get("id"),
                    "version_code": from_ver.get("versionCode"),
                    "version_name": from_ver.get("versionName"),
                    "release_date": from_date,
                },
                "to_version": {
                    "id": to_ver.get("id"),
                    "version_code": to_ver.get("versionCode"),
                    "version_name": to_ver.get("versionName"),
            ...
            semantic_key="version_comparison",
...
def graph(
    ...
    if flat:
        ...
        tree = _build_graph_tree_with_paths(
            direct_deps, adjacency, all_paths, [root_name],
