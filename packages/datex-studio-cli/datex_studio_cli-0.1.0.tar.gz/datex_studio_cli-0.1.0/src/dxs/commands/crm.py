...
def _download_attachments(
    ...
    for incident in incidents:
        ...
        if url_to_path:
            incident["description"] = _replace_attachment_urls(
                incident["description"], url_to_path
            )
...
def search(
    ...
    """
    # Apply command-level global options to context
    from dxs.core.output import OutputFormat
    ...
    if output_format is not None:
        ctx.output_format = OutputFormat(output_format.lower())
        ctx.explicit_output_format = True
    if verbose:
        ctx.verbose = True
    if quiet:
        ctx.quiet = True
...
def query(
    ...
    if save_attachments:
        ...
        attachments_downloaded = _download_attachments(client, formatted_cases, save_attachments, ctx)
    ...
    # Build response
    if attachments_downloaded:
        ctx.output(list_response(items=formatted_cases, semantic_key="cases", attachments=attachments_downloaded))
    else:
...
@require_auth
def account_list(
    ctx, limit: int | None, batch_size: int | None, count_only: bool
) -> None:
...
@require_auth
def search(
    ctx, query: str, limit: int | None, batch_size: int | None, count_only: bool
) -> None:
...
def _get_localized_label(obj: dict[str, Any] | None) -> str | None:
    ...
    if localized_labels:
        return localized_labels[0].get("Label")
    return None


def _list_entities(ctx) -> None:
...
def _get_entity_metadata(ctx, entity_name: str, relationships: bool) -> None:
...
@click.pass_obj
@require_auth
def metadata(ctx, entity_name: str | None, relationships: bool) -> None:
