"""Explore commands: dxs source explore [configs|config|summary|trace|all|info|graph]."""
...
def _extract_type_summary(config: dict[str, Any], config_type: str) -> dict[str, Any]:
    ...
    if config_type == "grid":
        ...
        return {
            "fields": [{"id": f.get("id"), "control": f.get("control"), "label": f.get("label")} for f in json_content.get("fields", [])],
        ...
        for s in json_content.get("sections", []):
            sections.append({"id": s.get("id"), "title": s.get("title"), "fields": [f.get("id") for f in s.get("fields", [])]})
        return {"datasource": json_content.get("datasource"), "sections": sections, "save_flow": json_content.get("onSave")}
        ...
        return {
            ...
            "return_fields": [f.get("id") for f in json_content.get("returnParams", []) if isinstance(f, dict)],
        }
    elif config_type == "selector":
        return {"datasource": json_content.get("datasource"), "value_field": json_content.get("valueField"), "display_field": json_content.get("displayField")}
        ...
        menubar = [{"id": m.get("id"), "label": m.get("label"), "items": [i.get("label") for i in m.get("items", []) if isinstance(i, dict)]} for m in json_content.get("menubar", [])]
        return {"home": json_content.get("home"), "menubar": menubar, "toolbar": _extract_toolbar_actions(json_content.get("toolbar"))}
        ...
        if json_content.get("enumTypeDef"):
            result["enumValues"] = [
                e.get("id") for e in json_content.get("enumTypeDef", [])
...
def _extract_all_references(
    ...
    json_content = _get_json_content(config)
    refs: dict[str, set[str]] = {"datasources": set(), "private_frontend_flows": set(), "forms": set(), "editors": set(), "selectors": set(), "grids": set(), "hubs": set(), "dialogs": set(), "widgets": set()}
    ...
    def _scan(obj: Any) -> None:
        if isinstance(obj, dict):
            ...
            for key in ["onClick", "onSubmit", "onCancel", "onSave", "flow", "onRowDoubleClick", "flowId"]:
                if key in obj and isinstance(obj[key], str):
...
def explore() -> None:
    ...
    Examples:
        ...
        dxs source explore all --branch 100 --output-dir ./export
    """
    pass
...
def info(ctx: DxsContext, branch: int | None) -> None:
    ...
    try:
        ...
        references = [ref.get("referenceName") for ref in references_data if ref.get("referenceName")]
    except Exception:
        references = []
    ...
    output = {
        "application": {
            ...
            "type": "Library" if app_def.get("applicationTypeId") == 3 else "Application",
...
@click.option("--search", "-s", type=str, help="Search by reference name or label (case-insensitive)")
@click.option("--limit", "-n", type=int, default=0, help="Limit results (0=unlimited)")
@click.option("--owned-only", is_flag=True, help="Only show configs owned by this branch (exclude referenced)")
@pass_context
@require_auth
def configs(
    ctx: DxsContext,
    ...
    ctx.output(list_response(items=all_configs, semantic_key="configurations", **metadata_kwargs, **cache_info))
...
def config(
    ...
    if not config_type:
        ...
        if lookup:
            ...
            if found_id:
                ctx.log(f"Found {found_type} config with ID {found_id} via index, fetching content...")
    ...
    if not found_config:
        ...
        for ctype in types_to_search:
            ...
            try:
                configs_list = client.get(ConfigurationEndpoints.list_all(branch_id, normalized_type))
                ...
                if found_id:
                    ctx.log(f"Found {normalized_type} config with ID {found_id}, fetching content...")
                    found_config = client.get(
    ...
    if raw:
        content = found_config.get("config") or found_config.get("json") or found_config.get("json_content") or {}
        cache_info = end_cache_session() or {}
...
def _trace_single_file(
    ...
    if base_dir:
        ...
        try:
            ...
            references = _enrich_with_library(references, config_index, library_branches, main_branch_id)
        except Exception:
...
def _trace_batch(
    ...
    """Batch trace all YAML files in a directory structure.

    Walks the document build directory, traces each config file,
    and saves results alongside as <name>.trace.yaml.
    Always includes library info for each reference.
    ...
    # Scan filesystem for configs (excluding .trace.yaml files)
    ctx.log("Scanning filesystem for configs...")
    all_configs = _scan_config_directory(directory)
    configs = [(p, r, t, lib, b) for p, r, t, lib, b in all_configs if not p.name.endswith(".trace.yaml")]
    total_files = len(configs)
    ctx.log(f"Found {total_files} config files (excluded {len(all_configs) - total_files} .trace.yaml files)")
    ...
    ctx.log("Processing configs...")
    for i, (path, ref_name, config_type, _lib_name, _branch_id) in enumerate(configs):
        # Progress reporting
        ...
        try:
            ...
            # Always enrich with library info
            references = _enrich_with_library(
                references, config_index, library_branches, main_branch_id
            )
            ...
            # Write trace file alongside original
            trace_path = path.with_suffix(".trace.yaml")
            with open(trace_path, "w") as f:
                yaml.dump(trace_output, f, default_flow_style=False, allow_unicode=True, sort_keys=False)
...
def trace(
    ...
    if offline:
        if not reference:
            raise ValidationError(
                ...
                suggestions=["Provide full path: dxs source explore trace ./path/to/config.yaml --offline"],
            )
    ...
    references = _get_references_cached(
    ...
    references = _enrich_with_library(
        references, config_index, library_branches, branch_id
    )
    ...
    ctx.output(single(item=output, semantic_key="trace", branch_id=branch_id, mode="api", **cache_info))
...
@click.option("--full", is_flag=True, help="Include full config content (not just listings)")
@click.option("--include-summaries", is_flag=True, help="Generate summary files alongside configs")
@click.option("--owned-only", is_flag=True, help="Only export configs owned by this branch")
@pass_context
@require_auth
def all_configs(
    ctx: DxsContext,
    ...
    Organizes output by ownership (owned vs referenced) and creates one file
    per configuration for easy reading. Each config type gets its own subdirectory.

    \b
    Examples:
        dxs source explore all --branch 63393 --output-dir ./exploration
        dxs source explore all --branch 100 -o ./dump --full
        dxs source explore all --branch 100 -o ./dump --owned-only --include-summaries
    """
    branch_id = branch
    client = ApiClient()
    ...
    # Get app name for ownership detection
    app_name = _get_branch_app_name(client, branch_id)
    ctx.log(f"Application: {app_name}")

    # Create output directories
    out_path = Path(output_dir)
    owned_path = out_path / "owned"
    referenced_path = out_path / "referenced"

    owned_path.mkdir(parents=True, exist_ok=True)
    if not owned_only:
        referenced_path.mkdir(parents=True, exist_ok=True)

    total_configs = 0
    files_created = 0
    owned_counts: dict[str, int] = {}
    referenced_counts: dict[str, int] = {}
    referenced_modules: dict[str, dict[str, int]] = {}  # module_name -> {type: count}

    for ctype in CONFIGURATION_TYPES:
        ctx.log(f"Fetching {ctype} configurations...")

        try:
            configs_list = client.get(ConfigurationEndpoints.list_all(branch_id, ctype))
            if not isinstance(configs_list, list):
                configs_list = [configs_list] if configs_list else []
            ...
            for cfg in configs_list:
                ...
                cfg_app = cfg.get("applicationReferenceName", "")
                is_owned = _is_config_owned(cfg)

                # Skip referenced configs if owned_only
                ...
                if is_owned:
                    type_dir = owned_path / ctype
                    owned_counts[ctype] = owned_counts.get(ctype, 0) + 1
                else:
                    # Organize referenced by module name
                    module_name = cfg_app or "unknown"
                    type_dir = referenced_path / module_name / ctype
                    referenced_counts[ctype] = referenced_counts.get(ctype, 0) + 1

                    # Track module counts
                    if module_name not in referenced_modules:
                        referenced_modules[module_name] = {}
                    referenced_modules[module_name][ctype] = referenced_modules[module_name].get(ctype, 0) + 1
                ...
                if full and cfg_id:
                    try:
                        cfg = client.get(
                            ConfigurationEndpoints.get_content(branch_id, ctype, cfg_id)
                        )
                    except Exception:
                        pass

                # Write config file with literal strings for code fields
                file_path = type_dir / f"{ref_name}.yaml"
                with open(file_path, "w") as f:
                    yaml.dump(convert_code_to_literal(cfg), f, default_flow_style=False, allow_unicode=True, sort_keys=False)
                files_created += 1

                # Write summary file if requested
                if include_summaries and full:
                    try:
                        type_summary = _extract_type_summary(cfg, ctype)
                        summary_data = {
                            "referenceName": ref_name,
                            "configType": ctype,
                            "label": cfg.get("label") or cfg.get("title"),
                            "description": cfg.get("description"),
                            "applicationReferenceName": cfg_app,
                            **type_summary,
                        }
                        summary_path = type_dir / f"{ref_name}.summary.yaml"
            ...
            ctx.log(f"Error fetching {ctype}: {e}")

    # Warn if --owned-only produced no results
    if owned_only and total_configs == 0:
        ctx.log("Warning: No owned configs found with --owned-only.")
        ctx.log("This application may primarily reference library modules.")
        ctx.log("Try running without --owned-only to see all referenced configs.")
    ...
    info_data = {
        ...
        "referenced_modules": referenced_modules if not owned_only else {},
    }

    info_path = out_path / "info.yaml"
    ...
    output = {
        ...
        "owned": owned_counts,
        "referenced": referenced_counts if not owned_only else {},
    }

    # Get cache session info for metadata
    cache_info = end_cache_session() or {}

    ctx.output(single(item=output, semantic_key="export_summary", branch_id=branch_id, full=full, **cache_info))


@explore.command("graph")
@click.option("--branch", "-b", type=int, required=True, help="Branch ID")
@click.option("--output-dir", "-o", type=click.Path(), help="Output directory (optional, prints to stdout if not set)")
@click.option("--format", "-f", "output_format", type=click.Choice(["mermaid", "yaml"]), default="mermaid", help="Output format")
...
def cache_cmd(
    ...
    (INACTIVE, ACTIVE, WORKSPACE_HISTORY) to avoid redundant API calls
    across CLI invocations.

    Draft and WorkspaceActive (feature) branches are never cached
    as they are mutable.
...
def _bfs_from_roots(
    graph: Any, roots: list[str], max_depth: int = -1
...
def _check_analysis_exists(base_dir: Path, library: str, branch_id: int, config_type: str, name: str) -> bool:
...
def manifest(
    ...
    for ref_lower, info in reachable.items():
        ...
        library = app_ref if app_ref else next(
        ...
        analysis_exists = _check_analysis_exists(base_path, library, branch_id, config_type, ref_name)
        if analysis_exists:
            analyzed_count += 1
        ...
        components.append({
            ...
            "library": library,
            "branch_id": branch_id,
            "config_path": str(config_path.relative_to(base_path)) if config_path.is_relative_to(base_path) else str(config_path),
...
def verify(
    ...
    filtered = [
        ...
        if priority_levels.get(c.get("priority", "low"), 3) <= min_priority
    ]
    ...
    for comp in filtered:
        analysis_path = (
            base_dir / comp["library"] / str(comp["branch_id"]) /
            comp["type"] / f"{comp['name']}.analysis.md"
        )
    ...
    missing_by_priority: dict[str, list[str]] = {"critical": [], "high": [], "medium": [], "low": []}
    for comp in missing:
    ...
    result = {
        ...
        "missing_by_priority": {
            k: v for k, v in missing_by_priority.items() if v
        },
    }
