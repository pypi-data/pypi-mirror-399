...
from dxs.core.api.models import APPLICATION_TYPE_ALIASES
...
def transform_repo_output(repo: dict) -> dict:
    ...
    Removes unwanted fields, renames applicationType to type,
    ...
    excluded_fields = {
        ...
        "applicationType",   # Will be renamed
    ...
    # Rename applicationType -> type
    if "applicationType" in repo:
        transformed["type"] = repo["applicationType"]
...
def enrich_repo_with_branches(
    ...
    - activeBranch: Info from Active branch (applicationStatusId=3)
    ...
    """
    from dxs.core.api import BranchEndpoints
    ...
    try:
        ...
        if history_branches:
            history_branches.sort(
                key=lambda b: b.get("commitDate", ""),
                reverse=True
            )
...
def repo_list(
    ...
    if type:
        type_id = APPLICATION_TYPE_ALIASES.get(type.lower())
        if type_id:
            ctx.log(f"Filtering by type: {type}...")
            repos = [r for r in repos if r.get("applicationTypeId") == type_id]
