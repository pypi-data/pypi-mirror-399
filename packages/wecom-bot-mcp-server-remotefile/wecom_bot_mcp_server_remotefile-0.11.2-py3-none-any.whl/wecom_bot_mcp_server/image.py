...
async def send_wecom_image(
    ...
    try:
        ...
        image_path_p, temp_image_path = await _process_image_path(image_path, ctx)
        ...
        if temp_image_path is not None:
            try:
                ...
                logger.warning(f"Failed to cleanup temp image {temp_image_path}: {exc!s}")


async def _process_image_path(image_path: str | Path, ctx: Context | None = None) -> tuple[Path, Path | None]:
    ...
    return image_path, temp_image_path
