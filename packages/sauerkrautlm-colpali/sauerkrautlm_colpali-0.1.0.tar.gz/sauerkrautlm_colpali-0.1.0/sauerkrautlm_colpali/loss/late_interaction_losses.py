...
class ColbertPairwiseNegativeCELoss(ColbertModule):
    ...
    def forward(
        ...
        query_embeddings: torch.Tensor,
        doc_embeddings: torch.Tensor,
        neg_doc_embeddings: torch.Tensor = None,
        offset: int = 0,
    ) -> torch.Tensor:
        """
        Compute pairwise softplus loss with explicit negatives and optional in-batch term.
        
        HYBRID MODE: Falls neg_doc_embeddings None ist, nutze nur in-batch negatives!

        Args:
            query_embeddings (Tensor): [B, Nq, D]
            doc_embeddings (Tensor): [B, Nd, D] positive docs
            neg_doc_embeddings (Tensor, optional): [B, Nneg, D] negative docs (None = nur in-batch!)
            offset (int): Positional offset for positives.

        Returns:
            Tensor: Scalar loss value.
        """
        # HYBRID: Falls keine expliziten Negatives, nutze nur in-batch loss
        if neg_doc_embeddings is None or neg_doc_embeddings.numel() == 0:
            return self.inner_pairwise(query_embeddings, doc_embeddings, offset)
        
        # Mit expliziten Negatives
        lengths = (query_embeddings[:, :, 0] != 0).sum(dim=1)
        pos_raw = torch.einsum("bnd,bsd->bns", query_embeddings, doc_embeddings)
        neg_raw = torch.einsum("bnd,bsd->bns", query_embeddings, neg_doc_embeddings)
        pos_scores = self._aggregate(pos_raw, self.use_smooth_max, dim_max=2, dim_sum=1)
...
class ColbertHybridPairwiseLoss(ColbertModule):
    """
    HYBRID Pairwise Loss - unterstützt BEIDE Modi:
    - Mit expliziten Negatives (wenn vorhanden)
    - Ohne explizite Negatives (In-Batch Negatives)
    
    Perfekt für gemischte Datensätze (z.B. TatDQA ohne + vdr-multilingual mit Negatives)!
    
    Args:
        temperature (float): Scaling für logits.
        normalize_scores (bool): Normalize scores by query lengths.
        use_smooth_max (bool): Use log-sum-exp instead of amax.
        pos_aware_negative_filtering (bool): Apply pos-aware negative filtering.
    """
    ...
    def __init__(
        ...
        # Erstelle beide Sub-Losses
        self.in_batch_loss = ColbertPairwiseCELoss(
            temperature=temperature,
            normalize_scores=normalize_scores,
            use_smooth_max=use_smooth_max,
            pos_aware_negative_filtering=pos_aware_negative_filtering,
    ...
    def forward(
        ...
        """
        Hybrid forward: Nutzt explizite Negatives falls vorhanden, sonst In-Batch.

        Args:
            query_embeddings (Tensor): [B, Nq, D]
            doc_embeddings (Tensor): [B, Nd, D] positive docs
            neg_doc_embeddings (Tensor, optional): [B, Nneg, D] negative docs (kann None sein!)
            offset (int): Positional offset für positives.

        Returns:
            Tensor: Scalar loss value.
        """
        # Check ob explizite Negatives vorhanden
        if neg_doc_embeddings is not None and neg_doc_embeddings.numel() > 0:
            # Mode 1: Mit expliziten Negatives
            lengths = (query_embeddings[:, :, 0] != 0).sum(dim=1)
            pos_raw = torch.einsum("bnd,bsd->bns", query_embeddings, doc_embeddings)
            neg_raw = torch.einsum("bnd,bsd->bns", query_embeddings, neg_doc_embeddings)
            pos_scores = self._aggregate(pos_raw, self.use_smooth_max, dim_max=2, dim_sum=1)
            ...
            # Mode 2: Ohne explizite Negatives (In-Batch)
            return self.in_batch_loss(query_embeddings, doc_embeddings, offset)
