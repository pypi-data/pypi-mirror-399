...
def compute_g_max(
...
) -> tuple[float | np.ndarray, tuple[int, int] | np.ndarray, np.ndarray]:
    """
    Compute G_max and return (G_max, (start_1based, end_1based), biased_profiles).
    The effective potential used is:
      eq_eff = eq_pot + op    (for oxidation)
      eq_eff = eq_pot - op    (for reduction)
    and the bias matrix is eq_eff * n_e_up_to (n_e_up_to >= 0 is number of el steps up to each intermediate).
    The code then applies the historical rule:
      - oxidation: biased = rif_row - bias_matrix
      - reduction: biased = rif_row + bias_matrix
    (This matches your current semantics while being more explicit about eq_eff.)
    """
    if sym_fac is None:
    ...
    if scalar_op:
        ...
        return G_max_list.copy(), idx_pairs.copy(), biased_matrix.copy()
