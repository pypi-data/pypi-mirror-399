...
def plot_free_energy(
    ...
    **step_kwargs,
    ...
    - Requires compute_g_max(mech, op, sym_fac) to return a 3-tuple
      (Gmax, (i,j), biased_profile) — otherwise raises.
    ...
    - base_fontsize controls the overall text scaling; other sizes are derived from it.
    """
    ...
    # Require compute_g_max to return the 3-tuple (Gmax, (i,j), biased_profile)
    if not (isinstance(gmax_res, tuple) and len(gmax_res) == 3):
        raise RuntimeError(
            "plot_free_energy requires compute_g_max(mech, op) to return a 3-tuple "
            "(Gmax, (i,j), biased_profile). The called compute_g_max did not provide "
            "a biased_profile — cannot continue without it."
        )

    # unpack expected 3-tuple
