...
def data_table_view(request, **kwargs):
    ...
    return render(
        ...
        context={
            ...
            "is_date": [True if type(field) is DateField else False for field in model_class._meta.get_fields()],
            "total_pages": range(1, math.ceil(all_data.count() / entries) + 1),
            "has_prev": False if page_number == 1 else (True if all_data.count() != 0 else False),
            "has_next": (
                False if page_number == math.ceil(all_data.count() / entries) else (True if all_data.count() != 0 else False)
            ),
...
def add_record(request, **kwargs):
    try:
        ...
        return HttpResponse(json.dumps({"message": "this model is not activated or not exist.", "success": False}), status=400)
    body = json.loads(request.body.decode("utf-8"))
    try:
        thing = model_manager.create(**body)
    except Exception as ve:
        return HttpResponse(json.dumps({"detail": str(ve), "success": False}), status=400)
    return HttpResponse(json.dumps({"id": thing.id, "message": "Record Created.", "success": True}), status=200)
...
def delete_record(request, **kwargs):
    ...
    try:
        ...
        return HttpResponse(json.dumps({"message": "matching object not found.", "success": False}), status=404)
    to_delete_object.delete()
    return HttpResponse(json.dumps({"message": "Record Deleted.", "success": True}), status=200)
...
def edit_record(request, **kwargs):
    ...
    try:
        ...
        return HttpResponse(json.dumps({"detail": str(ve), "success": False}), status=400)
    return HttpResponse(json.dumps({"message": "Record Updated.", "success": True}), status=200)
...
def export(request, **kwargs):
    ...
    headings = filter(lambda field: field.name not in hidden, [field for field in model_class._meta.get_fields()])
    ...
    df = pd.DataFrame(table_data, columns=tuple(heading.name for heading in headings))
    if export_type == "pdf":
    ...
    return HttpResponse(json.dumps({"content": base64encoded, "file_format": export_type, "success": True}), status=200)
