    ...
    def handle(self, *args, **options):
        ...
        for model_name in API_GENERATOR.values():
            ...
            try:
                ...
                self.stdout.write("Error [" + model_name + " Model]: NOT_FOUND or not migrated in DB.")
                self.stdout.write(" > Hint: Update core/settings.py -> API_GENERATOR Section")
                exit(1)
        ...
        if len(not_migrated_models) > 0:
            ...
            if options["f"]:
                generate()
                self.stdout.write("API codes forced to generate. operation is successful.")
        else:
            generate()
            self.stdout.write("API codes successfully generated.")
