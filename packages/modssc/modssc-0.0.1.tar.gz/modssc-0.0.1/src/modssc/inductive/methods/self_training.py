...
class SelfTrainingMethod(InductiveMethod):
    """Classic self-training loop (CPU/GPU)."""
    ...
    info = MethodInfo(
        ...
        family="classic",
        supports_gpu=True,
        paper_title="Self-training for text classification",
        paper_pdf="",
        official_code="",
    )
    ...
    def fit(self, data: Any, *, device: DeviceSpec, seed: int = 0) -> SelfTrainingMethod:
        ...
        if backend == "numpy":
            ...
            y_l = np.asarray(y_l)
            logger.info(
                "Self-training sizes: n_labeled=%s n_unlabeled=%s",
                int(X_l.shape[0]),
                int(X_u.shape[0]),
            )
            ...
            while iter_count < int(self.spec.max_iter):
                ...
                logger.debug(
                    "Self-training iter=%s accepted=%s remaining=%s",
                    iter_count,
                    int(idx.size),
