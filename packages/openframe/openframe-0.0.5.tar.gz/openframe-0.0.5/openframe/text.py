...
from openframe.util import TextAlign, WrapMode
from openframe.text_layout import wrap_text
...
class TextClip(FrameElement):
    ...
    Attributes:
        ...
        wrap_mode: Wrapping strategy when a size is provided.
    """
    ...
    wrap_mode: WrapMode = WrapMode.AUTO
    ...
    def __post_init__(self) -> None:
        ...
        font = self.load_font()
        render_text = self.text if self.size is None else wrap_text(
            self.text,
            font,
            max(1, self.size[0]),
            self.wrap_mode,
