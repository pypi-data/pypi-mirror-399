...
class TestDockerResourceManagerWaitHealthy:
    ...
    def test_wait_healthy_returns_true_when_healthy(self):
        ...
        with patch("systemeval.plugins.docker_manager.urlopen", return_value=mock_response):
    ...
    def test_wait_healthy_returns_false_on_timeout(self):
        ...
        with patch("systemeval.plugins.docker_manager.urlopen", side_effect=URLError("error")):
            result = manager.wait_healthy(config)
    ...
    def test_wait_healthy_retries_on_url_error(self):
        ...
        with patch("systemeval.plugins.docker_manager.urlopen", side_effect=mock_urlopen):
            result = manager.wait_healthy(config)
    ...
    def test_wait_healthy_uses_correct_url(self):
        ...
        with patch("systemeval.plugins.docker_manager.urlopen", return_value=mock_response) as mock_url:
    ...
    def test_wait_healthy_exponential_backoff(self):
        ...
        with patch("systemeval.plugins.docker_manager.urlopen", side_effect=URLError("error")):
            with patch("systemeval.plugins.docker_manager.time.sleep", side_effect=mock_sleep):
                manager.wait_healthy(config)
    ...
    def test_wait_healthy_handles_generic_exception(self):
        ...
        with patch("systemeval.plugins.docker_manager.urlopen", side_effect=Exception("unexpected error")):
