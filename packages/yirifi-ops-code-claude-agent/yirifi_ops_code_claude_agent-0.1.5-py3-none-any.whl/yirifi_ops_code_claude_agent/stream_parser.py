...
class StreamParser:
    ...
    def _parse_event(self, data: dict[str, Any], raw: str) -> StreamEvent:
        ...
        if event_type == EventType.ASSISTANT:
            # Claude CLI has nested structure: message.content[0].text
            ...
            # Claude CLI uses 'result' field for the summary text
            event_data['summary'] = data.get('result', data.get('summary', ''))
            event_data['is_error'] = data.get('is_error', False)
            event_data['duration_ms'] = data.get('duration_ms', 0)
            event_data['num_turns'] = data.get('num_turns', 1)

        elif event_type == EventType.SYSTEM:
            # System events may have subtype (init, etc.) with config data
            event_data['subtype'] = data.get('subtype', '')
            event_data['message'] = data.get('message', data.get('content', ''))
            # For init subtype, capture session info
            if data.get('subtype') == 'init':
                event_data['session_id'] = data.get('session_id')
                event_data['model'] = data.get('model')
                event_data['tools'] = data.get('tools', [])
                event_data['permission_mode'] = data.get('permissionMode')
