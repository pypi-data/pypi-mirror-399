...
@click.option('--context', '-c', default=None,
              type=click.Choice(['fraud_detection', 'churn_prediction', 'medical_diagnosis',
                                'spam_detection', 'loan_approval', 'marketing_campaign',
                                'quality_control']),
              help='Business context for prediction explanations')
@click.option('--report', '-r', is_flag=True, help='Generate a business-friendly prediction report')
def predict(model_path, data_path, output, proba, context, report):
    """
    ...
    Examples:
        ...
        tabulaml predict model.pkl data.csv -c loan_approval --report  # Business report
    """
    from .business_report import BusinessReportGenerator, format_prediction_report_text
    import numpy as np
    ...
    probabilities = None
    try:
        probabilities = pipeline.predict_proba(df, model_path=model_path)
    except Exception:
        pass

    # Generate business report if requested
    if report or context:
        generator = BusinessReportGenerator(context=context)
        pred_report = generator.generate_prediction_report(
            predictions=np.array(predictions),
            probabilities=probabilities,
            class_labels=list(np.unique(predictions))
        )

        click.echo("\n" + "=" * 60)
        click.echo("PREDICTION REPORT")
        click.echo("=" * 60)
        click.echo(f"\n{pred_report.summary}")

        if pred_report.high_priority_count > 0:
            click.echo(f"\n⚠️  HIGH PRIORITY: {pred_report.high_priority_count:,} items need immediate attention")

        click.echo("\nACTION ITEMS:")
        for action in pred_report.action_items:
            click.echo(f"  • {action}")

        click.echo("\nCLASS MEANINGS:")
        for label, meaning in pred_report.class_meanings.items():
            priority_marker = "[!]" if meaning.priority == "high" else "   "
            click.echo(f"  {priority_marker} Class {label} = {meaning.class_name}")
            click.echo(f"        {meaning.description}")
            click.echo(f"        Action: {meaning.action_required}")

        click.echo("=" * 60)

    # Output results
    if output:
        if proba and probabilities is not None:
        ...
        if context:
            ...
            for pred in predictions:
                if pred in generator.class_meanings:
                    m = generator.class_meanings[pred]
                    meanings.append(m.class_name)
                    priorities.append(m.priority)
                    actions.append(m.action_required)
                else:
                    meanings.append(str(pred))
                    priorities.append("medium")
                    actions.append("Review")

            pred_df['meaning'] = meanings
            pred_df['priority'] = priorities
