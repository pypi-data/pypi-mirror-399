...
class User(TgUser, TsTrait):
    ...
    async def get_hot_ads(self, ex_ids: list[int]) -> tuple[list["MyAd"], list["MyAd"], dict[int, float]]:
        ...
        hot_db, _ = await Hot.get_or_create(
            {}, user=self, updated_at__gt=now() - timedelta(hours=2), status=HotStatus.opened
        )
        logging.info(f"Hot {hot_db.id} {'created' if _ else 'fetched'} for user {self.id}:{self.username_id}:")
    ...
    async def get_last_hot(self) -> "Hot":
        return (
            await Hot.filter(updated_at__gte=now() - timedelta(hours=2), status=HotStatus.opened, user=self)
            .order_by("-updated_at")
            .first()
        )
