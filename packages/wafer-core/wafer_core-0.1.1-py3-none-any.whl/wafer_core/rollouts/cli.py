...
from wafer_core.rollouts.environments import (
    CalculatorEnvironment,
    GitWorktreeEnvironment,
    LocalFilesystemEnvironment,
)
from wafer_core.rollouts.store import FileSessionStore

if TYPE_CHECKING:
    from wafer_core.rollouts.environments.base import Environment
...
def get_oauth_client() -> object:
    ...
    from wafer_core.rollouts.frontends.tui.oauth import get_oauth_client as _get_oauth_client
...
def create_endpoint(
    ...
    if thinking == "enabled":
        ...
        from wafer_core.rollouts.models import Provider
...
def cmd_list_presets() -> int:
    ...
    from wafer_core.rollouts.agent_presets import list_presets
...
def cmd_oauth(login: bool) -> int:
    ...
    from wafer_core.rollouts.frontends.tui.oauth import OAuthError, logout
    from wafer_core.rollouts.frontends.tui.oauth import login as do_login
...
def cmd_export(
    ...
    from wafer_core.rollouts.export import session_to_html, session_to_markdown
...
def cmd_handoff(config: CLIConfig, session_store: FileSessionStore) -> int:
    ...
    from wafer_core.rollouts.export import run_handoff_command
...
def cmd_slice(config: CLIConfig, session_store: FileSessionStore) -> int:
    ...
    Output (stdout):
        <child_session_id>
    """
    from wafer_core.rollouts.slice import run_slice_command
...
def apply_preset(config: CLIConfig) -> bool:
    ...
    from wafer_core.rollouts.agent_presets import load_preset
...
def create_environment(config: CLIConfig) -> tuple[Environment | None, bool]:
    ...
    if config.env == "coding":
        from wafer_core.rollouts.environments.coding import TOOL_PRESETS
    ...
    if config.env == "repl":
        from wafer_core.rollouts.environments.repl import REPLEnvironment
    ...
    if config.env == "repl_blocks":
        from wafer_core.rollouts.environments.repl import MessageParsingREPLEnvironment
    ...
    if "+" in config.env:
        from wafer_core.rollouts.environments.compose import compose
        from wafer_core.rollouts.environments.repl import REPLEnvironment
...
async def run_agent(config: CLIConfig) -> int:
    ...
    from wafer_core.rollouts.agents import resume_session
    ...
    if config.system_prompt:
        ...
        from wafer_core.rollouts.prompt import build_system_prompt
...
async def _run_print_mode(
    ...
    from wafer_core.rollouts.frontends import JsonFrontend, NoneFrontend, run_interactive
...
async def _run_interactive_mode(
    ...
    if config.frontend == "none":
        from wafer_core.rollouts.frontends import NoneFrontend, run_interactive
