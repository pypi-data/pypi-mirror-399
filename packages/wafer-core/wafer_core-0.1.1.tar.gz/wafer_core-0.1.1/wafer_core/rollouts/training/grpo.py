...
Usage:
    from wafer_core.rollouts.training.grpo import GRPOConfig, grpo_train
...
if TYPE_CHECKING:
    from wafer_core.rollouts.dtypes import Environment, Score
...
def grpo_train(
    ...
    Example:
        >>> from wafer_core.rollouts.training.grpo import GRPOConfig, grpo_train
        >>> from wafer_core.rollouts.environments.no_tools import BasicEnvironment
        >>>
...
def _create_inference_engine(
    ...
    from wafer_core.rollouts.training.weight_sync import SGLangEngine, VLLMEngine
...
def _setup_training_backend(
    ...
    from wafer_core.rollouts.training.backends.pytorch_factory import create_pytorch_backend
    from wafer_core.rollouts.training.losses import grpo_loss
...
def _create_tito_generate_fn(
    ...
    from wafer_core.rollouts.inference.backends import compute_suffix_ids
    from wafer_core.rollouts.providers import rollout_sglang_token_level, rollout_vllm_token_level
    ...
    async def generate_fn(batch_prompts: list[dict], **kwargs: Any) -> list:
        from wafer_core.rollouts.dtypes import Actor, Message, Trajectory
...
async def _process_training_step(
    ...
    from wafer_core.rollouts.training.losses import compute_group_advantages
    ...
    if should_sync:
        if should_checkpoint:
            ...
            from wafer_core.rollouts.training.weight_sync import get_fast_sync_dir
...
async def _grpo_train_async(
    ...
    from wafer_core.rollouts._logging import setup_logging
    ...
    from wafer_core.rollouts.training.rollout_gen.async_rollout_manager import AsyncRolloutManager
    from wafer_core.rollouts.training.types import RolloutConfig
...
def _get_message_content(msg: Any) -> str:
    ...
    from wafer_core.rollouts.dtypes import TextContent, ThinkingContent
