...
def validate_path(
    ...
    for forbidden in security.forbidden_paths:
        ...
        if normalized.startswith(forbidden_normalized) or normalized == forbidden_normalized:
            logger.warning(
                "path_forbidden",
                path=path,
                forbidden=forbidden,
    ...
    for allowed in security.allowed_paths:
        ...
        if normalized.startswith(allowed_normalized) or normalized == allowed_normalized:
            logger.debug(
                "path_allowed",
                path=path,
                allowed=allowed,
