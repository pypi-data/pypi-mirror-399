...
MODEL_REVISION: str = "v0.3.0-intl"
...
class EOUModelBase(ABC):
    ...
    async def unlikely_threshold(self, language: str = "zh") -> float | None:
        if language is None:
            return None
...
class MultilingualTurnDetector(EOUModelBase):
    ...
    async def unlikely_threshold(self, language: str = "zh") -> float | None:
        return self._unlikely_threshold or await super().unlikely_threshold(
            language=language
        )
    ...
    async def predict_end_of_turn(
        ...
        res = await super().predict_end_of_turn(chat_ctx, timeout=timeout)
        elapsed_time = perf_counter() - start_time
        logger.info(f"EOU prediction took {elapsed_time:.2f} seconds")
        return res
