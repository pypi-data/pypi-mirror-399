...
.PHONY: docker-build
docker-build:
	docker build -t vibe:latest -f dockerfile .

.PHONY: clean
clean:
	rm -rf dist
	rm -rf vibe_dsl.egg-info
	rm -rf *.db*
...
release:
	...
	@echo Replace version in vibe/__init__.py
	@sed -i.bak "s/__version__ = \"$(CURRENT_VERSION)\"/__version__ = \"$(NEW_VERSION)\"/" src/vibe/__init__.py

	@echo Remove backup files
	@rm pyproject.toml.bak src/vibe/__init__.py.bak

	@uv sync --all-extras

	@echo "Committing version bump..."
	@git add pyproject.toml src/vibe/__init__.py uv.lock
