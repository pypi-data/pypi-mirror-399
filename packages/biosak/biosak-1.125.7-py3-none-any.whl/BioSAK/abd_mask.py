...
def abd_mask(args):
    ...
    # get rRNA regions
    shell_cmd_1_metaxa2_ssu_cmd     = 'metaxa2 --plus --mode m --cpu %s --multi_thread T --table T -g ssu --not_found T -i %s -o %s/%s.metaxa2_ssu --temp %s/metaxa2_temp_ssu'  % (num_threads, renamed_combined_fna, op_dir, op_prefix, op_dir)
    shell_cmd_2_metaxa2_lsu_cmd     = 'metaxa2 --plus --mode m --cpu %s --multi_thread T --table T -g lsu --not_found T -i %s -o %s/%s.metaxa2_lsu --temp %s/metaxa2_temp_lsu'  % (num_threads, renamed_combined_fna, op_dir, op_prefix, op_dir)
