...
def test_initialization_regression(mf_type, bidirectional):
    lstmanfis = LSTMANFIS(variables_reg, mf_type, seq_len, bidirectional=bidirectional)
...
def test_initialization_classification(mf_type, bidirectional):
    lstmanfis = LSTMANFIS(variables_cla, mf_type, seq_len, bidirectional=bidirectional)
    assert len(lstmanfis.input_n_sets) == 3
    assert len(lstmanfis.input_uod) == 3
    assert len(lstmanfis.input_var_names) == 3
    assert lstmanfis.output_var_names == 'd'
