...
class StartupBanner:
    ...
    def _create_side_panel(self) -> Panel:
        ...
        if branding:
            side_lines.append("")  # Add spacing
            if "name" in branding:
                side_lines.append(f"[bold {self._resolve_color('$1')}]{branding['name']}[/]")
            if "tagline" in branding:
                side_lines.append(f"[{self._resolve_color('$2')}]{branding['tagline']}[/]")
            if "founder" in branding:
                side_lines.append(f"[{self._resolve_color('$3')}]Founder: {branding['founder']}[/]")
            if "version" in branding:
                side_lines.append(f"[{self._resolve_color('$4')}]Version: {branding['version']}[/]")
            if "prompt" in branding:
                side_lines.append(f"[{self._resolve_color('$9')}]{branding['prompt']}[/]")
    ...
    def _render_static_banner(self):
        ...
        # Print art directly to avoid encoding issues
        self._print_art_directly()
        console.print()
        console.print(self._create_side_panel())
        console.print()

    def _print_art_directly(self):
        """Print ASCII art directly to avoid Rich encoding issues."""
        for line in self.config.get("asciiArt", []):
            # Remove color codes for now to avoid encoding issues
            for i in range(10):  # $0 to $9
                color_code = f"${i}"
    ...
    def _render_animated_banner(self):
        """Render the banner with animation."""
        # For now, just render static - animation can be added later
    ...
    def display(self):
        ...
        if self.config.get("boot", {}).get("animate", False):
            self._render_animated_banner()
        else:
            self._render_static_banner()
