...
class TasksClient(BaseClient):
        ...
        def _get_source() -> Iterator[Any]:
            if isinstance(zip_path, (str, os.PathLike)):
                with open(zip_path, "rb") as f:
                    yield _ProgressReader(f, progress_callback)
            else:
                yield _ProgressReader(zip_path, progress_callback)  # type: ignore

        def _make_request(source: Any) -> Iterator[str]:
            filename = (
                os.path.basename(os.fspath(zip_path)) if isinstance(zip_path, (str, os.PathLike)) else None
            )
    ...
    def update_checker(self, hub: str, task_id: int, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        # Validate checker_type
        c_type = payload["checker_type"]
        try:
            # Allow passing the Enum member directly or the valid string value
            valid_type = CheckerType(c_type)
            # Normalize to the string value for the API
            payload["checker_type"] = valid_type.value
        except ValueError:
            allowed = [ct.value for ct in CheckerType]
            raise ValueError(f"Invalid checker_type '{c_type}'. Allowed values: {allowed}") from None
...
class _ProgressReader:
    def __init__(self, stream: Any, callback: Callable[[int], Any] | None) -> None:
        self._stream = stream
