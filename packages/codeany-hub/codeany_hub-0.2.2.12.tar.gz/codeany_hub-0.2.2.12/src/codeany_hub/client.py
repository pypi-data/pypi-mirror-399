    ...
    def from_env(
        ...
        *,
        ...
        try:
            ...
            timeout_val = (
                timeout if timeout is not None else cls._env_float(env_mapping.get(f"{prefix}TIMEOUT"), 20.0)
            )
            fetch_docs_val = (
                fetch_docs
            ...
            retries_val = (
                retries if retries is not None else cls._env_int(env_mapping.get(f"{prefix}RETRIES"), 0)
            )
            retry_backoff_val = (
                retry_backoff
        ...
        *,
        ...
        try:
            ...
            timeout_val = (
                ...
                else CodeanyClient._env_float(env_mapping.get(f"{prefix}TIMEOUT"), 20.0)
            )
            fetch_docs_val = (
                fetch_docs
                if fetch_docs is not None
            ...
            retries_val = (
                ...
                else CodeanyClient._env_int(env_mapping.get(f"{prefix}RETRIES"), 0)
            )
            retry_backoff_val = (
                retry_backoff
                if retry_backoff is not None
