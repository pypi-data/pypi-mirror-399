'''Copy & modify from `https://github.com/nrchan/chinese-number-converter`'''
...
class Zh2NumConvertError(Exception):
...
def chinese2number(string: str)->int:
    ...
    for i in range(len(string)):
        ...
        if c in ChineseNumber2Number:
            ...
            if curDigit == 0:
                has_set_zero = True
        elif c in ChineseUnit2Number:
            if curDigit == 0:
                curNum += ChineseUnit2Number[c]
            else:
                curNum += curDigit*ChineseUnit2Number[c]
            curDigit = 0
        elif c in ChineseBigUnit2Number:
            if not has_set_zero and curDigit == 0:
                curNum = 1
            curNum += curDigit
            curDigit = 0
...
def number2chinese(number, language = "T", bigNumber = False, forceErLian = "auto"):
    ...
    if number >= 1e+52:
        raise Zh2NumConvertError("Number is too large. Maximum is (1e+52)-1.")
...
def chinese2number_within_text(text: str)->str:
    ...
    buf = ''
    has_set_zero = False
    has_set_number = False
    curDigit = 0
    curNum = 0
    num = 0
    last_char_type: Literal['none', 'digit', 'unit', 'big_unit'] = 'none'
    
    def append_num_to_buf():
        nonlocal buf, has_set_number, has_set_zero, num, curNum, curDigit
        has_set_zero = has_set_number = False
        curNum += curDigit
        num += curNum
        curNum = curDigit = 0
        buf += str(num)
    ...
    for combined, normal in ChineseCombinedNumber2ChineseNumber.items():
        text = text.replace(combined, normal)
    ...
    for i in range(len(text)):
        ...
        if c in ChineseNumber2Number:
            ...
            curDigit += ChineseNumber2Number[c]
            #for "一百一" is 110 (not 101) issue
            if i == len(text)-1 and len(text) >= 2 and (text[i-1] == "百" or text[i-1] == "佰"):
                curDigit *= 10
            #for "一千一" is 1100 (not 1001) issue
            if i == len(text)-1 and len(text) >= 2 and (text[i-1] == "千" or text[i-1] == "仟"):
                curDigit *= 100
            #for "一萬一" is 11000 (not 10001) issue
            if i == len(text)-1 and len(text) >= 2 and (text[i-1] == "萬" or text[i-1] == "万"):
                curDigit *= 1000
            if curDigit == 0:
                has_set_zero = True
        elif c in ChineseUnit2Number:
