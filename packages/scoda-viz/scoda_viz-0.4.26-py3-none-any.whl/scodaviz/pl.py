...
def add_to_subset_markers( 
    ...
    celltype_subset_rev = 'celltype_subset_rev'
): 
    ...
    adata.obs[celltype_subset_rev] = adata.obs['celltype_subset'].copy(deep = True).astype(str)
    b1 = adata.obs[ref_taxo_level] == tumor_origin
    b2 = adata.obs['ploidy_dec'] == 'Diploid'
    adata.obs.loc[b1 & b2, celltype_subset_rev] = f'{tumor_origin} (Diploid)'
    ...
    if (normal_ref_cond is not None):
        ...
        for c in clst:
            if c not in normal_ref_cond:
                ...
                adata.obs.loc[b1 & b2 & b3, celltype_subset_rev] = f'{tumor_origin} ({c})'
            
        for c in normal_ref_cond:
            if c in list(adata.obs['condition'].unique()):
                b2 = adata.obs['condition'] == c
                adata.obs.loc[b1 & b2, celltype_subset_rev] = f'{tumor_origin} ({c})'  
    else:
        clst = list(adata.obs['condition'].unique())    
        for c in clst:
            b2 = adata.obs['ploidy_dec'] == 'Aneuploid'
            b3 = adata.obs['condition'] == c
            adata.obs.loc[b1 & b2 & b3, celltype_subset_rev] = f'Tumor {tumor_origin} ({c})'
