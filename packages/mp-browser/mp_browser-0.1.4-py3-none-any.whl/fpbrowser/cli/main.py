...
@click.option('--default/--no-default', 'set_default', default=True, help='Set as default browser for Playwright (default: yes)')
@click.option('--playwright/--no-playwright', 'install_playwright', default=True, help='Also install/align Playwright v1.56 (default: yes)')
def install_browser(component, set_default, install_playwright):
    """Install browser components.
    
    Examples:
      fpbrowser install           # Full install: Orbita + default + Playwright
      fpbrowser install orbita    # Same as above
      fpbrowser install orbita --no-default  # Orbita only, no Playwright setup
    ...
    try:
        ...
        if component == 'orbita':
            ...
            success = installer.install(set_default=set_default)
            if success:
...
def _install_playwright():
    """Install or align Playwright version (v1.56 for Chromium 141)."""
    import subprocess
    ...
    try:
        import playwright
        current = getattr(playwright, '__version__', None)
        if current and current.startswith("1.56"):
            click.echo(f"✅ Playwright {current} already installed (compatible)")
            return
        click.echo(f"📦 Aligning Playwright {current} → {PLAYWRIGHT_VERSION}...")
    except ImportError:
        click.echo(f"📦 Installing Playwright {PLAYWRIGHT_VERSION}...")
    
    # Install
    result = subprocess.run(
        [sys.executable, "-m", "pip", "install", f"playwright=={PLAYWRIGHT_VERSION}", "-q"],
        capture_output=True
    )
    
    if result.returncode == 0:
        click.echo(f"✅ Playwright {PLAYWRIGHT_VERSION} installed")
    else:
        click.echo(f"❌ Playwright installation failed", err=True)
