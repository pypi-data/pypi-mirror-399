...
class OrbitaInstaller:
    ...
    def install(self, set_default: bool = False) -> bool:
        """Install Orbita browser (v141 fixed).
        
        Args:
            set_default: Set as default browser for Playwright
    ...
    def _set_as_default(self, install_dir: Path) -> None:
        ...
        Creates Playwright-compatible directory structure and sets env var.
        """
        if self.system not in ["Linux", "Darwin"]:
            print("⚠️  --default only supported on Linux/macOS")
            return
        
        # Create Playwright-compatible structure
        # ~/orbita-browser/chromium-orbita/chrome-linux/chrome
        playwright_dir = install_dir.parent / "playwright-browsers"
        chromium_dir = playwright_dir / "chromium-orbita"
        ...
        print(f"✅ Created symlink: {target_dir} → {install_dir}")
        ...
        if bashrc.exists():
            ...
            if "PLAYWRIGHT_BROWSERS_PATH" in content:
                # Update existing
                lines = content.split('\n')
                lines = [l for l in lines if "PLAYWRIGHT_BROWSERS_PATH" not in l]
                lines.append(env_line)
                bashrc.write_text('\n'.join(lines))
                print(f"✅ Updated PLAYWRIGHT_BROWSERS_PATH in ~/.bashrc")
            else:
                # Append
                with open(bashrc, 'a') as f:
                    f.write(f'\n# Orbita as default Playwright browser\n{env_line}\n')
                print(f"✅ Added PLAYWRIGHT_BROWSERS_PATH to ~/.bashrc")
        else:
            bashrc.write_text(f'# Orbita as default Playwright browser\n{env_line}\n')
            print(f"✅ Created ~/.bashrc with PLAYWRIGHT_BROWSERS_PATH")
        
        print(f"\n💡 Run: source ~/.bashrc")
        print(f"   Or restart terminal to apply changes")
