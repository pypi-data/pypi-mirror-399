...
# 🔧 NEW FLAGS
@click.option("--ai", "-a", is_flag=True, help="Run AI security analysis")
@click.option("--ncc", "-n", is_flag=True, help="Run naming convention checks")

def plan(env, path, ai, ncc):
    api_key = load_config()
    if not api_key:
    ...
    # 🔧 GUARD: no analysis flags selected
    if not ai and not ncc:
        console.print(
            "No analysis flags selected. Use --ai and/or --ncc.",
            style="yellow",
        )

    # 🔧 NAMING CONVENTION CHECK (conditional)
    rg_naming_issues = []
    if ncc:
