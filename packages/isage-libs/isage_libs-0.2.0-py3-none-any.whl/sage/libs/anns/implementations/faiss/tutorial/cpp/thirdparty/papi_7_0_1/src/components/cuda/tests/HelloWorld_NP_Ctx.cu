 ...
 *  Modified: To test that we work with a non-primary context, this
 *  version creates a non-primary context and then destroys it.
 */
...
int main(int argc, char** argv)
{
  ...
  DRIVER_API_CALL(cuCtxCreate(&context, 0, device));
    ...
    // destroy the non-primary context we created.
