...
To enable reading of STEALTIME counters the user needs to link against a PAPI library that was
configured with the STEALTIME component enabled. As an example the following command:
`./configure --with-components="stealtime"` is sufficient to enable the component.
