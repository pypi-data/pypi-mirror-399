...
The SDE component enables PAPI to read Software Defined Events (SDEs) exported by third party
libraries.
...
To enable reading of SDE counters the user needs to link against a PAPI library that was configured
with the SDE component enabled. As an example the following command:
`./configure --with-components="sde"` is sufficient to enable the component.
