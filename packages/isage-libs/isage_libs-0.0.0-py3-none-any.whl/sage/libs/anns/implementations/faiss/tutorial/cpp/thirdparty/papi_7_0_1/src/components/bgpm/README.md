...
Five components have been added to PAPI to support hardware performance monitoring for the BG/Q
platform; in particular the BG/Q network, the I/O system, the Compute Node Kernel in addition to the
processing core.
...
To enable reading of BGPM counters the user needs to link against a PAPI library that was configured
with the BGPM component enabled. There are no specific component configure scripts for L2unit,
IOunit, NWunit, CNKunit. In order to configure PAPI for BG/Q, use the following configure options at
the papi/src level:
