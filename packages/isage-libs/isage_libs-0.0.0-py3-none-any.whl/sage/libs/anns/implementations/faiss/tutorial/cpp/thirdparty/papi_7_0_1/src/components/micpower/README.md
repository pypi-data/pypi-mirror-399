...
To enable reading of MICPOWER counters the user needs to link against a PAPI library that was
configured with the MICPOWER component enabled. As an example the following command:
`./configure --with-components="MICPOWER"` is sufficient to enable the component.
