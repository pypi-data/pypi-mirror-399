...
class PIVectorizeSpec(HistoSpec):
    ...
    def weight_function(params):
        ...
        if isinstance(params, tuple):
            if params[0] == "atan":
                return homhisto.atan_weight_function(params[1], params[2])
            if params[0] == "linear":
                return homhisto.linear_weight_function(1.0 / params[1])
...
class Histogram:
    ...
    def _zspec(colorbar):
        ...
        if "colormap" in colorbar:
            ...
            colormap = None
        if ztype == "linear":
            return plot_PD.ZSpec.Linear(vmax, vmin, colormap, label)
        elif ztype == "log":
            return plot_PD.ZSpec.Log(vmax, vmin, colormap, label)
        elif ztype == "loglog":
            return plot_PD.ZSpec.LogLog(vmax, vmin, colormap, label)
        elif ztype == "linear-midpoint":
            return plot_PD.ZSpec.LinearMidPoint(colorbar["midpoint"], vmax, vmin, colormap, label)
        elif ztype == "power":
            return plot_PD.ZSpec.Power(colorbar["p"], vmax, vmin, colormap, label)
        else:
