...
Contributing
------------
*NiQuery* is a project of the *NiPreps Community*, `which specifies the contributing guidelines <https://www.nipreps.org/community/>`__.
Before delving into the code, please make sure you have read all the guidelines offered online.

Documentation
-------------
Documentation sources are found under the ``docs/`` folder, and builds are archived in the `gh-pages <https://github.com/nipreps/niquery/tree/gh-pages>`__ branch of the repository.
With GitHub Pages, the documentation is posted under https://www.nipreps.org/niquery.
...
The ``gh-pages`` branch is automatically maintained with `a GitHub Action <https://github.com/nipreps/niquery/blob/main/.github/workflows/docs-build-update.yml>`__.
...
To build the documentation locally, you first need to make sure that ``setuptools_scm[toml] >= 6.2`` is installed in your environment and then::
  ...
  cd <niquery-repository>/
  python -m setuptools_scm  # This will generate ``src/niquery/_version.py``
  make -C docs/ html
