#!/usr/bin/env python3
"""
Полный тест всех 8 режимов MCP Proxy Adapter
...
class MCPProxyAdapterTester:
    ...
    def test_http_basic(self) -> Dict[str, Any]:
        ...
        print("\n🔍 Тестирование HTTP Basic (порт 15000)")
        ...
        try:
            ...
            health_url = f"{self.base_url}:15000/health"
            health_response = requests.get(health_url, timeout=10)
            
            # Тест JSON-RPC
            jsonrpc_url = f"{self.base_url}:15000/api/jsonrpc"
            jsonrpc_data = {
                "jsonrpc": "2.0",
                "method": "echo",
    ...
    def test_http_token(self) -> Dict[str, Any]:
        ...
        print("\n🔍 Тестирование HTTP + Token (порт 15001)")
        ...
        try:
            ...
            # Тест health endpoint
            health_url = f"{self.base_url}:8080/health"
            health_response = requests.get(health_url, headers=headers, timeout=10)
            
            # Тест JSON-RPC
            jsonrpc_url = f"{self.base_url}:8080/api/jsonrpc"
            jsonrpc_data = {
                "jsonrpc": "2.0",
                "method": "echo",
    ...
    def test_https_basic(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Тест health endpoint
            health_url = f"{self.https_base_url}:8443/health"
            health_response = requests.get(health_url, verify=False, timeout=10)
            
            # Тест JSON-RPC
            jsonrpc_url = f"{self.https_base_url}:8443/api/jsonrpc"
            jsonrpc_data = {
                "jsonrpc": "2.0",
                "method": "echo",
