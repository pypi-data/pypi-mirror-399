...
def table():
    ...
    return spikesafe_python.Compensation.load_custom_compensation_table(os.path.join(os.path.dirname(__file__), 'test_compensation_files', 'valid.json'))
...
def test_load_custom_compensation_table_error(file_path, expected_error_message):
    with pytest.raises(ValueError) as exc_info:
        spikesafe_python.Compensation.load_custom_compensation_table(os.path.join(os.path.dirname(__file__), 'test_compensation_files', file_path))
...
def test_get_custom_compensation_table_exceptions(custom_compensation_table, expected_error_message):
    with pytest.raises(Exception) as exc_info:
        spikesafe_python.Compensation.get_custom_compensation(0.5, 0.6, 'laser_green', custom_compensation_table, 0.0001)
...
def test_get_custom_compensation_current_value_errors(spikesafe_model_max_current, measurement_current, device_type, expected_error_message, table):
    with pytest.raises(ValueError) as exc_info:
        spikesafe_python.Compensation.get_custom_compensation(spikesafe_model_max_current, measurement_current, device_type, table, 0.0001)
    assert expected_error_message in str(exc_info.value)
...
@pytest.mark.parametrize("spikesafe_model_max_current, measurement_current, device_type, expected_load_impedance, expected_rise_time", [
    (5, 0.126, 'laser_green', spikesafe_python.SpikeSafeEnums.LoadImpedance.HIGH, spikesafe_python.SpikeSafeEnums.RiseTime.FAST),  # Upper 1/3 of range
    (5, 0.1, 'laser_green', spikesafe_python.SpikeSafeEnums.LoadImpedance.MEDIUM, spikesafe_python.SpikeSafeEnums.RiseTime.MEDIUM),  # Middle 1/3 of range
    (5, 0.005, 'laser_green', spikesafe_python.SpikeSafeEnums.LoadImpedance.VERY_LOW, spikesafe_python.SpikeSafeEnums.RiseTime.VERY_SLOW),  # Lower 1/3 of range
])
def test_get_custom_compensation_range(spikesafe_model_max_current, measurement_current, device_type, expected_load_impedance, expected_rise_time, table):
    load_impedance, rise_time = spikesafe_python.Compensation.get_custom_compensation(spikesafe_model_max_current, measurement_current, device_type, table, 0.0005)
...
@pytest.mark.parametrize("spikesafe_model_max_current, measurement_current, device_type, pulse_on_time_seconds, expected_load_impedance, expected_rise_time", [
    (0.5, 0.1, 'laser_green', 0.0005, spikesafe_python.SpikeSafeEnums.LoadImpedance.MEDIUM, spikesafe_python.SpikeSafeEnums.RiseTime.MEDIUM),  # Small pulse under 500us that requires optimum compensation
    (0.5, 0.1, 'laser_green', 0.00051, spikesafe_python.SpikeSafeEnums.LoadImpedance.VERY_LOW, spikesafe_python.SpikeSafeEnums.RiseTime.VERY_SLOW),  # Large pulse over 500us that does not require compensation
    (100, 0, 'laser_green', None, spikesafe_python.SpikeSafeEnums.LoadImpedance.VERY_LOW, spikesafe_python.SpikeSafeEnums.RiseTime.VERY_SLOW),  # DC mode that does not require compensation (pulse_on_time_seconds=None)
])
def test_get_custom_compensation_pulse_on_time_seconds(spikesafe_model_max_current, measurement_current, device_type, pulse_on_time_seconds, expected_load_impedance, expected_rise_time, table):
    load_impedance, rise_time = spikesafe_python.Compensation.get_custom_compensation(spikesafe_model_max_current, measurement_current, device_type, table, pulse_on_time_seconds)
    assert load_impedance == expected_load_impedance
    assert rise_time == expected_rise_time