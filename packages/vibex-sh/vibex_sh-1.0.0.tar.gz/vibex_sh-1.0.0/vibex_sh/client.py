...
class VibexClient:
    ...
    def send_log(self, log_type: str, payload: Any, timestamp: Optional[int] = None) -> bool:
        ...
        Args:
            log_type: Type of log. Supported types:
                - 'json': Structured JSON logs (payload should be dict)
                - 'text': Plain text logs (payload should be str)
                - 'web-server': Web server access logs (Nginx, Apache) - payload should be str
                - 'loadbalancer': Load balancer logs (HAProxy, AWS ALB) - payload should be str
                - 'stacktrace': Stack trace logs - payload should be str
                - 'firewall': Firewall logs (iptables, pfSense, Cisco ASA) - payload should be str
                - 'kubernetes': Kubernetes pod/container logs - payload should be str
                - 'docker': Docker container logs - payload should be str
                - 'network': Network logs (tcpdump, wireshark) - payload should be str
                - 'keyvalue': Key-value pair logs - payload should be str
                - 'json-in-text': JSON embedded in text - payload should be str
                - 'smart-pattern': Smart pattern logs (multi-language) - payload should be str
                - 'raw': Raw logs (fallback) - payload should be str
            payload: Log payload (dict for 'json', str for all other types)
        ...
        # Validate log type (default to 'json' for backward compatibility)
        valid_log_types = [
            'json', 'text', 'web-server', 'loadbalancer', 'stacktrace', 'firewall',
            'kubernetes', 'docker', 'network', 'keyvalue', 'json-in-text',
            'smart-pattern', 'raw'
        ]
        
        # Default to 'json' if log_type is not provided or invalid (backward compatibility)
        normalized_log_type = log_type if log_type and log_type in valid_log_types else 'json'
        ...
        try:
            ...
            log_entry = (normalized_log_type, payload, timestamp or int(time.time() * 1000))
