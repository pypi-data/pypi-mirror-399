...
class VibexHandler(logging.Handler):
    ...
    def __init__(self, config: VibexConfig = None, verbose: bool = False, 
        ...
        Args:
            ...
            log_type: Optional log type. If specified, uses this type instead of auto-detection.
                Supported types: 'json', 'text', 'web-server', 'loadbalancer', 'stacktrace', 'firewall',
                'kubernetes', 'docker', 'network', 'keyvalue', 'json-in-text', 'smart-pattern', 'raw'.
                If not specified, defaults to auto-detection (JSON or text based on message content).
        """
        super().__init__()
        ...
        self.log_type = log_type  # Store log_type for use in emit() method
    ...
    def emit(self, record: logging.LogRecord) -> None:
        ...
        try:
            ...
            if self.client.is_enabled():
                # Determine log type: use specified log_type, or default to 'json' with hybrid structure
                send_log_type = 'json'
                send_payload = hybrid
                
                if self.log_type:
                    # User specified a log type - use it directly
                    send_log_type = self.log_type
                    # For non-JSON types, send raw message as string
                    if self.log_type != 'json':
                        send_payload = message
                    # For JSON type, use hybrid structure
                else:
                    # Default behavior: auto-detect and use hybrid JSON structure
                    send_log_type = 'json'
                    send_payload = hybrid
                
                send_succeeded = self.client.send_log(send_log_type, send_payload, int(record.created * 1000))
