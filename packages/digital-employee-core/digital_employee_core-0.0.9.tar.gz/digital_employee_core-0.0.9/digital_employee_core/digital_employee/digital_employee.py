...
class _DependencyBundle:
    ...
    def merged_with(
        ...
        *,
        ...
        Args:
            extra (_DependencyBundle): The other dependency bundle to merge with.
            merge_configs (bool): Whether to merge configuration maps.

        Returns:
            _DependencyBundle: A new dependency bundle with merged dependencies and configurations.
        """
        merged_tools = list(self.tools)
        merged_mcps = list(self.mcps)
        ...
        for mcp in extra.mcps:
            if mcp not in merged_mcps:
                merged_mcps.append(mcp)

        tool_configs = self.tool_configs
        mcp_configs = self.mcp_configs
        if merge_configs:
            tool_configs = _DependencyBundle.merge_config_maps(self.tool_configs, extra.tool_configs)
            mcp_configs = _DependencyBundle.merge_config_maps(self.mcp_configs, extra.mcp_configs)

        return _DependencyBundle(
            tools=merged_tools,
            mcps=merged_mcps,
...
class DigitalEmployee:
    ...
    def build_tool_config(self, configurations: list[DigitalEmployeeConfiguration]) -> dict[str, dict[str, Any]]:
        ...
        return self._build_merged_configs(
            configurations, lambda loader, configs, items: loader.load_tool_configs(configs, items), self.tools
        )
    ...
    def build_mcp_config(self, configurations: list[DigitalEmployeeConfiguration]) -> dict[str, dict[str, Any]]:
        ...
        """
        return self._build_merged_configs(
            configurations, lambda loader, configs, items: loader.load_mcp_configs(configs, items), self.mcps
        )
    ...
    def _build_base_dependency_bundle(self) -> _DependencyBundle:
        """Build the base dependency bundle containing tools, MCPs, and their configurations.

        Returns:
            _DependencyBundle: Bundle containing base tools, MCPs, and their config maps.
        """
        tools, mcps, tool_configs, mcp_configs = self._build_base_tools_mcps_and_configs()
        return _DependencyBundle(tools=tools, mcps=mcps, tool_configs=tool_configs, mcp_configs=mcp_configs)

    def _build_escalation_dependency_bundle(self) -> _DependencyBundle:
        """Build the escalation dependency bundle containing tools, MCPs, and their configurations.

        Returns:
            _DependencyBundle: Bundle containing escalation tools, MCPs, and their config maps.
        """
    ...
    def _build_escalation_dependencies(self) -> tuple[list[Tool], list[MCP]]:
        ...
        required_mcps = _DependencyBundle.dedupe_preserve_order(
            [mcp for channel in self.escalation_config.channels for mcp in channel.get_required_mcps()]
        )
        required_tools = _DependencyBundle.dedupe_preserve_order(
