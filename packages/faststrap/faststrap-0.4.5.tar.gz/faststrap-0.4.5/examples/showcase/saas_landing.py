...
def home():
    ...
    features = Section(
        Container(
            ...
            Row(
                render_feature("lightning", "Instant Deployment", "Push your code and we handle the build & hosting automatically."),
                render_feature("shield-check", "Enterprise Security", "SSL, DDoS protection, and isolated environments out of the box."),
                render_feature("graph-up", "Autoscaling", "Your app scales with your traffic. Only pay for what you use."),
            )
        ),
        cls="py-5 bg-light"
    )
    ...
    metrics = Section(
        Container(
            Row(
                Col(StatCard("Active Workers", "1,200+", trend="+15%", trend_type="up"), md=4, cls="mb-4"),
                Col(StatCard("Build Time (Avg)", "42s", trend="-2s", trend_type="up"), md=4, cls="mb-4"),
    ...
    faq = Section(
        Container(
            ...
            Accordion(
                AccordionItem("We support all major versions of Python from 3.10 upwards.", title="What Python versions are supported?"),
                AccordionItem("Yes! Our free tier includes 1GB of bandwidth and 512MB RAM.", title="Is there a free tier?"),
                AccordionItem("Simply connect your repository and we'll auto-detect your FastHTML app.", title="How do I migrate from Heroku?"),
