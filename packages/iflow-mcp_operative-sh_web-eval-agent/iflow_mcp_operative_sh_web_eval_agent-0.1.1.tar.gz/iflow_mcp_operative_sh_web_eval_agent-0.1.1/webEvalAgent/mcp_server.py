...
api_key = os.environ.get('OPERATIVE_API_KEY', 'test_key_for_local_testing')

# Validate the API key (simplified for local testing)
if api_key:
    # Skip actual validation for local testing
    is_valid = True
    if not is_valid:
...
async def web_eval_agent(url: str, task: str, ctx: Context, headless_browser: bool = False) -> list[TextContent]:
    ...
    # Skip API validation for local testing
    is_valid = True
