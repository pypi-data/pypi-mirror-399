...
def evaluate(
    ...
    Attributes must include 'content_embedding' (numpy array) for semantic matching.
...
def _evaluate_semantic(
    ...
    Args:
        attributes: Must include 'content_embedding' (numpy array).
        event_type: The EventType to evaluate against.

    Returns:
        List of matching trigger IDs.
    """
    from .semantic_evaluator import evaluate_semantic_trigger, SemanticEvaluationError

    content_embedding = attributes.get("content_embedding")
    if content_embedding is None:
        logger.warning(
            f"EventType {event_type.id} uses semantic-embedding but "
            "attributes missing 'content_embedding'"
        )
        return []
