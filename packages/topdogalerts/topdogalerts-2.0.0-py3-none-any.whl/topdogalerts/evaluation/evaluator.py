...
class SemanticTriggerMatch:
    """Details about a matched semantic trigger."""
    trigger_id: str
    similarity: float
...
def _evaluate_semantic(
    ...
    content_preview = content_embedding_string[:80] + "..." if len(content_embedding_string) > 80 else content_embedding_string
    logger.info(f"Encoding content: '{content_preview}'")
    try:
        content_embedding = encode_text(content_embedding_string)
        logger.info("Content encoding complete")
    ...
    # Count enabled triggers for logging
    enabled_triggers = [t for t in triggers if t.enabled is not False]
    logger.info(f"Evaluating {len(enabled_triggers)} triggers...")

    matching_trigger_ids: List[str] = []
    match_details: List[SemanticTriggerMatch] = []
    ...
    for trigger in triggers:
        ...
        try:
            ...
            result = evaluate_semantic_trigger(
                content_embedding, keywords_csv, float(threshold_percent)
            )
            if result.triggered:
                matching_trigger_ids.append(trigger.id)
                match_details.append(SemanticTriggerMatch(
                    trigger_id=trigger.id,
                    similarity=result.similarity,
                    threshold_percent=result.threshold_percent,
                    best_keyword=result.best_keyword,
                ))
                # Log threshold details for each fired trigger
                similarity_pct = result.similarity * 100
                logger.info(
                    f"Trigger {trigger.id} matched: "
                    f"similarity={similarity_pct:.1f}%, threshold={result.threshold_percent:.0f}%, "
                    f"best_match='{result.best_keyword}'"
                )
        except SemanticEvaluationError as e:
    ...
    logger.info(f"Evaluation complete: {len(matching_trigger_ids)} triggers matched")
    return matching_trigger_ids
