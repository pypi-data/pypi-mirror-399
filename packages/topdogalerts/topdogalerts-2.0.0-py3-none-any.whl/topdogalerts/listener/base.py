...
class BaseListener(ABC):
    ...
    def __init__(
        ...
        # Serialize trigger change processing to prevent duplicate work
        # when multiple notifications arrive in rapid succession
        self._trigger_change_lock = asyncio.Lock()
    ...
    async def _handle_trigger_notification(self, payload: Dict[str, Any]) -> None:
        ...
        # Serialize trigger change handling to prevent duplicate work when
        # multiple notifications arrive rapidly (e.g., bulk trigger updates).
        # The first handler processes all changes; subsequent handlers see no diff.
        async with self._trigger_change_lock:
