...
class ColumnSelector:
    ...
    def __call__(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        Returns:
            pd.DataFrame: New DataFrame containing only specified columns.
        """
...
class DropNa:
    ...
    Args:
        columns (list[str]): Columns to consider when dropping rows with NaN.
    """
    ...
    def __call__(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        Returns:
            pd.DataFrame: New DataFrame with rows containing NaN in the specified columns removed.
        """
...
class OneHotEncoder:
    ...
    Args:
        columns (list[str] or None): Columns to one-hot encode or None for all. Defaults to None.
    ...
    def __call__(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        Returns:
            pd.DataFrame: DataFrame with selected columns one-hot encoded.
        """
        cols = self._columns or df.columns

        return pd.get_dummies(
            df, columns=cols, drop_first=self._drop_first, dtype=self._dtype
        )
...
class SplitXY:
    ...
    def __cal__(self, df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
        """
        ...
        X = df.drop(columns=self._target_cols).copy()
...
class ToTensor:
    """
    Convert pandas DataFrame or Series (or their collection) to torch.Tensor(s).
    """

    def __call__(
        self, data: PandasObj | tuple[PandasObj]
    ) -> torch.Tensor | tuple[torch.Tensor]:
        """
        Return tensor version of pandas objects.

        Args:
            data: Input DataFrame(s) or Series.

        Returns:
            Tensor or tuple[Tensor]: output tensor(s).
        """
        if isinstance(data, (pd.Series, pd.DataFrame)):
            return torch.from_numpy(data.to_numpy(), dtype=torch.float32)

        if isinstance(data, (tuple, list)):
            return tuple(
                torch.from_numpy(d.to_numpy(), dtype=torch.float32) for d in data
            )
...
class Binarizer(ColwiseTransform):
    ...
    Args:
        columns (list[str] or None): Columns to binarize or None for all. Defaults to None.
...
class LogTransformer(ColwiseTransform):
    ...
    Args:
        columns (list[str] or None): Columns to log-transform or None for all. Defaults to None.
...
class FrequencyEncoder(StatefulColwiseTransform):
    ...
    Args:
        columns (list[str] or None): Columns to frequency-encode or None for all. Defaults to None.
        ...
        use_params (bool): Whether to use previously stored parameters. Defaults to False.
    """
    ...
    def __init__(
        ...
        super().__init__(columns, retain_params, use_params)
...
class MaxAbsScaler(StatefulColwiseTransform):
    ...
    Args:
        columns (list[str] or None): Columns to scale or None for all. Defaults to None.
        retain_params (bool): Whether to store computed parameters. Defaults to True.
        use_params (bool): Whether to use previously stored parameters. Defaults to False.
    """
...
class MinMaxScaler(StatefulColwiseTransform):
    ...
    Args:
        columns (list[str] or None): Columns to min-max scale or None for all. Defaults to None.
        feature_range (tuple[float, float]): Desired output range (min, max).
        retain_params (bool): Whether to store computed parameters.
        use_params (bool): Whether to use previously computed parameters.
    """
...
class OrdinalEncoder(StatefulColwiseTransform):
    ...
    Args:
        columns (list[str] or None): Columns to ordinally encode or None for all. Defaults to None.
        handle_unknown (str): Strategy for unseen categories during transform. Defaults to "error".
            Options:
                "error": raise an error
                "use_encoded_value": encode as unknown_value
