...
""" CType classes for PyObject *, PyObject **, and struct OxNJAC_CellObject *

"""

from darkmatterpy.__past__ import iterItems, xrange
from darkmatterpy.code_generation.ErrorCodes import (
    getErrorExitBoolCode,
    getReleaseCode,
)
from darkmatterpy.code_generation.templates.CodeTemplatesVariables import (
...
from darkmatterpy.Constants import getConstantValueGuide, isMutable
...
class CTypePyObjectPtr(CPythonPyObjectPtrBase):
    ...
    def getCellObjectAssignmentCode(cls, target_cell_code, variable_code_name, emit):
        emit("%s = OxNJAC_Cell_New0(%s);" % (target_cell_code, variable_code_name))
...
class CTypeCellObject(CTypeBase):
    c_type = "struct OxNJAC_CellObject *"
    ...
    def getInitValue(cls, init_from):
        ...
        if init_from is not None:
            return "OxNJAC_Cell_New1(%s)" % init_from
        else:
            return "OxNJAC_Cell_NewEmpty()"
    ...
    def emitValueAccessCode(cls, value_name, emit, context):
        ...
        return VariableDeclaration(
            "PyObject *", "OxNJAC_Cell_GET(%s)" % value_name, None, None
        )

    @classmethod
    def getVariableArgDeclarationCode(cls, variable_code_name):
        return "struct OxNJAC_CellObject *%s" % variable_code_name
