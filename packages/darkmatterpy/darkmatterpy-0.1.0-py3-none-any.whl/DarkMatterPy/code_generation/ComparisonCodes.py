...
from darkmatterpy.nodes.shapes.StandardShapes import tshape_unknown
from darkmatterpy.PythonOperators import (
    comparison_inversions,
    rich_comparison_arg_swaps,
)

from .c_types.CTypeBooleans import CTypeBool
from .c_types.CTypeOxNJACBooleans import CTypeOxNJACBoolEnum
from .c_types.CTypeOxNJACVoids import CTypeOxNJACVoidEnum
...
def getRichComparisonCode(
    ...
    if needs_check:
        ...
        # instead can use the CTypeOxNJACBoolEnum that will easily convert to
        # it.
        if helper_type is CTypeOxNJACVoidEnum:
            helper_type = CTypeOxNJACBoolEnum
        ...
        # avoid the CTypeOxNJACBoolEnum (NBOOL) helpers except for things that
    ...
    if helper_function is None and not report_missing:
        helper_type, helper_function = selectCodeHelper(
            ...
            specialized_helpers_set=specialized_helpers_set,
            non_specialized_helpers_set=non_specialized_helpers_set,
            result_type=CTypeOxNJACBoolEnum,
            left_shape=left_shape,
            right_shape=right_shape,
            left_c_type=left_c_type,
            right_c_type=right_c_type,
...
def generateSubtypeCheckCode(to_name, expression, emit, context):
    ...
    emit(
        "%s = OxNJAC_Type_IsSubtype((PyTypeObject *)%s, (PyTypeObject *)%s);"
        % (res_name, left_name, right_name)
    )
