...
"""
This is the main program of OxN, it checks the options and then translates
...
def getLaunchingOxNProcessEnvironmentValue(environment_variable_name):
...
def main():
    ...
    if (
        ...
        ctypes.windll.user32.MessageBoxW(
            None,
            "You have to use the 'python.exe' and not a 'pythonw.exe' to run OxN",
            "Error",
    ...
    getLaunchingOxNProcessEnvironmentValue("DEVILPY_SITE_FILENAME")

    JACK_binary_name = getLaunchingOxNProcessEnvironmentValue("DEVILPY_BINARY_NAME")
    if JACK_binary_name is not None:
        sys.argv[0] = JACK_binary_name

    JACK_pythonpath_ast = getLaunchingOxNProcessEnvironmentValue(
        "DEVILPY_PYTHONPATH_AST"
    )

    if JACK_pythonpath_ast is not None:
        ...
        sys.path = [JACK_pythonpath_ast]
        import ast

        sys.path = ast.literal_eval(
            getLaunchingOxNProcessEnvironmentValue("DEVILPY_PYTHONPATH")
        )
    else:
        ...
        # from "JACK.distutils". Also ignore path elements that do not really
        # exist.
        sys.path = [
            path_element
    ...
    # changing it every single time OxN is run. This kills any attempt at
    ...
    if needs_re_execution:
        from JACK.utils.ReExecute import reExecuteOxN  # isort:skip

        # Does not return
        reExecuteOxN(pgo_filename=None)
    ...
    # module of JACK package and renamed to where we can get at easily for
    # other uses.
    __import__("JACK").getLaunchingOxNProcessEnvironmentValue = (
        getLaunchingOxNProcessEnvironmentValue
    )

    from JACK import Options  # isort:skip
    ...
    if "DEVILPY_NAMESPACES" in os.environ:
        ...
        from JACK.importing.PreloadedPackages import setPreloadedPackagePaths
    ...
    if "DEVILPY_PTH_IMPORTED" in os.environ:
        ...
        from JACK.importing.PreloadedPackages import setPthImportedPackages
    ...
    if "DEVILPY_USER_SITE" in os.environ:
        from JACK.utils.Distributions import setUserSiteDirectory

        setUserSiteDirectory(ast.literal_eval(os.environ["DEVILPY_USER_SITE"]))

        del os.environ["DEVILPY_USER_SITE"]

    # Now the real main program of OxN can take over.
    from JACK import MainControl  # isort:skip
...
if __name__ == "__main__":
    _JACK_package_home = getLaunchingOxNProcessEnvironmentValue(
        "DEVILPY_PACKAGE_HOME"
    )
    if _JACK_package_home is not None:
        sys.path.insert(0, _JACK_package_home)

        import JACK  # just to have it loaded from there, pylint: disable=unused-import

        assert sys.path[0] is _JACK_package_home
        del sys.path[0]
