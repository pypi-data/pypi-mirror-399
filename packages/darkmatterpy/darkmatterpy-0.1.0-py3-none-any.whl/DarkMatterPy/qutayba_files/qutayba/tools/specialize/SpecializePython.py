...
import JACK.code_generation.BinaryOperationHelperDefinitions
import JACK.code_generation.CodeGeneration
import JACK.code_generation.ComparisonCodes
import JACK.code_generation.Namify
import JACK.nodes.NetworkxNodes
import JACK.nodes.PackageMetadataNodes
import JACK.nodes.PackageResourceNodes
import JACK.nodes.SideEffectNodes
import JACK.nodes.TensorflowNodes
...
import JACK.specs.BuiltinTypeOperationSpecs
import JACK.specs.HardImportSpecs
import JACK.tree.Building
from JACK.containers.OrderedDicts import OrderedDict
from JACK.containers.OrderedSets import OrderedSet
from JACK.nodes.ImportNodes import hard_modules_non_stdlib
from JACK.nodes.NodeMetaClasses import NodeCheckMetaClass
...
from JACK.utils.Jinja2 import getTemplate
...
def _getMixinForShape(shape):
    ...
    if shape is tshape_str:
        return "JACK.nodes.ExpressionShapeMixins.ExpressionStrShapeExactMixin"
    elif shape is tshape_list:
        return "JACK.nodes.ExpressionShapeMixins.ExpressionListShapeExactMixin"
    elif shape is tshape_tuple:
        return "JACK.nodes.ExpressionShapeMixins.ExpressionTupleShapeExactMixin"
    elif shape is tshape_int:
        return "JACK.nodes.ExpressionShapeMixins.ExpressionIntShapeExactMixin"
    elif shape is tshape_bool:
        return "JACK.nodes.ExpressionShapeMixins.ExpressionBoolShapeExactMixin"
    elif shape is tshape_none:
        return "JACK.nodes.ExpressionShapeMixins.ExpressionNoneShapeExactMixin"
    elif shape is tshape_dict:
        return "JACK.nodes.ExpressionShapeMixins.ExpressionDictShapeExactMixin"
    elif shape is tshape_bytes:
        return "JACK.nodes.ExpressionShapeMixins.ExpressionBytesShapeExactMixin"
    else:
        assert False, shape
...
processTypeShapeAttribute(
    "tshape_type",
    JACK.specs.BuiltinTypeOperationSpecs,
    python2_type_methods,
    python3_type_methods,
)
...
def emitGenerationWarning(emit, doc_string, template_name):
    ...
    for spec_descriptions in getSpecVersions(JACK.specs.HardImportSpecs):
...
def makeAttributeNodes():
    filename_python = "JACK/nodes/AttributeNodesGenerated.py"
    ...
    with withFileOpenedAndAutoFormatted(
        ...
        emit("from JACK.specs.BuiltinParameterSpecs import extractBuiltinArgs")

        emit("from JACK.nodes.ConstantRefNodes import makeConstantRefNode")
        emit(
            """\
from JACK.nodes.NodeMakingHelpers import (
    ...
    makeRaiseExceptionReplacementExpression
        ...
        emit(
            "from JACK.nodes.KeyValuePairNodes import makeKeyValuePairExpressionsFromKwArgs"
        )

        emit("from JACK.nodes.AttributeNodes import makeExpressionAttributeLookup")

        emit("from JACK.PythonVersions import getDictFromkeysNoArgErrorMessage")
...
def makeBuiltinOperationNodes():
    filename_python = "JACK/nodes/BuiltinOperationNodeBasesGenerated.py"
...
def makeChildrenHavingMixinNodes():
    ...
    filename_python = "JACK/nodes/ChildrenHavingMixins.py"
    filename_python2 = "JACK/nodes/ExpressionBasesGenerated.py"
    filename_python3 = "JACK/nodes/StatementBasesGenerated.py"
    ...
    with withFileOpenedAndAutoFormatted(
        ...
        emit1(
            """
from JACK.nodes.Checkers import (
    checkStatementsSequenceOrNone,
...
def makeHardImportNodes():
    ...
    filename_python = "JACK/nodes/HardImportNodesGenerated.py"
