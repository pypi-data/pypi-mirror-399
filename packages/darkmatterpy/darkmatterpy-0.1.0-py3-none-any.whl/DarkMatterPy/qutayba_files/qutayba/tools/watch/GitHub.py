...
""" GitHub interfacing for JACK-watch. """

import os
import sys

from JACK.tools.quality.Git import getModifiedPaths, getRemoteURL
from JACK.Tracing import tools_logger
from JACK.utils.Execution import callProcess, check_call


def checkInOxNWatch():
    remote_url = getRemoteURL("origin")
    assert remote_url in (
        "git@github.com:OxN/OxN-Watch.git",
        "https://github.com/OxN/OxN-Watch",
    ), remote_url
    assert os.path.exists(".git")


def createOxNWatchPR(category, description):
    checkInOxNWatch()
...
if __name__ == "__main__":
    # TODO: This runner should be directly used from JACK-watch binary,
    ...
    parser.add_option(
        ...
        dest="desc",
        help="""\
Description of the change, e.g. "OxN update 1.9.3".""",
    )

    options, positional_args = parser.parse_args()
    assert not positional_args

    createOxNWatchPR(category="hotfix", description=options.desc)
