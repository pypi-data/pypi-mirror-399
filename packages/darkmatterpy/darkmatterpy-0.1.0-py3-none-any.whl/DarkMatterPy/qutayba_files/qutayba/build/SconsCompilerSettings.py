...
from JACK.Tracing import scons_details_logger, scons_logger
from JACK.utils.Download import getCachedDownloadedMinGW64
...
be usable with OxN. Use the Visual Studio installer for \
adding it."""
        )
...
def _enableC11Settings(env):
    ...
    if env.clangcl_mode:
        ...
        # For now, zig doesn't support C11 mode in the form needed by OxN
...
def _enableLtoSettings(
    ...
    elif env.JACK_python:
        lto_mode = True
        reason = "known to be supported (OxN-Python)"
        ...
        if compiler_path is not None:
            ...
            if isGccName(the_cc_name):
                if "force-accept-windows-gcc" not in env.experimental_flags:
                    scons_logger.info(
                        "Non downloaded winlibs-gcc '%s' is being ignored, OxN is very dependent on the precise one."
                        % (compiler_path,)
                    )
...
def addConstantBlobFile(env, blob_filename, resource_desc, target_arch):
    ...
    if resource_mode == "win_resource":
        # On Windows constants can be accessed as a resource by OxN at run time afterwards.
...
def setupCCompiler(env, lto_mode, pgo_mode, job_count, onefile_compile):
    ...
    if env.gcc_mode and env.lto_mode:
        if env.debug_mode:
            ...
            # thing, and OxN-Python is not affected.
            env.Append(
                LINKFLAGS=[
                    (
                        "-O3"
    ...
    zlib_inline_copy_dir = os.path.join(env.JACK_src, "inline_copy", "zlib")
