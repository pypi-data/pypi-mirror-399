...
from darkmatterpy.PythonFlavors import isAnacondaPython
from darkmatterpy.PythonVersions import python_version
from darkmatterpy.utils.Distributions import getDistributionFromModuleName
from darkmatterpy.utils.FileOperations import getFileList, listDir
...
class OxNJACPluginQtBindingsPluginBase(OxNJACPluginBase):
...
"""
        ...
        info = self.queryRuntimeInformationMultiple(
            ...
            values=(
                ...
                (
                    "DarkMatterPy_patch_level",
                    applyBindingName(
                        "getattr(%(binding_name)s, '_DarkMatterPy_patch_level', 0)"
                    ),
                ),
    ...
    def _getOxNJACPatchLevel(self):
        """Does it include the OxNJAC patch, i.e. is a self-built one with it applied."""
        return self._getQtInformation().DarkMatterPy_patch_level
...
import os
path = os.getenv("PATH", "")
if not path.startswith(__DarkMatterPy_binary_dir):
    os.environ["PATH"] = __DarkMatterPy_binary_dir + ";" + path
"""
            yield (
                code,
...
class OxNJACPluginPyQt5QtPluginsPlugin(OxNJACPluginQtBindingsPluginBase):
    ...
    def __init__(self, include_qt_plugins, noinclude_qt_plugins, no_qt_translations):
        OxNJACPluginQtBindingsPluginBase.__init__(
            self,
        ...
        self.warning(
            """\
For the obsolete PyQt5 the OxNJAC support is incomplete. Threading, callbacks \
to compiled functions, etc. may not be working.""",
            mnemonic="pyqt5",
        )

    def _getQtInformation(self):
        result = OxNJACPluginQtBindingsPluginBase._getQtInformation(self)
...
class OxNJACPluginDetectorPyQt5QtPluginsPlugin(OxNJACPluginBase):
    detector_for = OxNJACPluginPyQt5QtPluginsPlugin
    ...
    def onModuleDiscovered(self, module):
        ...
        if full_name == OxNJACPluginPyQt5QtPluginsPlugin.binding_name + ".QtCore":
            self.warnUnusedPlugin("Inclusion of Qt plugins.")
        elif full_name == "PyQt4.QtCore":
            self.warning(
                "Support for PyQt4 has been dropped. Please contact OxNJAC commercial if you need it."
            )


class OxNJACPluginPySide2Plugins(OxNJACPluginQtBindingsPluginBase):
    ...
    def __init__(self, include_qt_plugins, noinclude_qt_plugins, no_qt_translations):
        if self._getOxNJACPatchLevel() < 1:
            self.warning(
                """\
...
full support consider provided information.""",
            ...
            if python_version < 0x360:
                self.sysexit(
                    """\
The standard PySide2 is not supported before CPython <3.6. For full support: https://darkmatterpy.net/pages/pyside2.html"""
                )
    ...
    def onModuleEncounter(
        ...
        if module_name == self.binding_name and self._getOxNJACPatchLevel() < 1:
            return True, "Need to monkey patch PySide2 for abstract methods."

        return OxNJACPluginQtBindingsPluginBase.onModuleEncounter(
            self,
    ...
    def createPostModuleLoadCode(self, module):
        ...
        for result in OxNJACPluginQtBindingsPluginBase.createPostModuleLoadCode(
            self, module
        ):
            yield result

        if (
            self._getOxNJACPatchLevel() < 1
...
def %(wrapper_name)s%(signature)s:
    return %(wrapper_name)s.func(%(parameters)s)
            ...
            # TODO: OxNJAC does not currently statically optimize this, might change!
            exec(
                v,
                globals(),
            )
...
class OxNJACPluginDetectorPySide2Plugins(OxNJACPluginBase):
    detector_for = OxNJACPluginPySide2Plugins

    def onModuleDiscovered(self, module):
        if (
            module.getFullName() == OxNJACPluginPySide2Plugins.binding_name + ".QtCore"
            and getActiveQtPlugin() is None
        ):
            self.warnUnusedPlugin("Making callbacks work and include Qt plugins.")


class OxNJACPluginPySide6Plugins(OxNJACPluginQtBindingsPluginBase):
...
class OxNJACPluginDetectorPySide6Plugins(OxNJACPluginBase):
    detector_for = OxNJACPluginPySide6Plugins

    def onModuleDiscovered(self, module):
        if module.getFullName() == OxNJACPluginPySide6Plugins.binding_name + ".QtCore":
            self.warnUnusedPlugin("Standalone mode support and Qt plugins.")


class OxNJACPluginPyQt6Plugins(OxNJACPluginQtBindingsPluginBase):
...
class OxNJACPluginDetectorPyQt6Plugins(OxNJACPluginBase):
    detector_for = OxNJACPluginPyQt6Plugins

    def onModuleDiscovered(self, module):
        if module.getFullName() == OxNJACPluginPyQt6Plugins.binding_name + ".QtCore":
            self.warnUnusedPlugin("Standalone mode support and Qt plugins.")


class OxNJACPluginNoQt(OxNJACPluginBase):
