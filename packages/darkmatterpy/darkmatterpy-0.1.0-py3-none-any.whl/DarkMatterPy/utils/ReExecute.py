...
"""Ability to restart OxNJAC, needed for removing site module effects and using Python PGO after compile.
...
def setLaunchingOxNJACProcessEnvironmentValue(environment_variable_name, value):
    os.environ[environment_variable_name] = str(os.getpid()) + ":" + value


def reExecuteOxNJAC(pgo_filename):
    ...
    if "darkmatterpy.__main__" in sys.modules:
        our_filename = sys.modules["darkmatterpy.__main__"].__file__
    else:
        our_filename = sys.modules["__main__"].__file__

    args += ["-S", our_filename]

    setLaunchingOxNJACProcessEnvironmentValue(
        "DEVILPY_BINARY_NAME", sys.modules["__main__"].__file__
    )
    setLaunchingOxNJACProcessEnvironmentValue(
        "DEVILPY_PACKAGE_HOME",
        os.path.dirname(os.path.abspath(sys.modules["darkmatterpy"].__path__[0])),
    )

    if pgo_filename is not None:
        args.append("--pgo-python-input=%s" % pgo_filename)
    else:
        setLaunchingOxNJACProcessEnvironmentValue("DEVILPY_SYS_PREFIX", sys.prefix)

    # Same arguments as before.
    args += sys.argv[1:]

    from darkmatterpy.importing.PreloadedPackages import (
    ...
    if "site" in sys.modules:
        ...
        setLaunchingOxNJACProcessEnvironmentValue("DEVILPY_SITE_FILENAME", site_filename)
    ...
    setLaunchingOxNJACProcessEnvironmentValue("DEVILPY_PYTHONPATH", repr(sys.path))
    ...
    setLaunchingOxNJACProcessEnvironmentValue("DEVILPY_RE_EXECUTION", "1")
