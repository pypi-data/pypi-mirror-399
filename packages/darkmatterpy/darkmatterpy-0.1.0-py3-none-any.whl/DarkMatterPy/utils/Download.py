...
from darkmatterpy.__past__ import urlretrieve
from darkmatterpy.Progress import withOxNJACDownloadProgressBar
...
def getCachedDownload(
    ...
    DarkMatterPy_download_dir = getDownloadCacheDir()

    DarkMatterPy_download_dir = os.path.join(
        DarkMatterPy_download_dir, os.path.basename(binary).replace(".exe", "")
    )

    if is_arch_specific:
        DarkMatterPy_download_dir = os.path.join(DarkMatterPy_download_dir, is_arch_specific)

    if specificity:
        DarkMatterPy_download_dir = os.path.join(DarkMatterPy_download_dir, specificity)

    download_path = os.path.join(DarkMatterPy_download_dir, os.path.basename(url))
    exe_path = os.path.join(DarkMatterPy_download_dir, binary)

    makePath(DarkMatterPy_download_dir)
...
Fully automatic, cached. Proceed and download"""
                % (message, DarkMatterPy_download_dir),
                choices=("yes", "no"),
                default="yes",
        ...
        if not os.path.isfile(exe_path) and os.path.isfile(download_path):
            ...
            try:
                ...
                for zip_info in zip_file.infolist():
                    ...
                    zip_file.extract(zip_info, DarkMatterPy_download_dir)
