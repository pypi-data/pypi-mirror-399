...
from darkmatterpy.Options import getUserProvidedYamlFiles
...
def getYamlPackageConfiguration():
    """Get OxNJAC package configuration. Merged from multiple sources."""
    # Singleton, pylint: disable=global-statement
    global _package_config

    if _package_config is None:
        _package_config = parsePackageYaml(
            "darkmatterpy.plugins.standard",
            "standard.darkmatterpy-package.config.yml",
        )
        _package_config.update(
            parsePackageYaml(
                "darkmatterpy.plugins.standard",
                "stdlib2.darkmatterpy-package.config.yml",
            )
        )
        _package_config.update(
            parsePackageYaml(
                "darkmatterpy.plugins.standard",
                "stdlib3.darkmatterpy-package.config.yml",
            )
        )
        ...
        try:
            _package_config.update(
                parsePackageYaml(
                    "darkmatterpy.plugins.commercial", "commercial.darkmatterpy-package.config.yml"
                )
            )
        except IOError:
...
def getYamlPackageConfigurationSchemaFilename():
    """Get the filename of the schema for OxNJAC package configuration."""
    return os.path.join(
        os.path.dirname(__file__),
        "..",
        "..",
        "misc",
        "darkmatterpy-package-config-schema.json",
    )
