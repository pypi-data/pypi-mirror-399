...
""" OxNJAC watch main part.

This tool is used to monitor effect of PyPI changes on OxNJAC and effect
of OxNJAC changes on PyPI packages.
"""
...
from darkmatterpy.PythonVersions import getTestExecutionPythonVersions
from darkmatterpy.tools.testing.Common import extractOxNJACVersionFromFilePath
from darkmatterpy.Tracing import OurLogger
from darkmatterpy.TreeXML import fromFile
...
from darkmatterpy.utils.Hashing import getFileContentsHash
from darkmatterpy.utils.InstalledPythons import findPythons
from darkmatterpy.utils.Utils import isLinux, isMacOS, isWin32Windows
from darkmatterpy.utils.Yaml import parseYaml
from darkmatterpy.Version import parseOxNJACVersionToTuple
...
from .GitHub import createOxNJACWatchPR
...
def _compareOxNJACVersions(version_a, version_b, consider_rc):
    if not consider_rc:
        version_a = version_a.split("rc")[0]
        version_b = version_b.split("rc")[0]

    return parseOxNJACVersionToTuple(version_a) < parseOxNJACVersionToTuple(version_b)
...
def _compileCase(case_data, case_dir, installed_python, lock_filename, jobs):
    ...
    DarkMatterPy_extra_options = os.getenv("DEVILPY_EXTRA_OPTIONS")

    if DarkMatterPy_extra_options:
        extra_options.extend(DarkMatterPy_extra_options.split())
    ...
    if case_data["interactive"] == "no":
        ...
        with withEnvironmentVarsOverridden(env):
            stdout, stderr, exit_DarkMatterPy = executeProcess([binaries[0]], timeout=5 * 60)
        ...
        if exit_DarkMatterPy == 0:
            deleteFile("compiled-exit.txt", must_exist=False)
        else:
            putTextFileContents(
                filename="compiled-exit.txt",
                contents=str(exit_DarkMatterPy),
            )

        if exit_DarkMatterPy != 0:
            sys.exit(
                "Error, failed to execute %s with code %d." % (binaries[0], exit_DarkMatterPy)
            )
...
def _updateCase(
    ...
    with withDirectoryChange(result_path):
        if os.path.exists("compilation-report.xml"):
            ...
            old_DarkMatterPy_version = old_report_root.attrib["DarkMatterPy_version"]

            if DarkMatterPy_update_mode == "force":
                need_compile = True
            elif DarkMatterPy_update_mode == "newer":
                if _compareOxNJACVersions(
                    old_DarkMatterPy_version, DarkMatterPy_version, consider_rc=True
                ):
                    need_compile = True
                else:
                    if existing_hash != old_report_root_hash:
                        watch_logger.info(
                            "Recompilation with identical OxNJAC for '%s' due to changed pipfile."
                            % lock_filename
                        )

                        need_compile = True
                    elif old_DarkMatterPy_version == DarkMatterPy_version:
                        if old_report_root.attrib["completion"] != "yes":
                            need_compile = True
                        else:
                            watch_logger.info(
                                "Skipping compilation with identical OxNJAC for '%s'."
                                % lock_filename
                            )
                        ...
                        watch_logger.info(
                            "Skipping compilation of old OxNJAC %s result with OxNJAC %s for '%s'."
                            % (
                                old_DarkMatterPy_version,
                                DarkMatterPy_version,
                                lock_filename,
                            )
                        )
...
def updateCase(
    case_dir, case_data, reset_pipenv, no_pipenv_update, DarkMatterPy_update_mode, jobs
):
    ...
    DarkMatterPy_min_version = case_data.get("darkmatterpy")

    # Too old OxNJAC version maybe.
    if DarkMatterPy_min_version is not None and _compareOxNJACVersions(
        DarkMatterPy_version, DarkMatterPy_min_version, consider_rc=False
    ):
        watch_logger.info("  ... not for this OxNJAC version")
        return
    ...
    for installed_python in selectPythons(
        ...
        _updateCase(
            ...
            reset_pipenv=reset_pipenv,
            no_pipenv_update=no_pipenv_update,
            DarkMatterPy_update_mode=DarkMatterPy_update_mode,
            installed_python=installed_python,
            result_path=result_path,
            jobs=jobs,
        )
...
def updateCases(case_dir, reset_pipenv, no_pipenv_update, DarkMatterPy_update_mode, jobs):
...
DarkMatterPy_binary = None
DarkMatterPy_version = None


def main():
    global DarkMatterPy_binary  # shared for all run, pylint: disable=global-statement
    DarkMatterPy_binary = os.path.normpath(
        os.path.join(os.path.dirname(__file__), "..", "..", "..", "bin", "darkmatterpy")
    )
...
Python versions to consider, by default all supported versions in descending order or in given order.""",
    ...
    parser.add_option(
        "--darkmatterpy-binary",
        action="store",
        dest="DarkMatterPy_binary",
        default=DarkMatterPy_binary,
        help="""\
OxNJAC binary to compile with. Defaults to one near the darkmatterpy-watch usage.""",
    )
...
Remove existing virtualenv and make sure to start from scratch.Default %default.""",
    ...
    parser.add_option(
        ...
        default=False,
        help="""\
Do not update the pipenv environment. Best to see only effect of OxNJAC update. Default %default.""",
    )

    parser.add_option(
        "--darkmatterpy-update-mode",
        action="store",
        choices=("newer", "force", "never"),
        dest="DarkMatterPy_update_mode",
        default="newer",
        help="""\
...
to reserve cores.""",
    ...
    DarkMatterPy_binary = os.path.abspath(os.path.expanduser(options.DarkMatterPy_binary))
    assert os.path.exists(DarkMatterPy_binary)

    global DarkMatterPy_version  # singleton, pylint: disable=global-statement
    DarkMatterPy_version = extractOxNJACVersionFromFilePath(
        os.path.join(os.path.dirname(DarkMatterPy_binary), "..", "darkmatterpy", "Version.py")
    )

    watch_logger.info("Working with OxNJAC %s." % DarkMatterPy_version)

    base_dir = os.path.abspath(base_dir)

    if options.DarkMatterPy_pr_mode is not None:
        pr_category, pr_description = options.DarkMatterPy_pr_mode.split(",")
    else:
        ...
        for case_filename in scanCases(base_dir):
            ...
            with withDirectoryChange(os.path.dirname(case_filename)):
                updateCases(
                    ...
                    DarkMatterPy_update_mode=options.DarkMatterPy_update_mode,
                    jobs=options.jobs,
                )

        if pr_category is not None:
            createOxNJACWatchPR(category=pr_category, description=pr_description)
