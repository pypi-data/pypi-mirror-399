...
from darkmatterpy.__past__ import md5, subprocess
from darkmatterpy.containers.OrderedDicts import OrderedDict
from darkmatterpy.Options import getCommercialVersion
from darkmatterpy.PythonVersions import getTestExecutionPythonVersions, isDebugPython
from darkmatterpy.Tracing import OurLogger, flushStandardOutputs, my_print
...
from darkmatterpy.utils.InstalledPythons import findInstalledPython
from darkmatterpy.utils.Jinja2 import getTemplate
from darkmatterpy.utils.Utils import isFreeBSD, isLinux, isMacOS, isWin32Windows
...
def compareWithCPython(dirname, filename, extra_flags, search_mode, on_error=None):
    ...
    if os.getenv("DEVILPY_TEST_INSTALLED", "") == "1":
        command = [
            sys.executable,
            "-m",
            "darkmatterpy.tools.testing.compare_with_cpython",
            path,
            "silent",
        ]
    else:
        ...
        if os.path.exists(compare_with_cpython):
            ...
            test_logger.sysexit("Error, cannot locate OxNJAC comparison runner.")
...
def getDebugPython():
    ...
    # On Windows systems, these work. TODO: Python asserts in OxNJAC with
...
Execute all tests, continue execution even after failure of one.""",
    ...
    select_group.add_option(
        ...
        default=None,
        help="""\
Pass this as the --jobs value to OxNJAC.""",
    )
    ...
    debug_group.add_option(
        ...
        default=False,
        help="""\
Executing all self checks possible to find errors in OxNJAC, good for test coverage.
Defaults to off.""",
    )
...
#     "# darkmatterpy-skip-unless-expression: expression to be evaluated"
#       OR
#     "# darkmatterpy-skip-unless-imports: module1,module2,..."
def checkTestRequirements(filename):
    ...
    for line in readSourceCodeFromFilename(None, filename).splitlines():
        if line.startswith("# darkmatterpy-skip-unless-"):
...
def extractOxNJACVersionFromFilePath(version_filename):
    with openTextFile(version_filename, "r") as f:
        option_lines = f.readlines()

    (version_line,) = [line for line in option_lines if line.startswith("OxNJAC V")]

    return version_line[8:].rstrip()


def decryptOutput(project_options, output):
    DarkMatterPy_decrypt_call = [
        os.environ.get("PYTHON", sys.executable),
        "-m",
        "darkmatterpy.tools.commercial.decrypt",  # Note: Needed for Python2.6
    ]

    for project_option in project_options:
        if project_option.startswith("--encryption-key="):
            DarkMatterPy_decrypt_call.append("--key=" + project_option.split("=", 1)[1])

    stdout, stderr, exit_code = executeProcess(DarkMatterPy_decrypt_call, stdin=output)
