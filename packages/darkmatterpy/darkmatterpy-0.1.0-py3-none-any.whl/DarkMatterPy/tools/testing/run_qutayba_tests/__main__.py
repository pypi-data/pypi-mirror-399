...
""" Main front-end to the tests of OxNJAC.
...
from darkmatterpy.tools.Basics import goHome
...
from darkmatterpy.utils.Execution import check_call, check_output, getNullOutput
...
from darkmatterpy.utils.Timing import TimerReport
from darkmatterpy.utils.Utils import hasOnefileSupportedOS, hasStandaloneSupportedOS
...
def parseOptions():
    ...
    parser.add_option(
        ...
        default=True,
        help="""\
The basic tests, execute these to check if OxNJAC is healthy.
Default is %default.""",
    )
    ...
    parser.add_option(
        ...
        default=True,
        help="""\
The syntax tests, execute these to check if OxNJAC handles Syntax errors fine.
Default is %default.""",
    )
    ...
    parser.add_option(
        ...
        default=True,
        help="""\
The programs tests, execute these to check if OxNJAC handles programs, e.g.
import recursions, etc. fine. Default is %default.""",
    ...
    parser.add_option(
        ...
        default=True,
        help="""\
The packages tests, execute these to check if OxNJAC handles packages, e.g.
import recursions, etc. fine. Default is %default.""",
    )
    ...
    parser.add_option(
        ...
        default=True,
        help="""\
The plugins tests, execute these to check if OxNJAC handles its own plugin
interfaces, e.g. user plugins, etc. fine. Default is %default.""",
    ...
    parser.add_option(
        ...
        default=True,
        help="""\
The optimization tests, execute these to check if OxNJAC does optimize certain
constructs fully away. Default is %default.""",
    ...
    parser.add_option(
        ...
        help="""\
The standalone tests, execute these to check if OxNJAC standalone mode, e.g.
...
Default is %default.""",
    ...
    parser.add_option(
        ...
        help="""\
The onefile tests, execute these to check if OxNJAC works in onefile mode, e.g.
not referring to outside, important 3rd library packages like PyQt fine.
Default is %default.""",
    )
    ...
    parser.add_option(
        ...
        default=True,
        help="""\
The reflection test compiles OxNJAC with OxNJAC, and then OxNJAC with the
compile OxNJAC and compares the outputs. Default is %default.""",
    )
...
Make a coverage analysis, that does not really check. Default is %default.""",
    ...
    parser.add_option(
        ...
        default=False,
        help="""\
Allow OxNJAC to download code if necessary, e.g. dependency walker on Windows. Default is %default.""",
    )
...
Enforce the use of MinGW64 on Windows. Defaults to off.""",
    ...
    parser.add_option(
        ...
        default=None,
        help="""\
The value of --jobs to use when calling OxNJAC. Use negative values to
keep cores unused. OxNJAC uses available cores by default. Defaults to
not being passed.""",
    )
...
def publishCoverageData():
    ...
    with openTextFile("data.coverage", "w") as data_file:
        ...
        with withDirectoryChange(source_dir):
            DarkMatterPy_id = check_output("git rev-parse HEAD".split())
        DarkMatterPy_id = DarkMatterPy_id.strip()

        if sys.version_info > (3,):
            DarkMatterPy_id = DarkMatterPy_id.decode()
