...
import darkmatterpy.code_generation.BinaryOperationHelperDefinitions
import darkmatterpy.code_generation.CodeGeneration
import darkmatterpy.code_generation.ComparisonCodes
import darkmatterpy.code_generation.Namify
import darkmatterpy.nodes.NetworkxNodes
import darkmatterpy.nodes.PackageMetadataNodes
import darkmatterpy.nodes.PackageResourceNodes
import darkmatterpy.nodes.SideEffectNodes
import darkmatterpy.nodes.TensorflowNodes
...
import darkmatterpy.specs.BuiltinTypeOperationSpecs
import darkmatterpy.specs.HardImportSpecs
import darkmatterpy.tree.Building
from darkmatterpy.containers.OrderedDicts import OrderedDict
from darkmatterpy.containers.OrderedSets import OrderedSet
from darkmatterpy.nodes.ImportNodes import hard_modules_non_stdlib
from darkmatterpy.nodes.NodeMetaClasses import NodeCheckMetaClass
...
from darkmatterpy.utils.Jinja2 import getTemplate
...
def _getMixinForShape(shape):
    ...
    if shape is tshape_str:
        return "darkmatterpy.nodes.ExpressionShapeMixins.ExpressionStrShapeExactMixin"
    elif shape is tshape_list:
        return "darkmatterpy.nodes.ExpressionShapeMixins.ExpressionListShapeExactMixin"
    elif shape is tshape_tuple:
        return "darkmatterpy.nodes.ExpressionShapeMixins.ExpressionTupleShapeExactMixin"
    elif shape is tshape_int:
        return "darkmatterpy.nodes.ExpressionShapeMixins.ExpressionIntShapeExactMixin"
    elif shape is tshape_bool:
        return "darkmatterpy.nodes.ExpressionShapeMixins.ExpressionBoolShapeExactMixin"
    elif shape is tshape_none:
        return "darkmatterpy.nodes.ExpressionShapeMixins.ExpressionNoneShapeExactMixin"
    elif shape is tshape_dict:
        return "darkmatterpy.nodes.ExpressionShapeMixins.ExpressionDictShapeExactMixin"
    elif shape is tshape_bytes:
        return "darkmatterpy.nodes.ExpressionShapeMixins.ExpressionBytesShapeExactMixin"
    else:
        assert False, shape
...
processTypeShapeAttribute(
    "tshape_type",
    darkmatterpy.specs.BuiltinTypeOperationSpecs,
    python2_type_methods,
    python3_type_methods,
)
...
def emitGenerationWarning(emit, doc_string, template_name):
    ...
    for spec_descriptions in getSpecVersions(darkmatterpy.specs.HardImportSpecs):
...
def makeAttributeNodes():
    filename_python = "darkmatterpy/nodes/AttributeNodesGenerated.py"
    ...
    with withFileOpenedAndAutoFormatted(
        ...
        emit("from darkmatterpy.specs.BuiltinParameterSpecs import extractBuiltinArgs")

        emit("from darkmatterpy.nodes.ConstantRefNodes import makeConstantRefNode")
        emit(
            """\
from darkmatterpy.nodes.NodeMakingHelpers import (
    ...
    makeRaiseExceptionReplacementExpression
        ...
        emit(
            "from darkmatterpy.nodes.KeyValuePairNodes import makeKeyValuePairExpressionsFromKwArgs"
        )

        emit("from darkmatterpy.nodes.AttributeNodes import makeExpressionAttributeLookup")

        emit("from darkmatterpy.PythonVersions import getDictFromkeysNoArgErrorMessage")
...
def makeBuiltinOperationNodes():
    filename_python = "darkmatterpy/nodes/BuiltinOperationNodeBasesGenerated.py"
...
def makeChildrenHavingMixinNodes():
    ...
    filename_python = "darkmatterpy/nodes/ChildrenHavingMixins.py"
    filename_python2 = "darkmatterpy/nodes/ExpressionBasesGenerated.py"
    filename_python3 = "darkmatterpy/nodes/StatementBasesGenerated.py"
    ...
    with withFileOpenedAndAutoFormatted(
        ...
        emit1(
            """
from darkmatterpy.nodes.Checkers import (
    checkStatementsSequenceOrNone,
...
def makeHardImportNodes():
    ...
    filename_python = "darkmatterpy/nodes/HardImportNodesGenerated.py"
