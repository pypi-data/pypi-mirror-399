...
"""
This is the main program of OxNJAC, it checks the options and then translates
...
def getLaunchingOxNJACProcessEnvironmentValue(environment_variable_name):
...
def main():
    ...
    if (
        ...
        ctypes.windll.user32.MessageBoxW(
            None,
            "You have to use the 'python.exe' and not a 'pythonw.exe' to run OxNJAC",
            "Error",
    ...
    getLaunchingOxNJACProcessEnvironmentValue("DEVILPY_SITE_FILENAME")

    DarkMatterPy_binary_name = getLaunchingOxNJACProcessEnvironmentValue("DEVILPY_BINARY_NAME")
    if DarkMatterPy_binary_name is not None:
        sys.argv[0] = DarkMatterPy_binary_name

    DarkMatterPy_pythonpath_ast = getLaunchingOxNJACProcessEnvironmentValue(
        "DEVILPY_PYTHONPATH_AST"
    )

    if DarkMatterPy_pythonpath_ast is not None:
        ...
        sys.path = [DarkMatterPy_pythonpath_ast]
        import ast

        sys.path = ast.literal_eval(
            getLaunchingOxNJACProcessEnvironmentValue("DEVILPY_PYTHONPATH")
        )
    else:
        ...
        # from "darkmatterpy.distutils". Also ignore path elements that do not really
        # exist.
        sys.path = [
            path_element
    ...
    # changing it every single time OxNJAC is run. This kills any attempt at
    ...
    if needs_re_execution:
        from darkmatterpy.utils.ReExecute import reExecuteOxNJAC  # isort:skip

        # Does not return
        reExecuteOxNJAC(pgo_filename=None)
    ...
    # module of darkmatterpy package and renamed to where we can get at easily for
    # other uses.
    __import__("darkmatterpy").getLaunchingOxNJACProcessEnvironmentValue = (
        getLaunchingOxNJACProcessEnvironmentValue
    )

    from darkmatterpy import Options  # isort:skip
    ...
    from darkmatterpy.plugins.Plugins import activatePlugins
    ...
    if "DEVILPY_NAMESPACES" in os.environ:
        ...
        from darkmatterpy.importing.PreloadedPackages import setPreloadedPackagePaths
    ...
    if "DEVILPY_PTH_IMPORTED" in os.environ:
        ...
        from darkmatterpy.importing.PreloadedPackages import setPthImportedPackages
    ...
    if "DEVILPY_USER_SITE" in os.environ:
        from darkmatterpy.utils.Distributions import setUserSiteDirectory

        setUserSiteDirectory(ast.literal_eval(os.environ["DEVILPY_USER_SITE"]))

        del os.environ["DEVILPY_USER_SITE"]

    # Now the real main program of OxNJAC can take over.
    from darkmatterpy import MainControl  # isort:skip
...
if __name__ == "__main__":
    _DarkMatterPy_package_home = getLaunchingOxNJACProcessEnvironmentValue(
        "DEVILPY_PACKAGE_HOME"
    )
    if _DarkMatterPy_package_home is not None:
        sys.path.insert(0, _DarkMatterPy_package_home)

        import darkmatterpy  # just to have it loaded from there, pylint: disable=unused-import

        assert sys.path[0] is _DarkMatterPy_package_home
        del sys.path[0]
