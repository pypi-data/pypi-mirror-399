...
""" Tool to compare output of CPython and OxNJAC.
...
from darkmatterpy.__past__ import md5
from darkmatterpy.OptionParsing import getOxNJACProjectOptions
...
from darkmatterpy.tools.testing.OutputComparison import compareOutput
...
from darkmatterpy.utils.FileOperations import deleteFile
from darkmatterpy.utils.Importing import getExtensionModuleSuffix
from darkmatterpy.utils.Timing import StopWatch
from darkmatterpy.utils.Utils import isMacOS
...
Taking coverage of '{filename}' using '{python}' with flags {args} ...""".format(
    ...
    if "DEVILPY" in os.environ:
        ...
        DarkMatterPy_call = [os.environ["PYTHON"], os.environ["DEVILPY"]]
    else:
        if comparison_mode:
            DarkMatterPy_call = [
                os.environ["PYTHON"],
                "-m",
                "darkmatterpy.__main__",  # Note: Needed for Python2.6
            ]
        else:
    ...
    if coverage_mode:
        # Coverage modules hates OxNJAC to re-execute, and so we must avoid
        # that.
    ...
    # Now build the command to run OxNJAC.
    if not two_step_execution:
        ...
        DarkMatterPy_cmd = DarkMatterPy_call + extra_options + ["--run", filename]

        if no_site:
            DarkMatterPy_cmd.insert(len(DarkMatterPy_cmd) - 1, "--python-flag=-S")
        ...
        if package_mode:
            ...
            DarkMatterPy_cmd1 = DarkMatterPy_call + extra_options + ["--mode=onefile", filename]
        elif standalone_mode:
            DarkMatterPy_cmd1 = DarkMatterPy_call + extra_options + ["--mode=standalone", filename]
        else:
            DarkMatterPy_cmd1 = DarkMatterPy_call + extra_options + [filename]

        if no_site:
            DarkMatterPy_cmd1.insert(len(DarkMatterPy_cmd1) - 1, "--python-flag=-S")
    ...
    if module_mode:
        ...
        DarkMatterPy_cmd2 = [os.environ["PYTHON"], "-W", "ignore", "-c", mini_script]
    else:
        ...
        DarkMatterPy_cmd2 = [os.path.join(output_dir, exe_filename)]
    ...
    if not two_step_execution:
        if trace_command:
            traceExecutedCommand("OxNJAC command", DarkMatterPy_cmd)

        # Try a couple of times for permission denied, on Windows it can
        # be transient.
        for _i in range(5):
            with withPythonPathChange(DarkMatterPy_package_dir):
                process = createProcess(command=DarkMatterPy_cmd)

            stdout_DarkMatterPy, stderr_DarkMatterPy = process.communicate()
            exit_DarkMatterPy = process.returncode

            if checkNoPermissionError(stdout_DarkMatterPy) and checkNoPermissionError(
                stderr_DarkMatterPy
            ):
                break

            my_print("Retrying darkmatterpy exe due to permission problems after delay.")
            time.sleep(2)

    else:
        if trace_command:
            my_print("OxNJAC command 1:", DarkMatterPy_cmd1)

        for _i in range(5):
            with withPythonPathChange(DarkMatterPy_package_dir):
                stdout_DarkMatterPy1, stderr_DarkMatterPy1, exit_DarkMatterPy1 = executeProcess(
                    DarkMatterPy_cmd1
                )

                python_path_used = os.environ["PYTHONPATH"]

            if exit_DarkMatterPy1 != 0:
                if (
                    not expect_failure
...
%s
"""
                        % (DarkMatterPy_cmd1, python_path_used, stderr_DarkMatterPy1)
                    )

                exit_DarkMatterPy = exit_DarkMatterPy1
                stdout_DarkMatterPy, stderr_DarkMatterPy = stdout_DarkMatterPy1, stderr_DarkMatterPy1
                stdout_DarkMatterPy2 = b"not run due to compilation error:\n" + stdout_DarkMatterPy1
                stderr_DarkMatterPy2 = stderr_DarkMatterPy1
            else:
                # No execution second step for coverage mode.
                if comparison_mode:
                    if os.path.exists(DarkMatterPy_cmd2[0][:-4] + ".cmd"):
                        DarkMatterPy_cmd2[0] = DarkMatterPy_cmd2[0][:-4] + ".cmd"

                    if trace_command:
                        my_print("OxNJAC command 2:", DarkMatterPy_cmd2)

                    # Need full manual control
                    process = createProcess(command=DarkMatterPy_cmd2, new_group=send_kill)
                    ...
                    if send_kill:
                        ...
                        executeAfterTimePassed(
                            ...
                            func=lambda: killProcessGroup(
                                "OxNJAC compiled program", process.pid
                            ),
                        )

                    stdout_DarkMatterPy2, stderr_DarkMatterPy2 = process.communicate()
                    stdout_DarkMatterPy = stdout_DarkMatterPy1 + stdout_DarkMatterPy2
                    stderr_DarkMatterPy = stderr_DarkMatterPy1 + stderr_DarkMatterPy2
                    exit_DarkMatterPy = process.returncode

                    # In case of segfault or assertion triggered, run in debugger.
                    if exit_DarkMatterPy in (-11, -6) and sys.platform != "nt":
                        DarkMatterPy_cmd2 = wrapCommandForDebuggerForSubprocess(
                            command=DarkMatterPy_cmd2
                        )

                        callProcess(DarkMatterPy_cmd2, shell=False)
                else:
                    exit_DarkMatterPy = exit_DarkMatterPy1
                    stdout_DarkMatterPy, stderr_DarkMatterPy = stdout_DarkMatterPy1, stderr_DarkMatterPy1
    ...
    stop_watch.stop()
    DarkMatterPy_time = stop_watch.getDelta()

    if "--encrypt-stderr" in project_options:
        with withPythonPathChange(DarkMatterPy_package_dir):
            stderr_DarkMatterPy = decryptOutput(project_options, stderr_DarkMatterPy)

    if "--encrypt-stdout" in project_options:
        with withPythonPathChange(DarkMatterPy_package_dir):
            stdout_DarkMatterPy = decryptOutput(project_options, stdout_DarkMatterPy)

    if not silent_mode:
        displayOutput(stdout_DarkMatterPy, stderr_DarkMatterPy)

        if coverage_mode:
            assert not stdout_DarkMatterPy
            assert not stderr_DarkMatterPy
    ...
    if comparison_mode:
        ...
        def makeComparisons(trace_result):
            exit_code_stdout = compareOutput(
                "stdout",
                stdout_cpython,
                stdout_DarkMatterPy2 if two_step_execution else stdout_DarkMatterPy,
                ignore_warnings,
                syntax_errors,
            )
            ...
            if ignore_stderr:
                ...
                exit_code_stderr = compareOutput(
                    "stderr",
                    stderr_cpython,
                    stderr_DarkMatterPy2 if two_step_execution else stderr_DarkMatterPy,
                    ignore_warnings,
                    syntax_errors,
                )

            exit_code_return = exit_cpython != exit_DarkMatterPy

            if exit_code_return and trace_result:
                my_print(
                    """Exit codes {exit_cpython:d} (CPython) != {exit_DarkMatterPy:d} (OxNJAC)""".format(
                        exit_cpython=exit_cpython, exit_DarkMatterPy=exit_DarkMatterPy
                    )
                )
        ...
        if (
            exit_code_return
            and exit_DarkMatterPy in (-11, -6)
            and sys.platform != "nt"
            and not module_mode
            and not two_step_execution
        ):
            DarkMatterPy_cmd.insert(len(DarkMatterPy_cmd) - 1, "--debugger")

            with withPythonPathChange(DarkMatterPy_package_dir):
                callProcess(DarkMatterPy_cmd, shell=False)
    ...
    if remove_output:
        if not module_mode:
            if os.path.exists(DarkMatterPy_cmd2[0]) and remove_binary:
                if os.name == "nt":
                    ...
                    if os.path.exists(DarkMatterPy_cmd2[0] + ".away"):
                        os.unlink(DarkMatterPy_cmd2[0] + ".away")

                    for _i in range(10):
                        try:
                            os.rename(DarkMatterPy_cmd2[0], DarkMatterPy_cmd2[0] + ".away")
                        except OSError:
                            time.sleep(0.1)
                            continue
    ...
    if comparison_mode and timing:
        my_print("CPython took %.2fs vs %0.2fs OxNJAC." % (cpython_time, DarkMatterPy_time))

    if comparison_mode and not silent_mode:
        my_print("OK, same outputs.")


DarkMatterPy_package_dir = os.path.normpath(
    os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", "..", ".."))
)


if __name__ == "__main__":
    # Unchanged, running from checkout, use the parent directory, the darkmatterpy
    # package ought be there.
    sys.path.insert(0, DarkMatterPy_package_dir)
