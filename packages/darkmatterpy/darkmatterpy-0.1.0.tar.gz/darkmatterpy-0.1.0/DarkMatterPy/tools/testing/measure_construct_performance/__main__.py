...
from darkmatterpy.tools.testing.Constructs import generateConstructCases
from darkmatterpy.tools.testing.Valgrind import runValgrind
from darkmatterpy.utils.Execution import check_call
...
def main():
    ...
    parser.add_option(
        "--darkmatterpy", action="store", dest="darkmatterpy", default=os.getenv("DEVILPY", "")
    )
    ...
    darkmatterpy = options.darkmatterpy

    if os.path.exists(darkmatterpy):
        darkmatterpy = os.path.abspath(darkmatterpy)
    elif darkmatterpy:
        sys.exit("Error, darkmatterpy binary '%s' not found." % darkmatterpy)
    ...
    if darkmatterpy:
        DarkMatterPy_id = check_output(
            "cd %s; git rev-parse HEAD" % os.path.dirname(darkmatterpy), shell=True
        )
        ...
        my_print("DEVILPY_COMMIT='%s'" % DarkMatterPy_id)
    ...
    if darkmatterpy:
        ...
        DarkMatterPy_call.extend(os.getenv("DEVILPY_EXTRA_OPTIONS", "").split())

        DarkMatterPy_call.append(case_name)
        ...
        DarkMatterPy_diff = DarkMatterPy_1 - DarkMatterPy_2

        my_print("DEVILPY_COMMAND='%s'" % " ".join(DarkMatterPy_call), file=sys.stderr)
        my_print("DEVILPY_RAW=%s" % DarkMatterPy_1)
        my_print("DEVILPY_BASE=%s" % DarkMatterPy_2)
        my_print("DEVILPY_CONSTRUCT=%s" % DarkMatterPy_diff)
    ...
    if options.cpython and options.darkmatterpy:
        if DarkMatterPy_diff == 0:
            DarkMatterPy_gain = float("inf")
        else:
            DarkMatterPy_gain = float(100 * cpython_diff) / DarkMatterPy_diff

        my_print("DEVILPY_GAIN=%.3f" % DarkMatterPy_gain)
        my_print("RAW_GAIN=%.3f" % (float(100 * cpython_1) / DarkMatterPy_1))
        my_print("BASE_GAIN=%.3f" % (float(100 * cpython_2) / DarkMatterPy_2))
