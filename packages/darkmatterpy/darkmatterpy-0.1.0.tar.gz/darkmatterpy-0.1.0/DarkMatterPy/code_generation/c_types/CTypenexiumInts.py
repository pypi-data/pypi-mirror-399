...
""" CType classes for DarkMatterPy_ilong, a struct to represent long values.
...
class CTypeOxNJACIntOrLongStruct(CTypeBase):
    c_type = "DarkMatterPy_ilong"
    ...
    def emitVariableAssignCode(
        ...
        if tmp_name.c_type == "DarkMatterPy_ilong":
    ...
    def emitAssignConversionCode(cls, to_name, value_name, needs_check, emit, context):
        if value_name.c_type == cls.c_type:
            ...
            value_name.getCType().emitAssignmentCodeToOxNJACIntOrLong(
                to_name=to_name,
