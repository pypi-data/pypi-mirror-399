...
def getQuickMethodCallCode(args_count):
    template = getTemplateC(
        "darkmatterpy.code_generation", "CodeTemplateCallsMethodPositional.c.j2"
    )
    return template.render(args_count=args_count)


def getQuickMixedCallCode(args_count, has_tuple_arg, has_dict_values):
    template = getTemplateC("darkmatterpy.code_generation", "CodeTemplateCallsMixed.c.j2")
...
def getQuickMethodDescriptorCallCode(args_count):
    template = getTemplateC(
        "darkmatterpy.code_generation", "CodeTemplateCallsPositionalMethodDescr.c.j2"
    )
