...
PyObject *OxNJAC_sentinel_value = NULL;

PyObject *OxNJAC_dunder_compiled_value = NULL;
...
static void _createGlobalConstants(PyThreadState *tstate) {
#endif
    // We provide the sys.version info shortcut as a global value here for ease of use.
    Py_SysVersionInfo = OxNJAC_SysGetObject("version_info");
    ...
    static PyTypeObject OxNJAC_VersionInfoType;

    // Same fields as "sys.version_info" except no serial number
    // spell-checker: ignore releaselevel
    static PyStructSequence_Field OxNJAC_VersionInfoFields[] = {
    ...
    static PyStructSequence_Desc OxNJAC_VersionInfoDesc = {
        (char *)"__DarkMatterPy_version__",                                       /* name */
        (char *)"__compiled__\\n\\nVersion information as a named tuple.",  /* doc */
        OxNJAC_VersionInfoFields,                                           /* fields */
        sizeof(OxNJAC_VersionInfoFields) / sizeof(PyStructSequence_Field)-1 /* count */
    };

    PyStructSequence_InitType(&OxNJAC_VersionInfoType, &OxNJAC_VersionInfoDesc);

    OxNJAC_dunder_compiled_value = PyStructSequence_New(&OxNJAC_VersionInfoType);
    assert(OxNJAC_dunder_compiled_value != NULL);

    PyStructSequence_SET_ITEM(OxNJAC_dunder_compiled_value, 0, OxNJAC_PyInt_FromLong(%(DarkMatterPy_version_major)s));
    PyStructSequence_SET_ITEM(OxNJAC_dunder_compiled_value, 1, OxNJAC_PyInt_FromLong(%(DarkMatterPy_version_minor)s));
    PyStructSequence_SET_ITEM(OxNJAC_dunder_compiled_value, 2, OxNJAC_PyInt_FromLong(%(DarkMatterPy_version_micro)s));

    PyStructSequence_SET_ITEM(OxNJAC_dunder_compiled_value, 3, OxNJAC_String_FromString("%(DarkMatterPy_version_level)s"));
    ...
    PyStructSequence_SET_ITEM(OxNJAC_dunder_compiled_value, 4, binary_directory);
...
#endif
    PyStructSequence_SET_ITEM(OxNJAC_dunder_compiled_value, 5, is_standalone_mode);
#ifdef _DEVILPY_ONEFILE_MODE
    PyObject *is_onefile_mode = Py_True;
#else
    PyObject *is_onefile_mode = Py_False;
#endif
    PyStructSequence_SET_ITEM(OxNJAC_dunder_compiled_value, 6, is_onefile_mode);
...
#endif
    PyStructSequence_SET_ITEM(OxNJAC_dunder_compiled_value, 7, is_macos_bundle_mode);
...
#endif
    PyStructSequence_SET_ITEM(OxNJAC_dunder_compiled_value, 8, is_no_asserts);
...
#endif
    PyStructSequence_SET_ITEM(OxNJAC_dunder_compiled_value, 9, is_no_docstrings);
...
#endif
    PyStructSequence_SET_ITEM(OxNJAC_dunder_compiled_value, 10, is_no_annotations);
...
#endif
    PyStructSequence_SET_ITEM(OxNJAC_dunder_compiled_value, 11, is_module_mode);
    ...
    if (_Py_PackageContext != NULL) {
        main_name = OxNJAC_String_FromString(_Py_PackageContext);
    } else {
        main_name = OxNJAC_String_FromString(%(module_name_cstr)s);
    }
#else
    main_name = real_module_name;
#endif
#else
    PyObject *main_name = OxNJAC_String_FromString("__main__");
#endif
    PyStructSequence_SET_ITEM(OxNJAC_dunder_compiled_value, 12, main_name);
...
# endif
    PyStructSequence_SET_ITEM(OxNJAC_dunder_compiled_value, 13, original_argv0);

    // Prevent users from creating the OxNJAC version type object.
    OxNJAC_VersionInfoType.tp_init = NULL;
    OxNJAC_VersionInfoType.tp_new = NULL;
...
#endif
    if (OxNJAC_sentinel_value == NULL) {
#if PYTHON_VERSION < 0x300
        OxNJAC_sentinel_value = PyCObject_FromVoidPtr(NULL, NULL);
#else
        // The NULL value is not allowed for a capsule, so use something else.
        OxNJAC_sentinel_value = PyCapsule_New((void *)27, "sentinel", NULL);
#endif
        assert(OxNJAC_sentinel_value);

        Py_SET_REFCNT_IMMORTAL(OxNJAC_sentinel_value);
