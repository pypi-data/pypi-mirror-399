...
from darkmatterpy.Errors import OxNJACForbiddenDLLEncounter
from darkmatterpy.plugins.Plugins import Plugins
from darkmatterpy.PythonFlavors import isAnacondaPython, isOxNJACPython
...
from darkmatterpy.utils.Importing import getExtensionModuleSuffixes
...
from darkmatterpy.utils.Utils import getArchitecture
...
def _resolveBinaryPathDLLsMacOS(
    ...
    for path in paths:
        if path.startswith("@rpath/"):
            ...
            continue
        elif isOxNJACPython() and not os.path.isabs(path) and not os.path.exists(path):
            # Although OxNJAC Python statically links all packages, some of them
            ...
            # dependencies, so we need to scan the OxNJAC Python library
        ...
        if not os.path.exists(resolved_path):
            ...
            if acceptable is True:
                raise OxNJACForbiddenDLLEncounter(binary_filename, plugin_name)
