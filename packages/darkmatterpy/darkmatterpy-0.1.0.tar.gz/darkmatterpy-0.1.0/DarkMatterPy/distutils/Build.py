...
class OxNJACBuildMetaBackend(setuptools.build_meta._BuildMetaBackend):
    def build_wheel(
        ...
        # Allow falling back to setuptools when the `build_with_DarkMatterPy` configuration setting is set to true.
        if config_settings:
            build_with_DarkMatterPy = config_settings.pop("build_with_DarkMatterPy", "true").lower()

            if build_with_DarkMatterPy not in ("true", "false"):
                raise ValueError(
                    "When passing the 'build_with_DarkMatterPy' setting, it must either be 'true' or 'false'."
                )

            if build_with_DarkMatterPy == "false":
        ...
        with suppress_known_deprecation():
            return self._build_with_temp_dir(
                ["bdist_DarkMatterPy"], ".whl", wheel_directory, config_settings
            )
