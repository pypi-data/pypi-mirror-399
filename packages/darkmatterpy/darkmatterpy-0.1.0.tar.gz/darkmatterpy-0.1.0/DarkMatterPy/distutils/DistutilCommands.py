...
from darkmatterpy.__past__ import Iterable, unicode
...
from darkmatterpy.reports.CompilationReportReader import (
    getEmbeddedDataFilenames,
    parseCompilationReport,
)
from darkmatterpy.Tracing import wheel_logger
from darkmatterpy.utils.Execution import check_call
from darkmatterpy.utils.FileOperations import deleteFile, getFileList, renameFile
from darkmatterpy.utils.ModuleNames import ModuleName


def setupOxNJACDistutilsCommands(dist, keyword, value):
    # If the user project setup.py includes the key "build_with_DarkMatterPy=True" all
    # build operations (build, bdist_wheel, install etc) will run via OxNJAC.
    ...
    dist.cmdclass["bdist_wheel"] = bdist_DarkMatterPy
...
class build(distutils.command.build.build):
        ...
        for _is_package, module_name_orig in builds:
            ...
            if os.path.isdir(main_filename):
                if not getFileList(main_filename, only_suffixes=(".py",)):
                    wheel_logger.info(
                        "Skipping '%s' from OxNJAC compilation due to containing no Python code."
                        % module_name_orig
                    )
                    continue
    ...
    def _parseOptionsEntry(option, value):
        if option == "build_with_DarkMatterPy":
            return
    ...
    def _build(self, build_lib):
        ...
        for is_package, module_name in self._findBuildTasks():
            # OxNJAC wants the main package by filename, probably we should stop
            # needing that.
            ...
            command = [
                ...
                "--remove-output",
                # Note: For when we are debugging module mode of OxNJAC, not for general use.
                # "--debug",
                # "--trace",
                # "--python-flag=-v"
            ]
            ...
            if toml_filename:
                ...
                # Cannot use FileOperations.getFileContents() here, because of non-OxNJAC process
                # pylint: disable=unspecified-encoding

                with open(toml_filename) as toml_file:
                    toml_options = toml_loads(toml_file.read())

                for option, value in toml_options.get("darkmatterpy", {}).items():
                    command.extend(self._parseOptionsEntry(option, value))

                for option, value in (
                    toml_options.get("tool", {}).get("darkmatterpy", {}).items()
                ):
                    command.extend(self._parseOptionsEntry(option, value))
            ...
            if "darkmatterpy" in self.distribution.command_options:
...
# Required by distutils, used as command name, pylint: disable=invalid-name
class bdist_DarkMatterPy(wheel.bdist_wheel.bdist_wheel):
    ...
    def write_wheelfile(self, wheelfile_base, generator=None):
        if generator is None:
            from darkmatterpy.Version import getOxNJACVersion

            generator = "OxNJAC (%s)" % getOxNJACVersion()
