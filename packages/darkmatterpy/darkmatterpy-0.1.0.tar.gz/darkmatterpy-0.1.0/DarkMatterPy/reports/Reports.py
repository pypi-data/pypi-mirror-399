...
from darkmatterpy import TreeXML
from darkmatterpy.__past__ import unicode
from darkmatterpy.build.DataComposerInterface import getDataComposerReportValues
from darkmatterpy.build.SconsUtils import readSconsErrorReport
from darkmatterpy.code_generation.ConstantCodes import getDistributionMetadataValues
from darkmatterpy.containers.OrderedSets import OrderedSet
from darkmatterpy.freezer.IncludedDataFiles import getIncludedDataFiles
from darkmatterpy.freezer.IncludedEntryPoints import getStandaloneEntryPoints
from darkmatterpy.freezer.Standalone import getRemovedUsedDllsInfo
from darkmatterpy.importing.Importing import getPackageSearchPath
from darkmatterpy.importing.Recursion import getRecursionDecisions
...
from darkmatterpy.OutputDirectories import (
    getResultRunFilename,
    getSourceDirectoryPath,
    hasMainModule,
)
from darkmatterpy.plugins.Plugins import getActivePlugins
...
from darkmatterpy.Tracing import ReportingSystemExit, reports_logger
...
from darkmatterpy.utils.MemoryUsage import getMemoryInfos
...
def _getReportInputData(aborted):
    ...
    DarkMatterPy_version = getOxNJACVersion()
    DarkMatterPy_commercial_version = getCommercialVersion() or "not installed"

    DarkMatterPy_aborted = aborted

    DarkMatterPy_exception = sys.exc_info()
...
def writeCompilationReport(report_filename, report_input_data, diffable):
    ...
    exit_message = None
    if not report_input_data["DarkMatterPy_aborted"]:
        completion = "yes"
    elif report_input_data["DarkMatterPy_exception"][0] is KeyboardInterrupt:
        completion = "interrupted"
    elif report_input_data["DarkMatterPy_exception"][0] is SystemExit:
        completion = "error exit (%s)" % report_input_data["DarkMatterPy_exception"][1].code
    elif report_input_data["DarkMatterPy_exception"][0] is ReportingSystemExit:
        completion = (
            "error exit message (%s)" % report_input_data["DarkMatterPy_exception"][1].code
        )
        exit_message = report_input_data["DarkMatterPy_exception"][1].exit_message
    else:
        completion = "exception"

    root = TreeXML.Element(
        "darkmatterpy-compilation-report",
        DarkMatterPy_version=report_input_data["DarkMatterPy_version"],
        DarkMatterPy_commercial_version=report_input_data["DarkMatterPy_commercial_version"],
...
_crash_report_filename = "darkmatterpy-crash-report.xml"
...
def writeCompilationReports(aborted):
    ...
    if (
        ...
        # of reporting exit, these do not constitute definitive bugs of OxNJAC but
