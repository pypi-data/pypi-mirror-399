...
from darkmatterpy.utils.ThreadedExecutor import RLock
...
class OxNJACProgressBar(object):
...
def setupProgressBar(stage, unit, total, min_total=0):
    ...
    if use_progress_bar:
        Tracing.progress = OxNJACProgressBar(
            iterable=None,
            stage=stage,
            total=total,
            min_total=min_total,
...
@contextmanager
def withOxNJACDownloadProgressBar(*args, **kwargs):
    if not use_progress_bar or _getTqdmModule() is None:
        yield
    else:

        class OxNJACDownloadProgressBar(tqdm):
        ...
        with OxNJACDownloadProgressBar(*args, **kwargs) as progress_bar:
