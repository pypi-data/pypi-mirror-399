...
from darkmatterpy import Options, Tracing
...
from darkmatterpy.utils.AppDirs import getCacheDirEnvironmentVariableName
from darkmatterpy.utils.Download import getDownloadCacheDir, getDownloadCacheName
...
from darkmatterpy.utils.SharedLibraries import detectBinaryMinMacOS
...
def _getPythonForSconsExePath():
    ...
    if python_for_scons is None:
        ...
        Tracing.scons_logger.sysexit(
            """\
Error, while OxNJAC works with older Python, Scons does not, and therefore
OxNJAC needs to find a %s executable, so please install
it.
...
def _setupSconsEnvironment2():
    ...
    os.environ["DEVILPY_PACKAGE_DIR"] = os.path.abspath(darkmatterpy.__path__[0])
...
def runScons(scons_options, env_values, scons_filename):
    with _setupSconsEnvironment():
        ...
        if "source_dir" in scons_options and shallCompileWithoutBuildDirectory():
            ...
            scons_options["DarkMatterPy_src"] = getExternalUsePath(
                scons_options["DarkMatterPy_src"]
            )
...
def getCommonSconsOptions():
    ...
    scons_options["DarkMatterPy_src"] = getSconsDataPath()
    ...
    if isOxNJACPython() and not isWin32OrPosixWindows():
