...
from darkmatterpy.__past__ import basestring, unicode
from darkmatterpy.containers.OrderedDicts import OrderedDict
from darkmatterpy.Progress import enableThreading
from darkmatterpy.PythonFlavors import isTermuxPython
from darkmatterpy.Tracing import scons_details_logger, scons_logger
from darkmatterpy.utils.Execution import executeProcess
...
from darkmatterpy.utils.Utils import isLinux, isMacOS, isPosixWindows, isWin32Windows
...
def createEnvironment(
    ...
    env.DarkMatterPy_python = getArgumentBool("DarkMatterPy_python", False)
...
def provideStaticSourceFile(env, sub_path, c11_mode):
    source_filename = os.path.join(env.DarkMatterPy_src, "static_src", sub_path)
...
b) Using "--mingw64" lets OxNJAC download MinGW64 for you. Note: MinGW64
...
def makeResultPathFileSystemEncodable(env, result_exe):
    ...
    if os.name == "nt" and not isFilesystemEncodable(result_exe):
        result_exe = os.path.join(
            ...
            "_DarkMatterPy_temp.pyd" if env.module_mode else "_DarkMatterPy_temp.exe",
        )
