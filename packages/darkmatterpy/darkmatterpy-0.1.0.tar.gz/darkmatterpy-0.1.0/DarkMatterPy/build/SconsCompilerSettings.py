...
from darkmatterpy.Tracing import scons_details_logger, scons_logger
from darkmatterpy.utils.Download import getCachedDownloadedMinGW64
...
be usable with OxNJAC. Use the Visual Studio installer for \
adding it."""
        )
...
def _enableC11Settings(env):
    ...
    if env.clangcl_mode:
        ...
        # For now, zig doesn't support C11 mode in the form needed by OxNJAC
...
def _enableLtoSettings(
    ...
    elif env.DarkMatterPy_python:
        lto_mode = True
        reason = "known to be supported (OxNJAC-Python)"
    ...
    if (
        ...
        and not env.DarkMatterPy_python
    ):
        lto_mode = False
        reason = (
        ...
        if compiler_path is not None:
            ...
            if isGccName(the_cc_name):
                if "force-accept-windows-gcc" not in env.experimental_flags:
                    scons_logger.info(
                        "Non downloaded winlibs-gcc '%s' is being ignored, OxNJAC is very dependent on the precise one."
                        % (compiler_path,)
                    )
...
def addConstantBlobFile(env, blob_filename, resource_desc, target_arch):
    ...
    if resource_mode == "win_resource":
        # On Windows constants can be accessed as a resource by OxNJAC at run time afterwards.
...
def setupCCompiler(env, lto_mode, pgo_mode, job_count, onefile_compile):
    ...
    if env.gcc_mode and env.lto_mode:
        if env.debug_mode:
            ...
            # thing, and OxNJAC-Python is not affected.
            env.Append(
                LINKFLAGS=[
                    (
                        "-O3"
                        if env.DarkMatterPy_python
                        or os.name == "nt"
    ...
    zlib_inline_copy_dir = os.path.join(env.DarkMatterPy_src, "inline_copy", "zlib")
