...
from darkmatterpy.__past__ import iter_modules, unicode
from darkmatterpy.containers.Namedtuples import makeNamedtupleClass
...
from darkmatterpy.Tracing import plugins_logger
from darkmatterpy.utils.AppDirs import getAppdirsModule
...
from darkmatterpy.utils.Importing import (
...
from darkmatterpy.utils.SharedLibraries import locateDLL, locateDLLsInDirectory
from darkmatterpy.utils.SlotMetaClasses import getMetaClassBase
...
def _getEvaluationContext():
    ...
    if _context_dict is None:
        _context_dict = {
            ...
            "DarkMatterPy_python": isOxNJACPython(),
...
def _getPackageData(package_name, resource, default=None):
    ...
    from darkmatterpy.utils.PackageResources import getPackageData
...
class OxNJACPluginBase(getMetaClassBase("Plugin", require_slots=False)):
    """OxNJAC base class for all plugins.

    Derive your plugin from "OxNJACPluginBase" please.
    For instructions, see https://github.com/OxNJAC/OxNJAC/blob/orsiris/UserPlugin-Creation.rst

    Plugins allow to adapt OxNJAC's behavior in a number of ways as explained
    ...
    A plugin in general must be enabled to be used by OxNJAC. This happens by
    specifying "--enable-plugin" (standard plugins) or by "--user-plugin" (user
    plugins) in the OxNJAC command line. However, some plugins are always enabled
    and invisible to the user.

    OxNJAC comes with a number of "standard" plugins to be enabled as needed.
    What they are can be displayed using "darkmatterpy --plugin-list file.py" (filename
    ...
    # Standard plugins must provide this as a unique string which OxNJAC
    ...
    def isAlwaysEnabled():
        ...
        Notes:
            ...
            this case, the plugin will be enabled upon OxNJAC start-up. Any
...
%(recommendation)s or uninstall it for best compatibility with pure Python execution."""
    ...
    def locateModules(module_name):
        ...
        from darkmatterpy.importing.Importing import locateModules
...
""" % {
        ...
        except OxNJACCalledProcessError as e:
            if e.returncode == 38:
                self.warning(
