...
proper OxNJAC, and some of which are waiting to be created and submitted for
inclusion by you.
...
from darkmatterpy import Options, OutputDirectories
from darkmatterpy.__past__ import basestring, iter_modules
from darkmatterpy.build.DataComposerInterface import deriveModuleConstantsBlobName
from darkmatterpy.containers.OrderedDicts import OrderedDict
from darkmatterpy.containers.OrderedSets import OrderedSet
from darkmatterpy.Errors import OxNJACSyntaxError
from darkmatterpy.freezer.IncludedDataFiles import IncludedDataFile
from darkmatterpy.freezer.IncludedEntryPoints import IncludedEntryPoint
from darkmatterpy.ModuleRegistry import addUsedModule
from darkmatterpy.PythonVersions import python_version
from darkmatterpy.Tracing import plugins_logger, printLine
from darkmatterpy.utils.CommandLineOptions import OurOptionGroup
...
from darkmatterpy.utils.Importing import importFileAsModule
from darkmatterpy.utils.ModuleNames import (
    ModuleName,
    checkModuleName,
...
from .PluginBase import OxNJACPluginBase, control_tags
...
def _addActivePlugin(plugin_class, args, force=False):
    ...
    assert isinstance(plugin_instance, OxNJACPluginBase), plugin_instance
    ...
    for item in iter_modules(scan_path):
        ...
        for detector in detectors:
            ...
            if detector.__name__.replace(
                "OxNJACPluginDetector", ""
            ) != plugin_class.__name__.replace("OxNJACPlugin", ""):
                plugins_logger.warning(
                    "Class names %r and %r do not match OxNJACPlugin* and OxNJACPluginDetector* naming convention."
...
def loadStandardPluginClasses():
    ...
    Returns:
        None
    """
    import darkmatterpy.plugins.standard

    _loadPluginClassesFromPackage(darkmatterpy.plugins.standard)

    try:
        import darkmatterpy.plugins.commercial
    except ImportError:
        pass
    else:
        _loadPluginClassesFromPackage(darkmatterpy.plugins.commercial)
...
class Plugins(object):
    ...
    def _reportImplicitImports(plugin, module, implicit_imports):
        from darkmatterpy.importing.Importing import getModuleNameAndKindFromFilename
        from darkmatterpy.importing.Recursion import decideRecursion, recurseTo
    ...
    def _createTriggerLoadedModule(cls, module, trigger_name, code, flags):
        ...
        from darkmatterpy.tree.Building import buildModule
    ...
    def onModuleUsageLookAhead(cls, module_name, module_filename, module_kind):
        ...
        if parent_package_name is not None:
            from darkmatterpy.importing.Importing import locateModule
        ...
        def getModuleSourceCode():
            ...
            from darkmatterpy.tree.SourceHandling import readSourceCodeFromFilename
    ...
    def onModuleInitialSet():
        ...
        from darkmatterpy.ModuleRegistry import addRootModule
    ...
    def onModuleCompleteSet():
        ...
        from darkmatterpy.ModuleRegistry import getDoneModules
        ...
        for plugin in getActivePlugins():
            ...
            if value is not None:
                ...
                for key, value in sorted(value.items()):
                    ...
                    if not key.startswith("DarkMatterPy_"):
    ...
    def writeExtraCodeFiles(onefile):
        # Circular dependency.
        from darkmatterpy.tree.SourceHandling import writeSourceCode
...
def listPlugins():
    ...
    printLine("The following plugins are available in OxNJAC".center(80))
    printLine("-" * 80)
...
def isObjectAUserPluginBaseClass(obj):
    ...
    try:
        return (
            obj is not OxNJACPluginBase
            and issubclass(obj, OxNJACPluginBase)
    ...
    for plugin_name, (plugin_class, plugin_detector) in sorted(
        ...
        if plugin_name in Options.getPluginsEnabled():
            ...
            if plugin_class.isRelevant():
                ...
                plugin_class.warning(
                    "Not relevant with this OS, or OxNJAC arguments given, not activated."
                )
...
def getPluginOptions(plugin_name):
    ...
    for option in plugin_options.get(plugin_name, {}):
        ...
        if "[REQUIRED]" in option.help:
            if not arg_value:
                plugins_logger.sysexit(
                    "Error, required plugin argument '%s' of OxNJAC plugin '%s' not given."
