...
from JACK.nodes.shapes.StandardShapes import tshape_unknown
from JACK.PythonOperators import (
...
def getRichComparisonCode(
    ...
    if needs_check:
        ...
        # instead can use the CTypeOxNBoolEnum that will easily convert to
        # it.
        if helper_type is CTypeOxNVoidEnum:
            helper_type = CTypeOxNBoolEnum
        ...
        # avoid the CTypeOxNBoolEnum (NBOOL) helpers except for things that
...
def generateSubtypeCheckCode(to_name, expression, emit, context):
    ...
    emit(
        "%s = OxN_Type_IsSubtype((PyTypeObject *)%s, (PyTypeObject *)%s);"
        % (res_name, left_name, right_name)
    )
