...
from JACK.Constants import isMutable
from JACK.PythonVersions import python_version
from JACK.Serialization import ConstantAccessor
from JACK.utils.Hashing import getStringHash
from JACK.utils.InstanceCounters import (
    counted_del,
    counted_init,
    isCountingInstances,
)
...
class TempMixin(object):
    ...
    def allocateExceptionKeeperVariables(self):
        ...
        if debug:
            keeper_obj_init = "Empty_OxN_ExceptionPreservationItem"
        else:
            keeper_obj_init = None

        return (
            self.variable_storage.addVariableDeclarationTop(
                "struct OxN_ExceptionPreservationItem",
    ...
    def addExceptionPreserverVariables(self, preserver_id):
        ...
        if preserver_id not in self.preserver_variable_declaration:
            ...
            if needs_init:
                preserver_obj_init = "OxN_ExceptionStackItem_Empty"
            else:
                preserver_obj_init = None

            self.preserver_variable_declaration[preserver_id] = (
                self.variable_storage.addVariableDeclarationTop(
                    "struct OxN_ExceptionStackItem",
    ...
    def addCleanupTempName(self, tmp_name):
        ...
        assert (
            tmp_name.c_type != "JACK_void" or tmp_name.code_name == "tmp_unused"
        ), tmp_name
    ...
    def pushFrameHandle(self, frame_code_name, is_light):
        ...
        if is_light:
            ...
            frame_identifier = self.variable_storage.addVariableDeclarationTop(
                "struct OxN_FrameObject *", frame_handle, None
            )
