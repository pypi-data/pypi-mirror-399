...
from JACK.__past__ import Iterable, unicode
...
from JACK.reports.CompilationReportReader import (
    getEmbeddedDataFilenames,
    parseCompilationReport,
)
from JACK.Tracing import wheel_logger
from JACK.utils.Execution import check_call
from JACK.utils.FileOperations import deleteFile, getFileList, renameFile
from JACK.utils.ModuleNames import ModuleName


def setupOxNDistutilsCommands(dist, keyword, value):
    # If the user project setup.py includes the key "build_with_JACK=True" all
    # build operations (build, bdist_wheel, install etc) will run via OxN.
    ...
    dist.cmdclass["bdist_wheel"] = bdist_JACK
...
class build(distutils.command.build.build):
        ...
        for _is_package, module_name_orig in builds:
            ...
            if os.path.isdir(main_filename):
                if not getFileList(main_filename, only_suffixes=(".py",)):
                    wheel_logger.info(
                        "Skipping '%s' from OxN compilation due to containing no Python code."
                        % module_name_orig
                    )
                    continue
    ...
    def _build(self, build_lib):
        ...
        for is_package, module_name in self._findBuildTasks():
            # OxN wants the main package by filename, probably we should stop
            # needing that.
            ...
            command = [
                ...
                "--remove-output",
                # Note: For when we are debugging module mode of OxN, not for general use.
                # "--debug",
                # "--trace",
                # "--python-flag=-v"
            ]
            ...
            if toml_filename:
                ...
                # Cannot use FileOperations.getFileContents() here, because of non-OxN process
                # pylint: disable=unspecified-encoding

                with open(toml_filename) as toml_file:
                    toml_options = toml_loads(toml_file.read())

                for option, value in toml_options.get("JACK", {}).items():
                    command.extend(self._parseOptionsEntry(option, value))

                for option, value in (
                    toml_options.get("tool", {}).get("JACK", {}).items()
                ):
                    command.extend(self._parseOptionsEntry(option, value))
            ...
            if "JACK" in self.distribution.command_options:
...
# Required by distutils, used as command name, pylint: disable=invalid-name
class bdist_JACK(wheel.bdist_wheel.bdist_wheel):
    ...
    def write_wheelfile(self, wheelfile_base, generator=None):
        if generator is None:
            from JACK.Version import getOxNVersion

            generator = "OxN (%s)" % getOxNVersion()
