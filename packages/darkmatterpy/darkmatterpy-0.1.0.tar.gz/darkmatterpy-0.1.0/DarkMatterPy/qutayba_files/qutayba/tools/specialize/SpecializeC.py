...
JACK.Options.is_full_compat = False

# isort:start

import os

import JACK.specs.BuiltinBytesOperationSpecs
import JACK.specs.BuiltinDictOperationSpecs
import JACK.specs.BuiltinListOperationSpecs
import JACK.specs.BuiltinStrOperationSpecs
import JACK.specs.BuiltinUnicodeOperationSpecs
from JACK.code_generation.BinaryOperationHelperDefinitions import (
    getSpecializedBinaryOperations,
    isCommutativeOperation,
    parseTypesFromHelper,
)
from JACK.code_generation.c_types.CTypePyObjectPointers import (
    make_list_constant_direct_threshold,
    make_list_constant_hinted_threshold,
)
from JACK.code_generation.CallCodes import (
    getQuickCallCode,
...
from JACK.code_generation.ComparisonHelperDefinitions import (
    getSpecializedComparisonOperations,
)
from JACK.code_generation.ImportCodes import getImportModuleHardCodeName
from JACK.containers.OrderedSets import OrderedSet
from JACK.HardImportRegistry import (
    hard_modules,
...
from JACK.utils.Jinja2 import getTemplateC
...
def getDoExtensionUsingTemplateC(template_name):
    return getTemplateC(
        package_name="JACK.code_generation",
...
def makeHelpersComparisonOperation(operand, op_code):
    ...
    filename_c = "JACK/build/static_src/HelpersComparison%s.c" % op_code.capitalize()
    filename_h = "JACK/build/include/JACK/helper/comparisons_%s.h" % op_code.lower()
...
def makeHelpersComparisonDualOperation(operand, op_code):
    ...
    filename_c = (
        "JACK/build/static_src/HelpersComparisonDual%s.c" % op_code.capitalize()
    )
    filename_h = (
        "JACK/build/include/JACK/helper/comparisons_dual_%s.h" % op_code.lower()
    )
...
def makeHelpersBinaryOperation(operator, op_code):
    ...
    filename_c = (
        "JACK/build/static_src/HelpersOperationBinary%s.c" % op_code.capitalize()
    )
    filename_h = (
        "JACK/build/include/JACK/helper/operations_binary_%s.h" % op_code.lower()
    )
...
def makeHelpersInplaceOperation(operator, op_code):
    ...
    filename_c = (
        "JACK/build/static_src/HelpersOperationInplace%s.c" % op_code.capitalize()
    )
    filename_h = (
        "JACK/build/include/JACK/helper/operations_inplace_%s.h" % op_code.lower()
    )
...
def makeHelpersBinaryDualOperation(operand, op_code):
    ...
    filename_c = (
        "JACK/build/static_src/HelpersOperationBinaryDual%s.c" % op_code.capitalize()
    )
    filename_h = (
        "JACK/build/include/JACK/helper/operations_binary_dual_%s.h"
        % op_code.lower()
    )
...
def makeHelpersImportHard():
    filename_c = "JACK/build/static_src/HelpersImportHard.c"
    filename_h = "JACK/build/include/JACK/helper/import_hard.h"
...
def makeHelperCalls():
    filename_c = "JACK/build/static_src/HelpersCallingGenerated.c"
    filename_h = "JACK/build/include/JACK/helper/calling_generated.h"
    ...
    with withFileOpenedAndAutoFormatted(filename_c) as output_c:
        with withFileOpenedAndAutoFormatted(filename_h) as output_h:
            ...
            template = getTemplateC(
                "JACK.code_generation", "CodeTemplateCallsPositional.c.j2"
            )
            ...
            template = getTemplateC(
                "JACK.code_generation", "CodeTemplateCallsMixed.c.j2"
            )
...
def makeHelperLists():
    filename_c = "JACK/build/static_src/HelpersListsGenerated.c"
    filename_h = "JACK/build/include/JACK/helper/lists_generated.h"
    ...
    with withFileOpenedAndAutoFormatted(filename_c) as output_c:
        with withFileOpenedAndAutoFormatted(filename_h) as output_h:
            ...
            template = getTemplateC(
                "JACK.code_generation", "CodeTemplateMakeListSmall.c.j2"
            )
            ...
            template = getTemplateC(
                "JACK.code_generation", "CodeTemplateMakeListHinted.c.j2"
            )
...
generate_builtin_type_operations = [
    ...
    # OxN-Python eventually.
    (
        "tshape_dict",
        dict_desc,
        JACK.specs.BuiltinDictOperationSpecs,
        ("pop", "popitem", "setdefault"),
    ),
    (
        "tshape_list",
        list_desc,
        JACK.specs.BuiltinListOperationSpecs,
        (
            "pop",
    ...
    # that we do not have easy access to, but we might one day for OxN-Python
    # expose it for the static linking of it and then we could in fact call
    # these directly.
    (
        "tshape_str",
        str_desc,
        JACK.specs.BuiltinStrOperationSpecs,
        (
            "strip",
            "rstrip",
            "lstrip",
    ...
    (
        "tshape_unicode",
        unicode_desc,
        JACK.specs.BuiltinUnicodeOperationSpecs,
        (
            "strip",
            "rstrip",
            "lstrip",
    ...
    (
        "tshape_bytes",
        bytes_desc,
        JACK.specs.BuiltinBytesOperationSpecs,
        (
            "capitalize",
            "center",
            "count",
...
def makeDictCopyHelperCodes():
    filename_c = "JACK/build/static_src/HelpersDictionariesGenerated.c"
...
def makeHelperBuiltinTypeMethods():
    ...
    filename_c = "JACK/build/static_src/HelpersBuiltinTypeMethods.c"
    filename_h = "JACK/build/include/JACK/helper/operations_builtin_types.h"
    with withFileOpenedAndAutoFormatted(filename_c) as output_c:
        with withFileOpenedAndAutoFormatted(filename_h) as output_h:
