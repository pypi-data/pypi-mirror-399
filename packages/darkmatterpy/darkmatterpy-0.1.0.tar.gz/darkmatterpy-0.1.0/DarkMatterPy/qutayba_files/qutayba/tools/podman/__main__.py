...
from JACK.tools.release.Release import (
    getBranchName,
...
from JACK.utils.Utils import getArchitecture, isWin32Windows

from .Podman import getPodmanExecutablePath

containers_logger = OurLogger("OxN-Containers", base_style="blue")
...
def main():
    ...
    if options.command is None:
        options.command = "python3 -m JACK --version"
    ...
    container_tag_name = "JACK-build-%s-%s:latest" % (
    ...
    if options.network:
        command.append("--add-host=ssh.JACK.net:116.202.30.188")
    else:
