...
from JACK import Options, Tracing
...
from JACK.utils.AppDirs import getCacheDirEnvironmentVariableName
from JACK.utils.Download import getDownloadCacheDir, getDownloadCacheName
...
from JACK.utils.SharedLibraries import detectBinaryMinMacOS
...
def _getPythonForSconsExePath():
    ...
    if python_for_scons is None:
        ...
        Tracing.scons_logger.sysexit(
            """\
Error, while OxN works with older Python, Scons does not, and therefore
OxN needs to find a %s executable, so please install
it.
...
def _setupSconsEnvironment2():
    ...
    os.environ["DEVILPY_PACKAGE_DIR"] = os.path.abspath(JACK.__path__[0])
...
def runScons(scons_options, env_values, scons_filename):
    with _setupSconsEnvironment():
        ...
        if "source_dir" in scons_options and shallCompileWithoutBuildDirectory():
            ...
            scons_options["JACK_src"] = getExternalUsePath(
                scons_options["JACK_src"]
            )
...
def getCommonSconsOptions():
    ...
    scons_options["JACK_src"] = getSconsDataPath()
    ...
    if isOxNPython() and not isWin32OrPosixWindows():
