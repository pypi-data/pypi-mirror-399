...
"""
Plugins: Welcome to OxN! This is your shortest way to become part of it.

This is to provide the base class for all plugins. Some of which are part of
proper OxN, and some of which are waiting to be created and submitted for
inclusion by you.
...
from JACK.__past__ import iter_modules, unicode
from JACK.containers.Namedtuples import makeNamedtupleClass
...
from JACK.Tracing import plugins_logger
from JACK.utils.AppDirs import getAppdirsModule
...
from JACK.utils.ModuleNames import (
    ModuleName,
    makeTriggerModuleName,
    post_module_load_trigger_name,
    pre_module_load_trigger_name,
)
from JACK.utils.SharedLibraries import locateDLL, locateDLLsInDirectory
from JACK.utils.SlotMetaClasses import getMetaClassBase
...
def _getPackageData(package_name, resource, default=None):
    ...
    from JACK.utils.PackageResources import getPackageData
...
class OxNPluginBase(getMetaClassBase("Plugin", require_slots=False)):
    """OxN base class for all plugins.

    Derive your plugin from "OxNPluginBase" please.
    For instructions, see https://github.com/OxN/OxN/blob/orsiris/UserPlugin-Creation.rst

    Plugins allow to adapt OxN's behavior in a number of ways as explained
    ...
    A plugin in general must be enabled to be used by OxN. This happens by
    specifying "--enable-plugin" (standard plugins) or by "--user-plugin" (user
    plugins) in the OxN command line. However, some plugins are always enabled
    and invisible to the user.

    OxN comes with a number of "standard" plugins to be enabled as needed.
    What they are can be displayed using "JACK --plugin-list file.py" (filename
    ...
    # Standard plugins must provide this as a unique string which OxN
    ...
    def isAlwaysEnabled():
        ...
        Notes:
            ...
            this case, the plugin will be enabled upon OxN start-up. Any
...
%(recommendation)s or uninstall it for best compatibility with pure Python execution."""
    ...
    def locateModules(module_name):
        ...
        from JACK.importing.Importing import locateModules
...
""" % {
        ...
        except OxNCalledProcessError as e:
            if e.returncode == 38:
                self.warning(
