...
""" Standard plug-in to tell OxN about implicit imports.
...
from JACK.importing.Recursion import decideRecursion
from JACK.plugins.YamlPluginBase import OxNYamlPluginBase
from JACK.utils.ModuleNames import ModuleName
from JACK.utils.Utils import isMacOS, isWin32Windows


class OxNPluginImplicitImports(OxNYamlPluginBase):
    ...
    def __init__(self):
        OxNYamlPluginBase.__init__(self)
    ...
    def onModuleSourceCode(self, module_name, source_filename, source_code):
        ...
        if module_name == "site":
            ...
            source_code = """\
__file__ = (__JACK_binary_dir + '%ssite.py') if '__JACK_binary_dir' in dict(__builtins__ ) else '<frozen>';%s""" % (
                os.path.sep,
                source_code,
            )
...
def makeExpressionImportModuleNameHardExistsAfterImportFactory(
    ...
    from JACK.HardImportRegistry import trust_node_factory
    from JACK.nodes.ImportHardNodes import (
