...
from JACK.PythonFlavors import isAnacondaPython
from JACK.PythonVersions import python_version
from JACK.utils.Distributions import getDistributionFromModuleName
from JACK.utils.FileOperations import getFileList, listDir
from JACK.utils.ModuleNames import ModuleName
from JACK.utils.Utils import getArchitecture, isMacOS, isWin32Windows


class OxNPluginQtBindingsPluginBase(OxNPluginBase):
...
"""
        ...
        info = self.queryRuntimeInformationMultiple(
            ...
            values=(
                ...
                (
                    "JACK_patch_level",
                    applyBindingName(
                        "getattr(%(binding_name)s, '_JACK_patch_level', 0)"
                    ),
                ),
    ...
    def _getOxNPatchLevel(self):
        """Does it include the OxN patch, i.e. is a self-built one with it applied."""
        return self._getQtInformation().JACK_patch_level
...
import os
path = os.getenv("PATH", "")
if not path.startswith(__JACK_binary_dir):
    os.environ["PATH"] = __JACK_binary_dir + ";" + path
"""
            yield (
                code,
...
class OxNPluginPyQt5QtPluginsPlugin(OxNPluginQtBindingsPluginBase):
    ...
    def __init__(self, include_qt_plugins, noinclude_qt_plugins, no_qt_translations):
        OxNPluginQtBindingsPluginBase.__init__(
            self,
        ...
        self.warning(
            """\
For the obsolete PyQt5 the OxN support is incomplete. Threading, callbacks \
to compiled functions, etc. may not be working.""",
            mnemonic="pyqt5",
        )

    def _getQtInformation(self):
        result = OxNPluginQtBindingsPluginBase._getQtInformation(self)
...
class OxNPluginDetectorPyQt5QtPluginsPlugin(OxNPluginBase):
    detector_for = OxNPluginPyQt5QtPluginsPlugin
    ...
    def onModuleDiscovered(self, module):
        ...
        if full_name == OxNPluginPyQt5QtPluginsPlugin.binding_name + ".QtCore":
            self.warnUnusedPlugin("Inclusion of Qt plugins.")
        elif full_name == "PyQt4.QtCore":
            self.warning(
                "Support for PyQt4 has been dropped. Please contact OxN commercial if you need it."
            )


class OxNPluginPySide2Plugins(OxNPluginQtBindingsPluginBase):
...
full support consider provided information.""",
            ...
            if python_version < 0x360:
                self.sysexit(
                    """\
The standard PySide2 is not supported before CPython <3.6. For full support: https://JACK.net/pages/pyside2.html"""
                )
    ...
    def onModuleEncounter(
        ...
        if module_name == self.binding_name and self._getOxNPatchLevel() < 1:
            return True, "Need to monkey patch PySide2 for abstract methods."

        return OxNPluginQtBindingsPluginBase.onModuleEncounter(
            self,
    ...
    def createPostModuleLoadCode(self, module):
        ...
        for result in OxNPluginQtBindingsPluginBase.createPostModuleLoadCode(
            self, module
        ):
            yield result

        if (
            self._getOxNPatchLevel() < 1
...
def %(wrapper_name)s%(signature)s:
    return %(wrapper_name)s.func(%(parameters)s)
            ...
            # TODO: OxN does not currently statically optimize this, might change!
            exec(
                v,
                globals(),
            )
...
class OxNPluginDetectorPySide2Plugins(OxNPluginBase):
    detector_for = OxNPluginPySide2Plugins

    def onModuleDiscovered(self, module):
        if (
            module.getFullName() == OxNPluginPySide2Plugins.binding_name + ".QtCore"
            and getActiveQtPlugin() is None
        ):
            self.warnUnusedPlugin("Making callbacks work and include Qt plugins.")


class OxNPluginPySide6Plugins(OxNPluginQtBindingsPluginBase):
...
class OxNPluginDetectorPySide6Plugins(OxNPluginBase):
    detector_for = OxNPluginPySide6Plugins

    def onModuleDiscovered(self, module):
        if module.getFullName() == OxNPluginPySide6Plugins.binding_name + ".QtCore":
            self.warnUnusedPlugin("Standalone mode support and Qt plugins.")


class OxNPluginPyQt6Plugins(OxNPluginQtBindingsPluginBase):
...
class OxNPluginDetectorPyQt6Plugins(OxNPluginBase):
    detector_for = OxNPluginPyQt6Plugins

    def onModuleDiscovered(self, module):
        if module.getFullName() == OxNPluginPyQt6Plugins.binding_name + ".QtCore":
            self.warnUnusedPlugin("Standalone mode support and Qt plugins.")


class OxNPluginNoQt(OxNPluginBase):
