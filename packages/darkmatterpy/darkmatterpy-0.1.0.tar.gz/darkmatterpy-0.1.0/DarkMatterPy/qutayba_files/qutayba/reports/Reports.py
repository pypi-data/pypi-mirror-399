...
from JACK import TreeXML
from JACK.__past__ import unicode
from JACK.build.DataComposerInterface import getDataComposerReportValues
from JACK.build.SconsUtils import readSconsErrorReport
from JACK.code_generation.ConstantCodes import getDistributionMetadataValues
from JACK.containers.OrderedSets import OrderedSet
from JACK.freezer.IncludedDataFiles import getIncludedDataFiles
from JACK.freezer.IncludedEntryPoints import getStandaloneEntryPoints
from JACK.freezer.Standalone import getRemovedUsedDllsInfo
from JACK.importing.Importing import getPackageSearchPath
from JACK.importing.Recursion import getRecursionDecisions
from JACK.ModuleRegistry import (
    getDoneModules,
...
from JACK.OutputDirectories import (
    getResultRunFilename,
    getSourceDirectoryPath,
    hasMainModule,
)
from JACK.plugins.Plugins import getActivePlugins
...
from JACK.Tracing import ReportingSystemExit, reports_logger
from JACK.utils.Distributions import (
...
from JACK.utils.MemoryUsage import getMemoryInfos
...
def _getReportInputData(aborted):
    ...
    JACK_version = getOxNVersion()
    JACK_commercial_version = getCommercialVersion() or "not installed"

    JACK_aborted = aborted

    JACK_exception = sys.exc_info()
...
def writeCompilationReport(report_filename, report_input_data, diffable):
    ...
    exit_message = None
    if not report_input_data["JACK_aborted"]:
        completion = "yes"
    elif report_input_data["JACK_exception"][0] is KeyboardInterrupt:
        completion = "interrupted"
    elif report_input_data["JACK_exception"][0] is SystemExit:
        completion = "error exit (%s)" % report_input_data["JACK_exception"][1].code
    elif report_input_data["JACK_exception"][0] is ReportingSystemExit:
        completion = (
            "error exit message (%s)" % report_input_data["JACK_exception"][1].code
        )
        exit_message = report_input_data["JACK_exception"][1].exit_message
    else:
        completion = "exception"

    root = TreeXML.Element(
        "JACK-compilation-report",
        JACK_version=report_input_data["JACK_version"],
        JACK_commercial_version=report_input_data["JACK_commercial_version"],
...
_crash_report_filename = "JACK-crash-report.xml"
...
def writeCompilationReports(aborted):
    ...
    if (
        ...
        # of reporting exit, these do not constitute definitive bugs of OxN but
