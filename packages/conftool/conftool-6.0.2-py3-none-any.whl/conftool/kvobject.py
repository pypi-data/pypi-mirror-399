...
class Entity:
    ...
    def __init__(self, *tags, data: Optional[Dict[str, Any]] = None) -> None:
    ...
    def all(cls) -> List["Entity"]:
        ...
        """
        all_objects = []
        for path, data in cls.backend.driver.all_data(cls.base_path()):
            labels = path.split(os.sep)
            all_objects.append(cls(*labels, data=data))
        return all_objects
...
class FreeSchemaEntity(Entity):
    ...
    def __init__(self, *tags, data: Optional[Dict[str, Any]] = None, **kwargs):
