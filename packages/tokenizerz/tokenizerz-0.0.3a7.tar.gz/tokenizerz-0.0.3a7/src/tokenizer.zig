...
pub const Tokenizer = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, repo_name: []const u8, model_name: []const u8, verbose: bool) !Self {
        ...
        // We can keep your manual replacements if you need them for specific models (like Llama/Gemma)
        const Replacement = struct { bytes: []const u8, replacement: u8 };
        const explicit_replacements = [_]Replacement{
            .{ .bytes = "\xE2\x96\x81", .replacement = ' ' }, // SPIECE_UNDERLINE
        ...
        while (vocab_iter.next()) |entry| {
            ...
            while (i < token_str.len) {
                // 1. Check for explicit replacements (the "fix" you added in new_tokenizer)
                var replaced = false;
                ...
                // 2. If no explicit replacement, use the decoder map (the "necessary part" for UTF-8)
    ...
    fn createDecoderMap(allocator: std.mem.Allocator) std.AutoHashMap(u21, u8) {
        ...
        while (b < 256) : (b += 1) {
            ...
            var cp: u21 = 0;
            // This range logic matches the standard GPT-2 byte-to-unicode mapping
