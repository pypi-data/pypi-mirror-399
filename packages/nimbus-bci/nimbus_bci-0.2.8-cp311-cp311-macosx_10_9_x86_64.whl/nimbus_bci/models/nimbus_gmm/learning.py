...
def nimbus_gmm_update(model: NimbusModel, X: np.ndarray, y: np.ndarray) -> NimbusModel:
    """Update an existing GMM/QDA model with additional labeled observations.

    Args:
        model: A model returned by `nimbus_gmm_fit` or `nimbus_gmm_update`.
        X: Feature matrix of shape (n_trials, n_features).
