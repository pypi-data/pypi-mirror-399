...
def process(args):
    assert args.for_inference or args.target_unit is not None, (
    ...
    with (
        open(os.path.join(args.output_dir, f"{args.data_name}.tsv"), "w") as o_t,
        open(os.path.join(args.output_dir, f"{args.data_name}.unit"), "w") as o_u,
    ):
