...
def preprocess(
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        with open(
            f"{tmp}/dict.txt", mode="w", encoding="utf-8", errors="surrogateescape"
        ) as f:
            for word in vocab:
                print(word, 1, file=f)
...
def read_nli(path, langs=None):
    ...
    print(
        f"Loaded {lens[0]} samples in {nlangs} languages from {path}", file=sys.stderr
    )
    return data
...
def main():
    ...
    parser.add_argument(
        "--valid", required=True, help="Validation data in jsonl format"
    )
    ...
    for split, path in ("train", args.train), ("valid", args.valid):
        ...
        assert all(
            sample["gold_label"] in ("contradiction", "entailment", "neutral")
            for sample in data
        )
        filtered_size = len(data)
        if filtered_size != original_size:
            print(
                f"Filtered {filtered_size}/{original_size} samples from {path}",
                file=sys.stderr,
            )
        for name, field in (
            ("input0", "sentence1"),
            ...
            write_lines(
                [sample[field] for sample in data],
                f"{args.destdir}/raw/{split}.{name}.txt",
            )
    ...
    subprocess.run(
        [
            ...
            "fairseq_cli.preprocess",
            "--trainpref",
            f"{args.destdir}/raw/train.label.txt",
            "--validpref",
            f"{args.destdir}/raw/valid.label.txt",
            "--only-source",
            "--thresholdsrc",
            "0",
            "--destdir",
            f"{args.destdir}/bin/label",
            "--workers",
            "20",
        ]
    )
