...
def _scaled_dot_product_attention(
    ...
    if subset_heads is None:
        ...
        output = torch.stack(
            [
                mixed_output[torch.arange(bsz), subset_heads[:, col], :, :]
...
def multi_head_attention_forward(
    ...
    if static_k is None:
        k = (
            k.contiguous()
            .view(k.shape[0], bsz * total_num_heads, head_dim)
            .transpose(0, 1)
        )
    else:
    ...
    if static_v is None:
        v = (
            v.contiguous()
            .view(v.shape[0], bsz * total_num_heads, head_dim)
            .transpose(0, 1)
        )
    else:
        # TODO finish disentangling control flow so we don't do in-projections when statics are passed
    ...
    if key_padding_mask is not None:
        ...
        key_padding_mask = (
            ...
            .expand(-1, total_num_heads, -1, -1)
            .reshape(bsz * total_num_heads, 1, src_len)
        )
    ...
    attn_output, attn_output_weights = _scaled_dot_product_attention(
        q, k, v, attn_mask, dropout_p, bsz, subset_heads, subset_weights
    )
