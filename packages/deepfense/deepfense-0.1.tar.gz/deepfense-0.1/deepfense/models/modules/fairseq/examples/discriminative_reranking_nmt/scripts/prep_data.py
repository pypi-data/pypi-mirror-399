...
def main(args):
    assert args.split.startswith("train") or args.num_shards == 1, (
    ...
    assert num_sents == len(target_sents), (
    ...
    assert len(hypo_sents) % args.beam == 0, (
    ...
    assert num_sents == len(hypo_sents), (
    ...
    for ns in range(args.num_shards):
        ...
        with (
            open(source_output_dir / f"{args.split}.bpe", "w") as s_o,
            open(hypo_output_dir / f"{args.split}.bpe", "w") as h_o,
            open(metric_output_dir / f"{args.split}.{args.metric}", "w") as m_o,
        ):
