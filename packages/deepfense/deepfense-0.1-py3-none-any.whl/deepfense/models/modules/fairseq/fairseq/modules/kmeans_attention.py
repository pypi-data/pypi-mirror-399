...
class SelfAttention(nn.Module):
    def __init__(
        ...
        assert (max_seq_len % window_size) == 0, (
            "maximum sequence length must be divisible by the target window size"
        )
        assert local_attn_heads <= heads, (
        ...
        assert not (receives_context and causal), (
