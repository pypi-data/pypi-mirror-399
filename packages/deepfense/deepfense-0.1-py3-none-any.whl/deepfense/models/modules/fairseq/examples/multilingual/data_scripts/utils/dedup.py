...
def deup(src_file, tgt_file, src_file_out, tgt_file_out):
    ...
    dup_count = 0
    with (
        open(src_file, encoding="utf-8") as fsrc,
...
def main():
    ...
    parser.add_argument("--tgt-file", type=str, required=True, help="tgt file")
    parser.add_argument(
        "--src-file-out", type=str, required=True, help="src ouptut file"
    )
    parser.add_argument(
        "--tgt-file-out", type=str, required=True, help="tgt ouput file"
    )
