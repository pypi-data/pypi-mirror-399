...
parser.add_argument(
    "--everything",
    action="store_true",
    help="print everything instead of only last match",
)
parser.add_argument("--path_contains", help="only consider matching file pattern")
parser.add_argument(
    "--group_on", help="if set, groups by this metric and shows table of differences"
)
...
parser.add_argument(
    "--unique_epochs",
    action="store_true",
    help="only consider the last line fore each epoch",
)
...
parser.add_argument(
    "--compact",
    action="store_true",
    help="if true, just prints checkpoint <tab> best val",
)
parser.add_argument(
    "--hydra", action="store_true", help="if true, uses hydra param conventions"
)

parser.add_argument(
    "--best_biggest",
    action="store_true",
    help="if true, best is the biggest number, not smallest",
)
parser.add_argument("--key_len", type=int, default=10, help="max length of key")

parser.add_argument(
    "--best_only", action="store_true", help="if set, only prints the best value"
)
        ...
        with open(os.path.join(root, file), "r") as fin:
            ...
            for line in fin:
                ...
                if line.find(args.target) != -1 and (
                    args.skip_containing is None
                    or line.find(args.skip_containing) == -1
                ):
                    try:
                        idx = line.index("{")
                        line = line[idx:]
            ...
            if args.best:
                ...
                for i in range(len(found)):
                    cand_best = (
                        found[i][args.target] if args.target in found[i] else None
                    )
                    ...
                    if (
                        cand_best is not None
                        and not math.isnan(float(cand_best))
                        and (curr_best is None or cmp(cand_best, curr_best))
                    ):
                        curr_best = cand_best
            ...
            if len(found) == 0:
                if print_output and (
                    args.last_files is not None or not args.skip_empty
                ):
                    # print(root.split('/')[-1])
                ...
                if not args.everything:
                    if (
                        best is not None
                        and args.group_on is None
                        and not args.best_only
                        and not args.flat
                    ):
                        print(best, "(best)")
                    if (
                        args.group_on is None
                        and args.last
                        and not args.best_only
                        and not args.flat
                    ):
                        for f in found[-args.last :]:
                            if args.extract_prev is not None:
                                try:
                                    print(
                                        "{}\t{}".format(
                                            f[args.extract_prev], f[args.target]
                                        )
                                    )
                    ...
                    try:
                        metric = (
                            found[-1][args.target]
                            if not args.best or best is None
                            else best[args.target]
                        )
                    except:
                        print(found[-1])
                        raise
