...
class ConversationManager:
    ...
    def load_session(self, session_id: str) -> bool:
        ...
        """
        try:
            # Find session file - support both old (session_*) and new (YYMMDDHHMM-name) formats
            session_file = None

            # Try exact match first (new format: 2512111701-frost-blade.jsonl)
            exact_jsonl = self.conversations_dir / f"{session_id}.jsonl"
            if exact_jsonl.exists():
                session_file = exact_jsonl

            # Try old format with session_ prefix
            if not session_file:
                # Strip 'session_' prefix if present for normalization
                normalized_id = session_id
                if session_id.startswith("session_"):
                    normalized_id = session_id[8:]  # len("session_") = 8

                # Try session_* patterns (old format)
                for file_path in self.conversations_dir.glob(f"session_{normalized_id}_*.json"):
                    session_file = file_path
                    break
                if not session_file:
                    for file_path in self.conversations_dir.glob(f"session_{normalized_id}*.jsonl"):
                        session_file = file_path
                        break
                # Also try exact match with session_ prefix
                if not session_file and session_id.startswith("session_"):
                    exact_old = self.conversations_dir / f"{session_id}.jsonl"
                    if exact_old.exists():
                        session_file = exact_old
                    if not session_file:
                        for file_path in self.conversations_dir.glob(f"{session_id}_*.json"):
                            session_file = file_path
                            break
    ...
    def validate_session(self, session_id: str) -> Dict[str, Any]:
        ...
        try:
            # Find session file - support both old and new formats
            session_file = None
            ...
            if session_file.suffix != ".jsonl":
                required_fields = ["messages", "metadata", "session_id"]
            ...
            for message in messages:
                ...
                import re
                file_paths = re.findall(r'[\w\-/\.]+\.(py|js|rs|md|json|yaml|yml|toml)', content)
    ...
    def branch_session(self, source_session_id: str, branch_point_index: int) -> Dict[str, Any]:
        ...
        try:
            ...
            new_session_id = generate_branch_name(source_session_id)
