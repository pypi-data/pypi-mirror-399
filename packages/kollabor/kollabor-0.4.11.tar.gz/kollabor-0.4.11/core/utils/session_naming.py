...
def generate_branch_name(parent_name: str) -> str:
    """Generate a branch name based on parent session.

    Args:
        parent_name: Name of the parent session

    Returns:
        Branch name like '2512111430-quantum-spark-branch'
    """
    # Generate a fresh themed name with 'branch' suffix
    base_name = generate_session_name(include_timestamp=False)
    timestamp = datetime.now().strftime("%y%m%d%H%M")
    return f"{timestamp}-{base_name}-branch"
