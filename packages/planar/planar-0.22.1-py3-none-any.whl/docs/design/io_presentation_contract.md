...
| `io.select.multiple` | `{ "type": "array", "items": { "type": "string", "enum": [...] } }` | `label`, `options`, `search` | `help_text`, `default`, `maxSelections` |
| `io.entity.select` | single:`{ "type": "string" }` / multi:`{ "type": "array", "items": { "type": "string" } }` | `label`, `entity`, `multiple` | `displayField`, `help_text`, `default` |
| `io.upload.file` | PlanarFile object (`id`, `filename`, `content_type`, `size`) | `label` | `accept`, `maxSizeMb`, `help_text` |
...
- `entity.select` adds `x-planar-presentation`: `{ "inputType": "entity-select", "entity":
  <ModelName>, "displayField": <field-or-null> }`.
