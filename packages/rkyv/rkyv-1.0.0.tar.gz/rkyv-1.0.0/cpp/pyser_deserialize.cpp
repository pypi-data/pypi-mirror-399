...
// Note: we avoid including internal headers like <funcobject.h> for portability.
// Closure handling uses public C-API (PyObject_GetAttrString / PyObject_SetAttrString)
// to remain compatible across Python builds.
// For robust closure assignment we use the internal header when available.
#ifdef HAVE_CPYTHON_FUNCOBJECT
        ...
        for (const auto &ptr: graph.all_pointers) {
            ...
            if (PyList_Check(src_obj)) {
                ...
                if (field.find("closure:") == 0) {
...
#ifdef HAVE_CPYTHON_FUNCOBJECT
                    PyFunctionObject *func = reinterpret_cast<PyFunctionObject *>(src_obj);
                    PyObject *old_closure = func->func_closure; // borrowed or NULL
                    ...
                    if (PyTuple_SetItem(new_tuple, static_cast<Py_ssize_t>(idx), cell) < 0) { Py_DECREF(cell); Py_DECREF(new_tuple); continue; }

                    // Swap into function: DECREF old closure, assign new one (new_tuple already has new ref)
                    Py_XDECREF(func->func_closure);
                    func->func_closure = new_tuple;
#else
#ifdef PYSER_ENABLE_DEBUG_PRINTS
                    fprintf(stderr, "pyser: skipping optimized closure assignment because HAVE_CPYTHON_FUNCOBJECT not defined\n");
#endif
                    // Portable fallback: we cannot assign to function internals without
                    // CPython internal headers. The public API does not allow setting
                    // __closure__ (it's read-only). We skip closure assignment in this
                    // build to remain safe. Tests which rely on closure behavior may
                    // fail; enabling HAVE_CPYTHON_FUNCOBJECT on supported toolchains
                    // will restore optimized behavior.
#endif
                }
             } else {
