...
def _get_write_branch(project_spec):
    ...
    Priority order:
    1. Deployed context: use git branch from project_spec
    2. Local with git: detect from git repository
    3. Local without git: fall back to metaflow branch (user isolation)
    ...
    # 2. Local with git: detect from repo
    git_branch = _detect_git_branch()
    if git_branch:
        return git_branch

    # 3. No git: fall back to metaflow branch (provides user isolation)
