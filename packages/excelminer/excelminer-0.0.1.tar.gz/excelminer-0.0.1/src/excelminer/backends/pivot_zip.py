...
def _get_pivot_table_to_sheet(
    zf: zipfile.ZipFile, sheet_path_to_name: dict[str, str]
) -> dict[str, str]:
...
def _parse_cache_definition(
    zf: zipfile.ZipFile, cache_path: str
        ...
        try:
            with zipfile.ZipFile(p) as zf:
                ...
                for cache_id, rid in cache_id_to_rid.items():
                    ...
                    if not cache_path:
                        continue
                    source_type, source_ref, field_names, field_idx_map = (
                        _parse_cache_definition(zf, cache_path)
                    )
                        ...
                        if m:
                            ...
                            for n in graph.nodes.values():
                                ...
                                if str(n.attrs.get("connection_id") or "") == conn_id:
                                    graph.add_edge(
                                        cache_node.id, n.id, "uses_connection"
                                    )
                                    break
