...
class BulkOp:
    ...
    def __init__(self, session_or_tx: Any, models: list[Any], op: str, mode: str = "insert"):
    ...
    def Returning(self, *cols) -> "BulkOp":
        """Agrega cláusula RETURNING a la operación (solo mode='insert')."""
        if self.mode == "copy":
    ...
    def Limit(self, n: int) -> "BulkOp":
        """Limit the number of returned results (applied after execution)."""
        self._limit = n
        return self
    
    def Skip(self, n: int) -> "BulkOp":
        """Skip the first n results (applied after execution)."""
        self._skip = n
        return self
    
    def _apply_slice(self, results: list) -> list:
        """Apply Limit/Skip slicing to results."""
        if not results:
            return results
    ...
    def _execute_sync(self):
        ...
        if self.op == "insert":
            ...
            sql, values = build_bulk_insert_sql(self.models, style="qmark") # Sync usually uses qmark or pyformat depending on driver
            # Important: psqlmodel historically uses %s (pyformat) via build_insert_sql default?
            # Let's check: Transaction._execute_sql_in_tx_sync uses cursor.execute(sql, values).
            # If using psycopg2/psycopg3 sync, it expects %s. 
            # build_bulk_insert_sql default style="pyformat" usually.
            sql, values = build_bulk_insert_sql(self.models, style="pyformat") # Force standard %s
            ...
            if self.returning_cols:
                ...
                if len(self.returning_cols) == 1 and isinstance(self.returning_cols[0], type) and hasattr(self.returning_cols[0], "__tablename__"):
                     ...
                     # Expand to explicit columns to guarantee order for tuple unpacking
            ...
            if self.returning_cols and rows:
                if model_to_hydrate:
                     ...
                     if self._to_dicts:
                         ...
                         return self._apply_slice(dicts)
                     return self._apply_slice(results)

                single_col = len(self.returning_cols) == 1
                if single_col: # If model_to_hydrate is set, we returned above.
                    try:
                        return self._apply_slice([r[0] for r in rows])
                    except (IndexError, TypeError):
                        return self._apply_slice(rows)
                return self._apply_slice(rows)
        ...
        # --- BULK DELETE (SYNC) ---
            ...
            if pk_name:
                ...
                if ids:
                    ...
                    rows = tx._execute_sql_in_tx_sync(sql, ids)
    ...
    async def _execute_async(self):
        ...
        if self.op == "insert":
            ...
            if self.returning_cols:
                names = []
                # Metadata for hydration if returning a single Model
                model_to_hydrate = None
                
                # Check if we are checking for a single Model wildcard
                if len(self.returning_cols) == 1 and isinstance(self.returning_cols[0], type) and hasattr(self.returning_cols[0], "__tablename__"):
                     model_to_hydrate = self.returning_cols[0]
                     # Expand to explicit columns for safety (especially sync tuple order)
                     # Sort by key to ensure deterministic order if __columns__ is dict
                     cols_map = getattr(model_to_hydrate, "__columns__", {})
                    ...
                    for c in self.returning_cols:
                        if hasattr(c, "name"): names.append(c.name)
                             ...
                             # Mixed usage or multiple models - expand to * for now?
                             # Or better expand to columns too? Let's use * for mixed to avoid complexity in slicing
                             names.append("*")
                        else: names.append(str(c))
        ...
        # --- BULK DELETE ---
            ...
            if pk_name:
                ...
                if ids:
                    ...
                    rows = await tx._execute_sql_in_tx_async(sql, ids)
