...
class BulkOp:
    ...
    def _execute_sync(self):
        ...
        if self.op == "insert":
            ...
            if self.returning_cols and rows:
                ...
                is_wildcard = (
                    single_col 
                    and isinstance(self.returning_cols[0], type) 
                    and hasattr(self.returning_cols[0], "__tablename__")
                )
                if single_col and not is_wildcard:
                    try:
                         # Sync cursors often return tuples.
    ...
    async def _execute_async(self):
        ...
        if self.op == "insert":
                ...
                for c in self.returning_cols:
                    if hasattr(c, "name"):
                        ...
                        names.append(c.key)
                         ...
                         # It's a Model class -> Returning(User) -> RETURNING *
                         names.append("*")
                    else:
                        names.append(str(c))
            ...
            if self.returning_cols and rows:
                # If only 1 column requested AND it's NOT a wildcard (* from Model), return scalar list.
                single_col = len(self.returning_cols) == 1
                is_wildcard = (
                    single_col 
                    and isinstance(self.returning_cols[0], type) 
                ...
                if single_col and not is_wildcard:
                    try:
                        # Check if row is indexable (Record)
                        return [r[0] for r in rows]
                    except (IndexError, TypeError):
                        return rows
                return rows
