...
Define requirements for CLI help content quality, ensuring all `--help` output is optimized for AI assistant consumption.
...
**Modified:**
- `theauditor/commands/*.py` - All 38 command files (34 require AI ASSISTANT CONTEXT)

**Unchanged:**
- `theauditor/cli.py` - RichCommand class (already complete)
- `theauditor/pipeline/*.py` - Pipeline implementation
...
```bash
# Verify no deprecated "aud index" references (except in deprecation file)
grep -rn "aud index" theauditor/commands/*.py | grep -v "DEPRECATED\|deprecated\|index.py"
# Expected: empty output

# Verify AI ASSISTANT CONTEXT coverage
...
# Expected: empty output (all files have AI ASSISTANT CONTEXT)

# Count total "aud index" references to fix
grep -rn "aud index" theauditor/commands/*.py | wc -l
# Expected: 91 occurrences across 19 files (before fix)
# Expected: ~18 occurrences in index.py only (after fix - deprecation notices)
```
