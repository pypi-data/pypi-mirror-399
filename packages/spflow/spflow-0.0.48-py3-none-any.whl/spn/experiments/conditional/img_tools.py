...
def rescale(ds, original_size=(28, 28), new_size=(20, 20)):
    ...
    zoom = (new_size[0] / original_size[0], new_size[1] / original_size[1])
    resized = [scipy.ndimage.zoom(image, zoom=zoom, order=1) for image in img_data]
    return np.asarray(resized, dtype=np.float64).reshape(ds.shape[0], -1)
