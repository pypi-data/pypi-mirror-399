...
def _fit_transform_tsne(data, *, n_components=3, verbose=10, n_jobs=4, random_state=17):
    try:
        from MulticoreTSNE import MulticoreTSNE as TSNE

        return TSNE(n_components=n_components, verbose=verbose, n_jobs=n_jobs, random_state=random_state).fit_transform(data)
    except ModuleNotFoundError:
        from sklearn.manifold import TSNE

        logger.warning(
            "MulticoreTSNE is not installed; falling back to sklearn.manifold.TSNE. "
            "For improved speed, install Multicore-TSNE: https://github.com/DmitryUlyanov/Multicore-TSNE"
        )
        try:
            return TSNE(n_components=n_components, verbose=verbose, n_jobs=n_jobs, random_state=random_state).fit_transform(data)
        except TypeError:
            return TSNE(n_components=n_components, verbose=verbose, random_state=random_state).fit_transform(data)
...
if __name__ == "__main__":
    ...
    tsne_embedding_data = _fit_transform_tsne(data, n_components=3, verbose=10, n_jobs=4, random_state=17)
