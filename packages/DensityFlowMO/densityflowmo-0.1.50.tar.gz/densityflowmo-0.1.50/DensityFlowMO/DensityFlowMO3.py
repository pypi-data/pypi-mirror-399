            ...
            for epoch in range(num_epochs):
                ...
                for batch_x_rna, idx in dataloader:
                    ...
                    with torch.no_grad():
                        ...
                        rna_shifts = self.rna_perturb_model._cell_shift(rna_ns_zs, perturb=batch_u, perturb_type=batch_t).detach().clone()
                        
                        atac_ns_zs = self.atac_perturb_model._codebook_map(batch_x_atac, soft_assign=False).detach().clone()
                        atac_shifts = self.atac_perturb_model._cell_shift(atac_ns_zs, perturb=batch_u, perturb_type=batch_t).detach().clone()
