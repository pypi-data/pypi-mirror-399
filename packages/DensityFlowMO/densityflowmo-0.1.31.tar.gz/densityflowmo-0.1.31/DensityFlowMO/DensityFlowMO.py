...
class DensityFlowMO(nn.Module):
    ...
    def model(self, xs_rna, xs_atac, us=None, cs=None):
        ...
        with pyro.plate('data'):
            ###############################################################
            # RNA 
            rna_ns_logits = torch.zeros(batch_size, self.code_size, **self.options)
            rna_ns = pyro.sample('rna_n', dist.OneHotCategorical(logits=rna_ns_logits))

            zn_loc = torch.matmul(rna_ns,rna_acs_loc)
            #zn_scale = torch.matmul(ns,acs_scale)
            zn_scale = rna_acs_scale

            if self.latent_dist == 'studentt':
                prior_dof = torch.matmul(rna_ns,rna_acs_dof)
            ...
            ###############################################################
            # atac 
            atac_ns_logits = torch.zeros(batch_size, self.code_size, **self.options)
            atac_ns = pyro.sample('atac_n', dist.OneHotCategorical(logits=atac_ns_logits))

            zn_loc = torch.matmul(atac_ns,atac_acs_loc)
            #zn_scale = torch.matmul(ns,acs_scale)
            zn_scale = atac_acs_scale

            if self.latent_dist == 'studentt':
                prior_dof = torch.matmul(atac_ns,atac_acs_dof)
            ...
            rna_logits = pyro.sample('rna_logits', ZukoToPyro(self.decoder_atac2rna(rna_ns_logits)))
            ...
            atac_logits = pyro.sample('atac_logits', ZukoToPyro(self.decoder_rna2atac(atac_ns_logits)))
