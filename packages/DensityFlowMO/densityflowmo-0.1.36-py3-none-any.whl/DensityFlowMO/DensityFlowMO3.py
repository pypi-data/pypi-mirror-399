...
class DensityFlowMO3(nn.Module):
    ...
    def setup_networks(self):
        self.decoder_atac2rna = zuko.flows.NSF(features=self.latent_dim, context=self.latent_dim, transforms=self.transforms, hidden_features=self.flow_hidden_layers)
        self.decoder_rna2atac = zuko.flows.NSF(features=self.latent_dim, context=self.latent_dim, transforms=self.transforms, hidden_features=self.flow_hidden_layers)
    ...
    def cross_model(self, rna_ns_zs, atac_ns_zs):
        pyro.module('DensityFlowMO3', self)

        with pyro.plate('data'):
            ###############################################################
            # RNA 
            _ = pyro.sample('atac2rna_zs', ZukoToPyro(self.decoder_atac2rna(atac_ns_zs)), obs=rna_ns_zs)

            ###############################################################
            # atac 
            _ = pyro.sample('rna2atac_zs', ZukoToPyro(self.decoder_rna2atac(rna_ns_zs)), obs=atac_ns_zs)
                    
    def cross_guide(self, rna_ns_zs, atac_ns_zs):
        pass
    ...
    def fit(self, 
            xs_paired_dict, 
            xs_rna_single_list = None,
            xs_atac_single_list = None,
            us = None, 
            ts = None,
            cs = None,
            num_epochs: int = 100, 
        ...
        print('#### RNA ####')
        xs_rna_list = [self.preprocess(a,'rna') for a in xs_paired_dict['rna']]
        if xs_rna_single_list is not None:
            for xs_rna_single in xs_rna_single_list:
                xs_rna_list.append(self.preprocess(xs_rna_single,'rna'))
        ...
        print('#### ATAC ####')
        xs_atac_list = [self.preprocess(a,'atac') for a in xs_paired_dict['atac']]
        if xs_atac_single_list is not None:
            for xs_atac_single in xs_atac_single_list:
                xs_atac_list.append(self.preprocess(xs_atac_single,'rna'))
        ...
        with tqdm(total=num_epochs, disable=not show_progress, desc='Training', unit='epoch') as pbar:
            for epoch in range(num_epochs):
                ...
                for batch_x_rna, idx in dataloader:
                    ...
                    # cross-omics
                    loss_id = 2
                    new_loss = losses[loss_id].step(rna_ns_zs=rna_ns_zs, atac_ns_zs=atac_ns_zs)
