...
def cmd_isprime(args: argparse.Namespace) -> int:
    ...
    # If n is huge, api.isprime will automatically fall back to MR
...
def cmd_benchmark(args: argparse.Namespace) -> int:
    ...
    if args.chunk is not None:
        ...
        chunk = autotune_chunk(
            ...
            cpu_features=feats,
            prefer_large=True,
            micro_tune=(not args.no_microtune),
            persist=True,   # persist best chunk
        )
        chunk_note = " (auto+persistent)"

    print(f"Chunk: {chunk:,}{chunk_note}")
    print("Tip: set LOOKPRIME_CHUNK=32768 to override globally.\n")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    ip = sub.add_parser("isprime", help="Check primality of a number (uses sieve or MR fallback)")
    ip.add_argument("n", type=int, help="Number to test")
    ip.add_argument("--limit", type=int, default=None, help="Ensure sieve covers at least this limit (capped)")
    ...
    fac.add_argument("--limit", type=int, default=None, help="Ensure sieve covers at least this limit (capped)")
    ...
    b.add_argument("--no-microtune", action="store_true", help="Disable autotune micro-probe")
