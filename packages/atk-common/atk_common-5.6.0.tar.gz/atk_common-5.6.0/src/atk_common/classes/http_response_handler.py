...
class HttpResponseHandler(IHttpResponseHandler):
    ...
    def http_response(self, method, response):
        ...
        if is_http_status_internal(status_code):
            ...
            return self.error_handler.get_error_entity(body, method, ApiErrorType.INTERNAL, status_code)
        return self.error_handler.get_error_entity(body, method, ApiErrorType.CONNECTION, status_code)
    ...
    def client_get_amqp_request(
            ...
            item_id: Optional[str] = None,
            accept_header: Optional[str] = None):
        ctx = self._ctx_from_amqp(body, message, accept_header, None)
    ...
    def client_post_http_request(
            ...
            item_id: Optional[str] = None,
            accept_header: Optional[str] = None):
        ctx = self._ctx_from_http(request, body, accept_header=accept_header)  # body handled inside
