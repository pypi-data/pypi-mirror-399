...
class TestWizardAPIErrorHandling:
    ...
    def mock_wizards(self):
        ...
        mock_process_wizard.process = AsyncMock(return_value={
        ...
        mock_ai_wizard.analyze = AsyncMock(return_value={
            "issues": [],
                ...
                try:
                    if hasattr(wizard, "process"):
                        ...
                        issues = wizard.analyze_code(
                            code=request.input,
                            file_path=request.context.get("file_path", "demo.py") if request.context else "demo.py",
    ...
    def test_404_includes_available_wizards(self, client):
        ...
        response = client.post(
            "/api/wizard/fake_wizard/process",
            json={"input": "test"}
        )
        assert response.status_code == 404
    ...
    def test_special_characters_in_input(self, client):
        ...
        special_input = "Test with special chars: <script>alert('xss')</script> & \"quotes\" 'single'"
        response = client.post(
            "/api/wizard/test_domain/process",
