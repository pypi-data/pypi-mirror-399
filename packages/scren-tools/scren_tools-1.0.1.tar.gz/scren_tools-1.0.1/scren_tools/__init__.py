...
class AdvancedScreenCapture:
    def __init__(self, url, name="screen", fmt="png", width=3840, zoom=3, delay=4000, 
        ...
        if self.fmt not in ("png", "jpg", "jpeg"): 
            raise ValueError("فرمت باید png/jpg/jpeg باشد")
    ...
    def _enhanced_capture(self):
        ...
        for attempt in range(max_retries):
            try:
                response = requests.get(api_url, headers=headers, timeout=90, stream=True)
                ...
                if response.status_code == 200:
                    ...
                    if len(image_data) < 15000:
                        if attempt < max_retries - 1:
                            time.sleep(2)
                            continue
                        raise RuntimeError("کیفیت تصویر بسیار پایین است")
                ...
                if attempt == max_retries - 1:
                    raise RuntimeError(f"خطا در گرفتن اسکرین‌شات: {str(e)}")
                time.sleep(2)
        
        raise RuntimeError("اسکرین‌شات ناموفق بعد از تلاش‌های متعدد")

    def _add_text_below(self, image):
        original_width, original_height = image.size
        
        new_height = original_height + 50
        new_image = Image.new("RGB" if self.fmt in ("jpg", "jpeg") else "RGBA", 
                             (original_width, new_height), (255, 255, 255))
        ...
        try:
            ...
            text_position = ((original_width - text_width) // 2, original_height + 15)
...
class MultiScreenCapture:
    def __init__(self, url_list, name_list=None, fmt="png", width=3840, zoom=3, 
                 delay=4000, device="desktop", quality=100, max_workers=5):
        self.urls = url_list
    ...
    def _parallel_capture(self):
        with concurrent.futures.ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            ...
            for url, name in zip(self.urls, self.names):
                future = executor.submit(self._capture_single, url, name)
            ...
            for future in concurrent.futures.as_completed(futures):
                try:
                    ...
                    print(f"خطا در پردازش یک URL: {str(e)}")
