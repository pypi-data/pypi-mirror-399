...
### 最近版本
- **v0.1.1** (2025-12-29)：
- - 将函数`conin`面对`keyboard`事件的未知按键时的返回值由`"unknown"`更改为`""`。
- **v0.1.0** (2025-12-29)：
- - 新增了函数`is_admin`和`reboot_to_admin`。
- - 将函数`coninit`重构更改为`conset`。支持可选关键字参数`title`。
- - 重构函数`conin`并更改了其返回值。且提升了其安全性。
- **v0.0.2** (2025-12-02)：修复了函数`conin`返回字典中控制键的映射错误。
